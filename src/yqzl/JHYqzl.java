...
public class JHYqzl {
	...
	private void addReceive(String one, HttpServletRequest req) {
		...
		saveValues.put("subzlAccount",getValue(one,"ACCNO1"));
		saveValues.put("subAccount",getValue(one,"ACCNO1"));
				...
				session.doWork(new Work() {
					...
					public void execute(Connection conn) {
						try {
							...
							String sql = " select subzlAccount as zlaccount from tblSubAccount where Account = '100201' and LEN(subzlAccount) >20 ";
							PreparedStatement pss = conn.prepareStatement(sql);
							ResultSet rss = pss.executeQuery();
							String zlaccount = "";
							while (rss.next()) {
				...
				session.doWork(new Work() {
					...
					public void execute(Connection conn) {
						try {
							...
							String sql = "select a.classCode" + " from tblCompany a join tblSubAccount b on a.id =b.f_ref " + "  where b.subzlAccount=?";
							PreparedStatement pss = conn.prepareStatement(sql);
