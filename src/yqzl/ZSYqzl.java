	...
	public String deal(HttpServletRequest request)
    {
        ...
        if("now".equals(op))
        {
           /*
            ...
            Logger.getLogger("YQZL").info("接收到当天报文");
            Logger.getLogger("YQZL").info("*********************");
        	*/
        	BaseEnv.log.info("*********************");
        	BaseEnv.log.info("接收到当天报文");
        	BaseEnv.log.info("*********************");
        } else
        if("history".equals(op))
        {
            /*
        	Logger.getLogger("YQZL").info("*********************");
            Logger.getLogger("YQZL").info("接收到历史报文");
            Logger.getLogger("YQZL").info("*********************");
            */
            BaseEnv.log.info("*********************");
        	BaseEnv.log.info("接收到历史报文");
        	BaseEnv.log.info("*********************");
        }
        Jzb jzb = new Jzb();
        if(data == null || "".equals(data))
        {
            //Logger.getLogger("YQZL").info("报文为空");
        	BaseEnv.log.info("报文为空");
        	return "data empty";
        }
        try
        {
        ...
        {
            //Logger.getLogger("YQZL").error("报文解密失败", e);
        	BaseEnv.log.error("报文解密失败", e);
        	return "data decrypt failed";
        }
       /*
        Logger.getLogger("YQZL").info(">>>>>>>>>>>>>>>>>>>>>");
        Logger.getLogger("YQZL").info((new StringBuilder("接收到解密后报文")).append(data).toString());
        Logger.getLogger("YQZL").info(">>>>>>>>>>>>>>>>>>>>>");
       */
        BaseEnv.log.info(">>>>>>>>>>>>>>>>>>>>>");
        BaseEnv.log.info((new StringBuilder("接收到解密后报文")).append(data).toString());
        BaseEnv.log.info(">>>>>>>>>>>>>>>>>>>>>");
