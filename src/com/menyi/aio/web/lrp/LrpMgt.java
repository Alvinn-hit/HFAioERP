                ...
                session.doWork(new Work() {
                    public void execute(Connection connection) throws
                        ...
                        try {
                        	...
                        	if(fieldName.equals("WOClassType") && tableName.equals("PDLrpProdPlan") && value.equals("1")){
                        ...
                        session.doWork(new Work() {
                            public void execute(Connection connection) throws
                                ...
                                try {
                                    ...
                                    pstmt.execute();
                                	compute(computeNo,computeId,lrpId,isloss,isLowQty,isReplace,"",map.get("startDate"),map.get("endDate"),userId,billType, map.get("billId"), map.get("billNo"), map.get("sourceId"), map.get("GoodsCode"),Double.parseDouble(map.get("qty")),"",1, connection);
                                	//更新单据信息
                                	if(billType.equals("tblSalesOrder")){
                                		sql = " update tblSalesOrderDet set MRPQty=? where id=? ";
                            		}else if(billType.equals("PDPlan")){
...
//		缺货数量=毛需求量-(库存量+计划生产量+计划采购量+预计生产量+采购在途量-计划领料量-预计领料量-预计销货量)
	...
	public void compute(final String computeNo,final String computeId,String lrpId,final String isloss,final String isLowQty,final String isReplace,final String parentPlanId,final String startDate,
        ...
        String sql = "select tblGoods.attrType,PDBom.id bomId,tblGoods.sendRound,isnull(tblGoods.lowerQty,0) lowerQty,isnull(tblGoods.reTimes,0) reTimes,PDBom.workflowNodeName,PDBom.BillNo "
        		+ "from tblGoods left join PDBom on tblGoods.classCode=PDBom.GoodsCode and PDBom.isLast=1 "
        ...
        if(rset.next()){
        	...
        	if(attrType.equals("0")||attrType.equals("2")){
        		if(newBomId==null){
        			throw new Exception("GoodsCode:"+GoodsCode+"不存在BOM！");
        		}
	        	if(!rset.getString("workflowNodeName").equals("finish")){
	        		throw new Exception("BOM单:"+rset.getString("BillNo")+"处在反审核状态！");
	        	}
        	}
        }else{
...
//        }
        ...
        if(newBomId !=null && attrType.equals("0")||attrType.equals("2")){
        	bomId = newBomId;
        	//自制或委外
            ...
            pstmt.setString(9, attrType.equals("0")?"1":"2");
            pstmt.setString(10, "");
            pstmt.setString(11, "");
        ...
        }else if(newBomId !=null && attrType.equals("3")){
        	//虚拟件， 不产生工令，采用上级bom的工令
        	bomId = newBomId;
        }else {
        	//采购件
...
//            }
        ...
        if(newBomId !=null && attrType.equals("0")||attrType.equals("2")||attrType.equals("3")){//有下级bom时递归到下级,自制，委外，虚拟
	        sql = " select PDBomDet.GoodsCode,PDBomDet.qty,PDBomDet.lossRate from "
	        		+ " PDBomDet where  PDBomDet.f_ref=? ";
	        pstmt = conn.prepareStatement(sql);
	        pstmt.setString(1, newBomId);
	        rset = pstmt.executeQuery();
	        while(rset.next()){
	        	BigDecimal q = rset.getBigDecimal("qty");
	        	q = q.multiply(LossProdQty);//缺料生产数*bom数量
	        	Double lr = rset.getDouble("lossRate");
	        	...
	        	compute(computeNo, computeId, lrpId, isloss, isLowQty, isReplace, planId, startDate, endDate, 
	        			userId, billType, billId, billNo, sourceId, rset.getString("GoodsCode"), q.doubleValue(), newBomId, bomGrade+1, conn);
	        }
        }
	}
	...
	public Result selCompute(String startDate,String endDate,String computeNo,String billType,String BillNo,String GoodsNumber,String GoodsFullName){
		ArrayList param = new ArrayList();
		String sql = " select a.id,BillDate,BillNo,b.EmpFullName,row_number() over ( order by BillDate desc,billNo desc) row_id from PDLRPCompute a join tblEmployee b on a.createBy=b.id where 1=1 ";
		...
		if((billType != null && billType.length() > 0) || (BillNo != null && BillNo.length() > 0) || 
				(GoodsNumber != null && GoodsNumber.length() > 0) || (GoodsFullName != null && GoodsFullName.length() > 0)){
			sql += " and a.id in (select ComputerId from PDLrp join tblGoods on PDLrp.GoodsCode=tblGoods.classCode where 1=1  ";
