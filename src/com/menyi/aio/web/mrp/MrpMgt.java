...
//		}
	...
	public Result selGoods(HashMap<String,HashMap> goodsMaps){
		for(String GoodsCode:goodsMaps.keySet()){
			...
			sql = " select "+goodF+" from tblGoods where classCode =? ";
			param = new ArrayList();
			param.add(GoodsCode);
...
//			//将所有计算结果，保存在PDMRPStocks表中，通过这个表，能观察MRP运算的历史记录
//			Result rs = mrpStocks(bomId, mrpId, parentCode);
//			if(rs.retCode != ErrorCanst.DEFAULT_SUCCESS){
//				return rs;
//			}
//			//查库存量 =当前主仓库中所有最后结存+车间，委外仓中所有欠料
//			sql = " select GoodsCode,SUM(Qty) lastQty from PDMRPStocks a where f_ref=? and parentCode=? "
//					+ "and type in ('curStocks','oweStocks') group by GoodsCode ";
//			BaseEnv.log.debug("根据Bom查询对应商品的当前可用库存（主仓结存+车间/委外仓欠料）"+sql);
//			param = new ArrayList();
//			param.add(mrpId);
...
//			
//			
//			//查在途量 =当前未结案请购单（请购数量-入库数量）
//			sql = " select GoodsCode,SUM(Qty) lastQty from PDMRPStocks a where f_ref=? and parentCode=? and "
//					+ "type in ('buyApplication') group by GoodsCode ";
//			BaseEnv.log.debug("查在途量 =当前未结案请购单（未入库数量）"+sql);
//			param = new ArrayList();
//			param.add(mrpId);
//			param.add(parentCode);
//			Result oRs = sqlListMap(sql,param);
//			if(oRs.retCode != ErrorCanst.DEFAULT_SUCCESS){
//				return oRs;
//			}
//			//查在制量 =当前未结案工令单（未缴库数量）
//			sql = " select GoodsCode,SUM(Qty) lastQty from PDMRPStocks a where f_ref=? and parentCode=? and type in ('produce') "
//					+ "group by GoodsCode ";
//			BaseEnv.log.debug("查在制量 =当前未结案工令单（生产数量-缴库数量）"+sql);
//			param = new ArrayList();
//			param.add(mrpId);
//			param.add(parentCode);
//			Result pRs = sqlListMap(sql,param);
//			if(pRs.retCode != ErrorCanst.DEFAULT_SUCCESS){
//				return pRs;
//			}
//			/**
//			 * 占用量
//			 */
//			sql = " select GoodsCode,SUM(Qty) lastQty from PDMRPStocks a where f_ref=? and parentCode=? "
//					+ "and type in ('process') group by GoodsCode ";
//			BaseEnv.log.debug("查占用量 =（销售订单/制造需求单占用量+已请购量-已发料量）"+sql);
//			param = new ArrayList();
//			param.add(mrpId);
//			param.add(parentCode);
//			Result possesRs = sqlListMap(sql,param);
//			if(possesRs.retCode != ErrorCanst.DEFAULT_SUCCESS){
//				return possesRs;
//			}
//			
//			ArrayList<HashMap> stockQty = (ArrayList<HashMap>)tRs.retVal;
//			ArrayList<HashMap> OrderQty = (ArrayList<HashMap>)oRs.retVal;
//			ArrayList<HashMap> ProduceQty = (ArrayList<HashMap>)pRs.retVal;
//			ArrayList<HashMap> PossesQty = (ArrayList<HashMap>)possesRs.retVal;
//			
//			
//			for(HashMap map : (ArrayList<HashMap>)bomRs.retVal){
//				map.put("StockQty", 0); //库存量
//				map.put("OrderQty", 0); //在途量
//				map.put("ProduceQty", 0); //在制量
//				map.put("PossesQty", 0); //占用量
//				
//				for(HashMap st: stockQty){
//					if(st.get("GoodsCode").equals(map.get("classCode"))){
//						map.put("StockQty", st.get("lastQty")); //库存量
//						break;
//					}
//				}
//				for(HashMap st: OrderQty){
//					if(st.get("GoodsCode").equals(map.get("classCode"))){
//						map.put("OrderQty", st.get("lastQty")); //在途量
//						break;
//					}
//				}
//				for(HashMap st: ProduceQty){
//					if(st.get("GoodsCode").equals(map.get("classCode"))){
//						map.put("ProduceQty", st.get("lastQty")); //在制量
//						break;
//					}
//				}
//				for(HashMap st: PossesQty){
//					if(st.get("GoodsCode").equals(map.get("classCode"))){
//						map.put("PossesQty", st.get("lastQty")); //占用量
//						break;
//					}
//				}
//			}
	...
	public Result showBom(String keyId){
		...
		sql = " select bomId,needQty,PDMRP.id mrpId,PDBom.version,startWork,endWork,StockQty,OrderQty,ProduceQty,PossesQty, StockQty+OrderQty+ProduceQty-PossesQty  totalQty  ,tblGoods.* from PDMRP join tblGoods on PDMRP.GoodsCode=tblGoods.classCode "
		...
		sql = " select PDMRPBom.*,tblGoods.GoodsNumber,GoodsFullName,GoodsSpec,BaseUnit from  PDMRPBom join tblGoods on PDMRPBom.GoodsCode=tblGoods.classCode "
				+ " where f_ref =? order by PDMRPBom.id ";
		param = new ArrayList();
		param.add(keyId);
		Result bomRs = sqlListMap(sql,param);
		...
		Result rs = new Result();
		rs.retVal = new Object[]{salesRs.retVal,goodsRs.retVal,bomRs.retVal,qty.doubleValue()};
		return rs;
	}
...
//	}
	...
	public Result saveBom(final HashMap values,final LoginBean lb,Connection connection) throws Exception{
        ...
        String sql = "INSERT INTO [PDMRP]([id],[isCatalog],[BillDate],[DepartmentCode],[EmployeeID],[Remark],"
        		...
        		+ "StockQty,OrderQty,ProduceQty,PossesQty,CompanyCode,BillNo,MRPType,startWork,endWork)";
        ...
        pstmt.setString(14, values.get("StockQty")+"");
        pstmt.setString(15, values.get("OrderQty")+"");
        pstmt.setString(16, values.get("ProduceQty")+"");
        pstmt.setString(17, values.get("PossesQty")+"");
        //这里记录第一个单 据明细的往来代号到PDMRP表中，因为如果启用客供料，只允许同一客户的订单合并在一起规划，
        //相当于这个字段就是记录了这次规划的客户代号，有利于后面的算客供料和工令发料时选择客供料
        pstmt.setString(18, ((List<HashMap>)values.get("salesList")).get(0).get("CompanyCode")+"");
        pstmt.setString(19, bNo);
        pstmt.setString(20, values.get("MRPType")+"");
        pstmt.setString(21, values.get("startWork")+"");
        pstmt.setString(22, values.get("endWork")+"");
        ...
        if(Double.parseDouble(values.get("MRPType")+"")==1){
        	//直接出库
        }else if(Double.parseDouble(values.get("MRPType")+"")==2){
        	//直接采购
        	String buyId = IDGenerater.getId();
        	BillNo buybillno = BillNoManager.find("tblBuyApplication_BillNo");
        	String bBillNo="";
    		if (buybillno != null) {
    				//启用单据编号连续后，从数据库读编号,或者单据编号为空
    				String valStr = BillNoManager.find("tblBuyApplication_BillNo", values, lb);
    				if ("".equals(valStr)) {
    					/* 单据编号无法生成 */
    					rst.retCode=ErrorCanst.DEFAULT_FAILURE;
                		rst.retVal="请购单单据编号无法生成";
                		return rst;
    				} else {
    					bBillNo = valStr;
    				}
    		}
        	sql = "INSERT INTO [tblBuyApplication]([id],[classCode],[workFlowNode],[workFlowNodeName],[checkPersons],[BillDate],[BillNo],"
        			+ "[DepartmentCode],[EmployeeID],[Period],[PeriodMonth],[PeriodYear],[createBy],[lastUpdateBy],[createTime],[lastUpdateTime],"
        			+ "[statusId],[SCompanyID],[finishTime],[CheckPersont],[SalesOrderID],[PDMRPId],[PDWorkOrderID],[ArriveDate],[Status],"
        			+ "[isMRP],[MRPSalesOrderId],[GoodsCode]) "
        			+ " values(?,'','0','draft','',CONVERT(varchar(10),getdate(),120),?,"
        			+ "?,?,MONTH(GETDATE()),MONTH(GETDATE()),YEAR(GETDATE()),?,?,CONVERT(varchar(19),getdate(),120),CONVERT(varchar(19),getdate(),120),"
        			+ "0,'00001',CONVERT(varchar(19),getdate(),120),'','',?,'','','','yes','','')";
        	pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, buyId);
            pstmt.setString(2, bBillNo);
            pstmt.setString(3, lb.getDepartCode());
            pstmt.setString(4, lb.getId());
            pstmt.setString(5, lb.getId());
            pstmt.setString(6, lb.getId());	
            pstmt.setString(7, id);
            pstmt.execute();
            sql = "INSERT INTO [tblBuyApplicationDet]([id],[f_ref],[GoodsCode],[BatchNo],[color],[Hue],[Inch],[yearNO],[Qty],[Price],[Amount],[UnitQty],"
            		+ "[UnitPrice],[UnitBaseQty],[OrderQty],[NoOrderQty],[FinishStatus],[SCompanyID],[EmployeeID],[OutStockQty],[NotOutStockQty],[ArriveDate],"
            		+ "[OrderDate],[ProDate],[Availably],[SecQty],[BaseQty],[UnitIntQty],[SalesOrderID],[SourceID],[lackQty],[totalLoss],[hasQty],"
            		+ "[PDWorkOrderID],[InQty],[NotInQty]) "
            		+ "values(SUBSTRING(convert(varchar(40),newid()),1,30),?,?,'','','','','',?,0,0,0,0,0,0,?,0,'00001','',0,0,'','','','',1,1,0,'','',?,0,0,'',0,?)";
        	pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, buyId);
            pstmt.setString(2, values.get("GoodsCode")+"");
            pstmt.setString(3, values.get("needQty")+"");
            pstmt.setString(4, values.get("needQty")+"");
            pstmt.setString(5, values.get("needQty")+"");	
            pstmt.setString(6, values.get("needQty")+"");
            pstmt.execute();
            values.put("BuyAppId", buyId);
        }else{
            for(HashMap map:(List<HashMap>)values.get("bomList")){
            	sql = "INSERT INTO [PDMRPBom]([f_ref],[GoodsCode],baseQty,[BomDetId],[isOpen],[BomId],[childBomId],[qty],[lossRate],"
            			+ "[totalQty],[totalLoss],[attrType],[dtype],detCode,baseLossRate,clientSupper,ProduceQty)";
        		sql += " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                BaseEnv.log.debug(sql);
                pstmt = conn.prepareStatement(sql);
                pstmt.setString(1, id);
                ...
                pstmt.setString(17, map.get("ProduceQty")+"");
                pstmt.execute();
            ...
            if(map.get("BillType").equals("tblSalesOrder")){
                ...
                while(rset.next()){
                	...
                	if(qty<=lQty){
                		...
                		pstmt1.execute();
                		if(Double.parseDouble(values.get("MRPType")+"")==1||Double.parseDouble(values.get("MRPType")+"")==2){
                			//如果是直接出库或直接购买
                			sql = " update tblSalesOrderDet set PossesQty=isnull(PossesQty,0)+ ? where id=? ";
                    		pstmt1 = conn.prepareStatement(sql);
                    		pstmt1.setDouble(1, qty);
                    		pstmt1.setString(2, rset.getString("id"));
                    		pstmt1.execute();
                		}
...
//                                    if(qty>0){
//                                    	//如果全部分配完后，数量还是大于0，说明规划数量超过订单数量，每配到第一行
//                                    	sql = " update tblSalesOrderDet set MRPQty=MRPQty+? where id=? ";
//                                		PreparedStatement pstmt1 = conn.prepareStatement(sql);
//                                		pstmt1.setDouble(1, qty);
//                                		pstmt1.setString(2, firstId);
//                                		pstmt1.execute();
//                                		//记录分配过程，方便，删除时扣减数量
//                                		sql = " insert into  PDMRPAllot(f_ref,BillId,BillDetId,BillType,Qty) values(?,?,?,?,?) ";
//                                		pstmt1 = conn.prepareStatement(sql);
...
//                                    if(qty>0){
//                                    	//如果全部分配完后，数量还是大于0，说明规划数量超过订单数量，每配到第一行
//                                    	sql = " update PDProduceRequireDet set MRPQty=MRPQty+? where id=? ";
//                                		PreparedStatement pstmt1 = conn.prepareStatement(sql);
//                                		pstmt1.setDouble(1, qty);
//                                		pstmt1.setString(2, firstId);
//                                		pstmt1.execute();
