...
//    									if("112200003".equals(map.get("SubCode"))){
//    	                            		String cc = map.get("classCode").toString();                					
//    	                					
//    	                            		map.put(s, "111");
//    									} else   
    									map.put(s, moneys);
							...
							Object obj = new Object[]{accTypeInfoList,accBalanceMap,noBindMap,noBindInitMap};
							//BaseEnv.log.debug("data="+new Gson().toJson(accBalanceMap)) ;
							result.setRetVal(obj);
				...
				for(String s : str1){
					...
					if(o!=null && !"".equals(o)){
						...
						if(Integer.parseInt(accmap.get("jdFlag").toString()) == 1){
							accmap.put("PeriodIniDebit"+s, dealDataDouble(String.valueOf(o), BaseEnv.systemSet.get("DigitsAmount").getSetting(), ""));
							//判断金额是否为负值
							if(accmap.get("PeriodIniDebit"+s).toString().contains("-")){
								accmap.put("PeriodIniCredit"+s,accmap.get("PeriodIniDebit"+s).toString().replaceAll("-", ""));
								accmap.put("PeriodIniDebit"+s,"");								
							}
							...
							o = (Object)(0-Double.parseDouble(o.toString()));
							//判断金额是否为负值
							if(accmap.get("PeriodIniCredit"+s).toString().contains("-")){
								accmap.put("PeriodIniDebit"+s,accmap.get("PeriodIniCredit"+s).toString().replaceAll("-", ""));
								accmap.put("PeriodIniCredit"+s,"");							
							}
						}
					}
					accmap.put("PeriodIni"+s, o);
					...
					if(Integer.parseInt(accmap.get("jdFlag").toString()) == 1){
						...
						//***判断期末借方金额是否为负值
						if(accmap.get("PeriodDebitBala"+s).toString().contains("-")){
							accmap.put("PeriodCreditBala"+s,accmap.get("PeriodDebitBala"+s).toString().replaceAll("-", ""));
							accmap.put("PeriodDebitBala"+s,"");								
						}
					}else if(Integer.parseInt(accmap.get("jdFlag").toString()) == 2){
						money = new BigDecimal("0").subtract(new BigDecimal(money)).toString();
						accmap.put("PeriodCreditBala"+s, dealDataDouble(money, BaseEnv.systemSet.get("DigitsAmount").getSetting(), ""));
						//***判断期末借方金额是否为负值
						if(accmap.get("PeriodCreditBala"+s).toString().contains("-")){
							accmap.put("PeriodDebitBala"+s,accmap.get("PeriodCreditBala"+s).toString().replaceAll("-", ""));
							accmap.put("PeriodCreditBala"+s,"");								
						}
					}
