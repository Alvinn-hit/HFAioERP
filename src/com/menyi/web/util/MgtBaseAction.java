...
public abstract class MgtBaseAction extends BaseAction {
    ...
    protected ActionForward doAuth(HttpServletRequest req,ActionMapping mapping) {
        ...
        if(parameter.startsWith("/CashierAction.do")){        	
        	 MOperation m1 = (MOperation) getLoginBean(req).getOperationMap().get(parameter +"?operation="+OperationConst.OP_CASHIER_BANK);
        	 MOperation m2 = (MOperation) getLoginBean(req).getOperationMap().get(parameter +"?operation="+OperationConst.OP_CASHIER_CASH);
        	 if(m1!=null || m2 !=null) return null;
        }
        
        MOperation mo = (MOperation) getLoginBean(req).getOperationMap().get(parameter);
        //BaseEnv.log.debug(getLoginBean(req).getOperationMap());
