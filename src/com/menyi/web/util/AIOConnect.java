...
//							return detailBean;
	...
	public static Message add(String locale,LoginBean loginBean,String tableName,String parentCode,String defineInfo,String saveType,String valuesJson,String deliverTo) {
		...
		//获取当前单据用户loginBean
		 ...
		 Result result = userMgt.login((String)values.get("createBy"));
		 List userList = (ArrayList) result.retVal;
		 EmployeeBean userBean = (EmployeeBean) userList.get(0);
		 LoginBean lgBean = new LoginBean();
		 lgBean.setId(userBean.getId());
		 lgBean.setMobile(userBean.getMobile());
		 lgBean.setDefSys(userBean.getDefSys());
		 lgBean.setLoginTime(BaseDateFormat.format(new Date(), BaseDateFormat.yyyyMMddHHmmss));         
		 lgBean.setEmpFullName(userBean.getEmpFullName());
		 lgBean.setLbxMonitorCh(userBean.getLbxMonitorCh());//来电通道设定
		 lgBean.setTelPrefix(userBean.getTelPrefix());//电话前缀
		 lgBean.setTelAreaCode(userBean.getTelAreaCode());//用户区位
		 lgBean.setTelpop(userBean.getTelpop());
		 lgBean.setDepartCode(userBean.getDepartmentCode()) ;
		 lgBean.setShopName(userBean.getShopName()) ;
		 lgBean.setShopPwd(userBean.getShopPwd()) ;
		 lgBean.setDefaultPage(userBean.getDefaultPage()) ;
		 lgBean.setExtension(userBean.getExtension()) ;
		 lgBean.setViewLeftMenu(userBean.getViewLeftMenu()) ;
		 lgBean.setViewTopMenu(userBean.getViewTopMenu()) ;                 
		 lgBean.setModuleId(userBean.getModuleId());
		 lgBean.setTitleId(userBean.getTitleID());
		 lgBean.setDefDesk(userBean.getDefDesk());
		 lgBean.setEmail(userBean.getEmail());
		 lgBean.setQq(userBean.getQq());
		 lgBean.setShowDesk(userBean.getShowDesk());//显示桌面
		 lgBean.setFirstShow(userBean.getFirstShow());
		 lgBean.setShowWebNote(userBean.getShowWebNote());		 	
		// lg = lgBean;
...
//		BaseEnv.log.debug("AIOConnect发送远程请求，url="+url+",time="+System.currentTimeMillis()+",data=："+json);
	...
	public static Message deliverToNext(String locale,LoginBean loginBean,String keyId,String tableName) {
		...
		//BaseEnv.log.debug("测试----" +checkPersion );
