...
public interface OperationConst
{
    ...
    int OP_CASHIER_AUDIT = 99;//审核
    
    int OP_CASHIER_REAUDIT = 100;//反审核
    
    int OP_CASHIER_IMPORT_PRE = 101;//Excel导入数据
    
    int OP_CASHIER_IMPORT_DATA = 102;//保存Excel数据
    
    int OP_CASHIER_INIT_PRE = 103;//出纳模块初始化
    
    int OP_CASHIER_INIT = 104;//出纳初始化参数保存
    
    int OP_CASHIER_CASH = 105;//出纳-现金日记账
    
    int OP_CASHIER_CASH_QUERY = 106;//出纳-现金日记账查询
    
    int OP_CASHIER_CASH_ADD = 107;//出纳-现金日记账添加
    
    int OP_CASHIER_CASH_QUOTE = 108;//出纳-现金日记账引入
    
    int OP_CASHIER_CASH_GENBYSINGLE = 109;//出纳-现金日记账生成凭证（按单）
    
    int OP_CASHIER_CASH_GENBYSUMMARY = 110;//出纳-现金日记账生成凭证（汇总） 
    
    int OP_CASHIER_CASH_EDIT = 111;//出纳-现金日记账修改
    
    int OP_CASHIER_CASH_UPDATE = 112;//出纳-现金日记账修改保存

    int OP_CASHIER_CASH_DELETE = 113;//出纳-现金日记账删除
    
    int OP_CASHIER_CASH_EXPORT = 114;//出纳-现金日记账导出
    
    int OP_CASHIER_CASHACCOUNT_PRE = 115;//出纳-现金对账单          
    
    int OP_CASHIER_CASHACCOUNT = 116;//出纳-现金对账单    
    
    int OP_CASHIER_BANKACCOUNT_PRE = 119;//出纳-银行存款对账单
    
    int OP_CASHIER_BANKACCOUNT = 120;//出纳-银行存款对账单查询
    
    int OP_CASHIER_BANK = 121;//出纳-银行存款日记账
    
    int OP_CASHIER_BANK_QUERY = 122;//出纳-银行存款日记账查询
    
    int OP_CASHIER_BANK_ADD = 123;//出纳-银行存款日记账添加
    
    int OP_CASHIER_BANK_QUOTE = 124;//出纳-现金日记账引入
    
    int OP_CASHIER_BANK_GENBYSINGLE = 125;//出纳-银行日记账生成凭证（按单）
    
    int OP_CASHIER_BANK_GENBYSUMMARY = 126;//出纳-银行日记账生成凭证（汇总）
    
    int OP_CASHIER_BANK_EXPORT = 131;//出纳-银行存款导出excel
    
    int OP_CASHIER_BANK_DELETE = 129 ; //出纳-银行日记账删除
    
    int OP_CASHIER_BANK_EDIT = 127;//出纳-银行日记账修改
    
    int OP_CASHIER_BANK_UPDATE = 128;//出纳-银行日记账修改保存    
    
    int OP_CASHIER_DOWNLOAD = 132;//下載附件
    
    int OP_CASHIER_BALANCE_PRE = 133;//日记账月结
    
    int OP_CASHIER_BALANCE = 134;//月结提交
    
    int OP_CASHIER_REBALANCE_PRE = 135;//日记账反月结
    
    int OP_CASHIER_REBALANCE = 136;//反月结提交
    
    int OP_CASHIERSET_UPPRE= 137;//出纳权限设置
    int OP_CASHIERSET_SAVE= 138;//出纳权限保存
    
    int OP_CASHIER_CASH_POST = 139;//出纳过账
    int OP_CASHIER_CASH_REPOST = 140;//出纳反过账
