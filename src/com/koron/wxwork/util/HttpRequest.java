...
public class HttpRequest {
	 ...
	 * @return
	 */
	public static BaseResultBean getFile(String url,String filePath) {
		HttpClient client = new HttpClient();
		Protocol myhttps = new Protocol("https", new MySSLProtocolSocketFactory(), 443);
		Protocol.registerProtocol("https", myhttps);
		GetMethod get = new GetMethod(url);
		try {
			...
			if(respStr.startsWith("{\"errcode\"")){
				BaseResultBean bean = new Gson().fromJson(respStr, BaseResultBean.class);			
				return bean;			
			}
			...
			return BaseResultBean.generate("1", e.getMessage());
		} catch (IOException e) {
			return BaseResultBean.generate("2", e.getMessage());
		}
		get.releaseConnection();
		return BaseResultBean.generate("0", "success");
	}
}
