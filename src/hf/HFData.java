...
public class HFData extends AIODBManager{
	...
	private static HFData instance = null;  
    private HFData(){}  
    public static synchronized HFData newInstance(){  
	...
	public synchronized String exec(HttpServletRequest request){
...
//	}
		...
		if(rs.retCode== ErrorCanst.DEFAULT_SUCCESS){
			...
			if(list.size() ==0){
				...
				sql = " select PDWorkOrder.id,PDWorkOrder.BillNo,PDWorkOrder.BillDate,PDWorkOrder.WOClassType,"
						...
						//+ " and PDWorkOrder.statusId=0 and datepart(yy,PDWorkOrder.BillDate) = year(getDate()) and datepart(mm,PDWorkOrder.BillDate) = month(getDate()) order by PDWorkOrder.BillDate ";
						+ " and PDWorkOrder.statusId=0 and datepart(yy,PDWorkOrder.BillDate) = year(getDate())  order by PDWorkOrder.BillDate ";
				param.clear();
	 ...
	 * @throws Exception
	 */  
	public void importSeq(final Connection conn,final String id,String WorkOrder,final String userId,HashMap rMap,final String BillDate) throws Exception{
		String sql = "select * from HFimport where id=?";
		...
		String qsql = " select top 1 1 a from tblStockDet where seq=? and instoreQty>0 ";
		...
		for(int i=0;i<ds.length;i++){
			...
			if(one.trim().length() > 0&&one.split(",").length>3){
				...
				sql = " select classCode from tblGoods where GoodsNumber=? ";
				pst = conn.prepareStatement(sql);
				...
				if(!hasExist){
                    try {
                        ...
                        cs.registerOutParameter(9, Types.VARCHAR, 50);
                        cs.execute();
                        int retCode = cs.getInt(8);
                        if(retCode!=0){
                        	throw new Exception(cs.getString(9));
                        }
		...
		//记录状态信息
		sql = " update HFImport set SuccessSCode=? ,OldNum=?,SuccessNum=? where id=? ";
		qpst = conn.prepareStatement(sql);
	 ...
	 * @throws Exception
	 */  
	public void importSeqDel(final Connection conn,final String id,final String userId,HashMap rMap,final String BillDate) throws Exception{
		CallableStatement cs=null;
//		if(true){
//			return;
//		}
		...
		String data=null;
		if(rset.next()){
			data = rset.getString("SuccessSCode");
		}else{
			throw new Exception("数据不存在");
		}
		String[] ds = data.split("\\n");
