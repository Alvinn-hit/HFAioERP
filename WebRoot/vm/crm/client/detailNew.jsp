...
<input type="hidden" id="erpCompanyCode" name="erpCompanyCode" value='$!erpCompanyCode'>
...
<input type="hidden" name="keyInfo" id="keyInfo" value='ClientId:$!result.get("id")'>
<input type="hidden" id="openSelectName" name="openSelectName" value=""/>
	...
	<div class="inputbox">
		...
		<!-- <div class="btn btn-small rf" onclick="addTaskAssign();">任务分派</div> -->
			...
			<div class="btn btn-small rf" onclick="checkDialog('/OAMyWorkFlow.do?keyId=$!keyId&tableName=CRMClientInfo&operation=82')">审核</div>	
			#end
		...
		#if("$!erpCompanyCode" == "")
			<div class="btn btn-small rf" id="transferToErpBtn" onclick="transferToErp()">转入ERP</div>
		#end
		#end
		
		#if($loginBean.operationMap.get("/CRMClientTransferAction.do").query() && "$loginBean.id" == "$!result.get('createBy')")
		...
		#if("$updateOp" == "true")
			#if($loginBean.id=="1" || $loginBean.id=="$result.get('createBy')")
				<div class="btn btn-small rf" onclick="shareClient('$result.get("id")','$!result.get('ModuleId')','$!viewId','detail','dealdiv')">共享</div>
			#end
        	#if(("$!OAWorkFlow" == "") || ("$!OAWorkFlow" == "true" && $result.get("workFlowNode")=="0") || ($result.get("checkPersons").indexOf(";${loginBean.id};")>-1 && $result.get("workFlowNode")!="-1"))
				#set($isEdit = "true")
			...
			<div class="f_dv">
				...
				<ul class="bk_ul">
					#foreach ($row in $mainMap.get($maps))
			        	...
			        	#set($readStr = "true")
			        	#if($!readMainFieldsStr.indexOf(",${row},")==-1 && "$isEdit" == "true")
							#set($readStr = "")
			        	#end
						...
						#set($resultVal = $!result.get($fieldBean.fieldName))
					        	...
					        	<li class="bk_li">
					        		...
					        		<div class="ip_txt" name="$fieldBean.fieldName" id="$fieldBean.fieldName" readonly="readonly" title="$!globals.dealDoubleDigits("$!result.get($fieldBean.fieldName)","amount")">$!globals.dealDoubleDigits("$!resultVal","amount")</div>
					        		#if($!readStr == "")<i class="update_i" title='修改' fieldName='$fieldBean.fieldName' inputType='$fieldBean.inputType' fieldType='$fieldBean.fieldType' fieldValue='$!globals.dealDoubleDigits("$!resultVal","amount")'></i> #end
					        	</li>
							#elseif("$fieldBean.fieldType" == "5")
								<li class="bk_li">
					        		<div class="swa_c1 " title='$fieldBean.display.get("$globals.getLocale()")'>$fieldBean.display.get("$globals.getLocale()")：</div>
					        		<div class="ip_txt" name="$fieldBean.fieldName" id="$fieldBean.fieldName" readonly="readonly" title="$!resultVal">$!resultVal</div>
					        		#if($!readStr == "")<i class="update_i" title='修改' fieldName='$fieldBean.fieldName' inputType='$fieldBean.inputType' fieldType='$fieldBean.fieldType'  fieldValue='$!resultVal'></i> #end
					        	</li>	
								...
								<li class="bk_li ess">
					        		<div class="swa_c1 " title='$fieldBean.display.get("$globals.getLocale()")'>$fieldBean.display.get("$globals.getLocale()")：</div>
					        		<textarea class="ip_txt" name="$fieldBean.fieldName" id="$fieldBean.fieldName" readonly="readonly" title="$globals.replaceHTML($!resultVal)">$globals.replaceHTML($!resultVal)</textarea>
					        		#if($readStr == "")<i class="update_i" title='修改' fieldName='$fieldBean.fieldName' inputType='$fieldBean.inputType' fieldType='$fieldBean.fieldType' fieldValue='$!resultVal'></i> #end
					        	</li>
							...
							#else
								#if("$fieldBean.fieldName" == "Trade")
								<li class="bk_li ">
					        		<div class="swa_c1 " title='$fieldBean.display.get("$globals.getLocale()")'>$fieldBean.display.get("$globals.getLocale()")：</div>
									<div class="ip_txt" name="$fieldBean.fieldName" id="$fieldBean.fieldName" readonly="readonly" title="$!workProfessionMap.get($!resultVal)">$!workProfessionMap.get($!resultVal)</div>
					        		#if($readStr == "")<i class="update_i" title='修改' fieldName='$fieldBean.fieldName' inputType='$fieldBean.inputType' fieldType='$fieldBean.fieldType' fieldValue='$!resultVal:$!workProfessionMap.get($!resultVal)'></i> #end
					        	</li>
								#elseif("$fieldBean.fieldName" == "District")
								<li class="bk_li ">
					        		<div class="swa_c1 " title='$fieldBean.display.get("$globals.getLocale()")'>$fieldBean.display.get("$globals.getLocale()")：</div>
									<div class="ip_txt" name="$fieldBean.fieldName" id="$fieldBean.fieldName" readonly="readonly" title="$!districtMap.get($!resultVal)">$!districtMap.get($!resultVal)</div>
					        		#if($readStr == "")<i class="update_i" title='修改' fieldName='$fieldBean.fieldName' inputType='$fieldBean.inputType' fieldType='$fieldBean.fieldType' fieldValue='$!resultVal:$!districtMap.get($!resultVal)'></i> #end
					        	</li>	
								#else
									<li class="bk_li ">
						        		<div class="swa_c1 " title='$fieldBean.display.get("$globals.getLocale()")'>$fieldBean.display.get("$globals.getLocale()")：</div>
							...
							<li class="bk_li">
				          		<div class="swa_c1" title='$fieldBean.display.get("$globals.getLocale()")'>$fieldBean.display.get("$globals.getLocale()")：</div>
					            <div class="ip_txt" name="$fieldBean.fieldName" id="$fieldBean.fieldName" readonly="readonly" title='$globals.getEnumerationItemsDisplay("$fieldBean.refEnumerationName","$!resultVal")'>$globals.getEnumerationItemsDisplay("$fieldBean.refEnumerationName","$!resultVal")</div>
				           		#if($readStr == "")
				           		<i class="update_i" title='修改' fieldName='$fieldBean.fieldName' inputType='$fieldBean.inputType' fieldType='$fieldBean.fieldType'></i> 
				           		<select class="slt_in" id="${fieldBean.fieldName}_select" style="display: none;">
			        		 	 	 <option value="" >--请选择--</option>
					             	 #foreach($item in $globals.getEnumerationItems("$fieldBean.refEnumerationName"))	 
					        	       <option value="$item.value"#if("$!resultVal" == "$item.value") selected="selected" #end>$item.name</option>
							         #end
					            </select>
				           		#end
				           	</li>
				        	...
				        	#else
				        		#set($disNames = "")
					    		#set($selectName="popup_"+$fieldBean.fieldName)
					    		#set($disNames = $!defineDisMap.get("$selectName")) 
				        		<div class="ip_txt" name="$fieldBean.fieldName" id="$fieldBean.fieldName" readonly="readonly" title="$disNames">$disNames</div>
				        	#end
				        	</li>
							...
							<li class="bk_li">
				          		<div class="swa_c1" title='$fieldBean.display.get("$globals.getLocale()")'>$fieldBean.display.get("$globals.getLocale()")：</div>
				          		#set($valStr ="")
				          		#foreach($str in $!resultVal.split(","))
				          			#set($valStr = $valStr +$globals.getEnumerationItemsDisplay("$fieldBean.refEnumerationName","$str")+",")	 
				          		#end
				          		#if($valStr != "")
				          			#set($valStr =$globals.subEndwith($valStr,$valStr.length()))
				          			
				          		#end
					            <div class="ip_txt" name="$fieldBean.fieldName" id="$fieldBean.fieldName" readonly="readonly" title='$valStr'>$valStr</div>
				           	</li>	
							...
							<li class="bk_li">
				        		<div class="swa_c1" title='$fieldBean.display.get("$globals.getLocale()")'>$fieldBean.display.get("$globals.getLocale()")：</div>
				        		<div class="ip_txt" name="$fieldBean.fieldName" id="$fieldBean.fieldName" readonly="readonly" title="$!deptMap.get($!resultVal)">$!deptMap.get($!resultVal)</div>
				        		#if($readStr == "")<i class="update_i" title='修改' fieldName='$fieldBean.fieldName' inputType='$fieldBean.inputType' fieldType='$fieldBean.fieldType'  fieldValue='$!resultVal:$!deptMap.get($!resultVal)'></i> #end
				        	</li>
						#elseif("$fieldBean.inputType" == "15")
							<li class="bk_li">
				        		<div class="swa_c1" title='$fieldBean.display.get("$globals.getLocale()")'>$fieldBean.display.get("$globals.getLocale()")：</div>
				        		<div class="ip_txt" name="$fieldBean.fieldName" id="$fieldBean.fieldName" readonly="readonly" title="$!globals.getEmpFullNameByUserId($!resultVal)">$!globals.getEmpFullNameByUserId($!resultVal)</div>
				        		#if($readStr == "")<i class="update_i" title='修改' fieldName='$fieldBean.fieldName' inputType='$fieldBean.inputType' fieldType='$fieldBean.fieldType' fieldValue='$!resultVal:$!globals.getEmpFullNameByUserId($!resultVal)'></i> #end
				        	</li>
       ...
       <div class="f_dv">
			...
			<div class="four-block">
				...
				<div class="four-block-1 swtoDiv">
					...
					<div id="StrengthsDiv">
						<div style="height: 65px;width: 175px;margin-top: 10px;" id="StrengthsCont" title="$!result.get("Strengths")">$!result.get("Strengths")</div>
						#if($!readStr == "")<i class="det_update" style="margin-right:0px;" onclick="editSWOT(this)" fieldName="Strengths" fieldValue="$!result.get("Strengths")" title="修改" id="StrengthsEditBtn"></i>#end
					</div>
				</div>
				...
				<div class="four-block-2 swtoDiv">
					...
					<div id="WeaknessesDiv">
						<div style="height: 65px;width: 175px;margin-top: 10px;" id="WeaknessesCont" title="$!result.get("Weaknesses")">$!result.get("Weaknesses")</div>
						#if($!readStr == "")<i class="det_update" style="margin-right:0px;" onclick="editSWOT(this)" fieldName="Weaknesses" fieldValue="$!result.get("Weaknesses")" title="修改" id="WeaknessesEditBtn"></i>#end
					</div>
				</div>
				<div class="four-block-3 swtoDiv">
					<p id="OpportunitiesTitle">机遇 Opportunities</p>
					<div id="OpportunitiesDiv">
						<div style="height: 65px;width: 175px;margin-top: 10px;" id="OpportunitiesCont" title="$!result.get("Opportunities")">$!result.get("Opportunities")</div>
						#if($!readStr == "")<i class="det_update" style="margin-right:0px;" onclick="editSWOT(this)" fieldName="Opportunities" fieldValue="$!result.get("Opportunities")" title="修改" id="OpportunitiesEditBtn"></i>#end
					</div>
				</div>
				<div class="four-block-4 swtoDiv">
					<p id="ThreatsTitle">挑战 Threats</p>
					<div id="ThreatsDiv">
						<div style="height: 65px;width: 175px;margin-top: 10px;" id="ThreatsCont" title="$!result.get("Threats")">$!result.get("Threats")</div>
						#if($!readStr == "")<i class="det_update" style="margin-right:0px;" onclick="editSWOT(this)" fieldName="Threats" fieldValue="$!result.get("Threats")" title="修改" id="ThreatsEditBtn"></i>#end
					</div>
				</div>
			</div>
		</div>
		...
		<!-- 自定义明细开始 -->
		        <ul>
		        	#foreach($childTableName in $moduleViewBean.childTableInfo.split(","))
		        		...
		        		<table border="0" cellspacing="0" class="d-table" id="${childTableName}_table">
		        			<thead>
				        		<tr>
						        		...
						        		<tr>
						        			#foreach($childFieldName in $moduleViewBean.childDisplayFields.split(","))
													...
													#set($fieldVal = $det.get("$bean.fieldName"))
														...
														#else
															<td width="$defaultWidth">$fieldVal</td>
														#end
														...
														#foreach($srow in $bean.getSelectBean().viewFields)
															...
															#end
															#if($selectFieldBean.inputType!="100")
																<td width="$defaultWidth" title="$defineVal">
																	$defineVal
																	#set($returnFields = $returnFields + $srow.fieldName+",")
													            </td>
															#end
														#end
														<input type="hidden" id="${bean.fieldName}ReturnFieldsStr" value="$returnFields"/>
        ...
        明细 NewEnd -->
	        ...
	        <div class="title_dv" style="overflow:hidden;">
					...
					#if($loginBean.operationMap.get("/CRMClientAction.do").update())
						<span class="btn btn-mini lf" style="margin:2px 0 0 5px;" onclick="upload('AFFIX','uploadBtn');">上传</span>
					#end
			</div>
	        <ul id="affixuploadul_uploadBtn">
	        	...
	        	#foreach($uprow in $result.get("Attachment").split(";"))
						...
						<input type=hidden id="uploadBtn" name=uploadBtn value='$uprow'/><div class="showAffix" title="$uprow">$uprow</div>&nbsp;&nbsp;&nbsp;			
							#if($loginBean.operationMap.get("/CRMClientAction.do").update())
	...
	<iframe src="/DiscussAction.do?tableName=CRMClientInfoDiscuss&&parentIframeName=Frame_${keyIdStr}&f_ref=${keyIdStr}" width="100%" frameborder="no" scrolling="no" id="Frame_${keyIdStr}"></iframe>
	<!-- 评论结束 -->
	</div>
        <div class="icon32_btns">
        	...
        	<div class="btn_item"  onclick="pinrtSetting();">
				<span>
					<b class="icon32 b-2"></b>
					<em>快递单打印</em>
					<p class="bg-p"></p>
				</span>
			</div>
        	#end
        	#if("$!scopeMap.get('printFlag')" == "true" || "$!loginBean.id" == "1")
        ...
        <!-- 流程展示开始 -->
        #if($brotherTableList.size()>0 && "$!flowList" !="")
        	#foreach($flow in $flowList)
        		#set($filterTables = "")
        		#foreach($tableName in $globals.get($flow,2).split(","))
        		#if("$!moduleMap.get($tableName)" != "" && ($loginBean.operationMap.get("/CRMBrotherAction.do?tableName=$tableName").query() || $loginBean.operationMap.get("/UserFunctionQueryAction.do?parentTableName=CRMClientInfo&tableName=$tableName").query()) || $loginBean.operationMap.get("/UserFunctionQueryAction.do?tableName=$tableName").query())
	...
	<div class="rightBlock">
		<div class="task-sche-contact">
			...
			<div class="t-s-c">
				<div class="tags-contact">
						...
						<div class="fast-add">
							<textarea class="txta" placeholder="快速添加联系记录" id="followUpContent" onKeyDown="if(event.keyCode==13) fastSubmit('CRMSaleFollowUp');"></textarea>
							<span class="btn-s" tableName="CRMSaleFollowUp" onclick="commentInfoSubmit(this)">添加</span>
						</div>
					#end
						...
						<li id="$!globals.get($follow,0)">
							<em title="$!globals.get($follow,1)" class="commonDetail">$!globals.get($follow,1)</em>
							<i>#if("$!globals.get($follow,2)"!="")$!globals.get($follow,2).substring(5) #end</i>
						</li>
						#end
						</ul>
						<p class="p-more">
				...
				<div class="tags-scha" style="display:none;">
						...
						<div class="fast-add">
							<textarea class="txta" placeholder="快速添加日程" id="calendarContent" onKeyDown="if(event.keyCode==13) fastSubmit('OACalendar');"></textarea>
							<span class="btn-s" tableName="OACalendar" onclick="commentInfoSubmit(this)">添加</span>
							<span class="time-sp">
								<b class="icons default-time" onclick="WdatePicker({el:$dp.$('calendarStartTime')})" title="选择日程时间"></b>
								<span>
									<input class="time-inp" type="text" id="calendarStartTime" value='$globals.getHongVal("sys_date")' onclick="WdatePicker({lang:'$globals.getLocale()'})" readonly="readonly"/>
									<div class="clear"></div>
								</span>
							</span>
						</div>
						<ul class="tags-ul-list" id="OACalendarUL">
							...
							<li class="task-li" id="$!globals.get($calendar,0)">
								<em title="$!globals.get($calendar,1)" class="commonDetail">$!globals.get($calendar,1)</em>
								
								<i >#if("$!globals.get($calendar,2)"!="")$!globals.get($calendar,2).substring(5)#end</i>
								<b class="b-finish icons" onclick="finishCommentStatus('$!globals.get($calendar,0)','OACalendar');" title="点击完成" /></b>
							</li>
				...
				<div class="tags-log" style="display:none;">
					#if($LoginBean.operationMap.get("/OACalendarAction.do?operation=4&queryFlag=List&crmEnter=true").query())
						<div class="fast-add">
							<textarea class="txta" placeholder="快速添加日志" id="OAWorkLogContent" onKeyDown="if(event.keyCode==13) fastSubmit('OAWorkLog');"></textarea>
							<span class="btn-s" tableName="OAWorkLog" onclick="commentInfoSubmit(this)">添加</span>
							<span class="time-sp">
								<b class="icons default-time" onclick="WdatePicker({el:$dp.$('calendarStartTime')})" title="选择日程时间"></b>
								<span>
									<input class="time-inp" type="text" id="time" value='$globals.getHongVal("sys_date")' onclick="WdatePicker({lang:'$globals.getLocale()'})" readonly="readonly"/>
									<div class="clear"></div>
								</span>
							</span>
						</div>
						<ul class="tags-ul-list" id="OAWorkLogUL">
							...
							<li class="task-li" id="$!globals.get($OAWorkLog,0)">
								<em title="$!globals.get($OAWorkLog,2)" class="commonDetail">$!globals.get($OAWorkLog,2)</em>
								
								<i >#if("$!globals.get($OAWorkLog,1)"!="")$!globals.get($OAWorkLog,1).substring(5)#end</i>
								<b onclick="finishCommentStatus('$!globals.get($OAWorkLog,0)','OACalendar');" title="点击完成" /></b>
							</li>
				...
				<div class="tags-task" style="display:none;">
						...
						<div class="fast-add">
							<textarea class="txta" placeholder="快速添加任务" id="taskContent" onKeyDown="if(event.keyCode==13) fastSubmit('OATask');"></textarea>
							<span class="btn-s" tableName="OATask" onclick="commentInfoSubmit(this)">添加</span>
							<span class="person-sp">
								<b class="icons" title="选择执行人" onclick="deptPop('userGroup','taskUserId')"></b>
								<input type="hidden" id="taskUserId">
								<input class="em-name" id="taskUserIdName" value="执行人" onclick="deptPop('userGroup','taskUserId')" readonly="readonly"/>
							</span>
							<span class="time-sp">
								<b class="icons default-time" onclick="WdatePicker({el:$dp.$('taskFinishTime')})" title="选择结束时间"></b>
								<span>
									<input class="time-inp" type="text" id="taskFinishTime" value='$globals.getHongVal("sys_date")' onclick="WdatePicker({lang:'$globals.getLocale()'})" readonly="readonly"/>
									<div class="clear"></div>
								</span>
							</span>
						</div>	
					...
					<ul class="tags-ul-list" id="OATaskUL">
						#set($taskList = $!commonInfoMap.get("OATask"))
						#foreach($task in $taskList)
						#set($taskStatus = "2")
						#if("$!globals.get($task,3)"!="" && "$!globals.get($task,3)"!="$LoginBean.id")
							#set($taskStatus = "3")
						#end
						
						<li class="task-li" id="$!globals.get($task,0)" taskStatus="$!taskStatus">
							<em title="$!globals.get($task,1)" class="commonDetail">$!globals.get($task,1)</em>
							<i>#if("$!globals.get($task,2)"!="")$!globals.get($task,2).substring(5)#end</i>
							<b class="b-finish icons" onclick="finishCommentStatus('$!globals.get($task,0)','OATask');" title="点击完成" /></b>
						</li>
						#end
					</ul>
						<p class="p-more">
		...
		#foreach ($contact in $!result.get("TABLENAME_${contactTableName}"))
				...
				<div class="contactBlock">
					<span><b></b>$!contact.get("UserName") #if($!readStr == "")<i class="det_update"  onclick="updateClientDet('$!contact.get("id")')" title="修改"></i>#end</span>
					<ul>
						<li><b class="bPhone"></b><i #if("$!contact.get('Mobile')" !="") onclick="sendMsg('detailsms','$contact.get("id")')" style="cursor: pointer;" #end>$!contact.get("Mobile")</i></li>
						<li><b class="bEmail"></b><i #if("$!contact.get('ClientEmail')" !="") onclick="sendMsg('detailemail','$contact.get("id")')" style="cursor: pointer;" #end>$!contact.get("ClientEmail")</i></li>
...
<!-- 定位导航 Start -->
<div style="position:fixed;top:0px;width:200px;left: 60px;" id="quickPositionDiv">
	<div class="fix_dv">
...
<!-- 定位导航 End -->
<div class="addWrap pop-layer" id="addFeedbackDiv" style="top: 50px; left: 500px; display:none;"></div>

</form>
<div class="add-calendar" id="addCalendar" style="display:none;left:35%;top: 50px;"></div>

<div class="add-calendar" id="deliverGoodsDiv" style="top: 86px; left: 604px;width:260px;display: none;">
	<p class="inp-txt" style="width:260px;height:20px;">发货信息</p>
	<div class="point-block" align="center">
		<label class="lf">手机串码</label>
		<input class="lf point-i"  type="text" id="mobileCode">
	</div>
	<div class="point-block" align="center">
	...
	<div class="point-block">
		<input class="lf btn-add" type="button" id="dns_btn" value="确定" onclick="deliverGoodsSubmit('$!result.get("id")');">
		<input class="lf btn-cel" type="button" value="取消" onclick="deliverGoodsCancel(this);">
	</div>
