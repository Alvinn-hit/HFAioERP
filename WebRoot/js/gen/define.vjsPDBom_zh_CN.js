...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDBom&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDBom&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDBom&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDBom&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDBomDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDBomDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDBomDet"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDBom\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDBom\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'totalHours',dis:'工位加总工时(秒)',digit:0,isMain:true};
 ...
 fieldDigit[fieldDigit.length] = {name:'PDBomDet_qty',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDBomDet_lossRate',dis:'损耗率%',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDBom",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDBom&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDBom&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDBom&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDBom&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='PDBomDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='生产BOM明细';
childCount++;
 var PDBomDetData = new gridData();
 gridDatas[gridDataNum] = PDBomDetData;
 gridDataNum++;
 var PDBomDet=1;
 ...
 addCols(PDBomDetData,'PDBomDet_GoodsCode','品名','200',2,false,'',50,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("PDBomDet_GoodsCode","品名","any","",false,minValue,50,false,'');
 
 
 addCols(PDBomDetData,'PDBomDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'PDBomDet:GoodsCode:PDBomDet_tblGoods_GoodsNumber:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDBomDetData,'PDBomDet_tblGoods_GoodsFullName','品名','150',2,true,'',1000000,2,'PDBomDet:GoodsCode:PDBomDet_tblGoods_GoodsFullName:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDBomDetData,'PDBomDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDBomDet:GoodsCode:PDBomDet_tblGoods_GoodsSpec:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDBomDetData,'PDBomDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDBomDet:GoodsCode:PDBomDet_tblGoods_BaseUnit:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDBomDetData,'PDBomDet_tblGoods_attrType','来源码','80',2,true,'',1000000,2,'PDBomDet:GoodsCode:PDBomDet_tblGoods_attrType:'+param4,'',"","","0",'',"2",false); 
 
 
 
 
 addCols(PDBomDetData,'PDBomDet_qty','数量','60',1,false,'',0,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("PDBomDet_qty","数量","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDBomDetData,'PDBomDet_lossRate','损耗率%','50',1,true,'',0,0,'','',"Qty","","2",'',"0",false); 
 putChildValidateItem("PDBomDet_lossRate","损耗率%","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDBomDetData,'PDBomDet_canSuper','客供包料','50',2,true,'2',10,1,';isClient:客供;isSupper:包料','',"","","3",'',"1",false);
 putChildValidateItem("PDBomDet_canSuper","客供包料","any","",true,minValue,10,true,'2',true); 
 
 
 
 addCols(PDBomDetData,'PDBomDet_remark','备注','200',18,true,'',500,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("PDBomDet_remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 addCols(PDBomDetData,'PDBomDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDBom&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDBom,PDBomDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDBom&allTableName=PDBom,PDBomDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDBom&allTableName=PDBomDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDBom&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDBom&allTableName=PDBomDet,&colType=colWidth" ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDBom&designId=0badecf0_1603161024387910025&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDBom&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDBom&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDBom&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '量产BOM';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDBom&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
