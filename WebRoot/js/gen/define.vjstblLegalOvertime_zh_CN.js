...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblLegalOvertime&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblLegalOvertime&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblLegalOvertime&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblLegalOvertime&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblLegalOvertimeDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblLegalOvertimeDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblLegalOvertimeDet"){
...
function execStat2(){
 execStatOne('tblLegalOvertimeDet_Total',2);
 setTotalCount(getTotalCount());
}
...
function execDetailStat2(){
 execDetStatOne('tblLegalOvertimeDet_Total',2); 
 }
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=d2751396_1402211114390342326&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=d2751396_1402211114390342326&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblLegalOvertime\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblLegalOvertime\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'finishTime',dis:'基准完成时间',digit:0,isMain:true};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblLegalOvertime",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblLegalOvertime&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblLegalOvertime&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'打印控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"d2751396_1402211114390342326",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=d2751396_1402211114390342326&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=d2751396_1402211114390342326&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=d2751396_1402211114390342326&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblLegalOvertime"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblLegalOvertime&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblLegalOvertime&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblLegalOvertimeDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='法定假日加班申请明细表';
childCount++;
 var tblLegalOvertimeDetData = new gridData();
 gridDatas[gridDataNum] = tblLegalOvertimeDetData;
 gridDataNum++;
 var tblLegalOvertimeDet=10;
 ...
 var param4="";
 param4=param4+":0申请部门@DepartmentCode"; 
 
 addCols(tblLegalOvertimeDetData,'tblLegalOvertimeDet_EmployeeCode','职员','80',2,false,'',100,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("tblLegalOvertimeDet_EmployeeCode","职员","any","",false,minValue,100,false,'');
 addCols(tblLegalOvertimeDetData,'tblLegalOvertimeDet_tblEmployee_EmpFullName','职员','120',2,true,'',1000000,2,'tblLegalOvertimeDet:EmployeeCode:tblLegalOvertimeDet_tblEmployee_EmpFullName:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(tblLegalOvertimeDetData,'tblLegalOvertimeDet_Business','职位','60',2,true,'',100,-2,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblLegalOvertimeDet_Business","职位","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblLegalOvertimeDetData,'tblLegalOvertimeDet_OvertimeDate','加班日期','100',5,true,'',20,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblLegalOvertimeDet_OvertimeDate","加班日期","date","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblLegalOvertimeDetData,'tblLegalOvertimeDet_Total','合计(天数)','100',2,true,'',100,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblLegalOvertimeDet_Total","合计(天数)","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblLegalOvertimeDetData,'tblLegalOvertimeDet_Remark','备注','200',2,true,'',8000,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblLegalOvertimeDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 
 addCols(tblLegalOvertimeDetData,'tblLegalOvertimeDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblLegalOvertime&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblLegalOvertime,tblLegalOvertimeDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblLegalOvertime&allTableName=tblLegalOvertime,tblLegalOvertimeDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblLegalOvertime&allTableName=tblLegalOvertimeDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblLegalOvertime&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblLegalOvertime&allTableName=tblLegalOvertimeDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=d2751396_1402211114390342326&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=d2751396_1402211114390342326&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblLegalOvertimeDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=d2751396_1402211114390342326&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=d2751396_1402211114390342326&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblLegalOvertime&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){
 location.href="/UserFunctionAction.do?tableName=tblLegalOvertime&designId=d2751396_1402211114390342326&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblLegalOvertime&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblLegalOvertime&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblLegalOvertime&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '加班申请表';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblLegalOvertime&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
