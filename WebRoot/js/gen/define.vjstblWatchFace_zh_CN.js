...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblWatchFace&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblWatchFace&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblWatchFace&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblWatchFace&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchFaceDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchFaceDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblWatchFaceDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblWatchFaceDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchFaceDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchFaceDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblWatchFaceDet"; 
 tblName[1]="tblWatchFaceDet1"; 
...
function execStat2(){
 execStatOne('tblWatchFaceDet_Qty',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblWatchFace\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblWatchFace\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblWatchFaceDet_Qty',dis:'用量',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblWatchFace",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchFace&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchFace&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblWatchFace"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblWatchFace&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchFace&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblWatchFaceDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表面物料明细';
childCount++;
 var tblWatchFaceDetData = new gridData();
 gridDatas[gridDataNum] = tblWatchFaceDetData;
 gridDataNum++;
 var tblWatchFaceDet=1;
  var param4="";
 param4=param4+":1样板编号@SampleOrderID"; 
 addCols(tblWatchFaceDetData,'tblWatchFaceDet_GoodsNumber','物料编号','150',2,false,'',500,2,'tblWatchFaceDet:GoodsNumber:tblWatchFaceDet_GoodsNumber:'+param4,"","","","2",'',"2",true);
 putChildValidateItem("tblWatchFaceDet_GoodsNumber","物料编号","any","",false,minValue,500,false,'',true);
  addCols(tblWatchFaceDetData,'tblWatchFaceDet_Shading','底纹','200',2,true,'',200,8,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchFaceDet_Shading","底纹","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchFaceDetData,'tblWatchFaceDet_CuletColor','面底颜色/A 印刷颜色','200',2,true,'',200,8,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblWatchFaceDet_CuletColor","面底颜色/A 印刷颜色","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchFaceDetData,'tblWatchFaceDet_Othercolor','其它印刷色','120',2,true,'',200,8,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblWatchFaceDet_Othercolor","其它印刷色","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchFaceDetData,'tblWatchFaceDet_NDColor','ND颜色','200',2,true,'',200,8,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblWatchFaceDet_NDColor","ND颜色","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchFaceDetData,'tblWatchFaceDet_Circle','内影圈','200',2,true,'',200,8,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblWatchFaceDet_Circle","内影圈","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchFaceDetData,'tblWatchFaceDet_Qty','用量','80',1,true,'1',0,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblWatchFaceDet_Qty","用量","double","",true,minValue,maxValue,true,'1',true); 
  addCols(tblWatchFaceDetData,'tblWatchFaceDet_Remark','备注','200',2,true,'',500,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblWatchFaceDet_Remark","备注","any","",true,minValue,500,false,'',true); 
    addCols(tblWatchFaceDetData,'tblWatchFaceDet_GoodsCode','物料名称','100',2,true,'',1000,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblWatchFaceDet_GoodsCode","物料名称","any","",true,minValue,1000,false,''); 
                
 addCols(tblWatchFaceDetData,'tblWatchFaceDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblWatchFaceDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表面变更明细';
childCount++;
 var tblWatchFaceDet1Data = new gridData();
 gridDatas[gridDataNum] = tblWatchFaceDet1Data;
 gridDataNum++;
 var tblWatchFaceDet1=1;
  addCols(tblWatchFaceDet1Data,'tblWatchFaceDet1_ChangeContent','变更内容','200',2,false,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblWatchFaceDet1_ChangeContent","变更内容","any","",false,minValue,500,false,'',true); 
  addCols(tblWatchFaceDet1Data,'tblWatchFaceDet1_ChangeDate','变更日期','90',5,false,'2013-12-24',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchFaceDet1_ChangeDate","变更日期","date","",false,minValue,10,true,'2013-12-24',true); 
  addCols(tblWatchFaceDet1Data,'tblWatchFaceDet1_ChangePer','变更人员','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchFaceDet1_ChangePer","变更人员","any","",true,minValue,100,false,'',true); 
  addCols(tblWatchFaceDet1Data,'tblWatchFaceDet1_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchFaceDet1_Remark","备注","any","",true,minValue,500,false,'',true); 
       
 addCols(tblWatchFaceDet1Data,'tblWatchFaceDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblWatchFace&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=5d4be509_1412111501422990071&noback="+noback+"&parentCode=&operation=6&parentTableName=tblSampleOrder&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblWatchFace,tblWatchFaceDet,tblWatchFaceDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchFace&allTableName=tblWatchFace,tblWatchFaceDet,tblWatchFaceDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchFace&allTableName=tblWatchFaceDet,tblWatchFaceDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchFace&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchFace&allTableName=tblWatchFaceDet,tblWatchFaceDet1,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblWatchFace&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblWatchFace&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblWatchFace&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchFace&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblWatchFace&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '样板订单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblWatchFace&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
