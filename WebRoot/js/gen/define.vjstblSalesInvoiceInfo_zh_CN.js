...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblSalesInvoiceInfo&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblSalesInvoiceInfo&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblSalesInvoiceInfo&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblSalesInvoiceInfo&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblSalesInfoDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblSalesInfoDet_GoodsCode');
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblSalesInfoDet"){
 
 copyFields=n('tblSalesInfoDet_CompanyCode');
 ...
 var tblSalesInfoDet_tblCompany_ComFullName = n('tblSalesInfoDet_tblCompany_ComFullName');
 if(typeof(tblSalesInfoDet_tblCompany_ComFullName) != 'undefined' && tblSalesInfoDet_tblCompany_ComFullName.length >index){
 tblSalesInfoDet_tblCompany_ComFullName[index].value = tblSalesInfoDet_tblCompany_ComFullName[index-1].value;
 }
 }
 copyFields=n('tblSalesInfoDet_RefBillNo');
 if((order > 1 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 copyFields=n('tblSalesInfoDet_InvoiceNo');
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblSalesInfoDet"){
...
function execStat2(){
 execStatOne('tblSalesInfoDet_BillAmt',2);
 execStatOne('tblSalesInfoDet_InvoiceAmount',2);
 execStatOne('tblSalesInfoDet_NeedReturnAmt',2);
 execStatOne('tblSalesInfoDet_ExeBalAmt',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=8bab04e8_0907160938238900027&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=8bab04e8_0907160938238900027&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblSalesInvoiceInfo\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblSalesInvoiceInfo\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblSalesInfoDet_ExeBalAmt',"tblSalesInfoDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'InvoiceAmount',dis:'开票金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblSalesInfoDet_BillAmt',dis:'应开票金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalesInfoDet_InvoiceAmount',dis:'已开票金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalesInfoDet_NeedReturnAmt',dis:'未开票金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalesInfoDet_ExeBalAmt',dis:'本次开票金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblSalesInvoiceInfo",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSalesInvoiceInfo&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSalesInvoiceInfo&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"8bab04e8_0907160938238900027",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "FreightCom"){
 var saveField=n("FreightCom"); 
 saveField[0].value = "";
 var obj = m("tbl_tblFreightCom_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblFreightCom_ComFullName");
 saveField[0].value = ""; 
 m("tbl_tblFreightCom_ComFullName").value = "" ;
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=8bab04e8_0907160938238900027&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=8bab04e8_0907160938238900027&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=8bab04e8_0907160938238900027&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblSalesInvoiceInfo"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblSalesInvoiceInfo&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblSalesInvoiceInfo&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 var z=n('InvoiceAmount');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TaxAmount')[0].value=f(nn('InvoiceAmount',0)*nn('TaxRate',0),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesInfoDet_ExeBalAmt'); 
 n('InvoiceAmount')[0].value=f(sm('tblSalesInfoDet_ExeBalAmt'),2);
n('TaxAmount')[0].value=f(nn('InvoiceAmount',0)*nn('TaxRate',0),2);
...
childData[childCount]='tblSalesInfoDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='销售开票管理明细';
childCount++;
 var tblSalesInfoDetData = new gridData();
 gridDatas[gridDataNum] = tblSalesInfoDetData;
 gridDataNum++;
 var tblSalesInfoDet=1;
 ...
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_CompanyCode','客户','80',2,false,'',100,-2,'','',"Customer","copy","0",'',"2",true);
 putChildValidateItem("tblSalesInfoDet_CompanyCode","客户","any","",false,minValue,100,false,'');
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_tblCompany_ComFullName','客户','200',18,true,'',1000000,2,'tblSalesInfoDet:CompanyCode:tblSalesInfoDet_tblCompany_ComFullName:'+param4,'',"Customer","copy","0",'',"2",false); 
 ...
 var param4="";
 param4=param4+":1客户@tblSalesInfoDet_CompanyCode";
 param4=param4+":1单据日期@BillDate"; 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_RefBillNo','关联单据号','120',2,true,'',50,2,'tblSalesInfoDet:RefBillNo:tblSalesInfoDet_RefBillNo:'+param4,"","","copy","1",'',"2",true);
 putChildValidateItem("tblSalesInfoDet_RefBillNo","关联单据号","any","",true,minValue,50,false,'',true);
 
 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_InvoiceNo','发票号','110',2,true,'',50,0,'','',"","copy","3",'',"0",false); 
 putChildValidateItem("tblSalesInfoDet_InvoiceNo","发票号","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_RefBillID','关联单据ID','80',2,true,'',50,-2,'','',"","","4",'',"3",false); 
 putChildValidateItem("tblSalesInfoDet_RefBillID","关联单据ID","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_GoodsCode','商品名称','200',2,true,'',50,-2,'','',"","triggerCopy","5",'',"2",false); 
 putChildValidateItem("tblSalesInfoDet_GoodsCode","商品名称","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_BillAmt','应开票金额','80',1,true,'',0,8,'','',"SAmountIdentifier","","7",'',"0",false); 
 putChildValidateItem("tblSalesInfoDet_BillAmt","应开票金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_InvoiceAmount','已开票金额','80',1,true,'',0,8,'','',"SAmountIdentifier","","9",'',"0",false); 
 putChildValidateItem("tblSalesInfoDet_InvoiceAmount","已开票金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_NeedReturnAmt','未开票金额','80',1,true,'',0,8,'','',"SAmountIdentifier","","11",'',"0",false); 
 putChildValidateItem("tblSalesInfoDet_NeedReturnAmt","未开票金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_BillType','单据类型','200',2,true,'',100,-2,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblSalesInfoDet_BillType","单据类型","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_ExeBalAmt','本次开票金额','80',1,true,'',0,0,'','',"SAmountIdentifier","","13",'',"0",false); 
 putChildValidateItem("tblSalesInfoDet_ExeBalAmt","本次开票金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_Remark','备注','160',2,true,'',8000,0,'','',"","","50",'',"0",false); 
 putChildValidateItem("tblSalesInfoDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_SourceID','源ID','224',2,true,'',100,-2,'','',"","","94",'',"3",false); 
 putChildValidateItem("tblSalesInfoDet_SourceID","源ID","any","",true,minValue,100,false,''); 
 
 
 
 
 addCols(tblSalesInfoDetData,'tblSalesInfoDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblSalesInvoiceInfo&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblSalesInvoiceInfo,tblSalesInfoDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSalesInvoiceInfo&allTableName=tblSalesInvoiceInfo,tblSalesInfoDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSalesInvoiceInfo&allTableName=tblSalesInfoDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSalesInvoiceInfo&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSalesInvoiceInfo&allTableName=tblSalesInfoDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=8bab04e8_0907160938238900027&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=8bab04e8_0907160938238900027&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblSalesInvoiceInfoDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=8bab04e8_0907160938238900027&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=8bab04e8_0907160938238900027&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblSalesInvoiceInfo&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=8bab04e8_0907160938238900027&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblSalesInvoiceInfo&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblSalesInvoiceInfo&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblSalesInvoiceInfo&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblSalesInvoiceInfo&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '销售开票单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblSalesInvoiceInfo&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
