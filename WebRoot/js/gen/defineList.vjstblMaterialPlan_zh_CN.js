...
function showAdvancePanel(){
 var urls = "/ReportDataAction.do?operation=advance&reportNumber=tblMaterialPlan";
 asyncbox.open({id:'advancePanel',title:'条件高级设置',url:urls,width:450,height:400});
}
function showAdvanceUpdatePanel(opType,keyId){
 var urls = "/ReportDataAction.do?operation=advance&opType="+opType+"&keyId="+keyId+"&reportNumber=tblMaterialPlan";
...
function closeAdvanceUpdate(){
 ...
 jQuery.reload('advancePanel',"/ReportDataAction.do?operation=advance&reportNumber=tblMaterialPlan"); 
}
...
function onLockClick(str){
 AjaxRequest("/UtilServlet?operation=lockColumn&report=tblMaterialPlan&number="+str); 
}
...
function printData(){
 ...
 var urls = "/UserFunctionQueryAction.do?tableName=tblMaterialPlan&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+bid+"&parentTableName=&winCurIndex="+winCurIndex;
...
function printLabelData(){
 ...
 var urls = "/UserFunctionQueryAction.do?tableName=tblMaterialPlan&reportNumber=PrintSeqtblMaterialPlan&printType=labelPrint&operation=18&keyId="+keyVal+"&parentTableName=&winCurIndex="+winCurIndex;
...
function onItemClick(str){
 ...
 if(currentId!=str){
 currentId = str ;
 setIframSrc2("/UserFunctionAction.do?tableName=tblMaterialPlan&keyId="+str+"&operation=7&winCurIndex="+winCurIndex,currentId);
 }
}
...
function setColumnSet(str,lockName){
 ...
 lockName = "";
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblMaterialPlan&colType=list&lockName="+lockName+"&colSelect="+str ;
 AjaxRequest(strURL);
...
function onDefaultCol(){
 ...
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblMaterialPlan&colType=list" ;
 AjaxRequest(strURL);
 if(location.href.indexOf("src=menu")!=-1)
 {
 window.location.reload();
...
function copyOperation()
{
 ...
 usrc="/UserFunctionAction.do?&operation=17&tableName=tblMaterialPlan&saveDraft=&parentTableName=&moduleType="+moduleTypeValue+"&f_brother="+f_brother+"&noback=false&winCurIndex="+winCurIndex+"&keyId="+document.getElementById("varKeyIds").value.split("|")[0];
...
function telCall()
{
 ...
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblMaterialPlan&keyId="+keyIds,"","dialogWidth=600px;dialogHeight=350px"); 
 } 
 }else
 {
 alert("请选择至少一条记录"); 
...
function addPrepare(){
 ...
 usrc ='/UserFunctionAction.do?tableName=tblMaterialPlan&parentCode='+parentCode+'&operation=6&moduleType='+moduleTypeValue+'&f_brother='+f_brother+'&parentTableName=&winCurIndex='+winCurIndex;
...
function openSelect(selectName,fieldName,display){
 ...
 var urlstr = '/UserFunctionAction.do?operation=22&src=menu&parentTableName='+parentTableName+'&moduleType='+moduleType+'&displayName='+encodeURI(display)+'&LinkType=@URL:&tableName=tblMaterialPlan&selectName='+selectName+encodeURI(linkStr) ;
...
function exePopdiv(returnValue){
 ...
 if(typeof(str)!="undefined"){
 if(selectName=="ReportSelectMZProject"){
 ...
 if(content[1-1].length>0){clear=false;document.getElementById("ProjectNo").value+=content[1-1]+',';} 
 if(content[2-1].length>0){clear=false;document.getElementById("ProjectName").value+=content[2-1]+',';} 
 }
 }
 if(clear){
 document.getElementById("ProjectNo").value="";
 ...
 document.getElementById("ProjectNo").value=document.getElementById("ProjectNo").value.substring(0,document.getElementById("ProjectNo").value.length-1) ;
 document.getElementById("ProjectName").value=document.getElementById("ProjectName").value.substring(0,document.getElementById("ProjectName").value.length-1) ;
 }
 }
 }
}
...
function exePopadvancediv(returnValue){
 ...
 if(typeof(str)!="undefined"){
 if(selectName=="ReportSelectMZProject"){
 jQuery.opener("advanceUpdatePanel").document.getElementById("ProjectNo").value="";
 jQuery.opener("advanceUpdatePanel").document.getElementById("ProjectName").value="";
 var group=str.split("|") ;
 var clear=true;
 for(var i=0;i<group.length;i++){
 if(group[i].length>0){
 var content = group[i].split(";") ;
 if(content[1-1].length>0){clear=false;jQuery.opener("advanceUpdatePanel").document.getElementById("ProjectNo").value+=content[1-1]+',';} 
 if(content[2-1].length>0){clear=false;jQuery.opener("advanceUpdatePanel").document.getElementById("ProjectName").value+=content[2-1]+',';} 
 }
 }
 if(clear){
 jQuery.opener("advanceUpdatePanel").document.getElementById("ProjectNo").value="";
 ...
 jQuery.opener("advanceUpdatePanel").document.getElementById("ProjectNo").value=jQuery.opener("advanceUpdatePanel").document.getElementById("ProjectNo").value.substring(0,jQuery.opener("advanceUpdatePanel").document.getElementById("ProjectNo").value.length-1) ;
 jQuery.opener("advanceUpdatePanel").document.getElementById("ProjectName").value=jQuery.opener("advanceUpdatePanel").document.getElementById("ProjectName").value.substring(0,jQuery.opener("advanceUpdatePanel").document.getElementById("ProjectName").value.length-1) ;
 }
 }
 }
}
...
function setFieldValue(str,selectName,fieldName){
 var fs=str.split("#;#");
 if(selectName=="ReportSelectMZProject"){
 document.getElementById("ProjectNo").value=fs[1-1];
 document.getElementById("ProjectName").value=fs[2-1];
 var lastObject = document.getElementById("ProjectName");
...
function showDivCustomSetTable(){ 
 var returnValue = window.showModalDialog("/UserFunctionQueryAction.do?tableName=tblMaterialPlan&operation=4&parentTableName=&configType=listConfig&checkTab=Y&winCurIndex="+winCurIndex,"","dialogWidth=670px;dialogHeight=450px") ;
...
function detail(str){
 turl="/UserFunctionAction.do?tableName=tblMaterialPlan&keyId="+str.split(",")[0]+"&moduleType="+moduleType+"&f_brother=&operation=5&winCurIndex="+winCurIndex+"&pageNo=&parentCode="+str.split(",")[1]+"&parentTableName=&saveDraft=";
...
function update(str){
 turl="/UserFunctionAction.do?tableName=tblMaterialPlan&keyId="+str.split(",")[0]+"&moduleType="+moduleType+"&f_brother=&operation=7&winCurIndex="+winCurIndex+"&pageNo=&parentCode="+str.split(",")[1]+"&parentTableName=&saveDraft=";
...
function next(str){
 location.href="/UserFunctionQueryAction.do?tableName=tblMaterialPlan&keyId="+str.split(",")[0]+"&parentCode="+str.split(",")[1]+"&operation=4&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&parentTableName=&checkTab=Y&selectType="+$("#selectType").val();
}


function doChangeParent(parentCode,name,ttableName){
 location.href="/UserFunctionQueryAction.do?tableName=tblMaterialPlan&parentCode="+parentCode+"&operation=4&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&parentTableName=&checkTab=Y&selectType="+$("#selectType").val();
}
...
function addClass(parentCodev){
 ...
 usrc ='/UserFunctionAction.do?tableName=tblMaterialPlan&parentCode='+parentCodev+'&operation=6&moduleType='+moduleTypeValue+'&f_brother='+f_brother+'&parentTableName=&winCurIndex='+winCurIndex;
...
function hurryTrans(str){
 ...
 hurryTransDialog("/vm/oa/workflow/noteModel.jsp?tableName=tblMaterialPlan&moduleName="+encodeURI(ren)+"&keyId="+str.split(",")[0],winObj,"dialogWidth=565px;dialogHeight=350px");
}
...
function check(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblMaterialPlan&operation=25&ButtonType=saveCheckBill&varKeyIds="+str.split(",")[0]+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+str.split(",")[0]+"&tableName=tblMaterialPlan&operation=82&fromPage=erp");
 ...
 location.href="/UserFunctionQueryAction.do?tableName=tblMaterialPlan&moduleType="+moduleType+"&keyId="+str.split(",")[0]+"&f_brother=&operation=83&winCurIndex="+winCurIndex+"&pageNo=&parentTableName=&saveDraft=";
 }else if(str.split(",")[1]=="cancel"){
 if(confirm('确定撤回?')){
 location.href='/UserFunctionQueryAction.do?nextStep=cancel&moduleType='+moduleType+'&tableName=tblMaterialPlan&lastNodeId='+str.split(",")[2]+'&keyId='+str.split(",")[0]+'&currNode='+str.split(",")[3]+'&f_brother=&operation=79&winCurIndex='+winCurIndex+'&pageNo=&parentTableName=&saveDraft='
 }
 }
}
