...
{ 
 cFireEvent('tblApplyCuttingDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblApplyCutting&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblApplyCutting&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblApplyCutting&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblApplyCutting&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblApplyCuttingDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblApplyCuttingDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblApplyCuttingDet"){
...
function execStat2(){
 execStatOne('tblApplyCuttingDet_Amount',2);
 execStatOne('tblApplyCuttingDet_TotalAmount',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=556374c3_1507141541553750418&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=556374c3_1507141541553750418&MOOP=add'; 
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblApplyCutting\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblApplyCuttingDet_Price',"tblApplyCuttingDet"); 
 delFireEventOne(rowId,tableName,'tblApplyCuttingDet_Amount',"tblApplyCuttingDet"); 
 delFireEventOne(rowId,tableName,'tblApplyCuttingDet_TotalAmount',"tblApplyCuttingDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'TotalCost',dis:'总金额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblApplyCuttingDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblApplyCuttingDet_Amount',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblApplyCuttingDet_TotalAmount',dis:'金额',digit:0,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblApplyCutting",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblApplyCutting&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblApplyCutting&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"556374c3_1507141541553750418",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=556374c3_1507141541553750418&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=556374c3_1507141541553750418&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=556374c3_1507141541553750418&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblApplyCutting"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblApplyCutting&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblApplyCutting&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblApplyCuttingDet_Price'); 
 r(0,"TotalAmount")[p].value=f(rn(0,"Amount",p)*rn(0,"Price",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblApplyCuttingDet_Amount'); 
 r(0,"TotalAmount")[p].value=f(rn(0,"Amount",p)*rn(0,"Price",p),2);

 execStat(); 
 }
 }
 var z=r(0,'TotalAmount');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblApplyCuttingDet_TotalAmount'); 
 n('TotalCost')[0].value=f(sm('tblApplyCuttingDet_TotalAmount'),2);
...
childData[childCount]='tblApplyCuttingDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='领料单明细';
childCount++;
 var tblApplyCuttingDetData = new gridData();
 gridDatas[gridDataNum] = tblApplyCuttingDetData;
 gridDataNum++;
 var tblApplyCuttingDet=1;
 ...
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_WareNo','材料编码','100',2,true,'',100,2,'tblApplyCuttingDet:WareNo:tblApplyCuttingDet_WareNo:'+param4,"","","","0",'',"2",true);
 putChildValidateItem("tblApplyCuttingDet_WareNo","材料编码","any","",true,minValue,100,false,'',true);
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_tblStocks_lastQty','实际库存','100',1,true,'',1000000,2,'tblApplyCuttingDet:WareNo:tblApplyCuttingDet_tblStocks_lastQty:'+param4,'',"","","0",'',"2",false); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_BatchNo','批号','100',2,true,'',100,-2,'','',"","","1",'',"8",false); 
 putChildValidateItem("tblApplyCuttingDet_BatchNo","批号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_WareName','材料名称','150',2,true,'',200,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblApplyCuttingDet_WareName","材料名称","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_Specification','规格/型号','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblApplyCuttingDet_Specification","规格/型号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_Brand','品牌','100',2,true,'',100,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblApplyCuttingDet_Brand","品牌","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_Price','单价','100',1,true,'',0,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblApplyCuttingDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_WareUnit','计量单位','100',2,true,'',100,0,'','',"","","6",'',"8",false); 
 putChildValidateItem("tblApplyCuttingDet_WareUnit","计量单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_CurrentStore','当前库存','100',1,true,'',0,-2,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblApplyCuttingDet_CurrentStore","当前库存","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_Amount','数量','100',1,true,'0',0,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblApplyCuttingDet_Amount","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_MaterialDetId','材料总计划明细ID','100',2,true,'',100,-2,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblApplyCuttingDet_MaterialDetId","材料总计划明细ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_goodscode','商品编码','100',2,true,'',100,-2,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblApplyCuttingDet_goodscode","商品编码","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_TotalAmount','金额','100',1,true,'',0,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblApplyCuttingDet_TotalAmount","金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_sourceid','入库明细id','200',2,true,'',100,-2,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblApplyCuttingDet_sourceid","入库明细id","any","",true,minValue,100,false,''); 
 
 
 
 
 addCols(tblApplyCuttingDetData,'tblApplyCuttingDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblApplyCutting&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblApplyCutting,tblApplyCuttingDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblApplyCutting&allTableName=tblApplyCutting,tblApplyCuttingDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblApplyCutting&allTableName=tblApplyCuttingDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblApplyCutting&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblApplyCutting&allTableName=tblApplyCuttingDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=556374c3_1507141541553750418&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=556374c3_1507141541553750418&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblApplyCuttingDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=556374c3_1507141541553750418&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=556374c3_1507141541553750418&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblApplyCutting&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=556374c3_1507141541553750418&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblApplyCutting&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblApplyCutting&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblApplyCutting&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblApplyCutting&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '领料单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblApplyCutting&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
