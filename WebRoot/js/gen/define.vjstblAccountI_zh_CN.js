...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblAccountI&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblAccountI&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblAccountI&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblAccountI&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblAccountIDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblAccountIDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblAccountIDet"){
...
function execStat2(){
 execStatOne('tblAccountIDet_PayAmount',2);
 execStatOne('tblAccountIDet_PayRate',8);
 execStatOne('tblAccountIDet_RMB',2);
 execStatOne('tblAccountIDet_HZSerAmount',2);
 execStatOne('tblAccountIDet_DLFee',2);
 execStatOne('tblAccountIDet_DLSerAmount',2);
 execStatOne('tblAccountIDet_Amount',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=9b802566_1211121523320000141&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=9b802566_1211121523320000141&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblAccountI\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblAccountI\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblAccountIDet_PayAmount',"tblAccountIDet"); 
 delFireEventOne(rowId,tableName,'tblAccountIDet_PayRate',"tblAccountIDet"); 
 delFireEventOne(rowId,tableName,'tblAccountIDet_RMB',"tblAccountIDet"); 
 delFireEventOne(rowId,tableName,'tblAccountIDet_HZDays',"tblAccountIDet"); 
 delFireEventOne(rowId,tableName,'tblAccountIDet_HZSerAmount',"tblAccountIDet"); 
 delFireEventOne(rowId,tableName,'tblAccountIDet_DLFee',"tblAccountIDet"); 
 delFireEventOne(rowId,tableName,'tblAccountIDet_DLDays',"tblAccountIDet"); 
 delFireEventOne(rowId,tableName,'tblAccountIDet_DLSerAmount',"tblAccountIDet"); 
 delFireEventOne(rowId,tableName,'tblAccountIDet_Amount',"tblAccountIDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'ProxyRate',dis:'综合费率',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblAccountIDet_PayAmount',dis:'付汇金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountIDet_PayRate',dis:'汇率',digit:8,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountIDet_RMB',dis:'人民币货值',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountIDet_HZSerAmount',dis:'货值服务费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountIDet_DLFee',dis:'代理费及杂费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountIDet_DLSerAmount',dis:'代理费杂费服务费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountIDet_Amount',dis:'资金占用费金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblAccountI",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAccountI&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAccountI&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"9b802566_1211121523320000141",
 MOOP:"add",
...
function resetSubmit(fieldName){
 if(fieldName == "ComFullName"){
 var saveField=n("ComFullName"); 
 saveField[0].value = "";
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=9b802566_1211121523320000141&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=9b802566_1211121523320000141&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=9b802566_1211121523320000141&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblAccountI"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblAccountI&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblAccountI&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
r(0,"RMB")[j].value=f(rn(0,"PayAmount",j)*rn(0,"PayRate",j),2);
}

for(var j=0;j<r(0,'HZSerAmount').length;j++){
r(0,"HZSerAmount")[j].value=f(rn(0,"RMB",j)*rn(0,"HZDays",j)*nn('ProxyRate',0),2);
}

for(var j=0;j<r(0,'DLSerAmount').length;j++){
r(0,"DLSerAmount")[j].value=f(rn(0,"DLFee",j)*rn(0,"DLDays",j)*nn('ProxyRate',0),2);
}

for(var j=0;j<r(0,'Amount').length;j++){
r(0,"Amount")[j].value=f(Number(rn(0,"HZSerAmount",j))+Number(rn(0,"DLSerAmount",j)),2);
}

n('TotalRMB')[0].value=f(sm('tblAccountIDet_RMB'),2);

n('TotalHZSerAmount')[0].value=f(sm('tblAccountIDet_HZSerAmount'),2);

n('TotalDLFee')[0].value=f(sm('tblAccountIDet_DLFee'),2);

n('TotalDLSerAmount')[0].value=f(sm('tblAccountIDet_DLSerAmount'),2);

n('TotalAmount')[0].value=f(sm('tblAccountIDet_Amount'),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountIDet_PayAmount'); 
 r(0,"RMB")[p].value=f(rn(0,"PayAmount",p)*rn(0,"PayRate",p),2);
r(0,"HZSerAmount")[p].value=f(rn(0,"RMB",p)*rn(0,"HZDays",p)*nn('ProxyRate',0),2);
r(0,"DLSerAmount")[p].value=f(rn(0,"DLFee",p)*rn(0,"DLDays",p)*nn('ProxyRate',0),2);
r(0,"Amount")[p].value=f(Number(rn(0,"HZSerAmount",p))+Number(rn(0,"DLSerAmount",p)),2);
n('TotalRMB')[0].value=f(sm('tblAccountIDet_RMB'),2);
n('TotalHZSerAmount')[0].value=f(sm('tblAccountIDet_HZSerAmount'),2);
n('TotalDLFee')[0].value=f(sm('tblAccountIDet_DLFee'),2);
n('TotalDLSerAmount')[0].value=f(sm('tblAccountIDet_DLSerAmount'),2);
n('TotalAmount')[0].value=f(sm('tblAccountIDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountIDet_PayRate'); 
 r(0,"RMB")[p].value=f(rn(0,"PayAmount",p)*rn(0,"PayRate",p),2);
r(0,"HZSerAmount")[p].value=f(rn(0,"RMB",p)*rn(0,"HZDays",p)*nn('ProxyRate',0),2);
r(0,"DLSerAmount")[p].value=f(rn(0,"DLFee",p)*rn(0,"DLDays",p)*nn('ProxyRate',0),2);
r(0,"Amount")[p].value=f(Number(rn(0,"HZSerAmount",p))+Number(rn(0,"DLSerAmount",p)),2);
...
n('TotalAmount')[0].value=f(sm('tblAccountIDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountIDet_RMB'); 
 r(0,"RMB")[p].value=f(rn(0,"PayAmount",p)*rn(0,"PayRate",p),2);
r(0,"HZSerAmount")[p].value=f(rn(0,"RMB",p)*rn(0,"HZDays",p)*nn('ProxyRate',0),2);
r(0,"DLSerAmount")[p].value=f(rn(0,"DLFee",p)*rn(0,"DLDays",p)*nn('ProxyRate',0),2);
r(0,"Amount")[p].value=f(Number(rn(0,"HZSerAmount",p))+Number(rn(0,"DLSerAmount",p)),2);
...
n('TotalAmount')[0].value=f(sm('tblAccountIDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountIDet_HZDays'); 
 r(0,"RMB")[p].value=f(rn(0,"PayAmount",p)*rn(0,"PayRate",p),2);
r(0,"HZSerAmount")[p].value=f(rn(0,"RMB",p)*rn(0,"HZDays",p)*nn('ProxyRate',0),2);
r(0,"DLSerAmount")[p].value=f(rn(0,"DLFee",p)*rn(0,"DLDays",p)*nn('ProxyRate',0),2);
r(0,"Amount")[p].value=f(Number(rn(0,"HZSerAmount",p))+Number(rn(0,"DLSerAmount",p)),2);
...
n('TotalAmount')[0].value=f(sm('tblAccountIDet_Amount'),2);
 ...
 var z=r(0,'HZSerAmount');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountIDet_HZSerAmount'); 
 r(0,"RMB")[p].value=f(rn(0,"PayAmount",p)*rn(0,"PayRate",p),2);
r(0,"HZSerAmount")[p].value=f(rn(0,"RMB",p)*rn(0,"HZDays",p)*nn('ProxyRate',0),2);
r(0,"DLSerAmount")[p].value=f(rn(0,"DLFee",p)*rn(0,"DLDays",p)*nn('ProxyRate',0),2);
r(0,"Amount")[p].value=f(Number(rn(0,"HZSerAmount",p))+Number(rn(0,"DLSerAmount",p)),2);
...
n('TotalAmount')[0].value=f(sm('tblAccountIDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountIDet_DLFee'); 
 r(0,"RMB")[p].value=f(rn(0,"PayAmount",p)*rn(0,"PayRate",p),2);
r(0,"HZSerAmount")[p].value=f(rn(0,"RMB",p)*rn(0,"HZDays",p)*nn('ProxyRate',0),2);
r(0,"DLSerAmount")[p].value=f(rn(0,"DLFee",p)*rn(0,"DLDays",p)*nn('ProxyRate',0),2);
r(0,"Amount")[p].value=f(Number(rn(0,"HZSerAmount",p))+Number(rn(0,"DLSerAmount",p)),2);
...
n('TotalAmount')[0].value=f(sm('tblAccountIDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountIDet_DLDays'); 
 r(0,"RMB")[p].value=f(rn(0,"PayAmount",p)*rn(0,"PayRate",p),2);
r(0,"HZSerAmount")[p].value=f(rn(0,"RMB",p)*rn(0,"HZDays",p)*nn('ProxyRate',0),2);
r(0,"DLSerAmount")[p].value=f(rn(0,"DLFee",p)*rn(0,"DLDays",p)*nn('ProxyRate',0),2);
r(0,"Amount")[p].value=f(Number(rn(0,"HZSerAmount",p))+Number(rn(0,"DLSerAmount",p)),2);
...
n('TotalAmount')[0].value=f(sm('tblAccountIDet_Amount'),2);
 ...
 var z=r(0,'DLSerAmount');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountIDet_DLSerAmount'); 
 r(0,"RMB")[p].value=f(rn(0,"PayAmount",p)*rn(0,"PayRate",p),2);
r(0,"HZSerAmount")[p].value=f(rn(0,"RMB",p)*rn(0,"HZDays",p)*nn('ProxyRate',0),2);
r(0,"DLSerAmount")[p].value=f(rn(0,"DLFee",p)*rn(0,"DLDays",p)*nn('ProxyRate',0),2);
r(0,"Amount")[p].value=f(Number(rn(0,"HZSerAmount",p))+Number(rn(0,"DLSerAmount",p)),2);
...
n('TotalAmount')[0].value=f(sm('tblAccountIDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountIDet_Amount'); 
 r(0,"RMB")[p].value=f(rn(0,"PayAmount",p)*rn(0,"PayRate",p),2);
r(0,"HZSerAmount")[p].value=f(rn(0,"RMB",p)*rn(0,"HZDays",p)*nn('ProxyRate',0),2);
r(0,"DLSerAmount")[p].value=f(rn(0,"DLFee",p)*rn(0,"DLDays",p)*nn('ProxyRate',0),2);
r(0,"Amount")[p].value=f(Number(rn(0,"HZSerAmount",p))+Number(rn(0,"DLSerAmount",p)),2);
...
childData[childCount]='tblAccountIDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='资金占用费结算单明细';
childCount++;
 var tblAccountIDetData = new gridData();
 gridDatas[gridDataNum] = tblAccountIDetData;
 gridDataNum++;
 var tblAccountIDet=1;
 ...
 var param4="";
 param4=param4+":0客户关联ID@ClientID"; 
 
 addCols(tblAccountIDetData,'tblAccountIDet_SheetID','合同号','130',2,true,'',30,-2,'','',"","","2",'',"2",true);
 putChildValidateItem("tblAccountIDet_SheetID","合同号","any","",true,minValue,30,false,'');
 addCols(tblAccountIDetData,'tblAccountIDet_tblSheet_BillNo','合同号','120',2,true,'',1000000,2,'tblAccountIDet:SheetID:tblAccountIDet_tblSheet_BillNo:'+param4,'',"","","2",'',"2",false); 
 addCols(tblAccountIDetData,'tblAccountIDet_tblSheet_BGDate','报关日期','120',5,true,'',1000000,2,'tblAccountIDet:SheetID:tblAccountIDet_tblSheet_BGDate:'+param4,'',"","","2",'',"2",false); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_PayAmount','付汇金额','120',1,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblAccountIDet_PayAmount","付汇金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_PayRate','汇率','100',1,true,'0',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblAccountIDet_PayRate","汇率","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_PayTime','付汇时间','110',5,true,'',20,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblAccountIDet_PayTime","付汇时间","date","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_RMB','人民币货值','110',1,true,'0',0,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblAccountIDet_RMB","人民币货值","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_HZDays','占用天数','100',0,true,'',0,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblAccountIDet_HZDays","占用天数","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_HZSerAmount','货值服务费','100',1,true,'0',0,0,'','',"","","14",'',"0",false); 
 putChildValidateItem("tblAccountIDet_HZSerAmount","货值服务费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_DLFee','代理费及杂费','100',1,true,'0',0,0,'','',"","","16",'',"0",false); 
 putChildValidateItem("tblAccountIDet_DLFee","代理费及杂费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_DLDays','占用天数','100',0,true,'',0,0,'','',"","","18",'',"0",false); 
 putChildValidateItem("tblAccountIDet_DLDays","占用天数","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_DLSerAmount','代理费杂费服务费','120',1,true,'0',0,0,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblAccountIDet_DLSerAmount","代理费杂费服务费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_Amount','资金占用费金额','120',1,true,'0',0,0,'','',"","","22",'',"0",false); 
 putChildValidateItem("tblAccountIDet_Amount","资金占用费金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_Remark','备注','120',2,true,'',8000,0,'','',"","","24",'',"0",false); 
 putChildValidateItem("tblAccountIDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblAccountIDetData,'tblAccountIDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblAccountI&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblAccountI,tblAccountIDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAccountI&allTableName=tblAccountI,tblAccountIDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAccountI&allTableName=tblAccountIDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAccountI&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAccountI&allTableName=tblAccountIDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=9b802566_1211121523320000141&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=9b802566_1211121523320000141&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=9b802566_1211121523320000141&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=9b802566_1211121523320000141&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblAccountI&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblAccountI&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblAccountI&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblAccountI&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblAccountI&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '资金占用费结算单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblAccountI&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
