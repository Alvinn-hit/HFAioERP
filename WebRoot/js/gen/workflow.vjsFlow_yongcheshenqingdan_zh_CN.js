...
$(document).ready(function(){
 ...
 var imgUrl="if(popMainInput(''))openPopup('Flow_yongcheshenqingdan','"+$(this).attr('value')+"','popup_"+Id+"','"+Id+"','"+isMutiplse+"')";
 ...
 var imgUrl="if(popMainInput(''))openPopup('Flow_yongcheshenqingdan','"+$(this).attr('value')+"','"+popupNames+"','"+Id+"','"+isMutiplse+"')";
...
function checkTable(allHidFields){
 ...
 if(inputType == "integer"){
 if(labelText.val().length>0 && !isInt(labelText.val())){
 alert("车辆牌照号"+"必须为整形数字，请重新输入!");
 labelText.focus();
 return false;
 }
 }else if(inputType=="float"){
 if(labelText.val().length>0 && !isFloat(labelText.val())){
 alert("车辆牌照号"+"必须为小数，请重新输入!");
 labelText.focus();
 return false;
 }
 }
