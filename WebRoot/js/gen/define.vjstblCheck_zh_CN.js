...
{ 
 cFireEvent('tblCheckDet_Qty',''); 
 cFireEvent('tblCheckDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblCheck&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblCheck&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblCheck&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblCheck&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblCheckDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblCheckDet"){
 
 copyFields=n('tblCheckDet_StockCode');
 ...
 var tblCheckDet_tblStock_StockFullName1 = n('tblCheckDet_tblStock_StockFullName1');
 if(typeof(tblCheckDet_tblStock_StockFullName1) != 'undefined' && tblCheckDet_tblStock_StockFullName1.length >index){
 tblCheckDet_tblStock_StockFullName1[index].value = tblCheckDet_tblStock_StockFullName1[index-1].value;
 }
 }
 copyFields=n('tblCheckDet_GoodsCode');
 if((order > 3 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblCheckDet_tblGoods_GoodsNumber = n('tblCheckDet_tblGoods_GoodsNumber');
 if(typeof(tblCheckDet_tblGoods_GoodsNumber) != 'undefined' && tblCheckDet_tblGoods_GoodsNumber.length >index){
 tblCheckDet_tblGoods_GoodsNumber[index].value = tblCheckDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblCheckDet_tblGoods_GoodsFullName = n('tblCheckDet_tblGoods_GoodsFullName');
 if(typeof(tblCheckDet_tblGoods_GoodsFullName) != 'undefined' && tblCheckDet_tblGoods_GoodsFullName.length >index){
 tblCheckDet_tblGoods_GoodsFullName[index].value = tblCheckDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblCheckDet_tblGoods_GoodsSpec = n('tblCheckDet_tblGoods_GoodsSpec');
 if(typeof(tblCheckDet_tblGoods_GoodsSpec) != 'undefined' && tblCheckDet_tblGoods_GoodsSpec.length >index){
 tblCheckDet_tblGoods_GoodsSpec[index].value = tblCheckDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblCheckDet_tblGoods_BaseUnit = n('tblCheckDet_tblGoods_BaseUnit');
 if(typeof(tblCheckDet_tblGoods_BaseUnit) != 'undefined' && tblCheckDet_tblGoods_BaseUnit.length >index){
 tblCheckDet_tblGoods_BaseUnit[index].value = tblCheckDet_tblGoods_BaseUnit[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblCheckDet"){
...
function execStat2(){
 execStatOne('tblCheckDet_Qty',4);
 execStatOne('tblCheckDet_Amount',2);
 execStatOne('tblCheckDet_OQty',4);
 execStatOne('tblCheckDet_OAmt',2);
 execStatOne('tblCheckDet_OCQty',4);
 execStatOne('tblCheckDet_OCAmt',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblCheck\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblCheck\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblCheckDet_Qty',"tblCheckDet"); 
 delFireEventOne(rowId,tableName,'tblCheckDet_Price',"tblCheckDet"); 
 delFireEventOne(rowId,tableName,'tblCheckDet_Amount',"tblCheckDet"); 
 delFireEventOne(rowId,tableName,'tblCheckDet_OQty',"tblCheckDet"); 
 delFireEventOne(rowId,tableName,'tblCheckDet_OAmt',"tblCheckDet"); 
 delFireEventOne(rowId,tableName,'tblCheckDet_OCQty',"tblCheckDet"); 
 delFireEventOne(rowId,tableName,'tblCheckDet_OCAmt',"tblCheckDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'BillAmt',dis:'合计金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblCheckDet_Qty',dis:'盘前数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCheckDet_Price',dis:'结存单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCheckDet_Amount',dis:'结存金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCheckDet_OQty',dis:'盘点后数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCheckDet_OAmt',dis:'盘点后金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCheckDet_OCQty',dis:'盘盈数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCheckDet_OCAmt',dis:'盘盈金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblCheck",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblCheck&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblCheck&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function initSeqQtySet(){
 if(false){
   ...
   if(false){
 var fields=n('tblCheckDet_OCQty'); 
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblCheck&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblCheck&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblCheckDet_Qty'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
 
 r(0,"OPrice")[p].value=f(rn(0,"Price",p),6);

 r(0,"OAmt")[p].value=f(rn(0,"OPrice",p)*rn(0,"OQty",p),2);

 r(0,"OCQty")[p].value=f(rn(0,"OQty",p)-rn(0,"Qty",p),4);

 r(0,"OCAmt")[p].value=f(rn(0,"OAmt",p)-rn(0,"Amount",p),2);

 n('BillAmt')[0].value=f(sm('tblCheckDet_OCAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblCheckDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblCheckDet_Amount'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblCheckDet_OQty'); 
 r(0,"OAmt")[p].value=f(rn(0,"OPrice",p)*rn(0,"OQty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblCheckDet_OPrice'); 
 r(0,"OAmt")[p].value=f(rn(0,"OPrice",p)*rn(0,"OQty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblCheckDet_OAmt'); 
 r(0,"OPrice")[p].value=f(rn(0,"OAmt",p)/rn(0,"OQty",p),6);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblCheckDet_OCQty'); 
 r(0,"OQty")[p].value=f(rn(0,"OCQty",p)+rn(0,"Qty",p),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblCheckDet_OCAmt'); 
 r(0,"OAmt")[p].value=f(rn(0,"OCAmt",p)+rn(0,"Amount",p),2);
...
childData[childCount]='tblCheckDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='盘盈盘亏单明细';
childCount++;
 var tblCheckDetData = new gridData();
 gridDatas[gridDataNum] = tblCheckDetData;
 gridDataNum++;
 var tblCheckDet=1;
 ...
 addCols(tblCheckDetData,'tblCheckDet_StockCode','仓库','80',2,true,'',100,-2,'','',"","copy","0",'',"3",false); 
 putChildValidateItem("tblCheckDet_StockCode","仓库","any","",true,minValue,100,true,''); 
 ...
 param4=param4+":0仓库@tblCheckDet_StockCode";
 
 addCols(tblCheckDetData,'tblCheckDet_GoodsCode','商品名称','150',2,false,'',100,-2,'','',"","triggerOrCopy","3",'',"2",true);
 putChildValidateItem("tblCheckDet_GoodsCode","商品名称","any","",false,minValue,100,false,'');
 addCols(tblCheckDetData,'tblCheckDet_tblGoods_GoodsNumber','品号','200',20,true,'',1000000,2,'tblCheckDet:GoodsCode:tblCheckDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 addCols(tblCheckDetData,'tblCheckDet_tblGoods_GoodsFullName','商品名称','300',20,true,'',1000000,2,'tblCheckDet:GoodsCode:tblCheckDet_tblGoods_GoodsFullName:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 addCols(tblCheckDetData,'tblCheckDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'tblCheckDet:GoodsCode:tblCheckDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 addCols(tblCheckDetData,'tblCheckDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblCheckDet:GoodsCode:tblCheckDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","9",'',"2",false); 
 putChildValidateItem("tblCheckDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_color','色彩','60',2,true,'',100,-2,'','',"","","12",'',"2",false); 
 putChildValidateItem("tblCheckDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","15",'',"2",false); 
 putChildValidateItem("tblCheckDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","18",'',"2",false); 
 putChildValidateItem("tblCheckDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","21",'',"2",false); 
 putChildValidateItem("tblCheckDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","24",'',"0",false); 
 putChildValidateItem("tblCheckDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","27",'',"0",false); 
 putChildValidateItem("tblCheckDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_Seq','序列号','130',2,true,'',5000,-2,'','',"","triggerCopy","28",'',"3",false); 
 putChildValidateItem("tblCheckDet_Seq","序列号","any","GoodsField",true,minValue,5000,false,''); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_Qty','盘前数量','60',1,false,'',0,8,'','',"Qty","","33",'',"8",false); 
 putChildValidateItem("tblCheckDet_Qty","盘前数量","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_Price','结存单价','80',1,false,'',0,8,'','',"priceIdentifier","","36",'',"8",false); 
 putChildValidateItem("tblCheckDet_Price","结存单价","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_Amount','结存金额','100',1,true,'',0,8,'','',"AmountIdentifier","","39",'',"8",false); 
 putChildValidateItem("tblCheckDet_Amount","结存金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_OQty','盘点后数量','70',1,false,'',0,0,'','',"Qty","","42",'',"0",false); 
 putChildValidateItem("tblCheckDet_OQty","盘点后数量","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_OPrice','盘后单价','80',1,true,'',0,-2,'','',"priceIdentifier","","45",'',"8",false); 
 putChildValidateItem("tblCheckDet_OPrice","盘后单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_OAmt','盘点后金额','70',1,false,'',0,8,'','',"AmountIdentifier","","48",'',"0",false); 
 putChildValidateItem("tblCheckDet_OAmt","盘点后金额","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_OCQty','盘盈数量','80',1,true,'',0,0,'','',"Qty","","51",'',"0",false); 
 putChildValidateItem("tblCheckDet_OCQty","盘盈数量","double","SeqQty",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_OCAmt','盘盈金额','70',1,false,'',0,8,'','',"AmountIdentifier","","54",'',"0",false); 
 putChildValidateItem("tblCheckDet_OCAmt","盘盈金额","double","",false,minValue,maxValue,false,'',true); 
 ...
 addCols(tblCheckDetData,'tblCheckDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblCheckDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblCheckDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","108",'',"100",false); 
 putChildValidateItem("tblCheckDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 
 addCols(tblCheckDetData,'tblCheckDet_Remark','备注','130',2,true,'',8000,0,'','',"","","120",'',"0",false); 
 putChildValidateItem("tblCheckDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 addCols(tblCheckDetData,'tblCheckDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblCheck&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblCheck,tblCheckDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblCheck&allTableName=tblCheck,tblCheckDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblCheck&allTableName=tblCheckDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblCheck&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblCheck&allTableName=tblCheckDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblCheck&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblCheck&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblCheck&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblCheck&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblCheck&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '盘盈盘亏单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblCheck&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
