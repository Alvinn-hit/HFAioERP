...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblAccountA&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblAccountA&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblAccountA&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblAccountA&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblAccountADetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblAccountADet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblAccountADet"){
...
function execStat2(){
 execStatOne('tblAccountADet_Quantity',2);
 execStatOne('tblAccountADet_TotalACUSD',2);
 execStatOne('tblAccountADet_TotalACRMB',2);
 execStatOne('tblAccountADet_TotalPayUSD',2);
 execStatOne('tblAccountADet_Rate',8);
 execStatOne('tblAccountADet_RMB',2);
 execStatOne('tblAccountADet_VAT',2);
 execStatOne('tblAccountADet_Tariff',2);
 execStatOne('tblAccountADet_DumpTax',2);
 execStatOne('tblAccountADet_OtherTax',2);
 execStatOne('tblAccountADet_XFTax',2);
 execStatOne('tblAccountADet_DLFee',2);
 execStatOne('tblAccountADet_SJFee',2);
 execStatOne('tblAccountADet_YZFee',2);
 execStatOne('tblAccountADet_Others',2);
 execStatOne('tblAccountADet_RMBTaxAmount',2);
 execStatOne('tblAccountADet_RMBLastTaxAmount',2);
 execStatOne('tblAccountADet_ProxyInvoice',2);
 execStatOne('tblAccountADet_YSK',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=4dfd5379_1208061541362180065&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=4dfd5379_1208061541362180065&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblAccountA\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblAccountA\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblAccountADet_Quantity',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_ACPrice',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_TotalACUSD',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_TotalACRMB',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_PayPrice',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_TotalPayUSD',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_Rate',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_RMB',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_VAT',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_Tariff',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_DumpTax',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_OtherTax',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_XFTax',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_DLFee',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_SJFee',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_YZFee',"tblAccountADet"); 
 delFireEventOne(rowId,tableName,'tblAccountADet_Others',"tblAccountADet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'CustomsRate',dis:'当月海关汇率',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalDLFee',dis:'综合服务费小计',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'YShouKuan',dis:'已收款',digit:2,isMain:true};
 ...
 fieldDigit[fieldDigit.length] = {name:'TotalAmount',dis:'剩余应收款',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalVatInvoice',dis:'增值税(17%)开票金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalProxyInvoice',dis:'增值税(6%)开票金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalVATLastKp',dis:'增值税(17%)可开票金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_Quantity',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_ACPrice',dis:'报关外币货值单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_TotalACUSD',dis:'报关外币货值合计',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_TotalACRMB',dis:'报关货值合计(RMB)',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_PayPrice',dis:'实际外币货值单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_TotalPayUSD',dis:'实际外币货值合计',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_Rate',dis:'汇率',digit:8,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_RMB',dis:'人民币货款(RMB)',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_VAT',dis:'增值税',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_Tariff',dis:'关税',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_DumpTax',dis:'反倾销税',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_OtherTax',dis:'其他税',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_XFTax',dis:'消费税',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_DLFee',dis:'综合服务费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_SJFee',dis:'商检费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_YZFee',dis:'运杂费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_Others',dis:'其他杂费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_RMBTaxPrice',dis:'含税开票单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_RMBTaxAmount',dis:'含税开票金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_RMBLastTaxAmount',dis:'含税可开票金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_ProxyInvoice',dis:'增6%开票金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccountADet_YSK',dis:'应收款',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblAccountA",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAccountA&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAccountA&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"4dfd5379_1208061541362180065",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "ContactMan"){
 var saveField=n("ContactMan"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyEmployeeDet_ContactMan");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("Mobile"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyEmployeeDet_Mobile");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("Telephone"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyEmployeeDet_Telephone");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("ClientEmail"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyEmployeeDet_ClientEmail");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("Tax"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyEmployeeDet_Tax");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("LinkPersonID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyEmployeeDet_Code");
 if(obj != null){
 obj.value = "";
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=4dfd5379_1208061541362180065&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=4dfd5379_1208061541362180065&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=4dfd5379_1208061541362180065&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblAccountA"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblAccountA&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblAccountA&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 for(var j=0;j<r(0,'TotalACUSD').length;j++){
if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[j].value=f(Number((Number(rn(0,"SheetAmount",j))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",j)).toFixed(4)*1)).toFixed(2)*1,2);
}

for(var j=0;j<r(0,'TotalACUSD').length;j++){
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[j].value=f(rn(0,"SheetAmount",j),2);
}

for(var j=0;j<r(0,'RMBTax').length;j++){
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[j].value=f(Number((Number(rn(0,"SheetAmount",j))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",j)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",j)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",j)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
}

for(var j=0;j<r(0,'RMBTax').length;j++){
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[j].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",j)).toFixed(0),2);
}

for(var j=0;j<r(0,'Tariff').length;j++){
r(0,"Tariff")[j].value=f(Number(rn(0,"RMBTax",j)*rn(0,"TariffRate",j)/100).toFixed(2),2);
}

for(var j=0;j<r(0,'XFTax').length;j++){
r(0,"XFTax")[j].value=f(Number((rn(0,"Tariff",j)+rn(0,"RMBTax",j))*(rn(0,"ConRate",j)/100)/(1-(rn(0,"ConRate",j)/100))).toFixed(2),2);
}

for(var j=0;j<r(0,'VAT').length;j++){
r(0,"VAT")[j].value=f(Number((rn(0,"Tariff",j)+rn(0,"XFTax",j)+rn(0,"RMBTax",j)+rn(0,"DumpTax",j)+rn(0,"OtherTax",j))*(rn(0,"VatRate",j)/100)).toFixed(2),2);
}

for(var j=0;j<r(0,'ACPrice').length;j++){
r(0,"ACPrice")[j].value=f(rn(0,"TotalACUSD",j)/rn(0,"Quantity",j),4);
}

for(var j=0;j<r(0,'VAT').length;j++){
if(rn(0,"TotalACUSD",j)==0)r(0,"VAT")[j].value=f(0,2);
}

for(var j=0;j<r(0,'Tariff').length;j++){
if(rn(0,"TotalACUSD",j)==0)r(0,"Tariff")[j].value=f(0,2);
}

for(var j=0;j<r(0,'XFTax').length;j++){
if(rn(0,"TotalACUSD",j)==0)r(0,"XFTax")[j].value=f(0,2);
}

for(var j=0;j<r(0,'DumpTax').length;j++){
if(rn(0,"TotalACUSD",j)==0)r(0,"DumpTax")[j].value=f(0,2);
}

for(var j=0;j<r(0,'OtherTax').length;j++){
if(rn(0,"TotalACUSD",j)==0)r(0,"OtherTax")[j].value=f(0,2);
}

for(var j=0;j<r(0,'SJFee').length;j++){
if(rn(0,"TotalACUSD",j)==0)r(0,"SJFee")[j].value=f(0,2);
}

for(var j=0;j<r(0,'YZFee').length;j++){
if(rn(0,"TotalACUSD",j)==0)r(0,"YZFee")[j].value=f(0,2);
}

for(var j=0;j<r(0,'Others').length;j++){
if(rn(0,"TotalACUSD",j)==0)r(0,"Others")[j].value=f(0,2);
}

for(var j=0;j<r(0,'TotalACRMB').length;j++){
r(0,"TotalACRMB")[j].value=f(rn(0,"TotalACUSD",j)*rn(0,"Rate",j),2);
}

for(var j=0;j<r(0,'PayPrice').length;j++){
r(0,"PayPrice")[j].value=f(rn(0,"TotalPayUSD",j)/rn(0,"Quantity",j),4);
}

for(var j=0;j<r(0,'RMB').length;j++){
r(0,"RMB")[j].value=f(rn(0,"TotalPayUSD",j)*rn(0,"Rate",j),2);
}

for(var j=0;j<r(0,'DLFee').length;j++){
if(n('ProxyMethod')[0].value==0)r(0,"DLFee")[j].value=f((Number(rn(0,"TotalACRMB",j))+Number(rn(0,"Tariff",j))+Number(rn(0,"XFTax",j))+Number(rn(0,"VAT",j))+Number(rn(0,"DumpTax",j))+Number(rn(0,"OtherTax",j)))*nn('ProxyRate',0),2);
}

for(var j=0;j<r(0,'DLFee').length;j++){
if(n('ProxyMethod')[0].value==1)r(0,"DLFee")[j].value=f((Number(rn(0,"RMB",j))+Number(rn(0,"Tariff",j))+Number(rn(0,"XFTax",j))+Number(rn(0,"VAT",j))+Number(rn(0,"DumpTax",j))+Number(rn(0,"OtherTax",j)))*nn('ProxyRate',0),2);
}

for(var j=0;j<r(0,'DLFee').length;j++){
if(n('ProxyMethod')[0].value==2)r(0,"DLFee")[j].value=f(rn(0,"TotalACRMB",j)*nn('ProxyRate',0),2);
}

for(var j=0;j<r(0,'DLFee').length;j++){
if(n('ProxyMethod')[0].value==3)r(0,"DLFee")[j].value=f(rn(0,"RMB",j)*nn('ProxyRate',0),2);
}

n('TotalRmb')[0].value=f(sm('tblAccountADet_RMB'),2);

n('TotalVat')[0].value=f(sm('tblAccountADet_VAT'),2);

n('TotalTariff')[0].value=f(sm('tblAccountADet_Tariff'),2);

n('TotalXFTax')[0].value=f(sm('tblAccountADet_XFTax'),2);

n('TotalDumpTax')[0].value=f(sm('tblAccountADet_DumpTax'),2);

n('TotalOtherTax')[0].value=f(sm('tblAccountADet_OtherTax'),2);

n('TotalDLFee')[0].value=f(sm('tblAccountADet_DLFee'),2);

n('TotalSJFee')[0].value=f(sm('tblAccountADet_SJFee'),2);

n('TotalYZFee')[0].value=f(sm('tblAccountADet_YZFee'),2);

n('TotalOther')[0].value=f(sm('tblAccountADet_Others'),2);

n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_Quantity'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
r(0,"Tariff")[p].value=f(Number(rn(0,"RMBTax",p)*rn(0,"TariffRate",p)/100).toFixed(2),2);
r(0,"XFTax")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"RMBTax",p))*(rn(0,"ConRate",p)/100)/(1-(rn(0,"ConRate",p)/100))).toFixed(2),2);
r(0,"VAT")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"XFTax",p)+rn(0,"RMBTax",p)+rn(0,"DumpTax",p)+rn(0,"OtherTax",p))*(rn(0,"VatRate",p)/100)).toFixed(2),2);
r(0,"ACPrice")[p].value=f(rn(0,"TotalACUSD",p)/rn(0,"Quantity",p),4);
if(rn(0,"TotalACUSD",p)==0)r(0,"VAT")[p].value=f(0,2);
if(rn(0,"TotalACUSD",p)==0)r(0,"Tariff")[p].value=f(0,2);
if(rn(0,"TotalACUSD",p)==0)r(0,"XFTax")[p].value=f(0,2);
if(rn(0,"TotalACUSD",p)==0)r(0,"DumpTax")[p].value=f(0,2);
if(rn(0,"TotalACUSD",p)==0)r(0,"OtherTax")[p].value=f(0,2);
if(rn(0,"TotalACUSD",p)==0)r(0,"SJFee")[p].value=f(0,2);
if(rn(0,"TotalACUSD",p)==0)r(0,"YZFee")[p].value=f(0,2);
if(rn(0,"TotalACUSD",p)==0)r(0,"Others")[p].value=f(0,2);
r(0,"TotalACRMB")[p].value=f(rn(0,"TotalACUSD",p)*rn(0,"Rate",p),2);
r(0,"PayPrice")[p].value=f(rn(0,"TotalPayUSD",p)/rn(0,"Quantity",p),4);
r(0,"RMB")[p].value=f(rn(0,"TotalPayUSD",p)*rn(0,"Rate",p),2);
if(n('ProxyMethod')[0].value==0)r(0,"DLFee")[p].value=f((Number(rn(0,"TotalACRMB",p))+Number(rn(0,"Tariff",p))+Number(rn(0,"XFTax",p))+Number(rn(0,"VAT",p))+Number(rn(0,"DumpTax",p))+Number(rn(0,"OtherTax",p)))*nn('ProxyRate',0),2);
if(n('ProxyMethod')[0].value==1)r(0,"DLFee")[p].value=f((Number(rn(0,"RMB",p))+Number(rn(0,"Tariff",p))+Number(rn(0,"XFTax",p))+Number(rn(0,"VAT",p))+Number(rn(0,"DumpTax",p))+Number(rn(0,"OtherTax",p)))*nn('ProxyRate',0),2);
if(n('ProxyMethod')[0].value==2)r(0,"DLFee")[p].value=f(rn(0,"TotalACRMB",p)*nn('ProxyRate',0),2);
if(n('ProxyMethod')[0].value==3)r(0,"DLFee")[p].value=f(rn(0,"RMB",p)*nn('ProxyRate',0),2);
n('TotalRmb')[0].value=f(sm('tblAccountADet_RMB'),2);
n('TotalVat')[0].value=f(sm('tblAccountADet_VAT'),2);
n('TotalTariff')[0].value=f(sm('tblAccountADet_Tariff'),2);
n('TotalXFTax')[0].value=f(sm('tblAccountADet_XFTax'),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_ACPrice'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 var z=r(0,'TotalACUSD');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_TotalACUSD'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 var z=r(0,'TotalACRMB');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_TotalACRMB'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_PayPrice'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 var z=r(0,'TotalPayUSD');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_TotalPayUSD'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_Rate'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_RMB'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_VAT'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_Tariff'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_DumpTax'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_OtherTax'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_XFTax'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_DLFee'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_SJFee'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_YZFee'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_Others'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 var z=r(0,'TariffRate');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_TariffRate'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_ConRate'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
n('TotalAmount')[0].value=f(Number(nn('TotalRmb',0))+Number(nn('TotalVat',0))+Number(nn('TotalTariff',0))+Number(nn('TotalXFTax',0))+Number(nn('TotalDumpTax',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDLFee',0))+Number(nn('TotalSJFee',0))+Number(nn('TotalYZFee',0))+Number(nn('TotalOther',0))-Number(nn('YShouKuan',0)),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccountADet_RMBTax'); 
 if(n('SheetDelivery')[0].value=="FOB")r(0,"TotalACUSD")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number((Number(nn('SheetFare',0))*1+Number(nn('SheetInsurance',0))*1+Number(nn('SheetOtherFare',0))*1)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)).toFixed(2)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"TotalACUSD")[p].value=f(rn(0,"SheetAmount",p),2);
if(n('SheetDelivery')[0].value=="FOB")r(0,"RMBTax")[p].value=f(Number((Number(rn(0,"SheetAmount",p))*1+Number(nn('SheetOtherFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*1)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetFare',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1+Number(Number(nn('SheetInsurance',0)/nn('SheetTotalAmount',0)*rn(0,"SheetAmount",p)).toFixed(4)*nn('CustomsRate',0)).toFixed(0)*1,2);
if(n('SheetDelivery')[0].value!="FOB")r(0,"RMBTax")[p].value=f(Number(nn('CustomsRate',0)*rn(0,"SheetAmount",p)).toFixed(0),2);
...
childData[childCount]='tblAccountADet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='结算单A明细';
childCount++;
 var tblAccountADetData = new gridData();
 gridDatas[gridDataNum] = tblAccountADetData;
 gridDataNum++;
 var tblAccountADet=1;
 ...
 addCols(tblAccountADetData,'tblAccountADet_SupplyID','供应商','170',2,true,'',50,-2,'','',"","","2",'',"3",false); 
 putChildValidateItem("tblAccountADet_SupplyID","供应商","any","",true,minValue,50,false,''); 
 ...
 addCols(tblAccountADetData,'tblAccountADet_SupplyNameEn','供应商名称(英)','150',2,true,'',400,2,'tblAccountADet:SupplyNameEn:tblAccountADet_SupplyNameEn:'+param4,"","","","3",'',"2",true);
 putChildValidateItem("tblAccountADet_SupplyNameEn","供应商名称(英)","any","",true,minValue,400,false,'',true);
 ...
 var param4="";
 param4=param4+":1合同编号@SheetID"; 
 
 addCols(tblAccountADetData,'tblAccountADet_Model','型号','130',2,true,'',50,2,'tblAccountADet:Model:tblAccountADet_Model:'+param4,"","","","4",'',"2",true);
 putChildValidateItem("tblAccountADet_Model","型号","any","",true,minValue,50,false,'',true);
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_Brand','品牌','100',2,true,'',50,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblAccountADet_Brand","品牌","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_GoodsName','品名','100',2,true,'',50,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblAccountADet_GoodsName","品名","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_Quantity','数量','50',1,true,'',0,0,'','',"Qty","","10",'',"0",false); 
 putChildValidateItem("tblAccountADet_Quantity","数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_Currency','外币币种','80',2,true,'',10,1,';RMB:人民币;ATS:ATS;AUD:AUD;CAD:CAD;CHF:CHF;CNY:CNY;DEM:DEM;DKK:DKK;ESP:ESP;EUR:EUR;FIM:FIM;GBP:GBP;HKD:HKD;ITL:ITL;JPY:JPY;KRW:KRW;MOP:MOP;FRF:FRF;NOK:NOK;NTD:NTD;NZD:NZD;PHP:PHP;SEK:SEK;SGD:SGD;THB:THB;USD:USD','',"","","11",'',"1",false);
 putChildValidateItem("tblAccountADet_Currency","外币币种","any","",true,minValue,10,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_ACPrice','报关外币货值单价','120',1,true,'',0,8,'','',"Price","","12",'',"8",false); 
 putChildValidateItem("tblAccountADet_ACPrice","报关外币货值单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_TotalACUSD','报关外币货值合计','120',1,true,'',0,0,'','',"Amount","","14",'',"0",false); 
 putChildValidateItem("tblAccountADet_TotalACUSD","报关外币货值合计","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_TotalACRMB','报关货值合计(RMB)','135',1,true,'',0,0,'','',"Amount","","16",'',"0",false); 
 putChildValidateItem("tblAccountADet_TotalACRMB","报关货值合计(RMB)","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_PayPrice','实际外币货值单价','120',1,true,'',0,8,'','',"Price","","18",'',"8",false); 
 putChildValidateItem("tblAccountADet_PayPrice","实际外币货值单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_TotalPayUSD','实际外币货值合计','120',1,true,'',0,0,'','',"Amount","","20",'',"0",false); 
 putChildValidateItem("tblAccountADet_TotalPayUSD","实际外币货值合计","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_Rate','汇率','60',1,true,'0',0,0,'','',"","","22",'',"0",false); 
 putChildValidateItem("tblAccountADet_Rate","汇率","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_RMB','人民币货款(RMB)','100',1,true,'',0,8,'','',"Amount","","24",'',"8",false); 
 putChildValidateItem("tblAccountADet_RMB","人民币货款(RMB)","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_VAT','增值税','80',1,true,'',0,0,'','',"Amount","","26",'',"0",false); 
 putChildValidateItem("tblAccountADet_VAT","增值税","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_Tariff','关税','80',1,true,'',0,0,'','',"Amount","","28",'',"0",false); 
 putChildValidateItem("tblAccountADet_Tariff","关税","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_DumpTax','反倾销税','80',1,true,'',0,0,'','',"Amount","","30",'',"0",false); 
 putChildValidateItem("tblAccountADet_DumpTax","反倾销税","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_OtherTax','其他税','80',1,true,'',0,0,'','',"Amount","","30",'',"0",false); 
 putChildValidateItem("tblAccountADet_OtherTax","其他税","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_XFTax','消费税','80',1,true,'',0,0,'','',"","","30",'',"0",false); 
 putChildValidateItem("tblAccountADet_XFTax","消费税","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_DLFee','综合服务费','80',1,true,'',0,0,'','',"","","31",'',"0",false); 
 putChildValidateItem("tblAccountADet_DLFee","综合服务费","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_SJFee','商检费','80',1,true,'',0,0,'','',"","","32",'',"0",false); 
 putChildValidateItem("tblAccountADet_SJFee","商检费","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_YZFee','运杂费','80',1,true,'',0,0,'','',"","","33",'',"0",false); 
 putChildValidateItem("tblAccountADet_YZFee","运杂费","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_Others','其他杂费','80',1,true,'',0,0,'','',"Amount","","40",'',"0",false); 
 putChildValidateItem("tblAccountADet_Others","其他杂费","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_RMBTaxPrice','含税开票单价','100',1,true,'',0,8,'','',"","","42",'',"8",false); 
 putChildValidateItem("tblAccountADet_RMBTaxPrice","含税开票单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_RMBTaxAmount','含税开票金额','100',1,true,'',0,8,'','',"Amount","","44",'',"8",false); 
 putChildValidateItem("tblAccountADet_RMBTaxAmount","含税开票金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_RMBLastTaxAmount','含税可开票金额','100',1,true,'',0,8,'','',"Amount","","45",'',"8",false); 
 putChildValidateItem("tblAccountADet_RMBLastTaxAmount","含税可开票金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_ProxyInvoice','增6%开票金额','100',1,true,'',0,8,'','',"","","46",'',"8",false); 
 putChildValidateItem("tblAccountADet_ProxyInvoice","增6%开票金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_YSK','应收款','100',1,true,'',0,8,'','',"","","48",'',"8",false); 
 putChildValidateItem("tblAccountADet_YSK","应收款","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_SourceID','源ID','0',2,true,'',50,-2,'','',"","","50",'',"3",false); 
 putChildValidateItem("tblAccountADet_SourceID","源ID","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_Unit','单位','224',2,true,'',50,0,'','',"","","52",'',"0",false); 
 putChildValidateItem("tblAccountADet_Unit","单位","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_Code','明细关联ID','224',2,true,'',50,-2,'','',"","","60",'',"3",false); 
 putChildValidateItem("tblAccountADet_Code","明细关联ID","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_TariffRate','关税税率','224',1,true,'',0,-2,'','',"","","65",'',"3",false); 
 putChildValidateItem("tblAccountADet_TariffRate","关税税率","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_ConRate','消费税率','224',1,true,'',0,-2,'','',"","","66",'',"3",false); 
 putChildValidateItem("tblAccountADet_ConRate","消费税率","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_VatRate','增值税率','224',1,true,'',0,-2,'','',"","","67",'',"3",false); 
 putChildValidateItem("tblAccountADet_VatRate","增值税率","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_RMBTax','完税价格','224',1,true,'',0,-2,'','',"","","68",'',"3",false); 
 putChildValidateItem("tblAccountADet_RMBTax","完税价格","double","",true,minValue,maxValue,false,''); 
 
 
 
 
 addCols(tblAccountADetData,'tblAccountADet_SheetAmount','进口制单金额','224',1,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblAccountADet_SheetAmount","进口制单金额","double","",true,minValue,maxValue,true,'0'); 
 
 
 addCols(tblAccountADetData,'tblAccountADet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblAccountA&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblAccountA,tblAccountADet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAccountA&allTableName=tblAccountA,tblAccountADet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAccountA&allTableName=tblAccountADet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAccountA&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAccountA&allTableName=tblAccountADet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=4dfd5379_1208061541362180065&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=4dfd5379_1208061541362180065&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=4dfd5379_1208061541362180065&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=4dfd5379_1208061541362180065&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblAccountA&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblAccountA&designId=21786bdc_1410140903190461207&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblAccountA&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblAccountA&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblAccountA&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '结算单A';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblAccountA&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
