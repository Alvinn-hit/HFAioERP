...
{ 
 cFireEvent('DXOutStockDet_Qty',''); 
 cFireEvent('DXOutStockDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=DXOutStock&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=DXOutStock&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=DXOutStock&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=DXOutStock&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "DXOutStockDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 ...
 var z=n('DXOutStockDet_DXOrderID');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('DXOutStockDet_GoodsCode');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="DXOutStockDet"){
 
 copyFields=n('DXOutStockDet_StockCode');
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="DXOutStockDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="DXOutStockDet"; 
 var goodsCodeLogicValidate_DXOutStockDet='@GN:Seq:Qty:tblSalesOutStockDet_GoodsSeqSet:tblSalesOutStockDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'DXOutStockDet',value:goodsCodeLogicValidate_DXOutStockDet};
...
function execStat2(){
 execStatOne('DXOutStockDet_Qty',2);
 execStatOne('DXOutStockDet_Amount',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"DXOutStock\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"DXOutStock\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'DXOutStockDet_Qty',"DXOutStockDet"); 
 delFireEventOne(rowId,tableName,'DXOutStockDet_Price',"DXOutStockDet"); 
 delFireEventOne(rowId,tableName,'DXOutStockDet_Amount',"DXOutStockDet"); 
 }
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'DXOutStockDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'DXOutStockDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'DXOutStockDet_Amount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=DXOutStock",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=DXOutStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=DXOutStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('DXOutStockDet_Qty'); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "DXOutStock"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=DXOutStock&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=DXOutStock&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
n('TotalAmount')[0].value=f(sm('DXOutStockDet_DisBackAmt'),2);

n('TotalTaxAmount')[0].value=f(sm('DXOutStockDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('DXOutStockDet_CoTaxAmt'),2);

for(var j=0;j<r(0,'DisAmount').length;j++){
r(0,"DisAmount")[j].value=f(rn(0,"Amount",j)-rn(0,"DisBackAmt",j),2);
}

n('DisAmount')[0].value=f(sm('DXOutStockDet_DisAmount'),2);
 ...
 n('CurTotalAmount')[0].value=f(sm('DXOutStockDet_CurAmount')-nn('CurDiscountAmount',0),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('CurTotalAmount')[0].value=f(sm('DXOutStockDet_CurAmount')-nn('CurDiscountAmount',0)+nn('CurProceDureExp',0),2);
...
n('CurTotalAmount')[0].value=f(sm('DXOutStockDet_CurAmount')+nn('CurProceDureExp',0)-nn('CurDiscountAmount',0),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_Qty'); 
...
r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
n('DisAmount')[0].value=f(sm('DXOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_Amount'); 
...
n('DisAmount')[0].value=f(sm('DXOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,4);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('DXOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_DisPrice'); 
...
n('DisAmount')[0].value=f(sm('DXOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_DisBackAmt'); 
...
n('DisAmount')[0].value=f(sm('DXOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_TaxPrice'); 
...
n('DisAmount')[0].value=f(sm('DXOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_TaxAmount'); 
...
n('DisAmount')[0].value=f(sm('DXOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_CurPrice'); 
...
n('TotalAmount')[0].value=f(sm('DXOutStockDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('DXOutStockDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(sm('DXOutStockDet_CurAmount'),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
n('TotalCoTaxAmt')[0].value=f(sm('DXOutStockDet_CoTaxAmt'),2);
n('DisAmount')[0].value=f(sm('DXOutStockDet_DisAmount'),2);
n('CurTotalAmount')[0].value=f(sm('DXOutStockDet_CurAmount')-nn('CurDiscountAmount',0)+nn('CurProceDureExp',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_CurAmount'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),2);
...
n('CurTotalAmount')[0].value=f(sm('DXOutStockDet_CurAmount')-nn('CurDiscountAmount',0)+nn('CurProceDureExp',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_CostAmount'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_CostPrice'); 
 r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_PresentSampleType'); 
...
n('DisAmount')[0].value=f(sm('DXOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOutStockDet_SecQty'); 
...
childData[childCount]='DXOutStockDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='代销配货单明细';
childCount++;
 var DXOutStockDetData = new gridData();
 gridDatas[gridDataNum] = DXOutStockDetData;
 gridDataNum++;
 var DXOutStockDet=1;
 ...
 addCols(DXOutStockDetData,'DXOutStockDet_Seq','序列号','130',2,true,'',5000,-2,'','',"","triggerCopy","0",'',"2",false); 
 putChildValidateItem("DXOutStockDet_Seq","序列号","any","GoodsField",true,minValue,5000,false,''); 
 
 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_StockCode','仓库','80',2,true,'',100,-2,'','',"","copy","1",'',"3",false); 
 putChildValidateItem("DXOutStockDet_StockCode","仓库","any","",true,minValue,100,true,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_DXOrderID','订单编号','80',2,true,'',32,-2,'','',"","triggerCopy","2",'',"2",false); 
 putChildValidateItem("DXOutStockDet_DXOrderID","订单编号","any","ManyBillQuote",true,minValue,32,false,''); 
 ...
 param4=param4+":0仓库@DXOutStockDet_StockCode";
 param4=param4+":0批号@DXOutStockDet_BatchNo";
 param4=param4+":0厚度@DXOutStockDet_Inch";
 param4=param4+":0颜色@DXOutStockDet_Hue";
 param4=param4+":0尺码@DXOutStockDet_yearNO";
 param4=param4+":0生产日期@DXOutStockDet_ProDate";
 param4=param4+":0保质期限@DXOutStockDet_Availably";
 param4=param4+":1客户@CompanyCode"; 
 
 addCols(DXOutStockDetData,'DXOutStockDet_GoodsCode','商品名称','100',2,false,'',100,-2,'','',"","triggerCopy","3",'',"2",true);
 putChildValidateItem("DXOutStockDet_GoodsCode","商品名称","any","",false,minValue,100,false,'');
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'DXOutStockDet:GoodsCode:DXOutStockDet_tblGoods_GoodsNumber:'+param4,'',"","triggerCopy","3",'',"2",false); 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_tblGoods_GoodsFullName','商品名称','150',18,true,'',1000000,2,'DXOutStockDet:GoodsCode:DXOutStockDet_tblGoods_GoodsFullName:'+param4,'',"","triggerCopy","3",'',"2",false); 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'DXOutStockDet:GoodsCode:DXOutStockDet_tblGoods_GoodsSpec:'+param4,'',"","triggerCopy","3",'',"2",false); 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'DXOutStockDet:GoodsCode:DXOutStockDet_tblGoods_BaseUnit:'+param4,'',"","triggerCopy","3",'',"2",false); 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_tblStocks_lastQty','实际库存','90',1,true,'',1000000,2,'DXOutStockDet:GoodsCode:DXOutStockDet_tblStocks_lastQty:'+param4,'',"","triggerCopy","3",'',"2",false); 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_ViewVirtualStock_UseQty','虚拟库存','90',1,true,'',1000000,2,'DXOutStockDet:GoodsCode:DXOutStockDet_ViewVirtualStock_UseQty:'+param4,'',"","triggerCopy","3",'',"2",false); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","4",'',"2",false); 
 putChildValidateItem("DXOutStockDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","5",'',"2",false); 
 putChildValidateItem("DXOutStockDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","6",'',"2",false); 
 putChildValidateItem("DXOutStockDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"2",false); 
 putChildValidateItem("DXOutStockDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","8",'',"2",false); 
 putChildValidateItem("DXOutStockDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","9",'',"2",false); 
 putChildValidateItem("DXOutStockDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_Qty','数量','60',1,false,'',0,0,'','',"Qty","","10",'',"0",false); 
 putChildValidateItem("DXOutStockDet_Qty","数量","double","SeqQty",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_Price','单价','80',1,true,'',0,0,'','',"SPriceIdentifier","","11",'',"0",false); 
 putChildValidateItem("DXOutStockDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_Amount','金额','100',1,true,'',0,0,'','',"SAmountIdentifier","","12",'',"0",false); 
 putChildValidateItem("DXOutStockDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(DXOutStockDetData,'DXOutStockDet_Discount','折扣率%','100',1,true,'100',0,-2,'','',"Price","","18",'',"0",false); 
 putChildValidateItem("DXOutStockDet_Discount","折扣率%","double","BillDiscount",true,minValue,maxValue,true,'100'); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_DisPrice','折后单价','100',1,true,'',0,-2,'','',"SPriceIdentifier","","19",'',"0",false); 
 putChildValidateItem("DXOutStockDet_DisPrice","折后单价","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_DisBackAmt','折后金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","20",'',"0",false); 
 putChildValidateItem("DXOutStockDet_DisBackAmt","折后金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_DisAmount','折让金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","21",'',"8",false); 
 putChildValidateItem("DXOutStockDet_DisAmount","折让金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_TaxPrice','含税单价','100',1,true,'',0,-2,'','',"SPriceIdentifier","","22",'',"0",false); 
 putChildValidateItem("DXOutStockDet_TaxPrice","含税单价","double","BillTax",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_TaxAmount','含税金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","23",'',"0",false); 
 putChildValidateItem("DXOutStockDet_TaxAmount","含税金额","double","BillTax",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_CoTaxAmt','税额','100',1,true,'0',0,-2,'','',"SAmountIdentifier","","24",'',"8",false); 
 putChildValidateItem("DXOutStockDet_CoTaxAmt","税额","double","BillTax",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_CurPrice','外币单价','100',1,true,'',0,-2,'','',"SPriceIdentifier","","25",'',"0",false); 
 putChildValidateItem("DXOutStockDet_CurPrice","外币单价","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_CurAmount','外币金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","26",'',"0",false); 
 putChildValidateItem("DXOutStockDet_CurAmount","外币金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_ViewTotalQty','当前库存','80',1,true,'',0,-2,'','',"AmountIdentifier","","29",'',"6",false); 
 putChildValidateItem("DXOutStockDet_ViewTotalQty","当前库存","double","",true,minValue,maxValue,false,''); 
 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_lastQty','实际库存','80',1,true,'',0,-2,'','',"Qty","","31",'',"0",false); 
 putChildValidateItem("DXOutStockDet_lastQty","实际库存","double","",true,minValue,maxValue,false,''); 
 
 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_CostAmount','成本总额','60',1,true,'',0,-2,'','',"AmountIdentifier","","34",'',"3",false); 
 putChildValidateItem("DXOutStockDet_CostAmount","成本总额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_CostPrice','成本单价','60',1,true,'',0,-2,'','',"priceIdentifier","","35",'',"3",false); 
 putChildValidateItem("DXOutStockDet_CostPrice","成本单价","double","",true,minValue,maxValue,false,''); 
 ...
 addCols(DXOutStockDetData,'DXOutStockDet_SourceID','源ID','0',2,true,'',60,-2,'','',"","","43",'',"100",false); 
 putChildValidateItem("DXOutStockDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 ...
 addCols(DXOutStockDetData,'DXOutStockDet_NoInvoiceQty','未开票数','224',1,true,'0',0,-2,'','',"Qty","","48",'',"0",false); 
 putChildValidateItem("DXOutStockDet_NoInvoiceQty","未开票数","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_PresentSampleType','赠(样)品','80',2,true,'',10,-2,'','',"","","49",'',"1",false); 
 putChildValidateItem("DXOutStockDet_PresentSampleType","赠(样)品","any","BillPresentSampleType",true,minValue,10,false,''); 
 ...
 addCols(DXOutStockDetData,'DXOutStockDet_BackPayQty','退货付款数','224',1,true,'',0,-2,'','',"Qty","","54",'',"3",false); 
 putChildValidateItem("DXOutStockDet_BackPayQty","退货付款数","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","55",'',"3",false); 
 putChildValidateItem("DXOutStockDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_PayQty','付款数量','224',1,true,'',0,-2,'','',"Qty","","56",'',"3",false); 
 putChildValidateItem("DXOutStockDet_PayQty","付款数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_NotPayQty','未付款数量','224',1,true,'',0,-2,'','',"Qty","","57",'',"3",false); 
 putChildValidateItem("DXOutStockDet_NotPayQty","未付款数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","58",'',"3",false); 
 putChildValidateItem("DXOutStockDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_AfterChangeAmt','調價后金額','80',1,true,'',0,-2,'','',"SAmountIdentifier","","59",'',"0",false); 
 putChildValidateItem("DXOutStockDet_AfterChangeAmt","調價后金額","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_Remark','备注','100',2,true,'',8000,0,'','',"","","60",'',"0",false); 
 putChildValidateItem("DXOutStockDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(DXOutStockDetData,'DXOutStockDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=DXOutStock&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=null&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "DXOutStock,DXOutStockDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=DXOutStock&allTableName=DXOutStock,DXOutStockDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=DXOutStock&allTableName=DXOutStockDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=DXOutStock&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=DXOutStock&allTableName=DXOutStockDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=DXOutStock&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=DXOutStock&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=DXOutStock&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=DXOutStock&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=DXOutStock&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '代销配货单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=DXOutStock&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
