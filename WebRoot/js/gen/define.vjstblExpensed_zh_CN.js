...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblExpensed&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblExpensed&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblExpensed&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblExpensed&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblExpenseDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblExpenseDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblExpenseDet"){
...
function execStat2(){
 execStatOne('tblExpenseDet_Amt',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblExpensed\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblExpensed\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblExpenseDet_Amt',"tblExpenseDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'BillAmt',dis:'合计金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblExpenseDet_Amt',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblExpensed",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblExpensed&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblExpensed&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblExpensed"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblExpensed&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblExpensed&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 for(var j=0;j<r(0,'Amt').length;j++){
r(0,"Amt")[j].value=f(rn(0,"FcAmt",j)*nn('CurrencyRate',0),2);
}

n('BillAmt')[0].value=f(sm('tblExpenseDet_Amt'),2);

n('FcBillAmt')[0].value=f(sm('tblExpenseDet_FcAmt'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 for(var j=0;j<r(0,'FcAmt').length;j++){
r(0,"FcAmt")[j].value=f(rn(0,"Amt",j)/nn('CurrencyRate',0),2);
}
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblExpenseDet_Amt'); 
 r(0,"FcAmt")[p].value=f(rn(0,"Amt",p)/nn('CurrencyRate',0),2);
n('FcBillAmt')[0].value=f(sm('tblExpenseDet_FcAmt'),2);
n('BillAmt')[0].value=f(sm('tblExpenseDet_Amt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblExpenseDet_FcAmt'); 
 r(0,"Amt")[p].value=f(rn(0,"FcAmt",p)*nn('CurrencyRate',0),2);
n('BillAmt')[0].value=f(sm('tblExpenseDet_Amt'),2);
n('FcBillAmt')[0].value=f(sm('tblExpenseDet_FcAmt'),2);
...
childData[childCount]='tblExpenseDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='费用支出单明细';
childCount++;
 var tblExpenseDetData = new gridData();
 gridDatas[gridDataNum] = tblExpenseDetData;
 gridDataNum++;
 var tblExpenseDet=1;
 ...
 addCols(tblExpenseDetData,'tblExpenseDet_Remark','摘要','200',2,true,'',8000,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblExpenseDet_Remark","摘要","any","",true,minValue,8000,false,'',true); 
 ...
 addCols(tblExpenseDetData,'tblExpenseDet_SubCode','支出类型','200',2,true,'',100,-2,'','',"","","2",'',"2",true);
 putChildValidateItem("tblExpenseDet_SubCode","支出类型","any","",true,minValue,100,false,'');
 
 
 addCols(tblExpenseDetData,'tblExpenseDet_tblBillAccSetup_SubType','支出类型','200',2,true,'',1000000,2,'tblExpenseDet:SubCode:tblExpenseDet_tblBillAccSetup_SubType:'+param4,'',"","","2",'',"2",false); 
 
 
 
 addCols(tblExpenseDetData,'tblExpenseDet_Amt','金额','100',1,false,'0',0,0,'','',"Amount","","3",'',"0",false); 
 putChildValidateItem("tblExpenseDet_Amt","金额","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblExpenseDetData,'tblExpenseDet_CurrencyTypeID','币种','80',2,true,'',60,-2,'','',"","","4",'',"100",false); 
 putChildValidateItem("tblExpenseDet_CurrencyTypeID","币种","any","currency",true,minValue,60,false,''); 
 
 
 
 addCols(tblExpenseDetData,'tblExpenseDet_CurrencyRate','汇率','80',1,true,'1',0,-2,'','',"Price","","5",'',"100",false); 
 putChildValidateItem("tblExpenseDet_CurrencyRate","汇率","double","currency",true,minValue,maxValue,true,'1'); 
 
 
 
 addCols(tblExpenseDetData,'tblExpenseDet_CompanyCode','往来单位','150',2,true,'',100,-2,'','',"CompanyCodeIdentifier","","6",'',"3",false); 
 putChildValidateItem("tblExpenseDet_CompanyCode","往来单位","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblExpenseDetData,'tblExpenseDet_FcAmt','外币金额','80',1,true,'0',0,-2,'','',"Amount","","6",'',"0",false); 
 putChildValidateItem("tblExpenseDet_FcAmt","外币金额","double","currency",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblExpenseDetData,'tblExpenseDet_DepartmentCode','部门','60',2,true,'Sess_DepartmentCode',100,-2,'','',"","","10",'',"2",false); 
 putChildValidateItem("tblExpenseDet_DepartmentCode","部门","any","",true,minValue,100,true,'Sess_DepartmentCode'); 
 ...
 addCols(tblExpenseDetData,'tblExpenseDet_EmployeeID','经手人','60',2,false,'Sess_UserId',50,-2,'','',"","","11",'',"2",true);
 putChildValidateItem("tblExpenseDet_EmployeeID","经手人","any","",false,minValue,50,true,'Sess_UserId');
 
 
 addCols(tblExpenseDetData,'tblExpenseDet_tblEmployee_EmpFullName','经手人','50',2,true,'Sess_UserName',1000000,2,'tblExpenseDet:EmployeeID:tblExpenseDet_tblEmployee_EmpFullName:'+param4,'',"","","11",'',"2",false); 
 
 
 addCols(tblExpenseDetData,'tblExpenseDet_tblDepartment_DeptFullName','部门','75',2,true,'Sess_DepartmentName',1000000,2,'tblExpenseDet:EmployeeID:tblExpenseDet_tblDepartment_DeptFullName:'+param4,'',"","","11",'',"2",false); 
 ...
 addCols(tblExpenseDetData,'tblExpenseDet_RefBillID','关联单据','100',2,true,'',100,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblExpenseDet_RefBillID","关联单据","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblExpenseDetData,'tblExpenseDet_RefBillType','关联单据类型','200',2,true,'',100,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblExpenseDet_RefBillType","关联单据类型","any","",true,minValue,100,false,''); 
 
 
 addCols(tblExpenseDetData,'tblExpenseDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblExpensed&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblExpensed,tblExpenseDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblExpensed&allTableName=tblExpensed,tblExpenseDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblExpensed&allTableName=tblExpenseDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblExpensed&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblExpensed&allTableName=tblExpenseDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblExpensed&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblExpensed&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblExpensed&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblExpensed&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblExpensed&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '费用支出';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblExpensed&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
