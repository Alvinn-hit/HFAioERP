...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDBomOil&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDBomOil&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDBomOil&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDBomOil&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDBomOilDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDBomOilDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDBomOilDet"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDBomOil\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDBomOil\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'PDBomOilDet_qty',dis:'数量',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDBomOil",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDBomOil&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDBomOil&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDBomOil&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDBomOil&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='PDBomOilDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='配方原料明细';
childCount++;
 var PDBomOilDetData = new gridData();
 gridDatas[gridDataNum] = PDBomOilDetData;
 gridDataNum++;
 var PDBomOilDet=1;
 ...
 addCols(PDBomOilDetData,'PDBomOilDet_GoodsCode','品名','200',2,false,'',50,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("PDBomOilDet_GoodsCode","品名","any","",false,minValue,50,false,'');
 
 
 addCols(PDBomOilDetData,'PDBomOilDet_tblGoods_GoodsNumber','原料油编号','120',2,true,'',1000000,2,'PDBomOilDet:GoodsCode:PDBomOilDet_tblGoods_GoodsNumber:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDBomOilDetData,'PDBomOilDet_tblGoods_GoodsFullName','原料油名称','120',2,true,'',1000000,2,'PDBomOilDet:GoodsCode:PDBomOilDet_tblGoods_GoodsFullName:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDBomOilDetData,'PDBomOilDet_tblGoods_GoodsSpec','原料油规格','120',20,true,'',1000000,2,'PDBomOilDet:GoodsCode:PDBomOilDet_tblGoods_GoodsSpec:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDBomOilDetData,'PDBomOilDet_tblGoods_BaseUnit','单位','80',2,true,'',1000000,2,'PDBomOilDet:GoodsCode:PDBomOilDet_tblGoods_BaseUnit:'+param4,'',"","","0",'',"2",false); 
 
 
 
 
 addCols(PDBomOilDetData,'PDBomOilDet_StockCode','仓库','200',2,true,'',100,-2,'','',"","","1",'',"0",false); 
 putChildValidateItem("PDBomOilDet_StockCode","仓库","any","",true,minValue,100,false,''); 
 
 
 
 addCols(PDBomOilDetData,'PDBomOilDet_qty','数量','60',1,false,'',0,0,'','',"Qty","","2",'',"0",false); 
 putChildValidateItem("PDBomOilDet_qty","数量","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDBomOilDetData,'PDBomOilDet_remark','备注','200',18,true,'',500,0,'','',"Qty","","3",'',"0",false); 
 putChildValidateItem("PDBomOilDet_remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 addCols(PDBomOilDetData,'PDBomOilDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDBomOil&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDBomOil,PDBomOilDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDBomOil&allTableName=PDBomOil,PDBomOilDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDBomOil&allTableName=PDBomOilDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDBomOil&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDBomOil&allTableName=PDBomOilDet,&colType=colWidth" ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDBomOil&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDBomOil&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDBomOil&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDBomOil&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '配方';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDBomOil&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
