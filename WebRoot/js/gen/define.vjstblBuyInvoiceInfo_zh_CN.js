...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBuyInvoiceInfo&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBuyInvoiceInfo&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBuyInvoiceInfo&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBuyInvoiceInfo&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBuyInfoDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBuyInfoDet"){
 
 copyFields=n('tblBuyInfoDet_CompanyCode');
 ...
 var tblBuyInfoDet_tblCompany_ComNumber = n('tblBuyInfoDet_tblCompany_ComNumber');
 if(typeof(tblBuyInfoDet_tblCompany_ComNumber) != 'undefined' && tblBuyInfoDet_tblCompany_ComNumber.length >index){
 tblBuyInfoDet_tblCompany_ComNumber[index].value = tblBuyInfoDet_tblCompany_ComNumber[index-1].value;
 }
 var tblBuyInfoDet_tblCompany_ComName = n('tblBuyInfoDet_tblCompany_ComName');
 if(typeof(tblBuyInfoDet_tblCompany_ComName) != 'undefined' && tblBuyInfoDet_tblCompany_ComName.length >index){
 tblBuyInfoDet_tblCompany_ComName[index].value = tblBuyInfoDet_tblCompany_ComName[index-1].value;
 }
 }
 copyFields=n('tblBuyInfoDet_RefBillNo');
 if((order > 1 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 copyFields=n('tblBuyInfoDet_GoodsCode');
 if((order > 5 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 copyFields=n('tblBuyInfoDet_InvoiceNo');
 if((order > 11 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBuyInfoDet"){
 ...
 copyFields=n('tblBuyInfoDet_InvoiceNo'); 
 if( (order > 11 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 }
 }
...
function execStat2(){
 execStatOne('tblBuyInfoDet_BillAmt',2);
 execStatOne('tblBuyInfoDet_InvoiceAmount',2);
 execStatOne('tblBuyInfoDet_NeedPayAmt',2);
 execStatOne('tblBuyInfoDet_ExeBalAmt',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblBuyInvoiceInfo\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBuyInvoiceInfo\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblBuyInfoDet_ExeBalAmt',"tblBuyInfoDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'InvoiceAmount',dis:'开票金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblBuyInfoDet_Tax',dis:'税率',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyInfoDet_BillAmt',dis:'应开票金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyInfoDet_InvoiceAmount',dis:'已开票金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyInfoDet_NeedPayAmt',dis:'未开票金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyInfoDet_ExeBalAmt',dis:'本次开票金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBuyInvoiceInfo",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyInvoiceInfo&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyInvoiceInfo&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblBuyInvoiceInfo"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBuyInvoiceInfo&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBuyInvoiceInfo&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyInfoDet_ExeBalAmt'); 
 n('InvoiceAmount')[0].value=f(sm('tblBuyInfoDet_ExeBalAmt'),2);
n('TaxAmount')[0].value=f(nn('InvoiceAmount',0)*nn('TaxRate',0),2);
...
childData[childCount]='tblBuyInfoDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='采购开票管理明细';
childCount++;
 var tblBuyInfoDetData = new gridData();
 gridDatas[gridDataNum] = tblBuyInfoDetData;
 gridDataNum++;
 var tblBuyInfoDet=1;
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_CompanyCode','供应商','120',2,true,'',100,-2,'','',"Supplier","copy","0",'',"2",false); 
 putChildValidateItem("tblBuyInfoDet_CompanyCode","供应商","any","",true,minValue,100,false,''); 
 ...
 var param4="";
 param4=param4+":0供应商@CompanyCode"; 
 param4=param4+":1单据日期@BillDate"; 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_RefBillNo','关联单据号','120',2,true,'',50,2,'tblBuyInfoDet:RefBillNo:tblBuyInfoDet_RefBillNo:'+param4,"","","copy","1",'',"2",true);
 putChildValidateItem("tblBuyInfoDet_RefBillNo","关联单据号","any","",true,minValue,50,false,'',true);
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_BillName','单据类型','100',2,true,'',100,8,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblBuyInfoDet_BillName","单据类型","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_InVoiceType','发票类型','80',2,true,'',10,8,';1:普通发票;2:增值税发票;3:收据凭证','',"","","3",'',"1",false);
 putChildValidateItem("tblBuyInfoDet_InVoiceType","发票类型","any","",true,minValue,10,false,'',true); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_Tax','税率','60',1,true,'',0,8,'','',"Qty","","4",'',"0",false); 
 putChildValidateItem("tblBuyInfoDet_Tax","税率","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_GoodsCode','料件名称','200',2,true,'',50,-2,'','',"","copy","5",'',"0",false); 
 putChildValidateItem("tblBuyInfoDet_GoodsCode","料件名称","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_BillAmt','应开票金额','80',1,true,'',0,8,'','',"AmountIdentifier","","6",'',"0",false); 
 putChildValidateItem("tblBuyInfoDet_BillAmt","应开票金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_RefBillID','关联单据ID','80',2,true,'',50,-2,'','',"","","7",'',"3",false); 
 putChildValidateItem("tblBuyInfoDet_RefBillID","关联单据ID","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_InvoiceAmount','已开票金额','80',1,true,'',0,8,'','',"AmountIdentifier","","8",'',"0",false); 
 putChildValidateItem("tblBuyInfoDet_InvoiceAmount","已开票金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_NeedPayAmt','未开票金额','80',1,true,'',0,8,'','',"AmountIdentifier","","9",'',"0",false); 
 putChildValidateItem("tblBuyInfoDet_NeedPayAmt","未开票金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_ExeBalAmt','本次开票金额','80',1,true,'',0,0,'','',"AmountIdentifier","","10",'',"0",false); 
 putChildValidateItem("tblBuyInfoDet_ExeBalAmt","本次开票金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_InvoiceNo','发票号','120',2,true,'',50,0,'','',"","copy","11",'',"0",false); 
 putChildValidateItem("tblBuyInfoDet_InvoiceNo","发票号","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_BillType','单据类型','200',2,true,'',100,-2,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblBuyInfoDet_BillType","单据类型","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_Remark','备注','160',2,true,'',8000,0,'','',"","","13",'',"0",false); 
 putChildValidateItem("tblBuyInfoDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_SourceID','源ID','80',2,true,'',100,-2,'','',"","","15",'',"0",false); 
 putChildValidateItem("tblBuyInfoDet_SourceID","源ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyInfoDetData,'tblBuyInfoDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBuyInvoiceInfo&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBuyInvoiceInfo,tblBuyInfoDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyInvoiceInfo&allTableName=tblBuyInvoiceInfo,tblBuyInfoDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyInvoiceInfo&allTableName=tblBuyInfoDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyInvoiceInfo&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyInvoiceInfo&allTableName=tblBuyInfoDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblBuyInvoiceInfoDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblBuyInvoiceInfo&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblBuyInvoiceInfo&designId=f0cc9359_1010201023520780040&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBuyInvoiceInfo&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBuyInvoiceInfo&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBuyInvoiceInfo&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '采购发票';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBuyInvoiceInfo&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
