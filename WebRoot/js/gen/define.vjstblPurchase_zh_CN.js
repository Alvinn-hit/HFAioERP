...
{ 
 cFireEvent('tblPurchaseDet_Price','0'); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblPurchase&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblPurchase&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblPurchase&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblPurchase&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblPurchaseDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblPurchaseDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblPurchaseDet"){
...
function execStat2(){
 execStatOne('tblPurchaseDet_Quantity',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=bb2f12a7_1404300929039210114&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=bb2f12a7_1404300929039210114&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblPurchase\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblPurchase\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblPurchaseDet_Price',"tblPurchaseDet"); 
 delFireEventOne(rowId,tableName,'tblPurchaseDet_Amount',"tblPurchaseDet"); 
 delFireEventOne(rowId,tableName,'tblPurchaseDet_TotalPrice',"tblPurchaseDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'TotalCost',dis:'总金额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblPurchaseDet_Price',dis:'单价',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPurchaseDet_Amount',dis:'下料数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPurchaseDet_ZBPrice',dis:'材料中标单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPurchaseDet_Quantity',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPurchaseDet_TotalPrice',dis:'总价',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblPurchase",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblPurchase&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblPurchase&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"bb2f12a7_1404300929039210114",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "ContractBillNo"){
 var saveField=n("ContractBillNo"); 
 saveField[0].value = "";
 var obj = m("tbl_tblContract_BillNo");
 if(obj != null){
 obj.value = "";
 }
 }
 if(fieldName == "Provider"){
 var saveField=n("Provider"); 
 saveField[0].value = "";
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=bb2f12a7_1404300929039210114&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=bb2f12a7_1404300929039210114&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=bb2f12a7_1404300929039210114&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblPurchase"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblPurchase&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblPurchase&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblPurchaseDet_Price'); 
 r(0,"TotalPrice")[p].value=f(rn(0,"Amount",p)*rn(0,"Price",p),2);
n('TotalCost')[0].value=f(sm('tblPurchaseDet_TotalPrice'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPurchaseDet_Amount'); 
 r(0,"TotalPrice")[p].value=f(rn(0,"Amount",p)*rn(0,"Price",p),2);
n('TotalCost')[0].value=f(sm('tblPurchaseDet_TotalPrice'),2);
 ...
 var z=r(0,'TotalPrice');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPurchaseDet_TotalPrice'); 
 r(0,"Price")[p].value=f(rn(0,"TotalPrice",p)/rn(0,"Amount",p),2);
n('TotalCost')[0].value=f(sm('tblPurchaseDet_TotalPrice'),2);
...
childData[childCount]='tblPurchaseDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='采购订单明细';
childCount++;
 var tblPurchaseDetData = new gridData();
 gridDatas[gridDataNum] = tblPurchaseDetData;
 gridDataNum++;
 var tblPurchaseDet=1;
 
 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_MaterialDetPurchase','已更新材料总计划明细已下采购量','100',2,true,'',50,-2,'','',"","","0",'',"3",false); 
 putChildValidateItem("tblPurchaseDet_MaterialDetPurchase","已更新材料总计划明细已下采购量","any","",true,minValue,50,false,''); 
 ...
 addCols(tblPurchaseDetData,'tblPurchaseDet_WareNo','材料编码','100',2,false,'',50,2,'tblPurchaseDet:WareNo:tblPurchaseDet_WareNo:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblPurchaseDet_WareNo","材料编码","any","",false,minValue,50,false,'',true);
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_WareName','材料名称','150',2,false,'',200,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_WareName","材料名称","any","",false,minValue,200,false,'',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_Specification','规格/型号','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_Specification","规格/型号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_Brand','品牌','100',2,true,'',100,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_Brand","品牌","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_WareUnit','计量单位','100',2,true,'',100,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_WareUnit","计量单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_Price','单价','100',1,true,'0',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_Amount','下料数量','100',1,true,'0',0,0,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_Amount","下料数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_ZBPrice','材料中标单价','100',1,true,'0',0,8,'','',"","","8",'',"8",false); 
 putChildValidateItem("tblPurchaseDet_ZBPrice","材料中标单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_Quantity','数量','100',1,true,'',0,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_Quantity","数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_TotalPrice','总价','100',1,true,'0',0,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_TotalPrice","总价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_ExpectedTime','期望到场日期','100',5,true,'',50,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_ExpectedTime","期望到场日期","date","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_ArriveTime','到场时间','100',5,true,'',50,8,'','',"","","12",'',"8",false); 
 putChildValidateItem("tblPurchaseDet_ArriveTime","到场时间","date","",true,minValue,50,false,'',true); 
 ...
 addCols(tblPurchaseDetData,'tblPurchaseDet_CuttingMemo','下料单备注','150',2,true,'',500,0,'','',"","","16",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_CuttingMemo","下料单备注","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_Memo','备注','150',2,true,'',500,0,'','',"","","17",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_Memo","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_Attachment','附件','150',14,true,'',500,0,'','',"","","18",'',"0",false); 
 putChildValidateItem("tblPurchaseDet_Attachment","附件","affix","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_MaterialDetId','材料总计划明细ID','100',2,true,'',100,-2,'','',"","","19",'',"3",false); 
 putChildValidateItem("tblPurchaseDet_MaterialDetId","材料总计划明细ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_SourceID','引用明细ID','100',2,true,'',32,-2,'','',"","","20",'',"3",false); 
 putChildValidateItem("tblPurchaseDet_SourceID","引用明细ID","any","",true,minValue,32,false,''); 
 
 
 
 
 addCols(tblPurchaseDetData,'tblPurchaseDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblPurchase&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblPurchase,tblPurchaseDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblPurchase&allTableName=tblPurchase,tblPurchaseDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblPurchase&allTableName=tblPurchaseDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblPurchase&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblPurchase&allTableName=tblPurchaseDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=bb2f12a7_1404300929039210114&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=bb2f12a7_1404300929039210114&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=bb2f12a7_1404300929039210114&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=bb2f12a7_1404300929039210114&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblPurchase&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblPurchase&designId=bb2f12a7_1404301038280460401&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblPurchase&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblPurchase&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblPurchase&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '采购订单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblPurchase&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
