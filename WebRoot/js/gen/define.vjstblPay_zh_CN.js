...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblPay&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblPay&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblPay&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblPay&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblPayAccountDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblPaydetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblPayAccountDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblPaydet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblPayAccountDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblPaydet"){
...
function execStat2(){
 execStatOne('tblPayAccountDet_Amount',2);
 execStatOne('tblPaydet_BillAmt',2);
 execStatOne('tblPaydet_SettledAmt',2);
 execStatOne('tblPaydet_WexeBalAmt',2);
 execStatOne('tblPaydet_ExeBalAmt',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblPay\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblPay\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblPayAccountDet_Amount',"tblPayAccountDet"); 
 delFireEventOne(rowId,tableName,'tblPaydet_BillAmt',"tblPaydet"); 
 delFireEventOne(rowId,tableName,'tblPaydet_SettledAmt',"tblPaydet"); 
 delFireEventOne(rowId,tableName,'tblPaydet_WexeBalAmt',"tblPaydet"); 
 delFireEventOne(rowId,tableName,'tblPaydet_ExeBalAmt',"tblPaydet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'SettleAmt',dis:'往来结算合计',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblPayAccountDet_Amount',dis:'金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPaydet_BillAmt',dis:'单据金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPaydet_SettledAmt',dis:'已结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPaydet_WexeBalAmt',dis:'待结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPaydet_ExeBalAmt',dis:'本次结算金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblPay",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblPay&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblPay&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblPay&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblPay&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('FactOutcome')[0].value=f(Number(nn('SettleAmt',0)),2);

if(n('PaytypeID')[0].value=="ReturnReceive")n('FactOutcome')[0].value=f(Number(nn('SettleAmt',0)),2);

if(n('PaytypeID')[0].value=="RetrunPay")n('FactOutcome')[0].value=f(Number(nn('SettleAmt',0)),2);

n('CurrencyAmt')[0].value=f(nn('SettleAmt',0)/nn('CurrencyRate',0),2);

n('FactOutcome')[0].value=f(Number(sm('tblPayAccountDet_Amount')),2);

n('AccAmt')[0].value=f(sm('tblPayAccountDet_Amount'),2);

n('SettleAmt')[0].value=f(Number(sm('tblPayAccountDet_Amount')),2);

if(n('PaytypeID')[0].value=="ReturnReceive")n('SettleAmt')[0].value=f(Number(sm('tblPayAccountDet_Amount')),2);

if(n('PaytypeID')[0].value=="RetrunPay")n('SettleAmt')[0].value=f(Number(sm('tblPayAccountDet_Amount')),2);
...
n('CurrencyAmt')[0].value=f(nn('SettleAmt',0)/nn('CurrencyRate',0),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('SettleAmt')[0].value=f(nn('CurrencyRate',0)*nn('CurrencyAmt',0),2);
...
if(n('PaytypeID')[0].value=="RetrunPay")n('FactOutcome')[0].value=f(Number(nn('SettleAmt',0)),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 for(var j=0;j<r(1,'ExeBalFcAmt').length;j++){
r(1,"ExeBalFcAmt")[j].value=f(rn(1,"ExeBalAmt",j)/nn('CurrencyRate',0),2);
}
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPayAccountDet_Amount'); 
 n('FactOutcome')[0].value=f(Number(sm('tblPayAccountDet_Amount')),2);
n('AccAmt')[0].value=f(sm('tblPayAccountDet_Amount'),2);
n('SettleAmt')[0].value=f(Number(sm('tblPayAccountDet_Amount')),2);
if(n('PaytypeID')[0].value=="RetrunPay")n('SettleAmt')[0].value=f(Number(sm('tblPayAccountDet_Amount')),2);
...
n('CurrencyAmt')[0].value=f(nn('SettleAmt',0)/nn('CurrencyRate',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPayAccountDet_ExeBalFcAmt'); 
...
n('CurrencyAmt')[0].value=f(nn('SettleAmt',0)/nn('CurrencyRate',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPaydet_BackAmt'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPaydet_BillAmt'); 
 r(1,"ExeBalFcAmt")[p].value=f(rn(1,"ExeBalAmt",p)/nn('CurrencyRate',0),2);

 execStat(); 
 }
 }
 var z=r(1,'SettledAmt');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPaydet_SettledAmt'); 
 r(1,"ExeBalFcAmt")[p].value=f(rn(1,"ExeBalAmt",p)/nn('CurrencyRate',0),2);

 execStat(); 
 }
 }
 var z=r(1,'WexeBalAmt');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPaydet_WexeBalAmt'); 
 r(1,"ExeBalFcAmt")[p].value=f(rn(1,"ExeBalAmt",p)/nn('CurrencyRate',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPaydet_ExeBalAmt'); 
 r(1,"ExeBalFcAmt")[p].value=f(rn(1,"ExeBalAmt",p)/nn('CurrencyRate',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPaydet_ExeBalFcAmt'); 
...
childData[childCount]='tblPayAccountDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='付款账户明细表';
childCount++;
 var tblPayAccountDetData = new gridData();
 gridDatas[gridDataNum] = tblPayAccountDetData;
 gridDataNum++;
 var tblPayAccountDet=1;
 ...
 addCols(tblPayAccountDetData,'tblPayAccountDet_Paytype','付款方式','80',2,true,'1',100,1,';1:现金;2:支票;3:电汇;4:商业汇票;5:银行汇票','',"","","1",'',"1",false);
 putChildValidateItem("tblPayAccountDet_Paytype","付款方式","any","",true,minValue,100,true,'1',true); 
 ...
 addCols(tblPayAccountDetData,'tblPayAccountDet_Account','账户','150',2,false,'Sess_Account',100,-2,'','',"","","2",'',"2",true);
 putChildValidateItem("tblPayAccountDet_Account","账户","any","",false,minValue,100,true,'Sess_Account');
 
 
 addCols(tblPayAccountDetData,'tblPayAccountDet_tblAccTypeInfo_AccName','账户','150',2,true,'Sess_AccName',1000000,2,'tblPayAccountDet:Account:tblPayAccountDet_tblAccTypeInfo_AccName:'+param4,'',"","","2",'',"2",false); 
 
 
 
 addCols(tblPayAccountDetData,'tblPayAccountDet_Amount','金额','100',1,false,'0',0,0,'','',"Amount","","3",'',"0",false); 
 putChildValidateItem("tblPayAccountDet_Amount","金额","double","RowMarker",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblPayAccountDetData,'tblPayAccountDet_ExeBalFcAmt','外币结算金额','80',1,true,'',0,-2,'','',"AmountIdentifier","","4",'',"0",false); 
 putChildValidateItem("tblPayAccountDet_ExeBalFcAmt","外币结算金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblPayAccountDetData,'tblPayAccountDet_Remark','账户备注','200',2,true,'',8000,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblPayAccountDet_Remark","账户备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblPayAccountDetData,'tblPayAccountDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblPaydet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='付款单明细';
childCount++;
 var tblPaydetData = new gridData();
 gridDatas[gridDataNum] = tblPaydetData;
 gridDataNum++;
 var tblPaydet=1;
 ...
 addCols(tblPaydetData,'tblPaydet_RefbillID','关联单据ID','0',2,true,'',30,-2,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblPaydet_RefbillID","关联单据ID","any","",true,minValue,30,false,''); 
 ...
 param4=param4+":1付款类型@PaytypeID"; 
 param4=param4+":1单据日期@BillDate"; 
 
 addCols(tblPaydetData,'tblPaydet_RefBillNo','关联单据编号','120',2,true,'',30,2,'tblPaydet:RefBillNo:tblPaydet_RefBillNo:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblPaydet_RefBillNo","关联单据编号","any","",true,minValue,30,false,'',true);
 
 
 
 
 addCols(tblPaydetData,'tblPaydet_BillName','模块名称','100',2,true,'',100,-2,'','',"","","3",'',"8",false); 
 putChildValidateItem("tblPaydet_BillName","模块名称","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_BackAmt','退货金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","4",'',"8",false); 
 putChildValidateItem("tblPaydet_BackAmt","退货金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_BillAmt','单据金额','70',1,true,'',0,8,'','',"AmountIdentifier","","5",'',"8",false); 
 putChildValidateItem("tblPaydet_BillAmt","单据金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_SettledAmt','已结算金额','70',1,true,'',0,8,'','',"AmountIdentifier","","6",'',"8",false); 
 putChildValidateItem("tblPaydet_SettledAmt","已结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_WexeBalAmt','待结算金额','100',1,true,'',0,8,'','',"AmountIdentifier","","7",'',"8",false); 
 putChildValidateItem("tblPaydet_WexeBalAmt","待结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_ExeBalAmt','本次结算金额','100',1,true,'',0,0,'','',"AmountIdentifier","","8",'',"0",false); 
 putChildValidateItem("tblPaydet_ExeBalAmt","本次结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_CurBillAmt','外币单据金额','80',1,true,'',0,-2,'','',"Amount","","9",'',"3",false); 
 putChildValidateItem("tblPaydet_CurBillAmt","外币单据金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_CurBackAmt','外币退货金额','80',1,true,'',0,-2,'','',"Amount","","10",'',"3",false); 
 putChildValidateItem("tblPaydet_CurBackAmt","外币退货金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_CurSettledAmt','外币已结算金额','80',1,true,'',0,-2,'','',"Amount","","11",'',"3",false); 
 putChildValidateItem("tblPaydet_CurSettledAmt","外币已结算金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_CurWexeBalAmt','外币待结算金额','80',1,true,'',0,-2,'','',"Amount","","12",'',"3",false); 
 putChildValidateItem("tblPaydet_CurWexeBalAmt","外币待结算金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_ExeBalFcAmt','外币结算金额','80',1,true,'',0,-2,'','',"AmountIdentifier","","13",'',"0",false); 
 putChildValidateItem("tblPaydet_ExeBalFcAmt","外币结算金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_Remark','备注','150',2,true,'',8000,0,'','',"","","14",'',"0",false); 
 putChildValidateItem("tblPaydet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_SourceID','源ID','224',2,true,'',60,-2,'','',"","","15",'',"3",false); 
 putChildValidateItem("tblPaydet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 
 addCols(tblPaydetData,'tblPaydet_PayBillType','付款单据类型','200',2,true,'',100,-2,'','',"","","17",'',"3",false); 
 putChildValidateItem("tblPaydet_PayBillType","付款单据类型","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_RefBillDate','单据日期','200',5,true,'',10,-2,'','',"","","18",'',"3",false); 
 putChildValidateItem("tblPaydet_RefBillDate","单据日期","date","",true,minValue,10,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_Account','关联单据账户','100',2,true,'',50,-2,'','',"","","19",'',"3",false); 
 putChildValidateItem("tblPaydet_Account","关联单据账户","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_BuyOrderID','采购订单ID','0',2,true,'',80,-2,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblPaydet_BuyOrderID","采购订单ID","any","",true,minValue,80,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_BuyOrderNo','采购订单编号','0',2,true,'',80,-2,'','',"","","21",'',"0",false); 
 putChildValidateItem("tblPaydet_BuyOrderNo","采购订单编号","any","",true,minValue,80,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_CompanyCode','往来单位','200',2,true,'',50,-2,'','',"","","22",'',"3",false); 
 putChildValidateItem("tblPaydet_CompanyCode","往来单位","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblPaydetData,'tblPaydet_Currency','币种','200',2,true,'',50,-2,'','',"","","23",'',"0",false); 
 putChildValidateItem("tblPaydet_Currency","币种","any","",true,minValue,50,false,''); 
 
 
 
 
 addCols(tblPaydetData,'tblPaydet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblPay&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblPay,tblPayAccountDet,tblPaydet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblPay&allTableName=tblPay,tblPayAccountDet,tblPaydet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblPay&allTableName=tblPayAccountDet,tblPaydet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblPay&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblPay&allTableName=tblPayAccountDet,tblPaydet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblPay&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblPay&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblPay&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblPay&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblPay&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '付款单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblPay&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
