...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblWatchHand&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblWatchHand&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblWatchHand&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblWatchHand&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchHandDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchHandDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblWatchHandDet"){
 
  }
 if(typeof(tableName)!="undefined" && tableName=="tblWatchHandDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchHandDet"){
 
  }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchHandDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblWatchHandDet"; 
 tblName[1]="tblWatchHandDet1"; 
...
function execStat2(){
 execStatOne('tblWatchHandDet_Qty',2);
 execStatOne('tblWatchHandDet_Price',4);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblWatchHand\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblWatchHand\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblWatchHandDet_Qty',dis:'用量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchHandDet_Price',dis:'单价',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblWatchHand",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchHand&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchHand&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblWatchHand"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblWatchHand&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchHand&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblWatchHandDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表针物料明细';
childCount++;
 var tblWatchHandDetData = new gridData();
 gridDatas[gridDataNum] = tblWatchHandDetData;
 gridDataNum++;
 var tblWatchHandDet=1;
 ...
 addCols(tblWatchHandDetData,'tblWatchHandDet_GoodsCode','物料名称','100',2,true,'',1000,-2,'','',"","","1",'',"3",false); 
 putChildValidateItem("tblWatchHandDet_GoodsCode","物料名称","any","",true,minValue,1000,false,''); 
 ...
 addCols(tblWatchHandDetData,'tblWatchHandDet_GoodsNumber','物资编号','200',2,false,'',500,2,'tblWatchHandDet:GoodsNumber:tblWatchHandDet_GoodsNumber:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblWatchHandDet_GoodsNumber","物资编号","any","",false,minValue,500,false,'',true);
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_NeedleNum','表针型号','120',2,true,'',100,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchHandDet_NeedleNum","表针型号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_NeedleLog','表针长度','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchHandDet_NeedleLog","表针长度","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_NeedleMat','表针材料','120',2,true,'',200,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblWatchHandDet_NeedleMat","表针材料","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_GrindASK','打磨要求','120',2,true,'',200,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblWatchHandDet_GrindASK","打磨要求","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_Color1','电镀/电染颜色','120',2,true,'',100,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblWatchHandDet_Color1","电镀/电染颜色","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_Color2','喷油颜色','100',2,true,'',100,0,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblWatchHandDet_Color2","喷油颜色","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_Color3','荧光色/夜光','100',2,true,'',100,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblWatchHandDet_Color3","荧光色/夜光","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_Qty','用量','80',1,true,'1',0,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblWatchHandDet_Qty","用量","double","",true,minValue,maxValue,true,'1',true); 
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_Qty1','用量','80',1,true,'1',0,-2,'','',"","","10",'',"3",false); 
 putChildValidateItem("tblWatchHandDet_Qty1","用量","double","",true,minValue,maxValue,true,'1'); 
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_Price','单价','80',1,true,'',0,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblWatchHandDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchHandDet_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchHandDetData,'tblWatchHandDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblWatchHandDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表针变更明细';
childCount++;
 var tblWatchHandDet1Data = new gridData();
 gridDatas[gridDataNum] = tblWatchHandDet1Data;
 gridDataNum++;
 var tblWatchHandDet1=1;
 ...
 addCols(tblWatchHandDet1Data,'tblWatchHandDet1_ChangeContent','变更内容','200',2,false,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblWatchHandDet1_ChangeContent","变更内容","any","",false,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchHandDet1Data,'tblWatchHandDet1_ChangeDate','变更日期','90',5,false,'2013-12-24',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchHandDet1_ChangeDate","变更日期","date","",false,minValue,10,true,'2013-12-24',true); 
 
 
 
 addCols(tblWatchHandDet1Data,'tblWatchHandDet1_ChangePer','变更人员','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchHandDet1_ChangePer","变更人员","any","",true,minValue,100,false,'',true); 
 
 
 
 
 addCols(tblWatchHandDet1Data,'tblWatchHandDet1_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchHandDet1_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchHandDet1Data,'tblWatchHandDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblWatchHand&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=68fc380d_1408131539576500232&noback="+noback+"&parentCode=&operation=6&parentTableName=tblSampleOrder&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblWatchHand,tblWatchHandDet,tblWatchHandDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchHand&allTableName=tblWatchHand,tblWatchHandDet,tblWatchHandDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchHand&allTableName=tblWatchHandDet,tblWatchHandDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchHand&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchHand&allTableName=tblWatchHandDet,tblWatchHandDet1,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblWatchHand&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblWatchHand&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblWatchHand&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchHand&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblWatchHand&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '表针';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblWatchHand&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
