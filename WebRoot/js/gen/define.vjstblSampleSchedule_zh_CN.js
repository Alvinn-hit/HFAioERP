...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblSampleSchedule&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblSampleSchedule&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblSampleSchedule&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblSampleSchedule&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblSampleScheduleDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblSampleScheduleDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblSampleScheduleDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblSampleScheduleDet"; 
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=009710d8_1306261507141400172&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=009710d8_1306261507141400172&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblSampleSchedule\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblSampleSchedule\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblSampleSchedule",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSampleSchedule&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSampleSchedule&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"009710d8_1306261507141400172",
 MOOP:"add",
...
function resetSubmit(fieldName){
 if(fieldName == "SampleRequestID"){
 var saveField=n("SampleRequestID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblSampleRequest_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("DeliveryDate"); 
 saveField[0].value = "";
 var obj = m("tbl_tblSampleRequest_DeliveryDate");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblSampleRequest_BillNo");
 saveField[0].value = ""; 
 m("tbl_tblSampleRequest_BillNo").value = "" ;
 var saveField=n("tblSampleRequest_SampleModel");
 saveField[0].value = ""; 
 m("tbl_tblSampleRequest_SampleModel").value = "" ;
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=009710d8_1306261507141400172&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=009710d8_1306261507141400172&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=009710d8_1306261507141400172&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblSampleSchedule"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblSampleSchedule&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblSampleSchedule&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblSampleScheduleDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='样板进度明细表';
childCount++;
 var tblSampleScheduleDetData = new gridData();
 gridDatas[gridDataNum] = tblSampleScheduleDetData;
 gridDataNum++;
 var tblSampleScheduleDet=1;
 ...
 addCols(tblSampleScheduleDetData,'tblSampleScheduleDet_Part','配件','100',2,false,'',10,1,';0:其它;11:表壳(LCD);1:表壳;2:表面;12:表面(MOP);3:表带;13:表带(胶带);14:表带(胶皮);4:表针;5:巴的;6:扣制;7:玻璃;8:生耳;9:石仔;10:机芯','',"","","1",'',"1",false);
 putChildValidateItem("tblSampleScheduleDet_Part","配件","any","",false,minValue,10,false,'',true); 
 ...
 addCols(tblSampleScheduleDetData,'tblSampleScheduleDet_CompanyCode','供应商','120',2,false,'',1000,-2,'','',"","","2",'',"2",true);
 putChildValidateItem("tblSampleScheduleDet_CompanyCode","供应商","any","",false,minValue,1000,false,'');
 addCols(tblSampleScheduleDetData,'tblSampleScheduleDet_tblCompany_ComFullName','供应商','200',18,true,'',1000000,2,'tblSampleScheduleDet:CompanyCode:tblSampleScheduleDet_tblCompany_ComFullName:'+param4,'',"","","2",'',"2",false); 
 
 
 
 addCols(tblSampleScheduleDetData,'tblSampleScheduleDet_Schedule','进度情况','140',2,true,'',500,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblSampleScheduleDet_Schedule","进度情况","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblSampleScheduleDetData,'tblSampleScheduleDet_DeliveryDate','预计交期','90',5,false,'',10,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblSampleScheduleDet_DeliveryDate","预计交期","date","",false,minValue,10,false,'',true); 
 
 
 
 addCols(tblSampleScheduleDetData,'tblSampleScheduleDet_OverdueDays','逾期天数','80',0,true,'',0,8,'','',"","","5",'',"8",false); 
 putChildValidateItem("tblSampleScheduleDet_OverdueDays","逾期天数","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSampleScheduleDetData,'tblSampleScheduleDet_Remark','备注','200',2,true,'',1000,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblSampleScheduleDet_Remark","备注","any","",true,minValue,1000,false,'',true); 
 
 
 
 
 addCols(tblSampleScheduleDetData,'tblSampleScheduleDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblSampleSchedule&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblSampleSchedule,tblSampleScheduleDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSampleSchedule&allTableName=tblSampleSchedule,tblSampleScheduleDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSampleSchedule&allTableName=tblSampleScheduleDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSampleSchedule&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSampleSchedule&allTableName=tblSampleScheduleDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=009710d8_1306261507141400172&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=009710d8_1306261507141400172&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblSampleScheduleDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=009710d8_1306261507141400172&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=009710d8_1306261507141400172&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblSampleSchedule&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblSampleSchedule&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblSampleSchedule&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblSampleSchedule&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblSampleSchedule&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '样板进度表';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblSampleSchedule&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
