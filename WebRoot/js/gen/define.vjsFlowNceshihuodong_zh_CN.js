...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=FlowNceshihuodong&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=FlowNceshihuodong&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=FlowNceshihuodong&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=FlowNceshihuodong&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "FlowNceshihuodongDet1Table"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "FlowNceshihuodongDet2Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="FlowNceshihuodongDet1"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="FlowNceshihuodongDet2"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="FlowNceshihuodongDet1"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="FlowNceshihuodongDet2"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="FlowNceshihuodongDet1"; 
 tblName[1]="FlowNceshihuodongDet2"; 
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=0fee8c8c_1410281759287510002&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=0fee8c8c_1410281759287510002&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"FlowNceshihuodong\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"FlowNceshihuodong\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=FlowNceshihuodong",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=FlowNceshihuodong&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=FlowNceshihuodong&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"0fee8c8c_1410281759287510002",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0fee8c8c_1410281759287510002&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0fee8c8c_1410281759287510002&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=0fee8c8c_1410281759287510002&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "FlowNceshihuodong"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=FlowNceshihuodong&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=FlowNceshihuodong&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='FlowNceshihuodongDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='明细1';
childCount++;
 var FlowNceshihuodongDet1Data = new gridData();
 gridDatas[gridDataNum] = FlowNceshihuodongDet1Data;
 gridDataNum++;
 var FlowNceshihuodongDet1=1;
 
 
 
 
 addCols(FlowNceshihuodongDet1Data,'FlowNceshihuodongDet1_mone','字段1','200',2,true,'',100,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("FlowNceshihuodongDet1_mone","字段1","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(FlowNceshihuodongDet1Data,'FlowNceshihuodongDet1_mtwo','字段2','200',2,true,'',100,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("FlowNceshihuodongDet1_mtwo","字段2","any","",true,minValue,100,false,'',true); 
 
 
 addCols(FlowNceshihuodongDet1Data,'FlowNceshihuodongDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='FlowNceshihuodongDet2';
tabIsNulls[childCount]='0';
tabDiss[childCount]='明细2';
childCount++;
 var FlowNceshihuodongDet2Data = new gridData();
 gridDatas[gridDataNum] = FlowNceshihuodongDet2Data;
 gridDataNum++;
 var FlowNceshihuodongDet2=1;
 
 
 
 
 addCols(FlowNceshihuodongDet2Data,'FlowNceshihuodongDet2_m2one','字1','200',2,true,'',100,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("FlowNceshihuodongDet2_m2one","字1","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(FlowNceshihuodongDet2Data,'FlowNceshihuodongDet2_m2two','字2','200',2,true,'',100,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("FlowNceshihuodongDet2_m2two","字2","any","",true,minValue,100,false,'',true); 
 
 
 addCols(FlowNceshihuodongDet2Data,'FlowNceshihuodongDet2_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=FlowNceshihuodong&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "FlowNceshihuodong,FlowNceshihuodongDet1,FlowNceshihuodongDet2,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=FlowNceshihuodong&allTableName=FlowNceshihuodong,FlowNceshihuodongDet1,FlowNceshihuodongDet2,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=FlowNceshihuodong&allTableName=FlowNceshihuodongDet1,FlowNceshihuodongDet2,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=FlowNceshihuodong&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=FlowNceshihuodong&allTableName=FlowNceshihuodongDet1,FlowNceshihuodongDet2,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0fee8c8c_1410281759287510002&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0fee8c8c_1410281759287510002&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "FlowNceshihuodongDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=0fee8c8c_1410281759287510002&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=0fee8c8c_1410281759287510002&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=FlowNceshihuodong&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=FlowNceshihuodong&designId=0fee8c8c_1410281759287510002&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=FlowNceshihuodong&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=FlowNceshihuodong&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=FlowNceshihuodong&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '测试活动';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=FlowNceshihuodong&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
