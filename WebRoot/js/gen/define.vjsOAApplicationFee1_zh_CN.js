...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=OAApplicationFee1&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=OAApplicationFee1&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=OAApplicationFee1&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=OAApplicationFee1&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "OAApplicationFee1DetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="OAApplicationFee1Det"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="OAApplicationFee1Det"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="OAApplicationFee1Det"; 
...
function execStat2(){
 execStatOne('OAApplicationFee1Det_Subtotal',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=80475ff1_1406161456371213963&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=80475ff1_1406161456371213963&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"OAApplicationFee1\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"OAApplicationFee1\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'OAApplicationFee1Det_Quantity1',"OAApplicationFee1Det"); 
 delFireEventOne(rowId,tableName,'OAApplicationFee1Det_Unitprice',"OAApplicationFee1Det"); 
 delFireEventOne(rowId,tableName,'OAApplicationFee1Det_Otherfee',"OAApplicationFee1Det"); 
 delFireEventOne(rowId,tableName,'OAApplicationFee1Det_Subtotal',"OAApplicationFee1Det"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'ExpenseBudget',dis:'费用预算',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'OAApplicationFee1Det_Quantity1',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'OAApplicationFee1Det_Unitprice',dis:'单价',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'OAApplicationFee1Det_Otherfee',dis:'其它',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'OAApplicationFee1Det_Subtotal',dis:'小计',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=OAApplicationFee1",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=OAApplicationFee1&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=OAApplicationFee1&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"80475ff1_1406161456371213963",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=80475ff1_1406161456371213963&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=80475ff1_1406161456371213963&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=80475ff1_1406161456371213963&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "OAApplicationFee1"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=OAApplicationFee1&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=OAApplicationFee1&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'OAApplicationFee1Det_Quantity1'); 
 r(0,"Subtotal")[p].value=f(rn(0,"Unitprice",p)*rn(0,"Quantity1",p)+rn(0,"Otherfee",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'OAApplicationFee1Det_Unitprice'); 
 r(0,"Subtotal")[p].value=f(rn(0,"Unitprice",p)*rn(0,"Quantity1",p)+rn(0,"Otherfee",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'OAApplicationFee1Det_Otherfee'); 
 r(0,"Subtotal")[p].value=f(rn(0,"Unitprice",p)*rn(0,"Quantity1",p)+rn(0,"Otherfee",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'OAApplicationFee1Det_Subtotal'); 
 r(0,"Subtotal")[p].value=f(rn(0,"Unitprice",p)*rn(0,"Quantity1",p)+rn(0,"Otherfee",p),2);
...
childData[childCount]='OAApplicationFee1Det';
tabIsNulls[childCount]='0';
tabDiss[childCount]='费用申请明细';
childCount++;
 var OAApplicationFee1DetData = new gridData();
 gridDatas[gridDataNum] = OAApplicationFee1DetData;
 gridDataNum++;
 var OAApplicationFee1Det=1;
 
 
 
 
 addCols(OAApplicationFee1DetData,'OAApplicationFee1Det_Content','内容','150',2,true,'',200,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("OAApplicationFee1Det_Content","内容","any","",true,minValue,200,false,'',true); 
 ...
 addCols(OAApplicationFee1DetData,'OAApplicationFee1Det_Unit','单位','80',2,true,'',100,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("OAApplicationFee1Det_Unit","单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(OAApplicationFee1DetData,'OAApplicationFee1Det_Quantity1','数量','80',1,true,'0',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("OAApplicationFee1Det_Quantity1","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(OAApplicationFee1DetData,'OAApplicationFee1Det_Unitprice','单价','100',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("OAApplicationFee1Det_Unitprice","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(OAApplicationFee1DetData,'OAApplicationFee1Det_Otherfee','其它','100',1,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("OAApplicationFee1Det_Otherfee","其它","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(OAApplicationFee1DetData,'OAApplicationFee1Det_Subtotal','小计','150',1,true,'0',0,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("OAApplicationFee1Det_Subtotal","小计","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 
 addCols(OAApplicationFee1DetData,'OAApplicationFee1Det_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=OAApplicationFee1&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "OAApplicationFee1,OAApplicationFee1Det,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=OAApplicationFee1&allTableName=OAApplicationFee1,OAApplicationFee1Det,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=OAApplicationFee1&allTableName=OAApplicationFee1Det,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=OAApplicationFee1&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=OAApplicationFee1&allTableName=OAApplicationFee1Det,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=80475ff1_1406161456371213963&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=80475ff1_1406161456371213963&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "OAApplicationFee1Det";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=80475ff1_1406161456371213963&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=80475ff1_1406161456371213963&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=OAApplicationFee1&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=OAApplicationFee1&designId=80475ff1_1406161456371213963&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=OAApplicationFee1&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=OAApplicationFee1&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=OAApplicationFee1&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '费用申请';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=OAApplicationFee1&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
