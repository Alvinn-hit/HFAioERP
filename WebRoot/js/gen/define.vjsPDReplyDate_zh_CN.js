...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDReplyDate&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDReplyDate&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDReplyDate&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDReplyDate&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDReplyDateDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDReplyDateDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDReplyDateDet"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDReplyDate\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDReplyDate\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDReplyDate",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDReplyDate&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDReplyDate&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "tblSalesOrderID"){
 var saveField=n("tblSalesOrderID"); 
 saveField[0].value = "";
 var obj = m("tbl_viewSalesOrderQty_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("viewSalesOrderQty_BillNo");
 saveField[0].value = ""; 
 m("tbl_viewSalesOrderQty_BillNo").value = "" ;
 var saveField=n("viewSalesOrderQty_BillDate");
 saveField[0].value = ""; 
 m("tbl_viewSalesOrderQty_BillDate").value = "" ;
 var saveField=n("tblCompany_ComFullName2");
 saveField[0].value = ""; 
 m("tbl_tblCompany_ComFullName2").value = "" ;
 var saveField=n("tblSalesOrder_BuyNo");
 saveField[0].value = ""; 
 m("tbl_tblSalesOrder_BuyNo").value = "" ;
 }
 }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDReplyDate"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDReplyDate&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDReplyDate&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='PDReplyDateDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='回复交期单明细表';
childCount++;
 var PDReplyDateDetData = new gridData();
 gridDatas[gridDataNum] = PDReplyDateDetData;
 gridDataNum++;
 var PDReplyDateDet=1;
 ...
 var param4="";
 param4=param4+":1销售订单编号ID@tblSalesOrderID"; 
 
 addCols(PDReplyDateDetData,'PDReplyDateDet_GoodsCode','品号','200',2,true,'',100,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("PDReplyDateDet_GoodsCode","品号","any","",true,minValue,100,false,'');
 addCols(PDReplyDateDetData,'PDReplyDateDet_tblGoods_GoodsNumber','品号','200',20,true,'',1000000,2,'PDReplyDateDet:GoodsCode:PDReplyDateDet_tblGoods_GoodsNumber:'+param4,'',"","","0",'',"2",false); 
 addCols(PDReplyDateDetData,'PDReplyDateDet_tblGoods_GoodsFullName','品号','300',20,true,'',1000000,2,'PDReplyDateDet:GoodsCode:PDReplyDateDet_tblGoods_GoodsFullName:'+param4,'',"","","0",'',"2",false); 
 addCols(PDReplyDateDetData,'PDReplyDateDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDReplyDateDet:GoodsCode:PDReplyDateDet_tblGoods_GoodsSpec:'+param4,'',"","","0",'',"2",false); 
 addCols(PDReplyDateDetData,'PDReplyDateDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDReplyDateDet:GoodsCode:PDReplyDateDet_tblGoods_BaseUnit:'+param4,'',"","","0",'',"2",false); 
 addCols(PDReplyDateDetData,'PDReplyDateDet_tblSalesOrderDet_Qty','数量','60',1,true,'',1000000,2,'PDReplyDateDet:GoodsCode:PDReplyDateDet_tblSalesOrderDet_Qty:'+param4,'',"","","0",'',"2",false); 
 
 
 
 
 addCols(PDReplyDateDetData,'PDReplyDateDet_StartDate','开始生产日期','100',5,false,'',10,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("PDReplyDateDet_StartDate","开始生产日期","date","",false,minValue,10,false,'',true); 
 
 
 
 addCols(PDReplyDateDetData,'PDReplyDateDet_ReplyDate','发货日期','100',5,false,'',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("PDReplyDateDet_ReplyDate","发货日期","date","",false,minValue,10,false,'',true); 
 
 
 addCols(PDReplyDateDetData,'PDReplyDateDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDReplyDate&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDReplyDate,PDReplyDateDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDReplyDate&allTableName=PDReplyDate,PDReplyDateDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDReplyDate&allTableName=PDReplyDateDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDReplyDate&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDReplyDate&allTableName=PDReplyDateDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDReplyDate&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDReplyDate&designId=2d73ed96_1512241616306961213&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDReplyDate&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDReplyDate&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDReplyDate&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '回复交期单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDReplyDate&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
