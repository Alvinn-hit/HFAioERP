...
{ 
 cFireEvent('PDReturnOweDet_Qty',''); 
 cFireEvent('PDReturnOweDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDReturnOwe&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDReturnOwe&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDReturnOwe&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDReturnOwe&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDReturnOweDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('PDReturnOweDet_BatchNo');
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDReturnOweDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDReturnOweDet"){
...
function execStat2(){
 execStatOne('PDReturnOweDet_Qty',4);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDReturnOwe\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDReturnOwe\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'PDReturnOweDet_Qty',"PDReturnOweDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'PDReturnOweDet_Qty',dis:'还回数',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDReturnOwe",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDReturnOwe&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDReturnOwe&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "OutStockCode"){
 var saveField=n("OutStockCode"); 
 saveField[0].value = "";
 var obj = m("tbl_PD_StockCode");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("WorkShop"); 
 saveField[0].value = "";
 var obj = m("tbl_PD_WorkShop");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("CompanyCode"); 
 saveField[0].value = "";
 var obj = m("tbl_PD_CompanyCode");
 if(obj != null){
 obj.value = "";
 }
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('PDReturnOweDet_Qty'); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDReturnOwe"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDReturnOwe&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDReturnOwe&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'PDReturnOweDet_Qty'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDReturnOweDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
childData[childCount]='PDReturnOweDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='欠料还回明细';
childCount++;
 var PDReturnOweDetData = new gridData();
 gridDatas[gridDataNum] = PDReturnOweDetData;
 gridDataNum++;
 var PDReturnOweDet=1;
 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_StockCode','入料仓库','200',2,true,'',100,-2,'','',"","","0",'',"2",false); 
 putChildValidateItem("PDReturnOweDet_StockCode","入料仓库","any","",true,minValue,100,false,''); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_OutStockCode','出料仓库','200',2,true,'',100,-2,'','',"","","1",'',"0",false); 
 putChildValidateItem("PDReturnOweDet_OutStockCode","出料仓库","any","",true,minValue,100,false,''); 
 ...
 var param4="";
 param4=param4+":1工厂/车间@OutStockCode"; 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_GoodsCode','商品名称','120',2,false,'',100,-2,'','',"","","2",'',"2",true);
 putChildValidateItem("PDReturnOweDet_GoodsCode","商品名称","any","",false,minValue,100,false,'');
 addCols(PDReturnOweDetData,'PDReturnOweDet_tblGoods_GoodsNumber','品号','200',20,true,'',1000000,2,'PDReturnOweDet:GoodsCode:PDReturnOweDet_tblGoods_GoodsNumber:'+param4,'',"","","2",'',"2",false); 
 addCols(PDReturnOweDetData,'PDReturnOweDet_tblGoods_GoodsFullName','商品名称','300',20,true,'',1000000,2,'PDReturnOweDet:GoodsCode:PDReturnOweDet_tblGoods_GoodsFullName:'+param4,'',"","","2",'',"2",false); 
 addCols(PDReturnOweDetData,'PDReturnOweDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDReturnOweDet:GoodsCode:PDReturnOweDet_tblGoods_GoodsSpec:'+param4,'',"","","2",'',"2",false); 
 addCols(PDReturnOweDetData,'PDReturnOweDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDReturnOweDet:GoodsCode:PDReturnOweDet_tblGoods_BaseUnit:'+param4,'',"","","2",'',"2",false); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","3",'',"2",false); 
 putChildValidateItem("PDReturnOweDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","4",'',"2",false); 
 putChildValidateItem("PDReturnOweDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","5",'',"2",false); 
 putChildValidateItem("PDReturnOweDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_color','色彩','60',2,true,'',100,-2,'','',"","","6",'',"2",false); 
 putChildValidateItem("PDReturnOweDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"2",false); 
 putChildValidateItem("PDReturnOweDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","8",'',"2",false); 
 putChildValidateItem("PDReturnOweDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","9",'',"2",false); 
 putChildValidateItem("PDReturnOweDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_Qty','还回数','80',1,false,'',0,0,'','',"Qty","","10",'',"0",false); 
 putChildValidateItem("PDReturnOweDet_Qty","还回数","double","SeqQty",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_Price','单价','80',1,true,'',0,-2,'','',"priceIdentifier","","11",'',"0",false); 
 putChildValidateItem("PDReturnOweDet_Price","单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_Amount','金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","12",'',"8",false); 
 putChildValidateItem("PDReturnOweDet_Amount","金额","double","",true,minValue,maxValue,false,''); 
 ...
 addCols(PDReturnOweDetData,'PDReturnOweDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"Price","","18",'',"0",false); 
 putChildValidateItem("PDReturnOweDet_SecQty","辅助数量","int","BillDiscount",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","19",'',"3",false); 
 putChildValidateItem("PDReturnOweDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_Remark','备注','120',2,true,'',8000,0,'','',"","","21",'',"0",false); 
 putChildValidateItem("PDReturnOweDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_Seq','序列号','200',2,true,'',100,-2,'','',"","","22",'',"0",false); 
 putChildValidateItem("PDReturnOweDet_Seq","序列号","any","",true,minValue,100,false,''); 
 
 
 addCols(PDReturnOweDetData,'PDReturnOweDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDReturnOwe&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDReturnOwe,PDReturnOweDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDReturnOwe&allTableName=PDReturnOwe,PDReturnOweDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDReturnOwe&allTableName=PDReturnOweDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDReturnOwe&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDReturnOwe&allTableName=PDReturnOweDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDReturnOwe&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDReturnOwe&designId=5F1B041CCC394DD98E5723486F74&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDReturnOwe&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDReturnOwe&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDReturnOwe&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '欠料还回单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDReturnOwe&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
