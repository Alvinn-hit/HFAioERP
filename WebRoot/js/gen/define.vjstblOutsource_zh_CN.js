...
{ 
 cFireEvent('tblOutsourceDet_Qty','0'); 
 cFireEvent('tblOutsourceDet_Price','0'); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblOutsource&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblOutsource&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblOutsource&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblOutsource&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblOutsourceDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblOutsourceDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblOutsourceDet"){
...
function execStat2(){
 execStatOne('tblOutsourceDet_BuyOrderQty',2);
 execStatOne('tblOutsourceDet_UQty',3);
 execStatOne('tblOutsourceDet_Qty',4);
 execStatOne('tblOutsourceDet_SpareQty',4);
 execStatOne('tblOutsourceDet_Amount',4);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=d2a266cc_1307100953061041741&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=d2a266cc_1307100953061041741&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblOutsource\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblOutsource\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblOutsourceDet_Qty',"tblOutsourceDet"); 
 delFireEventOne(rowId,tableName,'tblOutsourceDet_Price',"tblOutsourceDet"); 
 delFireEventOne(rowId,tableName,'tblOutsourceDet_Amount',"tblOutsourceDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'Spare',dis:'备品率',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblOutsourceDet_BuyOrderQty',dis:'订单数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOutsourceDet_UQty',dis:'用量',digit:3,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOutsourceDet_Qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOutsourceDet_SpareQty',dis:'士碑数',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOutsourceDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOutsourceDet_Amount',dis:'金额',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblOutsource",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblOutsource&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblOutsource&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"d2a266cc_1307100953061041741",
 MOOP:"add",
...
function resetSubmit(fieldName){
 if(fieldName == "FeedCompany"){
 var saveField=n("FeedCompany"); 
 saveField[0].value = "";
 var obj = m("tbl_ViewCompanyTotal_classCode");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblFeed_ComFullName");
 saveField[0].value = ""; 
 m("tbl_tblFeed_ComFullName").value = "" ;
 }
 ...
 var saveField=n("TradeDays"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_SettleCys");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("Spare"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_Spare");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("deliverAddress"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_ShippingAddress");
 if(obj != null){
 obj.value = "";
 }
 ...
 if(fieldName == "BaseMat"){
 var saveField=n("BaseMat"); 
 saveField[0].value = "";
 var obj = m("tbl_tblBaseMat_MatNm");
 if(obj != null){
 obj.value = "";
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=d2a266cc_1307100953061041741&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=d2a266cc_1307100953061041741&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=d2a266cc_1307100953061041741&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblOutsource"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblOutsource&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblOutsource&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 for(var j=0;j<r(0,'SpareQty').length;j++){
r(0,"SpareQty")[j].value=f(nn('Spare',0)*rn(0,"Qty",j),4);
}
...
r(0,"Tax")[j].value=f(nn('Tax',0),4);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TaxAmount')[0].value=f(Number(sm('tblOutsourceDet_TaxAmount'))-Number(nn('DiscountAmount',0)),4);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_Qty'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),4);
r(0,"TaxPrice")[p].value=f((Number(nn('Tax',0))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"TaxAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"TaxPrice",p),4);
n('TotalAmount')[0].value=f(sm('tblOutsourceDet_DisBackAmt'),4);
n('TotalAmount')[0].value=f(sm('tblOutsourceDet_DisBackAmt'),4);
n('TotalTaxAmount')[0].value=f(sm('tblOutsourceDet_TaxAmount')-nn('DiscountAmount',0),4);
r(0,"CoTaxAmt")[p].value=f(Number(rn(0,"TaxAmount",p))-Number(rn(0,"DisBackAmt",p)),4);
n('TotalCoTaxAmt')[0].value=f(sm('tblOutsourceDet_CoTaxAmt'),4);
r(0,"SpareQty")[p].value=f(nn('Spare',0)*rn(0,"Qty",p),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),4);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblOutsourceDet_CoTaxAmt'),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_Amount'); 
...
n('TotalCoTaxAmt')[0].value=f(sm('tblOutsourceDet_CoTaxAmt'),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_DisPrice'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
r(0,"Discount")[p].value=f(rn(0,"DisPrice",p)/rn(0,"Price",p),4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),4);
r(0,"TaxPrice")[p].value=f((Number(nn('Tax',0))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"TaxAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"TaxPrice",p),4);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblOutsourceDet_CoTaxAmt'),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_DisAmount'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)-rn(0,"DisAmount",p),4);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblOutsourceDet_CoTaxAmt'),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_DisBackAmt'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"DisBackAmt",p)/rn(0,"Qty",p),4);
r(0,"Discount")[p].value=f(rn(0,"DisPrice",p)/rn(0,"Price",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblOutsourceDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_Discount'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),4);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblOutsourceDet_CoTaxAmt'),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_Tax'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblOutsourceDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_TaxPrice'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"TaxPrice",p)*rn(0,"Qty",p),4);
r(0,"Price")[p].value=f(rn(0,"TaxPrice",p)/((Number(nn('Tax',0))+1)*rn(0,"Discount",p)),4);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblOutsourceDet_CoTaxAmt'),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_TaxAmount'); 
...
n('TotalCoTaxAmt')[0].value=f(sm('tblOutsourceDet_CoTaxAmt'),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_CurRate'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblOutsourceDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_ConversionRate'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblOutsourceDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutsourceDet_PresentSampleType'); 
 if(r(0,"PresentSampleType")[p].value==1||r(0,"PresentSampleType")[p].value==2)r(0,"Price")[p].value=f(0,4);
r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),4);
...
childData[childCount]='tblOutsourceDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='委外加工明细';
childCount++;
 var tblOutsourceDetData = new gridData();
 gridDatas[gridDataNum] = tblOutsourceDetData;
 gridDataNum++;
 var tblOutsourceDet=1;
 ...
 var param4=""; 
 param4=param4+":1采购订单号@BuyOrderId"; 
 
 addCols(tblOutsourceDetData,'tblOutsourceDet_goods_GoodsNumber','主料编号','180',0,true,'',1000000,2,'tblOutsourceDet:GoodsCode:tblOutsourceDet_goods_GoodsNumber:'+param4,"","","","3","","2",false);
 ...
 var param4=""; 
 param4=param4+":1物料名称@tblOutsourceDet_GoodsCode";
 param4=param4+":1加工厂商@CompanyCode"; 
 ...
 addCols(tblOutsourceDetData,'tblOutsourceDet_tblGoods_GoodsNumber','物料编号','180',18,true,'',1000000,2,'tblOutsourceDet:GoodsCode:tblOutsourceDet_tblGoods_GoodsNumber:'+param4,"","","","3","","2",false);
 ...
 addCols(tblOutsourceDetData,'tblOutsourceDet_tblGoods_GoodsFullName','物料名称','180',2,true,'',1000000,2,'tblOutsourceDet:GoodsCode:tblOutsourceDet_tblGoods_GoodsFullName:'+param4,"","","","3","","2",false);
 ...
 param4=param4+":1加工厂商@CompanyCode"; 
      ...
      var param4="";
 addCols(tblOutsourceDetData,'tblOutsourceDet_GoodsSize','电镀(镶石)规格','200',2,true,'',200,2,'tblOutsourceDet:GoodsSize:tblOutsourceDet_GoodsSize:'+param4,"","","","4",'',"2",true);
 putChildValidateItem("tblOutsourceDet_GoodsSize","电镀(镶石)规格","any","",true,minValue,200,false,'',true);
       addCols(tblOutsourceDetData,'tblOutsourceDet_BuyOrderQty','订单数量','80',1,true,'0',0,8,'','',"","","30",'',"8",false); 
 putChildValidateItem("tblOutsourceDet_BuyOrderQty","订单数量","double","",true,minValue,maxValue,true,'0',true); 
      addCols(tblOutsourceDetData,'tblOutsourceDet_UQty','用量','60',1,true,'0',0,0,'','',"","","31",'',"0",false); 
 putChildValidateItem("tblOutsourceDet_UQty","用量","double","",true,minValue,maxValue,true,'0',true); 
      addCols(tblOutsourceDetData,'tblOutsourceDet_Qty','数量','60',1,false,'0',0,0,'','',"Qty","","32",'',"0",false); 
 putChildValidateItem("tblOutsourceDet_Qty","数量","double","",false,minValue,maxValue,true,'0',true); 
      addCols(tblOutsourceDetData,'tblOutsourceDet_SpareQty','士碑数','80',1,true,'0',0,0,'','',"","","33",'',"0",false); 
 putChildValidateItem("tblOutsourceDet_SpareQty","士碑数","double","",true,minValue,maxValue,true,'0',true); 
      var param4="";
 param4=param4+":1物料名称@tblOutsourceDet_GoodsCode";
 param4=param4+":1加工厂商@CompanyCode"; 
 addCols(tblOutsourceDetData,'tblOutsourceDet_Price','单价','80',1,true,'0',0,2,'tblOutsourceDet:Price:tblOutsourceDet_Price:'+param4,"1","priceIdentifier","","35",'',"2",true);
 putChildValidateItem("tblOutsourceDet_Price","单价","double","",true,minValue,maxValue,true,'0',true);
       addCols(tblOutsourceDetData,'tblOutsourceDet_Amount','金额','100',1,true,'0',0,0,'','',"AmountIdentifier","","38",'',"0",false); 
 putChildValidateItem("tblOutsourceDet_Amount","金额","double","",true,minValue,maxValue,true,'0',true); 
    addCols(tblOutsourceDetData,'tblOutsourceDet_BuyOrderID','采购单号','224',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_BuyOrderID","采购单号","any","",true,minValue,100,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_BuyOrderNo','采购单号','120',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_BuyOrderNo","采购单号","any","",true,minValue,100,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_WatchCode','主料编号','200',2,true,'',1000,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_WatchCode","主料编号","any","",true,minValue,1000,false,''); 
 var param4="";
 addCols(tblOutsourceDetData,'tblOutsourceDet_GoodsCode','物料名称','80',2,false,'',100,-2,'','','',"",'','',"2",false);
 putChildValidateItem("tblOutsourceDet_GoodsCode","物料名称","any","",false,minValue,100,false,'',true);
           addCols(tblOutsourceDetData,'tblOutsourceDet_BatchNo','批号','120',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_BatchNo","批号","any","",true,minValue,100,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_Inch','宽度','70',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_Inch","宽度","any","",true,minValue,100,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_Hue','颜色','70',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_Hue","颜色","any","",true,minValue,100,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_yearNO','高度','70',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_yearNO","高度","any","",true,minValue,100,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_ProDate','生产日期','80',5,true,'',10,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_ProDate","生产日期","date","",true,minValue,10,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_Availably','保质期限','80',5,true,'',10,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_Availably","保质期限","date","",true,minValue,10,false,''); 
     addCols(tblOutsourceDetData,'tblOutsourceDet_ArriveDate','到货日期','80',5,true,'',10,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_ArriveDate","到货日期","date","",true,minValue,10,false,''); 
             addCols(tblOutsourceDetData,'tblOutsourceDet_DisPrice','折后单价','80',1,true,'',0,-2,'','','',"",'','',"6",false); 
 putChildValidateItem("tblOutsourceDet_DisPrice","折后单价","double","",true,minValue,maxValue,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_DisAmount','折扣金额','80',1,true,'',0,-2,'','','',"",'','',"6",false); 
 putChildValidateItem("tblOutsourceDet_DisAmount","折扣金额","double","",true,minValue,maxValue,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_DisBackAmt','折后金额','80',1,true,'',0,-2,'','','',"",'','',"6",false); 
 putChildValidateItem("tblOutsourceDet_DisBackAmt","折后金额","double","",true,minValue,maxValue,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_Discount','折扣','0',1,true,'1',0,-2,'','','',"",'','',"6",false); 
 putChildValidateItem("tblOutsourceDet_Discount","折扣","double","",true,minValue,maxValue,true,'1'); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_Tax','税率','60',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_Tax","税率","double","",true,minValue,maxValue,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_TaxPrice','含税单价','80',1,true,'0',0,-2,'','','',"",'','',"6",false); 
 putChildValidateItem("tblOutsourceDet_TaxPrice","含税单价","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_TaxAmount','含税金额','100',1,true,'0',0,-2,'','','',"",'','',"6",false); 
 putChildValidateItem("tblOutsourceDet_TaxAmount","含税金额","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_CoTaxAmt','税额','70',1,true,'0',0,-2,'','','',"",'','',"6",false); 
 putChildValidateItem("tblOutsourceDet_CoTaxAmt","税额","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_CurRate','汇率','80',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_CurRate","汇率","double","",true,minValue,maxValue,true,'0'); 
    addCols(tblOutsourceDetData,'tblOutsourceDet_ConversionRate','换算率','80',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_ConversionRate","换算率","any","",true,minValue,100,false,''); 
    addCols(tblOutsourceDetData,'tblOutsourceDet_UnitTaxPrice','辅助单位含税单价','80',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_UnitTaxPrice","辅助单位含税单价","double","",true,minValue,maxValue,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_TotalPayAmt','累计付款金额','80',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_TotalPayAmt","累计付款金额","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_TrackNo','追踪单号','120',2,true,'',120,-2,'','',"","","100",'',"6",false); 
 putChildValidateItem("tblOutsourceDet_TrackNo","追踪单号","any","",true,minValue,120,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_StockCode','仓库','80',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_StockCode","仓库","any","",true,minValue,100,true,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_Remark','备注','200',2,true,'',255,-2,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblOutsourceDet_Remark","备注","any","",true,minValue,255,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_NeedPayAmt','待付款金额','80',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_NeedPayAmt","待付款金额","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_color','色彩','60',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_color","色彩","any","",true,minValue,100,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_colorNV','colorNV','80',2,true,'',500,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_colorNV","colorNV","any","",true,minValue,500,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_PresentSampleType','赠(样)品','80',2,true,'',10,-2,'','','',"",'','',"1",false); 
 putChildValidateItem("tblOutsourceDet_PresentSampleType","赠(样)品","any","",true,minValue,10,false,''); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_InPayQty','入库付款数','80',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_InPayQty","入库付款数","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_NotPayQty','未付款数量','80',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_NotPayQty","未付款数量","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblOutsourceDetData,'tblOutsourceDet_PayQty','付款数量','80',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutsourceDet_PayQty","付款数量","double","",true,minValue,maxValue,true,'0'); 
 
 addCols(tblOutsourceDetData,'tblOutsourceDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblOutsource&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblOutsource,tblOutsourceDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblOutsource&allTableName=tblOutsource,tblOutsourceDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblOutsource&allTableName=tblOutsourceDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblOutsource&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblOutsource&allTableName=tblOutsourceDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=d2a266cc_1307100953061041741&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=d2a266cc_1307100953061041741&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=d2a266cc_1307100953061041741&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=d2a266cc_1307100953061041741&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblOutsource&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblOutsource&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblOutsource&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblOutsource&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblOutsource&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '委外加工';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblOutsource&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
