...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblEnquiryMidBody&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblEnquiryMidBody&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblEnquiryMidBody&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblEnquiryMidBody&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblEnquiryMidBodyDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblEnquiryMidBodyDet1Table"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblEnquiryMidBodyDet2Table"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblEnquiryMidBodyDet3Table"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblEnquiryMidBodyDet4Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblEnquiryMidBodyDet"){
 
  }
 if(typeof(tableName)!="undefined" && tableName=="tblEnquiryMidBodyDet1"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblEnquiryMidBodyDet2"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblEnquiryMidBodyDet3"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblEnquiryMidBodyDet4"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblEnquiryMidBodyDet"){
 
  }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblEnquiryMidBodyDet1"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblEnquiryMidBodyDet2"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblEnquiryMidBodyDet3"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblEnquiryMidBodyDet4"){
 
 }
 }
}

 var tblName=new Array(5);
 
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblEnquiryMidBodyDet"; 
 tblName[1]="tblEnquiryMidBodyDet1"; 
 tblName[2]="tblEnquiryMidBodyDet2"; 
 tblName[3]="tblEnquiryMidBodyDet3"; 
 tblName[4]="tblEnquiryMidBodyDet4"; 
...
function execStat2(){
 execStatOne('tblEnquiryMidBodyDet_Quantity',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=84ac2fc4_1405051633395780338&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=84ac2fc4_1405051633395780338&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblEnquiryMidBody\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblEnquiryMidBody\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet_Price',dis:'价格',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet_ZBPrice',dis:'材料中标价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet_Quantity',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet1_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet1_Qty',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet1_Amount',dis:'金额',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet2_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet2_Qty',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet2_Amount',dis:'金额',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet3_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet3_Qty',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet3_Amount',dis:'金额',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet4_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet4_Qty',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryMidBodyDet4_Amount',dis:'金额',digit:0,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblEnquiryMidBody",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblEnquiryMidBody&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblEnquiryMidBody&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"84ac2fc4_1405051633395780338",
 MOOP:"add",
...
function resetSubmit(fieldName){
 if(fieldName == "EnquiryNo"){
 var saveField=n("ProjectName"); 
 saveField[0].value = "";
 var obj = m("tbl_tblEnquiry_ProjectName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("ProjectNo"); 
 saveField[0].value = "";
 var obj = m("tbl_tblEnquiry_ProjectNo");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("EnquiryNo"); 
 saveField[0].value = "";
 var obj = m("tbl_tblEnquiry_BillNo");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BillTitle"); 
 saveField[0].value = "";
 var obj = m("tbl_tblEnquiry_BillTitle");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblEnquiry_Bil");
 saveField[0].value = ""; 
 m("tbl_tblEnquiry_Bil").value = "" ;
 }
 if(fieldName == "Supplier"){
 var saveField=n("Supplier"); 
 saveField[0].value = "";
 var obj = m("tbl_CRMClientInfo_ClientName");
 if(obj != null){
 obj.value = "";
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84ac2fc4_1405051633395780338&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84ac2fc4_1405051633395780338&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=84ac2fc4_1405051633395780338&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblEnquiryMidBody"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblEnquiryMidBody&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblEnquiryMidBody&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblEnquiryMidBodyDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='采购询价中间表明细';
childCount++;
 var tblEnquiryMidBodyDetData = new gridData();
 gridDatas[gridDataNum] = tblEnquiryMidBodyDetData;
 gridDataNum++;
 var tblEnquiryMidBodyDet=1;
 ...
 var param4="";
 param4=param4+":1项目编号@ProjectNo"; 
 param4=param4+":1询价单编号@EnquiryNo"; 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_CuttingId','下料单号','100',2,true,'',100,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("tblEnquiryMidBodyDet_CuttingId","下料单号","any","",true,minValue,100,false,'');
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_tblCutting_billNo','引用下料单号','120',2,true,'',1000000,2,'tblEnquiryMidBodyDet:CuttingId:tblEnquiryMidBodyDet_tblCutting_billNo:'+param4,'',"","","0",'',"2",false); 
 ...
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_WareNo','材料编码','100',2,false,'',100,8,'','',"","","1",'',"8",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_WareNo","材料编码","any","",false,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_WareName','材料名称','150',2,false,'',200,8,'','',"","","2",'',"8",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_WareName","材料名称","any","",false,minValue,200,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_Specification','规格/型号','100',2,true,'',100,8,'','',"","","3",'',"8",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_Specification","规格/型号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_Brand','品牌','100',2,true,'',100,8,'','',"","","4",'',"8",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_Brand","品牌","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_WareUnit','计量单位','100',2,true,'',100,8,'','',"","","5",'',"8",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_WareUnit","计量单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_Price','价格','100',1,false,'0',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_Price","价格","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_ZBPrice','材料中标价','100',1,true,'',0,8,'','',"","","7",'',"8",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_ZBPrice","材料中标价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_Quantity','数量','100',1,true,'',0,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_Quantity","数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_ArriveTime','到场时间','100',5,true,'',100,-2,'','',"","","9",'',"3",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_ArriveTime","到场时间","date","",true,minValue,100,false,''); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_CuttingMemo','下料单备注','100',2,true,'',200,-2,'','',"","","10",'',"3",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_CuttingMemo","下料单备注","any","",true,minValue,200,false,''); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_Position','使用位置','150',2,true,'',200,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_Position","使用位置","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_Memo','备注（是否含税，是否含运，是否含卸货费、加工费等）','400',2,true,'',8000,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_Memo","备注（是否含税，是否含运，是否含卸货费、加工费等）","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_MaterialDetId','材料总计划明细ID','100',2,true,'',100,-2,'','',"","","13",'',"2",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_MaterialDetId","材料总计划明细ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_BuyBrand','采购品牌','100',2,true,'',100,0,'','',"","","14",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet_BuyBrand","采购品牌","any","",true,minValue,100,false,'',true); 
 
 
 
 
 addCols(tblEnquiryMidBodyDetData,'tblEnquiryMidBodyDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblEnquiryMidBodyDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='加工费';
childCount++;
 var tblEnquiryMidBodyDet1Data = new gridData();
 gridDatas[gridDataNum] = tblEnquiryMidBodyDet1Data;
 gridDataNum++;
 var tblEnquiryMidBodyDet1=1;
 
 
 addCols(tblEnquiryMidBodyDet1Data,'tblEnquiryMidBodyDet1_CostName','加工费名称','200',2,true,'',200,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet1_CostName","加工费名称","any","",true,minValue,200,false,'',true); 
 
 
 
 
 
 addCols(tblEnquiryMidBodyDet1Data,'tblEnquiryMidBodyDet1_Unit','单位','60',2,true,'',20,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet1_Unit","单位","any","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet1Data,'tblEnquiryMidBodyDet1_Price','单价','60',1,true,'0',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet1_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet1Data,'tblEnquiryMidBodyDet1_Qty','数量','60',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet1_Qty","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet1Data,'tblEnquiryMidBodyDet1_Amount','金额','80',1,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet1_Amount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 addCols(tblEnquiryMidBodyDet1Data,'tblEnquiryMidBodyDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblEnquiryMidBodyDet2';
tabIsNulls[childCount]='0';
tabDiss[childCount]='运费';
childCount++;
 var tblEnquiryMidBodyDet2Data = new gridData();
 gridDatas[gridDataNum] = tblEnquiryMidBodyDet2Data;
 gridDataNum++;
 var tblEnquiryMidBodyDet2=1;
 
 
 addCols(tblEnquiryMidBodyDet2Data,'tblEnquiryMidBodyDet2_CostName','运费名称','200',2,true,'',200,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet2_CostName","运费名称","any","",true,minValue,200,false,'',true); 
 
 
 
 
 
 addCols(tblEnquiryMidBodyDet2Data,'tblEnquiryMidBodyDet2_Unit','单位','60',2,true,'',20,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet2_Unit","单位","any","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet2Data,'tblEnquiryMidBodyDet2_Price','单价','60',1,true,'0',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet2_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet2Data,'tblEnquiryMidBodyDet2_Qty','数量','60',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet2_Qty","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet2Data,'tblEnquiryMidBodyDet2_Amount','金额','80',1,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet2_Amount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 addCols(tblEnquiryMidBodyDet2Data,'tblEnquiryMidBodyDet2_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblEnquiryMidBodyDet3';
tabIsNulls[childCount]='0';
tabDiss[childCount]='税费';
childCount++;
 var tblEnquiryMidBodyDet3Data = new gridData();
 gridDatas[gridDataNum] = tblEnquiryMidBodyDet3Data;
 gridDataNum++;
 var tblEnquiryMidBodyDet3=1;
 
 
 addCols(tblEnquiryMidBodyDet3Data,'tblEnquiryMidBodyDet3_CostName','税费名称','200',2,true,'',200,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet3_CostName","税费名称","any","",true,minValue,200,false,'',true); 
 
 
 
 
 
 addCols(tblEnquiryMidBodyDet3Data,'tblEnquiryMidBodyDet3_Unit','单位','60',2,true,'',20,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet3_Unit","单位","any","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet3Data,'tblEnquiryMidBodyDet3_Price','单价','60',1,true,'0',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet3_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet3Data,'tblEnquiryMidBodyDet3_Qty','数量','60',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet3_Qty","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet3Data,'tblEnquiryMidBodyDet3_Amount','金额','80',1,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet3_Amount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 addCols(tblEnquiryMidBodyDet3Data,'tblEnquiryMidBodyDet3_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblEnquiryMidBodyDet4';
tabIsNulls[childCount]='0';
tabDiss[childCount]='安装费';
childCount++;
 var tblEnquiryMidBodyDet4Data = new gridData();
 gridDatas[gridDataNum] = tblEnquiryMidBodyDet4Data;
 gridDataNum++;
 var tblEnquiryMidBodyDet4=1;
 
 
 addCols(tblEnquiryMidBodyDet4Data,'tblEnquiryMidBodyDet4_CostName','安装费名称','200',2,true,'',200,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet4_CostName","安装费名称","any","",true,minValue,200,false,'',true); 
 
 
 
 
 
 addCols(tblEnquiryMidBodyDet4Data,'tblEnquiryMidBodyDet4_Unit','单位','60',2,true,'',20,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet4_Unit","单位","any","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet4Data,'tblEnquiryMidBodyDet4_Price','单价','60',1,true,'0',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet4_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet4Data,'tblEnquiryMidBodyDet4_Qty','数量','60',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet4_Qty","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryMidBodyDet4Data,'tblEnquiryMidBodyDet4_Amount','金额','80',1,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblEnquiryMidBodyDet4_Amount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 addCols(tblEnquiryMidBodyDet4Data,'tblEnquiryMidBodyDet4_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblEnquiryMidBody&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblEnquiryMidBody,tblEnquiryMidBodyDet,tblEnquiryMidBodyDet1,tblEnquiryMidBodyDet2,tblEnquiryMidBodyDet3,tblEnquiryMidBodyDet4,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblEnquiryMidBody&allTableName=tblEnquiryMidBody,tblEnquiryMidBodyDet,tblEnquiryMidBodyDet1,tblEnquiryMidBodyDet2,tblEnquiryMidBodyDet3,tblEnquiryMidBodyDet4,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblEnquiryMidBody&allTableName=tblEnquiryMidBodyDet,tblEnquiryMidBodyDet1,tblEnquiryMidBodyDet2,tblEnquiryMidBodyDet3,tblEnquiryMidBodyDet4,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblEnquiryMidBody&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblEnquiryMidBody&allTableName=tblEnquiryMidBodyDet,tblEnquiryMidBodyDet1,tblEnquiryMidBodyDet2,tblEnquiryMidBodyDet3,tblEnquiryMidBodyDet4,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84ac2fc4_1405051633395780338&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84ac2fc4_1405051633395780338&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblEnquiryMidBodyDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=84ac2fc4_1405051633395780338&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=84ac2fc4_1405051633395780338&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblEnquiryMidBody&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=84ac2fc4_1405051633395780338&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblEnquiryMidBody&designId=971e12de_14ccc71141242030296&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblEnquiryMidBody&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblEnquiryMidBody&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblEnquiryMidBody&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '供应商报价单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblEnquiryMidBody&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
