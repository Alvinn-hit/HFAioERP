...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblWatchBuckle&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblWatchBuckle&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblWatchBuckle&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblWatchBuckle&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchBuckleDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchBuckleDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblWatchBuckleDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblWatchBuckleDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchBuckleDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchBuckleDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblWatchBuckleDet"; 
 tblName[1]="tblWatchBuckleDet1"; 
...
function execStat2(){
 execStatOne('tblWatchBuckleDet_Qty',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblWatchBuckle\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblWatchBuckle\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblWatchBuckleDet_Qty',dis:'用量',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblWatchBuckle",
...
function exeQuoteDraft(varStr,moduleType){
 ...
 window.location.href="/UserFunctionAction.do?tableName="+varTableName+"&operation=7&moduleType="+moduleType+"&parentTableName=tblSampleOrder&f_brother=1f7c2a27_1411101112503881427&keyId="+varStr+"&saveDraft=quoteDraft&winCurIndex="+winCurIndex+"&popWinName="+$("#popWinName").val() ; 
}
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchBuckle&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchBuckle&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblWatchBuckle"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblWatchBuckle&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchBuckle&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblWatchBuckleDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='扣制物料明细';
childCount++;
 var tblWatchBuckleDetData = new gridData();
 gridDatas[gridDataNum] = tblWatchBuckleDetData;
 gridDataNum++;
 var tblWatchBuckleDet=1;
  var param4="";
 param4=param4+":1样板编号@SampleOrderID"; 
 addCols(tblWatchBuckleDetData,'tblWatchBuckleDet_GoodsNumber','物料编号','150',2,false,'',500,2,'tblWatchBuckleDet:GoodsNumber:tblWatchBuckleDet_GoodsNumber:'+param4,"","","","2",'',"2",true);
 putChildValidateItem("tblWatchBuckleDet_GoodsNumber","物料编号","any","",false,minValue,500,false,'',true);
  addCols(tblWatchBuckleDetData,'tblWatchBuckleDet_CaseGrind','扣制打磨','200',2,true,'',200,8,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchBuckleDet_CaseGrind","扣制打磨","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchBuckleDetData,'tblWatchBuckleDet_CaseElectro','扣制电镀','200',2,true,'',200,8,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblWatchBuckleDet_CaseElectro","扣制电镀","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchBuckleDetData,'tblWatchBuckleDet_GrindRemark','打磨说明','200',2,true,'',500,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblWatchBuckleDet_GrindRemark","打磨说明","any","",true,minValue,500,false,'',true); 
  addCols(tblWatchBuckleDetData,'tblWatchBuckleDet_ElectroRemark','电镀说明','200',2,true,'',500,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblWatchBuckleDet_ElectroRemark","电镀说明","any","",true,minValue,500,false,'',true); 
  addCols(tblWatchBuckleDetData,'tblWatchBuckleDet_Qty','用量','80',1,true,'1',0,0,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblWatchBuckleDet_Qty","用量","double","",true,minValue,maxValue,true,'1',true); 
  addCols(tblWatchBuckleDetData,'tblWatchBuckleDet_Remark','备注','200',2,true,'',500,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblWatchBuckleDet_Remark","备注","any","",true,minValue,500,false,'',true); 
   addCols(tblWatchBuckleDetData,'tblWatchBuckleDet_GoodsCode','物料名称','100',2,true,'',1000,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblWatchBuckleDet_GoodsCode","物料名称","any","",true,minValue,1000,false,''); 
             
 addCols(tblWatchBuckleDetData,'tblWatchBuckleDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblWatchBuckleDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='扣制变更明细';
childCount++;
 var tblWatchBuckleDet1Data = new gridData();
 gridDatas[gridDataNum] = tblWatchBuckleDet1Data;
 gridDataNum++;
 var tblWatchBuckleDet1=1;
  addCols(tblWatchBuckleDet1Data,'tblWatchBuckleDet1_ChangeContent','变更内容','200',2,false,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblWatchBuckleDet1_ChangeContent","变更内容","any","",false,minValue,500,false,'',true); 
  addCols(tblWatchBuckleDet1Data,'tblWatchBuckleDet1_ChangeDate','变更日期','90',5,false,'2013-12-24',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchBuckleDet1_ChangeDate","变更日期","date","",false,minValue,10,true,'2013-12-24',true); 
  addCols(tblWatchBuckleDet1Data,'tblWatchBuckleDet1_ChangePer','变更人员','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchBuckleDet1_ChangePer","变更人员","any","",true,minValue,100,false,'',true); 
  addCols(tblWatchBuckleDet1Data,'tblWatchBuckleDet1_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchBuckleDet1_Remark","备注","any","",true,minValue,500,false,'',true); 
       
 addCols(tblWatchBuckleDet1Data,'tblWatchBuckleDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblWatchBuckle&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=1f7c2a27_1411101112503881427&noback="+noback+"&parentCode=&operation=6&parentTableName=tblSampleOrder&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblWatchBuckle,tblWatchBuckleDet,tblWatchBuckleDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchBuckle&allTableName=tblWatchBuckle,tblWatchBuckleDet,tblWatchBuckleDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchBuckle&allTableName=tblWatchBuckleDet,tblWatchBuckleDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchBuckle&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchBuckle&allTableName=tblWatchBuckleDet,tblWatchBuckleDet1,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblWatchBuckleDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblWatchBuckle&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblWatchBuckle&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblWatchBuckle&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchBuckle&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblWatchBuckle&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '样板订单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblWatchBuckle&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
