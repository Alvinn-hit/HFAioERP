...
{ 
 cFireEvent('DXReturnStockDet_Qty',''); 
 cFireEvent('DXReturnStockDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=DXReturnStock&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=DXReturnStock&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=DXReturnStock&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=DXReturnStock&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "DXReturnStockDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('DXReturnStockDet_GoodsCode');
 ...
 var z=n('DXReturnStockDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('DXReturnStockDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="DXReturnStockDet"){
 
 copyFields=n('DXReturnStockDet_StockCode');
 ...
 copyFields=n('DXReturnStockDet_GoodsCode');
 if((order > 3 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var DXReturnStockDet_tblGoods_GoodsNumber = n('DXReturnStockDet_tblGoods_GoodsNumber');
 if(typeof(DXReturnStockDet_tblGoods_GoodsNumber) != 'undefined' && DXReturnStockDet_tblGoods_GoodsNumber.length >index){
 DXReturnStockDet_tblGoods_GoodsNumber[index].value = DXReturnStockDet_tblGoods_GoodsNumber[index-1].value;
 }
 var DXReturnStockDet_tblGoods_GoodsFullName = n('DXReturnStockDet_tblGoods_GoodsFullName');
 if(typeof(DXReturnStockDet_tblGoods_GoodsFullName) != 'undefined' && DXReturnStockDet_tblGoods_GoodsFullName.length >index){
 DXReturnStockDet_tblGoods_GoodsFullName[index].value = DXReturnStockDet_tblGoods_GoodsFullName[index-1].value;
 }
 var DXReturnStockDet_tblGoods_GoodsSpec = n('DXReturnStockDet_tblGoods_GoodsSpec');
 if(typeof(DXReturnStockDet_tblGoods_GoodsSpec) != 'undefined' && DXReturnStockDet_tblGoods_GoodsSpec.length >index){
 DXReturnStockDet_tblGoods_GoodsSpec[index].value = DXReturnStockDet_tblGoods_GoodsSpec[index-1].value;
 }
 var DXReturnStockDet_tblGoods_BaseUnit = n('DXReturnStockDet_tblGoods_BaseUnit');
 if(typeof(DXReturnStockDet_tblGoods_BaseUnit) != 'undefined' && DXReturnStockDet_tblGoods_BaseUnit.length >index){
 DXReturnStockDet_tblGoods_BaseUnit[index].value = DXReturnStockDet_tblGoods_BaseUnit[index-1].value;
 }
 }
   }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="DXReturnStockDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="DXReturnStockDet"; 
 var goodsCodeLogicValidate_DXReturnStockDet='@GN:Seq:Qty:tblSalesReturnStockDet_GoodsSeqSet:tblSalesReturnStockDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'DXReturnStockDet',value:goodsCodeLogicValidate_DXReturnStockDet};
...
function execStat2(){
 execStatOne('DXReturnStockDet_Qty',2);
 execStatOne('DXReturnStockDet_Amount',2);
 execStatOne('DXReturnStockDet_ApplyQty',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"DXReturnStock\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"DXReturnStock\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'DXReturnStockDet_Qty',"DXReturnStockDet"); 
 delFireEventOne(rowId,tableName,'DXReturnStockDet_Price',"DXReturnStockDet"); 
 delFireEventOne(rowId,tableName,'DXReturnStockDet_Amount',"DXReturnStockDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'TotalAmount',dis:'单据总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'DXReturnStockDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'DXReturnStockDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'DXReturnStockDet_Amount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=DXReturnStock",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=DXReturnStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=DXReturnStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('DXReturnStockDet_Qty'); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "DXReturnStock"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=DXReturnStock&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=DXReturnStock&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
n('TotalAmount')[0].value=f(sm('DXReturnStockDet_DisBackAmt'),2);

n('TotalTaxAmount')[0].value=f(sm('DXReturnStockDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('DXReturnStockDet_CoTaxAmt'),2);

for(var j=0;j<r(0,'DisAmount').length;j++){
r(0,"DisAmount")[j].value=f(rn(0,"Amount",j)-rn(0,"DisBackAmt",j),2);
}

n('DisAmount')[0].value=f(sm('DXReturnStockDet_DisAmount'),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_Qty'); 
...
r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
n('DisAmount')[0].value=f(sm('DXReturnStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_Amount'); 
...
n('DisAmount')[0].value=f(sm('DXReturnStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,4);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('DXReturnStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_DisPrice'); 
...
n('DisAmount')[0].value=f(sm('DXReturnStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_DisBackAmt'); 
...
n('DisAmount')[0].value=f(sm('DXReturnStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_TaxPrice'); 
...
n('DisAmount')[0].value=f(sm('DXReturnStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_TaxAmount'); 
...
n('DisAmount')[0].value=f(sm('DXReturnStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_CurPrice'); 
...
n('TotalAmount')[0].value=f(sm('DXReturnStockDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('DXReturnStockDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(sm('DXReturnStockDet_CurAmount'),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
n('TotalCoTaxAmt')[0].value=f(sm('DXReturnStockDet_CoTaxAmt'),2);
n('DisAmount')[0].value=f(sm('DXReturnStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_CurAmount'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),2);
...
n('DisAmount')[0].value=f(sm('DXReturnStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_CostPrice'); 
 r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_CostAmount'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_PresentSampleType'); 
...
n('DisAmount')[0].value=f(sm('DXReturnStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXReturnStockDet_SecQty'); 
...
childData[childCount]='DXReturnStockDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='代销退货单明细表';
childCount++;
 var DXReturnStockDetData = new gridData();
 gridDatas[gridDataNum] = DXReturnStockDetData;
 gridDataNum++;
 var DXReturnStockDet=1;
 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_StockCode','仓库','80',2,true,'',100,-2,'','',"","copy","0",'',"3",false); 
 putChildValidateItem("DXReturnStockDet_StockCode","仓库","any","",true,minValue,100,true,''); 
 
 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_DXOutStockID','出库单号','80',2,true,'',32,-2,'','',"","","1",'',"2",false); 
 putChildValidateItem("DXReturnStockDet_DXOutStockID","出库单号","any","ManyBillQuote",true,minValue,32,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_DXOrderID','订单编号','80',2,true,'',32,-2,'','',"","","2",'',"3",false); 
 putChildValidateItem("DXReturnStockDet_DXOrderID","订单编号","any","",true,minValue,32,false,''); 
 ...
 param4=param4+":0仓库@DXReturnStockDet_StockCode";
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_GoodsCode','商品名称','120',2,false,'',100,-2,'','',"","triggerOrCopy","3",'',"2",true);
 putChildValidateItem("DXReturnStockDet_GoodsCode","商品名称","any","",false,minValue,100,false,'');
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'DXReturnStockDet:GoodsCode:DXReturnStockDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_tblGoods_GoodsFullName','商品名称','150',18,true,'',1000000,2,'DXReturnStockDet:GoodsCode:DXReturnStockDet_tblGoods_GoodsFullName:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'DXReturnStockDet:GoodsCode:DXReturnStockDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'DXReturnStockDet:GoodsCode:DXReturnStockDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","4",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","5",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","6",'',"2",false); 
 putChildValidateItem("DXReturnStockDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"2",false); 
 putChildValidateItem("DXReturnStockDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","8",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","9",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_Seq','序列号','130',2,true,'',5000,-2,'','',"","triggerCopy","10",'',"2",false); 
 putChildValidateItem("DXReturnStockDet_Seq","序列号","any","GoodsField",true,minValue,5000,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_Qty','数量','60',1,false,'',0,0,'','',"Qty","","11",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_Qty","数量","double","SeqQty",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_Price','单价','80',1,true,'',0,0,'','',"SPriceIdentifier","","12",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_Amount','金额','100',1,true,'',0,0,'','',"SAmountIdentifier","","13",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(DXReturnStockDetData,'DXReturnStockDet_Discount','折扣率%','100',1,true,'100',0,-2,'','',"Price","","19",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_Discount","折扣率%","double","BillDiscount",true,minValue,maxValue,true,'100'); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_DisPrice','折后单价','100',1,true,'',0,-2,'','',"SPriceIdentifier","","20",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_DisPrice","折后单价","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_DisBackAmt','折后金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","21",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_DisBackAmt","折后金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_DisAmount','折让金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","22",'',"8",false); 
 putChildValidateItem("DXReturnStockDet_DisAmount","折让金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_TaxPrice','含税单价','100',1,true,'',0,-2,'','',"SPriceIdentifier","","23",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_TaxPrice","含税单价","double","BillTax",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_TaxAmount','含税金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","24",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_TaxAmount","含税金额","double","BillTax",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_CoTaxAmt','税额','100',1,true,'0',0,-2,'','',"SAmountIdentifier","","25",'',"8",false); 
 putChildValidateItem("DXReturnStockDet_CoTaxAmt","税额","double","BillTax",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_CurPrice','外币单价','100',1,true,'',0,-2,'','',"SPriceIdentifier","","26",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_CurPrice","外币单价","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_CurAmount','外币金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","27",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_CurAmount","外币金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_CheckBarCode','复核码','160',2,true,'',200,-2,'','',"","","28",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_CheckBarCode","复核码","any","",true,minValue,200,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_ApplyQty','申请数','80',1,false,'0',0,-2,'','',"Qty","","29",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_ApplyQty","申请数","double","",false,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_CostPrice','成本单价','80',1,true,'',0,-2,'','',"priceIdentifier","","31",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_CostPrice","成本单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_CostAmount','成本总额','100',1,true,'0',0,-2,'','',"AmountIdentifier","","32",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_CostAmount","成本总额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_PresentSampleType','赠(样)品','60',2,true,'',20,-2,'','',"","","33",'',"1",false); 
 putChildValidateItem("DXReturnStockDet_PresentSampleType","赠(样)品","any","BillPresentSampleType",true,minValue,20,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_NoInvoiceQty','未开票数','224',1,true,'0',0,-2,'','',"Qty","","34",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_NoInvoiceQty","未开票数","double","",true,minValue,maxValue,true,'0'); 
 ...
 addCols(DXReturnStockDetData,'DXReturnStockDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","38",'',"100",false); 
 putChildValidateItem("DXReturnStockDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_ChangeAmt','调价金额','224',1,true,'0',0,-2,'','',"","","40",'',"3",false); 
 putChildValidateItem("DXReturnStockDet_ChangeAmt","调价金额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_AfterChangeAmt','调价后金额','224',1,true,'0',0,-2,'','',"","","41",'',"3",false); 
 putChildValidateItem("DXReturnStockDet_AfterChangeAmt","调价后金额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_NoInvoiceAmount','未开票金额','224',1,true,'0',0,-2,'','',"","","44",'',"3",false); 
 putChildValidateItem("DXReturnStockDet_NoInvoiceAmount","未开票金额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_NotPayQty','未付款数量','224',1,true,'',0,-2,'','',"Qty","","45",'',"3",false); 
 putChildValidateItem("DXReturnStockDet_NotPayQty","未付款数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_PayQty','付款数量','224',1,true,'',0,-2,'','',"Qty","","47",'',"3",false); 
 putChildValidateItem("DXReturnStockDet_PayQty","付款数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","48",'',"3",false); 
 putChildValidateItem("DXReturnStockDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_InvoiceAmount','已开票金额','224',1,true,'0',0,-2,'','',"","","49",'',"3",false); 
 putChildValidateItem("DXReturnStockDet_InvoiceAmount","已开票金额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","50",'',"3",false); 
 putChildValidateItem("DXReturnStockDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_Remark','备注','100',2,true,'',8000,0,'','',"","","51",'',"0",false); 
 putChildValidateItem("DXReturnStockDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(DXReturnStockDetData,'DXReturnStockDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=DXReturnStock&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "DXReturnStock,DXReturnStockDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=DXReturnStock&allTableName=DXReturnStock,DXReturnStockDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=DXReturnStock&allTableName=DXReturnStockDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=DXReturnStock&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=DXReturnStock&allTableName=DXReturnStockDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=DXReturnStock&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=DXReturnStock&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=DXReturnStock&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=DXReturnStock&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=DXReturnStock&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '代销退货单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=DXReturnStock&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
