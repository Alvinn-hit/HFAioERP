...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDGoodsReplace&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDGoodsReplace&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDGoodsReplace&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDGoodsReplace&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDGoodsReplaceDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDGoodsReplaceDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDGoodsReplaceDet"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDGoodsReplace\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDGoodsReplace\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDGoodsReplace",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDGoodsReplace&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDGoodsReplace&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDGoodsReplace"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDGoodsReplace&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDGoodsReplace&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='PDGoodsReplaceDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='替换料明细';
childCount++;
 var PDGoodsReplaceDetData = new gridData();
 gridDatas[gridDataNum] = PDGoodsReplaceDetData;
 gridDataNum++;
 var PDGoodsReplaceDet=1;
 ...
 addCols(PDGoodsReplaceDetData,'PDGoodsReplaceDet_GoodsCode','替换料','200',2,true,'',50,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("PDGoodsReplaceDet_GoodsCode","替换料","any","",true,minValue,50,false,'');
 
 
 addCols(PDGoodsReplaceDetData,'PDGoodsReplaceDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'PDGoodsReplaceDet:GoodsCode:PDGoodsReplaceDet_tblGoods_GoodsNumber:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDGoodsReplaceDetData,'PDGoodsReplaceDet_tblGoods_GoodsFullName','替换料','150',2,true,'',1000000,2,'PDGoodsReplaceDet:GoodsCode:PDGoodsReplaceDet_tblGoods_GoodsFullName:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDGoodsReplaceDetData,'PDGoodsReplaceDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDGoodsReplaceDet:GoodsCode:PDGoodsReplaceDet_tblGoods_GoodsSpec:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDGoodsReplaceDetData,'PDGoodsReplaceDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDGoodsReplaceDet:GoodsCode:PDGoodsReplaceDet_tblGoods_BaseUnit:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDGoodsReplaceDetData,'PDGoodsReplaceDet_tblGoods_attrType','来源码','80',2,true,'',1000000,2,'PDGoodsReplaceDet:GoodsCode:PDGoodsReplaceDet_tblGoods_attrType:'+param4,'',"","","0",'',"2",false); 
 
 
 
 
 addCols(PDGoodsReplaceDetData,'PDGoodsReplaceDet_Remark','备注','350',2,true,'',1000,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("PDGoodsReplaceDet_Remark","备注","any","",true,minValue,1000,false,'',true); 
 
 
 addCols(PDGoodsReplaceDetData,'PDGoodsReplaceDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDGoodsReplace&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDGoodsReplace,PDGoodsReplaceDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDGoodsReplace&allTableName=PDGoodsReplace,PDGoodsReplaceDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDGoodsReplace&allTableName=PDGoodsReplaceDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDGoodsReplace&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDGoodsReplace&allTableName=PDGoodsReplaceDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "PDGoodsReplaceDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDGoodsReplace&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDGoodsReplace&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDGoodsReplace&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDGoodsReplace&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDGoodsReplace&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '替换料';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDGoodsReplace&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
