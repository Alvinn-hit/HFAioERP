...
function printData(){
 ...
 var urls = "/UserFunctionQueryAction.do?tableName=PDGoodsReplace&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+bid+"&parentTableName=&winCurIndex="+winCurIndex;
...
function printLabelData(){
 ...
 var urls = "/UserFunctionQueryAction.do?tableName=PDGoodsReplace&reportNumber=PrintSeqPDGoodsReplace&printType=labelPrint&operation=18&keyId="+keyVal+"&parentTableName=&winCurIndex="+winCurIndex;
...
function onItemClick(str){
 ...
 if(currentId!=str){
 currentId = str ;
 setIframSrc2("/UserFunctionAction.do?tableName=PDGoodsReplace&keyId="+str+"&operation=7&winCurIndex="+winCurIndex,currentId);
 }
}
...
function setColumnSet(str,lockName){
 ...
 lockName = "";
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDGoodsReplace&colType=list&lockName="+lockName+"&colSelect="+str ;
 AjaxRequest(strURL);
...
function onDefaultCol(){
 ...
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDGoodsReplace&colType=list" ;
 AjaxRequest(strURL);
 if(location.href.indexOf("src=menu")!=-1)
 {
 window.location.reload();
...
function copyOperation()
{
 ...
 usrc="/UserFunctionAction.do?&operation=17&tableName=PDGoodsReplace&saveDraft=&parentTableName=&moduleType="+moduleTypeValue+"&f_brother="+f_brother+"&noback=false&winCurIndex="+winCurIndex+"&keyId="+document.getElementById("varKeyIds").value.split("|")[0];
...
function telCall()
{
 ...
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDGoodsReplace&keyId="+keyIds,"","dialogWidth=600px;dialogHeight=350px"); 
 } 
 }else
 {
 alert("请选择至少一条记录"); 
...
function addPrepare(){
 ...
 usrc ='/UserFunctionAction.do?tableName=PDGoodsReplace&parentCode='+parentCode+'&operation=6&moduleType='+moduleTypeValue+'&f_brother='+f_brother+'&parentTableName=&winCurIndex='+winCurIndex;
...
function openSelect(selectName,fieldName,display){
 ...
 var urlstr = '/UserFunctionAction.do?operation=22&src=menu&parentTableName='+parentTableName+'&moduleType='+moduleType+'&displayName='+encodeURI(display)+'&LinkType=@URL:&tableName=PDGoodsReplace&selectName='+selectName+encodeURI(linkStr) ;
...
function showDivCustomSetTable(){ 
 var returnValue = window.showModalDialog("/UserFunctionQueryAction.do?tableName=PDGoodsReplace&operation=4&parentTableName=&configType=listConfig&checkTab=Y&winCurIndex="+winCurIndex,"","dialogWidth=670px;dialogHeight=450px") ;
...
function deliverToAll(){ 
 ...
 checkDialog("/OAMyWorkFlow.do?keyId="+keyIds+"&tableName=PDGoodsReplace&moduleType="+moduleType+"&operation=82&fromPage=erp&type=deliverToAll");
...
function detail(str){
 ...
 turl="/UserFunctionAction.do?tableName=PDGoodsReplace&keyId="+str.split(",")[0]+"&moduleType="+moduleType+"&f_brother="+f_brother+"&operation=5&winCurIndex="+winCurIndex+"&pageNo=&parentCode="+str.split(",")[1]+"&parentTableName=&saveDraft=";
...
function update(str){
 turl="/UserFunctionAction.do?tableName=PDGoodsReplace&keyId="+str.split(",")[0]+"&moduleType="+moduleType+"&f_brother="+f_brother+"&operation=7&winCurIndex="+winCurIndex+"&pageNo=&parentCode="+str.split(",")[1]+"&parentTableName=&saveDraft=";
...
function next(str){
 location.href="/UserFunctionQueryAction.do?tableName=PDGoodsReplace&keyId="+str.split(",")[0]+"&parentCode="+str.split(",")[1]+"&operation=4&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&parentTableName=&checkTab=Y&selectType="+$("#selectType").val();
}


function doChangeParent(parentCode,name,ttableName){
 location.href="/UserFunctionQueryAction.do?tableName=PDGoodsReplace&parentCode="+parentCode+"&operation=4&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&parentTableName=&checkTab=Y&selectType="+$("#selectType").val();
}
...
function addClass(parentCodev){
 ...
 usrc ='/UserFunctionAction.do?tableName=PDGoodsReplace&parentCode='+parentCodev+'&operation=6&moduleType='+moduleTypeValue+'&f_brother='+f_brother+'&parentTableName=&winCurIndex='+winCurIndex;
...
function hurryTrans(str){
 ...
 hurryTransDialog("/vm/oa/workflow/noteModel.jsp?tableName=PDGoodsReplace&moduleName="+encodeURI(ren)+"&keyId="+str.split(",")[0],winObj,"dialogWidth=565px;dialogHeight=350px");
}
...
function check(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDGoodsReplace&parentTableName=&moduleType="+moduleType+"&operation=25&ButtonType=saveCheckBill&varKeyIds="+str.split(",")[0]+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+str.split(",")[0]+"&tableName=PDGoodsReplace&moduleType="+moduleType+"&operation=82&fromPage=erp");
 ...
 location.href="/UserFunctionQueryAction.do?tableName=PDGoodsReplace&moduleType="+moduleType+"&keyId="+str.split(",")[0]+"&f_brother="+f_brother+"&operation=83&winCurIndex="+winCurIndex+"&pageNo=&parentTableName=&saveDraft=";
 }else if(str.split(",")[1]=="cancel"){
 if(confirm('确定撤回?')){
 location.href='/UserFunctionQueryAction.do?nextStep=cancel&moduleType='+moduleType+'&tableName=PDGoodsReplace&lastNodeId='+str.split(",")[2]+'&keyId='+str.split(",")[0]+'&currNode='+str.split(",")[3]+'&f_brother='+f_brother+'&operation=79&winCurIndex='+winCurIndex+'&pageNo=&parentTableName=&saveDraft='
 }
 }
}
