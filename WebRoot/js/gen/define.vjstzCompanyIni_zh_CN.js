...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tzCompanyIni&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tzCompanyIni&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tzCompanyIni&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tzCompanyIni&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=b1531850_1501061134465940129&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=b1531850_1501061134465940129&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tzCompanyIni\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tzCompanyIni\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'price',dis:'单价',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'inQty',dis:'充值数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'inAmount',dis:'充值金额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'lastAmount',dis:'结存余额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'eEval3Amount',dis:'体验三个月',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'eEval3Qty',dis:'体验三个月数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'eEval10Amount',dis:'体验10天',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'eEval10Qty',dis:'体验10天数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'standardAmount',dis:'标准版',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'standardQty',dis:'标准版数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ExtremeAmount',dis:'至尊版',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ExtremeQty',dis:'至尊版数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'GoldAmount',dis:'金牌商盟',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'GoldQty',dis:'金牌商盟数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ExcellentAmount',dis:'卓越版',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ExcellentQty',dis:'卓越版数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'DiamondsAmount',dis:'钻石版',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'DiamondsQty',dis:'钻石版数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ExtremeSpecAmount',dis:'至尊特别版',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ExtremeSpecQty',dis:'至尊特别版数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'domainQty',dis:'域名数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'domainAmount',dis:'域名',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'emailQty',dis:'邮箱年限',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'emailAmount',dis:'邮箱数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define1Qty',dis:'自定义产品1年限',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define1Amount',dis:'自定义产品1数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define2Qty',dis:'自定义产品2年限',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define2Amount',dis:'自定义产品2数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define3Qty',dis:'自定义产品3年限',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define3Amount',dis:'自定义产品3数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define4Qty',dis:'自定义产品4年限',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define4Amount',dis:'自定义产品4数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define5Qty',dis:'自定义产品5年限',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define5Amount',dis:'自定义产品5数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define6Qty',dis:'自定义产品6年限',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define6Amount',dis:'自定义产品6数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define7Qty',dis:'自定义产品7年限',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define7Amount',dis:'自定义产品7数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define8Qty',dis:'自定义产品8年限',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define8Amount',dis:'自定义产品8数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define9Qty',dis:'自定义产品9年限',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define9Amount',dis:'自定义产品9数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define10Qty',dis:'自定义产品10年限',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'define10Amount',dis:'自定义产品10数量',digit:0,isMain:true};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tzCompanyIni",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tzCompanyIni&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tzCompanyIni&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"b1531850_1501061134465940129",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=b1531850_1501061134465940129&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=b1531850_1501061134465940129&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=b1531850_1501061134465940129&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tzCompanyIni"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tzCompanyIni&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tzCompanyIni&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tzCompanyIni&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tzCompanyIni&allTableName=tzCompanyIni,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tzCompanyIni&allTableName=&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tzCompanyIni&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tzCompanyIni&allTableName=&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=b1531850_1501061134465940129&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=b1531850_1501061134465940129&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=b1531850_1501061134465940129&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=b1531850_1501061134465940129&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tzCompanyIni&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=b1531850_1501061134465940129&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tzCompanyIni&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tzCompanyIni&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tzCompanyIni&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tzCompanyIni&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '往来对帐';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tzCompanyIni&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
