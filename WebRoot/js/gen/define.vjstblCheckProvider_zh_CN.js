...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblCheckProvider&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblCheckProvider&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblCheckProvider&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblCheckProvider&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblCheckProviderDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblCheckProviderDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblCheckProviderDet"){
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblCheckProvider\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblCheckProviderDet_CurAmount',"tblCheckProviderDet"); 
 }
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'TotalCoTaxAmt',dis:'本币总税额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CurTotalAmount',dis:'原币总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblCheckProviderDet_Qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCheckProviderDet_CurPrice',dis:'原币单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCheckProviderDet_CurAmount',dis:'原币金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCheckProviderDet_CoTaxAmt',dis:'税额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCheckProviderDet_Amount',dis:'本币未税金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCheckProviderDet_TaxAmount',dis:'本币含税金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblCheckProvider",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblCheckProvider&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblCheckProvider&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblCheckProvider"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblCheckProvider&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblCheckProvider&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblCheckProviderDet_CurAmount'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/(1+nn('Tax',0)/100),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"Amount",p),2);
n('CurTotalAmount')[0].value=f(sm('tblCheckProviderDet_CurAmount'),2);
n('TotalCoTaxAmt')[0].value=f(sm('tblCheckProviderDet_CoTaxAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblCheckProviderDet_TaxAmount'),2);
n('TotalAmount')[0].value=f(sm('tblCheckProviderDet_Amount'),2);
...
childData[childCount]='tblCheckProviderDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='对账明细';
childCount++;
 var tblCheckProviderDetData = new gridData();
 gridDatas[gridDataNum] = tblCheckProviderDetData;
 gridDataNum++;
 var tblCheckProviderDet=1;
 ...
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_CompanyIniID','对帐明细','80',2,false,'',50,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("tblCheckProviderDet_CompanyIniID","对帐明细","any","",false,minValue,50,false,'');
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_tblCompanyIni_BillDate','单据日期','80',2,true,'',1000000,2,'tblCheckProviderDet:CompanyIniID:tblCheckProviderDet_tblCompanyIni_BillDate:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_tblCompanyIni_BillNo','单据编号','100',2,true,'',1000000,2,'tblCheckProviderDet:CompanyIniID:tblCheckProviderDet_tblCompanyIni_BillNo:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'tblCheckProviderDet:CompanyIniID:tblCheckProviderDet_tblGoods_GoodsNumber:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_tblGoods_GoodsFullName','料件名称','150',2,true,'',1000000,2,'tblCheckProviderDet:CompanyIniID:tblCheckProviderDet_tblGoods_GoodsFullName:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'tblCheckProviderDet:CompanyIniID:tblCheckProviderDet_tblGoods_GoodsSpec:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblCheckProviderDet:CompanyIniID:tblCheckProviderDet_tblGoods_BaseUnit:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_tblGoods_BarCode','条码','100',2,true,'',1000000,2,'tblCheckProviderDet:CompanyIniID:tblCheckProviderDet_tblGoods_BarCode:'+param4,'',"","","0",'',"2",false); 
 
 
 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_BillID','单据ID','200',2,true,'',50,-2,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblCheckProviderDet_BillID","单据ID","any","",true,minValue,50,false,''); 
 
 
 
  addCols(tblCheckProviderDetData,'tblCheckProviderDet_BillType','单据类型','80',2,true,'',50,8,';PDBuyCutPay:采购品质异常;PDClientIn:客供入库单;PDClientOut:客供出库单;PDExceedMaterial:超领单;PDInvertoryAdj:库存调整单;PDInvertoryPay:缴库单;PDOutMaterials:发料单;PDReFetchMaterial:补料单;PDReturnItems:退料单;PDReturnOwe:欠料还回单;PDSalesCutPay:销售品质异常;PDScrapSheet:报废单;PDWorkOrder:工令单;PDWorkOrderfinish:工令强结单;tblAssemblyTemplate:组拆装模板;tblBorrowOrder:预订借料单;tblGoodsAssemblySplit:组拆装单;tblSalesRetail:零售单;tblAccAdjust:应收调账单;tblAccMain:凭证管理;tblAdjustPrice:存货调价单;tblAllot:同价调拨单;tblAllotChange:变价调拨单;tblBuyAskPrice:采购询价单;tblBuyInStock:采购入库单;tblBuyInvoiceInfo:采购开票单;tblBuyOrder:采购订单;tblBuyOutStock:采购退货单;tblCheck:盘盈盘亏单;tblExpensed:费用支出单;tblIncome:其他收入单;tblOtherIn:其他入库单;tblOtherOut:其他出库单;tblPay:采购付款单;tblPayAdjust:应付调账单;tblSaleReceive:销售收款单;tblSalesInvoiceInfo:销售开票;tblSalesOrder:销售订单;tblSalesOutStock:销售出库单;tblSalesReturnStock:销售退货单;tblSalseQuote:销售报价;tblTempCreditApply:临时额度申请表;tblTransferSale2:应付转应收;tblTransferSale4:应收转应付;tblUserTransAcc:现金银行转账单;tblAllotApplication:调拨申请单;tblBuyApplication:采购申请;tblBuyApplicationTotal:申请汇总单;tblSend:配送单;tblSendcheck:配送验收单;tblSendReturn:配送退货单;PDSampleReq:样品需求单;PDPurchForm:采购询样单','',"","","2",'',"1",false);
 putChildValidateItem("tblCheckProviderDet_BillType","单据类型","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_BillDate','单据日期','80',5,true,'',0,-2,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblCheckProviderDet_BillDate","单据日期","date","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_GoodsCode','料件名称','200',2,true,'',100,-2,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblCheckProviderDet_GoodsCode","料件名称","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_Qty','数量','60',1,true,'',0,8,'','',"Qty","","5",'',"0",false); 
 putChildValidateItem("tblCheckProviderDet_Qty","数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_CurPrice','原币单价','80',1,true,'',0,8,'','',"priceIdentifier","","6",'',"0",false); 
 putChildValidateItem("tblCheckProviderDet_CurPrice","原币单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_CurAmount','原币金额','80',1,true,'',0,8,'','',"SAmountIdentifier","","7",'',"0",false); 
 putChildValidateItem("tblCheckProviderDet_CurAmount","原币金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_CoTaxAmt','税额','80',1,true,'',0,8,'','',"SAmountIdentifier","","8",'',"0",false); 
 putChildValidateItem("tblCheckProviderDet_CoTaxAmt","税额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_Amount','本币未税金额','80',1,true,'',0,8,'','',"SAmountIdentifier","","9",'',"0",false); 
 putChildValidateItem("tblCheckProviderDet_Amount","本币未税金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_TaxAmount','本币含税金额','80',1,true,'',0,8,'','',"SAmountIdentifier","","10",'',"0",false); 
 putChildValidateItem("tblCheckProviderDet_TaxAmount","本币含税金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_OrderNo','订单编号','100',2,true,'',100,8,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblCheckProviderDet_OrderNo","订单编号","any","",true,minValue,100,false,'',true); 
 
 
 addCols(tblCheckProviderDetData,'tblCheckProviderDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblCheckProvider&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblCheckProvider,tblCheckProviderDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblCheckProvider&allTableName=tblCheckProvider,tblCheckProviderDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblCheckProvider&allTableName=tblCheckProviderDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblCheckProvider&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblCheckProvider&allTableName=tblCheckProviderDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblCheckProviderDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblCheckProvider&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblCheckProvider&designId=E45C66A3F57141EDAEB025D16EDC&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblCheckProvider&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblCheckProvider&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblCheckProvider&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '采购对账';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblCheckProvider&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
