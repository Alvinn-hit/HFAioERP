...
{ 
 cFireEvent('tblBuyOrderDet_Qty',''); 
 cFireEvent('tblBuyOrderDet_CurPrice',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBuyOrder&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBuyOrder&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBuyOrder&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBuyOrder&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBuyOrderDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblBuyOrderDet_BuyAskPriceID');
 ...
 var z=n('tblBuyOrderDet_BuyApplyID');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblBuyOrderDet_BuyAppTotalID');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBuyOrderDet"){
 
 copyFields=n('tblBuyOrderDet_StockCode');
 ...
 copyFields=n('tblBuyOrderDet_ArriveDate');
 if((order > 16 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 copyFields=n('tblBuyOrderDet_CoTaxAmt');
 if((order > 34 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBuyOrderDet"){
 ...
 copyFields=n('tblBuyOrderDet_ArriveDate'); 
 if( (order > 16 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 copyFields=n('tblBuyOrderDet_CoTaxAmt'); 
 if( (order > 34 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 }
 }
...
function execStat2(){
 execStatOne('tblBuyOrderDet_Qty',4);
 execStatOne('tblBuyOrderDet_Amount',2);
 execStatOne('tblBuyOrderDet_TaxAmount',2);
 execStatOne('tblBuyOrderDet_InQty',4);
 execStatOne('tblBuyOrderDet_NotInQty',4);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblBuyOrder\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBuyOrder\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblBuyOrderDet_Qty',"tblBuyOrderDet"); 
 delFireEventOne(rowId,tableName,'tblBuyOrderDet_CurPrice',"tblBuyOrderDet"); 
 delFireEventOne(rowId,tableName,'tblBuyOrderDet_CurAmount',"tblBuyOrderDet"); 
 delFireEventOne(rowId,tableName,'tblBuyOrderDet_Price',"tblBuyOrderDet"); 
 delFireEventOne(rowId,tableName,'tblBuyOrderDet_Amount',"tblBuyOrderDet"); 
 delFireEventOne(rowId,tableName,'tblBuyOrderDet_TaxPrice',"tblBuyOrderDet"); 
 delFireEventOne(rowId,tableName,'tblBuyOrderDet_TaxAmount',"tblBuyOrderDet"); 
 }
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'TotalAmount',dis:'本币未税总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalTaxAmount',dis:'本币含税总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalCoTaxAmt',dis:'总税额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CurTotalAmount',dis:'原币总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOrderDet_Qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOrderDet_CurPrice',dis:'原币单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOrderDet_CurAmount',dis:'原币金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOrderDet_Price',dis:'本币未税单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOrderDet_Amount',dis:'本币未税金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOrderDet_TaxPrice',dis:'本币含税单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOrderDet_TaxAmount',dis:'本币含税金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOrderDet_CoTaxAmt',dis:'税额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOrderDet_InQty',dis:'入库数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOrderDet_NotInQty',dis:'未入库数',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBuyOrder",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyOrder&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyOrder&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 m("tbl_tblCompany_ComFullName").value = "" ;
  ...
  if(fieldName == "BuyApplyID"){
 var saveField=n("BuyApplyID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblBuyApplication_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("PDWorkOrderID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblBuyApplication_PDWorkOrderID");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblBuyApplication_BillNo");
 saveField[0].value = ""; 
 m("tbl_tblBuyApplication_BillNo").value = "" ;
 var saveField=n("tblBuyApplication_BillDate");
 saveField[0].value = ""; 
 m("tbl_tblBuyApplication_BillDate").value = "" ;
 }
  }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblBuyOrder"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBuyOrder&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBuyOrder&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
n('TotalAmount')[0].value=f(sm('tblBuyOrderDet_DisBackAmt'),2);

n('TotalTaxAmount')[0].value=f(sm('tblBuyOrderDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblBuyOrderDet_CoTaxAmt'),2);

for(var j=0;j<r(0,'DisAmount').length;j++){
r(0,"DisAmount")[j].value=f(rn(0,"Amount",j)-rn(0,"DisBackAmt",j),2);
}

n('DisAmount')[0].value=f(sm('tblBuyOrderDet_DisAmount'),6);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOrderDet_Qty'); 
...
n('TotalCoTaxAmt')[0].value=f(sm('tblBuyOrderDet_CoTaxAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOrderDet_CurPrice'); 
...
n('TotalAmount')[0].value=f(sm('tblBuyOrderDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblBuyOrderDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(sm('tblBuyOrderDet_CurAmount'),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
n('TotalCoTaxAmt')[0].value=f(sm('tblBuyOrderDet_CoTaxAmt'),2);
n('DisAmount')[0].value=f(sm('tblBuyOrderDet_DisAmount'),6);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOrderDet_CurAmount'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),2);
...
n('DisAmount')[0].value=f(sm('tblBuyOrderDet_DisAmount'),6);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOrderDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
n('DisAmount')[0].value=f(sm('tblBuyOrderDet_DisAmount'),6);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOrderDet_Amount'); 
...
n('DisAmount')[0].value=f(sm('tblBuyOrderDet_DisAmount'),6);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOrderDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('tblBuyOrderDet_DisAmount'),6);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOrderDet_DisPrice'); 
...
n('DisAmount')[0].value=f(sm('tblBuyOrderDet_DisAmount'),6);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOrderDet_DisBackAmt'); 
...
n('DisAmount')[0].value=f(sm('tblBuyOrderDet_DisAmount'),6);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOrderDet_TaxPrice'); 
...
n('DisAmount')[0].value=f(sm('tblBuyOrderDet_DisAmount'),6);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOrderDet_TaxAmount'); 
...
n('DisAmount')[0].value=f(sm('tblBuyOrderDet_DisAmount'),6);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOrderDet_SecQty'); 
...
childData[childCount]='tblBuyOrderDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='采购订单明细';
childCount++;
 var tblBuyOrderDetData = new gridData();
 gridDatas[gridDataNum] = tblBuyOrderDetData;
 gridDataNum++;
 var tblBuyOrderDet=1;
 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_StockCode','仓库','80',2,true,'Sess_StockCode',100,-2,'','',"","copy","0",'',"2",false); 
 putChildValidateItem("tblBuyOrderDet_StockCode","仓库","any","",true,minValue,100,true,'Sess_StockCode'); 
 
 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_BuyAskPriceID','询价单号','80',2,true,'',32,-2,'','',"","triggerCopy","1",'',"2",false); 
 putChildValidateItem("tblBuyOrderDet_BuyAskPriceID","询价单号","any","ManyBillQuote",true,minValue,32,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_BuyApplyID','申请单号','80',2,true,'',32,-2,'','',"","triggerCopy","2",'',"2",false); 
 putChildValidateItem("tblBuyOrderDet_BuyApplyID","申请单号","any","ManyBillQuote",true,minValue,32,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_BuyAppTotalID','申请汇总','120',2,true,'',32,-2,'','',"","triggerCopy","3",'',"2",false); 
 putChildValidateItem("tblBuyOrderDet_BuyAppTotalID","申请汇总","any","ManyBillQuote",true,minValue,32,false,''); 
 ...
 var param4="";
 param4=param4+":1厂商@CompanyCode"; 
 param4=param4+":1请购单ID@BuyApplyID"; 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_GoodsCode','料件名称','120',2,false,'',100,-2,'','',"","","4",'',"2",true);
 putChildValidateItem("tblBuyOrderDet_GoodsCode","料件名称","any","",false,minValue,100,false,'');
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'tblBuyOrderDet:GoodsCode:tblBuyOrderDet_tblGoods_GoodsNumber:'+param4,'',"","","4",'',"2",false); 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_tblGoods_GoodsFullName','料件名称','150',2,true,'',1000000,2,'tblBuyOrderDet:GoodsCode:tblBuyOrderDet_tblGoods_GoodsFullName:'+param4,'',"","","4",'',"2",false); 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'tblBuyOrderDet:GoodsCode:tblBuyOrderDet_tblGoods_GoodsSpec:'+param4,'',"","","4",'',"2",false); 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblBuyOrderDet:GoodsCode:tblBuyOrderDet_tblGoods_BaseUnit:'+param4,'',"","","4",'',"2",false); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","","5",'',"2",false); 
 putChildValidateItem("tblBuyOrderDet_BatchNo","批号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","6",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"2",false); 
 putChildValidateItem("tblBuyOrderDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","8",'',"2",false); 
 putChildValidateItem("tblBuyOrderDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_InvoiceStatus','开票状态','200',2,true,'0',10,-2,'','',"","","13",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_InvoiceStatus","开票状态","any","",true,minValue,10,true,'0'); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_Qty','数量','60',1,false,'',0,0,'','',"Qty","","14",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_Qty","数量","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_CurPrice','原币单价','100',1,true,'',0,0,'','',"priceIdentifier","","15",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_CurPrice","原币单价","double","currency",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_ArriveDate','到货日期','80',5,true,'',10,0,'','',"","copy","16",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_ArriveDate","到货日期","date","",true,minValue,10,false,'',true); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_PresentSampleType','类型','80',2,true,'0',10,1,';0:正常;1:备品;2:议价','',"","","17",'',"1",false);
 putChildValidateItem("tblBuyOrderDet_PresentSampleType","类型","any","BillPresentSampleType",true,minValue,10,true,'0',true); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_Remark','备注','100',2,true,'',8000,0,'','',"priceIdentifier","","18",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_Remark","备注","any","currency",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_CurAmount','原币金额','100',1,true,'',0,8,'','',"AmountIdentifier","","19",'',"8",false); 
 putChildValidateItem("tblBuyOrderDet_CurAmount","原币金额","double","BillPresentSampleType",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_BuyQuoteID','报价单号','80',2,true,'',50,-2,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_BuyQuoteID","报价单号","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_Price','本币未税单价','80',1,true,'',0,8,'','',"priceIdentifier","","21",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_Price","本币未税单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_Amount','本币未税金额','100',1,true,'',0,8,'','',"AmountIdentifier","","22",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_Amount","本币未税金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_Discount','折扣率%','100',1,true,'100',0,-2,'','',"Price","","28",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_Discount","折扣率%","double","BillDiscount",true,minValue,maxValue,true,'100'); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_DisPrice','折后单价','100',1,true,'',0,-2,'','',"priceIdentifier","","29",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_DisPrice","折后单价","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_DisBackAmt','折后金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","30",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_DisBackAmt","折后金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_DisAmount','折让金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","31",'',"8",false); 
 putChildValidateItem("tblBuyOrderDet_DisAmount","折让金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_TaxPrice','本币含税单价','100',1,true,'',0,8,'','',"priceIdentifier","","32",'',"8",false); 
 putChildValidateItem("tblBuyOrderDet_TaxPrice","本币含税单价","double","BillTax",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_TaxAmount','本币含税金额','100',1,true,'',0,8,'','',"AmountIdentifier","","33",'',"8",false); 
 putChildValidateItem("tblBuyOrderDet_TaxAmount","本币含税金额","double","BillTax",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_CoTaxAmt','税额','100',1,true,'0',0,8,'','',"AmountIdentifier","copy","34",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_CoTaxAmt","税额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_InQty','入库数量','80',1,true,'',0,8,'','',"Qty","","35",'',"100",false); 
 putChildValidateItem("tblBuyOrderDet_InQty","入库数量","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_NeedPayAmt','待付款金额','80',1,true,'',0,-2,'','',"AmountIdentifier","","39",'',"2",false); 
 putChildValidateItem("tblBuyOrderDet_NeedPayAmt","待付款金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_NotInQty','未入库数','80',1,true,'',0,8,'','',"Qty","","40",'',"100",false); 
 putChildValidateItem("tblBuyOrderDet_NotInQty","未入库数","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_SourceID','源ID','200',2,true,'',100,-2,'','',"","","43",'',"3",false); 
 putChildValidateItem("tblBuyOrderDet_SourceID","源ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_TotalPayAmt','累计付款金额','80',1,true,'',0,-2,'','',"","","44",'',"2",false); 
 putChildValidateItem("tblBuyOrderDet_TotalPayAmt","累计付款金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_SalesOrderID','销售订单ID','200',2,true,'',50,-2,'','',"","","46",'',"3",false); 
 putChildValidateItem("tblBuyOrderDet_SalesOrderID","销售订单ID","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_PDWorkOrderID','工令单号','200',2,true,'',50,-2,'','',"","","47",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_PDWorkOrderID","工令单号","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","48",'',"3",false); 
 putChildValidateItem("tblBuyOrderDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_PayQty','付款数量','224',1,true,'',0,-2,'','',"Qty","","49",'',"3",false); 
 putChildValidateItem("tblBuyOrderDet_PayQty","付款数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_NotPayQty','未付款数量','224',1,true,'',0,-2,'','',"Qty","","50",'',"3",false); 
 putChildValidateItem("tblBuyOrderDet_NotPayQty","未付款数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_InPayQty','入库付款数','224',1,true,'',0,-2,'','',"Qty","","51",'',"3",false); 
 putChildValidateItem("tblBuyOrderDet_InPayQty","入库付款数","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","52",'',"0",false); 
 putChildValidateItem("tblBuyOrderDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyOrderDetData,'tblBuyOrderDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBuyOrder&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBuyOrder,tblBuyOrderDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyOrder&allTableName=tblBuyOrder,tblBuyOrderDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyOrder&allTableName=tblBuyOrderDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyOrder&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyOrder&allTableName=tblBuyOrderDet,&colType=colWidth" ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblBuyOrder&designId=16abe428_1603022149107840858&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBuyOrder&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBuyOrder&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBuyOrder&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '采购订单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBuyOrder&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
