...
{ 
 cFireEvent('tblBuyApplicationDet_Qty',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBuyApplication&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBuyApplication&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBuyApplication&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBuyApplication&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBuyApplicationDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblBuyApplicationDet_BatchNo');
 ...
 var z=n('tblBuyApplicationDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblBuyApplicationDet_Hue');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblBuyApplicationDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBuyApplicationDet"){
 
 copyFields=n('tblBuyApplicationDet_GoodsCode');
 ...
 var tblBuyApplicationDet_tblGoods_GoodsNumber = n('tblBuyApplicationDet_tblGoods_GoodsNumber');
 if(typeof(tblBuyApplicationDet_tblGoods_GoodsNumber) != 'undefined' && tblBuyApplicationDet_tblGoods_GoodsNumber.length >index){
 tblBuyApplicationDet_tblGoods_GoodsNumber[index].value = tblBuyApplicationDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblBuyApplicationDet_tblGoods_GoodsFullName = n('tblBuyApplicationDet_tblGoods_GoodsFullName');
 if(typeof(tblBuyApplicationDet_tblGoods_GoodsFullName) != 'undefined' && tblBuyApplicationDet_tblGoods_GoodsFullName.length >index){
 tblBuyApplicationDet_tblGoods_GoodsFullName[index].value = tblBuyApplicationDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblBuyApplicationDet_tblGoods_GoodsSpec = n('tblBuyApplicationDet_tblGoods_GoodsSpec');
 if(typeof(tblBuyApplicationDet_tblGoods_GoodsSpec) != 'undefined' && tblBuyApplicationDet_tblGoods_GoodsSpec.length >index){
 tblBuyApplicationDet_tblGoods_GoodsSpec[index].value = tblBuyApplicationDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblBuyApplicationDet_tblGoods_BaseUnit = n('tblBuyApplicationDet_tblGoods_BaseUnit');
 if(typeof(tblBuyApplicationDet_tblGoods_BaseUnit) != 'undefined' && tblBuyApplicationDet_tblGoods_BaseUnit.length >index){
 tblBuyApplicationDet_tblGoods_BaseUnit[index].value = tblBuyApplicationDet_tblGoods_BaseUnit[index-1].value;
 }
 var tblBuyApplicationDet_tblGoods_BarCode = n('tblBuyApplicationDet_tblGoods_BarCode');
 if(typeof(tblBuyApplicationDet_tblGoods_BarCode) != 'undefined' && tblBuyApplicationDet_tblGoods_BarCode.length >index){
 tblBuyApplicationDet_tblGoods_BarCode[index].value = tblBuyApplicationDet_tblGoods_BarCode[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBuyApplicationDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblBuyApplicationDet"; 
...
function execStat2(){
 execStatOne('tblBuyApplicationDet_Qty',4);
 execStatOne('tblBuyApplicationDet_OrderQty',4);
 execStatOne('tblBuyApplicationDet_NoOrderQty',4);
 execStatOne('tblBuyApplicationDet_InQty',4);
 execStatOne('tblBuyApplicationDet_NotInQty',4);
 setTotalCount(getTotalCount());
}
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBuyApplication\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblBuyApplicationDet_Qty',"tblBuyApplicationDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblBuyApplicationDet_lackQty',dis:'缺料量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyApplicationDet_totalLoss',dis:'备品量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyApplicationDet_Qty',dis:'申请数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyApplicationDet_OrderQty',dis:'已订购数',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyApplicationDet_NoOrderQty',dis:'未订购数',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyApplicationDet_InQty',dis:'已缴库数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyApplicationDet_NotInQty',dis:'未缴库数量',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBuyApplication",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyApplication&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyApplication&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblBuyApplication"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBuyApplication&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBuyApplication&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblBuyApplicationDet_Qty'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyApplicationDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyApplicationDet_Amount'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),2);
...
childData[childCount]='tblBuyApplicationDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='请购单明细';
childCount++;
 var tblBuyApplicationDetData = new gridData();
 gridDatas[gridDataNum] = tblBuyApplicationDetData;
 gridDataNum++;
 var tblBuyApplicationDet=1;
 ...
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_NotOutStockQty','未出库数量','70',1,true,'',0,-2,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_NotOutStockQty","未出库数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_OutStockQty','出库数量','70',1,true,'0',0,-2,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_OutStockQty","出库数量","double","",true,minValue,maxValue,true,'0'); 
 ...
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_GoodsCode','料件名称','120',2,false,'',100,-2,'','',"","copy","3",'',"2",true);
 putChildValidateItem("tblBuyApplicationDet_GoodsCode","料件名称","any","",false,minValue,100,false,'');
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'tblBuyApplicationDet:GoodsCode:tblBuyApplicationDet_tblGoods_GoodsNumber:'+param4,'',"","copy","3",'',"2",false); 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_tblGoods_GoodsFullName','料件名称','150',2,true,'',1000000,2,'tblBuyApplicationDet:GoodsCode:tblBuyApplicationDet_tblGoods_GoodsFullName:'+param4,'',"","copy","3",'',"2",false); 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'tblBuyApplicationDet:GoodsCode:tblBuyApplicationDet_tblGoods_GoodsSpec:'+param4,'',"","copy","3",'',"2",false); 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblBuyApplicationDet:GoodsCode:tblBuyApplicationDet_tblGoods_BaseUnit:'+param4,'',"","copy","3",'',"2",false); 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_tblGoods_BarCode','条码','100',2,true,'',1000000,2,'tblBuyApplicationDet:GoodsCode:tblBuyApplicationDet_tblGoods_BarCode:'+param4,'',"","copy","3",'',"2",false); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","4",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_BatchNo","批号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","5",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","6",'',"2",false); 
 putChildValidateItem("tblBuyApplicationDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_yearNO','箱号','100',2,true,'',100,-2,'','',"","triggerCopy","7",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_yearNO","箱号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_color','色彩','70',2,true,'',200,-2,'','',"","","8",'',"2",false); 
 putChildValidateItem("tblBuyApplicationDet_color","色彩","any","GoodsField",true,minValue,200,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_ProDate','生产日期','80',5,true,'',100,-2,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_ProDate","生产日期","date","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_Availably','保质期限','80',5,true,'',100,-2,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_Availably","保质期限","date","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_lackQty','缺料量','70',1,true,'',0,8,'','',"Qty","","11",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_lackQty","缺料量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_totalLoss','备品量','70',1,true,'',0,8,'','',"Qty","","12",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_totalLoss","备品量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_hasQty','累计已请购','70',1,true,'',0,-2,'','',"Qty","","13",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_hasQty","累计已请购","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_Qty','申请数量','70',1,true,'',0,0,'','',"Qty","","14",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_Qty","申请数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_Price','本币未税单价','70',1,true,'',0,-2,'','',"priceIdentifier","","15",'',"100",false); 
 putChildValidateItem("tblBuyApplicationDet_Price","本币未税单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_Amount','金额','70',1,true,'',0,-2,'','',"AmountIdentifier","","16",'',"100",false); 
 putChildValidateItem("tblBuyApplicationDet_Amount","金额","double","",true,minValue,maxValue,false,''); 
 ...
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_EmployeeID','采购员','80',2,true,'',50,-2,'','',"","","23",'',"2",false); 
 putChildValidateItem("tblBuyApplicationDet_EmployeeID","采购员","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_Userdefined1','自定义1','100',2,true,'',200,-2,'','',"","","24",'',"6",false); 
 putChildValidateItem("tblBuyApplicationDet_Userdefined1","自定义1","any","",true,minValue,200,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_Userdefined2','自定义2','100',2,true,'',200,-2,'','',"","","25",'',"6",false); 
 putChildValidateItem("tblBuyApplicationDet_Userdefined2","自定义2","any","",true,minValue,200,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_Userdefined3','自定义3','100',2,true,'',200,-2,'','',"","","26",'',"6",false); 
 putChildValidateItem("tblBuyApplicationDet_Userdefined3","自定义3","any","",true,minValue,200,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_OrderDate','期望到货日期','80',5,true,'',10,0,'','',"","","27",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_OrderDate","期望到货日期","date","",true,minValue,10,false,'',true); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_ArriveDate','回复到货日期','80',5,true,'',10,8,'','',"","","28",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_ArriveDate","回复到货日期","date","",true,minValue,10,false,'',true); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_Remark','备注','150',2,true,'',8000,0,'','',"","","29",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_SecQty','辅助数量','224',0,true,'1',0,-2,'','',"Qty","","30",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'1'); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_BaseQty','基本单位','224',0,true,'1',0,-2,'','',"","","31",'',"3",false); 
 putChildValidateItem("tblBuyApplicationDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'1'); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_SalesOrderID','销售订单号','200',2,true,'',50,-2,'','',"","","32",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_SalesOrderID","销售订单号","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_SourceID','SourceID','200',2,true,'',50,-2,'','',"","","33",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_SourceID","SourceID","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_PDWorkOrderID','工令单号','200',2,true,'',50,-2,'','',"","","34",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_PDWorkOrderID","工令单号","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_OrderQty','已订购数','70',1,true,'0',0,8,'','',"Qty","","35",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_OrderQty","已订购数","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_NoOrderQty','未订购数','70',1,true,'0',0,8,'','',"Qty","","36",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_NoOrderQty","未订购数","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_InQty','已缴库数量','70',1,true,'0',0,8,'','',"Qty","","37",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_InQty","已缴库数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_NotInQty','未缴库数量','70',1,true,'0',0,8,'','',"Qty","","38",'',"0",false); 
 putChildValidateItem("tblBuyApplicationDet_NotInQty","未缴库数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBuyApplicationDetData,'tblBuyApplicationDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBuyApplication&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBuyApplication,tblBuyApplicationDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyApplication&allTableName=tblBuyApplication,tblBuyApplicationDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyApplication&allTableName=tblBuyApplicationDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyApplication&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyApplication&allTableName=tblBuyApplicationDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblBuyApplicationDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblBuyApplication&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblBuyApplication&designId=16abe428_1603022146214180574&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBuyApplication&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBuyApplication&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBuyApplication&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '请购单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBuyApplication&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
