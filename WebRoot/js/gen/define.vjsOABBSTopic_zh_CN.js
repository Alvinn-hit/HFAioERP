...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=OABBSTopic&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=OABBSTopic&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=OABBSTopic&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=OABBSTopic&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "OABBSTopicUserTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "OABBSTopicWDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="OABBSTopicUser"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="OABBSTopicWDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="OABBSTopicUser"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="OABBSTopicWDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="OABBSTopicUser"; 
 tblName[1]="OABBSTopicWDet"; 
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=8aa5183b_1104230927167340052&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=8aa5183b_1104230927167340052&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"OABBSTopic\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"OABBSTopic\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=OABBSTopic",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=OABBSTopic&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=OABBSTopic&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"8aa5183b_1104230927167340052",
 MOOP:"add",
...
function resetSubmit(fieldName){
 if(fieldName == "bbsUserId"){
 var saveField=n("bbsUserId"); 
 saveField[0].value = "";
 ...
 if(fieldName == "bbsUserId2"){
 var saveField=n("bbsUserId2"); 
 saveField[0].value = "";
 var obj = m("tbl_tblEmployee_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblEmployee_EmpFullName2");
 saveField[0].value = ""; 
 m("tbl_tblEmployee_EmpFullName2").value = "" ;
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=8aa5183b_1104230927167340052&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=8aa5183b_1104230927167340052&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=8aa5183b_1104230927167340052&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "OABBSTopic"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=OABBSTopic&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=OABBSTopic&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='OABBSTopicUser';
tabIsNulls[childCount]='0';
tabDiss[childCount]='版块用户权限分配明细';
childCount++;
 var OABBSTopicUserData = new gridData();
 gridDatas[gridDataNum] = OABBSTopicUserData;
 gridDataNum++;
 var OABBSTopicUser=1;
 ...
 addCols(OABBSTopicUserData,'OABBSTopicUser_DepartmentCode','部门','120',2,true,'',50,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("OABBSTopicUser_DepartmentCode","部门","any","",true,minValue,50,false,'');
 addCols(OABBSTopicUserData,'OABBSTopicUser_FortuneDepartmentDet_DeptFullName','部门','70',0,true,'',1000000,2,'OABBSTopicUser:DepartmentCode:OABBSTopicUser_FortuneDepartmentDet_DeptFullName:'+param4,'',"","","0",'',"2",false); 
 ...
 var param4="";
 param4=param4+":0部门@OABBSTopicUser_DepartmentCode";
 
 addCols(OABBSTopicUserData,'OABBSTopicUser_userID','用户','120',2,true,'',50,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("OABBSTopicUser_userID","用户","any","",true,minValue,50,false,'');
 addCols(OABBSTopicUserData,'OABBSTopicUser_tblEmployee_EmpFullName','用户','80',2,true,'',1000000,2,'OABBSTopicUser:userID:OABBSTopicUser_tblEmployee_EmpFullName:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(OABBSTopicUserData,'OABBSTopicUser_defaultScope','用户权限','800',2,true,'',30,5,';1:禁止浏览;6:禁止查看他人贴子;2:禁止发新话题;3:禁止回复;4:禁止查看／下载附件;5:禁止上传附件','',"","","2",'',"5",false); 
 
 
 addCols(OABBSTopicUserData,'OABBSTopicUser_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='OABBSTopicWDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='版块分类明细';
childCount++;
 var OABBSTopicWDetData = new gridData();
 gridDatas[gridDataNum] = OABBSTopicWDetData;
 gridDataNum++;
 var OABBSTopicWDet=1;
 ...
 addCols(OABBSTopicWDetData,'OABBSTopicWDet_BBSLabel','版块分类','150',2,true,'',200,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("OABBSTopicWDet_BBSLabel","版块分类","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(OABBSTopicWDetData,'OABBSTopicWDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=OABBSTopic&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "OABBSTopic,OABBSTopicUser,OABBSTopicWDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=OABBSTopic&allTableName=OABBSTopic,OABBSTopicUser,OABBSTopicWDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=OABBSTopic&allTableName=OABBSTopicUser,OABBSTopicWDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=OABBSTopic&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=OABBSTopic&allTableName=OABBSTopicUser,OABBSTopicWDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=8aa5183b_1104230927167340052&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=8aa5183b_1104230927167340052&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=8aa5183b_1104230927167340052&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=8aa5183b_1104230927167340052&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=OABBSTopic&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=8aa5183b_1104230927167340052&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=OABBSTopic&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=OABBSTopic&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=OABBSTopic&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=OABBSTopic&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '论坛版块管理';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=OABBSTopic&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
