...
{ 
 cFireEvent('tblContractDet_Price','0'); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblContract&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblContract&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblContract&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblContract&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblContractDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblContractDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblContractDet"){
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=187d3e33_1404301148543120203&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=187d3e33_1404301148543120203&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblContract\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblContract\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblContractDet_Amount',"tblContractDet"); 
 delFireEventOne(rowId,tableName,'tblContractDet_Price',"tblContractDet"); 
 delFireEventOne(rowId,tableName,'tblContractDet_TotalPrice',"tblContractDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'TotalPrice',dis:'总金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'PayAmount',dis:'已付款',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblContractDet_Amount',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblContractDet_Price',dis:'单价',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblContractDet_TotalPrice',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblContract",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblContract&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblContract&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"187d3e33_1404301148543120203",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "CompareSheet"){
 var saveField=n("CompareSheet"); 
 saveField[0].value = "";
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=187d3e33_1404301148543120203&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=187d3e33_1404301148543120203&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=187d3e33_1404301148543120203&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblContract"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblContract&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblContract&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblContractDet_Amount'); 
 r(0,"TotalPrice")[p].value=f(rn(0,"Price",p)*rn(0,"Amount",p),2);
n('TotalPrice')[0].value=f(sm('tblContractDet_TotalPrice'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblContractDet_Price'); 
 r(0,"TotalPrice")[p].value=f(rn(0,"Price",p)*rn(0,"Amount",p),2);
n('TotalPrice')[0].value=f(sm('tblContractDet_TotalPrice'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblContractDet_TotalPrice'); 
...
childData[childCount]='tblContractDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='采购合同明细';
childCount++;
 var tblContractDetData = new gridData();
 gridDatas[gridDataNum] = tblContractDetData;
 gridDataNum++;
 var tblContractDet=1;
 ...
 addCols(tblContractDetData,'tblContractDet_WareNo','材料编码','100',2,false,'',100,2,'tblContractDet:WareNo:tblContractDet_WareNo:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblContractDet_WareNo","材料编码","any","",false,minValue,100,false,'',true);
 
 
 
 addCols(tblContractDetData,'tblContractDet_WareName','材料名称','150',2,true,'',200,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblContractDet_WareName","材料名称","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblContractDetData,'tblContractDet_Specification','规格/型号','100',2,true,'',100,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblContractDet_Specification","规格/型号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblContractDetData,'tblContractDet_Brand','品牌','100',2,true,'',100,0,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblContractDet_Brand","品牌","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblContractDetData,'tblContractDet_WareUnit','计量单位','100',2,true,'',100,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblContractDet_WareUnit","计量单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblContractDetData,'tblContractDet_Amount','数量','100',1,true,'0',0,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblContractDet_Amount","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblContractDetData,'tblContractDet_Price','单价','100',1,true,'0',0,0,'','',"","","13",'',"0",false); 
 putChildValidateItem("tblContractDet_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblContractDetData,'tblContractDet_TotalPrice','金额','100',1,true,'0',0,0,'','',"","","14",'',"0",false); 
 putChildValidateItem("tblContractDet_TotalPrice","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblContractDetData,'tblContractDet_Remark','备注','150',2,true,'',500,0,'','',"","","15",'',"0",false); 
 putChildValidateItem("tblContractDet_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 
 addCols(tblContractDetData,'tblContractDet_PurchaseDetID','采购订单明细ID','100',2,true,'',100,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblContractDet_PurchaseDetID","采购订单明细ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblContractDetData,'tblContractDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblContract&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblContract,tblContractDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblContract&allTableName=tblContract,tblContractDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblContract&allTableName=tblContractDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblContract&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblContract&allTableName=tblContractDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=187d3e33_1404301148543120203&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=187d3e33_1404301148543120203&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=187d3e33_1404301148543120203&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=187d3e33_1404301148543120203&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblContract&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=187d3e33_1404301148543120203&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblContract&designId=6466ed69_1408220901323120446&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblContract&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblContract&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblContract&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '采购合同';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblContract&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
