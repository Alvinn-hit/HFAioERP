...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblEnquiry&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblEnquiry&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblEnquiry&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblEnquiry&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblEnquiryDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblEnquiryDet1Table"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblEnquiryDet2Table"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblEnquiryDet3Table"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblEnquiryDet4Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblEnquiryDet"){
 
  }
 if(typeof(tableName)!="undefined" && tableName=="tblEnquiryDet1"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblEnquiryDet2"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblEnquiryDet3"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblEnquiryDet4"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblEnquiryDet"){
 
  }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblEnquiryDet1"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblEnquiryDet2"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblEnquiryDet3"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblEnquiryDet4"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblEnquiryDet"; 
 tblName[1]="tblEnquiryDet1"; 
 tblName[2]="tblEnquiryDet2"; 
 tblName[3]="tblEnquiryDet3"; 
 tblName[4]="tblEnquiryDet4"; 
...
function execStat2(){
 execStatOne('tblEnquiryDet_Quantity',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=84ac2fc4_1405051532441250167&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=84ac2fc4_1405051532441250167&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblEnquiry\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblEnquiry\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet_Price',dis:'单价',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet_Quantity',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet_ZBPrice',dis:'材料中标价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet1_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet1_Qty',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet1_Amount',dis:'金额',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet2_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet2_Qty',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet2_Amount',dis:'金额',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet3_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet3_Qty',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet3_Amount',dis:'金额',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet4_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet4_Qty',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblEnquiryDet4_Amount',dis:'金额',digit:0,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblEnquiry",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblEnquiry&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblEnquiry&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"84ac2fc4_1405051532441250167",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 saveField[0].value = "";
 var obj = m("tbl_tblProject_billNo");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("ProjectName"); 
 saveField[0].value = "";
 var obj = m("tbl_tblProject_name");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tc_ProNo");
 saveField[0].value = ""; 
 m("tbl_tc_ProNo").value = "" ;
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84ac2fc4_1405051532441250167&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84ac2fc4_1405051532441250167&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=84ac2fc4_1405051532441250167&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblEnquiry"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblEnquiry&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblEnquiry&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblEnquiryDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='采购询价单明细';
childCount++;
 var tblEnquiryDetData = new gridData();
 gridDatas[gridDataNum] = tblEnquiryDetData;
 gridDataNum++;
 var tblEnquiryDet=1;
 ...
 var param4="";
 param4=param4+":1项目编码@ProjectNo"; 
 param4=param4+":0下料单号@CuttingNo"; 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_CuttingId','引用下料单号','120',2,true,'',100,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("tblEnquiryDet_CuttingId","引用下料单号","any","",true,minValue,100,false,'');
 addCols(tblEnquiryDetData,'tblEnquiryDet_tblCutting_billNo','引用下料单号','120',2,true,'',1000000,2,'tblEnquiryDet:CuttingId:tblEnquiryDet_tblCutting_billNo:'+param4,'',"","","0",'',"2",false); 
 ...
 addCols(tblEnquiryDetData,'tblEnquiryDet_WareNo','材料编码','100',2,false,'',100,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblEnquiryDet_WareNo","材料编码","any","",false,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_WareName','材料名称','150',2,false,'',200,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblEnquiryDet_WareName","材料名称","any","",false,minValue,200,false,'',true); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_Specification','规格/型号','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblEnquiryDet_Specification","规格/型号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_Brand','品牌','100',2,true,'',100,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblEnquiryDet_Brand","品牌","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_WareUnit','计量单位','100',2,true,'',100,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblEnquiryDet_WareUnit","计量单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_Amount','下料数量','100',1,true,'0',0,-2,'','',"","","6",'',"3",false); 
 putChildValidateItem("tblEnquiryDet_Amount","下料数量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_Supplier','供应商','100',2,true,'',100,8,'','',"","","7",'',"3",false); 
 putChildValidateItem("tblEnquiryDet_Supplier","供应商","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_Price','单价','100',1,true,'0',0,8,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblEnquiryDet_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_Quantity','数量','100',1,true,'0',0,8,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblEnquiryDet_Quantity","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_ZBPrice','材料中标价','100',1,true,'',0,8,'','',"","","10",'',"8",false); 
 putChildValidateItem("tblEnquiryDet_ZBPrice","材料中标价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_ArriveTime','到场时间','100',5,true,'',50,-2,'','',"","","11",'',"3",false); 
 putChildValidateItem("tblEnquiryDet_ArriveTime","到场时间","date","",true,minValue,50,false,''); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_CuttingMemo','下料单备注','100',2,true,'',100,-2,'','',"","","12",'',"3",false); 
 putChildValidateItem("tblEnquiryDet_CuttingMemo","下料单备注","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_Position','使用位置','150',2,true,'',200,0,'','',"","","13",'',"0",false); 
 putChildValidateItem("tblEnquiryDet_Position","使用位置","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_Memo','备注（是否含税，是否含运，是否含卸货费、加工费等）','400',2,true,'',8000,0,'','',"","","14",'',"0",false); 
 putChildValidateItem("tblEnquiryDet_Memo","备注（是否含税，是否含运，是否含卸货费、加工费等）","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_MaterialDetId','材料总计划明细ID','100',2,true,'',100,-2,'','',"","","15",'',"0",false); 
 putChildValidateItem("tblEnquiryDet_MaterialDetId","材料总计划明细ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_BuyBrand','采购品牌','100',2,true,'',100,8,'','',"","","16",'',"0",false); 
 putChildValidateItem("tblEnquiryDet_BuyBrand","采购品牌","any","",true,minValue,100,false,'',true); 
 
 
 
 
 addCols(tblEnquiryDetData,'tblEnquiryDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblEnquiryDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='加工费';
childCount++;
 var tblEnquiryDet1Data = new gridData();
 gridDatas[gridDataNum] = tblEnquiryDet1Data;
 gridDataNum++;
 var tblEnquiryDet1=1;
 
 
 addCols(tblEnquiryDet1Data,'tblEnquiryDet1_CostName','加工费名称','120',2,true,'',200,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblEnquiryDet1_CostName","加工费名称","any","",true,minValue,200,false,'',true); 
 
 
 
 
 
 addCols(tblEnquiryDet1Data,'tblEnquiryDet1_Unit','单位','60',2,true,'',100,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblEnquiryDet1_Unit","单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryDet1Data,'tblEnquiryDet1_Price','单价','60',1,true,'0',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblEnquiryDet1_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryDet1Data,'tblEnquiryDet1_Qty','数量','60',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblEnquiryDet1_Qty","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryDet1Data,'tblEnquiryDet1_Amount','金额','60',1,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblEnquiryDet1_Amount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 addCols(tblEnquiryDet1Data,'tblEnquiryDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblEnquiryDet2';
tabIsNulls[childCount]='0';
tabDiss[childCount]='运费';
childCount++;
 var tblEnquiryDet2Data = new gridData();
 gridDatas[gridDataNum] = tblEnquiryDet2Data;
 gridDataNum++;
 var tblEnquiryDet2=1;
 
 
 addCols(tblEnquiryDet2Data,'tblEnquiryDet2_CostName','运费名称','120',2,true,'',200,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblEnquiryDet2_CostName","运费名称","any","",true,minValue,200,false,'',true); 
 
 
 
 
 
 addCols(tblEnquiryDet2Data,'tblEnquiryDet2_Unit','单位','60',2,true,'',100,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblEnquiryDet2_Unit","单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryDet2Data,'tblEnquiryDet2_Price','单价','60',1,true,'0',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblEnquiryDet2_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryDet2Data,'tblEnquiryDet2_Qty','数量','60',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblEnquiryDet2_Qty","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryDet2Data,'tblEnquiryDet2_Amount','金额','60',1,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblEnquiryDet2_Amount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 addCols(tblEnquiryDet2Data,'tblEnquiryDet2_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblEnquiryDet3';
tabIsNulls[childCount]='0';
tabDiss[childCount]='税费';
childCount++;
 var tblEnquiryDet3Data = new gridData();
 gridDatas[gridDataNum] = tblEnquiryDet3Data;
 gridDataNum++;
 var tblEnquiryDet3=1;
 
 
 addCols(tblEnquiryDet3Data,'tblEnquiryDet3_CostName','税费名称','120',2,true,'',200,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblEnquiryDet3_CostName","税费名称","any","",true,minValue,200,false,'',true); 
 
 
 
 
 
 addCols(tblEnquiryDet3Data,'tblEnquiryDet3_Unit','单位','60',2,true,'',100,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblEnquiryDet3_Unit","单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryDet3Data,'tblEnquiryDet3_Price','单价','60',1,true,'0',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblEnquiryDet3_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryDet3Data,'tblEnquiryDet3_Qty','数量','60',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblEnquiryDet3_Qty","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryDet3Data,'tblEnquiryDet3_Amount','金额','60',1,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblEnquiryDet3_Amount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 addCols(tblEnquiryDet3Data,'tblEnquiryDet3_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblEnquiryDet4';
tabIsNulls[childCount]='0';
tabDiss[childCount]='安装费';
childCount++;
 var tblEnquiryDet4Data = new gridData();
 gridDatas[gridDataNum] = tblEnquiryDet4Data;
 gridDataNum++;
 var tblEnquiryDet4=1;
 
 
 addCols(tblEnquiryDet4Data,'tblEnquiryDet4_CostName','安装费名称','120',2,true,'',200,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblEnquiryDet4_CostName","安装费名称","any","",true,minValue,200,false,'',true); 
 
 
 
 
 
 addCols(tblEnquiryDet4Data,'tblEnquiryDet4_Unit','单位','60',2,true,'',100,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblEnquiryDet4_Unit","单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblEnquiryDet4Data,'tblEnquiryDet4_Price','单价','60',1,true,'0',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblEnquiryDet4_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryDet4Data,'tblEnquiryDet4_Qty','数量','60',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblEnquiryDet4_Qty","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblEnquiryDet4Data,'tblEnquiryDet4_Amount','金额','60',1,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblEnquiryDet4_Amount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 addCols(tblEnquiryDet4Data,'tblEnquiryDet4_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblEnquiry&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblEnquiry,tblEnquiryDet,tblEnquiryDet1,tblEnquiryDet2,tblEnquiryDet3,tblEnquiryDet4,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblEnquiry&allTableName=tblEnquiry,tblEnquiryDet,tblEnquiryDet1,tblEnquiryDet2,tblEnquiryDet3,tblEnquiryDet4,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblEnquiry&allTableName=tblEnquiryDet,tblEnquiryDet1,tblEnquiryDet2,tblEnquiryDet3,tblEnquiryDet4,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblEnquiry&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblEnquiry&allTableName=tblEnquiryDet,tblEnquiryDet1,tblEnquiryDet2,tblEnquiryDet3,tblEnquiryDet4,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84ac2fc4_1405051532441250167&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84ac2fc4_1405051532441250167&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=84ac2fc4_1405051532441250167&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=84ac2fc4_1405051532441250167&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblEnquiry&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=84ac2fc4_1405051532441250167&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblEnquiry&designId=971e12de_1407071141242030296&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblEnquiry&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblEnquiry&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblEnquiry&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '采购询价单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblEnquiry&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
