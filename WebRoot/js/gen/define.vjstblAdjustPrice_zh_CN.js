...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblAdjustPrice&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblAdjustPrice&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblAdjustPrice&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblAdjustPrice&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblAdjustPriceDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblAdjustPriceDet_GoodsCode');
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblAdjustPriceDet"){
 
 copyFields=n('tblAdjustPriceDet_StockCode');
 ...
 var tblAdjustPriceDet_tblStock_StockFullName1 = n('tblAdjustPriceDet_tblStock_StockFullName1');
 if(typeof(tblAdjustPriceDet_tblStock_StockFullName1) != 'undefined' && tblAdjustPriceDet_tblStock_StockFullName1.length >index){
 tblAdjustPriceDet_tblStock_StockFullName1[index].value = tblAdjustPriceDet_tblStock_StockFullName1[index-1].value;
 }
 }
 copyFields=n('tblAdjustPriceDet_GoodsCode');
 if((order > 3 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblAdjustPriceDet_tblGoods_GoodsNumber = n('tblAdjustPriceDet_tblGoods_GoodsNumber');
 if(typeof(tblAdjustPriceDet_tblGoods_GoodsNumber) != 'undefined' && tblAdjustPriceDet_tblGoods_GoodsNumber.length >index){
 tblAdjustPriceDet_tblGoods_GoodsNumber[index].value = tblAdjustPriceDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblAdjustPriceDet_tblGoods_GoodsFullName = n('tblAdjustPriceDet_tblGoods_GoodsFullName');
 if(typeof(tblAdjustPriceDet_tblGoods_GoodsFullName) != 'undefined' && tblAdjustPriceDet_tblGoods_GoodsFullName.length >index){
 tblAdjustPriceDet_tblGoods_GoodsFullName[index].value = tblAdjustPriceDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblAdjustPriceDet_tblGoods_GoodsSpec = n('tblAdjustPriceDet_tblGoods_GoodsSpec');
 if(typeof(tblAdjustPriceDet_tblGoods_GoodsSpec) != 'undefined' && tblAdjustPriceDet_tblGoods_GoodsSpec.length >index){
 tblAdjustPriceDet_tblGoods_GoodsSpec[index].value = tblAdjustPriceDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblAdjustPriceDet_tblGoods_BaseUnit = n('tblAdjustPriceDet_tblGoods_BaseUnit');
 if(typeof(tblAdjustPriceDet_tblGoods_BaseUnit) != 'undefined' && tblAdjustPriceDet_tblGoods_BaseUnit.length >index){
 tblAdjustPriceDet_tblGoods_BaseUnit[index].value = tblAdjustPriceDet_tblGoods_BaseUnit[index-1].value;
 }
 }
 }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblAdjustPriceDet"){
...
function execStat2(){
 execStatOne('tblAdjustPriceDet_Qty',2);
 execStatOne('tblAdjustPriceDet_Amt',2);
 execStatOne('tblAdjustPriceDet_NewAmt',2);
 execStatOne('tblAdjustPriceDet_AdjustAmt',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblAdjustPrice\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblAdjustPrice\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblAdjustPriceDet_NewPrice',"tblAdjustPriceDet"); 
 delFireEventOne(rowId,tableName,'tblAdjustPriceDet_NewAmt',"tblAdjustPriceDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblAdjustPriceDet_Price',dis:'账面单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAdjustPriceDet_NewPrice',dis:'调价后单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAdjustPriceDet_AdjustAmt',dis:'盈亏金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblAdjustPrice",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAdjustPrice&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAdjustPrice&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblAdjustPrice"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblAdjustPrice&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblAdjustPrice&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAdjustPriceDet_NewPrice'); 
 r(0,"NewAmt")[p].value=f(rn(0,"Qty",p)*rn(0,"NewPrice",p),2);
r(0,"AdjustAmt")[p].value=f(rn(0,"NewAmt",p)-rn(0,"Amt",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAdjustPriceDet_NewAmt'); 
 r(0,"NewPrice")[p].value=f(rn(0,"NewAmt",p)/rn(0,"Qty",p),4);
r(0,"AdjustAmt")[p].value=f(rn(0,"NewAmt",p)-rn(0,"Amt",p),2);
...
childData[childCount]='tblAdjustPriceDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='存货调价单明细';
childCount++;
 var tblAdjustPriceDetData = new gridData();
 gridDatas[gridDataNum] = tblAdjustPriceDetData;
 gridDataNum++;
 var tblAdjustPriceDet=1;
 ...
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_StockCode','仓库','80',2,true,'',80,-2,'','',"","copy","0",'',"2",true);
 putChildValidateItem("tblAdjustPriceDet_StockCode","仓库","any","",true,minValue,80,true,'');
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_tblStock_StockFullName1','仓库','75',0,true,'Sess_StockFullName',1000000,2,'tblAdjustPriceDet:StockCode:tblAdjustPriceDet_tblStock_StockFullName1:'+param4,'',"","copy","0",'',"2",false); 
 ...
 var param4="";
 param4=param4+":0仓库@tblAdjustPriceDet_StockCode";
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_GoodsCode','商品名称','120',2,false,'',100,-2,'','',"","triggerOrCopy","3",'',"2",true);
 putChildValidateItem("tblAdjustPriceDet_GoodsCode","商品名称","any","",false,minValue,100,false,'');
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'tblAdjustPriceDet:GoodsCode:tblAdjustPriceDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_tblGoods_GoodsFullName','商品名称','150',18,true,'',1000000,2,'tblAdjustPriceDet:GoodsCode:tblAdjustPriceDet_tblGoods_GoodsFullName:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'tblAdjustPriceDet:GoodsCode:tblAdjustPriceDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblAdjustPriceDet:GoodsCode:tblAdjustPriceDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","","9",'',"2",false); 
 putChildValidateItem("tblAdjustPriceDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_Inch','厚度','70',2,true,'',100,-2,'','',"","","12",'',"2",false); 
 putChildValidateItem("tblAdjustPriceDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_Hue','颜色','70',2,true,'',100,-2,'','',"","","15",'',"2",false); 
 putChildValidateItem("tblAdjustPriceDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","","18",'',"2",false); 
 putChildValidateItem("tblAdjustPriceDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_ProDate','生产日期','80',2,true,'',10,-2,'','',"","","21",'',"8",false); 
 putChildValidateItem("tblAdjustPriceDet_ProDate","生产日期","any","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_Availably','保质期限','80',2,true,'',10,-2,'','',"","","24",'',"8",false); 
 putChildValidateItem("tblAdjustPriceDet_Availably","保质期限","any","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_Qty1','序列号回填数量','80',1,true,'',0,-2,'','',"Qty","","27",'',"0",false); 
 putChildValidateItem("tblAdjustPriceDet_Qty1","序列号回填数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_Qty','账面数量','80',1,false,'',0,-2,'','',"Qty","","30",'',"8",false); 
 putChildValidateItem("tblAdjustPriceDet_Qty","账面数量","double","",false,minValue,maxValue,false,''); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_Price','账面单价','80',1,false,'',0,8,'','',"priceIdentifier","","33",'',"0",false); 
 putChildValidateItem("tblAdjustPriceDet_Price","账面单价","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_Amt','账面金额','80',1,false,'0',0,-2,'','',"AmountIdentifier","","36",'',"8",false); 
 putChildValidateItem("tblAdjustPriceDet_Amt","账面金额","double","",false,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_NewPrice','调价后单价','80',1,false,'',0,0,'','',"priceIdentifier","","39",'',"0",false); 
 putChildValidateItem("tblAdjustPriceDet_NewPrice","调价后单价","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_NewAmt','调价后金额','100',1,false,'0',0,-2,'','',"AmountIdentifier","","42",'',"0",false); 
 putChildValidateItem("tblAdjustPriceDet_NewAmt","调价后金额","double","",false,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_AdjustAmt','盈亏金额','80',1,false,'0',0,8,'','',"AmountIdentifier","","45",'',"0",false); 
 putChildValidateItem("tblAdjustPriceDet_AdjustAmt","盈亏金额","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_SourceID','源ID','0',2,true,'',60,-2,'','',"","","105",'',"100",false); 
 putChildValidateItem("tblAdjustPriceDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_BaseQty','基本单位','224',0,true,'1',0,-2,'','',"","","120",'',"3",false); 
 putChildValidateItem("tblAdjustPriceDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'1'); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","120",'',"3",false); 
 putChildValidateItem("tblAdjustPriceDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_Remark','备注','120',2,true,'',8000,0,'','',"","","120",'',"0",false); 
 putChildValidateItem("tblAdjustPriceDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 addCols(tblAdjustPriceDetData,'tblAdjustPriceDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblAdjustPrice&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblAdjustPrice,tblAdjustPriceDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAdjustPrice&allTableName=tblAdjustPrice,tblAdjustPriceDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAdjustPrice&allTableName=tblAdjustPriceDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAdjustPrice&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAdjustPrice&allTableName=tblAdjustPriceDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblAdjustPriceDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblAdjustPrice&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblAdjustPrice&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblAdjustPrice&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblAdjustPrice&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblAdjustPrice&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '存货调价';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblAdjustPrice&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
