...
function showAdvancePanel(){
 var urls = "/ReportDataAction.do?operation=advance&reportNumber=tblApplyOutMaterials";
 asyncbox.open({id:'advancePanel',title:'条件高级设置',url:urls,width:450,height:400});
}
function showAdvanceUpdatePanel(opType,keyId){
 var urls = "/ReportDataAction.do?operation=advance&opType="+opType+"&keyId="+keyId+"&reportNumber=tblApplyOutMaterials";
 
 asyncbox.open({id:'advanceUpdatePanel',title:'条件方案设置',url:urls,width:450,height:500});
}
function closeAdvanceUpdate(){
 jQuery.close("advanceUpdatePanel");
 jQuery.reload('advancePanel',"/ReportDataAction.do?operation=advance&reportNumber=tblApplyOutMaterials"); 
}
...
function onLockClick(str){
 AjaxRequest("/UtilServlet?operation=lockColumn&report=tblApplyOutMaterials&number="+str); 
}
...
function printData(){
 ...
 var urls = "/UserFunctionQueryAction.do?tableName=tblApplyOutMaterials&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+keyVal.val()+"&parentTableName=&winCurIndex="+winCurIndex;
...
function printLabelData(){
 ...
 var urls = "/UserFunctionQueryAction.do?tableName=tblApplyOutMaterials&reportNumber=PrintSeqtblApplyOutMaterials&printType=labelPrint&operation=18&keyId="+keyVal+"&parentTableName=&winCurIndex="+winCurIndex;
...
function onItemClick(str){
 ...
 if(currentId!=str){
 currentId = str ;
 setIframSrc2("/UserFunctionAction.do?tableName=tblApplyOutMaterials&keyId="+str+"&operation=7&winCurIndex="+winCurIndex,currentId);
 }
}
...
function setIframSrc2(srcUrl,obj){
 ...
 if("tblApplyOutMaterials"!=checkItem){
...
function setColumnSet(str,lockName){
 ...
 lockName = "";
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblApplyOutMaterials&colType=list&lockName="+lockName+"&colSelect="+str ;
 AjaxRequest(strURL);
...
function onDefaultCol(){
 ...
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblApplyOutMaterials&colType=list" ;
 AjaxRequest(strURL);
 if(location.href.indexOf("src=menu")!=-1)
 {
 window.location.reload();
...
function copyOperation()
{
 ...
 usrc="/UserFunctionAction.do?&operation=17&tableName=tblApplyOutMaterials&saveDraft=&parentTableName=&moduleType="+moduleTypeValue+"&f_brother="+f_brother+"&noback=false&winCurIndex="+winCurIndex+"&keyId="+document.getElementById("varKeyIds").value.split("|")[0];
...
function telCall()
{
 ...
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblApplyOutMaterials&keyId="+keyIds,"","dialogWidth=600px;dialogHeight=350px"); 
 } 
 }else
 {
 alert("请选择至少一条记录"); 
...
function addPrepare(){
 ...
 usrc ='/UserFunctionAction.do?tableName=tblApplyOutMaterials&parentCode='+parentCode+'&operation=6&moduleType='+moduleTypeValue+'&f_brother='+f_brother+'&parentTableName=&winCurIndex='+winCurIndex;
...
function openSelect(selectName,fieldName,display){
 ...
 var urlstr = '/UserFunctionAction.do?operation=22&src=menu&parentTableName='+parentTableName+'&moduleType='+moduleType+'&displayName='+encodeURI(display)+'&LinkType=@URL:&tableName=tblApplyOutMaterials&selectName='+selectName+encodeURI(linkStr) ;
...
function exePopdiv(returnValue){
 ...
 if(typeof(str)!="undefined"){
 if(selectName=="ReportSelectGoods"){
 document.getElementById("GoodsNumber").value="";
 document.getElementById("GoodsFullName").value="";
 document.getElementById("GoodsSpec").value="";
 var group=str.split("|") ;
 var clear=true;
 for(var i=0;i<group.length;i++){
 if(group[i].length>0){
 var content = group[i].split(";") ;
 if(content[1-1].length>0){clear=false;document.getElementById("GoodsNumber").value+=content[1-1]+',';} 
 if(content[2-1].length>0){clear=false;document.getElementById("GoodsFullName").value+=content[2-1]+',';} 
 if(content[3-1].length>0){clear=false;document.getElementById("GoodsSpec").value+=content[3-1]+',';} 
 }
 }
 if(clear){
 document.getElementById("GoodsNumber").value="";
 ...
 document.getElementById("GoodsNumber").value=document.getElementById("GoodsNumber").value.substring(0,document.getElementById("GoodsNumber").value.length-1) ;
 document.getElementById("GoodsFullName").value=document.getElementById("GoodsFullName").value.substring(0,document.getElementById("GoodsFullName").value.length-1) ;
 document.getElementById("GoodsSpec").value=document.getElementById("GoodsSpec").value.substring(0,document.getElementById("GoodsSpec").value.length-1) ;
 }
 }
 }
}
...
function exePopadvancediv(returnValue){
 ...
 if(typeof(str)!="undefined"){
 if(selectName=="ReportSelectGoods"){
 jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsNumber").value="";
 jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsFullName").value="";
 jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsSpec").value="";
 var group=str.split("|") ;
 var clear=true;
 for(var i=0;i<group.length;i++){
 if(group[i].length>0){
 var content = group[i].split(";") ;
 if(content[1-1].length>0){clear=false;jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsNumber").value+=content[1-1]+',';} 
 if(content[2-1].length>0){clear=false;jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsFullName").value+=content[2-1]+',';} 
 if(content[3-1].length>0){clear=false;jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsSpec").value+=content[3-1]+',';} 
 }
 }
 if(clear){
 jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsNumber").value="";
 ...
 jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsNumber").value=jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsNumber").value.substring(0,jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsNumber").value.length-1) ;
 jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsFullName").value=jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsFullName").value.substring(0,jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsFullName").value.length-1) ;
 jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsSpec").value=jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsSpec").value.substring(0,jQuery.opener("advanceUpdatePanel").document.getElementById("GoodsSpec").value.length-1) ;
 }
 }
 }
}
...
function setFieldValue(str,selectName,fieldName){
 var fs=str.split(";");
 if(selectName=="ReportSelectGoods"){
 document.getElementById("GoodsNumber").value=fs[1-1];
 document.getElementById("GoodsFullName").value=fs[2-1];
 document.getElementById("GoodsSpec").value=fs[3-1];
 var lastObject = document.getElementById("GoodsSpec");
...
function showDivCustomSetTable(){ 
 var returnValue = window.showModalDialog("/UserFunctionQueryAction.do?tableName=tblApplyOutMaterials&operation=4&parentTableName=&configType=listConfig&checkTab=Y&winCurIndex="+winCurIndex,"","dialogWidth=670px;dialogHeight=450px") ;
...
function detail(str){
 turl="/UserFunctionAction.do?tableName=tblApplyOutMaterials&keyId="+str.split(",")[0]+"&moduleType="+moduleType+"&f_brother=&operation=5&winCurIndex="+winCurIndex+"&pageNo=&parentCode="+str.split(",")[1]+"&parentTableName=&saveDraft=";
...
function update(str){
 turl="/UserFunctionAction.do?tableName=tblApplyOutMaterials&keyId="+str.split(",")[0]+"&moduleType="+moduleType+"&f_brother=&operation=7&winCurIndex="+winCurIndex+"&pageNo=&parentCode="+str.split(",")[1]+"&parentTableName=&saveDraft=";
...
function next(str){
 location.href="/UserFunctionQueryAction.do?tableName=tblApplyOutMaterials&keyId="+str.split(",")[0]+"&parentCode="+str.split(",")[1]+"&operation=4&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&parentTableName=&checkTab=Y&selectType="+$("#selectType").val();
}


function doChangeParent(parentCode,name,ttableName){
 location.href="/UserFunctionQueryAction.do?tableName=tblApplyOutMaterials&parentCode="+parentCode+"&operation=4&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&parentTableName=&checkTab=Y&selectType="+$("#selectType").val();
}
...
function addClass(parentCodev){
 ...
 usrc ='/UserFunctionAction.do?tableName=tblApplyOutMaterials&parentCode='+parentCodev+'&operation=6&moduleType='+moduleTypeValue+'&f_brother='+f_brother+'&parentTableName=&winCurIndex='+winCurIndex;
...
function hurryTrans(str){
 ...
 hurryTransDialog("/vm/oa/workflow/noteModel.jsp?tableName=tblApplyOutMaterials&moduleName="+encodeURI(ren)+"&keyId="+str.split(",")[0],winObj,"dialogWidth=565px;dialogHeight=350px");
}
...
function check(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblApplyOutMaterials&operation=25&ButtonType=saveCheckBill&varKeyIds="+str.split(",")[0]+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+str.split(",")[0]+"&tableName=tblApplyOutMaterials&operation=82&fromPage=erp");
 ...
 location.href="/UserFunctionQueryAction.do?tableName=tblApplyOutMaterials&moduleType="+moduleType+"&keyId="+str.split(",")[0]+"&f_brother=&operation=83&winCurIndex="+winCurIndex+"&pageNo=&parentTableName=&saveDraft=";
 }else if(str.split(",")[1]=="cancel"){
 if(confirm('确定撤回?')){
 location.href='/UserFunctionQueryAction.do?nextStep=cancel&moduleType='+moduleType+'&tableName=tblApplyOutMaterials&lastNodeId='+str.split(",")[2]+'&keyId='+str.split(",")[0]+'&currNode='+str.split(",")[3]+'&f_brother=&operation=79&winCurIndex='+winCurIndex+'&pageNo=&parentTableName=&saveDraft='
 }
 }
}
...
function startFlow(){ 
 ...
 var urlstr = "/UtilServlet?operation=startFlow&moduleId="+keyId;
