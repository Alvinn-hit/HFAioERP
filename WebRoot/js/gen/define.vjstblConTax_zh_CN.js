...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblConTax&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblConTax&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblConTax&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblConTax&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblConTaxDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblConTaxDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblConTaxDet"){
...
function execStat2(){
 execStatOne('tblConTaxDet_CurAmount',2);
 execStatOne('tblConTaxDet_RMB',2);
 execStatOne('tblConTaxDet_Tariff',2);
 execStatOne('tblConTaxDet_ConTax',2);
 execStatOne('tblConTaxDet_DumpTax',2);
 execStatOne('tblConTaxDet_OtherTax',2);
 execStatOne('tblConTaxDet_Vat',2);
 execStatOne('tblConTaxDet_OtherTax2',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=fe844ae1_1207311340394840135&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=fe844ae1_1207311340394840135&MOOP=add'; 
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblConTax\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblConTaxDet_CurAmount',"tblConTaxDet"); 
 delFireEventOne(rowId,tableName,'tblConTaxDet_RMB',"tblConTaxDet"); 
 delFireEventOne(rowId,tableName,'tblConTaxDet_TariffRate',"tblConTaxDet"); 
 delFireEventOne(rowId,tableName,'tblConTaxDet_Tariff',"tblConTaxDet"); 
 delFireEventOne(rowId,tableName,'tblConTaxDet_ConRate',"tblConTaxDet"); 
 delFireEventOne(rowId,tableName,'tblConTaxDet_ConTax',"tblConTaxDet"); 
 delFireEventOne(rowId,tableName,'tblConTaxDet_DumpTax',"tblConTaxDet"); 
 delFireEventOne(rowId,tableName,'tblConTaxDet_OtherTax',"tblConTaxDet"); 
 delFireEventOne(rowId,tableName,'tblConTaxDet_VatRate',"tblConTaxDet"); 
 delFireEventOne(rowId,tableName,'tblConTaxDet_Vat',"tblConTaxDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'ActOtherTax2',dis:'实际其他(基金等)',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ExcRate',dis:'海关汇率',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblConTaxDet_CurAmount',dis:'外币货值',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblConTaxDet_RMB',dis:'本币货值',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblConTaxDet_TariffRate',dis:'关税税率(%)',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblConTaxDet_Tariff',dis:'关税',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblConTaxDet_ConRate',dis:'消费税率(%)',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblConTaxDet_ConTax',dis:'消费税',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblConTaxDet_DumpTax',dis:'反倾销税',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblConTaxDet_OtherTax',dis:'其他税',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblConTaxDet_VatRate',dis:'增值税率(%)',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblConTaxDet_Vat',dis:'增值税',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblConTaxDet_OtherTax2',dis:'其他(基金等)',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblConTax",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblConTax&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblConTax&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"fe844ae1_1207311340394840135",
 MOOP:"add",
...
function resetSubmit(fieldName){
 if(fieldName == "SheetNo"){
 var saveField=n("SheetNo"); 
 saveField[0].value = "";
 var obj = m("tbl_tblSheet_BillNo");
 if(obj != null){
 obj.value = "";
 }
 ...
 var saveField=n("ClientID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblSheet_ShipToID");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("Passageway"); 
 saveField[0].value = "";
 var obj = m("tbl_tblTruckNoList_Passageway");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblSheet_ShipTo");
 saveField[0].value = ""; 
 m("tbl_tblSheet_ShipTo").value = "" ;
 ...
 if(fieldName == "ExcRate"){
 var saveField=n("ExcRate"); 
 saveField[0].value = "";
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=fe844ae1_1207311340394840135&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=fe844ae1_1207311340394840135&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=fe844ae1_1207311340394840135&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblConTax"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblConTax&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblConTax&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('ActTaxAmount')[0].value=f(Number(nn('ActTariff',0))+Number(nn('ActConTax',0))+Number(nn('ActVat',0))+Number(nn('ActOtherTax',0))+Number(nn('ActDumpTax',0)),2);
...
if(n('Delivery')[0].value=="FOB")r(0,"RMB")[j].value=f(Number((Number(rn(0,"CurAmount",j))*1+Number(nn('OtherFare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",j)).toFixed(4)*1)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Fare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",j)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Insurance',0)/nn('TotalAmount',0)*rn(0,"CurAmount",j)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1,2);
}

for(var j=0;j<r(0,'RMB').length;j++){
if(n('Delivery')[0].value!="FOB")r(0,"RMB")[j].value=f(Number(nn('ExcRate',0)*rn(0,"CurAmount",j)).toFixed(0),2);
}

for(var j=0;j<r(0,'Tariff').length;j++){
r(0,"Tariff")[j].value=f(Number(rn(0,"RMB",j)*rn(0,"TariffRate",j)/100).toFixed(2),2);
}

for(var j=0;j<r(0,'ConTax').length;j++){
r(0,"ConTax")[j].value=f(Number((rn(0,"Tariff",j)+rn(0,"RMB",j))*(rn(0,"ConRate",j)/100)/(1-(rn(0,"ConRate",j)/100))).toFixed(2),2);
}

for(var j=0;j<r(0,'Vat').length;j++){
r(0,"Vat")[j].value=f(Number((rn(0,"Tariff",j)+rn(0,"ConTax",j)+rn(0,"RMB",j)+rn(0,"DumpTax",j)+rn(0,"OtherTax",j))*(rn(0,"VatRate",j)/100)).toFixed(2),2);
}

n('TotalTariff')[0].value=f(sm('tblConTaxDet_Tariff'),2);

n('TotalRMB')[0].value=f(sm('tblConTaxDet_RMB'),2);

n('TotalCurrency')[0].value=f(sm('tblConTaxDet_CurAmount'),2);

n('TotalConTax')[0].value=f(sm('tblConTaxDet_ConTax'),2);

n('TotalVat')[0].value=f(sm('tblConTaxDet_Vat'),2);

n('TotalOtherTax')[0].value=f(sm('tblConTaxDet_OtherTax'),2);

n('TotalDumpTax')[0].value=f(sm('tblConTaxDet_DumpTax'),2);

n('TotalTaxAmount')[0].value=f(nn('TotalTariff',0)+nn('TotalConTax',0)+nn('TotalVat',0)+nn('TotalOtherTax',0)+nn('TotalDumpTax',0),2);
...
n('TotalTaxAmount')[0].value=f(Number(nn('TotalTariff',0))+Number(nn('TotalConTax',0))+Number(nn('TotalVat',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDumpTax',0)),2);

 
 }
 } 

 var z=n('TotalCurrency');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalCurrency')[0].value=f(sm('tblConTaxDet_CurAmount'),2);
...
n('TotalTaxAmount')[0].value=f(Number(nn('TotalTariff',0))+Number(nn('TotalConTax',0))+Number(nn('TotalVat',0))+Number(nn('TotalOtherTax',0))+Number(nn('TotalDumpTax',0)),2);
 ...
 var z=n('BusinessType');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 if(n('BusinessType')[0].value==0)n('PaymentDays')[0].value=0;

if(n('BusinessType')[0].value==1)n('PaymentDays')[0].value=1;
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblConTaxDet_CurAmount'); 
 if(n('Delivery')[0].value=="FOB")r(0,"RMB")[p].value=f(Number((Number(rn(0,"CurAmount",p))*1+Number(nn('OtherFare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*1)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Fare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Insurance',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1,2);
if(n('Delivery')[0].value!="FOB")r(0,"RMB")[p].value=f(Number(nn('ExcRate',0)*rn(0,"CurAmount",p)).toFixed(0),2);
r(0,"Tariff")[p].value=f(Number(rn(0,"RMB",p)*rn(0,"TariffRate",p)/100).toFixed(2),2);
r(0,"ConTax")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"RMB",p))*(rn(0,"ConRate",p)/100)/(1-(rn(0,"ConRate",p)/100))).toFixed(2),2);
r(0,"Vat")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"ConTax",p)+rn(0,"RMB",p)+rn(0,"DumpTax",p)+rn(0,"OtherTax",p))*(rn(0,"VatRate",p)/100)).toFixed(2),2);
n('TotalTariff')[0].value=f(sm('tblConTaxDet_Tariff'),2);
n('TotalRMB')[0].value=f(sm('tblConTaxDet_RMB'),2);
n('TotalCurrency')[0].value=f(sm('tblConTaxDet_CurAmount'),2);
n('TotalConTax')[0].value=f(sm('tblConTaxDet_ConTax'),2);
...
n('TotalTaxAmount')[0].value=f(nn('TotalTariff',0)+nn('TotalConTax',0)+nn('TotalVat',0)+nn('TotalOtherTax',0)+nn('TotalDumpTax',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblConTaxDet_RMB'); 
 if(n('Delivery')[0].value=="FOB")r(0,"RMB")[p].value=f(Number((Number(rn(0,"CurAmount",p))*1+Number(nn('OtherFare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*1)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Fare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Insurance',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1,2);
if(n('Delivery')[0].value!="FOB")r(0,"RMB")[p].value=f(Number(nn('ExcRate',0)*rn(0,"CurAmount",p)).toFixed(0),2);
r(0,"Tariff")[p].value=f(Number(rn(0,"RMB",p)*rn(0,"TariffRate",p)/100).toFixed(2),2);
r(0,"ConTax")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"RMB",p))*(rn(0,"ConRate",p)/100)/(1-(rn(0,"ConRate",p)/100))).toFixed(2),2);
...
n('TotalTaxAmount')[0].value=f(nn('TotalTariff',0)+nn('TotalConTax',0)+nn('TotalVat',0)+nn('TotalOtherTax',0)+nn('TotalDumpTax',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblConTaxDet_TariffRate'); 
 if(n('Delivery')[0].value=="FOB")r(0,"RMB")[p].value=f(Number((Number(rn(0,"CurAmount",p))*1+Number(nn('OtherFare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*1)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Fare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Insurance',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1,2);
if(n('Delivery')[0].value!="FOB")r(0,"RMB")[p].value=f(Number(nn('ExcRate',0)*rn(0,"CurAmount",p)).toFixed(0),2);
r(0,"Tariff")[p].value=f(Number(rn(0,"RMB",p)*rn(0,"TariffRate",p)/100).toFixed(2),2);
r(0,"ConTax")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"RMB",p))*(rn(0,"ConRate",p)/100)/(1-(rn(0,"ConRate",p)/100))).toFixed(2),2);
...
n('TotalTaxAmount')[0].value=f(nn('TotalTariff',0)+nn('TotalConTax',0)+nn('TotalVat',0)+nn('TotalOtherTax',0)+nn('TotalDumpTax',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblConTaxDet_Tariff'); 
 if(n('Delivery')[0].value=="FOB")r(0,"RMB")[p].value=f(Number((Number(rn(0,"CurAmount",p))*1+Number(nn('OtherFare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*1)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Fare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Insurance',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1,2);
if(n('Delivery')[0].value!="FOB")r(0,"RMB")[p].value=f(Number(nn('ExcRate',0)*rn(0,"CurAmount",p)).toFixed(0),2);
r(0,"Tariff")[p].value=f(Number(rn(0,"RMB",p)*rn(0,"TariffRate",p)/100).toFixed(2),2);
r(0,"ConTax")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"RMB",p))*(rn(0,"ConRate",p)/100)/(1-(rn(0,"ConRate",p)/100))).toFixed(2),2);
...
n('TotalTaxAmount')[0].value=f(nn('TotalTariff',0)+nn('TotalConTax',0)+nn('TotalVat',0)+nn('TotalOtherTax',0)+nn('TotalDumpTax',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblConTaxDet_ConRate'); 
 if(n('Delivery')[0].value=="FOB")r(0,"RMB")[p].value=f(Number((Number(rn(0,"CurAmount",p))*1+Number(nn('OtherFare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*1)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Fare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Insurance',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1,2);
if(n('Delivery')[0].value!="FOB")r(0,"RMB")[p].value=f(Number(nn('ExcRate',0)*rn(0,"CurAmount",p)).toFixed(0),2);
r(0,"Tariff")[p].value=f(Number(rn(0,"RMB",p)*rn(0,"TariffRate",p)/100).toFixed(2),2);
r(0,"ConTax")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"RMB",p))*(rn(0,"ConRate",p)/100)/(1-(rn(0,"ConRate",p)/100))).toFixed(2),2);
...
n('TotalTaxAmount')[0].value=f(nn('TotalTariff',0)+nn('TotalConTax',0)+nn('TotalVat',0)+nn('TotalOtherTax',0)+nn('TotalDumpTax',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblConTaxDet_ConTax'); 
 if(n('Delivery')[0].value=="FOB")r(0,"RMB")[p].value=f(Number((Number(rn(0,"CurAmount",p))*1+Number(nn('OtherFare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*1)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Fare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Insurance',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1,2);
if(n('Delivery')[0].value!="FOB")r(0,"RMB")[p].value=f(Number(nn('ExcRate',0)*rn(0,"CurAmount",p)).toFixed(0),2);
r(0,"Tariff")[p].value=f(Number(rn(0,"RMB",p)*rn(0,"TariffRate",p)/100).toFixed(2),2);
r(0,"ConTax")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"RMB",p))*(rn(0,"ConRate",p)/100)/(1-(rn(0,"ConRate",p)/100))).toFixed(2),2);
...
n('TotalTaxAmount')[0].value=f(nn('TotalTariff',0)+nn('TotalConTax',0)+nn('TotalVat',0)+nn('TotalOtherTax',0)+nn('TotalDumpTax',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblConTaxDet_DumpTax'); 
 if(n('Delivery')[0].value=="FOB")r(0,"RMB")[p].value=f(Number((Number(rn(0,"CurAmount",p))*1+Number(nn('OtherFare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*1)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Fare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Insurance',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1,2);
if(n('Delivery')[0].value!="FOB")r(0,"RMB")[p].value=f(Number(nn('ExcRate',0)*rn(0,"CurAmount",p)).toFixed(0),2);
r(0,"Tariff")[p].value=f(Number(rn(0,"RMB",p)*rn(0,"TariffRate",p)/100).toFixed(2),2);
r(0,"ConTax")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"RMB",p))*(rn(0,"ConRate",p)/100)/(1-(rn(0,"ConRate",p)/100))).toFixed(2),2);
...
n('TotalTaxAmount')[0].value=f(nn('TotalTariff',0)+nn('TotalConTax',0)+nn('TotalVat',0)+nn('TotalOtherTax',0)+nn('TotalDumpTax',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblConTaxDet_OtherTax'); 
 if(n('Delivery')[0].value=="FOB")r(0,"RMB")[p].value=f(Number((Number(rn(0,"CurAmount",p))*1+Number(nn('OtherFare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*1)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Fare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Insurance',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1,2);
if(n('Delivery')[0].value!="FOB")r(0,"RMB")[p].value=f(Number(nn('ExcRate',0)*rn(0,"CurAmount",p)).toFixed(0),2);
r(0,"Tariff")[p].value=f(Number(rn(0,"RMB",p)*rn(0,"TariffRate",p)/100).toFixed(2),2);
r(0,"ConTax")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"RMB",p))*(rn(0,"ConRate",p)/100)/(1-(rn(0,"ConRate",p)/100))).toFixed(2),2);
...
n('TotalTaxAmount')[0].value=f(nn('TotalTariff',0)+nn('TotalConTax',0)+nn('TotalVat',0)+nn('TotalOtherTax',0)+nn('TotalDumpTax',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblConTaxDet_VatRate'); 
 if(n('Delivery')[0].value=="FOB")r(0,"RMB")[p].value=f(Number((Number(rn(0,"CurAmount",p))*1+Number(nn('OtherFare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*1)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Fare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Insurance',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1,2);
if(n('Delivery')[0].value!="FOB")r(0,"RMB")[p].value=f(Number(nn('ExcRate',0)*rn(0,"CurAmount",p)).toFixed(0),2);
r(0,"Tariff")[p].value=f(Number(rn(0,"RMB",p)*rn(0,"TariffRate",p)/100).toFixed(2),2);
r(0,"ConTax")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"RMB",p))*(rn(0,"ConRate",p)/100)/(1-(rn(0,"ConRate",p)/100))).toFixed(2),2);
...
n('TotalTaxAmount')[0].value=f(nn('TotalTariff',0)+nn('TotalConTax',0)+nn('TotalVat',0)+nn('TotalOtherTax',0)+nn('TotalDumpTax',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblConTaxDet_Vat'); 
 if(n('Delivery')[0].value=="FOB")r(0,"RMB")[p].value=f(Number((Number(rn(0,"CurAmount",p))*1+Number(nn('OtherFare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*1)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Fare',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1+Number(Number(nn('Insurance',0)/nn('TotalAmount',0)*rn(0,"CurAmount",p)).toFixed(4)*nn('ExcRate',0)).toFixed(0)*1,2);
if(n('Delivery')[0].value!="FOB")r(0,"RMB")[p].value=f(Number(nn('ExcRate',0)*rn(0,"CurAmount",p)).toFixed(0),2);
r(0,"Tariff")[p].value=f(Number(rn(0,"RMB",p)*rn(0,"TariffRate",p)/100).toFixed(2),2);
r(0,"ConTax")[p].value=f(Number((rn(0,"Tariff",p)+rn(0,"RMB",p))*(rn(0,"ConRate",p)/100)/(1-(rn(0,"ConRate",p)/100))).toFixed(2),2);
...
childData[childCount]='tblConTaxDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='缴税明细表明细';
childCount++;
 var tblConTaxDetData = new gridData();
 gridDatas[gridDataNum] = tblConTaxDetData;
 gridDataNum++;
 var tblConTaxDet=1;
 ...
 addCols(tblConTaxDetData,'tblConTaxDet_GoodsName','品名','110',2,false,'',50,8,'','',"","","2",'',"8",false); 
 putChildValidateItem("tblConTaxDet_GoodsName","品名","any","",false,minValue,50,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_GoodsNumber','税号','100',2,false,'',10,8,'','',"","","4",'',"8",false); 
 putChildValidateItem("tblConTaxDet_GoodsNumber","税号","any","",false,minValue,10,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_CurAmount','外币货值','70',1,true,'',0,8,'','',"Amount","","8",'',"8",false); 
 putChildValidateItem("tblConTaxDet_CurAmount","外币货值","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_RMB','本币货值','70',1,true,'',0,0,'','',"Amount","","12",'',"0",false); 
 putChildValidateItem("tblConTaxDet_RMB","本币货值","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_TariffRate','关税税率(%)','70',1,true,'',0,0,'','',"","","14",'',"0",false); 
 putChildValidateItem("tblConTaxDet_TariffRate","关税税率(%)","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_Tariff','关税','60',1,true,'',0,0,'','',"Amount","","16",'',"0",false); 
 putChildValidateItem("tblConTaxDet_Tariff","关税","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_ConRate','消费税率(%)','70',1,true,'',0,0,'','',"","","18",'',"0",false); 
 putChildValidateItem("tblConTaxDet_ConRate","消费税率(%)","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_ConTax','消费税','60',1,true,'',0,0,'','',"Amount","","20",'',"0",false); 
 putChildValidateItem("tblConTaxDet_ConTax","消费税","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_DumpTax','反倾销税','60',1,true,'',0,0,'','',"Amount","","22",'',"0",false); 
 putChildValidateItem("tblConTaxDet_DumpTax","反倾销税","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_OtherTax','其他税','60',1,true,'',0,0,'','',"Amount","","24",'',"0",false); 
 putChildValidateItem("tblConTaxDet_OtherTax","其他税","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_VatRate','增值税率(%)','70',1,true,'',0,0,'','',"","","26",'',"0",false); 
 putChildValidateItem("tblConTaxDet_VatRate","增值税率(%)","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_Vat','增值税','60',1,true,'',0,0,'','',"Amount","","28",'',"0",false); 
 putChildValidateItem("tblConTaxDet_Vat","增值税","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_OtherTax2','其他(基金等)','70',1,true,'',0,0,'','',"","","29",'',"0",false); 
 putChildValidateItem("tblConTaxDet_OtherTax2","其他(基金等)","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_Remark','备注','120',2,true,'',8000,0,'','',"","","30",'',"0",false); 
 putChildValidateItem("tblConTaxDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_SourceID','源ID','0',2,true,'',100,-2,'','',"","","32",'',"3",false); 
 putChildValidateItem("tblConTaxDet_SourceID","源ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblConTaxDetData,'tblConTaxDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblConTax&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblConTax,tblConTaxDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblConTax&allTableName=tblConTax,tblConTaxDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblConTax&allTableName=tblConTaxDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblConTax&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblConTax&allTableName=tblConTaxDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=fe844ae1_1207311340394840135&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=fe844ae1_1207311340394840135&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=fe844ae1_1207311340394840135&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=fe844ae1_1207311340394840135&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblConTax&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblConTax&designId=670dad46_1408011513589758963&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblConTax&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblConTax&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblConTax&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '缴税明细';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblConTax&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
