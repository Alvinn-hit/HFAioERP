...
{ 
 cFireEvent('PDWorkOrderDet_Qty',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDWorkOrder&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDWorkOrder&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDWorkOrder&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDWorkOrder&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDWorkOrderDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('PDWorkOrderDet_GoodsCode');
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDWorkOrderDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDWorkOrderDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="PDWorkOrderDet"; 
 var goodsCodeLogicValidate_PDWorkOrderDet='@IOO:Outstore';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'PDWorkOrderDet',value:goodsCodeLogicValidate_PDWorkOrderDet};
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDWorkOrder\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDWorkOrder\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'PDWorkOrderDet_UseQty',"PDWorkOrderDet"); 
 delFireEventOne(rowId,tableName,'PDWorkOrderDet_Qty',"PDWorkOrderDet"); 
 delFireEventOne(rowId,tableName,'PDWorkOrderDet_BakQty',"PDWorkOrderDet"); 
 delFireEventOne(rowId,tableName,'PDWorkOrderDet_StandLossRate',"PDWorkOrderDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'Qty',dis:'生产数量',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'InPayQty',dis:'缴库数',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_UseQty',dis:'单位用量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_Qty',dis:'需求量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_SendQty',dis:'已发料数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_SendQtyExt',dis:'未发料数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_BakQty',dis:'备品量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_SendBakQty',dis:'已发备品量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_SendBakQtyExt',dis:'未发备品量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_NoBakQty',dis:'非备品补料量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_ReturnQty',dis:'退货量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_OutQty',dis:'超领量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_CheckQty',dis:'验冲数',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_LossRate',dis:'损耗率%',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderDet_StandLossRate',dis:'标准损率',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDWorkOrder",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDWorkOrder&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDWorkOrder&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 saveField[0].value = "";
 var obj = m("tbl_PDWorkShop_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("PDWorkShop_name");
 saveField[0].value = ""; 
 m("tbl_PDWorkShop_name").value = "" ;
 }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDWorkOrder"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDWorkOrder&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDWorkOrder&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
r(0,"Qty")[j].value=f(rn(0,"UseQty",j)*nn('Qty',0),4);
}

for(var j=0;j<r(0,'BakQty').length;j++){
r(0,"BakQty")[j].value=f(rn(0,"Qty",j)*rn(0,"StandLossRate",j)/100,4);
}

for(var j=0;j<r(0,'SendQtyExt').length;j++){
r(0,"SendQtyExt")[j].value=f(rn(0,"Qty",j)-rn(0,"SendQty",j),4);
}

for(var j=0;j<r(0,'SendBakQtyExt').length;j++){
r(0,"SendBakQtyExt")[j].value=f(rn(0,"BakQty",j)-rn(0,"SendBakQty",j),4);
}
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDWorkOrderDet_UseQty'); 
 r(0,"Qty")[p].value=f(rn(0,"UseQty",p)*nn('Qty',0),4);
r(0,"SendQtyExt")[p].value=f(rn(0,"Qty",p)-rn(0,"SendQty",p),4);
r(0,"BakQty")[p].value=f(rn(0,"Qty",p)*rn(0,"StandLossRate",p)/100,4);
r(0,"SendBakQtyExt")[p].value=f(rn(0,"BakQty",p)-rn(0,"SendBakQty",p),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDWorkOrderDet_Qty'); 
 r(0,"UseQty")[p].value=f(rn(0,"Qty",p)/nn('Qty',0),4);
r(0,"SendQtyExt")[p].value=f(rn(0,"Qty",p)-rn(0,"SendQty",p),4);
r(0,"BakQty")[p].value=f(rn(0,"Qty",p)*rn(0,"StandLossRate",p)/100,4);
r(0,"SendBakQtyExt")[p].value=f(rn(0,"BakQty",p)-rn(0,"SendBakQty",p),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDWorkOrderDet_BakQty'); 
 r(0,"StandLossRate")[p].value=f(rn(0,"BakQty",p)/rn(0,"Qty",p)*100,4);
r(0,"SendBakQtyExt")[p].value=f(rn(0,"BakQty",p)-rn(0,"SendBakQty",p),4);

 execStat(); 
 }
 }
 var z=r(0,'StandLossRate');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDWorkOrderDet_StandLossRate'); 
 r(0,"BakQty")[p].value=f(rn(0,"Qty",p)*rn(0,"StandLossRate",p)/100,4);
r(0,"SendBakQtyExt")[p].value=f(rn(0,"BakQty",p)-rn(0,"SendBakQty",p),4);
...
childData[childCount]='PDWorkOrderDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='工令单原料明细';
childCount++;
 var PDWorkOrderDetData = new gridData();
 gridDatas[gridDataNum] = PDWorkOrderDetData;
 gridDataNum++;
 var PDWorkOrderDet=1;
 ...
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_GoodsCode','原料名称','100',2,false,'',100,-2,'','',"","triggerCopy","0",'',"2",true);
 putChildValidateItem("PDWorkOrderDet_GoodsCode","原料名称","any","GoodsField",false,minValue,100,false,'');
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'PDWorkOrderDet:GoodsCode:PDWorkOrderDet_tblGoods_GoodsNumber:'+param4,'',"","triggerCopy","0",'',"2",false); 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDWorkOrderDet:GoodsCode:PDWorkOrderDet_tblGoods_GoodsSpec:'+param4,'',"","triggerCopy","0",'',"2",false); 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDWorkOrderDet:GoodsCode:PDWorkOrderDet_tblGoods_BaseUnit:'+param4,'',"","triggerCopy","0",'',"2",false); 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_tblGoods_BarCode','条码','100',2,true,'',1000000,2,'PDWorkOrderDet:GoodsCode:PDWorkOrderDet_tblGoods_BarCode:'+param4,'',"","triggerCopy","0",'',"2",false); 
 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_bomId','商品对应的Bom','200',2,true,'',100,-2,'','',"","","1",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_bomId","商品对应的Bom","any","",true,minValue,100,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_BatchNo','批号','200',2,true,'',100,-2,'','',"","","2",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_BatchNo","批号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","3",'',"2",false); 
 putChildValidateItem("PDWorkOrderDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","4",'',"2",false); 
 putChildValidateItem("PDWorkOrderDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 ...
 var param4="";
 param4=param4+":1原料名称@PDWorkOrderDet_GoodsCode";
 param4=param4+":StockCode@PDWorkOrderDet_StockCode";
 param4=param4+":0仓库@StockCode"; 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_yearNO','箱号','100',2,true,'',100,2,'PDWorkOrderDet:yearNO:PDWorkOrderDet_yearNO:'+param4,"","","triggerCopy","5",'',"2",true);
 putChildValidateItem("PDWorkOrderDet_yearNO","箱号","any","GoodsField",true,minValue,100,false,'',true);
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_ProDate','生产日期','200',5,true,'',10,-2,'','',"","","6",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_ProDate","生产日期","date","",true,minValue,10,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_Availably','保质期限','200',5,true,'',10,-2,'','',"","","7",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_Availably","保质期限","date","",true,minValue,10,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_isClient','客供料','50',0,false,'2',0,1,';1:是;2:否','',"","","8",'',"1",false);
 putChildValidateItem("PDWorkOrderDet_isClient","客供料","int","",false,minValue,maxValue,true,'2',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_UseQty','单位用量','60',1,true,'',0,0,'','',"Qty","","9",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_UseQty","单位用量","double","GoodsField",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_Qty','需求量','80',1,true,'',0,0,'','',"Qty","","10",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_Qty","需求量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_SendQty','已发料数量','80',1,true,'',0,8,'','',"Qty","","11",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_SendQty","已发料数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_SendQtyExt','未发料数量','80',1,true,'',0,8,'','',"Qty","","12",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_SendQtyExt","未发料数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_BakQty','备品量','80',1,true,'',0,0,'','',"Qty","","13",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_BakQty","备品量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_SendBakQty','已发备品量','80',1,true,'',0,8,'','',"Qty","","14",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_SendBakQty","已发备品量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_SendBakQtyExt','未发备品量','80',1,true,'',0,8,'','',"Qty","","15",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_SendBakQtyExt","未发备品量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_NoBakQty','非备品补料量','80',1,true,'',0,8,'','',"Qty","","16",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_NoBakQty","非备品补料量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_ReturnQty','退货量','80',1,true,'',0,8,'','',"Qty","","17",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_ReturnQty","退货量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_OutQty','超领量','80',1,true,'',0,8,'','',"Qty","","18",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_OutQty","超领量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_CheckQty','验冲数','80',1,true,'',0,8,'','',"Qty","","19",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_CheckQty","验冲数","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_LossRate','损耗率%','80',1,true,'',0,8,'','',"Qty","","20",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_LossRate","损耗率%","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_StandLossRate','标准损率','80',1,true,'',0,0,'','',"Qty","","21",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_StandLossRate","标准损率","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_SecUnit','辅助单位','200',2,true,'',30,-2,'','',"","","22",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_SecUnit","辅助单位","any","",true,minValue,30,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_UnitIntQty','辅助单位整数','200',0,true,'',0,-2,'','',"Qty","","23",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_UnitIntQty","辅助单位整数","int","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_UnitBaseQty','辅助基本单位','200',1,true,'',0,-2,'','',"Qty","","24",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_UnitBaseQty","辅助基本单位","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_UnitQty','辅助单位数量','200',1,true,'',0,-2,'','',"Qty","","25",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_UnitQty","辅助单位数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_UnitPrice','辅助单位单价','200',1,true,'',0,-2,'','',"Price","","26",'',"2",false); 
 putChildValidateItem("PDWorkOrderDet_UnitPrice","辅助单位单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_SecQty','辅助数量','200',0,true,'',0,-2,'','',"Qty","","27",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_SecQty","辅助数量","int","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_BaseQty','基本单位','200',0,true,'',0,-2,'','',"Qty","","28",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_BaseQty","基本单位","int","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_Remark','备注','100',2,true,'',800,0,'','',"","","29",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_Remark","备注","any","",true,minValue,800,false,'',true); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_InPayQty','原料出库数','200',1,true,'',0,-2,'','',"Qty","","30",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_InPayQty","原料出库数","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_bakOutQty','补料出库数','200',1,true,'',0,-2,'','',"Qty","","31",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_bakOutQty","补料出库数","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_noBakOutQty','正常料出库','200',1,true,'',0,-2,'','',"Qty","","32",'',"0",false); 
 putChildValidateItem("PDWorkOrderDet_noBakOutQty","正常料出库","double","",true,minValue,maxValue,false,''); 
 
 
 addCols(PDWorkOrderDetData,'PDWorkOrderDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDWorkOrder&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDWorkOrder,PDWorkOrderDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDWorkOrder&allTableName=PDWorkOrder,PDWorkOrderDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDWorkOrder&allTableName=PDWorkOrderDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDWorkOrder&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDWorkOrder&allTableName=PDWorkOrderDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDWorkOrder&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDWorkOrder&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDWorkOrder&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDWorkOrder&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDWorkOrder&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '工令单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDWorkOrder&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
