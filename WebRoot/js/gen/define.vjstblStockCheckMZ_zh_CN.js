...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblStockCheckMZ&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblStockCheckMZ&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblStockCheckMZ&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblStockCheckMZ&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblStockCheckMZDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblStockCheckMZDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblStockCheckMZDet"){
...
function execStat2(){
 execStatOne('tblStockCheckMZDet_Qty',6);
 execStatOne('tblStockCheckMZDet_StockAmount',2);
 execStatOne('tblStockCheckMZDet_CheckQty',6);
 execStatOne('tblStockCheckMZDet_Amount',8);
 execStatOne('tblStockCheckMZDet_LostQty',6);
 execStatOne('tblStockCheckMZDet_LostAmount',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblStockCheckMZ\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblStockCheckMZ\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblStockCheckMZDet_CheckQty',"tblStockCheckMZDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblStockCheckMZDet_Price',dis:'单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblStockCheckMZDet_Qty',dis:'库存数',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblStockCheckMZDet_StockAmount',dis:'金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblStockCheckMZDet_CheckQty',dis:'盘点数',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblStockCheckMZDet_Amount',dis:'余额',digit:8,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblStockCheckMZDet_LostQty',dis:'盈亏数',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblStockCheckMZDet_LostAmount',dis:'盈亏金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblStockCheckMZ",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblStockCheckMZ&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblStockCheckMZ&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 var saveField=n("tblStock_StockFullNa"); 
 saveField[0].value = "";
 ...
 var saveField=n("tblProject_name");
 saveField[0].value = ""; 
 m("tbl_tblProject_name").value = "" ;
 ...
 var saveField=n("Stock_StockFullNa");
 saveField[0].value = ""; 
 m("tbl_Stock_StockFullNa").value = "" ;
 }
 }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblStockCheckMZ"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblStockCheckMZ&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblStockCheckMZ&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblStockCheckMZDet_CheckQty'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"CheckQty",p),8);
r(0,"LostQty")[p].value=f(rn(0,"CheckQty",p)-rn(0,"Qty",p),6);
r(0,"LostAmount")[p].value=f(rn(0,"LostQty",p)*rn(0,"Price",p),2);
...
childData[childCount]='tblStockCheckMZDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='美芝盘点明细';
childCount++;
 var tblStockCheckMZDetData = new gridData();
 gridDatas[gridDataNum] = tblStockCheckMZDetData;
 gridDataNum++;
 var tblStockCheckMZDet=1;
 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_GoodsCode','商品code','100',2,false,'',100,-2,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_GoodsCode","商品code","any","",false,minValue,100,false,''); 
 ...
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_WareName','材料名称','100',2,true,'',100,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("tblStockCheckMZDet_WareName","材料名称","any","",true,minValue,100,false,'');
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_(case tblGoods_isCatalog when 0 then tblStocks.lastQty else -99999999 end)','实际库存','100',0,true,'',1000000,2,'tblStockCheckMZDet:WareName:tblStockCheckMZDet_(case tblGoods_isCatalog when 0 then tblStocks.lastQty else -99999999 end):'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_Specification','规格/型号','100',2,true,'',100,8,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_Specification","规格/型号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_Brand','品牌','100',2,true,'',100,8,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_Brand","品牌","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_Batch','批号','90',2,true,'',100,8,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_Batch","批号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_WareUnit','计量单位','50',2,true,'',100,8,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_WareUnit","计量单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_Price','单价','50',1,true,'',0,8,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_Qty','库存数','60',1,false,'',0,8,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_Qty","库存数","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_StockAmount','金额','70',1,true,'',0,8,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_StockAmount","金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_CheckQty','盘点数','60',1,false,'',0,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_CheckQty","盘点数","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_Amount','余额','70',1,true,'',0,8,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_Amount","余额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_LostQty','盈亏数','60',1,true,'',0,8,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_LostQty","盈亏数","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_LostAmount','盈亏金额','60',1,true,'',0,8,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_LostAmount","盈亏金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_Remark','备注','100',18,true,'',5000,0,'','',"","","13",'',"0",false); 
 putChildValidateItem("tblStockCheckMZDet_Remark","备注","any","",true,minValue,5000,false,'',true); 
 
 
 
 
 addCols(tblStockCheckMZDetData,'tblStockCheckMZDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblStockCheckMZ&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblStockCheckMZ,tblStockCheckMZDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblStockCheckMZ&allTableName=tblStockCheckMZ,tblStockCheckMZDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblStockCheckMZ&allTableName=tblStockCheckMZDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblStockCheckMZ&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblStockCheckMZ&allTableName=tblStockCheckMZDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblStockCheckMZDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblStockCheckMZ&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblStockCheckMZ&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblStockCheckMZ&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblStockCheckMZ&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblStockCheckMZ&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '美芝盘点单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblStockCheckMZ&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
