...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblWatchBand1&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblWatchBand1&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblWatchBand1&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblWatchBand1&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchBand1DetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchBand1Det1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblWatchBand1Det"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblWatchBand1Det1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchBand1Det"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchBand1Det1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblWatchBand1Det"; 
 tblName[1]="tblWatchBand1Det1"; 
...
function execStat2(){
 execStatOne('tblWatchBand1Det_Qty',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblWatchBand1\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblWatchBand1\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'tblWatchBand1Det_Qty',dis:'用量',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblWatchBand1",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchBand1&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchBand1&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblWatchBand1"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblWatchBand1&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchBand1&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblWatchBand1Det';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表带物料明细';
childCount++;
 var tblWatchBand1DetData = new gridData();
 gridDatas[gridDataNum] = tblWatchBand1DetData;
 gridDataNum++;
 var tblWatchBand1Det=1;
  var param4="";
 param4=param4+":1样板编号@SampleOrderID"; 
 addCols(tblWatchBand1DetData,'tblWatchBand1Det_GoodsNumber','物料编号','200',2,false,'',500,2,'tblWatchBand1Det:GoodsNumber:tblWatchBand1Det_GoodsNumber:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblWatchBand1Det_GoodsNumber","物料编号","any","",false,minValue,500,false,'',true);
  addCols(tblWatchBand1DetData,'tblWatchBand1Det_BandNum','带编号','120',2,true,'',200,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchBand1Det_BandNum","带编号","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchBand1DetData,'tblWatchBand1Det_Shading','底料/底纹/底色','120',2,true,'',200,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchBand1Det_Shading","底料/底纹/底色","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchBand1DetData,'tblWatchBand1Det_CuletColor','面料/面纹/面色','120',2,true,'',200,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblWatchBand1Det_CuletColor","面料/面纹/面色","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchBand1DetData,'tblWatchBand1Det_Grind','扣打磨/打唛','120',2,true,'',200,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblWatchBand1Det_Grind","扣打磨/打唛","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchBand1DetData,'tblWatchBand1Det_LineColor','线色','120',2,true,'',200,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblWatchBand1Det_LineColor","线色","any","",true,minValue,200,false,'',true); 
  addCols(tblWatchBand1DetData,'tblWatchBand1Det_Qty','用量','80',1,false,'1',0,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblWatchBand1Det_Qty","用量","double","",false,minValue,maxValue,true,'1',true); 
  addCols(tblWatchBand1DetData,'tblWatchBand1Det_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchBand1Det_Remark","备注","any","",true,minValue,500,false,'',true); 
    addCols(tblWatchBand1DetData,'tblWatchBand1Det_GoodsCode','物料名称','100',2,true,'',1000,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblWatchBand1Det_GoodsCode","物料名称","any","",true,minValue,1000,false,''); 
                
 addCols(tblWatchBand1DetData,'tblWatchBand1Det_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblWatchBand1Det1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表带变更明细';
childCount++;
 var tblWatchBand1Det1Data = new gridData();
 gridDatas[gridDataNum] = tblWatchBand1Det1Data;
 gridDataNum++;
 var tblWatchBand1Det1=1;
  addCols(tblWatchBand1Det1Data,'tblWatchBand1Det1_ChangeContent','变更内容','200',2,false,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblWatchBand1Det1_ChangeContent","变更内容","any","",false,minValue,500,false,'',true); 
  addCols(tblWatchBand1Det1Data,'tblWatchBand1Det1_ChangeDate','变更日期','90',5,false,'2013-12-24',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchBand1Det1_ChangeDate","变更日期","date","",false,minValue,10,true,'2013-12-24',true); 
  addCols(tblWatchBand1Det1Data,'tblWatchBand1Det1_ChangePer','变更人员','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchBand1Det1_ChangePer","变更人员","any","",true,minValue,100,false,'',true); 
  addCols(tblWatchBand1Det1Data,'tblWatchBand1Det1_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchBand1Det1_Remark","备注","any","",true,minValue,500,false,'',true); 
       
 addCols(tblWatchBand1Det1Data,'tblWatchBand1Det1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblWatchBand1&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=68fc380d_1408131539576500232&noback="+noback+"&parentCode=&operation=6&parentTableName=tblSampleOrder&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblWatchBand1,tblWatchBand1Det,tblWatchBand1Det1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchBand1&allTableName=tblWatchBand1,tblWatchBand1Det,tblWatchBand1Det1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchBand1&allTableName=tblWatchBand1Det,tblWatchBand1Det1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchBand1&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchBand1&allTableName=tblWatchBand1Det,tblWatchBand1Det1,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblWatchBand1&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblWatchBand1&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblWatchBand1&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchBand1&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblWatchBand1&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '表带(胶带)';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblWatchBand1&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
