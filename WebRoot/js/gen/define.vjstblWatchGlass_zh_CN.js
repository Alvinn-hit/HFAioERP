...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblWatchGlass&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblWatchGlass&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblWatchGlass&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblWatchGlass&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchGlassDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchGlassDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblWatchGlassDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblWatchGlassDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchGlassDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchGlassDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblWatchGlassDet"; 
 tblName[1]="tblWatchGlassDet1"; 
...
function execStat2(){
 execStatOne('tblWatchGlassDet_Price',2);
 execStatOne('tblWatchGlassDet_Qty',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblWatchGlass\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblWatchGlass\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblWatchGlassDet_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchGlassDet_Qty',dis:'用量',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblWatchGlass",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchGlass&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchGlass&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblWatchGlass"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblWatchGlass&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchGlass&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblWatchGlassDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='玻璃物料明细';
childCount++;
 var tblWatchGlassDetData = new gridData();
 gridDatas[gridDataNum] = tblWatchGlassDetData;
 gridDataNum++;
 var tblWatchGlassDet=1;
 ...
 addCols(tblWatchGlassDetData,'tblWatchGlassDet_GoodsCode','物料名称','100',2,true,'',1000,-2,'','',"","","1",'',"3",false); 
 putChildValidateItem("tblWatchGlassDet_GoodsCode","物料名称","any","",true,minValue,1000,false,''); 
 ...
 addCols(tblWatchGlassDetData,'tblWatchGlassDet_GoodsNumber','物料编号','200',2,false,'',500,2,'tblWatchGlassDet:GoodsNumber:tblWatchGlassDet_GoodsNumber:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblWatchGlassDet_GoodsNumber","物料编号","any","",false,minValue,500,false,'',true);
 
 
 
 addCols(tblWatchGlassDetData,'tblWatchGlassDet_Color1','电镀边颜色','120',2,true,'',100,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchGlassDet_Color1","电镀边颜色","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblWatchGlassDetData,'tblWatchGlassDet_Color2','光边颜色','120',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchGlassDet_Color2","光边颜色","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblWatchGlassDetData,'tblWatchGlassDet_Grind','打磨','120',2,true,'',200,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblWatchGlassDet_Grind","打磨","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchGlassDetData,'tblWatchGlassDet_Price','单价','80',1,true,'0',0,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblWatchGlassDet_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblWatchGlassDetData,'tblWatchGlassDet_Qty','用量','80',1,true,'1',0,0,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblWatchGlassDet_Qty","用量","double","",true,minValue,maxValue,true,'1',true); 
 
 
 
 addCols(tblWatchGlassDetData,'tblWatchGlassDet_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchGlassDet_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 
 addCols(tblWatchGlassDetData,'tblWatchGlassDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblWatchGlassDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='玻璃变更明细';
childCount++;
 var tblWatchGlassDet1Data = new gridData();
 gridDatas[gridDataNum] = tblWatchGlassDet1Data;
 gridDataNum++;
 var tblWatchGlassDet1=1;
 ...
 addCols(tblWatchGlassDet1Data,'tblWatchGlassDet1_ChangeContent','变更内容','200',2,false,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblWatchGlassDet1_ChangeContent","变更内容","any","",false,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchGlassDet1Data,'tblWatchGlassDet1_ChangeDate','变更日期','90',5,false,'2013-12-24',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchGlassDet1_ChangeDate","变更日期","date","",false,minValue,10,true,'2013-12-24',true); 
 
 
 
 addCols(tblWatchGlassDet1Data,'tblWatchGlassDet1_ChangePer','变更人员','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchGlassDet1_ChangePer","变更人员","any","",true,minValue,100,false,'',true); 
 
 
 
 
 addCols(tblWatchGlassDet1Data,'tblWatchGlassDet1_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchGlassDet1_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchGlassDet1Data,'tblWatchGlassDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblWatchGlass&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=68fc380d_1408131539576500232&noback="+noback+"&parentCode=&operation=6&parentTableName=tblSampleOrder&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblWatchGlass,tblWatchGlassDet,tblWatchGlassDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchGlass&allTableName=tblWatchGlass,tblWatchGlassDet,tblWatchGlassDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchGlass&allTableName=tblWatchGlassDet,tblWatchGlassDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchGlass&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchGlass&allTableName=tblWatchGlassDet,tblWatchGlassDet1,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblWatchGlass&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblWatchGlass&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblWatchGlass&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchGlass&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblWatchGlass&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '玻璃';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblWatchGlass&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
