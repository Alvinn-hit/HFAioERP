...
str += '<IFRAME width=100% height=100% style="position:absolute;top:0px;left:0px;z-index:-1;border-style:none;filter:Alpha(opacity=0)" frameborder="0"></IFRAME><div Author="alin" class="cdrWatermark" id="cdrWatermark"></div><div id="cdrBody" style="position:absolute;left:0px;top:0px;z-index:2;width:200px;">';
str += this.getHeader();
str += this.getBody(); 
...
str += '.calendar{position:absolute;width:200px!important;width :202px;height:184px!important;height :184px;background-color:'+this.style.bodyBackColor+';border:1px solid ' + this.style.borderColor + ';left:0px;top:0px;z-index:9999;}\n';
str += '.cdrHeader{background-color:'+ this.style.headerBackColor +';width:200px;height:22px;font-size:12px;font-weight:bolder;color:'+this.style.headerFontColor+';}\n';
str += '.cdrWatermark{position:absolute;left:0px;top:55px;width:200px;font-family: Arial Black;font-size:50px;color:'+this.style.watermarkColor+';z-index:1;text-align:center;}\n';
str += '.cdrBodyBar{background-color:' + this.style.bodyBarBackColor + ';font-size:12px;font-weight:bolder;color:' + this.style.bodyBarFontColor + ';width:200px;height:20px;}\n';
str += '.cdrBody{width:200px;height:122px!important; height :110px;font-size:12px;font-weight:bolder;cursor:pointer;color:' + this.style.bodyFontColor + ';}\n';
...
str += '.menuOver{background-color:'+this.style.headerBackColor+';color:'+this.style.headerFontColor+';font-size:12px;font-weight:bolder;}\n';
str += '.headerOver{border:1px solid black;background-color:#f4f4f4;color:black;cursor:default;}\n';
str += '.cdrMenu{font-size:12px;font-weight:bolder;border:1px solid #000000;background-color:#ffffff;cursor:default;width:100%}\n';
str += 'html>body #Calendar{width:202px;174px;}';
str += '</style>\n'; 
return str;
};
...
str += '<td Author="alin" style="cursor:pointer;width:5px;"></td>\n';
str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="previousMonth" style="cursor:pointer;width:25px;FONT-SIZE: 9pt;font-family:宋体;" onclick="'+this.Obj+'.onChangeMonth(false);"><<</td>\n';
str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" id="currentYear" style="width:60px;" onclick="' + this.Obj + '.showMenu(true);" onmouseout="' + this.Obj + '.hideMenu();this.className=\'\';">0</td>\n';
str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" id="currentMonth" style="width:60px;" onclick="' + this.Obj + '.showMenu(false);" onmouseout="' + this.Obj + '.hideMenu();this.className=\'\';">0</td>\n';
str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="nextMonth" style="cursor:pointer;width:25px;FONT-SIZE: 9pt;font-family:宋体;" onclick="'+this.Obj+'.onChangeMonth(true);">>></td>\n';
str += '<td Author="alin" style="cursor:pointer;width:5px;"></td></tr>\n';
str += '</table>\n';
return str;
};
...
str += '</table>\n';
str += '<table Author="alin_bar" class="cdrBodyBar" cellSpacing="2" cellPadding="0"><tr align="center" Author="alin_bar"><td Author="alin_bar" style="cursor:move;" onclick="'+this.Obj+'.getToday();">今天：'+new Date().toFormatString("yyyy年mm月dd日")+'</td></tr></table>\n';
return str;
};
...
str += '<tr Author="alin" align="center"><td Author="alin"><table Author="alin" style="font-size:12px;font-weight:bolder;width:100%;" cellSpacing="0" cellPadding="0">\n';
str += '<tr Author="alin" align="center"><td Author="alin" onmouseover="this.className=\'menuOver\'" onmouseout="this.className=\'\'" style="FONT-SIZE: 9pt;font-family:宋体;" onclick="'+this.Obj+'.getYearMenu('+ (year - 10) + ')"><<</td>\n';
str += '<td Author="alin" onmouseover="this.className=\'menuOver\'" onmouseout="this.className=\'\'" style="FONT-SIZE: 9pt;font-family:宋体;" onclick="'+this.Obj+'.getYearMenu('+ (year + 10) +')">>></td><tr>\n';
str += '</table></td></tr>\n';
str += '</table>';
...
if(this.showMoreDay)
{
var _nextDate = new Date(_year,_month,_nextDay);
...
var dayI=obj.value.indexOf("-",5)
var month=obj.value.substring(5,dayI);
var day=obj.value.substr(dayI+1);
if(month.length==1){month="0"+month}
if(day.length==1){day="0"+day}

if(obj.innerHTML != "") this.dateInput.value = obj.value.substr(0,5)+month+"-"+day;
this.hide();
};
...
var temp=new Date(_year+"/"+(_month+1)+"/0");
var tempdate = temp.getDate();
if(_date > tempdate) _date = tempdate;
this.bindDate(_year + '-' + _month + '-' + _date);
};
...
document.onclick = function(){
 ...
 with(window.event)
 {
 if (typeof(outObject)!="undefined" && srcElement != outObject && typeof(outButton)!="undefined" && srcElement != outButton)
 closeLayer();
 }
}
