...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDEBom&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDEBom&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDEBom&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDEBom&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDEBomDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDEBomDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDEBomDet"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDEBom\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDEBom\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'totalHours',dis:'工位加总工时(秒)',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'workHours',dis:'标准工时(秒)',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'EquilibriumRate',dis:'平衡率%',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'loseRate',dis:'报废率%',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'PDEBomDet_qty',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDEBomDet_lossRate',dis:'损耗率%',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDEBom",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDEBom&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDEBom&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 var saveField=n("tblGoods_attrType");
 saveField[0].value = ""; 
 m("tbl_tblGoods_attrType").value = "" ;
 var saveField=n("tblGoods_tempGoods");
 saveField[0].value = ""; 
 m("tbl_tblGoods_tempGoods").value = "" ;
 }
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDEBom&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDEBom&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('EquilibriumRate')[0].value=f(nn('totalHours',0)/nn('workHours',0)*100,2);
...
childData[childCount]='PDEBomDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='研发BOM明细';
childCount++;
 var PDEBomDetData = new gridData();
 gridDatas[gridDataNum] = PDEBomDetData;
 gridDataNum++;
 var PDEBomDet=1;
 ...
 addCols(PDEBomDetData,'PDEBomDet_GoodsCode','料件名称','200',2,false,'',50,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("PDEBomDet_GoodsCode","料件名称","any","",false,minValue,50,false,'');
 addCols(PDEBomDetData,'PDEBomDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'PDEBomDet:GoodsCode:PDEBomDet_tblGoods_GoodsNumber:'+param4,'',"","","0",'',"2",false); 
 addCols(PDEBomDetData,'PDEBomDet_tblGoods_GoodsFullName','料件名称','150',2,true,'',1000000,2,'PDEBomDet:GoodsCode:PDEBomDet_tblGoods_GoodsFullName:'+param4,'',"","","0",'',"2",false); 
 addCols(PDEBomDetData,'PDEBomDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDEBomDet:GoodsCode:PDEBomDet_tblGoods_GoodsSpec:'+param4,'',"","","0",'',"2",false); 
 addCols(PDEBomDetData,'PDEBomDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDEBomDet:GoodsCode:PDEBomDet_tblGoods_BaseUnit:'+param4,'',"","","0",'',"2",false); 
 addCols(PDEBomDetData,'PDEBomDet_tblGoods_attrType','来源码','80',2,true,'',1000000,2,'PDEBomDet:GoodsCode:PDEBomDet_tblGoods_attrType:'+param4,'',"","","0",'',"2",false); 
 
 
 
 
 addCols(PDEBomDetData,'PDEBomDet_qty','数量','50',1,false,'',0,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("PDEBomDet_qty","数量","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDEBomDetData,'PDEBomDet_lossRate','损耗率%','50',1,true,'',0,0,'','',"Qty","","2",'',"0",false); 
 putChildValidateItem("PDEBomDet_lossRate","损耗率%","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDEBomDetData,'PDEBomDet_canSuper','允许包料','50',2,true,'2',10,-2,'','',"","","3",'',"1",false); 
 putChildValidateItem("PDEBomDet_canSuper","允许包料","any","",true,minValue,10,true,'2'); 
 
 
 
 addCols(PDEBomDetData,'PDEBomDet_remark','备注','200',18,true,'',500,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("PDEBomDet_remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 addCols(PDEBomDetData,'PDEBomDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDEBom&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDEBom,PDEBomDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDEBom&allTableName=PDEBom,PDEBomDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDEBom&allTableName=PDEBomDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDEBom&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDEBom&allTableName=PDEBomDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDEBom&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDEBom&designId=16abe428_1603022145576950553&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDEBom&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDEBom&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDEBom&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '研发BOM';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDEBom&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
