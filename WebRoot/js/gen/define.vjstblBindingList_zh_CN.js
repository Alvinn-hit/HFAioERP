...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBindingList&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBindingList&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBindingList&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBindingList&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBindingListDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBindingListDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBindingListDet"){
...
function execStat2(){
 execStatOne('tblBindingListDet_NetWeight',2);
 execStatOne('tblBindingListDet_Weight',2);
 execStatOne('tblBindingListDet_SGFare',2);
 execStatOne('tblBindingListDet_LoadFare',2);
 execStatOne('tblBindingListDet_OtherFare',2);
 execStatOne('tblBindingListDet_ACFare',2);
 execStatOne('tblBindingListDet_ACProxyFare',2);
 execStatOne('tblBindingListDet_CleanseFare',2);
 execStatOne('tblBindingListDet_BoxingFare',2);
 execStatOne('tblBindingListDet_LCheckFare',2);
 execStatOne('tblBindingListDet_OCFare',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=6266a383_1207271204072180124&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=6266a383_1207271204072180124&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblBindingList\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBindingList\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblBindingListDet_BoxQty',"tblBindingListDet"); 
 delFireEventOne(rowId,tableName,'tblBindingListDet_NetWeight',"tblBindingListDet"); 
 delFireEventOne(rowId,tableName,'tblBindingListDet_Weight',"tblBindingListDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'Board',dis:'板数',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblBindingListDet_NetWeight',dis:'净重',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBindingListDet_Weight',dis:'毛重',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBindingListDet_SGFare',dis:'深港运费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBindingListDet_LoadFare',dis:'装车费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBindingListDet_OtherFare',dis:'物流其他费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBindingListDet_ACFare',dis:'报关费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBindingListDet_ACProxyFare',dis:'报检代理费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBindingListDet_CleanseFare',dis:'消毒费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBindingListDet_BoxingFare',dis:'木质包装费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBindingListDet_LCheckFare',dis:'法检费',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBindingListDet_OCFare',dis:'关务其他费',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBindingList",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBindingList&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBindingList&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"6266a383_1207271204072180124",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 var saveField=n("TruckNoFrom"); 
 saveField[0].value = "";
 var obj = m("tbl_tblTruckNoList_TruckNoFrom");
 if(obj != null){
 obj.value = "";
 }
 ...
 var saveField=n("tblTruckNo_DriverName");
 saveField[0].value = ""; 
 m("tbl_tblTruckNo_DriverName").value = "" ;
 var saveField=n("tblTruckNo_DriverTel");
 saveField[0].value = ""; 
 m("tbl_tblTruckNo_DriverTel").value = "" ;
 }
 if(fieldName == "ACCompany"){
 var saveField=n("ACCompany"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyQ_ComName");
 if(obj != null){
 obj.value = "";
 }
 }
 if(fieldName == "SDCode"){
 var saveField=n("SDCode"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsSHAddress_SDCode");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SDFullName"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsSHAddress_SDFullName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SDContact"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsSHAddress_SDContact");
 if(obj != null){
 obj.value = "";
 }
 }
 if(fieldName == "SupplyName"){
 var saveField=n("SupplyNo"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsSupplyGW_SupplyNo");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SupplyName"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsSupplyGW_SupplyName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblLsSupplyGW_LinkNumber");
 saveField[0].value = ""; 
 m("tbl_tblLsSupplyGW_LinkNumber").value = "" ;
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=6266a383_1207271204072180124&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=6266a383_1207271204072180124&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=6266a383_1207271204072180124&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblBindingList"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBindingList&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBindingList&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('TotalPiece')[0].value=f(sm('tblBindingListDet_BoxQty'),0);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalWeight')[0].value=f(sm('tblBindingListDet_Weight'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalNetWeight')[0].value=f(sm('tblBindingListDet_NetWeight'),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBindingListDet_BoxQty'); 
 n('TotalPiece')[0].value=f(sm('tblBindingListDet_BoxQty'),0);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBindingListDet_NetWeight'); 
 n('TotalNetWeight')[0].value=f(sm('tblBindingListDet_NetWeight'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBindingListDet_Weight'); 
 n('TotalWeight')[0].value=f(sm('tblBindingListDet_Weight'),2);
...
childData[childCount]='tblBindingListDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='装车通知单明细';
childCount++;
 var tblBindingListDetData = new gridData();
 gridDatas[gridDataNum] = tblBindingListDetData;
 gridDataNum++;
 var tblBindingListDet=2;
 ...
 addCols(tblBindingListDetData,'tblBindingListDet_SheetID','合同ID','0',2,false,'',30,-2,'','',"","","0",'',"3",false); 
 putChildValidateItem("tblBindingListDet_SheetID","合同ID","any","RowMarker",false,minValue,30,false,''); 
 ...
 var param4="";
 param4=param4+":1载货清单号@TruckNoListID"; 
 
 addCols(tblBindingListDetData,'tblBindingListDet_CustomsNum','报关单号','120',2,true,'',100,2,'tblBindingListDet:CustomsNum:tblBindingListDet_CustomsNum:'+param4,"","","","2",'',"2",true);
 putChildValidateItem("tblBindingListDet_CustomsNum","报关单号","any","",true,minValue,100,false,'',true);
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_SheetNo','合同编号','100',2,true,'',100,8,'','',"","","4",'',"8",false); 
 putChildValidateItem("tblBindingListDet_SheetNo","合同编号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_ShipTo','客户名称','150',2,true,'',100,8,'','',"","","6",'',"8",false); 
 putChildValidateItem("tblBindingListDet_ShipTo","客户名称","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_GoodsName','品名','120',2,true,'',100,8,'','',"","","8",'',"8",false); 
 putChildValidateItem("tblBindingListDet_GoodsName","品名","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_BoxQty','箱数(箱)','50',0,true,'',0,8,'','',"","","10",'',"8",false); 
 putChildValidateItem("tblBindingListDet_BoxQty","箱数(箱)","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_NetWeight','净重','45',1,true,'0',0,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblBindingListDet_NetWeight","净重","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_Weight','毛重','45',1,true,'0',0,0,'','',"","","14",'',"0",false); 
 putChildValidateItem("tblBindingListDet_Weight","毛重","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_UnitWeight','计重单位','50',2,true,'',10,1,';KG:KG;G:G;T:T','',"","","16",'',"1",false);
 putChildValidateItem("tblBindingListDet_UnitWeight","计重单位","any","",true,minValue,10,false,'',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_SGFare','深港运费','75',1,true,'0',0,0,'','',"Amount","","18",'',"0",false); 
 putChildValidateItem("tblBindingListDet_SGFare","深港运费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_LoadFare','装车费','75',1,true,'0',0,0,'','',"Amount","","20",'',"0",false); 
 putChildValidateItem("tblBindingListDet_LoadFare","装车费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_OtherFare','物流其他费','75',1,true,'0',0,0,'','',"Amount","","22",'',"0",false); 
 putChildValidateItem("tblBindingListDet_OtherFare","物流其他费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_ACFare','报关费','75',1,true,'0',0,0,'','',"Amount","","24",'',"0",false); 
 putChildValidateItem("tblBindingListDet_ACFare","报关费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_ActACFare','报关费实付','224',1,true,'',0,-2,'','',"","","25",'',"3",false); 
 putChildValidateItem("tblBindingListDet_ActACFare","报关费实付","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_ACProxyFare','报检代理费','75',1,true,'0',0,0,'','',"Amount","","26",'',"0",false); 
 putChildValidateItem("tblBindingListDet_ACProxyFare","报检代理费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_ActACProxyFare','报检代理费实付','224',1,true,'',0,-2,'','',"","","27",'',"3",false); 
 putChildValidateItem("tblBindingListDet_ActACProxyFare","报检代理费实付","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_CleanseFare','消毒费','75',1,true,'0',0,0,'','',"Amount","","28",'',"0",false); 
 putChildValidateItem("tblBindingListDet_CleanseFare","消毒费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_BoxingFare','木质包装费','75',1,true,'0',0,0,'','',"Amount","","30",'',"0",false); 
 putChildValidateItem("tblBindingListDet_BoxingFare","木质包装费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_LCheckFare','法检费','75',1,true,'0',0,0,'','',"Amount","","32",'',"0",false); 
 putChildValidateItem("tblBindingListDet_LCheckFare","法检费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_OCFare','关务其他费','75',1,true,'0',0,0,'','',"Amount","","34",'',"0",false); 
 putChildValidateItem("tblBindingListDet_OCFare","关务其他费","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_Remark','备注','130',2,true,'',8000,0,'','',"","","40",'',"0",false); 
 putChildValidateItem("tblBindingListDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_BGSupplyNo','报关公司编号','140',2,true,'',40,-2,'','',"","","42",'',"3",false); 
 putChildValidateItem("tblBindingListDet_BGSupplyNo","报关公司编号","any","",true,minValue,40,false,''); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_BJSupplyNo','报检公司编号','140',2,true,'',40,-2,'','',"","","42",'',"3",false); 
 putChildValidateItem("tblBindingListDet_BJSupplyNo","报检公司编号","any","",true,minValue,40,false,''); 
 
 
 
 addCols(tblBindingListDetData,'tblBindingListDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBindingList&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBindingList,tblBindingListDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBindingList&allTableName=tblBindingList,tblBindingListDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBindingList&allTableName=tblBindingListDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBindingList&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBindingList&allTableName=tblBindingListDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=6266a383_1207271204072180124&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=6266a383_1207271204072180124&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblBindingListDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=6266a383_1207271204072180124&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=6266a383_1207271204072180124&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblBindingList&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblBindingList&designId=21786bdc_1409171711237919070&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBindingList&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBindingList&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBindingList&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '装车通知单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBindingList&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
