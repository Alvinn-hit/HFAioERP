...
{ 
 cFireEvent('tblBeginStockDet_Qty',''); 
 cFireEvent('tblBeginStockDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBeginStock&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBeginStock&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBeginStock&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBeginStock&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBeginStockDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblBeginStockDet_BatchNo');
 ...
 var z=n('tblBeginStockDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBeginStockDet"){
 
 copyFields=n('tblBeginStockDet_GoodsCode');
 ...
 var tblBeginStockDet_tblGoods_GoodsNumber = n('tblBeginStockDet_tblGoods_GoodsNumber');
 if(typeof(tblBeginStockDet_tblGoods_GoodsNumber) != 'undefined' && tblBeginStockDet_tblGoods_GoodsNumber.length >index){
 tblBeginStockDet_tblGoods_GoodsNumber[index].value = tblBeginStockDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblBeginStockDet_tblGoods_GoodsFullName = n('tblBeginStockDet_tblGoods_GoodsFullName');
 if(typeof(tblBeginStockDet_tblGoods_GoodsFullName) != 'undefined' && tblBeginStockDet_tblGoods_GoodsFullName.length >index){
 tblBeginStockDet_tblGoods_GoodsFullName[index].value = tblBeginStockDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblBeginStockDet_tblGoods_GoodsSpec = n('tblBeginStockDet_tblGoods_GoodsSpec');
 if(typeof(tblBeginStockDet_tblGoods_GoodsSpec) != 'undefined' && tblBeginStockDet_tblGoods_GoodsSpec.length >index){
 tblBeginStockDet_tblGoods_GoodsSpec[index].value = tblBeginStockDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblBeginStockDet_tblGoods_BaseUnit = n('tblBeginStockDet_tblGoods_BaseUnit');
 if(typeof(tblBeginStockDet_tblGoods_BaseUnit) != 'undefined' && tblBeginStockDet_tblGoods_BaseUnit.length >index){
 tblBeginStockDet_tblGoods_BaseUnit[index].value = tblBeginStockDet_tblGoods_BaseUnit[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBeginStockDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblBeginStockDet"; 
 var goodsCodeLogicValidate_tblBeginStockDet='@GN:Seq:Qty:tblBeginStockDet_GoodsSeqSet:tblBeginStockDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'tblBeginStockDet',value:goodsCodeLogicValidate_tblBeginStockDet};
...
function execStat2(){
 execStatOne('tblBeginStockDet_Qty',4);
 execStatOne('tblBeginStockDet_Amount',2);
 execStatOne('tblBeginStockDet_TaxAmount',2);
 execStatOne('tblBeginStockDet_CurAmount',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblBeginStock\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBeginStock\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblBeginStockDet_Qty',"tblBeginStockDet"); 
 delFireEventOne(rowId,tableName,'tblBeginStockDet_Price',"tblBeginStockDet"); 
 delFireEventOne(rowId,tableName,'tblBeginStockDet_Amount',"tblBeginStockDet"); 
 delFireEventOne(rowId,tableName,'tblBeginStockDet_TaxPrice',"tblBeginStockDet"); 
 delFireEventOne(rowId,tableName,'tblBeginStockDet_TaxAmount',"tblBeginStockDet"); 
 delFireEventOne(rowId,tableName,'tblBeginStockDet_CurPrice',"tblBeginStockDet"); 
 delFireEventOne(rowId,tableName,'tblBeginStockDet_CurAmount',"tblBeginStockDet"); 
 }
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'tblBeginStockDet_Qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBeginStockDet_Price',dis:'单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBeginStockDet_Amount',dis:'金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBeginStockDet_TaxPrice',dis:'含税单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBeginStockDet_TaxAmount',dis:'含税金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBeginStockDet_CoTaxAmt',dis:'税额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBeginStockDet_CurPrice',dis:'外币单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBeginStockDet_CurAmount',dis:'外币金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBeginStock",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBeginStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBeginStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('tblBeginStockDet_Qty'); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblBeginStock"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBeginStock&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBeginStock&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
n('TotalAmount')[0].value=f(sm('tblBeginStockDet_DisBackAmt'),2);

n('TotalTaxAmount')[0].value=f(sm('tblBeginStockDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblBeginStockDet_CoTaxAmt'),2);
...
n('DisAmount')[0].value=f(sm('tblBeginStockDet_DisAmount'),8);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBeginStockDet_Qty'); 
...
n('TotalCoTaxAmt')[0].value=f(sm('tblBeginStockDet_CoTaxAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBeginStockDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
n('DisAmount')[0].value=f(sm('tblBeginStockDet_DisAmount'),8);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBeginStockDet_Amount'); 
...
n('DisAmount')[0].value=f(sm('tblBeginStockDet_DisAmount'),8);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBeginStockDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('tblBeginStockDet_DisAmount'),8);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBeginStockDet_DisPrice'); 
...
n('DisAmount')[0].value=f(sm('tblBeginStockDet_DisAmount'),8);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBeginStockDet_DisBackAmt'); 
...
n('DisAmount')[0].value=f(sm('tblBeginStockDet_DisAmount'),8);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBeginStockDet_TaxPrice'); 
...
n('DisAmount')[0].value=f(sm('tblBeginStockDet_DisAmount'),8);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBeginStockDet_TaxAmount'); 
...
n('DisAmount')[0].value=f(sm('tblBeginStockDet_DisAmount'),8);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBeginStockDet_CurPrice'); 
...
n('TotalAmount')[0].value=f(sm('tblBeginStockDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblBeginStockDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(sm('tblBeginStockDet_CurAmount'),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
n('TotalCoTaxAmt')[0].value=f(sm('tblBeginStockDet_CoTaxAmt'),2);
n('DisAmount')[0].value=f(sm('tblBeginStockDet_DisAmount'),8);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBeginStockDet_CurAmount'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),2);
...
childData[childCount]='tblBeginStockDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='期初录入单明细';
childCount++;
 var tblBeginStockDetData = new gridData();
 gridDatas[gridDataNum] = tblBeginStockDetData;
 gridDataNum++;
 var tblBeginStockDet=1;
 ...
 addCols(tblBeginStockDetData,'tblBeginStockDet_GoodsCode','商品名称','120',2,false,'',50,-2,'','',"","copy","1",'',"2",true);
 putChildValidateItem("tblBeginStockDet_GoodsCode","商品名称","any","",false,minValue,50,false,'');
 addCols(tblBeginStockDetData,'tblBeginStockDet_tblGoods_GoodsNumber','料件编号','200',20,true,'',1000000,2,'tblBeginStockDet:GoodsCode:tblBeginStockDet_tblGoods_GoodsNumber:'+param4,'',"","copy","1",'',"2",false); 
 addCols(tblBeginStockDetData,'tblBeginStockDet_tblGoods_GoodsFullName','商品名称','300',20,true,'',1000000,2,'tblBeginStockDet:GoodsCode:tblBeginStockDet_tblGoods_GoodsFullName:'+param4,'',"","copy","1",'',"2",false); 
 addCols(tblBeginStockDetData,'tblBeginStockDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'tblBeginStockDet:GoodsCode:tblBeginStockDet_tblGoods_GoodsSpec:'+param4,'',"","copy","1",'',"2",false); 
 addCols(tblBeginStockDetData,'tblBeginStockDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblBeginStockDet:GoodsCode:tblBeginStockDet_tblGoods_BaseUnit:'+param4,'',"","copy","1",'',"2",false); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","5",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","8",'',"2",false); 
 putChildValidateItem("tblBeginStockDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","9",'',"2",false); 
 putChildValidateItem("tblBeginStockDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_Qty','数量','80',1,false,'',0,0,'','',"Qty","","13",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_Qty","数量","double","SeqQty",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_Price','单价','80',1,true,'',0,0,'','',"priceIdentifier","","14",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_Amount','金额','100',1,false,'',0,0,'','',"AmountIdentifier","","15",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_Amount","金额","double","",false,minValue,maxValue,false,'',true); 
 ...
 addCols(tblBeginStockDetData,'tblBeginStockDet_Seq','序列号','130',2,true,'',5000,-2,'','',"","","20",'',"2",false); 
 putChildValidateItem("tblBeginStockDet_Seq","序列号","any","GoodsField",true,minValue,5000,false,''); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_Discount','折扣率%','70',1,true,'100',0,-2,'','',"Price","","21",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_Discount","折扣率%","double","BillDiscount",true,minValue,maxValue,true,'100'); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_DisPrice','折后单价','80',1,true,'',0,-2,'','',"priceIdentifier","","23",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_DisPrice","折后单价","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_DisBackAmt','折后金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","24",'',"8",false); 
 putChildValidateItem("tblBeginStockDet_DisBackAmt","折后金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_DisAmount','折让金额','70',1,true,'',0,-2,'','',"AmountIdentifier","","25",'',"8",false); 
 putChildValidateItem("tblBeginStockDet_DisAmount","折让金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_TaxPrice','含税单价','80',1,true,'',0,0,'','',"priceIdentifier","","29",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_TaxPrice","含税单价","double","BillTax",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_TaxAmount','含税金额','100',1,true,'',0,0,'','',"AmountIdentifier","","31",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_TaxAmount","含税金额","double","BillTax",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_CoTaxAmt','税额','100',1,true,'',0,8,'','',"Amount","","32",'',"8",false); 
 putChildValidateItem("tblBeginStockDet_CoTaxAmt","税额","double","BillTax",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_CurPrice','外币单价','80',1,true,'',0,0,'','',"priceIdentifier","","33",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_CurPrice","外币单价","double","currency",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_CurAmount','外币金额','80',1,true,'',0,0,'','',"AmountIdentifier","","35",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_CurAmount","外币金额","double","currency",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_Remark','备注','100',2,true,'',8000,0,'','',"","","57",'',"0",false); 
 putChildValidateItem("tblBeginStockDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_SourceID','源ID','0',2,true,'',60,-2,'','',"","","101",'',"3",false); 
 putChildValidateItem("tblBeginStockDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 
 
 addCols(tblBeginStockDetData,'tblBeginStockDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBeginStock&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBeginStock,tblBeginStockDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBeginStock&allTableName=tblBeginStock,tblBeginStockDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBeginStock&allTableName=tblBeginStockDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBeginStock&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBeginStock&allTableName=tblBeginStockDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblBeginStock&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblBeginStock&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBeginStock&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBeginStock&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBeginStock&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '商品期初';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBeginStock&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
