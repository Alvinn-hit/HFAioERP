...
{ 
 cFireEvent('tblOtherOutDet_Qty',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblOtherOut&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblOtherOut&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblOtherOut&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblOtherOut&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblOtherOutDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 ...
 var z=n('tblOtherOutDet_GoodsCode');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblOtherOutDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblOtherOutDet"){
 
 copyFields=n('tblOtherOutDet_StockCode');
 ...
 var tblOtherOutDet_tblStock_StockFullName1 = n('tblOtherOutDet_tblStock_StockFullName1');
 if(typeof(tblOtherOutDet_tblStock_StockFullName1) != 'undefined' && tblOtherOutDet_tblStock_StockFullName1.length >index){
 tblOtherOutDet_tblStock_StockFullName1[index].value = tblOtherOutDet_tblStock_StockFullName1[index-1].value;
 }
 }
 copyFields=n('tblOtherOutDet_GoodsCode');
 if((order > 3 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblOtherOutDet_tblGoods_GoodsNumber = n('tblOtherOutDet_tblGoods_GoodsNumber');
 if(typeof(tblOtherOutDet_tblGoods_GoodsNumber) != 'undefined' && tblOtherOutDet_tblGoods_GoodsNumber.length >index){
 tblOtherOutDet_tblGoods_GoodsNumber[index].value = tblOtherOutDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblOtherOutDet_tblGoods_GoodsFullName = n('tblOtherOutDet_tblGoods_GoodsFullName');
 if(typeof(tblOtherOutDet_tblGoods_GoodsFullName) != 'undefined' && tblOtherOutDet_tblGoods_GoodsFullName.length >index){
 tblOtherOutDet_tblGoods_GoodsFullName[index].value = tblOtherOutDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblOtherOutDet_tblGoods_GoodsSpec = n('tblOtherOutDet_tblGoods_GoodsSpec');
 if(typeof(tblOtherOutDet_tblGoods_GoodsSpec) != 'undefined' && tblOtherOutDet_tblGoods_GoodsSpec.length >index){
 tblOtherOutDet_tblGoods_GoodsSpec[index].value = tblOtherOutDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblOtherOutDet_tblGoods_BaseUnit = n('tblOtherOutDet_tblGoods_BaseUnit');
 if(typeof(tblOtherOutDet_tblGoods_BaseUnit) != 'undefined' && tblOtherOutDet_tblGoods_BaseUnit.length >index){
 tblOtherOutDet_tblGoods_BaseUnit[index].value = tblOtherOutDet_tblGoods_BaseUnit[index-1].value;
 }
 var tblOtherOutDet_tblStocks_lastQty = n('tblOtherOutDet_tblStocks_lastQty');
 if(typeof(tblOtherOutDet_tblStocks_lastQty) != 'undefined' && tblOtherOutDet_tblStocks_lastQty.length >index){
 tblOtherOutDet_tblStocks_lastQty[index].value = tblOtherOutDet_tblStocks_lastQty[index-1].value;
 }
 var tblOtherOutDet_ViewVirtualStock_UseQty = n('tblOtherOutDet_ViewVirtualStock_UseQty');
 if(typeof(tblOtherOutDet_ViewVirtualStock_UseQty) != 'undefined' && tblOtherOutDet_ViewVirtualStock_UseQty.length >index){
 tblOtherOutDet_ViewVirtualStock_UseQty[index].value = tblOtherOutDet_ViewVirtualStock_UseQty[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblOtherOutDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblOtherOutDet"; 
 var goodsCodeLogicValidate_tblOtherOutDet='@GN:Seq:Qty:tblOtherOutDet_GoodsSeqSet:tblOtherOutDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'tblOtherOutDet',value:goodsCodeLogicValidate_tblOtherOutDet};
...
function execStat2(){
 execStatOne('tblOtherOutDet_Qty',2);
 execStatOne('tblOtherOutDet_Amount',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblOtherOut\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblOtherOut\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblOtherOutDet_Qty',"tblOtherOutDet"); 
 delFireEventOne(rowId,tableName,'tblOtherOutDet_Price',"tblOtherOutDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'BillAmt',dis:'合计金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CurrencyRate',dis:'汇率',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CurTotalAmount',dis:'外币总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblOtherOutDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOtherOutDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOtherOutDet_Amount',dis:'金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOtherOutDet_CurPrice',dis:'外币单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOtherOutDet_CurAmount',dis:'外币金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblOtherOut",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblOtherOut&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblOtherOut&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('tblOtherOutDet_Qty'); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblOtherOut"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblOtherOut&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblOtherOut&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
n('TotalTaxAmount')[0].value=f(sm('tblOtherOutDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblOtherOutDet_CoTaxAmt'),2);
...
n('DisAmount')[0].value=f(sm('tblOtherOutDet_DisAmount'),2);

n('BillAmt')[0].value=f(sm('tblOtherOutDet_DisBackAmt'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 if(n('InVoiceType')[0].value==2)n('Tax')[0].value=f("0",4);

if(n('InVoiceType')[0].value==1)n('Tax')[0].value=f("0",4);

if(n('InVoiceType')[0].value==3)n('Tax')[0].value=f(0,4);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherOutDet_Qty'); 
...
n('BillAmt')[0].value=f(sm('tblOtherOutDet_DisBackAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherOutDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
n('BillAmt')[0].value=f(sm('tblOtherOutDet_DisBackAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherOutDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,4);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('BillAmt')[0].value=f(sm('tblOtherOutDet_DisBackAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherOutDet_DisPrice'); 
 r(0,"Discount")[p].value=f(rn(0,"DisPrice",p)/rn(0,"Price",p)*100,4);
 
r(0,"DisBackAmt")[p].value=f(rn(0,"DisPrice",p)*rn(0,"Qty",p),2);
 
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
 
n('DisAmount')[0].value=f(sm('tblOtherOutDet_DisAmount'),2);
 n('BillAmt')[0].value=f(sm('tblOtherOutDet_DisBackAmt')-nn('DiscountAmount',0),2);
n('BillAmt')[0].value=f(sm('tblOtherOutDet_DisBackAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherOutDet_DisBackAmt'); 
 r(0,"Discount")[p].value=f(rn(0,"DisBackAmt",p)/rn(0,"Amount",p)*100,4);
...
childData[childCount]='tblOtherOutDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='其他出库单明细';
childCount++;
 var tblOtherOutDetData = new gridData();
 gridDatas[gridDataNum] = tblOtherOutDetData;
 gridDataNum++;
 var tblOtherOutDet=1;
 
 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_Seq','序列号','130',2,true,'',5000,-2,'','',"","triggerCopy","0",'',"2",false); 
 putChildValidateItem("tblOtherOutDet_Seq","序列号","any","GoodsField",true,minValue,5000,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_StockCode','仓库','80',2,true,'',100,-2,'','',"","copy","0",'',"3",false); 
 putChildValidateItem("tblOtherOutDet_StockCode","仓库","any","",true,minValue,100,true,''); 
 ...
 param4=param4+":0仓库@tblOtherOutDet_StockCode";
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_GoodsCode','商品名称','120',2,false,'',100,-2,'','',"","triggerOrCopy","3",'',"2",true);
 putChildValidateItem("tblOtherOutDet_GoodsCode","商品名称","any","",false,minValue,100,false,'');
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'tblOtherOutDet:GoodsCode:tblOtherOutDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_tblGoods_GoodsFullName','商品名称','150',18,true,'',1000000,2,'tblOtherOutDet:GoodsCode:tblOtherOutDet_tblGoods_GoodsFullName:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'tblOtherOutDet:GoodsCode:tblOtherOutDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblOtherOutDet:GoodsCode:tblOtherOutDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_tblStocks_lastQty','实际库存','90',1,true,'',1000000,2,'tblOtherOutDet:GoodsCode:tblOtherOutDet_tblStocks_lastQty:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_ViewVirtualStock_UseQty','虚拟库存','90',1,true,'',1000000,2,'tblOtherOutDet:GoodsCode:tblOtherOutDet_ViewVirtualStock_UseQty:'+param4,'',"","triggerOrCopy","3",'',"2",false); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","9",'',"2",false); 
 putChildValidateItem("tblOtherOutDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","12",'',"2",false); 
 putChildValidateItem("tblOtherOutDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","15",'',"2",false); 
 putChildValidateItem("tblOtherOutDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_color','色彩','60',2,true,'',100,-2,'','',"","","16",'',"2",false); 
 putChildValidateItem("tblOtherOutDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","19",'',"2",false); 
 putChildValidateItem("tblOtherOutDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","24",'',"2",false); 
 putChildValidateItem("tblOtherOutDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","27",'',"2",false); 
 putChildValidateItem("tblOtherOutDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_Qty','数量','80',1,false,'',0,0,'','',"Qty","","33",'',"0",false); 
 putChildValidateItem("tblOtherOutDet_Qty","数量","double","SeqQty",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_Price','单价','80',1,true,'',0,8,'','',"priceIdentifier","","36",'',"0",false); 
 putChildValidateItem("tblOtherOutDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_Amount','金额','100',1,true,'',0,8,'','',"AmountIdentifier","","39",'',"8",false); 
 putChildValidateItem("tblOtherOutDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(tblOtherOutDetData,'tblOtherOutDet_Discount','折扣率%','50',1,true,'100',0,-2,'','',"Price","","57",'',"0",false); 
 putChildValidateItem("tblOtherOutDet_Discount","折扣率%","double","BillDiscount",true,minValue,maxValue,true,'100'); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_DisPrice','折后单价','80',1,true,'',0,-2,'','',"priceIdentifier","","60",'',"8",false); 
 putChildValidateItem("tblOtherOutDet_DisPrice","折后单价","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_DisBackAmt','折后金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","65",'',"8",false); 
 putChildValidateItem("tblOtherOutDet_DisBackAmt","折后金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_DisAmount','折让金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","70",'',"8",false); 
 putChildValidateItem("tblOtherOutDet_DisAmount","折让金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_TaxPrice','含税单价','100',1,true,'',0,-2,'','',"priceIdentifier","","80",'',"3",false); 
 putChildValidateItem("tblOtherOutDet_TaxPrice","含税单价","double","BillTax",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_TaxAmount','含税金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","82",'',"3",false); 
 putChildValidateItem("tblOtherOutDet_TaxAmount","含税金额","double","BillTax",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_CoTaxAmt','税额','100',1,true,'',0,-2,'','',"AmountIdentifier","","83",'',"3",false); 
 putChildValidateItem("tblOtherOutDet_CoTaxAmt","税额","double","BillTax",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_CurPrice','外币单价','100',1,true,'',0,8,'','',"priceIdentifier","","90",'',"8",false); 
 putChildValidateItem("tblOtherOutDet_CurPrice","外币单价","double","currency",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","90",'',"100",false); 
 putChildValidateItem("tblOtherOutDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_CurAmount','外币金额','100',1,true,'',0,8,'','',"AmountIdentifier","","92",'',"8",false); 
 putChildValidateItem("tblOtherOutDet_CurAmount","外币金额","double","currency",true,minValue,maxValue,false,'',true); 
 
 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblOtherOutDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblOtherOutDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_Remark','备注','120',2,true,'',8000,0,'','',"","","120",'',"0",false); 
 putChildValidateItem("tblOtherOutDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 addCols(tblOtherOutDetData,'tblOtherOutDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblOtherOut&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblOtherOut,tblOtherOutDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblOtherOut&allTableName=tblOtherOut,tblOtherOutDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblOtherOut&allTableName=tblOtherOutDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblOtherOut&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblOtherOut&allTableName=tblOtherOutDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblOtherOut&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblOtherOut&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblOtherOut&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblOtherOut&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblOtherOut&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '其他出库';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblOtherOut&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
