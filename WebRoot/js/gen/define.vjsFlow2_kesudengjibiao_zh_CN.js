...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=Flow2_kesudengjibiao&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=Flow2_kesudengjibiao&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=Flow2_kesudengjibiao&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=Flow2_kesudengjibiao&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "kesudengjibiaoDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "OAJiuzhengyufangcuoshiTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "OAlipeidengjiTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="kesudengjibiaoDet"){
 
  }
 if(typeof(tableName)!="undefined" && tableName=="OAJiuzhengyufangcuoshi"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="OAlipeidengji"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="kesudengjibiaoDet"){
 
  }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="OAJiuzhengyufangcuoshi"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="OAlipeidengji"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="kesudengjibiaoDet"; 
 tblName[1]="OAJiuzhengyufangcuoshi"; 
 tblName[2]="OAlipeidengji"; 
...
function execStat2(){
 execStatOne('OAlipeidengji_Compensation',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=cfe3917e_1409042026283637799&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=cfe3917e_1409042026283637799&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"Flow2_kesudengjibiao\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"Flow2_kesudengjibiao\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'kesudengjibiaoDet_BadProductValue',dis:'不良品货值($)',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'kesudengjibiaoDet_SimilarProductAmt',dis:'同类产品出货金额($)',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'OAlipeidengji_Compensation',dis:'金额($)',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=Flow2_kesudengjibiao",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=Flow2_kesudengjibiao&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=Flow2_kesudengjibiao&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"cfe3917e_1409042026283637799",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=cfe3917e_1409042026283637799&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=cfe3917e_1409042026283637799&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=cfe3917e_1409042026283637799&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "Flow2_kesudengjibiao"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=Flow2_kesudengjibiao&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=Flow2_kesudengjibiao&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='kesudengjibiaoDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='客诉明细表';
childCount++;
 var kesudengjibiaoDetData = new gridData();
 gridDatas[gridDataNum] = kesudengjibiaoDetData;
 gridDataNum++;
 var kesudengjibiaoDet=1;
 
 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_ProductNo','产品编号','100',2,true,'',200,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_ProductNo","产品编号","any","",true,minValue,200,false,'',true); 
 
 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_OEMorCustNo','客户产品#或OEM#','150',2,true,'',200,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_OEMorCustNo","客户产品#或OEM#","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_ProductType','产品种类','200',2,true,'',200,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_ProductType","产品种类","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_PoorNum','不良数量','60',0,true,'',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_PoorNum","不良数量","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_ShipmentQTY','出货数量','60',0,true,'',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_ShipmentQTY","出货数量","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_BadProductValue','不良品货值($)','80',1,true,'',0,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_BadProductValue","不良品货值($)","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_SimilarProductAmt','同类产品出货金额($)','150',1,true,'',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_SimilarProductAmt","同类产品出货金额($)","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_QualityProblems','质量问题简述','250',2,true,'',200,0,'','',"","","7",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_QualityProblems","质量问题简述","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_VarietiesNum','同类产品本批次出货品种数','150',0,true,'',0,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_VarietiesNum","同类产品本批次出货品种数","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_PurchaseNum','本批次采购数量','120',0,true,'',0,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_PurchaseNum","本批次采购数量","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_Supplier','供应商简称(中文)','100',2,true,'',200,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_Supplier","供应商简称(中文)","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_OrderNum','采购订单号','80',2,true,'',200,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_OrderNum","采购订单号","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_DeliveryDate','发货日期','224',5,true,'',200,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_DeliveryDate","发货日期","date","",true,minValue,200,false,'',true); 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_Remark','备注','150',2,true,'',200,0,'','',"","","13",'',"0",false); 
 putChildValidateItem("kesudengjibiaoDet_Remark","备注","any","",true,minValue,200,false,'',true); 
 
 
 
 
 
 addCols(kesudengjibiaoDetData,'kesudengjibiaoDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='OAJiuzhengyufangcuoshi';
tabIsNulls[childCount]='0';
tabDiss[childCount]='纠正预防措施';
childCount++;
 var OAJiuzhengyufangcuoshiData = new gridData();
 gridDatas[gridDataNum] = OAJiuzhengyufangcuoshiData;
 gridDataNum++;
 var OAJiuzhengyufangcuoshi=1;
 ...
 addCols(OAJiuzhengyufangcuoshiData,'OAJiuzhengyufangcuoshi_FaultLiability','过失责任','100',2,true,'',200,1,';1:工厂;2:客户;3:业务员;4:采购;5:产品专员;6:产品工程师;7:品质检验;8:东莞仓库;9:其它情况','',"","","1",'',"1",false);
 putChildValidateItem("OAJiuzhengyufangcuoshi_FaultLiability","过失责任","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(OAJiuzhengyufangcuoshiData,'OAJiuzhengyufangcuoshi_AdopYN','是否采取措施','100',2,true,'',200,1,';1:是;2:否','',"","","2",'',"1",false);
 putChildValidateItem("OAJiuzhengyufangcuoshi_AdopYN","是否采取措施","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(OAJiuzhengyufangcuoshiData,'OAJiuzhengyufangcuoshi_AdopFileNo','措施处理单编号','200',2,true,'',500,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("OAJiuzhengyufangcuoshi_AdopFileNo","措施处理单编号","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(OAJiuzhengyufangcuoshiData,'OAJiuzhengyufangcuoshi_Others','其它','150',2,true,'',1000,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("OAJiuzhengyufangcuoshi_Others","其它","any","",true,minValue,1000,false,'',true); 
 
 
 
 
 
 addCols(OAJiuzhengyufangcuoshiData,'OAJiuzhengyufangcuoshi_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='OAlipeidengji';
tabIsNulls[childCount]='0';
tabDiss[childCount]='理赔登记';
childCount++;
 var OAlipeidengjiData = new gridData();
 gridDatas[gridDataNum] = OAlipeidengjiData;
 gridDataNum++;
 var OAlipeidengji=1;
 ...
 addCols(OAlipeidengjiData,'OAlipeidengji_CreditTpye','类型','100',2,true,'',200,1,';1:赔款;2:补货','',"","","1",'',"1",false);
 putChildValidateItem("OAlipeidengji_CreditTpye","类型","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(OAlipeidengjiData,'OAlipeidengji_CreditBillNo','赔款/补货单号','150',2,true,'',200,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("OAlipeidengji_CreditBillNo","赔款/补货单号","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(OAlipeidengjiData,'OAlipeidengji_Compensation','金额($)','80',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("OAlipeidengji_Compensation","金额($)","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(OAlipeidengjiData,'OAlipeidengji_Remarks','备注','100',2,true,'',2000,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("OAlipeidengji_Remarks","备注","any","",true,minValue,2000,false,'',true); 
 
 
 
 
 
 addCols(OAlipeidengjiData,'OAlipeidengji_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=Flow2_kesudengjibiao&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "Flow2_kesudengjibiao,OAJiuzhengyufangcuoshi,OAlipeidengji,kesudengjibiaoDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=Flow2_kesudengjibiao&allTableName=Flow2_kesudengjibiao,OAJiuzhengyufangcuoshi,OAlipeidengji,kesudengjibiaoDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=Flow2_kesudengjibiao&allTableName=kesudengjibiaoDet,OAJiuzhengyufangcuoshi,OAlipeidengji,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=Flow2_kesudengjibiao&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=Flow2_kesudengjibiao&allTableName=kesudengjibiaoDet,OAJiuzhengyufangcuoshi,OAlipeidengji,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=cfe3917e_1409042026283637799&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=cfe3917e_1409042026283637799&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "Flow2_kesudengjibiaoDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=cfe3917e_1409042026283637799&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=cfe3917e_1409042026283637799&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=Flow2_kesudengjibiao&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=Flow2_kesudengjibiao&designId=cfe3917e_1409042026283637799&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=Flow2_kesudengjibiao&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=Flow2_kesudengjibiao&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=Flow2_kesudengjibiao&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '常规客诉登记表';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=Flow2_kesudengjibiao&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
