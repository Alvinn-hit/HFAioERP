...
{ 
 cFireEvent('tblBuyAskPriceDet_Qty',''); 
 cFireEvent('tblBuyAskPriceDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBuyAskPrice&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBuyAskPrice&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBuyAskPrice&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBuyAskPrice&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBuyAskPriceDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblBuyAskPriceDet_BatchNo');
 ...
 var z=n('tblBuyAskPriceDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblBuyAskPriceDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBuyAskPriceDet"){
 
 copyFields=n('tblBuyAskPriceDet_GoodsCode');
 if(copyFields[index].value !=0 && copyFields[index].value != ""){return}
 if((order > 1 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && ((copyFields[index].value ==0) ||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 var tblBuyAskPriceDet_tblGoods_GoodsNumber = n('tblBuyAskPriceDet_tblGoods_GoodsNumber');
 if(typeof(tblBuyAskPriceDet_tblGoods_GoodsNumber) != 'undefined' && tblBuyAskPriceDet_tblGoods_GoodsNumber.length >index){
 tblBuyAskPriceDet_tblGoods_GoodsNumber[index].value = tblBuyAskPriceDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblBuyAskPriceDet_tblGoods_GoodsFullName = n('tblBuyAskPriceDet_tblGoods_GoodsFullName');
 if(typeof(tblBuyAskPriceDet_tblGoods_GoodsFullName) != 'undefined' && tblBuyAskPriceDet_tblGoods_GoodsFullName.length >index){
 tblBuyAskPriceDet_tblGoods_GoodsFullName[index].value = tblBuyAskPriceDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblBuyAskPriceDet_tblGoods_GoodsSpec = n('tblBuyAskPriceDet_tblGoods_GoodsSpec');
 if(typeof(tblBuyAskPriceDet_tblGoods_GoodsSpec) != 'undefined' && tblBuyAskPriceDet_tblGoods_GoodsSpec.length >index){
 tblBuyAskPriceDet_tblGoods_GoodsSpec[index].value = tblBuyAskPriceDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblBuyAskPriceDet_tblGoods_BaseUnit = n('tblBuyAskPriceDet_tblGoods_BaseUnit');
 if(typeof(tblBuyAskPriceDet_tblGoods_BaseUnit) != 'undefined' && tblBuyAskPriceDet_tblGoods_BaseUnit.length >index){
 tblBuyAskPriceDet_tblGoods_BaseUnit[index].value = tblBuyAskPriceDet_tblGoods_BaseUnit[index-1].value;
 }
 var tblBuyAskPriceDet_tblGoods_Spec1 = n('tblBuyAskPriceDet_tblGoods_Spec1');
 if(typeof(tblBuyAskPriceDet_tblGoods_Spec1) != 'undefined' && tblBuyAskPriceDet_tblGoods_Spec1.length >index){
 tblBuyAskPriceDet_tblGoods_Spec1[index].value = tblBuyAskPriceDet_tblGoods_Spec1[index-1].value;
 }
 var tblBuyAskPriceDet_tblGoods_Spec2 = n('tblBuyAskPriceDet_tblGoods_Spec2');
 if(typeof(tblBuyAskPriceDet_tblGoods_Spec2) != 'undefined' && tblBuyAskPriceDet_tblGoods_Spec2.length >index){
 tblBuyAskPriceDet_tblGoods_Spec2[index].value = tblBuyAskPriceDet_tblGoods_Spec2[index-1].value;
 }
 var tblBuyAskPriceDet_tblGoods_Spec3 = n('tblBuyAskPriceDet_tblGoods_Spec3');
 if(typeof(tblBuyAskPriceDet_tblGoods_Spec3) != 'undefined' && tblBuyAskPriceDet_tblGoods_Spec3.length >index){
 tblBuyAskPriceDet_tblGoods_Spec3[index].value = tblBuyAskPriceDet_tblGoods_Spec3[index-1].value;
 }
 }
  copyFields=n('tblBuyAskPriceDet_ArriveDate');
 if((order > 80 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && ((copyFields[index].value ==0) ||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 }
 }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBuyAskPriceDet"){
 
 copyFields=n('tblBuyAskPriceDet_GoodsCode'); 
 if(copyFields[index].value !=0 && copyFields[index].value != ""){return}
 if( (order > 1 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyFields[index].value ==0||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 var tblBuyAskPriceDet_tblGoods_GoodsNumber = n('tblBuyAskPriceDet_tblGoods_GoodsNumber'); 
 if(typeof(tblBuyAskPriceDet_tblGoods_GoodsNumber) != 'undefined' && tblBuyAskPriceDet_tblGoods_GoodsNumber.length >index){
 tblBuyAskPriceDet_tblGoods_GoodsNumber[index].value = tblBuyAskPriceDet_tblGoods_GoodsNumber[index-1].value;
 ...
 tblBuyAskPriceDet_tblGoods_Spec3[index].value = tblBuyAskPriceDet_tblGoods_Spec3[index-1].value;
  ...
  copyFields=n('tblBuyAskPriceDet_ArriveDate'); 
 if( (order > 80 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyFields[index].value ==0||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 }
 }
 }
...
function execStat2(){
 execStatOne('tblBuyAskPriceDet_Qty',2);
 execStatOne('tblBuyAskPriceDet_Amount',2);
 execStatOne('tblBuyAskPriceDet_UnitQty',2);
 execStatOne('tblBuyAskPriceDet_TaxAmount',2);
 execStatOne('tblBuyAskPriceDet_CoTaxAmt',2);
 execStatOne('tblBuyAskPriceDet_CurAmount',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=3909404f_1003181132036090551&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=3909404f_1003181132036090551&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblBuyAskPrice\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBuyAskPrice\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_Qty',"tblBuyAskPriceDet"); 
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_Price',"tblBuyAskPriceDet"); 
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_Amount',"tblBuyAskPriceDet"); 
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_SecUnit',"tblBuyAskPriceDet"); 
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_UnitIntQty',"tblBuyAskPriceDet"); 
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_UnitBaseQty',"tblBuyAskPriceDet"); 
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_UnitPrice',"tblBuyAskPriceDet"); 
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_TaxPrice',"tblBuyAskPriceDet"); 
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_TaxAmount',"tblBuyAskPriceDet"); 
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_CurPrice',"tblBuyAskPriceDet"); 
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_CurAmount',"tblBuyAskPriceDet"); 
 delFireEventOne(rowId,tableName,'tblBuyAskPriceDet_PresentSampleType',"tblBuyAskPriceDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'Tax',dis:'税率%',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CurrencyRate',dis:'汇率',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalAmount',dis:'单据总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalTaxAmount',dis:'含税总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalCoTaxAmt',dis:'总税额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CurTotalAmount',dis:'外币总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblBuyAskPriceDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyAskPriceDet_Price',dis:'单价',digit:8,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyAskPriceDet_Amount',dis:'金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyAskPriceDet_UnitBaseQty',dis:'辅助基本单位',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyAskPriceDet_UnitQty',dis:'辅助单位数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyAskPriceDet_UnitPrice',dis:'辅助单位单价',digit:8,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyAskPriceDet_TaxPrice',dis:'含税单价',digit:8,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyAskPriceDet_TaxAmount',dis:'含税金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyAskPriceDet_CoTaxAmt',dis:'税额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyAskPriceDet_CurPrice',dis:'外币单价',digit:8,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyAskPriceDet_CurAmount',dis:'外币金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBuyAskPrice",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyAskPrice&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyAskPrice&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"3909404f_1003181132036090551",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 var saveField=n("tblCompany_ComFax");
 saveField[0].value = ""; 
 m("tbl_tblCompany_ComFax").value = "" ;
 var saveField=n("tblCompany_ComContactor");
 saveField[0].value = ""; 
 m("tbl_tblCompany_ComContactor").value = "" ;
 }
 ...
 saveField[0].value = "";
 var obj = m("tbl_tblSetExchange_Currency");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblSetExchange_CurrencyName");
 saveField[0].value = ""; 
 m("tbl_tblSetExchange_CurrencyName").value = "" ;
 }
  }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=3909404f_1003181132036090551&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=3909404f_1003181132036090551&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=3909404f_1003181132036090551&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblBuyAskPrice"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBuyAskPrice&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBuyAskPrice&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 if(nn('InVoiceType',0)==2)n('Tax')[0].value=f("17",8);

if(nn('InVoiceType',0)==1)n('Tax')[0].value=f("3.67",8);

if(nn('InVoiceType',0)==3)n('Tax')[0].value=f(0,8);

for(var j=0;j<r(0,'TaxPrice').length;j++){
r(0,"TaxPrice")[j].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",j)*rn(0,"Price",j)/100,8);
}
...
r(0,"CurPrice")[j].value=f((((nn('Tax',0)/100)+1)*rn(0,"Discount",j)*rn(0,"Price",j)/100)/nn('CurrencyRate',0),8);
}
...
n('TotalAmount')[0].value=f(sm('tblBuyAskPriceDet_DisBackAmt'),2);

n('TotalTaxAmount')[0].value=f(sm('tblBuyAskPriceDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblBuyAskPriceDet_CoTaxAmt'),2);

for(var j=0;j<r(0,'DisAmount').length;j++){
r(0,"DisAmount")[j].value=f(rn(0,"Amount",j)-rn(0,"DisBackAmt",j),2);
}

n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
...
r(0,"Discount")[j].value=f(nn('Discount',0),8);
}

for(var j=0;j<r(0,'DisPrice').length;j++){
r(0,"DisPrice")[j].value=f(rn(0,"Discount",j)*rn(0,"Price",j)/100,8);
}
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 z[i].onchange = function(){ 
 for(var j=0;j<r(0,'TaxPrice').length;j++){
r(0,"TaxPrice")[j].value=f(rn(0,"CurPrice",j)*nn('CurrencyRate',0),8);
}

for(var j=0;j<r(0,'DisPrice').length;j++){
r(0,"DisPrice")[j].value=f(rn(0,"CurPrice",j)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),8);
}

for(var j=0;j<r(0,'Price').length;j++){
r(0,"Price")[j].value=f(rn(0,"CurPrice",j)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",j)/100),8);
}
...
for(var j=0;j<r(0,'UnitPrice').length;j++){
r(0,"UnitPrice")[j].value=f(rn(0,"Price",j)*(rn(0,"BaseQty",j)/rn(0,"SecQty",j)),8);
}
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_Qty'); 
...
n('TotalTaxAmount')[0].value=f(sm('tblBuyAskPriceDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(nn('TotalTaxAmount',0)/nn('CurrencyRate',0),2);
r(0,"UnitQty")[p].value=f(rn(0,"Qty",p)/(rn(0,"BaseQty",p)/rn(0,"SecQty",p)),2);
r(0,"UnitIntQty")[p].value=f(parseInt(rn(0,"Qty",p)/(rn(0,"BaseQty",p)/rn(0,"SecQty",p))),2);
r(0,"UnitBaseQty")[p].value=f(rn(0,"Qty",p)%(rn(0,"BaseQty",p)/rn(0,"SecQty",p)),2);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
r(0,"CoTaxAmt")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100-rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
n('TotalCoTaxAmt')[0].value=f(sm('tblBuyAskPriceDet_CoTaxAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,8);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,8);
r(0,"CurPrice")[p].value=f((((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100)/nn('CurrencyRate',0),8);
r(0,"UnitPrice")[p].value=f(rn(0,"Price",p)*(rn(0,"BaseQty",p)/rn(0,"SecQty",p)),8);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
n('TotalAmount')[0].value=f(sm('tblBuyAskPriceDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblBuyAskPriceDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_Amount'); 
 r(0,"Price")[p].value=f(rn(0,"Amount",p)/rn(0,"Qty",p),8);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,8);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*(rn(0,"Discount",p)*rn(0,"Price",p)/100),8);
r(0,"CurPrice")[p].value=f((((nn('Tax',0)/100)+1)*(rn(0,"Discount",p)*rn(0,"Price",p)/100))/nn('CurrencyRate',0),8);
r(0,"UnitPrice")[p].value=f(rn(0,"Price",p)*(rn(0,"BaseQty",p)/rn(0,"SecQty",p)),8);
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_SecUnit'); 
 r(0,"UnitQty")[p].value=f(rn(0,"Qty",p)*rn(0,"SecQty",p)/rn(0,"BaseQty",p),2);

r(0,"UnitPrice")[p].value=f(rn(0,"Price",p)*rn(0,"BaseQty",p)/rn(0,"SecQty",p),8);

r(0,"UnitBaseQty")[p].value=f(rn(0,"Qty",p)%((rn(0,"BaseQty",p)/rn(0,"SecQty",p))),2);

r(0,"UnitIntQty")[p].value=f(Math.floor(rn(0,"Qty",p)*rn(0,"SecQty",p)/rn(0,"BaseQty",p)),2);

 execStat(); 
 }
 }
 var z=r(0,'UnitIntQty');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_UnitIntQty'); 
 r(0,"Qty")[p].value=f(rn(0,"UnitIntQty",p)*(rn(0,"BaseQty",p)/rn(0,"SecQty",p))+rn(0,"UnitBaseQty",p),2);
r(0,"UnitQty")[p].value=f(rn(0,"Qty",p)/(rn(0,"BaseQty",p)/rn(0,"SecQty",p)),2);
r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
n('CurTotalAmount')[0].value=f(sm('tblBuyAskPriceDet_CurAmount'),2);
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 var z=r(0,'UnitBaseQty');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_UnitBaseQty'); 
 r(0,"Qty")[p].value=f(rn(0,"UnitIntQty",p)*(rn(0,"BaseQty",p)/rn(0,"SecQty",p))+rn(0,"UnitBaseQty",p),2);
 r(0,"UnitQty")[p].value=f(rn(0,"Qty",p)/(rn(0,"BaseQty",p)/rn(0,"SecQty",p)),2);
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
 r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*(rn(0,"Discount",p)/100),2);
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_UnitPrice'); 
 r(0,"Price")[p].value=f(rn(0,"UnitPrice",p)/(rn(0,"BaseQty",p)/rn(0,"SecQty",p)),8);
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
 r(0,"DisPrice")[p].value=f(rn(0,"Price",p)*(rn(0,"Discount",p)/100),8);
 r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*(rn(0,"Discount",p)/100),2);
 r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
 r(0,"TaxPrice")[p].value=f(rn(0,"Price",p)*(rn(0,"Discount",p)/100)*((nn('Tax',0)/100)+1),8);
 r(0,"TaxAmount")[p].value=f(rn(0,"Amount",p)*(rn(0,"Discount",p)/100)*((nn('Tax',0)/100)+1),2);
 r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
 n('TotalCoTaxAmt')[0].value=f(sm('tblBuyAskPriceDet_CoTaxAmt'),2);
n('TotalAmount')[0].value=f(sm('tblBuyAskPriceDet_DisBackAmt'),2);
 n('TotalTaxAmount')[0].value=f(sm('tblBuyAskPriceDet_TaxAmount')-nn('DiscountAmount',0),2);
 r(0,"CurPrice")[p].value=f(rn(0,"Price",p)*(rn(0,"Discount",p)/100)*((nn('Tax',0)/100)+1)/nn('CurrencyRate',0),8);
 r(0,"CurAmount")[p].value=f(rn(0,"Amount",p)*(rn(0,"Discount",p)/100)*((nn('Tax',0)/100)+1)/nn('CurrencyRate',0),2);
 n('CurTotalAmount')[0].value=f(sm('tblBuyAskPriceDet_CurAmount'),2);
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,8);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,8);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_DisPrice'); 
 r(0,"Discount")[p].value=f(rn(0,"DisPrice",p)/rn(0,"Price",p)*100,8);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"DisPrice",p),8);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
n('TotalAmount')[0].value=f(sm('tblBuyAskPriceDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblBuyAskPriceDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(nn('TotalTaxAmount',0)/nn('CurrencyRate',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblBuyAskPriceDet_CoTaxAmt'),2);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"CurPrice")[p].value=f((((nn('Tax',0)/100)+1)*rn(0,"DisPrice",p))/nn('CurrencyRate',0),8);
r(0,"CurAmount")[p].value=f((((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100)/nn('CurrencyRate',0),2);
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_DisBackAmt'); 
 r(0,"DisPrice")[p].value=f(rn(0,"DisBackAmt",p)/rn(0,"Qty",p),8);
r(0,"Discount")[p].value=f(rn(0,"DisPrice",p)/rn(0,"Price",p)*100,8);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"DisPrice",p),8);
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_TaxPrice'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"TaxPrice",p),2);
r(0,"DisPrice")[p].value=f(rn(0,"TaxPrice",p)/((nn('Tax',0)/100)+1),8);
r(0,"Price")[p].value=f((rn(0,"TaxPrice",p)/((nn('Tax',0)/100)+1)*100)/rn(0,"Discount",p),8);
r(0,"CurPrice")[p].value=f(rn(0,"TaxPrice",p)/nn('CurrencyRate',0),8);
r(0,"UnitPrice")[p].value=f(((rn(0,"TaxPrice",p)/((nn('Tax',0)/100)+1)*100)/rn(0,"Discount",p))*(rn(0,"BaseQty",p)/rn(0,"SecQty",p)),8);
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_TaxAmount'); 
 r(0,"TaxPrice")[p].value=f(rn(0,"TaxAmount",p)/rn(0,"Qty",p),8);
r(0,"DisPrice")[p].value=f(rn(0,"TaxPrice",p)/((nn('Tax',0)/100)+1),8);
r(0,"Price")[p].value=f((rn(0,"TaxPrice",p)/((nn('Tax',0)/100)+1))*100/rn(0,"Discount",p),8);
r(0,"CurPrice")[p].value=f((rn(0,"TaxAmount",p)/rn(0,"Qty",p))/nn('CurrencyRate',0),8);
r(0,"UnitPrice")[p].value=f(((rn(0,"TaxPrice",p)/((nn('Tax',0)/100)+1))*100/rn(0,"Discount",p))*(rn(0,"BaseQty",p)/rn(0,"SecQty",p)),8);
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_CurPrice'); 
 r(0,"TaxPrice")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0),8);
 r(0,"DisPrice")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),8);
r(0,"Price")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),8);
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_CurAmount'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),2);
r(0,"CurPrice")[p].value=f(rn(0,"CurAmount",p)/rn(0,"Qty",p),8);
r(0,"TaxPrice")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0),8);
r(0,"DisPrice")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),8);
r(0,"Price")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),8);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_PresentSampleType'); 
 r(0,"Price")[p].value=f(0,8);

r(0,"Amount")[p].value=f(0,2);

r(0,"DisPrice")[p].value=f(0,8);

r(0,"TaxPrice")[p].value=f(0,8);

r(0,"CurPrice")[p].value=f(0,8);

r(0,"UnitPrice")[p].value=f(0,8);
...
n('DisAmount')[0].value=f(sm('tblBuyAskPriceDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyAskPriceDet_SecQty'); 
 r(0,"UnitQty")[p].value=f(rn(0,"Qty",p)*rn(0,"SecQty",p)/rn(0,"BaseQty",p),2);
...
childData[childCount]='tblBuyAskPriceDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='采购询价单明细';
childCount++;
 var tblBuyAskPriceDetData = new gridData();
 gridDatas[gridDataNum] = tblBuyAskPriceDetData;
 gridDataNum++;
 var tblBuyAskPriceDet=1;
 ...
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_GoodsCode','商品名称','120',2,false,'',100,-2,'','',"","copy","1",'',"2",true);
 putChildValidateItem("tblBuyAskPriceDet_GoodsCode","商品名称","any","",false,minValue,100,false,'');
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_tblGoods_GoodsNumber','商品编号','80',2,true,'',1000000,2,'tblBuyAskPriceDet:GoodsCode:tblBuyAskPriceDet_tblGoods_GoodsNumber:'+param4,'',"","copy","1",'',"2",false); 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_tblGoods_GoodsFullName','商品名称','80',18,true,'',1000000,2,'tblBuyAskPriceDet:GoodsCode:tblBuyAskPriceDet_tblGoods_GoodsFullName:'+param4,'',"","copy","1",'',"2",false); 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_tblGoods_GoodsSpec','商品型号','150',2,true,'',1000000,2,'tblBuyAskPriceDet:GoodsCode:tblBuyAskPriceDet_tblGoods_GoodsSpec:'+param4,'',"","copy","1",'',"2",false); 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblBuyAskPriceDet:GoodsCode:tblBuyAskPriceDet_tblGoods_BaseUnit:'+param4,'',"","copy","1",'',"2",false); 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_tblGoods_Spec1','商品规格描述','180',2,true,'',1000000,2,'tblBuyAskPriceDet:GoodsCode:tblBuyAskPriceDet_tblGoods_Spec1:'+param4,'',"","copy","1",'',"2",false); 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_tblGoods_Spec2','封装','80',2,true,'',1000000,2,'tblBuyAskPriceDet:GoodsCode:tblBuyAskPriceDet_tblGoods_Spec2:'+param4,'',"","copy","1",'',"2",false); 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_tblGoods_Spec3','厂家','80',2,true,'',1000000,2,'tblBuyAskPriceDet:GoodsCode:tblBuyAskPriceDet_tblGoods_Spec3:'+param4,'',"","copy","1",'',"2",false); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_BatchNo','批号','80',2,true,'',100,0,'','',"","triggerCopy","5",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","8",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","11",'',"2",false); 
 putChildValidateItem("tblBuyAskPriceDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","14",'',"2",false); 
 putChildValidateItem("tblBuyAskPriceDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","17",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_Qty','数量','60',1,false,'',0,0,'','',"Qty","","30",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_Qty","数量","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_Price','单价','80',1,true,'',0,0,'','',"priceIdentifier","","31",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_Amount','金额','100',1,true,'',0,0,'','',"AmountIdentifier","","32",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 var param4="";
 param4=param4+":1商品名称@tblBuyAskPriceDet_GoodsCode";
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_SecUnit','辅助单位','80',2,true,'',30,2,'tblBuyAskPriceDet:SecUnit:tblBuyAskPriceDet_SecUnit:'+param4,"","","","33",'',"2",true);
 putChildValidateItem("tblBuyAskPriceDet_SecUnit","辅助单位","any","AssUnit",true,minValue,30,false,'',true);
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_UnitIntQty','辅助单位整数','100',0,true,'',0,0,'','',"Qty","","34",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_UnitIntQty","辅助单位整数","int","AssUnit",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_UnitBaseQty','辅助基本单位','100',1,true,'',0,0,'','',"Qty","","35",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_UnitBaseQty","辅助基本单位","double","AssUnit",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_UnitQty','辅助单位数量','100',1,true,'',0,8,'','',"Qty","","36",'',"8",false); 
 putChildValidateItem("tblBuyAskPriceDet_UnitQty","辅助单位数量","double","AssUnit",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_UnitPrice','辅助单位单价','100',1,true,'',0,0,'','',"priceIdentifier","","37",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_UnitPrice","辅助单位单价","double","AssUnit",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_Discount','折扣率%','100',1,true,'100',0,-2,'','',"Price","","38",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_Discount","折扣率%","double","BillDiscount",true,minValue,maxValue,true,'100'); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_DisPrice','折后单价','100',1,true,'',0,-2,'','',"priceIdentifier","","39",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_DisPrice","折后单价","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_DisBackAmt','折后金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","40",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_DisBackAmt","折后金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_DisAmount','折让金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","41",'',"8",false); 
 putChildValidateItem("tblBuyAskPriceDet_DisAmount","折让金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_TaxPrice','含税单价','100',1,true,'',0,0,'','',"priceIdentifier","","42",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_TaxPrice","含税单价","double","BillTax",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_TaxAmount','含税金额','100',1,true,'',0,0,'','',"AmountIdentifier","","43",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_TaxAmount","含税金额","double","BillTax",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_CoTaxAmt','税额','100',1,true,'0',0,8,'','',"AmountIdentifier","","44",'',"8",false); 
 putChildValidateItem("tblBuyAskPriceDet_CoTaxAmt","税额","double","BillTax",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_CurPrice','外币单价','100',1,true,'',0,0,'','',"priceIdentifier","","45",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_CurPrice","外币单价","double","currency",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_CurAmount','外币金额','100',1,true,'',0,0,'','',"AmountIdentifier","","46",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_CurAmount","外币金额","double","currency",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_ArriveDate','到货日期','80',5,false,'',10,0,'','',"","copy","80",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_ArriveDate","到货日期","date","",false,minValue,10,false,'',true); 
 ...
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_PresentSampleType','赠(样)品','80',2,true,'',10,1,'','',"","","108",'',"1",false);
 putChildValidateItem("tblBuyAskPriceDet_PresentSampleType","赠(样)品","any","BillPresentSampleType",true,minValue,10,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","114",'',"3",false); 
 putChildValidateItem("tblBuyAskPriceDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","117",'',"3",false); 
 putChildValidateItem("tblBuyAskPriceDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_dingdanhao','对方订单号','100',2,true,'',8000,0,'','',"","","-1",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_dingdanhao","对方订单号","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_Remark','备注','100',2,true,'',8000,0,'','',"","","-1",'',"0",false); 
 putChildValidateItem("tblBuyAskPriceDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 addCols(tblBuyAskPriceDetData,'tblBuyAskPriceDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBuyAskPrice&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBuyAskPrice,tblBuyAskPriceDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyAskPrice&allTableName=tblBuyAskPrice,tblBuyAskPriceDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyAskPrice&allTableName=tblBuyAskPriceDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyAskPrice&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyAskPrice&allTableName=tblBuyAskPriceDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=3909404f_1003181132036090551&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=3909404f_1003181132036090551&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblBuyAskPriceDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=3909404f_1003181132036090551&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=3909404f_1003181132036090551&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblBuyAskPrice&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblBuyAskPrice&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBuyAskPrice&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBuyAskPrice&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBuyAskPrice&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '采购询价单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBuyAskPrice&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
...
　　　 html:'<div style="text-align:center;"><div style="text-align: left;padding-left: 20px;"><p>'+
 '提示：请选择录入格式，分隔符可以是空格或，或；</p><p><select id="batchFieldsel">'+constr+'</select> +分隔符+数量(可为空)+分隔符+单价（可为空）</p></div>'
