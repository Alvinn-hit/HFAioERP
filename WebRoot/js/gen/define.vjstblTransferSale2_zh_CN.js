...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblTransferSale2&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblTransferSale2&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblTransferSale2&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblTransferSale2&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblTransferSaleDet2Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblTransferSaleDet2"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblTransferSaleDet2"){
...
function execStat2(){
 execStatOne('tblTransferSaleDet2_BillAmt',2);
 execStatOne('tblTransferSaleDet2_SettledAmt',2);
 execStatOne('tblTransferSaleDet2_WexeBalAmt',2);
 execStatOne('tblTransferSaleDet2_ExeBalAmt',2);
 execStatOne('tblTransferSaleDet2_ExeBalFcAmt',2);
 setTotalCount(getTotalCount());
}
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblTransferSale2\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblTransferSaleDet2_ExeBalAmt',"tblTransferSaleDet2"); 
 delFireEventOne(rowId,tableName,'tblTransferSaleDet2_ExeBalFcAmt',"tblTransferSaleDet2"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'ExeTransferAmt',dis:'转账金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CurrencyRate',dis:'汇率',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'FcExeTransferAmt',dis:'外币转账金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblTransferSaleDet2_BillAmt',dis:'单据金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblTransferSaleDet2_SettledAmt',dis:'已结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblTransferSaleDet2_WexeBalAmt',dis:'待结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblTransferSaleDet2_ExeBalAmt',dis:'本次结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblTransferSaleDet2_ExeBalFcAmt',dis:'外币结算金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblTransferSale2",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblTransferSale2&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblTransferSale2&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "InCompanyCode"){
 var saveField=n("InCompanyCode"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_classCode");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SelectClientIn_ComFullName");
 saveField[0].value = ""; 
 m("tbl_SelectClientIn_ComFullName").value = "" ;
 }
 ...
 if(fieldName == "RefBillNo"){
 var saveField=n("RefBillNo"); 
 saveField[0].value = "";
 var obj = m("tbl_tblBuyInStock_BillNo");
 if(obj != null){
 obj.value = "";
 }
 }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblTransferSale2"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblTransferSale2&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblTransferSale2&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 var z=n('ExeTransferAmt');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('FcExeTransferAmt')[0].value=f(nn('ExeTransferAmt',0)/nn('CurrencyRate',0),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 for(var j=0;j<r(0,'ExeBalAmt').length;j++){
r(0,"ExeBalAmt")[j].value=f(rn(0,"ExeBalFcAmt",j)*nn('CurrencyRate',0),2);
}

n('ExeTransferAmt')[0].value=f(sm('tblTransferSaleDet2_ExeBalAmt'),2);

n('FcExeTransferAmt')[0].value=f(sm('tblTransferSaleDet2_ExeBalFcAmt'),2);

 
 }
 } 

 var z=n('FcExeTransferAmt');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('ExeTransferAmt')[0].value=f(nn('FcExeTransferAmt',0)*nn('CurrencyRate',0),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblTransferSaleDet2_ExeBalAmt'); 
 r(0,"ExeBalFcAmt")[p].value=f(rn(0,"ExeBalAmt",p)/nn('CurrencyRate',0),2);
n('ExeTransferAmt')[0].value=f(sm('tblTransferSaleDet2_ExeBalAmt'),2);
n('FcExeTransferAmt')[0].value=f(sm('tblTransferSaleDet2_ExeBalFcAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblTransferSaleDet2_ExeBalFcAmt'); 
 r(0,"ExeBalAmt")[p].value=f(rn(0,"ExeBalFcAmt",p)*nn('CurrencyRate',0),2);
n('ExeTransferAmt')[0].value=f(sm('tblTransferSaleDet2_ExeBalAmt'),2);
n('FcExeTransferAmt')[0].value=f(sm('tblTransferSaleDet2_ExeBalFcAmt'),2);
...
childData[childCount]='tblTransferSaleDet2';
tabIsNulls[childCount]='0';
tabDiss[childCount]='应付转应收明细';
childCount++;
 var tblTransferSaleDet2Data = new gridData();
 gridDatas[gridDataNum] = tblTransferSaleDet2Data;
 gridDataNum++;
 var tblTransferSaleDet2=1;
 
 
 addCols(tblTransferSaleDet2Data,'tblTransferSaleDet2_CurBillAmt','外币单据金额','80',1,true,'',0,-2,'','',"AmountIdentifier","","0",'',"3",false); 
 putChildValidateItem("tblTransferSaleDet2_CurBillAmt","外币单据金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblTransferSaleDet2Data,'tblTransferSaleDet2_CurSettledAmt','外币已结算金额','80',1,true,'',0,-2,'','',"AmountIdentifier","","0",'',"3",false); 
 putChildValidateItem("tblTransferSaleDet2_CurSettledAmt","外币已结算金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblTransferSaleDet2Data,'tblTransferSaleDet2_CurWexeBalAmt','外币待结算金额','80',1,true,'',0,-2,'','',"AmountIdentifier","","0",'',"3",false); 
 putChildValidateItem("tblTransferSaleDet2_CurWexeBalAmt","外币待结算金额","double","currency",true,minValue,maxValue,false,''); 
 ...
 param4=param4+":1转入单位@InCompanyCode"; 
 
 addCols(tblTransferSaleDet2Data,'tblTransferSaleDet2_RefbillID','关联单据编号','120',2,true,'',70,2,'tblTransferSaleDet2:RefbillID:tblTransferSaleDet2_RefbillID:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblTransferSaleDet2_RefbillID","关联单据编号","any","",true,minValue,70,false,'',true);
 
 
 
 addCols(tblTransferSaleDet2Data,'tblTransferSaleDet2_BillAmt','单据金额','100',1,true,'',0,8,'','',"AmountIdentifier","","2",'',"8",false); 
 putChildValidateItem("tblTransferSaleDet2_BillAmt","单据金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblTransferSaleDet2Data,'tblTransferSaleDet2_SettledAmt','已结算金额','100',1,true,'',0,0,'','',"AmountIdentifier","","3",'',"0",false); 
 putChildValidateItem("tblTransferSaleDet2_SettledAmt","已结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblTransferSaleDet2Data,'tblTransferSaleDet2_WexeBalAmt','待结算金额','100',1,true,'',0,0,'','',"AmountIdentifier","","4",'',"0",false); 
 putChildValidateItem("tblTransferSaleDet2_WexeBalAmt","待结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblTransferSaleDet2Data,'tblTransferSaleDet2_ExeBalAmt','本次结算金额','100',1,true,'',0,0,'','',"AmountIdentifier","","5",'',"0",false); 
 putChildValidateItem("tblTransferSaleDet2_ExeBalAmt","本次结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblTransferSaleDet2Data,'tblTransferSaleDet2_ExeBalFcAmt','外币结算金额','80',1,true,'',0,0,'','',"AmountIdentifier","","6",'',"0",false); 
 putChildValidateItem("tblTransferSaleDet2_ExeBalFcAmt","外币结算金额","double","currency",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblTransferSaleDet2Data,'tblTransferSaleDet2_Remark','备注','200',2,true,'',8000,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblTransferSaleDet2_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 
 
 addCols(tblTransferSaleDet2Data,'tblTransferSaleDet2_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblTransferSale2&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblTransferSale2,tblTransferSaleDet2,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblTransferSale2&allTableName=tblTransferSale2,tblTransferSaleDet2,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblTransferSale2&allTableName=tblTransferSaleDet2,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblTransferSale2&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblTransferSale2&allTableName=tblTransferSaleDet2,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblTransferSale2Det";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblTransferSale2&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblTransferSale2&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblTransferSale2&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblTransferSale2&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblTransferSale2&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '应付转应收';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblTransferSale2&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
