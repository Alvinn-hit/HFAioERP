...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBuyApplicationTotal&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBuyApplicationTotal&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBuyApplicationTotal&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBuyApplicationTotal&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBuyApplicationTotalDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblBuyApplicationTotalDet_BatchNo');
 ...
 var z=n('tblBuyApplicationTotalDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblBuyApplicationTotalDet_Hue');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblBuyApplicationTotalDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBuyApplicationTotalDet"){
 
 copyFields=n('tblBuyApplicationTotalDet_GoodsCode');
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBuyApplicationTotalDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblBuyApplicationTotalDet"; 
...
function execStat2(){
 execStatOne('tblBuyApplicationTotalDet_Qty',4);
 execStatOne('tblBuyApplicationTotalDet_planQty',4);
 setTotalCount(getTotalCount());
}
...
function execDetailStat2(){
 execDetStatOne('tblBuyApplicationTotalDet_Qty',4); 
 execDetStatOne('tblBuyApplicationTotalDet_planQty',4); 
 }
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=e1924860_1212211628497770403&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 url2='&src=menu&MOID=e1924860_1212211628497770403&MOOP=add&'+url2;
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBuyApplicationTotal\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblBuyApplicationTotalDet_Qty',dis:'申请数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyApplicationTotalDet_planQty',dis:'预购数量',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBuyApplicationTotal",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyApplicationTotal&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyApplicationTotal&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'打印控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"e1924860_1212211628497770403",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=e1924860_1212211628497770403&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=e1924860_1212211628497770403&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=e1924860_1212211628497770403&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblBuyApplicationTotal"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBuyApplicationTotal&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBuyApplicationTotal&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblBuyApplicationTotalDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='申请汇总明细';
childCount++;
 var tblBuyApplicationTotalDetData = new gridData();
 gridDatas[gridDataNum] = tblBuyApplicationTotalDetData;
 gridDataNum++;
 var tblBuyApplicationTotalDet=1;
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_DepartmentCode','部门','224',2,false,'',100,-2,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_DepartmentCode","部门","any","",false,minValue,100,false,''); 
 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_AllotQty','已配送数量','70',1,true,'0',0,-2,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_AllotQty","已配送数量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_NotAllotQty','未配送数量','70',1,true,'',0,-2,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_NotAllotQty","未配送数量","double","",true,minValue,maxValue,false,''); 
 ...
 param4=param4+":tblBuyApplicationTotalDet_GoodsCode@tblBuyApplicationTotalDet_GoodsCode"; 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_BuyApplyID','申请单号','224',2,false,'',100,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("tblBuyApplicationTotalDet_BuyApplyID","申请单号","any","",false,minValue,100,false,'');
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_Department_DeptFullName','部门','80',2,true,'',1000000,2,'tblBuyApplicationTotalDet:BuyApplyID:tblBuyApplicationTotalDet_Department_DeptFullName:'+param4,'',"","","1",'',"2",false); 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_tblBuyApplication_BillNo','申请单号','120',2,true,'',1000000,2,'tblBuyApplicationTotalDet:BuyApplyID:tblBuyApplicationTotalDet_tblBuyApplication_BillNo:'+param4,'',"","","1",'',"2",false); 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_tblGoods_GoodsFullName','商品名称','150',2,true,'',1000000,2,'tblBuyApplicationTotalDet:BuyApplyID:tblBuyApplicationTotalDet_tblGoods_GoodsFullName:'+param4,'',"","","1",'',"2",false); 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'tblBuyApplicationTotalDet:BuyApplyID:tblBuyApplicationTotalDet_tblGoods_GoodsNumber:'+param4,'',"","","1",'',"2",false); 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'tblBuyApplicationTotalDet:BuyApplyID:tblBuyApplicationTotalDet_tblGoods_GoodsSpec:'+param4,'',"","","1",'',"2",false); 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblBuyApplicationTotalDet:BuyApplyID:tblBuyApplicationTotalDet_tblGoods_BaseUnit:'+param4,'',"","","1",'',"2",false); 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_tblGoods_BarCode','条码','100',2,true,'',1000000,2,'tblBuyApplicationTotalDet:BuyApplyID:tblBuyApplicationTotalDet_tblGoods_BarCode:'+param4,'',"","","1",'',"2",false); 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_tblEmployee_EmpFullName','采购员','80',2,true,'',1000000,2,'tblBuyApplicationTotalDet:BuyApplyID:tblBuyApplicationTotalDet_tblEmployee_EmpFullName:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_GoodsCode','商品名称','120',2,false,'',100,-2,'','',"","copy","2",'',"3",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_GoodsCode","商品名称","any","",false,minValue,100,false,''); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","5",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_color','色彩','70',2,true,'',200,-2,'','',"","","10",'',"3",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_color","色彩","any","GoodsField",true,minValue,200,false,''); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","10",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_OrderQty','已订数量','70',1,true,'0',0,-2,'','',"Qty","","12",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_OrderQty","已订数量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_NoOrderQty','未订数量','70',1,true,'0',0,-2,'','',"Qty","","13",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_NoOrderQty","未订数量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","15",'',"2",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_Price','单价','70',1,true,'0',0,-2,'','',"Price","","16",'',"100",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_Price","单价","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_Amount','金额','70',1,true,'0',0,-2,'','',"Amount","","17",'',"100",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_Amount","金额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","20",'',"2",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_ProDate','生产日期','80',5,true,'',100,-2,'','',"","","25",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_ProDate","生产日期","date","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_Availably','保质期限','80',5,true,'',100,-2,'','',"","","30",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_Availably","保质期限","date","GoodsField",true,minValue,100,false,''); 
 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_EmployeeID','采购员','80',2,true,'',50,-2,'','',"","","35",'',"3",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_EmployeeID","采购员","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_Qty','申请数量','70',1,true,'0',0,8,'','',"Qty","","36",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_Qty","申请数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_planQty','预购数量','70',1,true,'0',0,0,'','',"Qty","","40",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_planQty","预购数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_Userdefined1','自定义1','100',2,true,'',200,-2,'','',"","","40",'',"6",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_Userdefined1","自定义1","any","",true,minValue,200,false,''); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_Userdefined2','自定义2','100',2,true,'',200,-2,'','',"","","41",'',"6",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_Userdefined2","自定义2","any","",true,minValue,200,false,''); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_Userdefined3','自定义3','100',2,true,'',200,-2,'','',"","","42",'',"6",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_Userdefined3","自定义3","any","",true,minValue,200,false,''); 
 ...
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_Remark','备注','150',2,true,'',8000,0,'','',"","","45",'',"0",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_ReturnQty','配送退货数','224',1,true,'',0,-2,'','',"Qty","","100",'',"3",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_ReturnQty","配送退货数","double","",true,minValue,maxValue,false,''); 
 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_SourceID','源ID','224',2,true,'',100,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_SourceID","源ID","any","",true,minValue,100,false,''); 
 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_NotReturnQty','未配送退货数','224',1,true,'',0,-2,'','',"Qty","","100",'',"3",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_NotReturnQty","未配送退货数","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_SecQty','辅助数量','224',0,true,'1',0,-2,'','',"","","120",'',"3",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'1'); 
 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_BaseQty','基本单位','224',0,true,'1',0,-2,'','',"","","120",'',"3",false); 
 putChildValidateItem("tblBuyApplicationTotalDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'1'); 
 
 
 addCols(tblBuyApplicationTotalDetData,'tblBuyApplicationTotalDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBuyApplicationTotal&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBuyApplicationTotal,tblBuyApplicationTotalDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyApplicationTotal&allTableName=tblBuyApplicationTotal,tblBuyApplicationTotalDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyApplicationTotal&allTableName=tblBuyApplicationTotalDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyApplicationTotal&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyApplicationTotal&allTableName=tblBuyApplicationTotalDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=e1924860_1212211628497770403&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=e1924860_1212211628497770403&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblBuyApplicationTotalDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=e1924860_1212211628497770403&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=e1924860_1212211628497770403&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblBuyApplicationTotal&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){
 location.href="/UserFunctionAction.do?tableName=tblBuyApplicationTotal&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBuyApplicationTotal&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBuyApplicationTotal&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBuyApplicationTotal&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '申请汇总单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBuyApplicationTotal&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
