...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblExproxyApply&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblExproxyApply&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblExproxyApply&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblExproxyApply&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblExproxyApplyDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblExproxyApplyDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblExproxyApplyDet"){
...
function execStat2(){
 execStatOne('tblExproxyApplyDet_PayAmount',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=45329091_1302051645412180120&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=45329091_1302051645412180120&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblExproxyApply\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblExproxyApply\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblExproxyApplyDet_PayAmount',"tblExproxyApplyDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'TotalAmount',dis:'开票明细费用合计',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'DLFee',dis:'综合服务费',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalKPAmount',dis:'开票总金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblExproxyApplyDet_PayAmount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblExproxyApply",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblExproxyApply&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblExproxyApply&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"45329091_1302051645412180120",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 saveField[0].value = "";
 var obj = m("tbl_tblESheet_ShipTo");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblESheet_BillNo");
 saveField[0].value = ""; 
 m("tbl_tblESheet_BillNo").value = "" ;
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=45329091_1302051645412180120&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=45329091_1302051645412180120&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=45329091_1302051645412180120&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblExproxyApply"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblExproxyApply&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblExproxyApply&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('TotalAmount')[0].value=f(sm('tblExproxyApplyDet_PayAmount'),2);

n('TotalKPAmount')[0].value=f(Number(nn('TotalAmount',0))+Number(nn('DLFee',0))+Number(nn('GNFee',0))+Number(nn('KDFee',0)),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblExproxyApplyDet_PayAmount'); 
 n('TotalAmount')[0].value=f(sm('tblExproxyApplyDet_PayAmount'),2);
n('TotalKPAmount')[0].value=f(Number(nn('TotalAmount',0))+Number(nn('DLFee',0))+Number(nn('GNFee',0))+Number(nn('KDFee',0)),2);
...
childData[childCount]='tblExproxyApplyDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='出口增值税(6%)开票申请明细';
childCount++;
 var tblExproxyApplyDetData = new gridData();
 gridDatas[gridDataNum] = tblExproxyApplyDetData;
 gridDataNum++;
 var tblExproxyApplyDet=1;
 ...
 addCols(tblExproxyApplyDetData,'tblExproxyApplyDet_PayItemCode','结算项编号','100',2,true,'',30,2,'tblExproxyApplyDet:PayItemCode:tblExproxyApplyDet_PayItemCode:'+param4,"","","","2",'',"2",true);
 putChildValidateItem("tblExproxyApplyDet_PayItemCode","结算项编号","any","",true,minValue,30,false,'',true);
 
 
 
 addCols(tblExproxyApplyDetData,'tblExproxyApplyDet_PayItem','结算项','170',2,true,'',50,8,'','',"","","3",'',"8",false); 
 putChildValidateItem("tblExproxyApplyDet_PayItem","结算项","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblExproxyApplyDetData,'tblExproxyApplyDet_PayItemRemark','结算项全称','250',2,true,'',100,8,'','',"","","4",'',"8",false); 
 putChildValidateItem("tblExproxyApplyDet_PayItemRemark","结算项全称","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblExproxyApplyDetData,'tblExproxyApplyDet_PayAmount','金额','120',1,true,'0',0,0,'','',"Amount","","4",'',"0",false); 
 putChildValidateItem("tblExproxyApplyDet_PayAmount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblExproxyApplyDetData,'tblExproxyApplyDet_Remark','备注','250',2,true,'',8000,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblExproxyApplyDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblExproxyApplyDetData,'tblExproxyApplyDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblExproxyApply&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblExproxyApply,tblExproxyApplyDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblExproxyApply&allTableName=tblExproxyApply,tblExproxyApplyDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblExproxyApply&allTableName=tblExproxyApplyDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblExproxyApply&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblExproxyApply&allTableName=tblExproxyApplyDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=45329091_1302051645412180120&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=45329091_1302051645412180120&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblExproxyApplyDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=45329091_1302051645412180120&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=45329091_1302051645412180120&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblExproxyApply&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblExproxyApply&designId=tblExproxyApply_0001&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblExproxyApply&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblExproxyApply&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblExproxyApply&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '出口增值税(6%)开票申请';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblExproxyApply&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
