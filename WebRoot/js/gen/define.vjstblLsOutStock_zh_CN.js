...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblLsOutStock&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblLsOutStock&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblLsOutStock&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblLsOutStock&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblLsOutStockDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblLsOutStockDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblLsOutStockDet"){
...
function execStat2(){
 execStatOne('tblLsOutStockDet_Quantity',2);
 execStatOne('tblLsOutStockDet_Weight',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=29d399c2_1208101459269210104&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=29d399c2_1208101459269210104&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblLsOutStock\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblLsOutStock\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblLsOutStockDet_Weight',"tblLsOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblLsOutStockDet_BoxQty',"tblLsOutStockDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'TotalBoxQty',dis:'总箱数',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'GoodsValue',dis:'货物声明价值',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'YFee',dis:'运费',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ShFee',dis:'送货费',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'OtherFee',dis:'其他',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'SafeFee',dis:'保险',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalFee',dis:'小计',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'LowestAcc',dis:'最低结算价',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblLsOutStockDet_Quantity',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblLsOutStockDet_Weight',dis:'毛重',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblLsOutStock",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblLsOutStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblLsOutStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"29d399c2_1208101459269210104",
 MOOP:"add",
...
function resetSubmit(fieldName){
 if(fieldName == "TransportCom"){
 var saveField=n("TransportCom"); 
 saveField[0].value = "";
 ...
 var saveField=n("Contact"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyAddr_Contact");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("Phone"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyAddr_Phone");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("City"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyAddr_City");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("Address"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyAddr_Address");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("CompanyAddrID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompanyAddr_Code");
 if(obj != null){
 obj.value = "";
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=29d399c2_1208101459269210104&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=29d399c2_1208101459269210104&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblLsOutStock"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblLsOutStock&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblLsOutStock&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('TotalWeight')[0].value=f(sm('tblLsOutStockDet_Weight'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalBoxQty')[0].value=f(sm('tblLsOutStockDet_BoxQty'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalFee')[0].value=f(Number(nn('YFee',0))+Number(nn('ShFee',0))+Number(nn('OtherFee',0)),2);

n('LowestAcc')[0].value=f(nn('TotalFee',0)*1.2+nn('GoodsValue',0)*0.0008,2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblLsOutStockDet_Weight'); 
 n('TotalWeight')[0].value=f(sm('tblLsOutStockDet_Weight'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblLsOutStockDet_BoxQty'); 
 n('TotalBoxQty')[0].value=f(sm('tblLsOutStockDet_BoxQty'),2);
...
childData[childCount]='tblLsOutStockDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='国内货物出仓、派送通知单明细';
childCount++;
 var tblLsOutStockDetData = new gridData();
 gridDatas[gridDataNum] = tblLsOutStockDetData;
 gridDataNum++;
 var tblLsOutStockDet=1;
 ...
 addCols(tblLsOutStockDetData,'tblLsOutStockDet_Model','货物型号','120',2,true,'',100,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblLsOutStockDet_Model","货物型号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblLsOutStockDetData,'tblLsOutStockDet_GoodsName','品名','120',2,true,'',50,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblLsOutStockDet_GoodsName","品名","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblLsOutStockDetData,'tblLsOutStockDet_Quantity','数量','80',1,true,'',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblLsOutStockDet_Quantity","数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblLsOutStockDetData,'tblLsOutStockDet_Weight','毛重','80',1,true,'0',0,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblLsOutStockDet_Weight","毛重","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblLsOutStockDetData,'tblLsOutStockDet_BoxQty','箱数','80',0,true,'',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblLsOutStockDet_BoxQty","箱数","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblLsOutStockDetData,'tblLsOutStockDet_MNo','订单号/料号','220',2,true,'',100,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblLsOutStockDet_MNo","订单号/料号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblLsOutStockDetData,'tblLsOutStockDet_Remark','备注','200',2,true,'',8000,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblLsOutStockDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblLsOutStockDetData,'tblLsOutStockDet_SourceID','源ID','0',2,true,'',100,-2,'','',"","","12",'',"3",false); 
 putChildValidateItem("tblLsOutStockDet_SourceID","源ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblLsOutStockDetData,'tblLsOutStockDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblLsOutStock&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblLsOutStock,tblLsOutStockDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblLsOutStock&allTableName=tblLsOutStock,tblLsOutStockDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblLsOutStock&allTableName=tblLsOutStockDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblLsOutStock&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblLsOutStock&allTableName=tblLsOutStockDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=29d399c2_1208101459269210104&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=29d399c2_1208101459269210104&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=29d399c2_1208101459269210104&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=29d399c2_1208101459269210104&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblLsOutStock&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblLsOutStock&designId=21786bdc_1410131844355077670&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblLsOutStock&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblLsOutStock&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblLsOutStock&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '国内出仓派送通知单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblLsOutStock&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
