...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblIncome&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblIncome&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblIncome&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblIncome&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblIncomeDetlTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblIncomeDetl"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblIncomeDetl"){
...
function execStat2(){
 execStatOne('tblIncomeDetl_Amt',2);
 execStatOne('tblIncomeDetl_FcAmt',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblIncome\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblIncome\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblIncomeDetl_Amt',"tblIncomeDetl"); 
 delFireEventOne(rowId,tableName,'tblIncomeDetl_FcAmt',"tblIncomeDetl"); 
 }
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'FcBillAmt',dis:'外币合计金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblIncomeDetl_Amt',dis:'金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblIncomeDetl_FcAmt',dis:'外币金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblIncome",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblIncome&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblIncome&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblIncome"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblIncome&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblIncome&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
n('FcBillAmt')[0].value=f(sm('tblIncomeDetl_FcAmt'),2);

n('BillAmt')[0].value=f(sm('tblIncomeDetl_Amt'),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblIncomeDetl_Amt'); 
...
n('BillAmt')[0].value=f(sm('tblIncomeDetl_Amt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblIncomeDetl_FcAmt'); 
...
childData[childCount]='tblIncomeDetl';
tabIsNulls[childCount]='1';
tabDiss[childCount]='其他收入明细';
childCount++;
 var tblIncomeDetlData = new gridData();
 gridDatas[gridDataNum] = tblIncomeDetlData;
 gridDataNum++;
 var tblIncomeDetl=1;
 ...
 addCols(tblIncomeDetlData,'tblIncomeDetl_Remark','摘要','200',2,true,'',8000,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblIncomeDetl_Remark","摘要","any","",true,minValue,8000,false,'',true); 
 ...
 addCols(tblIncomeDetlData,'tblIncomeDetl_SubCode','收入类型','200',2,true,'',100,-2,'','',"","","4",'',"2",true);
 putChildValidateItem("tblIncomeDetl_SubCode","收入类型","any","",true,minValue,100,false,'');
 
 
 addCols(tblIncomeDetlData,'tblIncomeDetl_tblBillAccSetup_SubType','收入类型','200',2,true,'',1000000,2,'tblIncomeDetl:SubCode:tblIncomeDetl_tblBillAccSetup_SubType:'+param4,'',"","","4",'',"2",false); 
 
 
 
 
 addCols(tblIncomeDetlData,'tblIncomeDetl_Amt','金额','100',1,false,'',0,0,'','',"Amount","","11",'',"0",false); 
 putChildValidateItem("tblIncomeDetl_Amt","金额","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 
 addCols(tblIncomeDetlData,'tblIncomeDetl_FcAmt','外币金额','80',1,true,'',0,0,'','',"Amount","","13",'',"0",false); 
 putChildValidateItem("tblIncomeDetl_FcAmt","外币金额","double","currency",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblIncomeDetlData,'tblIncomeDetl_DepartmentCode','部门','80',2,true,'Sess_DepartmentCode',100,-2,'','',"","","19",'',"2",false); 
 putChildValidateItem("tblIncomeDetl_DepartmentCode","部门","any","",true,minValue,100,true,'Sess_DepartmentCode'); 
 ...
 addCols(tblIncomeDetlData,'tblIncomeDetl_EmployeeID','经手人','80',2,false,'Sess_UserId',100,-2,'','',"","","20",'',"2",true);
 putChildValidateItem("tblIncomeDetl_EmployeeID","经手人","any","",false,minValue,100,true,'Sess_UserId');
 
 
 addCols(tblIncomeDetlData,'tblIncomeDetl_tblEmployee_EmpFullName','经手人','50',2,true,'Sess_UserName',1000000,2,'tblIncomeDetl:EmployeeID:tblIncomeDetl_tblEmployee_EmpFullName:'+param4,'',"","","20",'',"2",false); 
 
 
 addCols(tblIncomeDetlData,'tblIncomeDetl_tblDepartment_DeptFullName','部门','75',2,true,'Sess_DepartmentName',1000000,2,'tblIncomeDetl:EmployeeID:tblIncomeDetl_tblDepartment_DeptFullName:'+param4,'',"","","20",'',"2",false); 
 ...
 addCols(tblIncomeDetlData,'tblIncomeDetl_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblIncome&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblIncome,tblIncomeDetl,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblIncome&allTableName=tblIncome,tblIncomeDetl,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblIncome&allTableName=tblIncomeDetl,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblIncome&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblIncome&allTableName=tblIncomeDetl,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblIncome&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblIncome&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblIncome&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblIncome&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblIncome&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '其他收入';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblIncome&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
