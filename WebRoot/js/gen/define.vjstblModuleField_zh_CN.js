...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblModuleField&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblModuleField&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblModuleField&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblModuleField&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblModuleFieldDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblModuleFieldDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblModuleFieldDet"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblModuleField\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblModuleField\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblModuleField",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblModuleField&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblModuleField&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 var saveField=n("moduleId"); 
 saveField[0].value = "";
 var obj = m("tbl_tblDBTableInfo_tableName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblDBTableInfo_languageId");
 saveField[0].value = ""; 
 m("tbl_tblDBTableInfo_languageId").value = "" ;
 }
 if(fieldName == "MenuPath"){
 var saveField=n("MenuPath"); 
 saveField[0].value = "";
 var obj = m("tbl_tblModules_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblModules_modelName");
 saveField[0].value = ""; 
 m("tbl_tblModules_modelName").value = "" ;
 }
 }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblModuleField"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblModuleField&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblModuleField&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblModuleFieldDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='模块字段设置明细';
childCount++;
 var tblModuleFieldDetData = new gridData();
 gridDatas[gridDataNum] = tblModuleFieldDetData;
 gridDataNum++;
 var tblModuleFieldDet=10;
 
 
 
 
 var param4="";
 param4=param4+":1数据表@moduleId"; 
 
 addCols(tblModuleFieldDetData,'tblModuleFieldDet_fieldName','字段名','200',2,false,'',100,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("tblModuleFieldDet_fieldName","字段名","any","",false,minValue,100,false,'');
 
 
 addCols(tblModuleFieldDetData,'tblModuleFieldDet_tblDBTableInfo_languageId','表名','200',2,true,'',1000000,2,'tblModuleFieldDet:fieldName:tblModuleFieldDet_tblDBTableInfo_languageId:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(tblModuleFieldDetData,'tblModuleFieldDet_tblDBFieldInfo_languageId','字段名','200',2,true,'',1000000,2,'tblModuleFieldDet:fieldName:tblModuleFieldDet_tblDBFieldInfo_languageId:'+param4,'',"","","0",'',"2",false); 
 ...
 addCols(tblModuleFieldDetData,'tblModuleFieldDet_tableName','表名','200',2,false,'',30,-2,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblModuleFieldDet_tableName","表名","any","",false,minValue,30,false,''); 
 
 
 
 addCols(tblModuleFieldDetData,'tblModuleFieldDet_fieldDisplay','显示名','224',2,true,'',500,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblModuleFieldDet_fieldDisplay","显示名","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblModuleFieldDetData,'tblModuleFieldDet_caculate','计算公式','200',2,true,'',8000,-2,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblModuleFieldDet_caculate","计算公式","any","",true,minValue,8000,false,''); 
 
 
 
 addCols(tblModuleFieldDetData,'tblModuleFieldDet_detOrderNo','序号','0',0,true,'',0,-2,'','',"","","4",'',"3",false); 
 putChildValidateItem("tblModuleFieldDet_detOrderNo","序号","int","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblModuleFieldDetData,'tblModuleFieldDet_readOnley','只读','60',2,true,'',10,5,';Yes:是','',"","","5",'',"5",false); 
 
 
 
 addCols(tblModuleFieldDetData,'tblModuleFieldDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblModuleField&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblModuleField,tblModuleFieldDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblModuleField&allTableName=tblModuleField,tblModuleFieldDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblModuleField&allTableName=tblModuleFieldDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblModuleField&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblModuleField&allTableName=tblModuleFieldDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblModuleFieldDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblModuleField&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblModuleField&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblModuleField&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblModuleField&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblModuleField&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '模块字段配置';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblModuleField&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
