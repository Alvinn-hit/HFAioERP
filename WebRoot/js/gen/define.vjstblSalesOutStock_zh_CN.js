...
{ 
 cFireEvent('tblSalesOutStockDet_Qty',''); 
 cFireEvent('tblSalesOutStockDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblSalesOutStock&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblSalesOutStock&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblSalesOutStock&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblSalesOutStock&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblSalesOutStockDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblSalesRecAccountTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblSalesOutStockDet_SalesOrderID');
 ...
 var z=n('tblSalesOutStockDet_GoodsCode');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblSalesOutStockDet_Hue');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblSalesOutStockDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblSalesOutStockDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblSalesOutStockDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblSalesOutStockDet_Seq');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblSalesOutStockDet"){
 
 copyFields=n('tblSalesOutStockDet_StockCode');
 ...
 if(typeof(tableName)!="undefined" && tableName=="tblSalesRecAccount"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblSalesOutStockDet"){
 ...
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblSalesRecAccount"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblSalesOutStockDet"; 
 var goodsCodeLogicValidate_tblSalesOutStockDet='@GN:Seq:Qty:tblSalesOutStockDet_GoodsSeqSet:tblSalesOutStockDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'tblSalesOutStockDet',value:goodsCodeLogicValidate_tblSalesOutStockDet};
 tblName[1]="tblSalesRecAccount"; 
...
function execStat2(){
 execStatOne('tblSalesOutStockDet_Qty',2);
 execStatOne('tblSalesOutStockDet_Amount',2);
 execStatOne('tblSalesOutStockDet_TaxAmount',2);
 execStatOne('tblSalesOutStockDet_CoTaxAmt',2);
 execStatOne('tblSalesOutStockDet_PresentAmt',2);
 execStatOne('tblSalesRecAccount_Amount',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblSalesOutStock\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblSalesOutStock\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblSalesOutStockDet_Qty',"tblSalesOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblSalesOutStockDet_Price',"tblSalesOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblSalesOutStockDet_Amount',"tblSalesOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblSalesOutStockDet_SecUnit',"tblSalesOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblSalesOutStockDet_UnitQty',"tblSalesOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblSalesOutStockDet_UnitPrice',"tblSalesOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblSalesOutStockDet_TaxPrice',"tblSalesOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblSalesOutStockDet_TaxAmount',"tblSalesOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblSalesOutStockDet_PresentAmt',"tblSalesOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblSalesOutStockDet_PresentPrice',"tblSalesOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblSalesRecAccount_Amount',"tblSalesRecAccount"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'DiscountAmount',dis:'优惠金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'Tax',dis:'税率%',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalAmount',dis:'单据总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalTaxAmount',dis:'优惠后金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalCoTaxAmt',dis:'总税额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblSalesOutStockDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalesOutStockDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalesOutStockDet_Amount',dis:'金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalesOutStockDet_UnitQty',dis:'结算数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalesOutStockDet_UnitPrice',dis:'结算单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalesOutStockDet_TaxPrice',dis:'含税单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalesOutStockDet_TaxAmount',dis:'含税金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalesOutStockDet_CoTaxAmt',dis:'税额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalesRecAccount_Amount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblSalesOutStock",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSalesOutStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSalesOutStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('tblSalesOutStockDet_Qty'); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblSalesOutStock"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblSalesOutStock&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblSalesOutStock&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
n('TotalAmount')[0].value=f(sm('tblSalesOutStockDet_DisBackAmt'),2);

n('TotalTaxAmount')[0].value=f(sm('tblSalesOutStockDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblSalesOutStockDet_CoTaxAmt'),2);

for(var j=0;j<r(0,'DisAmount').length;j++){
r(0,"DisAmount")[j].value=f(rn(0,"Amount",j)-rn(0,"DisBackAmt",j),2);
}

n('DisAmount')[0].value=f(sm('tblSalesOutStockDet_DisAmount'),2);
 ...
 n('CurTotalAmount')[0].value=f(sm('tblSalesOutStockDet_CurAmount')-nn('CurDiscountAmount',0),2);
...
n('DisAmount')[0].value=f(sm('tblSalesOutStockDet_DisAmount'),2);
 ...
 var z=n('CurProceDureExp');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('CurTotalAmount')[0].value=f(sm('tblSalesOutStockDet_CurAmount')-nn('CurDiscountAmount',0)+nn('CurProceDureExp',0),2);
 ...
 var z=n('deputyReceAmt');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('CurDeputyReceAmt')[0].value=f(nn('deputyReceAmt',0)/nn('CurrencyRate',0),2);

 
 }
 } 

 var z=n('CurDeputyReceAmt');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('deputyReceAmt')[0].value=f(nn('CurDeputyReceAmt',0)*nn('CurrencyRate',0),2);
...
n('CurTotalAmount')[0].value=f(sm('tblSalesOutStockDet_CurAmount')+nn('CurProceDureExp',0)-nn('CurDiscountAmount',0),2);
...
r(1,"Amount")[j].value=f(rn(1,"ExeBalFcAmt",j)*nn('CurrencyRate',0),2);
}

n('FactIncome')[0].value=f(sm('tblSalesRecAccount_Amount'),2);

n('AlrAccAmt')[0].value=f(sm('tblSalesRecAccount_Amount'),2);

n('AccAmt')[0].value=f(sm('tblSalesRecAccount_Amount'),2);

n('CurAlrAccAmt')[0].value=f(nn('AccAmt',0)/nn('CurrencyRate',0),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_Qty'); 
...
n('TotalTaxAmount')[0].value=f(sm('tblSalesOutStockDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(nn('TotalTaxAmount',0)/nn('CurrencyRate',0),2);
if(rn(0,"Rate",p)!=0)r(0,"UnitQty")[p].value=f(rn(0,"Qty",p)*rn(0,"Rate",p),2);
if(rn(0,"Rate",p)!=0)r(0,"UnitPrice")[p].value=f(rn(0,"Amount",p)/rn(0,"Qty",p),4);
...
r(0,"PresentAmt")[p].value=f(rn(0,"Qty",p)*rn(0,"PresentPrice",p),2);
n('PresentTotalAmt')[0].value=f(sm('tblSalesOutStockDet_PresentAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
if(rn(0,"Rate",p)!=0)r(0,"UnitPrice")[p].value=f(rn(0,"Price",p)*rn(0,"Rate",p),4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
n('TotalAmount')[0].value=f(sm('tblSalesOutStockDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblSalesOutStockDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('DisAmount')[0].value=f(sm('tblSalesOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_Amount'); 
...
n('DisAmount')[0].value=f(sm('tblSalesOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_SecUnit'); 
 if(rn(0,"Rate",p)!=0)r(0,"UnitQty")[p].value=f(rn(0,"Qty",p)*rn(0,"Rate",p),2);
if(rn(0,"Rate",p)!=0)r(0,"UnitPrice")[p].value=f(rn(0,"Price",p)*rn(0,"Rate",p),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_UnitQty'); 
 r(0,"Amount")[p].value=f(rn(0,"UnitQty",p)*rn(0,"UnitPrice",p),2);
r(0,"Price")[p].value=f(rn(0,"Amount",p)/rn(0,"Qty",p),4);
r(0,"DisBackAmt")[p].value=f((rn(0,"Discount",p)/100)*rn(0,"Price",p)*rn(0,"Qty",p),2);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
r(0,"TaxAmount")[p].value=f((rn(0,"Discount",p)/100)*rn(0,"Price",p)*rn(0,"Qty",p)*((nn('Tax',0)/100)+1),2);
n('TotalAmount')[0].value=f(sm('tblSalesOutStockDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblSalesOutStockDet_TaxAmount')-nn('DiscountAmount',0),2);
r(0,"CurAmount")[p].value=f((rn(0,"Discount",p)/100)*rn(0,"Price",p)*rn(0,"Qty",p)*((nn('Tax',0)/100)+1)/nn('CurrencyRate',0),2);
n('CurTotalAmount')[0].value=f(sm('tblSalesOutStockDet_CurAmount'),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
n('TotalCoTaxAmt')[0].value=f(sm('tblSalesOutStockDet_CoTaxAmt'),2);
n('DisAmount')[0].value=f(sm('tblSalesOutStockDet_DisAmount'),2);
r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_UnitPrice'); 
 r(0,"Amount")[p].value=f(rn(0,"UnitPrice",p)*rn(0,"UnitQty",p),2);
r(0,"Price")[p].value=f(rn(0,"Amount",p)/rn(0,"Qty",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Price",p)*(rn(0,"Discount",p)/100),4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*(rn(0,"Discount",p)/100),2);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
r(0,"TaxPrice")[p].value=f(rn(0,"Price",p)*(rn(0,"Discount",p)/100)*((nn('Tax',0)/100)+1),4);
r(0,"TaxAmount")[p].value=f(rn(0,"Amount",p)*(rn(0,"Discount",p)/100)*((nn('Tax',0)/100)+1),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
n('TotalCoTaxAmt')[0].value=f(sm('tblSalesOutStockDet_CoTaxAmt'),2);
n('TotalAmount')[0].value=f(sm('tblSalesOutStockDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblSalesOutStockDet_TaxAmount')-nn('DiscountAmount',0),2);
r(0,"CurPrice")[p].value=f(rn(0,"Price",p)*(rn(0,"Discount",p)/100)*((nn('Tax',0)/100)+1)/nn('CurrencyRate',0),4);
r(0,"CurAmount")[p].value=f(rn(0,"Amount",p)*(rn(0,"Discount",p)/100)*((nn('Tax',0)/100)+1)/nn('CurrencyRate',0),2);
n('CurTotalAmount')[0].value=f(sm('tblSalesOutStockDet_CurAmount'),2);
n('DisAmount')[0].value=f(sm('tblSalesOutStockDet_DisAmount'),2);
n('CurTotalAmount')[0].value=f(sm('tblSalesOutStockDet_CurAmount')-nn('CurDiscountAmount',0)+nn('CurProceDureExp',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,4);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('tblSalesOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_DisPrice'); 
...
n('DisAmount')[0].value=f(sm('tblSalesOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_DisBackAmt'); 
...
n('DisAmount')[0].value=f(sm('tblSalesOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_TaxPrice'); 
...
r(0,"UnitPrice")[p].value=f(rn(0,"Amount",p)/rn(0,"UnitQty",p),4);
...
n('DisAmount')[0].value=f(sm('tblSalesOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_TaxAmount'); 
...
n('DisAmount')[0].value=f(sm('tblSalesOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_CurPrice'); 
...
n('CurTotalAmount')[0].value=f(sm('tblSalesOutStockDet_CurAmount')-nn('CurDiscountAmount',0)+nn('CurProceDureExp',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_CurAmount'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),2);
...
n('CurTotalAmount')[0].value=f(sm('tblSalesOutStockDet_CurAmount')-nn('CurDiscountAmount',0)+nn('CurProceDureExp',0),2);
 ...
 var z=r(0,'PresentAmt');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_PresentAmt'); 
 r(0,"PresentPrice")[p].value=f(rn(0,"PresentAmt",p)/rn(0,"Qty",p),4);
n('PresentTotalAmt')[0].value=f(sm('tblSalesOutStockDet_PresentAmt'),2);

 execStat(); 
 }
 }
 var z=r(0,'PresentPrice');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_PresentPrice'); 
 r(0,"PresentAmt")[p].value=f(rn(0,"Qty",p)*rn(0,"PresentPrice",p),2);
n('PresentTotalAmt')[0].value=f(sm('tblSalesOutStockDet_PresentAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_CostAmount'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_CostPrice'); 
 r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesOutStockDet_PresentSampleType'); 
...
n('PresentTotalAmt')[0].value=f(sm('tblSalesOutStockDet_PresentAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesRecAccount_Amount'); 
 r(1,"ExeBalFcAmt")[p].value=f(rn(1,"Amount",p)/nn('CurrencyRate',0),2);
n('FactIncome')[0].value=f(sm('tblSalesRecAccount_Amount'),2);
...
n('CurAlrAccAmt')[0].value=f(nn('AccAmt',0)/nn('CurrencyRate',0),2);
 ...
 var z=r(1,'ExeBalFcAmt');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalesRecAccount_ExeBalFcAmt'); 
 r(1,"Amount")[p].value=f(rn(1,"ExeBalFcAmt",p)*nn('CurrencyRate',0),2);
n('FactIncome')[0].value=f(sm('tblSalesRecAccount_Amount'),2);
...
childData[childCount]='tblSalesOutStockDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='销售出库单明细';
childCount++;
 var tblSalesOutStockDetData = new gridData();
 gridDatas[gridDataNum] = tblSalesOutStockDetData;
 gridDataNum++;
 var tblSalesOutStockDet=1;
 ...
 var param4=""; 
 param4=param4+":1仓库@StockCode"; 
 param4=param4+":0仓库@tblSalesOutStockDet_StockCode";
 
 addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_tblGoods_GoodsNumber','商品编号','110',2,true,'',1000000,2,'tblSalesOutStockDet:GoodsCode:tblSalesOutStockDet_tblGoods_GoodsNumber:'+param4,"","","triggerCopy","2","","2",false);
 ...
 param4=param4+":1仓库@StockCode"; 
 param4=param4+":0仓库@tblSalesOutStockDet_StockCode";
 param4=param4+":1商品名称@tblSalesOutStockDet_GoodsCode";
 ...
 param4=param4+":0仓库@tblSalesOutStockDet_StockCode";
 param4=param4+":1仓库@StockCode"; 
 param4=param4+":0颜色@tblSalesOutStockDet_Hue";
 ...
 var param4=""; 
 param4=param4+":1单位@tblSalesOutStockDet_Unit";
 ...
 addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_tblGoods_GoodsFullName','商品名称','110',2,true,'',1000000,2,'tblSalesOutStockDet:GoodsCode:tblSalesOutStockDet_tblGoods_GoodsFullName:'+param4,"","","triggerCopy","2","","2",false);
 ...
 addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_tblGoods_BaseUnit','单位','40',2,true,'',1000000,2,'tblSalesOutStockDet:GoodsCode:tblSalesOutStockDet_tblGoods_BaseUnit:'+param4,"","","triggerCopy","2","","2",false);
 ...
 addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_tblGoodsColor_ColorName','颜色','70',2,true,'',1000000,2,'tblSalesOutStockDet:Hue:tblSalesOutStockDet_tblGoodsColor_ColorName:'+param4,"","","triggerCopy","4","","2",false);
 ...
 param4=param4+":1仓库@StockCode"; 
 param4=param4+":0颜色@tblSalesOutStockDet_Hue";
 addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_BatchNo','批/缸号','100',2,true,'',100,2,'tblSalesOutStockDet:BatchNo:tblSalesOutStockDet_BatchNo:'+param4,"","","triggerCopy","5",'',"2",true);
 putChildValidateItem("tblSalesOutStockDet_BatchNo","批/缸号","any","GoodsField",true,minValue,100,false,'',true);
        addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Pieces','匹数','50',0,false,'',0,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_Pieces","匹数","int","",false,minValue,maxValue,false,'',true); 
        addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Qty','数量','60',1,false,'',0,0,'','',"Qty","","11",'',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_Qty","数量","double","SeqQty",false,minValue,maxValue,false,'',true); 
        addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Price','单价','50',1,true,'',0,0,'','',"SPriceIdentifier","","12",'',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
        addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Amount','金额','60',1,true,'',0,0,'','',"SAmountIdentifier","","13",'',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_tblSecUnit_SecUnit','结算单位','50',2,true,'',1000000,2,'tblSalesOutStockDet:SecUnit:tblSalesOutStockDet_tblSecUnit_SecUnit:'+param4,"","","","14","","2",false);
 ...
 param4=param4+":1客户@CompanyCode"; 
        ...
        addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_UnitQty','结算数量','60',1,false,'',0,0,'','',"Qty","","15",'',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_UnitQty","结算数量","double","AssUnit",false,minValue,maxValue,false,'',true); 
        var param4="";
 param4=param4+":1商品名称@tblSalesOutStockDet_GoodsCode";
 param4=param4+":1客户@CompanyCode"; 
 addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_UnitPrice','结算单价','60',1,false,'',0,2,'tblSalesOutStockDet:UnitPrice:tblSalesOutStockDet_UnitPrice:'+param4,"","SPriceIdentifier","","16",'',"2",true);
 putChildValidateItem("tblSalesOutStockDet_UnitPrice","结算单价","double","AssUnit",false,minValue,maxValue,false,'',true);
        addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_QtyInputype','记数方式','50',2,false,'',10,1,';1:总数;2:细码','',"","","18",'',"1",false);
 putChildValidateItem("tblSalesOutStockDet_QtyInputype","记数方式","any","",false,minValue,10,false,'',true); 
        addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Remark','备注','120',2,true,'',500,0,'','',"","","65",'',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_Remark","备注","any","",true,minValue,500,false,'',true); 
        addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_CompanyGN','客户料号','100',2,true,'',50,0,'','',"","","67",'',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_CompanyGN","客户料号","any","",true,minValue,50,false,'',true); 
 ...
 param4=param4+":1客户@CompanyCode"; 
    ...
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_StockCode','仓库','80',2,true,'',100,-2,'','','',"copy",'','',"3",false); 
 putChildValidateItem("tblSalesOutStockDet_StockCode","仓库","any","",true,minValue,100,true,''); 
     addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_SalesOrderID','订单编号','80',2,true,'',32,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblSalesOutStockDet_SalesOrderID","订单编号","any","",true,minValue,32,false,''); 
 var param4="";
 addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_GoodsCode','商品名称','100',2,false,'',100,-2,'','','',"triggerCopy",'','',"2",false);
 putChildValidateItem("tblSalesOutStockDet_GoodsCode","商品名称","any","",false,minValue,100,false,'',true);
          addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_tblGoods_GoodsSpec','商品规格','100',2,false,'',100,-2,'',"2",false);
       addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_tblStocks_lastPieces','库存匹数','100',2,false,'',100,-2,'',"2",false);
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_tblStocks_lastQty','库存数','100',2,false,'',100,-2,'',"2",false);
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Unit','单位','50',2,false,'',50,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_Unit","单位","any","",false,minValue,50,false,''); 
 var param4="";
 addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Hue','颜色','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false);
 putChildValidateItem("tblSalesOutStockDet_Hue","颜色","any","",true,minValue,100,false,'',true);
           addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Inch','匹号','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"3",false); 
 putChildValidateItem("tblSalesOutStockDet_Inch","匹号","any","",true,minValue,100,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_yearNO','尺码','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblSalesOutStockDet_yearNO","尺码","any","",true,minValue,100,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_ProDate','生产日期','80',5,true,'',10,-2,'','','',"",'','',"2",false); 
 putChildValidateItem("tblSalesOutStockDet_ProDate","生产日期","date","",true,minValue,10,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Availably','保质期限','80',5,true,'',10,-2,'','','',"",'','',"2",false); 
 putChildValidateItem("tblSalesOutStockDet_Availably","保质期限","date","",true,minValue,10,false,''); 
               var param4="";
 addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_SecUnit','结算单位','60',2,false,'',30,-2,'','','',"",'','',"2",false);
 putChildValidateItem("tblSalesOutStockDet_SecUnit","结算单位","any","",false,minValue,30,false,'',true);
               addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Rate','换算率','60',1,true,'',0,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_Rate","换算率","double","",true,minValue,maxValue,false,''); 
        addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Discount','折扣率%','70',1,true,'100',0,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_Discount","折扣率%","double","",true,minValue,maxValue,true,'100'); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_DisPrice','折后单价','70',1,true,'',0,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_DisPrice","折后单价","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_DisBackAmt','折后金额','70',1,true,'',0,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_DisBackAmt","折后金额","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_DisAmount','折让金额','70',1,true,'',0,-2,'','','',"",'','',"8",false); 
 putChildValidateItem("tblSalesOutStockDet_DisAmount","折让金额","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_TaxPrice','含税单价','70',1,true,'',0,-2,'','',"SPriceIdentifier","","23",'',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_TaxPrice","含税单价","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_TaxAmount','含税金额','70',1,true,'',0,-2,'','',"SAmountIdentifier","","24",'',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_TaxAmount","含税金额","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_CoTaxAmt','税额','70',1,true,'0',0,-2,'','',"SAmountIdentifier","","25",'',"8",false); 
 putChildValidateItem("tblSalesOutStockDet_CoTaxAmt","税额","double","",true,minValue,maxValue,true,'0'); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_CurPrice','外币单价','70',1,true,'',0,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_CurPrice","外币单价","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_CurAmount','外币金额','70',1,true,'',0,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_CurAmount","外币金额","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_BuyQty','入库数量','70',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblSalesOutStockDet_BuyQty","入库数量","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_NotBuyQty','未入库数量','70',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblSalesOutStockDet_NotBuyQty","未入库数量","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_CheckBarCode','复核码','70',2,true,'',200,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_CheckBarCode","复核码","any","",true,minValue,200,false,''); 
     addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_PresentAmt','赠(样)品金额','70',1,true,'0',999999999,-2,'','',"AmountIdentifier","","33",'',"6",false); 
 putChildValidateItem("tblSalesOutStockDet_PresentAmt","赠(样)品金额","double","",true,minValue,999999999,true,'0'); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_PresentPrice','赠(样)品单价','70',1,true,'0',999999999,-2,'','',"priceIdentifier","","34",'',"6",false); 
 putChildValidateItem("tblSalesOutStockDet_PresentPrice","赠(样)品单价","double","",true,minValue,999999999,true,'0'); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_ViewTotalQty','当前库存','70',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblSalesOutStockDet_ViewTotalQty","当前库存","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_lastQty','实际库存','70',1,true,'',0,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_lastQty","实际库存","double","",true,minValue,maxValue,false,''); 
     addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_CostAmount','成本总额','60',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblSalesOutStockDet_CostAmount","成本总额","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_CostPrice','成本单价','60',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblSalesOutStockDet_CostPrice","成本单价","double","",true,minValue,maxValue,false,''); 
     addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_SourceID','源ID','0',2,true,'',60,-2,'','','',"",'','',"100",false); 
 putChildValidateItem("tblSalesOutStockDet_SourceID","源ID","any","",true,minValue,60,false,''); 
     addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_NoInvoiceQty','未开票数','70',1,true,'0',0,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblSalesOutStockDet_NoInvoiceQty","未开票数","double","",true,minValue,maxValue,true,'0'); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_PresentSampleType','赠(样)品','70',2,true,'',10,-2,'','','',"",'','',"1",false); 
 putChildValidateItem("tblSalesOutStockDet_PresentSampleType","赠(样)品","any","",true,minValue,10,false,''); 
     addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_BackPayQty','退货付款数','70',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblSalesOutStockDet_BackPayQty","退货付款数","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_PayQty','付款数量','70',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblSalesOutStockDet_PayQty","付款数量","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_NotPayQty','未付款数量','70',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblSalesOutStockDet_NotPayQty","未付款数量","double","",true,minValue,maxValue,false,''); 
    addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_AfterChangeAmt','調價后金額','70',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblSalesOutStockDet_AfterChangeAmt","調價后金額","double","",true,minValue,maxValue,false,''); 
           addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_Seq','序列号','130',2,true,'',500,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblSalesOutStockDet_Seq","序列号","any","",true,minValue,500,false,''); 
 
 addCols(tblSalesOutStockDetData,'tblSalesOutStockDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblSalesRecAccount';
tabIsNulls[childCount]='0';
tabDiss[childCount]='销售单收款账户明细表';
childCount++;
 var tblSalesRecAccountData = new gridData();
 gridDatas[gridDataNum] = tblSalesRecAccountData;
 gridDataNum++;
 var tblSalesRecAccount=1;
  addCols(tblSalesRecAccountData,'tblSalesRecAccount_SettleType','收款方式','80',2,true,'1',100,1,';1:现金;2:支票;3:电汇;4:商业汇票;5:银行汇票','',"","","1",'',"1",false);
 putChildValidateItem("tblSalesRecAccount_SettleType","收款方式","any","",true,minValue,100,true,'1',true); 
 ...
 addCols(tblSalesRecAccountData,'tblSalesRecAccount_tblAccTypeInfo_AccName','账户','150',2,true,'',1000000,2,'tblSalesRecAccount:Account:tblSalesRecAccount_tblAccTypeInfo_AccName:'+param4,"","","","2","","2",false);
  ...
  addCols(tblSalesRecAccountData,'tblSalesRecAccount_Amount','金额','100',1,false,'0',0,0,'','',"Amount","","3",'',"0",false); 
 putChildValidateItem("tblSalesRecAccount_Amount","金额","double","RowMarker",false,minValue,maxValue,true,'0',true); 
  addCols(tblSalesRecAccountData,'tblSalesRecAccount_Remark','账户备注','150',2,true,'',8000,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblSalesRecAccount_Remark","账户备注","any","",true,minValue,8000,false,'',true); 
   var param4="";
 addCols(tblSalesRecAccountData,'tblSalesRecAccount_Account','账户','150',2,false,'Sess_Account',100,-2,'','','',"",'','',"2",false);
 putChildValidateItem("tblSalesRecAccount_Account","账户","any","",false,minValue,100,true,'Sess_Account',true);
    addCols(tblSalesRecAccountData,'tblSalesRecAccount_ExeBalFcAmt','外币结算金额','80',1,true,'',0,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblSalesRecAccount_ExeBalFcAmt","外币结算金额","double","",true,minValue,maxValue,false,''); 
  
 addCols(tblSalesRecAccountData,'tblSalesRecAccount_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblSalesOutStock&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblSalesOutStock,tblSalesOutStockDet,tblSalesRecAccount,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSalesOutStock&allTableName=tblSalesOutStock,tblSalesOutStockDet,tblSalesRecAccount,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSalesOutStock&allTableName=tblSalesOutStockDet,tblSalesRecAccount,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSalesOutStock&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSalesOutStock&allTableName=tblSalesOutStockDet,tblSalesRecAccount,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblSalesOutStockDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblSalesOutStock&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblSalesOutStock&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblSalesOutStock&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblSalesOutStock&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblSalesOutStock&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '销售出库';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblSalesOutStock&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
