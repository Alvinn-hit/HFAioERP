...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblMaterialPlan&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblMaterialPlan&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblMaterialPlan&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblMaterialPlan&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblMaterialPlanDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblMaterialPlanDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblMaterialPlanDet"){
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=95aa4689_1404281347038120074&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=95aa4689_1404281347038120074&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblMaterialPlan\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblMaterialPlan\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblMaterialPlanDet_InitAmount',"tblMaterialPlanDet"); 
 delFireEventOne(rowId,tableName,'tblMaterialPlanDet_Wastagerate',"tblMaterialPlanDet"); 
 delFireEventOne(rowId,tableName,'tblMaterialPlanDet_InitPurchaseAmount',"tblMaterialPlanDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'InitCost',dis:'计划总价',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'RealCost',dis:'实际总价',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblMaterialPlanDet_ZBPrice',dis:'材料中标价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblMaterialPlanDet_InitAmount',dis:'清单工程量',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblMaterialPlanDet_Wastagerate',dis:'1+预估损耗率',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblMaterialPlanDet_InitPurchaseAmount',dis:'初始计划量',digit:6,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblMaterialPlan",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblMaterialPlan&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblMaterialPlan&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"95aa4689_1404281347038120074",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=95aa4689_1404281347038120074&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=95aa4689_1404281347038120074&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=95aa4689_1404281347038120074&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblMaterialPlan"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblMaterialPlan&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblMaterialPlan&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 var z=r(0,'InitAmount');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblMaterialPlanDet_InitAmount'); 
 r(0,"InitPurchaseAmount")[p].value=f(rn(0,"InitAmount",p)*rn(0,"Wastagerate",p),6);
r(0,"RealPurchase")[p].value=f(rn(0,"InitAmount",p)+rn(0,"ChangeAmount",p)+rn(0,"CalculateLoss",p)+rn(0,"MeasureLoss",p),2);
r(0,"PlanPurchase")[p].value=f(rn(0,"RealPurchase",p)*rn(0,"Wastagerate",p),2);
r(0,"RealWastageRate")[p].value=f((rn(0,"InstoreAmount",p)-rn(0,"RealPurchase",p))/rn(0,"RealPurchase",p),2);
r(0,"PurchaseBiasRate")[p].value=f(rn(0,"RealWastageRate",p)-rn(0,"Wastagerate",p),2);

 execStat(); 
 }
 }
 var z=r(0,'Wastagerate');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblMaterialPlanDet_Wastagerate'); 
 r(0,"InitPurchaseAmount")[p].value=f(rn(0,"InitAmount",p)*rn(0,"Wastagerate",p),6);
r(0,"RealPurchase")[p].value=f(rn(0,"InitAmount",p)+rn(0,"ChangeAmount",p)+rn(0,"CalculateLoss",p)+rn(0,"MeasureLoss",p),2);
r(0,"PlanPurchase")[p].value=f(rn(0,"RealPurchase",p)*rn(0,"Wastagerate",p),2);
r(0,"RealWastageRate")[p].value=f((rn(0,"InstoreAmount",p)-rn(0,"RealPurchase",p))/rn(0,"RealPurchase",p),2);
r(0,"PurchaseBiasRate")[p].value=f(rn(0,"RealWastageRate",p)-rn(0,"Wastagerate",p),2);
 ...
 var z=r(0,'InitPurchaseAmount');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblMaterialPlanDet_InitPurchaseAmount'); 
 r(0,"InitPurchaseAmount")[p].value=f(rn(0,"InitAmount",p)*rn(0,"Wastagerate",p),6);
r(0,"RealPurchase")[p].value=f(rn(0,"InitAmount",p)+rn(0,"ChangeAmount",p)+rn(0,"CalculateLoss",p)+rn(0,"MeasureLoss",p),2);
r(0,"PlanPurchase")[p].value=f(rn(0,"RealPurchase",p)*rn(0,"Wastagerate",p),2);
r(0,"RealWastageRate")[p].value=f((rn(0,"InstoreAmount",p)-rn(0,"RealPurchase",p))/rn(0,"RealPurchase",p),2);
...
childData[childCount]='tblMaterialPlanDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='材料总计划明细表';
childCount++;
 var tblMaterialPlanDetData = new gridData();
 gridDatas[gridDataNum] = tblMaterialPlanDetData;
 gridDataNum++;
 var tblMaterialPlanDet=1;
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_BillNo','材料编码','100',2,false,'',20,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblMaterialPlanDet_BillNo","材料编码","any","",false,minValue,20,false,'',true); 
 ...
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_Name','材料名称','150',2,false,'',100,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblMaterialPlanDet_Name","材料名称","any","",false,minValue,100,false,'',true); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_Specification','规格/型号','100',2,false,'',100,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblMaterialPlanDet_Specification","规格/型号","any","",false,minValue,100,false,'',true); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_Brand','品牌','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblMaterialPlanDet_Brand","品牌","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_Unit','计量单位','100',2,true,'',100,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblMaterialPlanDet_Unit","计量单位","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_Type','材料类型','70',2,false,'main',100,1,';main:主材;attach:辅材;other:耗材','',"","","5",'',"1",false);
 putChildValidateItem("tblMaterialPlanDet_Type","材料类型","any","",false,minValue,100,true,'main',true); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_ZBPrice','材料中标价','100',1,true,'0',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblMaterialPlanDet_ZBPrice","材料中标价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_IsSpecial','是否有特殊需求','100',2,true,'2',100,1,';1:是;2:否','',"","","7",'',"1",false);
 putChildValidateItem("tblMaterialPlanDet_IsSpecial","是否有特殊需求","any","",true,minValue,100,true,'2',true); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_InitAmount','清单工程量','100',1,true,'0',0,0,'','',"Qty","","8",'',"0",false); 
 putChildValidateItem("tblMaterialPlanDet_InitAmount","清单工程量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_Wastagerate','1+预估损耗率','100',1,true,'0',0,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblMaterialPlanDet_Wastagerate","1+预估损耗率","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_InitPurchaseAmount','初始计划量','100',1,true,'0',0,0,'','',"Qty","","10",'',"0",false); 
 putChildValidateItem("tblMaterialPlanDet_InitPurchaseAmount","初始计划量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_ChangeAmount','变更量','100',1,true,'0',0,-2,'','',"","","11",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_ChangeAmount","变更量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_CalculateLoss','少(多)算量','100',1,true,'0',0,-2,'','',"","","12",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_CalculateLoss","少(多)算量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_MeasureLoss','漏项工程量','100',1,true,'0',0,-2,'','',"","","13",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_MeasureLoss","漏项工程量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_RealPurchase','实际工程量','100',1,true,'0',0,-2,'','',"","","14",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_RealPurchase","实际工程量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_RealPurchaseCost','实际工程总价','100',1,true,'0',0,-2,'','',"","","15",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_RealPurchaseCost","实际工程总价","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_PlanPurchase','计划采购量','100',1,true,'0',0,-2,'','',"","","16",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_PlanPurchase","计划采购量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_Cutting','已下料数量','100',1,true,'0',0,-2,'','',"","","17",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_Cutting","已下料数量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_PlanCost','计划采购总价','100',1,true,'0',0,-2,'','',"","","18",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_PlanCost","计划采购总价","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_Purchase','已下订单数量','100',1,true,'0',0,-2,'','',"","","19",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_Purchase","已下订单数量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_PurchaseCost','已下订单总价','100',1,true,'0',0,-2,'','',"","","20",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_PurchaseCost","已下订单总价","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_InstoreAmount','实际采购量','100',1,true,'0',0,-2,'','',"","","21",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_InstoreAmount","实际采购量","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_InstoreCost','实际采购总价','100',1,true,'0',0,-2,'','',"","","22",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_InstoreCost","实际采购总价","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_RealWastageRate','实际损耗率','100',1,true,'0',0,-2,'','',"","","23",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_RealWastageRate","实际损耗率","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_PurchaseBiasRate','采购偏差率','100',1,true,'0',0,-2,'','',"","","24",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_PurchaseBiasRate","采购偏差率","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_Price','单价','100',1,true,'0',0,-2,'','',"","","25",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_Price","单价","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_Supplier','供应商','100',2,true,'',100,-2,'','',"","","26",'',"3",false); 
 putChildValidateItem("tblMaterialPlanDet_Supplier","供应商","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_isExt','计划外材料','80',2,true,'false',50,-2,'','',"","","27",'',"2",false); 
 putChildValidateItem("tblMaterialPlanDet_isExt","计划外材料","any","",true,minValue,50,true,'false'); 
 
 
 
 
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_Position','使用位置','150',2,true,'',200,0,'','',"","","29",'',"0",false); 
 putChildValidateItem("tblMaterialPlanDet_Position","使用位置","any","",true,minValue,200,false,'',true); 
 ...
 addCols(tblMaterialPlanDetData,'tblMaterialPlanDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblMaterialPlan&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblMaterialPlan,tblMaterialPlanDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblMaterialPlan&allTableName=tblMaterialPlan,tblMaterialPlanDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblMaterialPlan&allTableName=tblMaterialPlanDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblMaterialPlan&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblMaterialPlan&allTableName=tblMaterialPlanDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=95aa4689_1404281347038120074&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=95aa4689_1404281347038120074&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblMaterialPlanDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=95aa4689_1404281347038120074&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=95aa4689_1404281347038120074&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblMaterialPlan&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblMaterialPlan&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblMaterialPlan&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblMaterialPlan&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblMaterialPlan&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '材料总计划';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblMaterialPlan&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
