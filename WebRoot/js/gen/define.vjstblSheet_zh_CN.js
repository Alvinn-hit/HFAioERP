...
{ 
 cFireEvent('tblSheetDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblSheet&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblSheet&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblSheet&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblSheet&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblSheetDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblSheetDetInvoiceTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblSheetDet"){
 
  }
 if(typeof(tableName)!="undefined" && tableName=="tblSheetDetInvoice"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblSheetDet"){
 
  }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblSheetDetInvoice"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblSheetDet"; 
 tblName[1]="tblSheetDetInvoice"; 
...
function execStat2(){
 execStatOne('tblSheetDet_Quantity',2);
 execStatOne('tblSheetDet_AmountY',2);
 execStatOne('tblSheetDet_Amount',2);
 execStatOne('tblSheetDet_ConAmount',2);
 execStatOne('tblSheetDet_NetWeight',2);
 execStatOne('tblSheetDet_Weight',2);
 execStatOne('tblSheetDet_BoxQty',2);
 setTotalCount(getTotalCount());
}
...
function execDetailStat2(){
 execDetStatOne('tblSheetDet_Quantity',2); 
 execDetStatOne('tblSheetDet_AmountY',2); 
 execDetStatOne('tblSheetDet_Amount',2); 
 execDetStatOne('tblSheetDet_ConAmount',2); 
 execDetStatOne('tblSheetDet_NetWeight',2); 
 execDetStatOne('tblSheetDet_Weight',2); 
 execDetStatOne('tblSheetDet_BoxQty',2); 
 }
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=f8a843bd_1207261908194530464&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=f8a843bd_1207261908194530464&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblSheet\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblSheet\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblSheetDet_Quantity',"tblSheetDet"); 
 delFireEventOne(rowId,tableName,'tblSheetDet_AmountY',"tblSheetDet"); 
 delFireEventOne(rowId,tableName,'tblSheetDet_Price',"tblSheetDet"); 
 delFireEventOne(rowId,tableName,'tblSheetDet_YBPrice',"tblSheetDet"); 
 delFireEventOne(rowId,tableName,'tblSheetDet_Amount',"tblSheetDet"); 
 delFireEventOne(rowId,tableName,'tblSheetDet_NetWeight',"tblSheetDet"); 
 delFireEventOne(rowId,tableName,'tblSheetDet_Weight',"tblSheetDet"); 
 delFireEventOne(rowId,tableName,'tblSheetDet_BoxQty',"tblSheetDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'Fare',dis:'运费',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'Insurance',dis:'保险费',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'OtherFare',dis:'杂费',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'YBRate',dis:'运保费率',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalAmount',dis:'总金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblSheetDet_Quantity',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSheetDet_AmountY',dis:'原金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSheetDet_Price',dis:'原单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSheetDet_YBPrice',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSheetDet_Amount',dis:'金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSheetDet_NetWeight',dis:'净重',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSheetDet_Weight',dis:'毛重',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblSheet",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSheet&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSheet&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'打印控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"f8a843bd_1207261908194530464",
 MOOP:"add",
...
function resetSubmit(fieldName){
 if(fieldName == "Buyer"){
 var saveField=n("Buyer"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BuyerEn"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSNameEn");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BuyerAddr"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSAddr");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BuyerAddrEn"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSAddrEn");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BuyerTel"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSTel");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BuyerFax"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSFax");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BuyerBankName"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BankName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BuyerCreateAddr"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_CreateAddr");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BuyerBankID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BankID");
 if(obj != null){
 obj.value = "";
 }
 }
 if(fieldName == "Seller"){
 var saveField=n("Seller"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SellerEn"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSNameEn");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SellerAddr"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSAddr");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SellerAddrEn"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSAddrEn");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SellerTel"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSTel");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SellerFax"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BSFax");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SellerBankName"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BankName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SellerCreateAddr"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_CreateAddr");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SellerBankID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BankID");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SellerBankID2"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BankID2");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SellerBankID3"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BankID3");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SellerBankID4"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsBSer_BankID4");
 if(obj != null){
 obj.value = "";
 }
 }
 if(fieldName == "ShipTo"){
 var saveField=n("ShipTo"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_ComFullName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("RiskGrade"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_RiskGrade");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("ZHLowestFee"); 
 saveField[0].value = "";
 var obj = m("tbl_tblImpAgreement_ZHLowestFee");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("KFollowID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_KFollowID");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("KFollow"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_KFollow");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("YFollowID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_YFollowID");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("YFollow"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_YFollow");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("ShipToID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblLSCompany_ContentOrgCode");
 saveField[0].value = ""; 
 m("tbl_tblLSCompany_ContentOrgCode").value = "" ;
 var saveField=n("tblLSCompany_ImportExCode");
 saveField[0].value = ""; 
 m("tbl_tblLSCompany_ImportExCode").value = "" ;
 }
 if(fieldName == "TruckNoListID"){
 var saveField=n("TruckNoListID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblTruckNoList_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblTruckNoList_TruckID");
 saveField[0].value = ""; 
 m("tbl_tblTruckNoList_TruckID").value = "" ;
 var saveField=n("tblTruckNoList_ListNo");
 saveField[0].value = ""; 
 m("tbl_tblTruckNoList_ListNo").value = "" ;
 }
 if(fieldName == "BJSupplyNo"){
 var saveField=n("BJSupplyNo"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsSupplyGW_SupplyNo");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BJSupplyName"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsSupplyGW_SupplyName");
 if(obj != null){
 obj.value = "";
 }
 }
 if(fieldName == "BGSupplyNo"){
 var saveField=n("BGSupplyNo"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsSupplyGW_SupplyNo");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BGSupplyName"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLsSupplyGW_SupplyName");
 if(obj != null){
 obj.value = "";
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f8a843bd_1207261908194530464&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f8a843bd_1207261908194530464&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=f8a843bd_1207261908194530464&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblSheet&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblSheet&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
r(0,"Amount")[j].value=f(rn(0,"AmountY",j)*nn('YBRate',0),2);
}

for(var j=0;j<r(0,'YBPrice').length;j++){
r(0,"YBPrice")[j].value=f(rn(0,"Amount",j)/rn(0,"Quantity",j),4);
}

for(var j=0;j<r(0,'Price').length;j++){
r(0,"Price")[j].value=f(rn(0,"AmountY",j)/rn(0,"Quantity",j),4);
}

n('TotalAmount')[0].value=f(sm('tblSheetDet_Amount'),2);
...
n('TotalAmount')[0].value=f(sm('tblSheetDet_Amount'),2);
 ...
 var z=n('TotalNetWeight');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalNetWeight')[0].value=f(sm('tblSheetDet_NetWeight'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalWeight')[0].value=f(sm('tblSheetDet_Weight'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalBoxQty')[0].value=f(sm('tblSheetDet_BoxQty'),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSheetDet_Quantity'); 
 r(0,"Amount")[p].value=f(rn(0,"AmountY",p)*nn('YBRate',0),2);
r(0,"YBPrice")[p].value=f(rn(0,"Amount",p)/rn(0,"Quantity",p),4);
r(0,"Price")[p].value=f(rn(0,"AmountY",p)/rn(0,"Quantity",p),4);
n('TotalAmount')[0].value=f(sm('tblSheetDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSheetDet_AmountY'); 
 r(0,"Amount")[p].value=f(rn(0,"AmountY",p)*nn('YBRate',0),2);
r(0,"YBPrice")[p].value=f(rn(0,"Amount",p)/rn(0,"Quantity",p),4);
r(0,"Price")[p].value=f(rn(0,"AmountY",p)/rn(0,"Quantity",p),4);
n('TotalAmount')[0].value=f(sm('tblSheetDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSheetDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"AmountY",p)*nn('YBRate',0),2);
r(0,"YBPrice")[p].value=f(rn(0,"Amount",p)/rn(0,"Quantity",p),4);
r(0,"Price")[p].value=f(rn(0,"AmountY",p)/rn(0,"Quantity",p),4);
n('TotalAmount')[0].value=f(sm('tblSheetDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSheetDet_YBPrice'); 
 r(0,"Amount")[p].value=f(rn(0,"AmountY",p)*nn('YBRate',0),2);
r(0,"YBPrice")[p].value=f(rn(0,"Amount",p)/rn(0,"Quantity",p),4);
r(0,"Price")[p].value=f(rn(0,"AmountY",p)/rn(0,"Quantity",p),4);
n('TotalAmount')[0].value=f(sm('tblSheetDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSheetDet_Amount'); 
 r(0,"Amount")[p].value=f(rn(0,"AmountY",p)*nn('YBRate',0),2);
r(0,"YBPrice")[p].value=f(rn(0,"Amount",p)/rn(0,"Quantity",p),4);
r(0,"Price")[p].value=f(rn(0,"AmountY",p)/rn(0,"Quantity",p),4);
n('TotalAmount')[0].value=f(sm('tblSheetDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSheetDet_NetWeight'); 
 n('TotalNetWeight')[0].value=f(sm('tblSheetDet_NetWeight'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSheetDet_Weight'); 
 n('TotalWeight')[0].value=f(sm('tblSheetDet_Weight'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSheetDet_BoxQty'); 
 n('TotalBoxQty')[0].value=f(sm('tblSheetDet_BoxQty'),2);
...
childData[childCount]='tblSheetDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='进口制单明细';
childCount++;
 var tblSheetDetData = new gridData();
 gridDatas[gridDataNum] = tblSheetDetData;
 gridDataNum++;
 var tblSheetDet=2;
 ...
 addCols(tblSheetDetData,'tblSheetDet_Item','Item','30',0,true,'',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblSheetDet_Item","Item","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_LsGoodsID','商品','0',2,true,'',30,-2,'','',"","","4",'',"3",false); 
 putChildValidateItem("tblSheetDet_LsGoodsID","商品","any","",true,minValue,30,false,''); 
 ...
 addCols(tblSheetDetData,'tblSheetDet_Model','型号','150',2,true,'',100,2,'tblSheetDet:Model:tblSheetDet_Model:'+param4,"","","","6",'',"2",true);
 putChildValidateItem("tblSheetDet_Model","型号","any","",true,minValue,100,false,'',true);
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_Brand','品牌','80',2,true,'',50,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblSheetDet_Brand","品牌","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_GoodsName','品名','100',2,true,'',50,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblSheetDet_GoodsName","品名","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_Unit','单位','30',2,true,'',30,0,'','',"","","17",'',"0",false); 
 putChildValidateItem("tblSheetDet_Unit","单位","any","",true,minValue,30,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_Quantity','数量','80',1,true,'',0,0,'','',"Qty","","18",'',"0",false); 
 putChildValidateItem("tblSheetDet_Quantity","数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_QuantityLast','出仓可派送数量','224',1,true,'',0,-2,'','',"","","18",'',"3",false); 
 putChildValidateItem("tblSheetDet_QuantityLast","出仓可派送数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_AmountY','原金额','70',1,true,'',0,0,'','',"","","19",'',"0",false); 
 putChildValidateItem("tblSheetDet_AmountY","原金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_Price','原单价','60',1,true,'',0,8,'','',"","","20",'',"8",false); 
 putChildValidateItem("tblSheetDet_Price","原单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_YBPrice','单价','60',1,true,'',0,8,'','',"","","21",'',"8",false); 
 putChildValidateItem("tblSheetDet_YBPrice","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_Amount','金额','65',1,true,'',0,8,'','',"Amount","","22",'',"8",false); 
 putChildValidateItem("tblSheetDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_ConAmount','缴税金额','100',1,true,'',0,-2,'','',"Amount","","23",'',"6",false); 
 putChildValidateItem("tblSheetDet_ConAmount","缴税金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_GoodsNumber','商品编码','100',2,false,'',10,0,'','',"","","23",'',"0",false); 
 putChildValidateItem("tblSheetDet_GoodsNumber","商品编码","any","",false,minValue,10,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_Spec','规格','350',2,true,'',500,0,'','',"","","25",'',"0",false); 
 putChildValidateItem("tblSheetDet_Spec","规格","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_BoxNo','No.','30',2,true,'',10,0,'','',"","","27",'',"0",false); 
 putChildValidateItem("tblSheetDet_BoxNo","No.","any","",true,minValue,10,false,'',true); 
 ...
 addCols(tblSheetDetData,'tblSheetDet_GoodsPlace','产地','50',2,true,'',50,-2,'','',"","","29",'',"2",true);
 putChildValidateItem("tblSheetDet_GoodsPlace","产地","any","",true,minValue,50,false,'');
 addCols(tblSheetDetData,'tblSheetDet_tblCountry_Country','产地','50',2,true,'',1000000,2,'tblSheetDet:GoodsPlace:tblSheetDet_tblCountry_Country:'+param4,'',"","","29",'',"2",false); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_NetWeight','净重','50',1,true,'',0,0,'','',"","","31",'',"0",false); 
 putChildValidateItem("tblSheetDet_NetWeight","净重","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_Weight','毛重','50',1,true,'',0,0,'','',"","","33",'',"0",false); 
 putChildValidateItem("tblSheetDet_Weight","毛重","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_BoxQty','件数','30',0,true,'',0,0,'','',"","","35",'',"0",false); 
 putChildValidateItem("tblSheetDet_BoxQty","件数","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_Remark','备注','50',2,true,'',8000,0,'','',"","","37",'',"0",false); 
 putChildValidateItem("tblSheetDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_CustomsCond','监管条件','50',2,true,'',20,0,'','',"","","39",'',"0",false); 
 putChildValidateItem("tblSheetDet_CustomsCond","监管条件","any","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_GoodStatus','状态','50',2,true,'',100,8,'','',"","","40",'',"8",false); 
 putChildValidateItem("tblSheetDet_GoodStatus","状态","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_GoodStatus2','状态2','50',2,true,'',100,8,'','',"","","41",'',"8",false); 
 putChildValidateItem("tblSheetDet_GoodStatus2","状态2","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_TariffRate','关税税率','100',1,true,'',0,-2,'','',"","","45",'',"3",false); 
 putChildValidateItem("tblSheetDet_TariffRate","关税税率","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_ConRate','消费税率','100',1,true,'',0,-2,'','',"","","46",'',"3",false); 
 putChildValidateItem("tblSheetDet_ConRate","消费税率","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_VatRate','增值税率','100',1,true,'',0,-2,'','',"","","47",'',"3",false); 
 putChildValidateItem("tblSheetDet_VatRate","增值税率","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_Currency','明细币种','224',2,true,'',10,-2,'','',"","","50",'',"3",false); 
 putChildValidateItem("tblSheetDet_Currency","明细币种","any","",true,minValue,10,false,''); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_Code','明细关联ID','224',2,true,'',50,-2,'','',"","","50",'',"3",false); 
 putChildValidateItem("tblSheetDet_Code","明细关联ID","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_OutStockForCheck','已派送国内出仓通知','100',2,true,'',8000,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblSheetDet_OutStockForCheck","已派送国内出仓通知","any","",true,minValue,8000,false,''); 
 
 
 
 
 addCols(tblSheetDetData,'tblSheetDet_QuantityLastForCheck','待配送数量','100',1,true,'',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblSheetDet_QuantityLastForCheck","待配送数量","double","",true,minValue,maxValue,false,''); 
 
 
 addCols(tblSheetDetData,'tblSheetDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblSheetDetInvoice';
tabIsNulls[childCount]='0';
tabDiss[childCount]='进口发票';
childCount++;
 var tblSheetDetInvoiceData = new gridData();
 gridDatas[gridDataNum] = tblSheetDetInvoiceData;
 gridDataNum++;
 var tblSheetDetInvoice=1;
 ...
 addCols(tblSheetDetInvoiceData,'tblSheetDetInvoice_StartItem','开始序号','100',2,true,'',20,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblSheetDetInvoice_StartItem","开始序号","any","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblSheetDetInvoiceData,'tblSheetDetInvoice_EndItem','结束序号','100',2,true,'',20,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblSheetDetInvoice_EndItem","结束序号","any","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblSheetDetInvoiceData,'tblSheetDetInvoice_InvoiceNo','发票编号','150',2,true,'',20,-2,'','',"","","10",'',"3",false); 
 putChildValidateItem("tblSheetDetInvoice_InvoiceNo","发票编号","any","",true,minValue,20,false,''); 
 
 
 
 addCols(tblSheetDetInvoiceData,'tblSheetDetInvoice_CustomsNum','报关单号','150',2,true,'',50,0,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblSheetDetInvoice_CustomsNum","报关单号","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblSheetDetInvoiceData,'tblSheetDetInvoice_DischarTime','放行时间','150',6,true,'',19,0,'','',"","","22",'',"0",false); 
 putChildValidateItem("tblSheetDetInvoice_DischarTime","放行时间","datetime","",true,minValue,19,false,'',true); 
 
 
 
 
 addCols(tblSheetDetInvoiceData,'tblSheetDetInvoice_invId','发票关联号','224',2,true,'',50,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblSheetDetInvoice_invId","发票关联号","any","",true,minValue,50,false,''); 
 
 
 addCols(tblSheetDetInvoiceData,'tblSheetDetInvoice_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblSheet&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblSheet,tblSheetDet,tblSheetDetInvoice,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSheet&allTableName=tblSheet,tblSheetDet,tblSheetDetInvoice,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSheet&allTableName=tblSheetDet,tblSheetDetInvoice,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSheet&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSheet&allTableName=tblSheetDet,tblSheetDetInvoice,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f8a843bd_1207261908194530464&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f8a843bd_1207261908194530464&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=f8a843bd_1207261908194530464&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=f8a843bd_1207261908194530464&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblSheet&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblSheet&designId=26b15e4f_1404281031196313191&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblSheet&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblSheet&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblSheet&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '进口制单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblSheet&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
...
function scanBarCodeHelp(type){
 if(type == "seq")
 asyncbox.success('1、输入数字和“-”号并回车可修改单价<br/>2、输入数字和“*”号并回车可修改折扣','温馨提示'); 
 else
 asyncbox.success('1、输入数字和“+”号并回车可修改数量<br/>2、输入数字和“-”号并回车可修改单价<br/>3、输入数字和“*”号并回车可修改折扣','温馨提示'); 
}
function batchInput(){
 tableName = $(".tagSel").attr("id");
 tableName = tableName.substring(0,tableName.indexOf("TableDIVTitle"));
