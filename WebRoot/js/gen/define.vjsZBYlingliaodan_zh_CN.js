...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=ZBYlingliaodan&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=ZBYlingliaodan&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=ZBYlingliaodan&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=ZBYlingliaodan&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "ZBYlingliaodanDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="ZBYlingliaodanDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="ZBYlingliaodanDet"){
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=55009fed_1403042113397460064&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=55009fed_1403042113397460064&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"ZBYlingliaodan\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"ZBYlingliaodan\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'ZBYlingliaodanDet_UnitPrice',dis:'Unit Price',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'ZBYlingliaodanDet_TotalValue',dis:'Total Value',digit:0,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=ZBYlingliaodan",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=ZBYlingliaodan&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=ZBYlingliaodan&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'打印控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"55009fed_1403042113397460064",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=55009fed_1403042113397460064&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=55009fed_1403042113397460064&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=55009fed_1403042113397460064&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "ZBYlingliaodan"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=ZBYlingliaodan&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=ZBYlingliaodan&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='ZBYlingliaodanDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='Material list';
childCount++;
 var ZBYlingliaodanDetData = new gridData();
 gridDatas[gridDataNum] = ZBYlingliaodanDetData;
 gridDataNum++;
 var ZBYlingliaodanDet=1;
 ...
 addCols(ZBYlingliaodanDetData,'ZBYlingliaodanDet_CodeNo','Code No.','110',2,true,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("ZBYlingliaodanDet_CodeNo","Code No.","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(ZBYlingliaodanDetData,'ZBYlingliaodanDet_DescriPtion','Description Specifications','100',2,true,'',500,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("ZBYlingliaodanDet_DescriPtion","Description Specifications","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(ZBYlingliaodanDetData,'ZBYlingliaodanDet_Unit','Unit','80',2,true,'',500,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("ZBYlingliaodanDet_Unit","Unit","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(ZBYlingliaodanDetData,'ZBYlingliaodanDet_Requisition','Requisition','100',2,true,'',500,0,'','',"","","4",'Quantity',"0",false); 
 putChildValidateItem("ZBYlingliaodanDet_Requisition","Requisition","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(ZBYlingliaodanDetData,'ZBYlingliaodanDet_Actual','Actual','100',2,true,'',500,0,'','',"","","5",'Quantity',"0",false); 
 putChildValidateItem("ZBYlingliaodanDet_Actual","Actual","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(ZBYlingliaodanDetData,'ZBYlingliaodanDet_UnitPrice','Unit Price','100',1,true,'0',0,0,'','',"","","6",'Cost',"0",false); 
 putChildValidateItem("ZBYlingliaodanDet_UnitPrice","Unit Price","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(ZBYlingliaodanDetData,'ZBYlingliaodanDet_TotalValue','Total Value','100',1,true,'0',0,0,'','',"","","7",'Cost',"0",false); 
 putChildValidateItem("ZBYlingliaodanDet_TotalValue","Total Value","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(ZBYlingliaodanDetData,'ZBYlingliaodanDet_Production','Production Cost Centre No.','150',2,true,'',500,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("ZBYlingliaodanDet_Production","Production Cost Centre No.","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(ZBYlingliaodanDetData,'ZBYlingliaodanDet_Remarks','Remarks','200',2,true,'',1000,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("ZBYlingliaodanDet_Remarks","Remarks","any","",true,minValue,1000,false,'',true); 
 
 
 
 
 addCols(ZBYlingliaodanDetData,'ZBYlingliaodanDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=ZBYlingliaodan&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "ZBYlingliaodan,ZBYlingliaodanDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=ZBYlingliaodan&allTableName=ZBYlingliaodan,ZBYlingliaodanDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=ZBYlingliaodan&allTableName=ZBYlingliaodanDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=ZBYlingliaodan&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=ZBYlingliaodan&allTableName=ZBYlingliaodanDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=55009fed_1403042113397460064&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=55009fed_1403042113397460064&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "ZBYlingliaodanDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=55009fed_1403042113397460064&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=55009fed_1403042113397460064&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=ZBYlingliaodan&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function billTool(value,dis,width,height){
 ...
 }else if((obj==null || obj.length==0) && ct_name != curGridRowNum.curKey){
...
function update(){
 location.href="/UserFunctionAction.do?tableName=ZBYlingliaodan&designId=55009fed_1403042113397460064&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=ZBYlingliaodan&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=ZBYlingliaodan&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=ZBYlingliaodan&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = 'ZBY供应领料单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=ZBYlingliaodan&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
