...
{ 
 cFireEvent('PDInvertoryAdjDet_Qty',''); 
 cFireEvent('PDInvertoryAdjDet_Price',''); 
 cFireEvent('PDInvertoryAdjDetIn_Qty','0'); 
 cFireEvent('PDInvertoryAdjDetIn_Price','0'); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDInvertoryAdj&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDInvertoryAdj&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDInvertoryAdj&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDInvertoryAdj&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDInvertoryAdjDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDInvertoryAdjDetInTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDInvertoryAdjDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="PDInvertoryAdjDetIn"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDInvertoryAdjDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDInvertoryAdjDetIn"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="PDInvertoryAdjDet"; 
 tblName[1]="PDInvertoryAdjDetIn"; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDInvertoryAdj\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDInvertoryAdj\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'PDInvertoryAdjDet_Qty',"PDInvertoryAdjDet"); 
 delFireEventOne(rowId,tableName,'PDInvertoryAdjDetIn_Qty',"PDInvertoryAdjDetIn"); 
 delFireEventOne(rowId,tableName,'PDInvertoryAdjDetIn_Price',"PDInvertoryAdjDetIn"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'PDInvertoryAdjDet_Qty',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDInvertoryAdjDetIn_Qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDInvertoryAdjDetIn_Price',dis:'单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDInvertoryAdjDetIn_Amt',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDInvertoryAdj",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDInvertoryAdj&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDInvertoryAdj&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDInvertoryAdj"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDInvertoryAdj&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDInvertoryAdj&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'PDInvertoryAdjDet_Qty'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDInvertoryAdjDet_Price'); 
 r(0,"Amt")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDInvertoryAdjDet_Amt'); 
 r(0,"Amt")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDInvertoryAdjDetIn_Qty'); 
 r(1,"Amt")[p].value=f(rn(1,"Qty",p)*rn(1,"Price",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDInvertoryAdjDetIn_Price'); 
 r(1,"Amt")[p].value=f(rn(1,"Qty",p)*rn(1,"Price",p),2);
...
childData[childCount]='PDInvertoryAdjDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='库存调整出库明细';
childCount++;
 var PDInvertoryAdjDetData = new gridData();
 gridDatas[gridDataNum] = PDInvertoryAdjDetData;
 gridDataNum++;
 var PDInvertoryAdjDet=1;
 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_StockCode','仓库','100',2,true,'',100,-2,'','',"","","0",'',"2",false); 
 putChildValidateItem("PDInvertoryAdjDet_StockCode","仓库","any","",true,minValue,100,false,''); 
 ...
 var param4="";
 param4=param4+":1调整仓库@StockCode"; 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_GoodsCode','商品名称','200',2,false,'',100,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("PDInvertoryAdjDet_GoodsCode","商品名称","any","",false,minValue,100,false,'');
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_tblGoods_GoodsNumber','品号','200',20,true,'',1000000,2,'PDInvertoryAdjDet:GoodsCode:PDInvertoryAdjDet_tblGoods_GoodsNumber:'+param4,'',"","","1",'',"2",false); 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_tblGoods_GoodsFullName','商品名称','300',20,true,'',1000000,2,'PDInvertoryAdjDet:GoodsCode:PDInvertoryAdjDet_tblGoods_GoodsFullName:'+param4,'',"","","1",'',"2",false); 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDInvertoryAdjDet:GoodsCode:PDInvertoryAdjDet_tblGoods_GoodsSpec:'+param4,'',"","","1",'',"2",false); 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDInvertoryAdjDet:GoodsCode:PDInvertoryAdjDet_tblGoods_BaseUnit:'+param4,'',"","","1",'',"2",false); 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_tblStocks_lastQty','实际库存','90',1,true,'',1000000,2,'PDInvertoryAdjDet:GoodsCode:PDInvertoryAdjDet_tblStocks_lastQty:'+param4,'',"","","1",'',"2",false); 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_ViewVirtualStock_UseQty','虚拟库存','90',1,true,'',1000000,2,'PDInvertoryAdjDet:GoodsCode:PDInvertoryAdjDet_ViewVirtualStock_UseQty:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","","2",'',"2",false); 
 putChildValidateItem("PDInvertoryAdjDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_Inch','厚度','70',2,true,'',100,-2,'','',"","","3",'',"2",false); 
 putChildValidateItem("PDInvertoryAdjDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_Hue','颜色','70',2,true,'',100,-2,'','',"","","4",'',"2",false); 
 putChildValidateItem("PDInvertoryAdjDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","","5",'',"2",false); 
 putChildValidateItem("PDInvertoryAdjDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","6",'',"2",false); 
 putChildValidateItem("PDInvertoryAdjDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","7",'',"2",false); 
 putChildValidateItem("PDInvertoryAdjDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_Qty','数量','80',1,false,'',0,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDet_Qty","数量","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_Price','单价','80',1,true,'',0,-2,'','',"priceIdentifier","","9",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDet_Price","单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_Amt','金额','70',1,true,'',0,-2,'','',"AmountIdentifier","","10",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDet_Amt","金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_Remark','备注','130',2,true,'',8000,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_Seq','序列号','200',2,true,'',5000,-2,'','',"","","12",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDet_Seq","序列号","any","",true,minValue,5000,false,''); 
 
 
 addCols(PDInvertoryAdjDetData,'PDInvertoryAdjDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='PDInvertoryAdjDetIn';
tabIsNulls[childCount]='1';
tabDiss[childCount]='库存调整入库明细';
childCount++;
 var PDInvertoryAdjDetInData = new gridData();
 gridDatas[gridDataNum] = PDInvertoryAdjDetInData;
 gridDataNum++;
 var PDInvertoryAdjDetIn=1;
 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_StockCode','仓库','100',2,true,'',100,-2,'','',"","","0",'',"2",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_StockCode","仓库","any","",true,minValue,100,false,''); 
 ...
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_GoodsCode','商品名称','200',2,false,'',100,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("PDInvertoryAdjDetIn_GoodsCode","商品名称","any","",false,minValue,100,false,'');
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_tblGoods_GoodsNumber','品号','200',20,true,'',1000000,2,'PDInvertoryAdjDetIn:GoodsCode:PDInvertoryAdjDetIn_tblGoods_GoodsNumber:'+param4,'',"","","1",'',"2",false); 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_tblGoods_GoodsFullName','商品名称','300',20,true,'',1000000,2,'PDInvertoryAdjDetIn:GoodsCode:PDInvertoryAdjDetIn_tblGoods_GoodsFullName:'+param4,'',"","","1",'',"2",false); 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDInvertoryAdjDetIn:GoodsCode:PDInvertoryAdjDetIn_tblGoods_GoodsSpec:'+param4,'',"","","1",'',"2",false); 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDInvertoryAdjDetIn:GoodsCode:PDInvertoryAdjDetIn_tblGoods_BaseUnit:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_BatchNo','批号','200',2,true,'',100,-2,'','',"","","2",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_Inch','厚度','200',2,true,'',100,-2,'','',"","","3",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_Hue','颜色','200',2,true,'',100,-2,'','',"","","4",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_yearNO','尺码','200',2,true,'',100,-2,'','',"","","5",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_ProDate','生产日期','200',5,true,'',10,-2,'','',"","","6",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_Availably','保质期限','200',5,true,'',10,-2,'','',"","","7",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_Qty','数量','100',1,false,'0',0,0,'','',"Qty","","8",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_Qty","数量","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_Price','单价','100',1,false,'0',0,0,'','',"Price","","9",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_Price","单价","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_Amt','金额','100',1,true,'',0,8,'','',"Amount","","10",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_Amt","金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_Seq','序列号','200',2,true,'',5000,-2,'','',"","","11",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_Seq","序列号","any","",true,minValue,5000,false,''); 
 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_Remark','备注','200',2,true,'',5000,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("PDInvertoryAdjDetIn_Remark","备注","any","",true,minValue,5000,false,'',true); 
 
 
 addCols(PDInvertoryAdjDetInData,'PDInvertoryAdjDetIn_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDInvertoryAdj&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDInvertoryAdj,PDInvertoryAdjDet,PDInvertoryAdjDetIn,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDInvertoryAdj&allTableName=PDInvertoryAdj,PDInvertoryAdjDet,PDInvertoryAdjDetIn,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDInvertoryAdj&allTableName=PDInvertoryAdjDet,PDInvertoryAdjDetIn,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDInvertoryAdj&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDInvertoryAdj&allTableName=PDInvertoryAdjDet,PDInvertoryAdjDetIn,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "PDInvertoryAdjDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDInvertoryAdj&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDInvertoryAdj&designId=eae3edac_15112118011736&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDInvertoryAdj&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDInvertoryAdj&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDInvertoryAdj&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '库存调整单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDInvertoryAdj&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
