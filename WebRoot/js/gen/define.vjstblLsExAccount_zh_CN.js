...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblLsExAccount&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblLsExAccount&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblLsExAccount&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblLsExAccount&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblLsExAccountDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblLsExAccountDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblLsExAccountDet"){
...
function execStat2(){
 execStatOne('tblLsExAccountDet_Amount',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=84e88167_1302051205181250185&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=84e88167_1302051205181250185&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblLsExAccount\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblLsExAccount\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'TotalAmount',dis:'报关外币金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalCGInvoiceTax',dis:'国内采购发票价税合计',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ExZHRate',dis:'综合费率值',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ZHLowestFee',dis:'最低消费',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ExZHAmount',dis:'综合服务费',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ACRate',dis:'结算汇率',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CURate',dis:'外币收款汇率',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalYSK',dis:'合计应收款',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'YSKCur',dis:'外币应收款',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ProxyKPAmount',dis:'增值税6%开票金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblLsExAccountDet_Amount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblLsExAccount",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblLsExAccount&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblLsExAccount&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"84e88167_1302051205181250185",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 saveField[0].value = "";
 var obj = m("tbl_tblESheet_BillNo");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("BGDate"); 
 saveField[0].value = "";
 var obj = m("tbl_tblESheet_BGDate");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("TotalAmount"); 
 saveField[0].value = "";
 var obj = m("tbl_tblESheet_TotalAmount");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("Currency"); 
 saveField[0].value = "";
 var obj = m("tbl_tblESheet_Currency");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SheetID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblESheet_id");
 if(obj != null){
 obj.value = "";
 }
 }
 if(fieldName == "ExZHRate"){
 var saveField=n("ExZHRate"); 
 saveField[0].value = "";
 var obj = m("tbl_A_ExZHRate");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("ExZHRateMark"); 
 saveField[0].value = "";
 var obj = m("tbl_A_ExZHRateMark");
 if(obj != null){
 obj.value = "";
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84e88167_1302051205181250185&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84e88167_1302051205181250185&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=84e88167_1302051205181250185&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblLsExAccount"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblLsExAccount&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblLsExAccount&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 if(n('ExZHRateEmu')[0].value==4)n('ExZHAmount')[0].value=f(nn('TotalAmount',0)*nn('ExZHRate',0),2);
...
childData[childCount]='tblLsExAccountDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='出口结算单明细费用';
childCount++;
 var tblLsExAccountDetData = new gridData();
 gridDatas[gridDataNum] = tblLsExAccountDetData;
 gridDataNum++;
 var tblLsExAccountDet=1;
 ...
 addCols(tblLsExAccountDetData,'tblLsExAccountDet_PayItemCode','结算项编号','100',2,false,'',30,2,'tblLsExAccountDet:PayItemCode:tblLsExAccountDet_PayItemCode:'+param4,"","","","5",'',"2",true);
 putChildValidateItem("tblLsExAccountDet_PayItemCode","结算项编号","any","",false,minValue,30,false,'',true);
 
 
 
 addCols(tblLsExAccountDetData,'tblLsExAccountDet_PayItem','结算项','200',2,false,'',50,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblLsExAccountDet_PayItem","结算项","any","",false,minValue,50,false,'',true); 
 
 
 
 addCols(tblLsExAccountDetData,'tblLsExAccountDet_PayItemRemark','结算项全称','224',2,true,'',100,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblLsExAccountDet_PayItemRemark","结算项全称","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblLsExAccountDetData,'tblLsExAccountDet_Amount','金额','120',1,true,'0',0,0,'','',"","","16",'',"0",false); 
 putChildValidateItem("tblLsExAccountDet_Amount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblLsExAccountDetData,'tblLsExAccountDet_Remark','备注','300',2,true,'',8000,0,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblLsExAccountDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblLsExAccountDetData,'tblLsExAccountDet_SourceID','源ID','0',2,true,'',50,-2,'','',"","","30",'',"3",false); 
 putChildValidateItem("tblLsExAccountDet_SourceID","源ID","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblLsExAccountDetData,'tblLsExAccountDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblLsExAccount&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblLsExAccount,tblLsExAccountDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblLsExAccount&allTableName=tblLsExAccount,tblLsExAccountDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblLsExAccount&allTableName=tblLsExAccountDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblLsExAccount&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblLsExAccount&allTableName=tblLsExAccountDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84e88167_1302051205181250185&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=84e88167_1302051205181250185&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblLsExAccountDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=84e88167_1302051205181250185&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=84e88167_1302051205181250185&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblLsExAccount&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblLsExAccount&designId=21786bdc_1410140918484221432&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblLsExAccount&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblLsExAccount&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblLsExAccount&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '出口结算单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblLsExAccount&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
