...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDUBomBatchU&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDUBomBatchU&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDUBomBatchU&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDUBomBatchU&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDUBomBatchUDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDUBomBatchUDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDUBomBatchUDet"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDUBomBatchU\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDUBomBatchU\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'qty',dis:'数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'lossRate',dis:'损耗率',digit:0,isMain:true};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDUBomBatchU",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDUBomBatchU&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDUBomBatchU&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "RGoodsCode"){
 var saveField=n("RGoodsCode"); 
 saveField[0].value = "";
 var obj = m("tbl_tblGoods_classCode");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblGoods1_GoodsNumber");
 saveField[0].value = ""; 
 m("tbl_tblGoods1_GoodsNumber").value = "" ;
 var saveField=n("tblGoods1_GoodsFullName");
 saveField[0].value = ""; 
 m("tbl_tblGoods1_GoodsFullName").value = "" ;
 var saveField=n("tblGoods1_GoodsSpec");
 saveField[0].value = ""; 
 m("tbl_tblGoods1_GoodsSpec").value = "" ;
 var saveField=n("tblGoods1_BaseUnit");
 saveField[0].value = ""; 
 m("tbl_tblGoods1_BaseUnit").value = "" ;
 }
 }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDUBomBatchU"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDUBomBatchU&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDUBomBatchU&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='PDUBomBatchUDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='需修改BOM单';
childCount++;
 var PDUBomBatchUDetData = new gridData();
 gridDatas[gridDataNum] = PDUBomBatchUDetData;
 gridDataNum++;
 var PDUBomBatchUDet=1;
 ...
 addCols(PDUBomBatchUDetData,'PDUBomBatchUDet_PDBomId','生产BOM','200',2,true,'',50,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("PDUBomBatchUDet_PDBomId","生产BOM","any","",true,minValue,50,false,'');
 addCols(PDUBomBatchUDetData,'PDUBomBatchUDet_PDBom_BillNo','BOM单号','100',2,true,'',1000000,2,'PDUBomBatchUDet:PDBomId:PDUBomBatchUDet_PDBom_BillNo:'+param4,'',"","","0",'',"2",false); 
 addCols(PDUBomBatchUDetData,'PDUBomBatchUDet_tblGoods_GoodsNumber','品号','100',20,true,'',1000000,2,'PDUBomBatchUDet:PDBomId:PDUBomBatchUDet_tblGoods_GoodsNumber:'+param4,'',"","","0",'',"2",false); 
 addCols(PDUBomBatchUDetData,'PDUBomBatchUDet_tblGoods_GoodsFullName','品名','300',20,true,'',1000000,2,'PDUBomBatchUDet:PDBomId:PDUBomBatchUDet_tblGoods_GoodsFullName:'+param4,'',"","","0",'',"2",false); 
 addCols(PDUBomBatchUDetData,'PDUBomBatchUDet_tblGoods_GoodsSpec','规格','300',20,true,'',1000000,2,'PDUBomBatchUDet:PDBomId:PDUBomBatchUDet_tblGoods_GoodsSpec:'+param4,'',"","","0",'',"2",false); 
 addCols(PDUBomBatchUDetData,'PDUBomBatchUDet_tblGoods_BaseUnit','单位','50',2,true,'',1000000,2,'PDUBomBatchUDet:PDBomId:PDUBomBatchUDet_tblGoods_BaseUnit:'+param4,'',"","","0",'',"2",false); 
 addCols(PDUBomBatchUDetData,'PDUBomBatchUDet_PDBom_version','版本','50',0,true,'',1000000,2,'PDUBomBatchUDet:PDBomId:PDUBomBatchUDet_PDBom_version:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDUBomBatchUDetData,'PDUBomBatchUDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDUBomBatchU&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDUBomBatchU,PDUBomBatchUDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDUBomBatchU&allTableName=PDUBomBatchU,PDUBomBatchUDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDUBomBatchU&allTableName=PDUBomBatchUDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDUBomBatchU&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDUBomBatchU&allTableName=PDUBomBatchUDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDUBomBatchU&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDUBomBatchU&designId=6df8676a_1510251558170720301&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDUBomBatchU&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDUBomBatchU&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDUBomBatchU&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '批量更新BOM物料';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDUBomBatchU&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
