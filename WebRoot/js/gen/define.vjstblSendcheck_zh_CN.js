...
{ 
 cFireEvent('tblSendcheckDet_Qty','0'); 
 cFireEvent('tblSendcheckDet_Price','0'); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblSendcheck&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblSendcheck&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblSendcheck&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblSendcheck&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function addSeq(o,keyCode){
 ...
 var goodscodestr="";
 var isInstore = false ;
 if(seqFname=="tblSendcheckDet_Seq"){
...
if(isInstore){
 ...
 if(seqFname=="tblSendcheckDet_Seq"){
 var qtys=n('tblSendcheckDet'+"_"+'Qty');
 ...
 var out=false;
 var ss="@GN:Seq:Qty:tblSendcheckDet_GoodsSeqSet:tblSendcheckDet_GoodsCode:StockCode".split(":")
 ...
 for(var i=0;i<tempArr.length-1;i++){
 if(tempArr[i].toLowerCase()==o.value.toLowerCase()){
 alert('【系统提示】\n\n您录入的序列号'+'['+o.value+']'+'与第'+'['+(k+1)+']'+'行商品已录入的序列号有重复,请确认后重试.');
 return false;
 }
 } 
 ...
 var str=getStr(inputValue,ri);
 var urls="/UtilServlet?operation=validateDetailSeq&MOID=0af1b787_1302260934004050017&MOOP=add&tableName="+seqFname.substr(0,seqFname.lastIndexOf("_"))+"&"+seqFname+"="+o.value+"&validateField="+seqFname.substr(seqFname.lastIndexOf("_")+1,seqFname.length-1)+str;
 AjaxRequest(urls) ;
 if(response == "hasMore"){ 
 var str=getStr(inputValue,ri);
 ...
 if(props[m]!="" && props[m].indexOf(retAndSav[0])!=-1){
 numN = m ;
 break ;
 }
 }
 ...
 dataBackIn("falsle",retAndSav[2],retAndSav[1],window);

 n(seqFname+"_hid")[ri].value = n(seqFname)[ri].value ;
 n(seqFname)[ri].value=n(seqFname)[ri].value+"~";
 if("tblSendcheckDet"==seqFname.substr(0,seqFname.lastIndexOf("_"))){ 
 var qtys=n("tblSendcheckDet"+"_"+'Qty');
 qtys[ri].value="1";
 ...
 qtys[numN].value=parseInt(qtys[numN].value)+1;
 }
 $(qtys[numN]).change();
 
 var rseq = o.value.trim();
 var rasa = retAndSav[2].split(";");
 var rava = retAndSav[1].split(";");
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblSendcheckDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblSendcheckDet_SendNo');
 ...
 var z=n('tblSendcheckDet_GoodsCode');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblSendcheckDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblSendcheckDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblSendcheckDet"){
 
 copyFields=n('tblSendcheckDet_StockCode');
 if(copyFields[index].value !=0 && copyFields[index].value != ""){return}
 if((order > 0 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && ((copyFields[index].value ==0) ||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 var tblSendcheckDet_tblStock_StockFullName1 = n('tblSendcheckDet_tblStock_StockFullName1');
 if(typeof(tblSendcheckDet_tblStock_StockFullName1) != 'undefined' && tblSendcheckDet_tblStock_StockFullName1.length >index){
 tblSendcheckDet_tblStock_StockFullName1[index].value = tblSendcheckDet_tblStock_StockFullName1[index-1].value;
 }
 }
 copyFields=n('tblSendcheckDet_GoodsCode');
 if((order > 3 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && ((copyFields[index].value ==0) ||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 var tblSendcheckDet_tblGoods_GoodsFullName = n('tblSendcheckDet_tblGoods_GoodsFullName');
 if(typeof(tblSendcheckDet_tblGoods_GoodsFullName) != 'undefined' && tblSendcheckDet_tblGoods_GoodsFullName.length >index){
 tblSendcheckDet_tblGoods_GoodsFullName[index].value = tblSendcheckDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblSendcheckDet_tblGoods_GoodsNumber = n('tblSendcheckDet_tblGoods_GoodsNumber');
 if(typeof(tblSendcheckDet_tblGoods_GoodsNumber) != 'undefined' && tblSendcheckDet_tblGoods_GoodsNumber.length >index){
 tblSendcheckDet_tblGoods_GoodsNumber[index].value = tblSendcheckDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblSendcheckDet_tblGoods_GoodsSpec = n('tblSendcheckDet_tblGoods_GoodsSpec');
 if(typeof(tblSendcheckDet_tblGoods_GoodsSpec) != 'undefined' && tblSendcheckDet_tblGoods_GoodsSpec.length >index){
 tblSendcheckDet_tblGoods_GoodsSpec[index].value = tblSendcheckDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblSendcheckDet_tblGoods_BaseUnit = n('tblSendcheckDet_tblGoods_BaseUnit');
 if(typeof(tblSendcheckDet_tblGoods_BaseUnit) != 'undefined' && tblSendcheckDet_tblGoods_BaseUnit.length >index){
 tblSendcheckDet_tblGoods_BaseUnit[index].value = tblSendcheckDet_tblGoods_BaseUnit[index-1].value;
 }
 var tblSendcheckDet_tblGoods_BarCode = n('tblSendcheckDet_tblGoods_BarCode');
 if(typeof(tblSendcheckDet_tblGoods_BarCode) != 'undefined' && tblSendcheckDet_tblGoods_BarCode.length >index){
 tblSendcheckDet_tblGoods_BarCode[index].value = tblSendcheckDet_tblGoods_BarCode[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblSendcheckDet"){
 
 copyFields=n('tblSendcheckDet_StockCode'); 
 if(copyFields[index].value !=0 && copyFields[index].value != ""){return}
 if( (order > 0 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyFields[index].value ==0||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 var tblSendcheckDet_tblStock_StockFullName1 = n('tblSendcheckDet_tblStock_StockFullName1'); 
 if(typeof(tblSendcheckDet_tblStock_StockFullName1) != 'undefined' && tblSendcheckDet_tblStock_StockFullName1.length >index){
 tblSendcheckDet_tblStock_StockFullName1[index].value = tblSendcheckDet_tblStock_StockFullName1[index-1].value;
 ...
 copyFields=n('tblSendcheckDet_GoodsCode'); 
 if( (order > 3 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyFields[index].value ==0||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 var tblSendcheckDet_tblGoods_GoodsFullName = n('tblSendcheckDet_tblGoods_GoodsFullName'); 
 if(typeof(tblSendcheckDet_tblGoods_GoodsFullName) != 'undefined' && tblSendcheckDet_tblGoods_GoodsFullName.length >index){
 tblSendcheckDet_tblGoods_GoodsFullName[index].value = tblSendcheckDet_tblGoods_GoodsFullName[index-1].value;
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblSendcheckDet"; 
 var goodsCodeLogicValidate_tblSendcheckDet='@GN:Seq:Qty:tblSendcheckDet_GoodsSeqSet:tblSendcheckDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'tblSendcheckDet',value:goodsCodeLogicValidate_tblSendcheckDet};
...
function execStat2(){
 execStatOne('tblSendcheckDet_Qty',4);
 execStatOne('tblSendcheckDet_Amount',2);
 setTotalCount(getTotalCount());
}
...
function execDetailStat2(){
 execDetStatOne('tblSendcheckDet_Qty',4); 
 execDetStatOne('tblSendcheckDet_Amount',2); 
 }
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=0af1b787_1302260934004050017&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr.substring(0,index+"&seq=".length)+n(thisField.substr(0,thisField.lastIndexOf("_")))[ri].value+"&src=menu&seqFname="+thisField+"&newSeq="+mi[ri].value+"&tableName=tblSendcheck&childTableName="+thisField.split("_")[0]+"&fieldName="+field;
 var goodscode="";
 var goodscodestr="";
 var goodsProps = "";
 var stockCode = "" ;
 ...
 if(thisField=="tblSendcheckDet_Seq_hid"){
 ...
 stockCode = nz("tblSendcheckDet"+"_"+ss[5],ri) ;
 }
 }
 var ss="@IOO:Instore".split(":")
 if(ss[0]=="@GN"){
 ...
 AjaxRequest(urlsgood) ;
 if("false"==response){
 asyncbox.tips("该商品未启用序列号10！！","alert",2000) ;
 return ;
 }
 }
 var url2=urlstr.substr(urlstr.indexOf('?')+1);
 url2='&src=menu&MOID=0af1b787_1302260934004050017&MOOP=add&'+url2;
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblSendcheck\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblSendcheck\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblSendcheckDet_Qty',"tblSendcheckDet"); 
 delFireEventOne(rowId,tableName,'tblSendcheckDet_Price',"tblSendcheckDet"); 
 delFireEventOne(rowId,tableName,'tblSendcheckDet_Amount',"tblSendcheckDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'BillAmt',dis:'合计金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblSendcheckDet_Qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSendcheckDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSendcheckDet_Amount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblSendcheck",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSendcheck&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSendcheck&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'打印控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"0af1b787_1302260934004050017",
 MOOP:"add",
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('tblSendcheckDet_Qty'); 
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0af1b787_1302260934004050017&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0af1b787_1302260934004050017&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=0af1b787_1302260934004050017&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblSendcheck"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblSendcheck&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblSendcheck&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblSendcheckDet_Qty'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);

 n('BillAmt')[0].value=f(sm('tblSendcheckDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSendcheckDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSendcheckDet_Amount'); 
...
childData[childCount]='tblSendcheckDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='配送验收单明细';
childCount++;
 var tblSendcheckDetData = new gridData();
 gridDatas[gridDataNum] = tblSendcheckDetData;
 gridDataNum++;
 var tblSendcheckDet=1;
 ...
 addCols(tblSendcheckDetData,'tblSendcheckDet_StockCode','仓库','80',2,false,'',60,-2,'','',"","copy","0",'',"2",true);
 putChildValidateItem("tblSendcheckDet_StockCode","仓库","any","",false,minValue,60,true,'');
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_tblStock_StockFullName1','仓库','75',2,true,'',1000000,2,'tblSendcheckDet:StockCode:tblSendcheckDet_tblStock_StockFullName1:'+param4,'',"","copy","0",'',"2",false); 
 ...
 param4=param4+":1仓库@tblSendcheckDet_StockCode";
 param4=param4+":1部门@DepartmentCode"; 
 param4=param4+":1单据日期@BillDate"; 
 param4=param4+":tblSendcheckDet_GoodsCode@tblSendcheckDet_GoodsCode"; 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_SendNo','配送单号','120',2,false,'',100,-2,'','',"","triggerCopy","1",'',"2",true);
 putChildValidateItem("tblSendcheckDet_SendNo","配送单号","any","",false,minValue,100,false,'');
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_tblSend_BillNo','配送单号','100',2,true,'',1000000,2,'tblSendcheckDet:SendNo:tblSendcheckDet_tblSend_BillNo:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendcheckDetData,'tblSendcheckDet_tblGoods_GoodsFullName','商品名称','150',2,true,'',1000000,2,'tblSendcheckDet:SendNo:tblSendcheckDet_tblGoods_GoodsFullName:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendcheckDetData,'tblSendcheckDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'tblSendcheckDet:SendNo:tblSendcheckDet_tblGoods_GoodsNumber:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendcheckDetData,'tblSendcheckDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'tblSendcheckDet:SendNo:tblSendcheckDet_tblGoods_GoodsSpec:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendcheckDetData,'tblSendcheckDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblSendcheckDet:SendNo:tblSendcheckDet_tblGoods_BaseUnit:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendcheckDetData,'tblSendcheckDet_tblGoods_BarCode','条码','100',2,true,'',1000000,2,'tblSendcheckDet:SendNo:tblSendcheckDet_tblGoods_BarCode:'+param4,'',"","triggerCopy","1",'',"2",false); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_GoodsCode','商品名称','120',2,false,'',100,-2,'','',"","triggerOrCopy","3",'',"2",false); 
 putChildValidateItem("tblSendcheckDet_GoodsCode","商品名称","any","",false,minValue,100,false,''); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","5",'',"0",false); 
 putChildValidateItem("tblSendcheckDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_color','色彩','60',2,true,'',100,-2,'','',"","","5",'',"3",false); 
 putChildValidateItem("tblSendcheckDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","6",'',"0",false); 
 putChildValidateItem("tblSendcheckDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"2",false); 
 putChildValidateItem("tblSendcheckDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"2",false); 
 putChildValidateItem("tblSendcheckDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblSendcheckDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblSendcheckDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_Qty','数量','80',1,false,'0',0,0,'','',"Qty","","11",'',"0",false); 
 putChildValidateItem("tblSendcheckDet_Qty","数量","double","SeqQty",false,minValue,maxValue,true,'0',true); 
 ...
 addCols(tblSendcheckDetData,'tblSendcheckDet_Seq','序列号','130',2,true,'',5000,-2,'','',"","triggerCopy","11",'',"2",false);
 addCols(tblSendcheckDetData,'tblSendcheckDet_Seq'+'_hid','序列号','130',2,true,'',5000,2,'@SEQ@IOO:Instore',"","","triggerCopy","11",'',"2",false);
 putChildValidateItem("tblSendcheckDet_Seq","序列号","any","GoodsField",true,minValue,5000,false,'',true);
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_Price','单价','80',1,true,'0',0,0,'','',"priceIdentifier","","12",'',"0",false); 
 putChildValidateItem("tblSendcheckDet_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_Amount','金额','100',1,true,'',0,0,'','',"AmountIdentifier","","13",'',"0",false); 
 putChildValidateItem("tblSendcheckDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(tblSendcheckDetData,'tblSendcheckDet_Remark','备注','130',2,true,'',8000,0,'','',"","","19",'',"0",false); 
 putChildValidateItem("tblSendcheckDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_ReturnQty','配送退货数量','224',1,true,'',0,-2,'','',"Qty","","100",'',"3",false); 
 putChildValidateItem("tblSendcheckDet_ReturnQty","配送退货数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblSendcheckDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_NotReturnQty','未配送退货数量','224',1,true,'',0,-2,'','',"Qty","","100",'',"3",false); 
 putChildValidateItem("tblSendcheckDet_NotReturnQty","未配送退货数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblSendcheckDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","103",'',"3",false); 
 putChildValidateItem("tblSendcheckDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 addCols(tblSendcheckDetData,'tblSendcheckDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblSendcheck&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblSendcheck,tblSendcheckDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSendcheck&allTableName=tblSendcheck,tblSendcheckDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSendcheck&allTableName=tblSendcheckDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSendcheck&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSendcheck&allTableName=tblSendcheckDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0af1b787_1302260934004050017&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0af1b787_1302260934004050017&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=0af1b787_1302260934004050017&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=0af1b787_1302260934004050017&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblSendcheck&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function billTool(value,dis,width,height){
 ...
 nv = obj[0].value;
 }else if(objName.indexOf(curGridRowNum.curKey)==0) { 
 if(name.indexOf("_") > 0){
 ct_name = name.substring(0,name.indexOf("_"));
 
 if((obj==null || obj.length==0) && ct_name != curGridRowNum.curKey){
 ...
 nv = gdata.rows[curGridRowNum.curLine-1].get(objName.substring(objName.indexOf("_")+1)); 
 }else{
 if(curGridRowNum.curLine>=obj.length||curGridRowNum.curLine<0){
 alert("请选择要查询的明细行"); 
 return;
...
function update(){
 location.href="/UserFunctionAction.do?tableName=tblSendcheck&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblSendcheck&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblSendcheck&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblSendcheck&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '配送验收单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblSendcheck&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
...
function scanBarCodeEvOne(bsvalue,tableName,isMerger,search_favour){ 
 ...
 if(fdobj[lastLine].value.length>0&&fdobj[lastLine].value!=0){
...
function scanBarCodeHelp(type){
 if(type == "seq")
 asyncbox.success('1、输入数字和“-”号并回车可修改单价','温馨提示'); 
 else
 asyncbox.success('1、输入数字和“+”号并回车可修改数量<br/>2、输入数字和“-”号并回车可修改单价','温馨提示'); 
}
function batchInput(){
 tableName = $(".tagSel").attr("id");
 tableName = tableName.substring(0,tableName.indexOf("TableDIVTitle"));
