...
function onLockClick(str){
 AjaxRequest("/UtilServlet?operation=lockColumn&report=dldlb&number="+str); 
}
...
function printData(){
 ...
 var urls = "/UserFunctionQueryAction.do?tableName=dldlb&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+keyVal.val()+"&parentTableName=&winCurIndex="+winCurIndex;
...
function printLabelData(){
 ...
 var urls = "/UserFunctionQueryAction.do?tableName=dldlb&reportNumber=PrintSeqdldlb&printType=labelPrint&operation=18&keyId="+keyVal+"&parentTableName=&winCurIndex="+winCurIndex;
...
function deleteList(rowRemark){
 ...
 if(rowVals.length>0){
 alert("你没有权限删除"+rowVals.substring(0,rowVals.length-1));
 return false;
 }
...
function onItemClick(str){
 ...
 if(currentId!=str){
 currentId = str ;
 setIframSrc2("/UserFunctionAction.do?tableName=dldlb&keyId="+str+"&operation=7&winCurIndex="+winCurIndex,currentId);
 }
}
...
function setColumnSet(str){
 var strURL = "/UtilServlet?operation=colconfig&tableName=dldlb&colType=list&colSelect="+str ;
 AjaxRequest(strURL);
...
function onDefaultCol(){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=dldlb&colType=list" ;
 AjaxRequest(strURL);
 if( beforeButtonQuery()) {
 submitQuery();
 }
...
function copyOperation()
{
 ...
 usrc="/UserFunctionAction.do?&operation=17&tableName=dldlb&saveDraft=&parentTableName=&moduleType="+moduleTypeValue+"&f_brother="+f_brother+"&noback=false&winCurIndex="+winCurIndex+"&keyId="+document.getElementById("varKeyIds").value.split("|")[0];
...
function telCall()
{
 ...
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=dldlb&keyId="+keyIds,"","dialogWidth=600px;dialogHeight=350px"); 
 } 
 }else
 {
 alert("请选择至少一条记录"); 
...
function printMoreData(){
 ...
 cks = $(this).parents("tr").html();
 if(cks.indexOf("reverse")==-1){
...
function addPrepare(){
 ...
 usrc ='/UserFunctionAction.do?tableName=dldlb&parentCode='+parentCode+'&operation=6&moduleType='+moduleTypeValue+'&f_brother='+f_brother+'&parentTableName=&winCurIndex='+winCurIndex;
...
function openSelect(selectName,fieldName,display){
 ...
 var urlstr = '/UserFunctionAction.do?operation=22&src=menu&parentTableName='+parentTableName+'&moduleType='+moduleType+'&displayName='+encodeURI(display)+'&LinkType=@URL:&tableName=dldlb&selectName='+selectName+encodeURI(linkStr) ;
...
function draftAudit(rowRemark){
 ...
 var draft = getSelectRowValue("draft").split("|");
 var rowRemarkVal=getSelectRowValue(rowRemark).split("|");
 var rowVals="";
...
function showDivCustomSetTable(){ 
 var returnValue = window.showModalDialog("/UserFunctionQueryAction.do?tableName=dldlb&operation=4&parentTableName=&configType=listConfig&winCurIndex="+winCurIndex,"","dialogWidth=670px;dialogHeight=450px") ;
...
{
 var v = "";
 jQuery("input[name='keyId']").each(function(){if(this.checked)v+=jQuery(this.parentNode.parentNode).attr(field)+"|"});
 return v;
}
...
function detail(str){
 turl="/UserFunctionAction.do?tableName=dldlb&keyId="+str.split(",")[0]+"&moduleType="+moduleType+"&f_brother=&operation=5&winCurIndex="+winCurIndex+"&pageNo=&parentCode="+str.split(",")[1]+"&parentTableName=&saveDraft=";
...
function update(str){
 turl="/UserFunctionAction.do?tableName=dldlb&keyId="+str.split(",")[0]+"&moduleType="+moduleType+"&f_brother=&operation=7&winCurIndex="+winCurIndex+"&pageNo=&parentCode="+str.split(",")[1]+"&parentTableName=&saveDraft=";
...
function next(str){
 location.href="/UserFunctionQueryAction.do?tableName=dldlb&keyId="+str.split(",")[0]+"&parentCode="+str.split(",")[1]+"&operation=4&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&parentTableName=";
}
...
function addClass(parentCodev){
 ...
 usrc ='/UserFunctionAction.do?tableName=dldlb&parentCode='+parentCodev+'&operation=6&moduleType='+moduleTypeValue+'&f_brother='+f_brother+'&parentTableName=&winCurIndex='+winCurIndex;
...
function hurryTrans(str){
 ...
 hurryTransDialog("/vm/oa/workflow/noteModel.jsp?tableName=dldlb&moduleName="+encodeURI(ren)+"&keyId="+str.split(",")[0],winObj,"dialogWidth=565px;dialogHeight=350px");
}

function status(str){
 st = "/OAMyWorkFlow.do?keyId="+str.split(",")[0]+"&operation=5&tableName=dldlb&applyType=";
 var winWidth = 1024;
...
function check(str){ 
 ...
 jQuery.get("/UserFunctionQueryAction.do?tableName=dldlb&operation=25&ButtonType=saveCheckBill&varKeyIds="+str.split(",")[0]+defi,function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+str.split(",")[0]+"&tableName=dldlb&operation=82&fromPage=erp");
 ...
 location.href="/UserFunctionQueryAction.do?tableName=dldlb&moduleType="+moduleType+"&keyId="+str.split(",")[0]+"&f_brother=&operation=83&winCurIndex="+winCurIndex+"&pageNo=&parentTableName=&saveDraft=";
 }else if(str.split(",")[1]=="cancel"){
 if(confirm('确定撤回?')){
 location.href='/UserFunctionQueryAction.do?nextStep=cancel&moduleType='+moduleType+'&tableName=dldlb&lastNodeId='+str.split(",")[2]+'&keyId='+str.split(",")[0]+'&currNode='+str.split(",")[3]+'&f_brother=&operation=79&winCurIndex='+winCurIndex+'&pageNo=&parentTableName=&saveDraft='
 }
 }
}
...
function beforeOp(){
 ...
 document.getElementById("optionType").value="enable";
 document.form.submit();
}
...
function beforeEnOp(){
 ...
 document.getElementById("optionType").value="notenable";
 document.form.submit();
}
...
function billDetQuery(reportNumber,detTable,reportName){ 
 ...
 turl = '/ReportDataAction.do?reportNumber='+reportNumber+'&noback=true&detTable_list='+detTable+'&moduleType='+moduleType;
