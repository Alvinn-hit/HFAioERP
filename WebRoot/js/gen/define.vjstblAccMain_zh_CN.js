...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblAccMain&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblAccMain&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblAccMain&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblAccMain&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblAccDetailTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblAccDetail"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblAccDetail"){
...
function execStat2(){
 execStatOne('tblAccDetail_DebitAmount',2);
 execStatOne('tblAccDetail_LendAmount',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=2e9767a1_0811051341599530017&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=2e9767a1_0811051341599530017&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblAccMain\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblAccMain\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblAccDetail_DebitAmount',"tblAccDetail"); 
 delFireEventOne(rowId,tableName,'tblAccDetail_LendAmount',"tblAccDetail"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblAccDetail_DebitAmount',dis:'借方金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccDetail_LendAmount',dis:'贷方金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblAccMain",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAccMain&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAccMain&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"2e9767a1_0811051341599530017",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "PostingUser"){
 var saveField=n("PostingUser"); 
 saveField[0].value = "";
 var obj = m("tbl_tblEmployee_id");
 if(obj != null){
 obj.value = "";
 ...
 if(fieldName == "ReviewUser"){
 var saveField=n("ReviewUser"); 
 saveField[0].value = "";
 var obj = m("tbl_tblEmployee_id");
 if(obj != null){
 obj.value = "";
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=2e9767a1_0811051341599530017&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=2e9767a1_0811051341599530017&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=2e9767a1_0811051341599530017&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblAccMain"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblAccMain&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblAccMain&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 var z=r(0,'DebitAmount');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccDetail_DebitAmount'); 
 r(0,"DebitAmount")[p].value=f(rn(0,"DebitAmount",p),2);

 execStat(); 
 }
 }
 var z=r(0,'LendAmount');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccDetail_LendAmount'); 
 r(0,"LendAmount")[p].value=f(rn(0,"LendAmount",p),2);
...
childData[childCount]='tblAccDetail';
tabIsNulls[childCount]='0';
tabDiss[childCount]='会计凭证明细';
childCount++;
 var tblAccDetailData = new gridData();
 gridDatas[gridDataNum] = tblAccDetailData;
 gridDataNum++;
 var tblAccDetail=1;
 ...
 addCols(tblAccDetailData,'tblAccDetail_RecordComment','摘要','200',2,true,'',1000,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblAccDetail_RecordComment","摘要","any","",true,minValue,1000,false,'',true); 
 ...
 addCols(tblAccDetailData,'tblAccDetail_AccCode','会计科目','100',2,false,'',50,2,'tblAccDetail:AccCode:tblAccDetail_AccCode:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblAccDetail_AccCode","会计科目","any","",false,minValue,50,false,'',true);
 addCols(tblAccDetailData,'tblAccDetail_tblAccTypeInfo_AccFullName','科目全称','250',2,true,'',1000000,2,'tblAccDetail:AccCode:tblAccDetail_tblAccTypeInfo_AccFullName:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(tblAccDetailData,'tblAccDetail_DebitAmount','借方金额','100',1,true,'0',0,0,'','',"Amount","","2",'',"0",false); 
 putChildValidateItem("tblAccDetail_DebitAmount","借方金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblAccDetailData,'tblAccDetail_LendAmount','贷方金额','100',1,true,'0',0,0,'','',"Amount","","3",'',"0",false); 
 putChildValidateItem("tblAccDetail_LendAmount","贷方金额","double","",true,minValue,maxValue,true,'0',true); 
 ...
 addCols(tblAccDetailData,'tblAccDetail_ProjectCode','项目','1',2,true,'',50,-2,'','',"","","8",'',"2",true);
 putChildValidateItem("tblAccDetail_ProjectCode","项目","any","",true,minValue,50,false,'');
 addCols(tblAccDetailData,'tblAccDetail_tblProject_ProjectName','项目','150',2,true,'',1000000,2,'tblAccDetail:ProjectCode:tblAccDetail_tblProject_ProjectName:'+param4,'',"","","8",'',"2",false); 
 ...
 addCols(tblAccDetailData,'tblAccDetail_StockCode','仓库','100',2,true,'',50,-2,'','',"","","9",'',"2",true);
 putChildValidateItem("tblAccDetail_StockCode","仓库","any","",true,minValue,50,false,'');
 addCols(tblAccDetailData,'tblAccDetail_tblStock_StockFullName','仓库','75',2,true,'',1000000,2,'tblAccDetail:StockCode:tblAccDetail_tblStock_StockFullName:'+param4,'',"","","9",'',"2",false); 
 ...
 var param4="";
 param4=param4+":1会计科目@tblAccDetail_AccCode";
 
 addCols(tblAccDetailData,'tblAccDetail_CompanyCode','往来单位','100',2,true,'',50,-2,'','',"CompanyCodeIdentifier","","9",'',"2",true);
 putChildValidateItem("tblAccDetail_CompanyCode","往来单位","any","",true,minValue,50,false,'');
 addCols(tblAccDetailData,'tblAccDetail_tblCompany_ComFullName','往来单位','200',18,true,'',1000000,2,'tblAccDetail:CompanyCode:tblAccDetail_tblCompany_ComFullName:'+param4,'',"CompanyCodeIdentifier","","9",'',"2",false); 
 ...
 addCols(tblAccDetailData,'tblAccDetail_DepartmentCode','部门','70',2,true,'',50,-2,'','',"","","10",'',"2",true);
 putChildValidateItem("tblAccDetail_DepartmentCode","部门","any","",true,minValue,50,false,'');
 addCols(tblAccDetailData,'tblAccDetail_tblDepartment_DeptFullName','部门','80',2,true,'',1000000,2,'tblAccDetail:DepartmentCode:tblAccDetail_tblDepartment_DeptFullName:'+param4,'',"","","10",'',"2",false); 
 ...
 addCols(tblAccDetailData,'tblAccDetail_EmployeeID','经手人','70',2,true,'',50,-2,'','',"","","11",'',"2",true);
 putChildValidateItem("tblAccDetail_EmployeeID","经手人","any","",true,minValue,50,false,'');
 addCols(tblAccDetailData,'tblAccDetail_tblEmployee_EmpFullName','经手人','65',2,true,'',1000000,2,'tblAccDetail:EmployeeID:tblAccDetail_tblEmployee_EmpFullName:'+param4,'',"","","11",'',"2",false); 
 
 
 
 
 
 addCols(tblAccDetailData,'tblAccDetail_RefBillType','单据类型','0',2,true,'',100,-2,'','',"","","100",'',"100",false); 
 putChildValidateItem("tblAccDetail_RefBillType","单据类型","any","",true,minValue,100,false,''); 
 ...
 addCols(tblAccDetailData,'tblAccDetail_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblAccMain&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblAccMain,tblAccDetail,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAccMain&allTableName=tblAccMain,tblAccDetail,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAccMain&allTableName=tblAccDetail,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAccMain&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAccMain&allTableName=tblAccDetail,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=2e9767a1_0811051341599530017&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=2e9767a1_0811051341599530017&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=2e9767a1_0811051341599530017&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=2e9767a1_0811051341599530017&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblAccMain&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblAccMain&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblAccMain&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblAccMain&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblAccMain&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '凭证管理';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblAccMain&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
