...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblCombo&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblCombo&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblCombo&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblCombo&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblComboContentTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblComboDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblComboContent"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblComboDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblComboContent"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblComboDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblComboContent"; 
 tblName[1]="tblComboDet"; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblCombo\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblCombo\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblComboDet_ratio',dis:'比例(%)',digit:0,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblCombo",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblCombo&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblCombo&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblCombo&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblCombo&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblComboContent';
tabIsNulls[childCount]='0';
tabDiss[childCount]='套餐明细';
childCount++;
 var tblComboContentData = new gridData();
 gridDatas[gridDataNum] = tblComboContentData;
 gridDataNum++;
 var tblComboContent=1;
 
 
 
 
 addCols(tblComboContentData,'tblComboContent_Spec1','型号','100',2,true,'',50,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblComboContent_Spec1","型号","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblComboContentData,'tblComboContent_Condition','满足条件','100',0,true,'',0,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblComboContent_Condition","满足条件","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblComboContentData,'tblComboContent_Qty','赠送数量','100',0,true,'',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblComboContent_Qty","赠送数量","int","",true,minValue,maxValue,false,'',true); 
 
 
 addCols(tblComboContentData,'tblComboContent_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblComboDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='套餐礼品比例';
childCount++;
 var tblComboDetData = new gridData();
 gridDatas[gridDataNum] = tblComboDetData;
 gridDataNum++;
 var tblComboDet=1;
 ...
 addCols(tblComboDetData,'tblComboDet_Spec1','型号','100',2,false,'',100,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblComboDet_Spec1","型号","any","",false,minValue,100,false,'',true); 
 ...
 addCols(tblComboDetData,'tblComboDet_GoodsCode','商品名称','200',2,false,'',100,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("tblComboDet_GoodsCode","商品名称","any","",false,minValue,100,false,'');
 
 
 addCols(tblComboDetData,'tblComboDet_tblGoods_GoodsNumber','礼品编号','120',2,true,'',1000000,2,'tblComboDet:GoodsCode:tblComboDet_tblGoods_GoodsNumber:'+param4,'',"","","1",'',"2",false); 
 
 
 addCols(tblComboDetData,'tblComboDet_tblGoods_GoodsFullName','礼品名称','120',2,true,'',1000000,2,'tblComboDet:GoodsCode:tblComboDet_tblGoods_GoodsFullName:'+param4,'',"","","1",'',"2",false); 
 
 
 addCols(tblComboDetData,'tblComboDet_tblGoods_GoodsSpec','礼品规格','120',20,true,'',1000000,2,'tblComboDet:GoodsCode:tblComboDet_tblGoods_GoodsSpec:'+param4,'',"","","1",'',"2",false); 
 
 
 addCols(tblComboDetData,'tblComboDet_tblGoods_BaseUnit','单位','60',2,true,'',1000000,2,'tblComboDet:GoodsCode:tblComboDet_tblGoods_BaseUnit:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(tblComboDetData,'tblComboDet_BatchNo','批号','200',2,true,'',100,-2,'','',"","","2",'',"3",false); 
 putChildValidateItem("tblComboDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblComboDetData,'tblComboDet_Inch','厚度','200',2,true,'',100,-2,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblComboDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblComboDetData,'tblComboDet_Hue','颜色','200',2,true,'',100,-2,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblComboDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblComboDetData,'tblComboDet_yearNO','箱号','100',2,true,'',100,-2,'','',"","","5",'',"3",false); 
 putChildValidateItem("tblComboDet_yearNO","箱号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblComboDetData,'tblComboDet_ProDate','生产日期','200',2,true,'',10,-2,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblComboDet_ProDate","生产日期","any","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblComboDetData,'tblComboDet_Availably','保质期限','200',2,true,'',10,-2,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblComboDet_Availably","保质期限","any","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblComboDetData,'tblComboDet_Price','单价','200',1,true,'',0,-2,'','',"Qty","","8",'',"0",false); 
 putChildValidateItem("tblComboDet_Price","单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblComboDetData,'tblComboDet_ratio','比例(%)','80',1,false,'100',0,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblComboDet_ratio","比例(%)","double","",false,minValue,maxValue,true,'100',true); 
 
 
 addCols(tblComboDetData,'tblComboDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblCombo&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblCombo,tblComboContent,tblComboDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblCombo&allTableName=tblCombo,tblComboContent,tblComboDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblCombo&allTableName=tblComboContent,tblComboDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblCombo&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblCombo&allTableName=tblComboContent,tblComboDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblCombo&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblCombo&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblCombo&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblCombo&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblCombo&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '套餐管理';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblCombo&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
