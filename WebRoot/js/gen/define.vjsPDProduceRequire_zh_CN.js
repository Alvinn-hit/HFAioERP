...
{ 
 cFireEvent('PDProduceRequireDet_Qty',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDProduceRequire&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDProduceRequire&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDProduceRequire&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDProduceRequire&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDProduceRequireDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('PDProduceRequireDet_GoodsCode');
 ...
 var z=n('PDProduceRequireDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDProduceRequireDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDProduceRequireDet_Hue');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDProduceRequireDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDProduceRequireDet"){
 
 copyFields=n('PDProduceRequireDet_StockCode');
 ...
 copyFields=n('PDProduceRequireDet_GoodsCode');
 if((order > 1 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var PDProduceRequireDet_tblGoods_GoodsNumber = n('PDProduceRequireDet_tblGoods_GoodsNumber');
 if(typeof(PDProduceRequireDet_tblGoods_GoodsNumber) != 'undefined' && PDProduceRequireDet_tblGoods_GoodsNumber.length >index){
 PDProduceRequireDet_tblGoods_GoodsNumber[index].value = PDProduceRequireDet_tblGoods_GoodsNumber[index-1].value;
 }
 var PDProduceRequireDet_tblGoods_GoodsSpec = n('PDProduceRequireDet_tblGoods_GoodsSpec');
 if(typeof(PDProduceRequireDet_tblGoods_GoodsSpec) != 'undefined' && PDProduceRequireDet_tblGoods_GoodsSpec.length >index){
 PDProduceRequireDet_tblGoods_GoodsSpec[index].value = PDProduceRequireDet_tblGoods_GoodsSpec[index-1].value;
 }
 var PDProduceRequireDet_tblGoods_BaseUnit = n('PDProduceRequireDet_tblGoods_BaseUnit');
 if(typeof(PDProduceRequireDet_tblGoods_BaseUnit) != 'undefined' && PDProduceRequireDet_tblGoods_BaseUnit.length >index){
 PDProduceRequireDet_tblGoods_BaseUnit[index].value = PDProduceRequireDet_tblGoods_BaseUnit[index-1].value;
 }
 var PDProduceRequireDet_tblGoods_BarCode = n('PDProduceRequireDet_tblGoods_BarCode');
 if(typeof(PDProduceRequireDet_tblGoods_BarCode) != 'undefined' && PDProduceRequireDet_tblGoods_BarCode.length >index){
 PDProduceRequireDet_tblGoods_BarCode[index].value = PDProduceRequireDet_tblGoods_BarCode[index-1].value;
 }
 var PDProduceRequireDet_tblStocks_lastQty = n('PDProduceRequireDet_tblStocks_lastQty');
 if(typeof(PDProduceRequireDet_tblStocks_lastQty) != 'undefined' && PDProduceRequireDet_tblStocks_lastQty.length >index){
 PDProduceRequireDet_tblStocks_lastQty[index].value = PDProduceRequireDet_tblStocks_lastQty[index-1].value;
 }
 var PDProduceRequireDet_ViewVirtualStock_UseQty = n('PDProduceRequireDet_ViewVirtualStock_UseQty');
 if(typeof(PDProduceRequireDet_ViewVirtualStock_UseQty) != 'undefined' && PDProduceRequireDet_ViewVirtualStock_UseQty.length >index){
 PDProduceRequireDet_ViewVirtualStock_UseQty[index].value = PDProduceRequireDet_ViewVirtualStock_UseQty[index-1].value;
 }
 }
 copyFields=n('PDProduceRequireDet_SecQty');
 if((order > 15 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 $(copyFields[index]).change();
 }
 }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDProduceRequireDet"){
 ...
 copyFields=n('PDProduceRequireDet_SecQty'); 
 if( (order > 15 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 $(copyFields[index]).change();
 }
 }
...
function execStat2(){
 execStatOne('PDProduceRequireDet_Qty',4);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDProduceRequire\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDProduceRequire\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'PDProduceRequireDet_Qty',"PDProduceRequireDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'PDProduceRequireDet_Qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDProduceRequireDet_MRPQty',dis:'生产规划数',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDProduceRequireDet_WorkOrderQty',dis:'已下工令数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDProduceRequireDet_NoWorkOrderQty',dis:'未下工令数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDProduceRequireDet_PDInQty',dis:'缴库数',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDProduceRequireDet_NoPDInQty',dis:'未缴库数',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDProduceRequire",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDProduceRequire&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDProduceRequire&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDProduceRequire"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDProduceRequire&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDProduceRequire&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'PDProduceRequireDet_Qty'); 
 r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDProduceRequireDet_SecQty'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDProduceRequireDet_CostAmount'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDProduceRequireDet_CostPrice'); 
 r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),4);
...
childData[childCount]='PDProduceRequireDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='制造需求明细';
childCount++;
 var PDProduceRequireDetData = new gridData();
 gridDatas[gridDataNum] = PDProduceRequireDetData;
 gridDataNum++;
 var PDProduceRequireDet=1;
 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_StockCode','仓库','80',2,true,'Sess_StockCode',100,-2,'','',"","copy","0",'',"3",false); 
 putChildValidateItem("PDProduceRequireDet_StockCode","仓库","any","",true,minValue,100,true,'Sess_StockCode'); 
 ...
 param4=param4+":0仓库@PDProduceRequireDet_StockCode";
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_GoodsCode','料件名称','120',2,false,'',200,-2,'','',"","triggerOrCopy","1",'',"2",true);
 putChildValidateItem("PDProduceRequireDet_GoodsCode","料件名称","any","",false,minValue,200,false,'');
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'PDProduceRequireDet:GoodsCode:PDProduceRequireDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDProduceRequireDet:GoodsCode:PDProduceRequireDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDProduceRequireDet:GoodsCode:PDProduceRequireDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_tblGoods_BarCode','条码','100',2,true,'',1000000,2,'PDProduceRequireDet:GoodsCode:PDProduceRequireDet_tblGoods_BarCode:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_tblStocks_lastQty','实际库存','90',1,true,'',1000000,2,'PDProduceRequireDet:GoodsCode:PDProduceRequireDet_tblStocks_lastQty:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_ViewVirtualStock_UseQty','可用库存','90',1,true,'',1000000,2,'PDProduceRequireDet:GoodsCode:PDProduceRequireDet_ViewVirtualStock_UseQty:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","2",'',"3",false); 
 putChildValidateItem("PDProduceRequireDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","3",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","4",'',"2",false); 
 putChildValidateItem("PDProduceRequireDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_yearNO','箱号','100',2,true,'',100,-2,'','',"","triggerCopy","5",'',"3",false); 
 putChildValidateItem("PDProduceRequireDet_yearNO","箱号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","6",'',"2",false); 
 putChildValidateItem("PDProduceRequireDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","7",'',"2",false); 
 putChildValidateItem("PDProduceRequireDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_Qty','数量','60',1,false,'',0,0,'','',"Qty","","8",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_Qty","数量","double","",false,minValue,maxValue,false,'',true); 
 ...
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_ViewTotalQty','当前库存','80',1,true,'',0,-2,'','',"Price","","14",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_ViewTotalQty","当前库存","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","copy","15",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","16",'',"3",false); 
 putChildValidateItem("PDProduceRequireDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_EmployeeID','经手人','0',2,true,'Sess_UserId',100,-2,'','',"Qty","","17",'',"3",false); 
 putChildValidateItem("PDProduceRequireDet_EmployeeID","经手人","any","",true,minValue,100,true,'Sess_UserId'); 
 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_CostAmount','成本总额','60',1,true,'0',0,-2,'','',"Qty","","19",'',"100",false); 
 putChildValidateItem("PDProduceRequireDet_CostAmount","成本总额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_CostPrice','成本单价','60',1,true,'0',0,-2,'','',"priceIdentifier","","20",'',"3",false); 
 putChildValidateItem("PDProduceRequireDet_CostPrice","成本单价","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_DepartmentCode','部门','0',2,true,'Sess_DepartmentCode',100,-2,'','',"","","21",'',"3",false); 
 putChildValidateItem("PDProduceRequireDet_DepartmentCode","部门","any","",true,minValue,100,true,'Sess_DepartmentCode'); 
 ...
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_StartDate','开始生产日期','80',5,true,'',10,0,'','',"","","25",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_StartDate","开始生产日期","date","",true,minValue,10,false,'',true); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_SendDate','完工日期','80',5,true,'',10,0,'','',"","","26",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_SendDate","完工日期","date","",true,minValue,10,false,'',true); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_Remark','备注','100',2,true,'',8000,0,'','',"","","27",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_Remark","备注","any","BillPresentSampleType",true,minValue,8000,false,'',true); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_WorkOrderId','工令单Id','80',2,true,'',50,-2,'','',"","","28",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_WorkOrderId","工令单Id","any","",true,minValue,50,false,''); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_MRPQty','生产规划数','80',1,true,'',0,8,'','',"Qty","","29",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_MRPQty","生产规划数","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_WorkOrderQty','已下工令数量','80',1,true,'',0,8,'','',"","","30",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_WorkOrderQty","已下工令数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_NoWorkOrderQty','未下工令数量','80',1,true,'',0,8,'','',"","","31",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_NoWorkOrderQty","未下工令数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_PDInQty','缴库数','80',1,true,'',0,8,'','',"","","32",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_PDInQty","缴库数","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_NoPDInQty','未缴库数','80',1,true,'',0,8,'','',"Qty","","33",'',"0",false); 
 putChildValidateItem("PDProduceRequireDet_NoPDInQty","未缴库数","double","",true,minValue,maxValue,false,'',true); 
 
 
 addCols(PDProduceRequireDetData,'PDProduceRequireDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDProduceRequire&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDProduceRequire,PDProduceRequireDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDProduceRequire&allTableName=PDProduceRequire,PDProduceRequireDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDProduceRequire&allTableName=PDProduceRequireDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDProduceRequire&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDProduceRequire&allTableName=PDProduceRequireDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "PDProduceRequireDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDProduceRequire&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDProduceRequire&designId=210D9831581A4592860F7310448B&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDProduceRequire&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDProduceRequire&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDProduceRequire&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '制造需求单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDProduceRequire&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
