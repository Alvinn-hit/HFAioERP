...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblModuleFlow&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblModuleFlow&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblModuleFlow&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblModuleFlow&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblModuleFlowDetailreTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblReportNavigationTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblModuleFlowDetailre"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblReportNavigation"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblModuleFlowDetailre"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblReportNavigation"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblModuleFlowDetailre"; 
 tblName[1]="tblReportNavigation"; 
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=6d3441a2_1308081026952365241&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=6d3441a2_1308081026952365241&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblModuleFlow\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblModuleFlow\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblModuleFlow",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblModuleFlow&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblModuleFlow&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"6d3441a2_1308081026952365241",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=6d3441a2_1308081026952365241&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=6d3441a2_1308081026952365241&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=6d3441a2_1308081026952365241&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblModuleFlow"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblModuleFlow&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblModuleFlow&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblModuleFlowDetailre';
tabIsNulls[childCount]='0';
tabDiss[childCount]='模块导航功能列表';
childCount++;
 var tblModuleFlowDetailreData = new gridData();
 gridDatas[gridDataNum] = tblModuleFlowDetailreData;
 gridDataNum++;
 var tblModuleFlowDetailre=10;
 ...
 addCols(tblModuleFlowDetailreData,'tblModuleFlowDetailre_Link','链接地址','200',2,true,'',500,-2,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblModuleFlowDetailre_Link","链接地址","any","",true,minValue,500,false,''); 
 ...
 addCols(tblModuleFlowDetailreData,'tblModuleFlowDetailre_reportName','报表名称','200',2,true,'',50,-2,'','',"","","0",'',"4",true);
 putChildValidateItem("tblModuleFlowDetailre_reportName","报表名称","any","",true,minValue,50,false,'');
 addCols(tblModuleFlowDetailreData,'tblModuleFlowDetailre_tblModules_modelName','模块名称','100',2,true,'',1000000,2,'tblModuleFlowDetailre:reportName:tblModuleFlowDetailre_tblModules_modelName:'+param4,'',"","","0",'',"4",false); 
 
 
 
 addCols(tblModuleFlowDetailreData,'tblModuleFlowDetailre_Row','序号','100',0,true,'',30,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblModuleFlowDetailre_Row","序号","int","",true,minValue,30,false,'',true); 
 
 
 
 
 
 addCols(tblModuleFlowDetailreData,'tblModuleFlowDetailre_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblReportNavigation';
tabIsNulls[childCount]='0';
tabDiss[childCount]='模块导航报表';
childCount++;
 var tblReportNavigationData = new gridData();
 gridDatas[gridDataNum] = tblReportNavigationData;
 gridDataNum++;
 var tblReportNavigation=10;
 
 
 
 
 addCols(tblReportNavigationData,'tblReportNavigation_Link','链接地址','200',2,true,'',500,-2,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblReportNavigation_Link","链接地址","any","",true,minValue,500,false,''); 
 ...
 addCols(tblReportNavigationData,'tblReportNavigation_reportName','报表名称','200',2,true,'',50,-2,'','',"","","0",'',"0",true);
 putChildValidateItem("tblReportNavigation_reportName","报表名称","any","",true,minValue,50,false,'');
 addCols(tblReportNavigationData,'tblReportNavigation_tblModules_modelName','模块名称','100',2,true,'',1000000,2,'tblReportNavigation:reportName:tblReportNavigation_tblModules_modelName:'+param4,'',"","","0",'',"0",false); 
 
 
 
 addCols(tblReportNavigationData,'tblReportNavigation_Row','序号','100',0,true,'',30,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblReportNavigation_Row","序号","int","",true,minValue,30,false,'',true); 
 ...
 addCols(tblReportNavigationData,'tblReportNavigation_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblModuleFlow&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblModuleFlow,tblModuleFlowDetailre,tblReportNavigation,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblModuleFlow&allTableName=tblModuleFlow,tblModuleFlowDetailre,tblReportNavigation,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblModuleFlow&allTableName=tblModuleFlowDetailre,tblReportNavigation,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblModuleFlow&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblModuleFlow&allTableName=tblModuleFlowDetailre,tblReportNavigation,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=6d3441a2_1308081026952365241&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=6d3441a2_1308081026952365241&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=6d3441a2_1308081026952365241&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=6d3441a2_1308081026952365241&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblModuleFlow&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=6d3441a2_1308081026952365241&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblModuleFlow&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblModuleFlow&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblModuleFlow&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblModuleFlow&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '模块导航设置';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblModuleFlow&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
