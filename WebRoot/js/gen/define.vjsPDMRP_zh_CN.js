...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDMRP&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDMRP&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDMRP&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDMRP&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDMRPBillTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDMRPGoodsTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDMRPBill"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="PDMRPGoods"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDMRPBill"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDMRPGoods"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDMRP\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDMRP\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'needQty',dis:'需求数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'PDMRPBill_qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDMRPGoods_BuyApplyQty',dis:'以申请采购数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDMRPGoods_qty',dis:'单位数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDMRPGoods_lossRate',dis:'单位损耗',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDMRPGoods_totalQty',dis:'需求量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDMRPGoods_totalLoss',dis:'备品量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDMRPGoods_StockQty',dis:'库存量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDMRPGoods_OrderQty',dis:'在途量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDMRPGoods_ProduceQty',dis:'在制量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDMRPGoods_PossesQty',dis:'占用量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDMRPGoods_lackQty',dis:'缺料量',digit:0,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDMRP",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDMRP&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDMRP&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDMRP&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDMRP&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='PDMRPBill';
tabIsNulls[childCount]='0';
tabDiss[childCount]='物料需求单据明细';
childCount++;
 var PDMRPBillData = new gridData();
 gridDatas[gridDataNum] = PDMRPBillData;
 gridDataNum++;
 var PDMRPBill=1;
 
 
 addCols(PDMRPBillData,'PDMRPBill_BillType','单据类型','200',2,true,'',50,1,';0:销售订单;1:制造需求单','',"","","0",'',"1",false);
 putChildValidateItem("PDMRPBill_BillType","单据类型","any","",true,minValue,50,false,'',true); 
 
 
 
 
 
 addCols(PDMRPBillData,'PDMRPBill_BillId','单据编号','200',2,true,'',50,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("PDMRPBill_BillId","单据编号","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(PDMRPBillData,'PDMRPBill_qty','数量','200',1,true,'',0,0,'','',"Qty","","2",'',"0",false); 
 putChildValidateItem("PDMRPBill_qty","数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPBillData,'PDMRPBill_BillNo','单据编号','200',2,true,'',50,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("PDMRPBill_BillNo","单据编号","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(PDMRPBillData,'PDMRPBill_BillDate','单据日期','200',5,true,'',10,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("PDMRPBill_BillDate","单据日期","date","",true,minValue,10,false,'',true); 
 
 
 
 addCols(PDMRPBillData,'PDMRPBill_DepartmentCode','部门','200',2,true,'',50,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("PDMRPBill_DepartmentCode","部门","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(PDMRPBillData,'PDMRPBill_EmployeeId','经手人','200',2,true,'',50,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("PDMRPBill_EmployeeId","经手人","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(PDMRPBillData,'PDMRPBill_CompanyCode','客户名称','200',2,true,'',50,0,'','',"","","7",'',"0",false); 
 putChildValidateItem("PDMRPBill_CompanyCode","客户名称","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(PDMRPBillData,'PDMRPBill_createBy','制单人','200',2,true,'',50,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("PDMRPBill_createBy","制单人","any","",true,minValue,50,false,'',true); 
 
 
 addCols(PDMRPBillData,'PDMRPBill_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='PDMRPGoods';
tabIsNulls[childCount]='0';
tabDiss[childCount]='物料需求物料明细';
childCount++;
 var PDMRPGoodsData = new gridData();
 gridDatas[gridDataNum] = PDMRPGoodsData;
 gridDataNum++;
 var PDMRPGoods=1;
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_GoodsCode','物料','200',2,true,'',100,0,'','',"","","0",'',"0",false); 
 putChildValidateItem("PDMRPGoods_GoodsCode","物料","any","",true,minValue,100,false,'',true); 
 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_BomDetId','BOM物料明细','200',2,true,'',50,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("PDMRPGoods_BomDetId","BOM物料明细","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_isOpen','是否展开','200',0,true,'0',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("PDMRPGoods_isOpen","是否展开","int","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_BomId','BOM单号','200',2,true,'',50,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("PDMRPGoods_BomId","BOM单号","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_BuyApplyQty','以申请采购数量','200',1,true,'',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("PDMRPGoods_BuyApplyQty","以申请采购数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_childBomId','子半成品Bom','200',2,true,'',50,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("PDMRPGoods_childBomId","子半成品Bom","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_qty','单位数量','200',1,true,'',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("PDMRPGoods_qty","单位数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_lossRate','单位损耗','200',1,true,'',0,0,'','',"","","7",'',"0",false); 
 putChildValidateItem("PDMRPGoods_lossRate","单位损耗","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_totalQty','需求量','200',1,true,'',0,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("PDMRPGoods_totalQty","需求量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_totalLoss','备品量','200',1,true,'',0,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("PDMRPGoods_totalLoss","备品量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_StockQty','库存量','200',1,true,'',0,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("PDMRPGoods_StockQty","库存量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_OrderQty','在途量','200',1,true,'',0,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("PDMRPGoods_OrderQty","在途量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_ProduceQty','在制量','200',1,true,'',0,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("PDMRPGoods_ProduceQty","在制量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_PossesQty','占用量','200',1,true,'',0,0,'','',"","","13",'',"0",false); 
 putChildValidateItem("PDMRPGoods_PossesQty","占用量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_lackQty','缺料量','200',1,true,'',0,0,'','',"","","14",'',"0",false); 
 putChildValidateItem("PDMRPGoods_lackQty","缺料量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_attrType','商品属性','200',0,true,'',0,0,'','',"","","15",'',"0",false); 
 putChildValidateItem("PDMRPGoods_attrType","商品属性","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_alone','独立工令','200',2,true,'',10,0,'','',"","","16",'',"0",false); 
 putChildValidateItem("PDMRPGoods_alone","独立工令","any","",true,minValue,10,false,'',true); 
 
 
 addCols(PDMRPGoodsData,'PDMRPGoods_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDMRP&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDMRP,PDMRPBill,PDMRPGoods,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDMRP&allTableName=PDMRP,PDMRPBill,PDMRPGoods,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDMRP&allTableName=PDMRPBill,PDMRPGoods,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDMRP&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDMRP&allTableName=PDMRPBill,PDMRPGoods,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDMRP&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDMRP&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDMRP&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDMRP&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDMRP&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '物料需求计算';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDMRP&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
