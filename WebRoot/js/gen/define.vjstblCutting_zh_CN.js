...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblCutting&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblCutting&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblCutting&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblCutting&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblCuttingDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblCuttingDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblCuttingDet"){
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=f5c4a28d_1404291059505150103&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=f5c4a28d_1404291059505150103&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblCutting\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function selectPIC(btnId){
 ...
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblCutting\",\"PIC\");'>删除</a><em>"+str.substring(8)+"</em></div><div><a href=\"/ReadFile?type=PIC&fileName="+encodeURI(str)+"\" target=\"_blank\"><img src=\"/ReadFile?type=PIC&fileName="+encodeURI(str)+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 }); 
 　　　　　 }
 　　　 }
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblCutting\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblCuttingDet_Amount',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblCuttingDet_Price',dis:'单价',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblCutting",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblCutting&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblCutting&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"f5c4a28d_1404291059505150103",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f5c4a28d_1404291059505150103&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f5c4a28d_1404291059505150103&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=f5c4a28d_1404291059505150103&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblCutting"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblCutting&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblCutting&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblCuttingDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='下料单明细表　　(根据公司最新规定，下料单不再提供辅材、耗材的自主填写。主材、辅材及耗材，材料总计划明细表未有的，下料之前必须做材料总计划的变更，下料单只能引用材料总计划内的材料方能下料。)';
childCount++;
 var tblCuttingDetData = new gridData();
 gridDatas[gridDataNum] = tblCuttingDetData;
 gridDataNum++;
 var tblCuttingDet=1;
 ...
 param4=param4+":1材料类型@Type"; 
 
 addCols(tblCuttingDetData,'tblCuttingDet_anDet_Bil','材料编码','110',0,true,'',1000000,2,'tblCuttingDet:MaterialDetId:tblCuttingDet_anDet_Bil:'+param4,"","","","0","","3",false);
   ...
   addCols(tblCuttingDetData,'tblCuttingDet_WareName','材料名称','150',2,true,'',200,8,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblCuttingDet_WareName","材料名称","any","",true,minValue,200,false,'',true); 
 ...
 addCols(tblCuttingDetData,'tblCuttingDet_ialPlanDet_Spec','规格/型号','100',0,true,'',1000000,2,'tblCuttingDet:MaterialDetId:tblCuttingDet_ialPlanDet_Spec:'+param4,"","","","0","","3",false);
 ...
 addCols(tblCuttingDetData,'tblCuttingDet_alanDet_Bra','品牌','100',0,true,'',1000000,2,'tblCuttingDet:MaterialDetId:tblCuttingDet_alanDet_Bra:'+param4,"","","","0","","3",false);
 ...
 addCols(tblCuttingDetData,'tblCuttingDet_allanDet_Un','计量单位','60',0,true,'',1000000,2,'tblCuttingDet:MaterialDetId:tblCuttingDet_allanDet_Un:'+param4,"","","","0","","3",false);
   ...
   addCols(tblCuttingDetData,'tblCuttingDet_Amount','数量','100',1,true,'0',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblCuttingDet_Amount","数量","double","",true,minValue,maxValue,true,'0',true); 
   addCols(tblCuttingDetData,'tblCuttingDet_Position','使用位置','150',2,true,'',200,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblCuttingDet_Position","使用位置","any","",true,minValue,200,false,'',true); 
   addCols(tblCuttingDetData,'tblCuttingDet_Remark','备注','150',2,true,'',500,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblCuttingDet_Remark","备注","any","",true,minValue,500,false,'',true); 
   addCols(tblCuttingDetData,'tblCuttingDet_Price','单价','100',1,true,'',0,8,'','',"","","7",'',"3",false); 
 putChildValidateItem("tblCuttingDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
   addCols(tblCuttingDetData,'tblCuttingDet_ArriveDate','预计到场时间','100',5,false,'',50,0,'','',"","","9",'',"3",false); 
 putChildValidateItem("tblCuttingDet_ArriveDate","预计到场时间","date","",false,minValue,50,false,'',true); 
   addCols(tblCuttingDetData,'tblCuttingDet_Supplier','供应商','100',2,true,'',100,8,'','',"","","14",'',"0",false); 
 putChildValidateItem("tblCuttingDet_Supplier","供应商","any","",true,minValue,100,false,'',true); 
   addCols(tblCuttingDetData,'tblCuttingDet_BuyBrand','采购品牌','100',2,true,'',100,8,'','',"","","15",'',"0",false); 
 putChildValidateItem("tblCuttingDet_BuyBrand","采购品牌","any","",true,minValue,100,false,'',true); 
 var param4="";
 addCols(tblCuttingDetData,'tblCuttingDet_MaterialDetId','材料总计划明细ID','100',2,true,'-1',200,-2,'','','',"",'','',"3",false);
 putChildValidateItem("tblCuttingDet_MaterialDetId","材料总计划明细ID","any","",true,minValue,200,true,'-1',true);
            addCols(tblCuttingDetData,'tblCuttingDet_WareNo','材料编码','100',2,true,'',50,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblCuttingDet_WareNo","材料编码","any","",true,minValue,50,false,''); 
      addCols(tblCuttingDetData,'tblCuttingDet_Specification','规格/型号','100',2,true,'',100,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblCuttingDet_Specification","规格/型号","any","",true,minValue,100,false,''); 
   addCols(tblCuttingDetData,'tblCuttingDet_Brand','品牌','100',2,true,'',100,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblCuttingDet_Brand","品牌","any","",true,minValue,100,false,''); 
   addCols(tblCuttingDetData,'tblCuttingDet_WareUnit','计量单位','100',2,true,'',50,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblCuttingDet_WareUnit","计量单位","any","",true,minValue,50,false,''); 
        addCols(tblCuttingDetData,'tblCuttingDet_ZBPrice','材料中标价','100',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblCuttingDet_ZBPrice","材料中标价","double","",true,minValue,maxValue,true,'0'); 
        addCols(tblCuttingDetData,'tblCuttingDet_isBuyOrder','已下订单','100',2,true,'0',10,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblCuttingDet_isBuyOrder","已下订单","any","",true,minValue,10,true,'0'); 
      addCols(tblCuttingDetData,'tblCuttingDet_MaterialDetCutting','已更新材料总计划明细已下料数量','100',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblCuttingDet_MaterialDetCutting","已更新材料总计划明细已下料数量","any","",true,minValue,100,false,''); 
        addCols(tblCuttingDetData,'tblCuttingDet_BuyOrderQty','已下采购订单数','100',1,true,'0',0,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblCuttingDet_BuyOrderQty","已下采购订单数","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblCuttingDetData,'tblCuttingDet_AskPrice','是否已询价','100',1,true,'0',0,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblCuttingDet_AskPrice","是否已询价","double","",true,minValue,maxValue,true,'0'); 
 
 addCols(tblCuttingDetData,'tblCuttingDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblCutting&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblCutting,tblCuttingDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblCutting&allTableName=tblCutting,tblCuttingDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblCutting&allTableName=tblCuttingDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblCutting&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblCutting&allTableName=tblCuttingDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f5c4a28d_1404291059505150103&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f5c4a28d_1404291059505150103&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=f5c4a28d_1404291059505150103&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=f5c4a28d_1404291059505150103&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblCutting&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=f5c4a28d_1404291059505150103&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblCutting&designId=4f02f52b_1504291122335419533&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblCutting&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblCutting&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblCutting&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '下料单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblCutting&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
