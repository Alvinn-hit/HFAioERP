...
{ 
 cFireEvent('PDCostEstimateDet_Qty',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDCostEstimate&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDCostEstimate&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDCostEstimate&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDCostEstimate&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDCostEstimateDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDCostEstimateDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDCostEstimateDet"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDCostEstimate\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDCostEstimate\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'PDCostEstimateDet_Qty',"PDCostEstimateDet"); 
 delFireEventOne(rowId,tableName,'PDCostEstimateDet_Price',"PDCostEstimateDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'WorkRate',dis:'标准工时率(元/时)',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'materialCost',dis:'材料成本',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'assembleCost',dis:'组装成本',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'lossRate',dis:'损耗率%',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'lossCost',dis:'损耗',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'outCost',dis:'出厂成本',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'saleRate',dis:'销售费率%',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'saleCost',dis:'销售费用',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'manageRate',dis:'管理费系数%',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'manageCost',dis:'管理费用',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'trafficRate',dis:'运输费率%',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'trafficCost',dis:'运输费用',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'productCost',dis:'产品成本',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'reactionRate',dis:'加成率%',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'reactionCost',dis:'加成',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'mouldCost',dis:'模具摊提',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'noTaxCost',dis:'未税总价合计',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'taxRate',dis:'税率%',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'taxCost',dis:'含税总价合计',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'PDCostEstimateDet_Qty',dis:'用量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDCostEstimateDet_Price',dis:'本币未税单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDCostEstimateDet_Amount',dis:'金额',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDCostEstimateDet_WorkTime',dis:'标准工时(秒)',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDCostEstimateDet_WorkRate',dis:'标准工时率(元/时)',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDCostEstimateDet_WorkAmount',dis:'组装成本',digit:6,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDCostEstimate",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDCostEstimate&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDCostEstimate&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDCostEstimate"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDCostEstimate&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDCostEstimate&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 var z=n('materialCost');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('lossCost')[0].value=f((nn('assembleCost',0)+nn('materialCost',0))*nn('lossRate',0)/100
,6);

n('outCost')[0].value=f(nn('assembleCost',0)+nn('materialCost',0)+nn('lossCost',0),6);

n('saleCost')[0].value=f(nn('outCost',0)*(1+nn('saleRate',0)/100),6);

n('manageCost')[0].value=f(nn('assembleCost',0)*nn('manageRate',0)/100,6);

n('trafficCost')[0].value=f(nn('saleCost',0)*nn('trafficRate',0)/100,6);

n('productCost')[0].value=f(nn('saleCost',0)+nn('manageCost',0)+nn('trafficCost',0),6);

n('reactionCost')[0].value=f((nn('saleCost',0)+nn('manageCost',0))*nn('reactionRate',0)/100,6);

n('noTaxCost')[0].value=f(nn('productCost',0)+nn('reactionCost',0)+nn('mouldCost',0),6);

n('taxCost')[0].value=f(nn('noTaxCost',0)*(1+nn('taxRate',0)/100),6);

 
 }
 } 

 var z=n('assembleCost');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('lossCost')[0].value=f((nn('assembleCost',0)+nn('materialCost',0))*nn('lossRate',0)/100
,6);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDCostEstimateDet_Qty'); 
 r(0,"Amount")[p].value=f(Number(rn(0,"Price",p))*Number(rn(0,"Qty",p)),6);
n('materialCost')[0].value=f(sm('PDCostEstimateDet_Amount'),6);
 if(document.createEvent){ var evt = document.createEvent('HTMLEvents'); evt.initEvent('change',true,false);n('materialCost')[0].dispatchEvent(evt);
}else{ n('materialCost')[0].fireEvent('onChange'); } 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDCostEstimateDet_Price'); 
 r(0,"Amount")[p].value=f(Number(rn(0,"Price",p))*Number(rn(0,"Qty",p)),6);
n('materialCost')[0].value=f(sm('PDCostEstimateDet_Amount'),6);
 if(document.createEvent){ var evt = document.createEvent('HTMLEvents'); evt.initEvent('change',true,false);n('materialCost')[0].dispatchEvent(evt);
}else{ n('materialCost')[0].fireEvent('onChange'); } 
...
childData[childCount]='PDCostEstimateDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='材料成本';
childCount++;
 var PDCostEstimateDetData = new gridData();
 gridDatas[gridDataNum] = PDCostEstimateDetData;
 gridDataNum++;
 var PDCostEstimateDet=1;
 ...
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_GoodsCode','物料名称','200',2,true,'',100,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("PDCostEstimateDet_GoodsCode","物料名称","any","",true,minValue,100,false,'');
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,8,'PDCostEstimateDet:GoodsCode:PDCostEstimateDet_tblGoods_GoodsNumber:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_tblGoods_GoodsFullName','物料名称','150',2,true,'',1000000,8,'PDCostEstimateDet:GoodsCode:PDCostEstimateDet_tblGoods_GoodsFullName:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,8,'PDCostEstimateDet:GoodsCode:PDCostEstimateDet_tblGoods_GoodsSpec:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,8,'PDCostEstimateDet:GoodsCode:PDCostEstimateDet_tblGoods_BaseUnit:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_tblGoods_attrType','来源码','80',2,true,'',1000000,8,'PDCostEstimateDet:GoodsCode:PDCostEstimateDet_tblGoods_attrType:'+param4,'',"","","0",'',"2",false); 
 
 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_attrType','来源码','100',0,true,'',0,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("PDCostEstimateDet_attrType","来源码","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_Qty','用量','100',1,true,'',0,8,'','',"Qty","","2",'',"0",false); 
 putChildValidateItem("PDCostEstimateDet_Qty","用量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_Price','本币未税单价','100',1,true,'',0,0,'','',"Price","","3",'',"0",false); 
 putChildValidateItem("PDCostEstimateDet_Price","本币未税单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_Amount','金额','100',1,true,'',0,8,'','',"Price","","4",'',"0",false); 
 putChildValidateItem("PDCostEstimateDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_WorkTime','标准工时(秒)','100',1,true,'',0,8,'','',"Price","","5",'',"0",false); 
 putChildValidateItem("PDCostEstimateDet_WorkTime","标准工时(秒)","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_WorkRate','标准工时率(元/时)','100',1,true,'',0,8,'','',"Price","","6",'',"0",false); 
 putChildValidateItem("PDCostEstimateDet_WorkRate","标准工时率(元/时)","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_WorkAmount','组装成本','100',1,true,'',0,8,'','',"Price","","7",'',"0",false); 
 putChildValidateItem("PDCostEstimateDet_WorkAmount","组装成本","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_isOpen','是否展开','200',0,true,'1',0,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("PDCostEstimateDet_isOpen","是否展开","int","",true,minValue,maxValue,true,'1',true); 
 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_detCode','明细编号','200',2,true,'',50,-2,'','',"","","9",'',"0",false); 
 putChildValidateItem("PDCostEstimateDet_detCode","明细编号","any","",true,minValue,50,false,''); 
 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_ismaterial','是否物料','200',0,true,'0',0,-2,'','',"","","10",'',"0",false); 
 putChildValidateItem("PDCostEstimateDet_ismaterial","是否物料","int","",true,minValue,maxValue,true,'0'); 
 
 
 addCols(PDCostEstimateDetData,'PDCostEstimateDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDCostEstimate&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDCostEstimate,PDCostEstimateDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDCostEstimate&allTableName=PDCostEstimate,PDCostEstimateDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDCostEstimate&allTableName=PDCostEstimateDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDCostEstimate&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDCostEstimate&allTableName=PDCostEstimateDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "PDCostEstimateDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDCostEstimate&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDCostEstimate&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDCostEstimate&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDCostEstimate&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDCostEstimate&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '成本预核算';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDCostEstimate&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
