...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblWatchBarber&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblWatchBarber&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblWatchBarber&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblWatchBarber&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchBarberDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchBarberDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblWatchBarberDet"){
 
  }
 if(typeof(tableName)!="undefined" && tableName=="tblWatchBarberDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchBarberDet"){
 
  }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchBarberDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblWatchBarberDet"; 
 tblName[1]="tblWatchBarberDet1"; 
...
function execStat2(){
 execStatOne('tblWatchBarberDet_Spec1',4);
 execStatOne('tblWatchBarberDet_Spec2',4);
 execStatOne('tblWatchBarberDet_Spec3',4);
 execStatOne('tblWatchBarberDet_Spec4',4);
 execStatOne('tblWatchBarberDet_Qty',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=b103e944_1306061720235781327&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=b103e944_1306061720235781327&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblWatchBarber\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblWatchBarber\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblWatchBarberDet_Spec1',dis:'巴的直径(mm)',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchBarberDet_Spec2',dis:'巴的厚度(mm)',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchBarberDet_Spec3',dis:'巴管外露(mm)',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchBarberDet_Spec4',dis:'巴芯直径(mm)',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchBarberDet_Qty',dis:'用量',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblWatchBarber",
...
function quoteDraft(tableName,moduleType){
 ...
 var urlstr = "/UserFunctionQueryAction.do?tableName="+tableName+"&src=menu&moduleType="+moduleType+"&parentTableName=tblSampleOrder&draftQuery=draftPop&popupWin=QuoteDraft";
...
function exeQuoteDraft(varStr,moduleType){
 ...
 window.location.href="/UserFunctionAction.do?tableName="+varTableName+"&operation=7&moduleType="+moduleType+"&parentTableName=tblSampleOrder&f_brother=68fc380d_1408131539576500232&keyId="+varStr+"&saveDraft=quoteDraft&winCurIndex="+winCurIndex+"&popWinName="+$("#popWinName").val() ; 
}
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchBarber&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchBarber&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"b103e944_1306061720235781327",
 MOOP:"add",
...
function resetSubmit(fieldName){
 if(fieldName == "SampleOrderID"){
 var saveField=n("SampleOrderID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblSampleOrder_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("f_brother"); 
 saveField[0].value = "";
 var obj = m("tbl_tblSampleOrder_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblSampleOrder_BillNo");
 saveField[0].value = ""; 
 m("tbl_tblSampleOrder_BillNo").value = "" ;
 }
 ...
 var saveField=n("Linkman"); 
 saveField[0].value = "";
 var obj = m("tbl_ViewCompanyTotal_ComContactor");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("ClientNum"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_ClientNum");
 if(obj != null){
 obj.value = "";
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=b103e944_1306061720235781327&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=b103e944_1306061720235781327&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=b103e944_1306061720235781327&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblWatchBarber"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblWatchBarber&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchBarber&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblWatchBarberDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='巴的物料明细';
childCount++;
 var tblWatchBarberDetData = new gridData();
 gridDatas[gridDataNum] = tblWatchBarberDetData;
 gridDataNum++;
 var tblWatchBarberDet=1;
 ...
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_GoodsCode','物料名称','100',2,true,'',1000,-2,'','',"","","1",'',"3",false); 
 putChildValidateItem("tblWatchBarberDet_GoodsCode","物料名称","any","",true,minValue,1000,false,''); 
 ...
 var param4="";
 param4=param4+":1样板编号@SampleOrderID"; 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_GoodsNumber','物料编号','200',2,true,'',500,2,'tblWatchBarberDet:GoodsNumber:tblWatchBarberDet_GoodsNumber:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblWatchBarberDet_GoodsNumber","物料编号","any","",true,minValue,500,false,'',true);
 
 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_BarberNum','巴的型号','120',2,true,'',100,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet_BarberNum","巴的型号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_Spec1','巴的直径(mm)','80',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet_Spec1","巴的直径(mm)","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_Spec2','巴的厚度(mm)','80',1,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet_Spec2","巴的厚度(mm)","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_Spec3','巴管外露(mm)','80',1,true,'0',0,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet_Spec3","巴管外露(mm)","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_Spec4','巴芯直径(mm)','80',1,true,'0',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet_Spec4","巴芯直径(mm)","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_Request1','防水要求','120',2,true,'',200,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet_Request1","防水要求","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_Request2','打磨要求','120',2,true,'',200,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet_Request2","打磨要求","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_Request3','电镀/电染要求','120',2,true,'',200,0,'','',"","","13",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet_Request3","电镀/电染要求","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_Qty','用量','80',1,true,'1',0,0,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet_Qty","用量","double","",true,minValue,maxValue,true,'1',true); 
 
 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 
 addCols(tblWatchBarberDetData,'tblWatchBarberDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblWatchBarberDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='巴的变更明细';
childCount++;
 var tblWatchBarberDet1Data = new gridData();
 gridDatas[gridDataNum] = tblWatchBarberDet1Data;
 gridDataNum++;
 var tblWatchBarberDet1=1;
 ...
 addCols(tblWatchBarberDet1Data,'tblWatchBarberDet1_ChangeContent','变更内容','200',2,false,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet1_ChangeContent","变更内容","any","",false,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchBarberDet1Data,'tblWatchBarberDet1_ChangeDate','变更日期','90',5,false,'2013-12-24',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet1_ChangeDate","变更日期","date","",false,minValue,10,true,'2013-12-24',true); 
 
 
 
 addCols(tblWatchBarberDet1Data,'tblWatchBarberDet1_ChangePer','变更人员','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet1_ChangePer","变更人员","any","",true,minValue,100,false,'',true); 
 
 
 
 
 addCols(tblWatchBarberDet1Data,'tblWatchBarberDet1_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchBarberDet1_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchBarberDet1Data,'tblWatchBarberDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblWatchBarber&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=68fc380d_1408131539576500232&noback="+noback+"&parentCode=&operation=6&parentTableName=tblSampleOrder&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblWatchBarber,tblWatchBarberDet,tblWatchBarberDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchBarber&allTableName=tblWatchBarber,tblWatchBarberDet,tblWatchBarberDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchBarber&allTableName=tblWatchBarberDet,tblWatchBarberDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchBarber&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchBarber&allTableName=tblWatchBarberDet,tblWatchBarberDet1,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=b103e944_1306061720235781327&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=b103e944_1306061720235781327&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblWatchBarberDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=b103e944_1306061720235781327&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=b103e944_1306061720235781327&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblWatchBarber&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblWatchBarber&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblWatchBarber&saveDraft=&&keyId="+keyId+
 "&parentTableName=tblSampleOrder&moduleType="+moduleType+"&f_brother="+f_brother+"&noback="+noback+"&winCurIndex="+winCurIndex;
 form.submit();
}
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchBarber&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblWatchBarber&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '巴的';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblWatchBarber&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
