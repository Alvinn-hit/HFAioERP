...
{ 
 cFireEvent('tblAllotApplicationDet_Qty','0'); 
 cFireEvent('tblAllotApplicationDet_Price','0'); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblAllotApplication&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblAllotApplication&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblAllotApplication&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblAllotApplication&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblAllotApplicationDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblAllotApplicationDet_GoodsCode');
 ...
 var z=n('tblAllotApplicationDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblAllotApplicationDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblAllotApplicationDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblAllotApplicationDet_Hue');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblAllotApplicationDet"){
 
 copyFields=n('tblAllotApplicationDet_StockCode');
 ...
 var tblAllotApplicationDet_tblStock_StockFullName = n('tblAllotApplicationDet_tblStock_StockFullName');
 if(typeof(tblAllotApplicationDet_tblStock_StockFullName) != 'undefined' && tblAllotApplicationDet_tblStock_StockFullName.length >index){
 tblAllotApplicationDet_tblStock_StockFullName[index].value = tblAllotApplicationDet_tblStock_StockFullName[index-1].value;
 }
 }
 copyFields=n('tblAllotApplicationDet_InStockCode');
 if((order > 4 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && ((copyFields[index].value ==0) ||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 var tblAllotApplicationDet_ViewtblStocksChild_StockFullName = n('tblAllotApplicationDet_ViewtblStocksChild_StockFullName');
 if(typeof(tblAllotApplicationDet_ViewtblStocksChild_StockFullName) != 'undefined' && tblAllotApplicationDet_ViewtblStocksChild_StockFullName.length >index){
 tblAllotApplicationDet_ViewtblStocksChild_StockFullName[index].value = tblAllotApplicationDet_ViewtblStocksChild_StockFullName[index-1].value;
 }
 }
 copyFields=n('tblAllotApplicationDet_GoodsCode');
 ...
 copyFields[index].value = copyFields[index-1].value;
 var tblAllotApplicationDet_tblGoods_GoodsFullName = n('tblAllotApplicationDet_tblGoods_GoodsFullName');
 if(typeof(tblAllotApplicationDet_tblGoods_GoodsFullName) != 'undefined' && tblAllotApplicationDet_tblGoods_GoodsFullName.length >index){
 tblAllotApplicationDet_tblGoods_GoodsFullName[index].value = tblAllotApplicationDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblAllotApplicationDet_tblGoods_GoodsNumber = n('tblAllotApplicationDet_tblGoods_GoodsNumber');
 if(typeof(tblAllotApplicationDet_tblGoods_GoodsNumber) != 'undefined' && tblAllotApplicationDet_tblGoods_GoodsNumber.length >index){
 tblAllotApplicationDet_tblGoods_GoodsNumber[index].value = tblAllotApplicationDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblAllotApplicationDet_tblGoods_GoodsSpec = n('tblAllotApplicationDet_tblGoods_GoodsSpec');
 if(typeof(tblAllotApplicationDet_tblGoods_GoodsSpec) != 'undefined' && tblAllotApplicationDet_tblGoods_GoodsSpec.length >index){
 tblAllotApplicationDet_tblGoods_GoodsSpec[index].value = tblAllotApplicationDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblAllotApplicationDet_tblGoods_BaseUnit = n('tblAllotApplicationDet_tblGoods_BaseUnit');
 if(typeof(tblAllotApplicationDet_tblGoods_BaseUnit) != 'undefined' && tblAllotApplicationDet_tblGoods_BaseUnit.length >index){
 tblAllotApplicationDet_tblGoods_BaseUnit[index].value = tblAllotApplicationDet_tblGoods_BaseUnit[index-1].value;
 }
 var tblAllotApplicationDet_tblStocks_lastQty = n('tblAllotApplicationDet_tblStocks_lastQty');
 if(typeof(tblAllotApplicationDet_tblStocks_lastQty) != 'undefined' && tblAllotApplicationDet_tblStocks_lastQty.length >index){
 tblAllotApplicationDet_tblStocks_lastQty[index].value = tblAllotApplicationDet_tblStocks_lastQty[index-1].value;
 }
 var tblAllotApplicationDet_ViewVirtualStock_UseQty = n('tblAllotApplicationDet_ViewVirtualStock_UseQty');
 if(typeof(tblAllotApplicationDet_ViewVirtualStock_UseQty) != 'undefined' && tblAllotApplicationDet_ViewVirtualStock_UseQty.length >index){
 tblAllotApplicationDet_ViewVirtualStock_UseQty[index].value = tblAllotApplicationDet_ViewVirtualStock_UseQty[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey.search(/tblAllotApplicationDet/)!=-1){
 ...
 copyFields=n('tblAllotApplicationDet_InStockCode'); 
 if( (order > 4 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyFields[index].value ==0||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 var tblAllotApplicationDet_ViewtblStocksChild_StockFullName = n('tblAllotApplicationDet_ViewtblStocksChild_StockFullName'); 
 if(typeof(tblAllotApplicationDet_ViewtblStocksChild_StockFullName) != 'undefined' && tblAllotApplicationDet_ViewtblStocksChild_StockFullName.length >index){
 tblAllotApplicationDet_ViewtblStocksChild_StockFullName[index].value = tblAllotApplicationDet_ViewtblStocksChild_StockFullName[index-1].value;
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblAllotApplicationDet"; 
...
function execStat2(){
 execStatOne('tblAllotApplicationDet_Qty',2);
 execStatOne('tblAllotApplicationDet_Amt',2);
 setTotalCount(getTotalCount());
}
...
function execDetailStat2(){
 execDetStatOne('tblAllotApplicationDet_Qty',2); 
 execDetStatOne('tblAllotApplicationDet_Amt',2); 
 }
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=4c74a807_1305141736228200003&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 url2='&src=menu&MOID=4c74a807_1305141736228200003&MOOP=add&'+url2;
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblAllotApplication\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblAllotApplication\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblAllotApplicationDet_Qty',"tblAllotApplicationDet"); 
 delFireEventOne(rowId,tableName,'tblAllotApplicationDet_Price',"tblAllotApplicationDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblAllotApplicationDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAllotApplicationDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAllotApplicationDet_Amt',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblAllotApplication",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAllotApplication&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAllotApplication&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'打印控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"4c74a807_1305141736228200003",
 MOOP:"add",
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('tblAllotApplicationDet_Qty'); 
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=4c74a807_1305141736228200003&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=4c74a807_1305141736228200003&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=4c74a807_1305141736228200003&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblAllotApplication"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblAllotApplication&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblAllotApplication&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblAllotApplicationDet_Qty'); 
 r(0,"Amt")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
r(0,"RetailAmt")[p].value=f(rn(0,"RetailPrice",p)*rn(0,"Qty",p),3);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotApplicationDet_Price'); 
 r(0,"Amt")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotApplicationDet_RetailPrice'); 
 r(0,"RetailAmt")[p].value=f(rn(0,"RetailPrice",p)*rn(0,"Qty",p),3);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotApplicationDet_RetailAmt'); 
 r(0,"RetailPrice")[p].value=f(rn(0,"RetailAmt",p)/rn(0,"Qty",p),5);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotApplicationDet_SecQty'); 
...
childData[childCount]='tblAllotApplicationDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='调拨申请单明细';
childCount++;
 var tblAllotApplicationDetData = new gridData();
 gridDatas[gridDataNum] = tblAllotApplicationDetData;
 gridDataNum++;
 var tblAllotApplicationDet=1;
 ...
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_StockCode','出库仓库','80',2,false,'',100,-2,'','',"","copy","0",'',"2");
 putChildValidateItem("tblAllotApplicationDet_StockCode","出库仓库","any","",false,minValue,100,false,'');
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_tblStock_StockFullName','出库仓库','75',2,true,'',1000000,2,'tblAllotApplicationDet:StockCode:tblAllotApplicationDet_tblStock_StockFullName:'+param4,'',"","copy","0",'',"2"); 
 ...
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_InStockCode','入库仓库','80',2,false,'',100,-2,'','',"","copy","4",'',"2");
 putChildValidateItem("tblAllotApplicationDet_InStockCode","入库仓库","any","",false,minValue,100,false,'');
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_ViewtblStocksChild_StockFullName','入库仓库','75',2,true,'',1000000,2,'tblAllotApplicationDet:InStockCode:tblAllotApplicationDet_ViewtblStocksChild_StockFullName:'+param4,'',"","copy","4",'',"2"); 
 
 
 
 
 var param4="";
 param4=param4+":0调出仓库@StockCode"; 
 param4=param4+":1出库仓库@tblAllotApplicationDet_StockCode";
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_GoodsCode','商品名称','120',2,true,'',100,-2,'','',"","triggerOrCopy","8",'',"2");
 putChildValidateItem("tblAllotApplicationDet_GoodsCode","商品名称","any","",true,minValue,100,false,'');
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_tblGoods_GoodsFullName','商品名称','150',2,true,'',1000000,2,'tblAllotApplicationDet:GoodsCode:tblAllotApplicationDet_tblGoods_GoodsFullName:'+param4,'',"","triggerOrCopy","8",'',"2"); 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'tblAllotApplicationDet:GoodsCode:tblAllotApplicationDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","8",'',"2"); 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'tblAllotApplicationDet:GoodsCode:tblAllotApplicationDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","8",'',"2"); 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblAllotApplicationDet:GoodsCode:tblAllotApplicationDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","8",'',"2"); 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_tblStocks_lastQty','实际库存','90',2,true,'',1000000,2,'tblAllotApplicationDet:GoodsCode:tblAllotApplicationDet_tblStocks_lastQty:'+param4,'',"","triggerOrCopy","8",'',"2"); 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_ViewVirtualStock_UseQty','虚拟库存','90',2,true,'',1000000,2,'tblAllotApplicationDet:GoodsCode:tblAllotApplicationDet_ViewVirtualStock_UseQty:'+param4,'',"","triggerOrCopy","8",'',"2"); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","15",'',"2"); 
 putChildValidateItem("tblAllotApplicationDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_color','色彩','60',2,true,'',100,-2,'','',"","","18",'',"3"); 
 putChildValidateItem("tblAllotApplicationDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","21",'',"2"); 
 putChildValidateItem("tblAllotApplicationDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","24",'',"2"); 
 putChildValidateItem("tblAllotApplicationDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 
 var param4="";
 param4=param4+":0商品名称@tblAllotApplicationDet_GoodsCode";
 param4=param4+":1出库仓库@tblAllotApplicationDet_StockCode";
 param4=param4+":0调出仓库@StockCode"; 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_Hue','颜色','70',2,true,'',100,2,'tblAllotApplicationDet:Hue:tblAllotApplicationDet_Hue:'+param4,"","","triggerCopy","27",'',"2");
 putChildValidateItem("tblAllotApplicationDet_Hue","颜色","any","GoodsField",true,minValue,100,false,'',true);
 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","30",'',"8"); 
 putChildValidateItem("tblAllotApplicationDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","33",'',"8"); 
 putChildValidateItem("tblAllotApplicationDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_Qty','数量','80',1,false,'0',0,0,'','',"Qty","","37",'',"0"); 
 putChildValidateItem("tblAllotApplicationDet_Qty","数量","double","SeqQty",false,minValue,maxValue,true,'0',true); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_Price','单价','80',1,true,'0',0,8,'','',"priceIdentifier","","40",'',"8"); 
 putChildValidateItem("tblAllotApplicationDet_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_Amt','金额','70',1,true,'0',0,8,'','',"AmountIdentifier","","43",'',"8"); 
 putChildValidateItem("tblAllotApplicationDet_Amt","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_RetailPrice','零售价','80',1,true,'',0,-2,'','',"SPriceIdentifier","","46",'',"0"); 
 putChildValidateItem("tblAllotApplicationDet_RetailPrice","零售价","double","",true,minValue,maxValue,false,''); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_RetailAmt','零售金额','80',1,true,'',0,-2,'','',"SAmountIdentifier","","49",'',"0"); 
 putChildValidateItem("tblAllotApplicationDet_RetailAmt","零售金额","double","",true,minValue,maxValue,false,''); 
 ...
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","100",'',"3"); 
 putChildValidateItem("tblAllotApplicationDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","100",'',"3"); 
 putChildValidateItem("tblAllotApplicationDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","105",'',"3"); 
 putChildValidateItem("tblAllotApplicationDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 ...
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_Remark','备注','130',2,true,'',8000,0,'','',"","","120",'',"0"); 
 putChildValidateItem("tblAllotApplicationDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 
 addCols(tblAllotApplicationDetData,'tblAllotApplicationDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblAllotApplication&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblAllotApplication,tblAllotApplicationDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAllotApplication&allTableName=tblAllotApplication,tblAllotApplicationDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAllotApplication&allTableName=tblAllotApplicationDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAllotApplication&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAllotApplication&allTableName=tblAllotApplicationDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=4c74a807_1305141736228200003&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=4c74a807_1305141736228200003&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblAllotApplicationDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=4c74a807_1305141736228200003&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=4c74a807_1305141736228200003&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblAllotApplication&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function billTool(value,dis,width,height){
 ...
 nv = "";
 if(detail != "detail" &&( obj == null||obj.length==0)){
...
function update(){
 location.href="/UserFunctionAction.do?tableName=tblAllotApplication&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblAllotApplication&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblAllotApplication&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblAllotApplication&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '调拨申请单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblAllotApplication&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
...
function scanBarCodeEv(obj,tableName,isMerger){ 
 scanBarCodeEvOne(obj.value,tableName,isMerger);
 obj.value="";
