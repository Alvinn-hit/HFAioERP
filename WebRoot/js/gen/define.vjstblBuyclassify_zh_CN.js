...
{ 
 cFireEvent('tblBuyclassifyDet_Qty',''); 
 cFireEvent('tblBuyclassifyDet_CurPrice',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBuyclassify&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBuyclassify&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBuyclassify&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBuyclassify&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBuyclassifyDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblBuyclassifyDet_GoodsCode');
 ...
 var z=n('tblBuyclassifyDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblBuyclassifyDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBuyclassifyDet"){
 
 copyFields=n('tblBuyclassifyDet_BuyApplyID');
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBuyclassifyDet"){
...
function execStat2(){
 execStatOne('tblBuyclassifyDet_Amount',2);
 execStatOne('tblBuyclassifyDet_TaxAmount',2);
 setTotalCount(getTotalCount());
}
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBuyclassify\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblBuyclassifyDet_Qty',"tblBuyclassifyDet"); 
 delFireEventOne(rowId,tableName,'tblBuyclassifyDet_CurPrice',"tblBuyclassifyDet"); 
 delFireEventOne(rowId,tableName,'tblBuyclassifyDet_CurAmount',"tblBuyclassifyDet"); 
 delFireEventOne(rowId,tableName,'tblBuyclassifyDet_Price',"tblBuyclassifyDet"); 
 delFireEventOne(rowId,tableName,'tblBuyclassifyDet_Amount',"tblBuyclassifyDet"); 
 delFireEventOne(rowId,tableName,'tblBuyclassifyDet_TaxPrice',"tblBuyclassifyDet"); 
 delFireEventOne(rowId,tableName,'tblBuyclassifyDet_TaxAmount',"tblBuyclassifyDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblBuyclassifyDet_Qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyclassifyDet_CurPrice',dis:'原币单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyclassifyDet_CurAmount',dis:'原币金额',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyclassifyDet_CurrencyRate',dis:'汇率',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyclassifyDet_Tax',dis:'税率%',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyclassifyDet_Price',dis:'本币未税单价',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyclassifyDet_Amount',dis:'本币未税金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyclassifyDet_TaxPrice',dis:'本币含税单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyclassifyDet_TaxAmount',dis:'本币含税金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBuyclassify",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyclassify&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyclassify&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblBuyclassify"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBuyclassify&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBuyclassify&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblBuyclassifyDet_Qty'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyclassifyDet_CurPrice'); 
...
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
r(0,"DisAmount")[p].value=f(sm('tblBuyclassifyDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyclassifyDet_CurAmount'); 
...
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyclassifyDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyclassifyDet_Amount'); 
...
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyclassifyDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyclassifyDet_DisPrice'); 
...
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyclassifyDet_DisBackAmt'); 
...
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyclassifyDet_TaxPrice'); 
...
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyclassifyDet_TaxAmount'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyclassifyDet_SecQty'); 
...
childData[childCount]='tblBuyclassifyDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='分单管理明细';
childCount++;
 var tblBuyclassifyDetData = new gridData();
 gridDatas[gridDataNum] = tblBuyclassifyDetData;
 gridDataNum++;
 var tblBuyclassifyDet=1;
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_BuyApplyID','申请单号','80',2,true,'',32,-2,'','',"","copy","0",'',"3",false); 
 putChildValidateItem("tblBuyclassifyDet_BuyApplyID","申请单号","any","",true,minValue,32,false,''); 
 ...
 var param4="";
 param4=param4+":0请购单号@BuyApplyID"; 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_GoodsCode','料件名称','120',2,false,'',100,-2,'','',"","triggerCopy","1",'',"2",true);
 putChildValidateItem("tblBuyclassifyDet_GoodsCode","料件名称","any","",false,minValue,100,false,'');
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,8,'tblBuyclassifyDet:GoodsCode:tblBuyclassifyDet_tblGoods_GoodsNumber:'+param4,'',"","triggerCopy","1",'',"2",false); 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_tblGoods_GoodsFullName','料件名称','150',2,true,'',1000000,8,'tblBuyclassifyDet:GoodsCode:tblBuyclassifyDet_tblGoods_GoodsFullName:'+param4,'',"","triggerCopy","1",'',"2",false); 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,8,'tblBuyclassifyDet:GoodsCode:tblBuyclassifyDet_tblGoods_GoodsSpec:'+param4,'',"","triggerCopy","1",'',"2",false); 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,8,'tblBuyclassifyDet:GoodsCode:tblBuyclassifyDet_tblGoods_BaseUnit:'+param4,'',"","triggerCopy","1",'',"2",false); 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_tblGoods_BarCode','条码','100',2,true,'',1000000,8,'tblBuyclassifyDet:GoodsCode:tblBuyclassifyDet_tblGoods_BarCode:'+param4,'',"","triggerCopy","1",'',"2",false); 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_tblBuyApplicationDet_Qty','申请数量','0',1,true,'',1000000,8,'tblBuyclassifyDet:GoodsCode:tblBuyclassifyDet_tblBuyApplicationDet_Qty:'+param4,'',"","triggerCopy","1",'',"2",false); 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_tblBuyApplicationDet_id','id','0',2,true,'',1000000,8,'tblBuyclassifyDet:GoodsCode:tblBuyclassifyDet_tblBuyApplicationDet_id:'+param4,'',"","triggerCopy","1",'',"2",false); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","","2",'',"2",false); 
 putChildValidateItem("tblBuyclassifyDet_BatchNo","批号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","3",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","4",'',"2",false); 
 putChildValidateItem("tblBuyclassifyDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_yearNO','箱号','100',2,true,'',100,0,'','',"","triggerCopy","5",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_yearNO","箱号","any","GoodsField",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_Qty','数量','60',1,false,'',0,8,'','',"Qty","","8",'',"8",false); 
 putChildValidateItem("tblBuyclassifyDet_Qty","数量","double","currency",false,minValue,maxValue,false,'',true); 
 ...
 var param4="";
 param4=param4+":1料件名称@tblBuyclassifyDet_GoodsCode";
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_CompanyCode','供应商','100',2,true,'',100,-2,'','',"","","9",'',"2",true);
 putChildValidateItem("tblBuyclassifyDet_CompanyCode","供应商","any","",true,minValue,100,false,'');
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_tblCompany_ComNumber','厂商编号','100',2,true,'',1000000,2,'tblBuyclassifyDet:CompanyCode:tblBuyclassifyDet_tblCompany_ComNumber:'+param4,'',"","","9",'',"2",false); 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_tblCompany_ComName','厂商简称','100',2,true,'',1000000,2,'tblBuyclassifyDet:CompanyCode:tblBuyclassifyDet_tblCompany_ComName:'+param4,'',"","","9",'',"2",false); 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_tblSetExchange_CurrencyName','币种','50',2,true,'',1000000,2,'tblBuyclassifyDet:CompanyCode:tblBuyclassifyDet_tblSetExchange_CurrencyName:'+param4,'',"","","9",'',"2",false); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_CurPrice','原币单价','100',1,true,'',0,8,'','',"priceIdentifier","","10",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_CurPrice","原币单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_CurAmount','原币金额','100',1,true,'',0,8,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_CurAmount","原币金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_CurrencyRate','汇率','30',1,true,'',0,8,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_CurrencyRate","汇率","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_InVoiceType','发票类型','100',0,true,'3',0,8,';1:普通发票;2:增值税发票;3:收据凭证','',"","","13",'',"1",false);
 putChildValidateItem("tblBuyclassifyDet_InVoiceType","发票类型","int","",true,minValue,maxValue,true,'3',true); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_Tax','税率%','40',1,true,'',0,8,'','',"","","14",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_Tax","税率%","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_Price','本币未税单价','80',1,true,'',0,8,'','',"Amount","","15",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_Price","本币未税单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_Amount','本币未税金额','100',1,true,'',0,8,'','',"AmountIdentifier","","16",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_Amount","本币未税金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_Discount','折扣率%','100',1,true,'100',0,-2,'','',"Price","","22",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_Discount","折扣率%","double","BillDiscount",true,minValue,maxValue,true,'100'); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_DisPrice','折后单价','100',1,true,'',0,-2,'','',"priceIdentifier","","23",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_DisPrice","折后单价","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_DisBackAmt','折后金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","24",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_DisBackAmt","折后金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_DisAmount','折让金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","25",'',"8",false); 
 putChildValidateItem("tblBuyclassifyDet_DisAmount","折让金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_TaxPrice','本币含税单价','100',1,true,'',0,8,'','',"priceIdentifier","","26",'',"8",false); 
 putChildValidateItem("tblBuyclassifyDet_TaxPrice","本币含税单价","double","BillTax",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_TaxAmount','本币含税金额','100',1,true,'',0,8,'','',"AmountIdentifier","","27",'',"8",false); 
 putChildValidateItem("tblBuyclassifyDet_TaxAmount","本币含税金额","double","BillTax",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_CoTaxAmt','税额','100',1,true,'0',0,-2,'','',"AmountIdentifier","","28",'',"8",false); 
 putChildValidateItem("tblBuyclassifyDet_CoTaxAmt","税额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_SourceID','源ID','200',2,true,'',100,-2,'','',"","","29",'',"100",false); 
 putChildValidateItem("tblBuyclassifyDet_SourceID","源ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","30",'',"2",false); 
 putChildValidateItem("tblBuyclassifyDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"Qty","","31",'',"3",false); 
 putChildValidateItem("tblBuyclassifyDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_Remark','备注','100',2,true,'',8000,-2,'','',"","","32",'',"1",false); 
 putChildValidateItem("tblBuyclassifyDet_Remark","备注","any","",true,minValue,8000,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_Currency','币种','200',2,true,'',60,-2,'','',"","","33",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_Currency","币种","any","",true,minValue,60,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_tblSalesOrderId','订单ID','200',2,true,'',50,-2,'','',"","","34",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_tblSalesOrderId","订单ID","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_BuyQuoteID','核价单号','200',2,true,'',50,-2,'','',"","","35",'',"0",false); 
 putChildValidateItem("tblBuyclassifyDet_BuyQuoteID","核价单号","any","",true,minValue,50,false,''); 
 
 
 addCols(tblBuyclassifyDetData,'tblBuyclassifyDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBuyclassify&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBuyclassify,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyclassify&allTableName=tblBuyclassify,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyclassify&allTableName=tblBuyclassifyDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyclassify&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyclassify&allTableName=tblBuyclassifyDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblBuyclassifyDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblBuyclassify&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblBuyclassify&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBuyclassify&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBuyclassify&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBuyclassify&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '采购分单管理';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBuyclassify&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
