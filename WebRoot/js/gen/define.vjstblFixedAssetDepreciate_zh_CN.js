...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblFixedAssetDepreciate&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblFixedAssetDepreciate&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblFixedAssetDepreciate&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblFixedAssetDepreciate&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblFixedAssetDepreciate\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'ReWorthRate',dis:'残值率',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ReWorth',dis:'残值额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'netWorth',dis:'净值额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'currDeprecAmt',dis:'当前折旧额',digit:2,isMain:true};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblFixedAssetDepreciate",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblFixedAssetDepreciate&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblFixedAssetDepreciate&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 if(fieldName == "AssetsAddId"){
 var saveField=n("AssetsAddId"); 
 saveField[0].value = "";
 var obj = m("tbl_tblFixedAssetAdd_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblFixedAssetAdd_GoodsName");
 saveField[0].value = ""; 
 m("tbl_tblFixedAssetAdd_GoodsName").value = "" ;
 }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblFixedAssetDepreciate"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblFixedAssetDepreciate&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblFixedAssetDepreciate&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblFixedAssetDepreciate&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblFixedAssetDepreciate,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblFixedAssetDepreciate&allTableName=tblFixedAssetDepreciate,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblFixedAssetDepreciate&allTableName=&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblFixedAssetDepreciate&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblFixedAssetDepreciate&allTableName=&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblFixedAssetDepreciateDet";
 var stockCode="";
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblFixedAssetDepreciate&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblFixedAssetDepreciate&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblFixedAssetDepreciate&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblFixedAssetDepreciate&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '计提折旧';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblFixedAssetDepreciate&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
