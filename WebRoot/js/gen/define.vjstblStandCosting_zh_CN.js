...
{ 
 cFireEvent('tblStandCostingDet_Price','0'); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblStandCosting&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblStandCosting&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblStandCosting&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblStandCosting&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblStandCostingDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblStandCostingDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblStandCostingDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblStandCostingDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblStandCostingDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblStandCostingDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblStandCostingDet"; 
 tblName[1]="tblStandCostingDet1"; 
...
function execStat2(){
 execStatOne('tblStandCostingDet_Price',4);
 execStatOne('tblStandCostingDet_CurPrice',4);
 execStatOne('tblStandCostingDet_OtherAmt',4);
 execStatOne('tblStandCostingDet1_Price',4);
 execStatOne('tblStandCostingDet1_CurPrice',4);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=61030634_1306251101079940140&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=61030634_1306251101079940140&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblStandCosting\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblStandCosting\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblStandCostingDet_Price',"tblStandCostingDet"); 
 delFireEventOne(rowId,tableName,'tblStandCostingDet_CurPrice',"tblStandCostingDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'CurTotalAmount',dis:'总金额',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblStandCostingDet_CurPrice',dis:'外币单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblStandCostingDet_OtherAmt',dis:'模费',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblStandCostingDet1_CurPrice',dis:'外币单价',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblStandCosting",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblStandCosting&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblStandCosting&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"61030634_1306251101079940140",
 MOOP:"add",
...
function resetSubmit(fieldName){
 if(fieldName == "Brand"){
 var saveField=n("Brand"); 
 saveField[0].value = "";
 var obj = m("tbl_tblBrand_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblBrand_BrandName");
 saveField[0].value = ""; 
 m("tbl_tblBrand_BrandName").value = "" ;
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=61030634_1306251101079940140&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=61030634_1306251101079940140&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=61030634_1306251101079940140&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblStandCosting"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblStandCosting&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblStandCosting&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblStandCostingDet_Price'); 
 n('TotalAmount')[0].value=f(sm('tblStandCostingDet_Price'),4);
n('CurTotalAmount')[0].value=f(sm('tblStandCostingDet_CurPrice'),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblStandCostingDet_CurPrice'); 
 n('TotalAmount')[0].value=f(sm('tblStandCostingDet_Price'),4);
n('CurTotalAmount')[0].value=f(sm('tblStandCostingDet_CurPrice'),4);
...
childData[childCount]='tblStandCostingDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='成本核算明细';
childCount++;
 var tblStandCostingDetData = new gridData();
 gridDatas[gridDataNum] = tblStandCostingDetData;
 gridDataNum++;
 var tblStandCostingDet=1;
 ...
 addCols(tblStandCostingDetData,'tblStandCostingDet_Component','部件','100',2,false,'',10,1,';11:电镀;1:表壳;2:表面;3:表针;4:巴的;5:表带;6:扣制;7:镶石;8:机芯;9:装配;10:加工费','',"","","1",'',"1",false);
 putChildValidateItem("tblStandCostingDet_Component","部件","any","",false,minValue,10,false,'',true); 
 ...
 addCols(tblStandCostingDetData,'tblStandCostingDet_CompanyCode','供应商','120',2,true,'',1000,-2,'','',"","","2",'',"2",true);
 putChildValidateItem("tblStandCostingDet_CompanyCode","供应商","any","",true,minValue,1000,false,'');
 addCols(tblStandCostingDetData,'tblStandCostingDet_tblCompany_ComFullName','供应商','200',18,true,'',1000000,2,'tblStandCostingDet:CompanyCode:tblStandCostingDet_tblCompany_ComFullName:'+param4,'',"","","2",'',"2",false); 
 ...
 addCols(tblStandCostingDetData,'tblStandCostingDet_BaseMat','材料','120',2,true,'',100,2,'tblStandCostingDet:BaseMat:tblStandCostingDet_BaseMat:'+param4,"","","","2",'',"2",true);
 putChildValidateItem("tblStandCostingDet_BaseMat","材料","any","",true,minValue,100,false,'',true);
 
 
 
 addCols(tblStandCostingDetData,'tblStandCostingDet_Price','本币单价','100',1,false,'0',0,-2,'','',"","","4",'',"6",false); 
 putChildValidateItem("tblStandCostingDet_Price","本币单价","double","",false,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblStandCostingDetData,'tblStandCostingDet_CurPrice','外币单价','100',1,true,'0',0,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblStandCostingDet_CurPrice","外币单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblStandCostingDetData,'tblStandCostingDet_OtherAmt','模费','80',1,true,'',0,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblStandCostingDet_OtherAmt","模费","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblStandCostingDetData,'tblStandCostingDet_Remark','备注','200',2,true,'',1000,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblStandCostingDet_Remark","备注","any","",true,minValue,1000,false,'',true); 
 
 
 
 
 addCols(tblStandCostingDetData,'tblStandCostingDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblStandCostingDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='成本核算加工明细';
childCount++;
 var tblStandCostingDet1Data = new gridData();
 gridDatas[gridDataNum] = tblStandCostingDet1Data;
 gridDataNum++;
 var tblStandCostingDet1=1;
 ...
 addCols(tblStandCostingDet1Data,'tblStandCostingDet1_Component','部件','100',2,false,'',10,1,';11:电镀;1:表壳;2:表面;3:表针;4:巴的;5:表带;6:扣制;7:镶石;8:机芯;9:装配;10:加工费','',"","","1",'',"1",false);
 putChildValidateItem("tblStandCostingDet1_Component","部件","any","",false,minValue,10,false,'',true); 
 ...
 addCols(tblStandCostingDet1Data,'tblStandCostingDet1_CompanyCode','供应商','120',2,true,'',1000,-2,'','',"","","2",'',"2",true);
 putChildValidateItem("tblStandCostingDet1_CompanyCode","供应商","any","",true,minValue,1000,false,'');
 addCols(tblStandCostingDet1Data,'tblStandCostingDet1_tblCompany_ComFullName','供应商','200',18,true,'',1000000,2,'tblStandCostingDet1:CompanyCode:tblStandCostingDet1_tblCompany_ComFullName:'+param4,'',"","","2",'',"2",false); 
 ...
 addCols(tblStandCostingDet1Data,'tblStandCostingDet1_BaseMat','材料','120',2,true,'',100,2,'tblStandCostingDet1:BaseMat:tblStandCostingDet1_BaseMat:'+param4,"","","","2",'',"2",true);
 putChildValidateItem("tblStandCostingDet1_BaseMat","材料","any","",true,minValue,100,false,'',true);
 
 
 
 addCols(tblStandCostingDet1Data,'tblStandCostingDet1_Price','本币单价','100',1,false,'0',0,-2,'','',"","","4",'',"6",false); 
 putChildValidateItem("tblStandCostingDet1_Price","本币单价","double","",false,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblStandCostingDet1Data,'tblStandCostingDet1_CurPrice','外币单价','100',1,true,'0',0,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblStandCostingDet1_CurPrice","外币单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblStandCostingDet1Data,'tblStandCostingDet1_Remark','备注','200',2,true,'',1000,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblStandCostingDet1_Remark","备注","any","",true,minValue,1000,false,'',true); 
 
 
 
 
 addCols(tblStandCostingDet1Data,'tblStandCostingDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblStandCosting&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblStandCosting,tblStandCostingDet,tblStandCostingDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblStandCosting&allTableName=tblStandCosting,tblStandCostingDet,tblStandCostingDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblStandCosting&allTableName=tblStandCostingDet,tblStandCostingDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblStandCosting&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblStandCosting&allTableName=tblStandCostingDet,tblStandCostingDet1,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=61030634_1306251101079940140&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=61030634_1306251101079940140&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblStandCostingDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=61030634_1306251101079940140&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=61030634_1306251101079940140&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblStandCosting&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblStandCosting&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblStandCosting&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblStandCosting&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblStandCosting&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '成本核算';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblStandCosting&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
