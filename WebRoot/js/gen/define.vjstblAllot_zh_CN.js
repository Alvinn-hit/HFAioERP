...
{ 
 cFireEvent('tblAllotDet_Qty',''); 
 cFireEvent('tblAllotDet_Price','0'); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblAllot&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblAllot&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblAllot&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblAllot&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function addSeq(o,keyCode){
 ...
 var ss="@GN:Seq:Qty:tblAllotDet_GoodsSeqSet:tblAllotDet_GoodsCode:StockCode".split(":")
 if(ss[0]=="@GN"){
 goodscodestr=ss[4];
 goodscode=n(goodscodestr)[ri].value;
 }else if(ss[0]=="@IOO" && ss[1]=="Instore"){
...
function returnSeq(colNames,returnVales,compareVale,goodscodestr,ri,seqFname,inpVal){
 ...
 var saveField=n("tblAllotDet_tblGoods_GoodsNumber")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("tblAllotDet_tblGoods_GoodsFullName")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("tblAllotDet_tblGoods_GoodsSpec")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("tblAllotDet_tblGoods_BaseUnit")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("tblAllotDet_tblGoods_BarCode")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("tblAllotDet_tblGoods_Spec1")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("tblAllotDet_tblGoods_Spec2")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("tblAllotDet_tblGoods_Spec3")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("tblAllotDet_Hue")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("tblAllotDet_yearNO")[m].value; 
 goodsProps += saveField+"#;#" 
 ...
 if("tblAllotDet"==seqFname.substr(0,seqFname.lastIndexOf("_"))){ 
 var qtys=n("tblAllotDet"+"_"+'Qty');
 qtys[ri].value="1";
...
function getInputVal(seqFname) {
 ...
 var param4="";
 var dd="";
 if('tblAllotDet'==tableName){
 dd=dd+"StockCode";
 param4=param4+":1调出仓库@StockCode"; 
 dd=dd+"@TABLENAME_Seq";
 param4=param4+":0序列号@tblAllotDet_Seq";
 }
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblAllotDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblAllotDet"){
 
 copyFields=n('tblAllotDet_StockCode');
 if(copyFields[index].value !=0 && copyFields[index].value != ""){return}
 if((order > 6 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblAllotDet_tblStock_StockFullName1 = n('tblAllotDet_tblStock_StockFullName1');
 if(typeof(tblAllotDet_tblStock_StockFullName1) != 'undefined' && tblAllotDet_tblStock_StockFullName1.length >index){
 tblAllotDet_tblStock_StockFullName1[index].value = tblAllotDet_tblStock_StockFullName1[index-1].value;
 }
 }
 copyFields=n('tblAllotDet_InStockCode');
 if((order > 9 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblAllotDet_tblStockDet_StockFullNameDetIn = n('tblAllotDet_tblStockDet_StockFullNameDetIn');
 if(typeof(tblAllotDet_tblStockDet_StockFullNameDetIn) != 'undefined' && tblAllotDet_tblStockDet_StockFullNameDetIn.length >index){
 tblAllotDet_tblStockDet_StockFullNameDetIn[index].value = tblAllotDet_tblStockDet_StockFullNameDetIn[index-1].value;
 }
 }
 copyFields=n('tblAllotDet_GoodsCode');
 if((order > 12 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblAllotDet_tblGoods_GoodsNumber = n('tblAllotDet_tblGoods_GoodsNumber');
 if(typeof(tblAllotDet_tblGoods_GoodsNumber) != 'undefined' && tblAllotDet_tblGoods_GoodsNumber.length >index){
 tblAllotDet_tblGoods_GoodsNumber[index].value = tblAllotDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblAllotDet_tblGoods_GoodsFullName = n('tblAllotDet_tblGoods_GoodsFullName');
 if(typeof(tblAllotDet_tblGoods_GoodsFullName) != 'undefined' && tblAllotDet_tblGoods_GoodsFullName.length >index){
 tblAllotDet_tblGoods_GoodsFullName[index].value = tblAllotDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblAllotDet_tblGoods_GoodsSpec = n('tblAllotDet_tblGoods_GoodsSpec');
 if(typeof(tblAllotDet_tblGoods_GoodsSpec) != 'undefined' && tblAllotDet_tblGoods_GoodsSpec.length >index){
 tblAllotDet_tblGoods_GoodsSpec[index].value = tblAllotDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblAllotDet_tblGoods_BaseUnit = n('tblAllotDet_tblGoods_BaseUnit');
 if(typeof(tblAllotDet_tblGoods_BaseUnit) != 'undefined' && tblAllotDet_tblGoods_BaseUnit.length >index){
 tblAllotDet_tblGoods_BaseUnit[index].value = tblAllotDet_tblGoods_BaseUnit[index-1].value;
 }
 var tblAllotDet_tblGoods_BarCode = n('tblAllotDet_tblGoods_BarCode');
 if(typeof(tblAllotDet_tblGoods_BarCode) != 'undefined' && tblAllotDet_tblGoods_BarCode.length >index){
 tblAllotDet_tblGoods_BarCode[index].value = tblAllotDet_tblGoods_BarCode[index-1].value;
 }
 var tblAllotDet_tblGoods_Spec1 = n('tblAllotDet_tblGoods_Spec1');
 if(typeof(tblAllotDet_tblGoods_Spec1) != 'undefined' && tblAllotDet_tblGoods_Spec1.length >index){
 tblAllotDet_tblGoods_Spec1[index].value = tblAllotDet_tblGoods_Spec1[index-1].value;
 }
 var tblAllotDet_tblGoods_Spec2 = n('tblAllotDet_tblGoods_Spec2');
 if(typeof(tblAllotDet_tblGoods_Spec2) != 'undefined' && tblAllotDet_tblGoods_Spec2.length >index){
 tblAllotDet_tblGoods_Spec2[index].value = tblAllotDet_tblGoods_Spec2[index-1].value;
 }
 var tblAllotDet_tblGoods_Spec3 = n('tblAllotDet_tblGoods_Spec3');
 if(typeof(tblAllotDet_tblGoods_Spec3) != 'undefined' && tblAllotDet_tblGoods_Spec3.length >index){
 tblAllotDet_tblGoods_Spec3[index].value = tblAllotDet_tblGoods_Spec3[index-1].value;
 }
 var tblAllotDet_tblStocks_lastQty = n('tblAllotDet_tblStocks_lastQty');
 if(typeof(tblAllotDet_tblStocks_lastQty) != 'undefined' && tblAllotDet_tblStocks_lastQty.length >index){
 tblAllotDet_tblStocks_lastQty[index].value = tblAllotDet_tblStocks_lastQty[index-1].value;
 }
 var tblAllotDet_ViewVirtualStock_UseQty = n('tblAllotDet_ViewVirtualStock_UseQty');
 if(typeof(tblAllotDet_ViewVirtualStock_UseQty) != 'undefined' && tblAllotDet_ViewVirtualStock_UseQty.length >index){
 tblAllotDet_ViewVirtualStock_UseQty[index].value = tblAllotDet_ViewVirtualStock_UseQty[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblAllotDet"){
 
 copyFields=n('tblAllotDet_StockCode'); 
 if(copyFields[index].value !=0 && copyFields[index].value != ""){return}
 if( (order > 6 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblAllotDet_tblStock_StockFullName1 = n('tblAllotDet_tblStock_StockFullName1'); 
 if(typeof(tblAllotDet_tblStock_StockFullName1) != 'undefined' && tblAllotDet_tblStock_StockFullName1.length >index){
 tblAllotDet_tblStock_StockFullName1[index].value = tblAllotDet_tblStock_StockFullName1[index-1].value;
 ...
 copyFields=n('tblAllotDet_InStockCode'); 
 if( (order > 9 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblAllotDet_tblStockDet_StockFullNameDetIn = n('tblAllotDet_tblStockDet_StockFullNameDetIn'); 
 if(typeof(tblAllotDet_tblStockDet_StockFullNameDetIn) != 'undefined' && tblAllotDet_tblStockDet_StockFullNameDetIn.length >index){
 tblAllotDet_tblStockDet_StockFullNameDetIn[index].value = tblAllotDet_tblStockDet_StockFullNameDetIn[index-1].value;
 ...
 copyFields=n('tblAllotDet_GoodsCode'); 
 if( (order > 12 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblAllotDet_tblGoods_GoodsNumber = n('tblAllotDet_tblGoods_GoodsNumber'); 
 if(typeof(tblAllotDet_tblGoods_GoodsNumber) != 'undefined' && tblAllotDet_tblGoods_GoodsNumber.length >index){
 tblAllotDet_tblGoods_GoodsNumber[index].value = tblAllotDet_tblGoods_GoodsNumber[index-1].value;
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblAllotDet"; 
 var goodsCodeLogicValidate_tblAllotDet='@GN:Seq:Qty:tblAllotDet_GoodsSeqSet:tblAllotDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'tblAllotDet',value:goodsCodeLogicValidate_tblAllotDet};
...
function execStat2(){
 execStatOne('tblAllotDet_Qty',2);
 execStatOne('tblAllotDet_Amt',2);
 execStatOne('tblAllotDet_RetailAmt',2);
 setTotalCount(getTotalCount());
}
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr.substring(0,index+"&seq=".length)+n(thisField.substr(0,thisField.lastIndexOf("_")))[ri].value+"&src=menu&seqFname="+thisField+"&newSeq="+mi[ri].value+"&tableName=tblAllot&childTableName="+thisField.split("_")[0]+"&fieldName="+field;
 var goodscode="";
 var goodscodestr="";
 var goodsProps = "";
 var stockCode = "" ;
 ...
 if(thisField=="tblAllotDet_Seq_hid"){
 ...
 stockCode = nz("tblAllotDet"+"_"+ss[5],ri) ;
 }
 }
 var ss="@GN:Seq:Qty:tblAllotDet_GoodsSeqSet:tblAllotDet_GoodsCode:StockCode".split(":")
 if(ss[0]=="@GN"){
 ...
 goodsProps += "Hue="+nz("tblAllotDet"+"_Hue",ri)+"&"
 goodsProps += "yearNO="+nz("tblAllotDet"+"_yearNO",ri)+"&"
  }
 if(stockCode==""){
 ...
 AjaxRequest(urlsgood) ;
 if("false"==response){
 asyncbox.tips("该商品未启用序列号6！！","alert",2000) ;
 return ;
 }
 }
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblAllot\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblAllot\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblAllotDet_Qty',"tblAllotDet"); 
 delFireEventOne(rowId,tableName,'tblAllotDet_Price',"tblAllotDet"); 
 delFireEventOne(rowId,tableName,'tblAllotDet_RetailPrice',"tblAllotDet"); 
 delFireEventOne(rowId,tableName,'tblAllotDet_RetailAmt',"tblAllotDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblAllotDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAllotDet_RetailPrice',dis:'零售价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAllotDet_RetailAmt',dis:'零售金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblAllot",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAllot&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAllot&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblAllot&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblAllot&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblAllotDet_Qty'); 
 r(0,"Amt")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
r(0,"RetailAmt")[p].value=f(rn(0,"RetailPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotDet_Price'); 
 r(0,"Amt")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
 ...
 var z=r(0,'RetailPrice');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotDet_RetailPrice'); 
 r(0,"RetailAmt")[p].value=f(rn(0,"RetailPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotDet_RetailAmt'); 
 r(0,"RetailPrice")[p].value=f(rn(0,"RetailAmt",p)/rn(0,"Qty",p),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotDet_SecQty'); 
...
childData[childCount]='tblAllotDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='同价调拨单明细';
childCount++;
 var tblAllotDetData = new gridData();
 gridDatas[gridDataNum] = tblAllotDetData;
 gridDataNum++;
 var tblAllotDet=1;
 
 
 addCols(tblAllotDetData,'tblAllotDet_BuyApplyID','申请单号','224',2,true,'',100,-2,'','',"","","0",'',"2",false); 
 putChildValidateItem("tblAllotDet_BuyApplyID","申请单号","any","",true,minValue,100,false,''); 
 ...
 addCols(tblAllotDetData,'tblAllotDet_Seq','序列号','130',2,true,'',5000,-2,'','',"","triggerCopy","0",'',"2",false);
 addCols(tblAllotDetData,'tblAllotDet_Seq'+'_hid','序列号','130',2,true,'',5000,2,'@SEQ@IOO:Outstore',"","","triggerCopy","0",'',"2",false);
 putChildValidateItem("tblAllotDet_Seq","序列号","any","GoodsField",true,minValue,5000,false,'',true);
 
 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_BuyAppTotalID','申请汇总单号','120',2,true,'',0,-2,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblAllotDet_BuyAppTotalID","申请汇总单号","any","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_StockCode','出库仓库','80',2,true,'',100,-2,'','',"","copy","6",'',"3",false); 
 putChildValidateItem("tblAllotDet_StockCode","出库仓库","any","",true,minValue,100,true,''); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_InStockCode','入库仓库','80',2,true,'',100,-2,'','',"","copy","9",'',"3",false); 
 putChildValidateItem("tblAllotDet_InStockCode","入库仓库","any","",true,minValue,100,false,''); 
 ...
 var param4="";
 param4=param4+":1调出仓库@StockCode"; 
 param4=param4+":0出库仓库@tblAllotDet_StockCode";
 
 addCols(tblAllotDetData,'tblAllotDet_GoodsCode','商品名称','120',2,false,'',100,-2,'','',"","triggerOrCopy","12",'',"2",true);
 putChildValidateItem("tblAllotDet_GoodsCode","商品名称","any","",false,minValue,100,false,'');
 addCols(tblAllotDetData,'tblAllotDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'tblAllotDet:GoodsCode:tblAllotDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","12",'',"2",false); 
 addCols(tblAllotDetData,'tblAllotDet_tblGoods_GoodsFullName','商品名称','150',18,true,'',1000000,2,'tblAllotDet:GoodsCode:tblAllotDet_tblGoods_GoodsFullName:'+param4,'',"","triggerOrCopy","12",'',"2",false); 
 addCols(tblAllotDetData,'tblAllotDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'tblAllotDet:GoodsCode:tblAllotDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","12",'',"2",false); 
 addCols(tblAllotDetData,'tblAllotDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblAllotDet:GoodsCode:tblAllotDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","12",'',"2",false); 
 addCols(tblAllotDetData,'tblAllotDet_tblGoods_BarCode','条码','100',2,true,'',1000000,2,'tblAllotDet:GoodsCode:tblAllotDet_tblGoods_BarCode:'+param4,'',"","triggerOrCopy","12",'',"2",false); 
 addCols(tblAllotDetData,'tblAllotDet_tblGoods_Spec1','品牌','80',2,true,'',1000000,2,'tblAllotDet:GoodsCode:tblAllotDet_tblGoods_Spec1:'+param4,'',"","triggerOrCopy","12",'',"2",false); 
 addCols(tblAllotDetData,'tblAllotDet_tblGoods_Spec2','机型','80',2,true,'',1000000,2,'tblAllotDet:GoodsCode:tblAllotDet_tblGoods_Spec2:'+param4,'',"","triggerOrCopy","12",'',"2",false); 
 addCols(tblAllotDetData,'tblAllotDet_tblGoods_Spec3','自定义字段3','80',2,true,'',1000000,2,'tblAllotDet:GoodsCode:tblAllotDet_tblGoods_Spec3:'+param4,'',"","triggerOrCopy","12",'',"2",false); 
 addCols(tblAllotDetData,'tblAllotDet_tblStocks_lastQty','实际库存','90',1,true,'',1000000,2,'tblAllotDet:GoodsCode:tblAllotDet_tblStocks_lastQty:'+param4,'',"","triggerOrCopy","12",'',"2",false); 
 addCols(tblAllotDetData,'tblAllotDet_ViewVirtualStock_UseQty','虚拟库存','90',1,true,'',1000000,2,'tblAllotDet:GoodsCode:tblAllotDet_ViewVirtualStock_UseQty:'+param4,'',"","triggerOrCopy","12",'',"2",false); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","18",'',"2",false); 
 putChildValidateItem("tblAllotDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","21",'',"2",false); 
 putChildValidateItem("tblAllotDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_color','色彩','60',2,true,'',100,-2,'','',"","","22",'',"2",false); 
 putChildValidateItem("tblAllotDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 ...
 var param4="";
 param4=param4+":1商品名称@tblAllotDet_GoodsCode";
 param4=param4+":0出库仓库@tblAllotDet_StockCode";
 param4=param4+":1调出仓库@StockCode"; 
 
 addCols(tblAllotDetData,'tblAllotDet_Hue','颜色','70',2,true,'',100,2,'tblAllotDet:Hue:tblAllotDet_Hue:'+param4,"","","triggerCopy","24",'',"2",true);
 putChildValidateItem("tblAllotDet_Hue","颜色","any","GoodsField",true,minValue,100,false,'',true);
 ...
 addCols(tblAllotDetData,'tblAllotDet_yearNO','采购商','70',2,true,'',100,2,'tblAllotDet:yearNO:tblAllotDet_yearNO:'+param4,"","","triggerCopy","27",'',"2",true);
 putChildValidateItem("tblAllotDet_yearNO","采购商","any","GoodsField",true,minValue,100,false,'',true);
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","30",'',"2",false); 
 putChildValidateItem("tblAllotDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","33",'',"2",false); 
 putChildValidateItem("tblAllotDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_Qty','数量','80',1,false,'',0,0,'','',"Qty","","36",'',"0",false); 
 putChildValidateItem("tblAllotDet_Qty","数量","double","SeqQty",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_Price','单价','80',1,true,'0',0,-2,'','',"priceIdentifier","","39",'',"8",false); 
 putChildValidateItem("tblAllotDet_Price","单价","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_Amt','金额','70',1,true,'0',0,-2,'','',"AmountIdentifier","","42",'',"8",false); 
 putChildValidateItem("tblAllotDet_Amt","金额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_RetailPrice','零售价','80',1,true,'',0,0,'','',"SPriceIdentifier","","45",'',"0",false); 
 putChildValidateItem("tblAllotDet_RetailPrice","零售价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_RetailAmt','零售金额','80',1,true,'',0,0,'','',"SAmountIdentifier","","48",'',"0",false); 
 putChildValidateItem("tblAllotDet_RetailAmt","零售金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(tblAllotDetData,'tblAllotDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","85",'',"100",false); 
 putChildValidateItem("tblAllotDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_CheckBarCode','复核码','160',2,true,'',200,-2,'','',"","","90",'',"0",false); 
 putChildValidateItem("tblAllotDet_CheckBarCode","复核码","any","",true,minValue,200,false,''); 
 ...
 addCols(tblAllotDetData,'tblAllotDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblAllotDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblAllotDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_AppId','申请单ID','0',2,true,'',32,-2,'','',"","","105",'',"3",false); 
 putChildValidateItem("tblAllotDet_AppId","申请单ID","any","",true,minValue,32,false,''); 
 
 
 
 addCols(tblAllotDetData,'tblAllotDet_Remark','备注','130',2,true,'',8000,0,'','',"","","120",'',"0",false); 
 putChildValidateItem("tblAllotDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 addCols(tblAllotDetData,'tblAllotDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblAllot&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblAllot,tblAllotDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAllot&allTableName=tblAllot,tblAllotDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAllot&allTableName=tblAllotDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAllot&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAllot&allTableName=tblAllotDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblAllot&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblAllot&designId=d38eddfe_1603111215040366555&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblAllot&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblAllot&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblAllot&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '同价调拨单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblAllot&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
