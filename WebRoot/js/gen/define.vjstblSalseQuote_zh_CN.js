...
{ 
 cFireEvent('tblSalseQuoteDet_Qty','1'); 
 cFireEvent('tblSalseQuoteDet_CurPrice',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblSalseQuote&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblSalseQuote&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblSalseQuote&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblSalseQuote&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblSalseQuoteDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblSalseQuoteDet"){
 
 copyFields=n('tblSalseQuoteDet_GoodsCode');
 ...
 var tblSalseQuoteDet_tblGoods_GoodsNumber = n('tblSalseQuoteDet_tblGoods_GoodsNumber');
 if(typeof(tblSalseQuoteDet_tblGoods_GoodsNumber) != 'undefined' && tblSalseQuoteDet_tblGoods_GoodsNumber.length >index){
 tblSalseQuoteDet_tblGoods_GoodsNumber[index].value = tblSalseQuoteDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblSalseQuoteDet_tblGoods_GoodsFullName = n('tblSalseQuoteDet_tblGoods_GoodsFullName');
 if(typeof(tblSalseQuoteDet_tblGoods_GoodsFullName) != 'undefined' && tblSalseQuoteDet_tblGoods_GoodsFullName.length >index){
 tblSalseQuoteDet_tblGoods_GoodsFullName[index].value = tblSalseQuoteDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblSalseQuoteDet_tblGoods_GoodsSpec = n('tblSalseQuoteDet_tblGoods_GoodsSpec');
 if(typeof(tblSalseQuoteDet_tblGoods_GoodsSpec) != 'undefined' && tblSalseQuoteDet_tblGoods_GoodsSpec.length >index){
 tblSalseQuoteDet_tblGoods_GoodsSpec[index].value = tblSalseQuoteDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblSalseQuoteDet_tblGoods_BaseUnit = n('tblSalseQuoteDet_tblGoods_BaseUnit');
 if(typeof(tblSalseQuoteDet_tblGoods_BaseUnit) != 'undefined' && tblSalseQuoteDet_tblGoods_BaseUnit.length >index){
 tblSalseQuoteDet_tblGoods_BaseUnit[index].value = tblSalseQuoteDet_tblGoods_BaseUnit[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblSalseQuoteDet"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblSalseQuote\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblSalseQuote\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblSalseQuoteDet_CurPrice',"tblSalseQuoteDet"); 
 delFireEventOne(rowId,tableName,'tblSalseQuoteDet_Price',"tblSalseQuoteDet"); 
 delFireEventOne(rowId,tableName,'tblSalseQuoteDet_TaxPrice',"tblSalseQuoteDet"); 
 }
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'tblSalseQuoteDet_CurPrice',dis:'原币单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalseQuoteDet_Price',dis:'本币未税单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSalseQuoteDet_TaxPrice',dis:'本币含税单价',digit:6,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblSalseQuote",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSalseQuote&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSalseQuote&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblSalseQuote"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblSalseQuote&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblSalseQuote&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
n('TotalAmount')[0].value=f(sm('tblSalseQuoteDet_DisBackAmt'),2);

n('TotalTaxAmount')[0].value=f(sm('tblSalseQuoteDet_TaxAmount')-nn('DiscountAmount',0),2);

n('CurTotalAmount')[0].value=f(nn('TotalTaxAmount',0)/nn('CurrencyRate',0),2);

for(var j=0;j<r(0,'CoTaxAmt').length;j++){
r(0,"CoTaxAmt")[j].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",j)*rn(0,"Discount",j)/100-rn(0,"Amount",j)*rn(0,"Discount",j)/100,6);
}

n('TotalCoTaxAmt')[0].value=f(sm('tblSalseQuoteDet_CoTaxAmt'),2);
...
n('DisAmount')[0].value=f(sm('tblSalseQuoteDet_DisAmount'),2);
...
for(var j=0;j<r(0,'CoTaxAmt').length;j++){
r(0,"CoTaxAmt")[j].value=f(rn(0,"TaxAmount",j)-rn(0,"DisBackAmt",j),6);
}
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_Qty'); 
...
n('DisAmount')[0].value=f(sm('tblSalseQuoteDet_DisAmount'),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),6);
n('TotalCoTaxAmt')[0].value=f(sm('tblSalseQuoteDet_CoTaxAmt'),2);
r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_CurPrice'); 
...
n('TotalAmount')[0].value=f(sm('tblSalseQuoteDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblSalseQuoteDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(sm('tblSalseQuoteDet_CurAmount'),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),6);
n('TotalCoTaxAmt')[0].value=f(sm('tblSalseQuoteDet_CoTaxAmt'),2);
n('DisAmount')[0].value=f(sm('tblSalseQuoteDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
n('DisAmount')[0].value=f(sm('tblSalseQuoteDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_Amount'); 
...
n('DisAmount')[0].value=f(sm('tblSalseQuoteDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('tblSalseQuoteDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_DisPrice'); 
...
n('DisAmount')[0].value=f(sm('tblSalseQuoteDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_DisBackAmt'); 
...
n('TotalTaxAmount')[0].value=f(sm('tblSalseQuoteDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(nn('TotalTaxAmount',0)/nn('CurrencyRate',0),2);
r(0,"CoTaxAmt")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"DisBackAmt",p)-rn(0,"DisBackAmt",p),6);
n('TotalCoTaxAmt')[0].value=f(sm('tblSalseQuoteDet_CoTaxAmt'),2);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
...
n('DisAmount')[0].value=f(sm('tblSalseQuoteDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_TaxPrice'); 
...
n('DisAmount')[0].value=f(sm('tblSalseQuoteDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_TaxAmount'); 
...
n('TotalAmount')[0].value=f(sm('tblSalseQuoteDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblSalseQuoteDet_TaxAmount')-nn('DiscountAmount',0),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-(((((rn(0,"TaxAmount",p)/rn(0,"Qty",p))/((nn('Tax',0)/100)+1))*100/rn(0,"Discount",p))*rn(0,"Qty",p))*rn(0,"Discount",p)/100),6);
n('TotalCoTaxAmt')[0].value=f(sm('tblSalseQuoteDet_CoTaxAmt'),2);
r(0,"CurAmount")[p].value=f(rn(0,"TaxAmount",p)/nn('CurrencyRate',0),2);
n('CurTotalAmount')[0].value=f(nn('TotalTaxAmount',0)/nn('CurrencyRate',0),2);
n('DisAmount')[0].value=f(sm('tblSalseQuoteDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_CurAmount'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),2);
...
n('DisAmount')[0].value=f(sm('tblSalseQuoteDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_CostAmount'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_CostPrice'); 
 r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSalseQuoteDet_SecQty'); 
...
childData[childCount]='tblSalseQuoteDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='销售核价明细';
childCount++;
 var tblSalseQuoteDetData = new gridData();
 gridDatas[gridDataNum] = tblSalseQuoteDetData;
 gridDataNum++;
 var tblSalseQuoteDet=1;
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_BuyAskPriceDetID','询价单号','80',2,true,'',100,-2,'','',"","","0",'',"2",false); 
 putChildValidateItem("tblSalseQuoteDet_BuyAskPriceDetID","询价单号","any","",true,minValue,100,false,''); 
 ...
 var param4="";
 param4=param4+":0仓库@tblSalseQuoteDet_StockCode";
 param4=param4+":0批号@tblSalseQuoteDet_BatchNo";
 param4=param4+":0厚度@tblSalseQuoteDet_Inch";
 param4=param4+":0颜色@tblSalseQuoteDet_Hue";
 param4=param4+":0尺码@tblSalseQuoteDet_yearNO";
 param4=param4+":0生产日期@tblSalseQuoteDet_ProDate";
 param4=param4+":0保质期限@tblSalseQuoteDet_Availably";
 param4=param4+":1客户@CompanyCode"; 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_GoodsCode','料件名称','120',2,true,'',500,-2,'','',"","copy","1",'',"2",true);
 putChildValidateItem("tblSalseQuoteDet_GoodsCode","料件名称","any","",true,minValue,500,false,'');
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'tblSalseQuoteDet:GoodsCode:tblSalseQuoteDet_tblGoods_GoodsNumber:'+param4,'',"","copy","1",'',"2",false); 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_tblGoods_GoodsFullName','料件名称','150',2,true,'',1000000,2,'tblSalseQuoteDet:GoodsCode:tblSalseQuoteDet_tblGoods_GoodsFullName:'+param4,'',"","copy","1",'',"2",false); 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'tblSalseQuoteDet:GoodsCode:tblSalseQuoteDet_tblGoods_GoodsSpec:'+param4,'',"","copy","1",'',"2",false); 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblSalseQuoteDet:GoodsCode:tblSalseQuoteDet_tblGoods_BaseUnit:'+param4,'',"","copy","1",'',"2",false); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_Inch','厚度','70',2,true,'',100,-2,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_Hue','颜色','70',2,true,'',100,-2,'','',"","","4",'',"2",false); 
 putChildValidateItem("tblSalseQuoteDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","","5",'',"2",false); 
 putChildValidateItem("tblSalseQuoteDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_Qty','数量','60',1,false,'1',0,-2,'','',"Qty","","8",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_Qty","数量","double","",false,minValue,maxValue,true,'1'); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_CurPrice','原币单价','100',1,true,'',0,0,'','',"SPriceIdentifier","","9",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_CurPrice","原币单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_Price','本币未税单价','80',1,true,'',0,8,'','',"SPriceIdentifier","","10",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_Price","本币未税单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_Amount','金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","11",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_Amount","金额","double","",true,minValue,maxValue,false,''); 
 ...
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_Discount','折扣率%','100',1,true,'100',0,-2,'','',"Price","","17",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_Discount","折扣率%","double","BillDiscount",true,minValue,maxValue,true,'100'); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_DisPrice','折后单价','100',1,true,'',0,-2,'','',"SPriceIdentifier","","18",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_DisPrice","折后单价","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_DisBackAmt','折后金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","19",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_DisBackAmt","折后金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_DisAmount','折让金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","20",'',"8",false); 
 putChildValidateItem("tblSalseQuoteDet_DisAmount","折让金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_TaxPrice','本币含税单价','100',1,true,'',0,8,'','',"SPriceIdentifier","","21",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_TaxPrice","本币含税单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_TaxAmount','含税金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","22",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_TaxAmount","含税金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_CoTaxAmt','税额','100',1,true,'0',0,-2,'','',"SPriceIdentifier","","23",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_CoTaxAmt","税额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_CurAmount','原币金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","24",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_CurAmount","原币金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_SourceID','源ID','80',2,true,'',100,-2,'','',"","","25",'',"3",false); 
 putChildValidateItem("tblSalseQuoteDet_SourceID","源ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_CostAmount','成本总额','200',1,true,'0',0,-2,'','',"AmountIdentifier","","26",'',"3",false); 
 putChildValidateItem("tblSalseQuoteDet_CostAmount","成本总额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_CostPrice','成本单价','200',1,true,'0',0,-2,'','',"priceIdentifier","","27",'',"3",false); 
 putChildValidateItem("tblSalseQuoteDet_CostPrice","成本单价","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","29",'',"3",false); 
 putChildValidateItem("tblSalseQuoteDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","30",'',"3",false); 
 putChildValidateItem("tblSalseQuoteDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_StockCode','仓库','0',2,true,'',100,-2,'','',"","","31",'',"3",false); 
 putChildValidateItem("tblSalseQuoteDet_StockCode","仓库","any","",true,minValue,100,true,''); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_Remark','描述','150',2,true,'',8000,0,'','',"","","32",'',"0",false); 
 putChildValidateItem("tblSalseQuoteDet_Remark","描述","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblSalseQuoteDetData,'tblSalseQuoteDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblSalseQuote&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblSalseQuote,tblSalseQuoteDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSalseQuote&allTableName=tblSalseQuote,tblSalseQuoteDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSalseQuote&allTableName=tblSalseQuoteDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSalseQuote&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSalseQuote&allTableName=tblSalseQuoteDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblSalseQuote&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblSalseQuote&designId=df66bd5c_1603022203459680002&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblSalseQuote&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblSalseQuote&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblSalseQuote&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '销售核价';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblSalseQuote&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
