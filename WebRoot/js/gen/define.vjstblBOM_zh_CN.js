...
{ 
 cFireEvent('tblBOMDet_Qty','1'); 
 cFireEvent('tblBOMDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBOM&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBOM&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBOM&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBOM&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBOMDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBOMDet"){
 
 copyFields=n('tblBOMDet_GoodsCode');
 ...
 var tblBOMDet_ViewBomGoods_GoodsNumber = n('tblBOMDet_ViewBomGoods_GoodsNumber');
 if(typeof(tblBOMDet_ViewBomGoods_GoodsNumber) != 'undefined' && tblBOMDet_ViewBomGoods_GoodsNumber.length >index){
 tblBOMDet_ViewBomGoods_GoodsNumber[index].value = tblBOMDet_ViewBomGoods_GoodsNumber[index-1].value;
 }
 var tblBOMDet_ViewBomGoods_GoodsFullName = n('tblBOMDet_ViewBomGoods_GoodsFullName');
 if(typeof(tblBOMDet_ViewBomGoods_GoodsFullName) != 'undefined' && tblBOMDet_ViewBomGoods_GoodsFullName.length >index){
 tblBOMDet_ViewBomGoods_GoodsFullName[index].value = tblBOMDet_ViewBomGoods_GoodsFullName[index-1].value;
 }
 var tblBOMDet_ViewBomGoods_GoodsSpec = n('tblBOMDet_ViewBomGoods_GoodsSpec');
 if(typeof(tblBOMDet_ViewBomGoods_GoodsSpec) != 'undefined' && tblBOMDet_ViewBomGoods_GoodsSpec.length >index){
 tblBOMDet_ViewBomGoods_GoodsSpec[index].value = tblBOMDet_ViewBomGoods_GoodsSpec[index-1].value;
 }
 var tblBOMDet_ViewBomGoods_Spec1 = n('tblBOMDet_ViewBomGoods_Spec1');
 if(typeof(tblBOMDet_ViewBomGoods_Spec1) != 'undefined' && tblBOMDet_ViewBomGoods_Spec1.length >index){
 tblBOMDet_ViewBomGoods_Spec1[index].value = tblBOMDet_ViewBomGoods_Spec1[index-1].value;
 }
 var tblBOMDet_ViewBomGoods_Spec2 = n('tblBOMDet_ViewBomGoods_Spec2');
 if(typeof(tblBOMDet_ViewBomGoods_Spec2) != 'undefined' && tblBOMDet_ViewBomGoods_Spec2.length >index){
 tblBOMDet_ViewBomGoods_Spec2[index].value = tblBOMDet_ViewBomGoods_Spec2[index-1].value;
 }
 var tblBOMDet_ViewBomGoods_Spec3 = n('tblBOMDet_ViewBomGoods_Spec3');
 if(typeof(tblBOMDet_ViewBomGoods_Spec3) != 'undefined' && tblBOMDet_ViewBomGoods_Spec3.length >index){
 tblBOMDet_ViewBomGoods_Spec3[index].value = tblBOMDet_ViewBomGoods_Spec3[index-1].value;
 }
 var tblBOMDet_ViewBomGoods_MaterielAttribute = n('tblBOMDet_ViewBomGoods_MaterielAttribute');
 if(typeof(tblBOMDet_ViewBomGoods_MaterielAttribute) != 'undefined' && tblBOMDet_ViewBomGoods_MaterielAttribute.length >index){
 tblBOMDet_ViewBomGoods_MaterielAttribute[index].value = tblBOMDet_ViewBomGoods_MaterielAttribute[index-1].value;
 }
 var tblBOMDet_ViewBomGoods_UnitName = n('tblBOMDet_ViewBomGoods_UnitName');
 if(typeof(tblBOMDet_ViewBomGoods_UnitName) != 'undefined' && tblBOMDet_ViewBomGoods_UnitName.length >index){
 tblBOMDet_ViewBomGoods_UnitName[index].value = tblBOMDet_ViewBomGoods_UnitName[index-1].value;
 }
 var tblBOMDet_tblGoods_ProSalesPrice = n('tblBOMDet_tblGoods_ProSalesPrice');
 if(typeof(tblBOMDet_tblGoods_ProSalesPrice) != 'undefined' && tblBOMDet_tblGoods_ProSalesPrice.length >index){
 tblBOMDet_tblGoods_ProSalesPrice[index].value = tblBOMDet_tblGoods_ProSalesPrice[index-1].value;
 }
 var tblBOMDet_tblGoods_BuyAff = n('tblBOMDet_tblGoods_BuyAff');
 if(typeof(tblBOMDet_tblGoods_BuyAff) != 'undefined' && tblBOMDet_tblGoods_BuyAff.length >index){
 tblBOMDet_tblGoods_BuyAff[index].value = tblBOMDet_tblGoods_BuyAff[index-1].value;
 }
 var tblBOMDet_tblGoods_PreBuyPrice = n('tblBOMDet_tblGoods_PreBuyPrice');
 if(typeof(tblBOMDet_tblGoods_PreBuyPrice) != 'undefined' && tblBOMDet_tblGoods_PreBuyPrice.length >index){
 tblBOMDet_tblGoods_PreBuyPrice[index].value = tblBOMDet_tblGoods_PreBuyPrice[index-1].value;
 }
 var tblBOMDet_tblGoods_BuyCost = n('tblBOMDet_tblGoods_BuyCost');
 if(typeof(tblBOMDet_tblGoods_BuyCost) != 'undefined' && tblBOMDet_tblGoods_BuyCost.length >index){
 tblBOMDet_tblGoods_BuyCost[index].value = tblBOMDet_tblGoods_BuyCost[index-1].value;
 }
 }
 copyFields=n('tblBOMDet_Unit');
 if((order > 20 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBOMDet"){
 ...
 copyFields=n('tblBOMDet_Unit'); 
 if( (order > 20 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 }
 }
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=392dc4c2_0905191727565460622&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=392dc4c2_0905191727565460622&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblBOM\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBOM\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblBOMDet_Qty',"tblBOMDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblBOMDet_Qty',dis:'用量',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBOM",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBOM&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBOM&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"392dc4c2_0905191727565460622",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 var saveField=n("Unit"); 
 saveField[0].value = "";
 var obj = m("tbl_tblGoods_BaseUnit");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("MaterielAttribute"); 
 saveField[0].value = "";
 var obj = m("tbl_tblGoods_MaterielAttribute");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblGoods_BarCode");
 saveField[0].value = ""; 
 m("tbl_tblGoods_BarCode").value = "" ;
 ...
 var saveField=n("tblGoods_Spec1");
 saveField[0].value = ""; 
 m("tbl_tblGoods_Spec1").value = "" ;
 var saveField=n("tblGoods_Spec2");
 saveField[0].value = ""; 
 m("tbl_tblGoods_Spec2").value = "" ;
 var saveField=n("tblGoods_Spec3");
 saveField[0].value = ""; 
 m("tbl_tblGoods_Spec3").value = "" ;
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=392dc4c2_0905191727565460622&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=392dc4c2_0905191727565460622&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=392dc4c2_0905191727565460622&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBOM&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBOM&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('Amount')[0].value=f(nn('Qty',0)*nn('Price',0),4);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('Price')[0].value=f(nn('Amount',0)/nn('Qty',0),4);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBOMDet_TheoryQty'); 
 r(0,"Qty")[p].value=f(rn(0,"TheoryQty",p)*(Number(rn(0,"Loss",p))/100+1),4);
r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
n('Amount')[0].value=f(sm('tblBOMDet_Amount'),4);
n('Price')[0].value=f(nn('Amount',0)/nn('Qty',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBOMDet_Loss'); 
 r(0,"Qty")[p].value=f(rn(0,"TheoryQty",p)*(Number(rn(0,"Loss",p))/100+1),4);
r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
n('Amount')[0].value=f(sm('tblBOMDet_Amount'),4);
n('Price')[0].value=f(nn('Amount',0)/nn('Qty',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBOMDet_Qty'); 
 r(0,"TheoryQty")[p].value=f(rn(0,"Qty",p)/(Number(rn(0,"Loss",p))/100+1),4);
r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
n('Amount')[0].value=f(sm('tblBOMDet_Amount'),4);
n('Price')[0].value=f(nn('Amount',0)/nn('Qty',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBOMDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
n('Amount')[0].value=f(sm('tblBOMDet_Amount'),4);
n('Price')[0].value=f(nn('Amount',0)/nn('Qty',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBOMDet_Amount'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
n('Amount')[0].value=f(sm('tblBOMDet_Amount'),4);
n('Price')[0].value=f(nn('Amount',0)/nn('Qty',0),4);
...
childData[childCount]='tblBOMDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='物料清单明细表';
childCount++;
 var tblBOMDetData = new gridData();
 gridDatas[gridDataNum] = tblBOMDetData;
 gridDataNum++;
 var tblBOMDet=1;
 ...
 addCols(tblBOMDetData,'tblBOMDet_ViewBomGoods_GoodsNumber','物料编号','120',2,true,'',1000000,2,'tblBOMDet:GoodsCode:tblBOMDet_ViewBomGoods_GoodsNumber:'+param4,"","","copy","1","","2",false);
 ...
 addCols(tblBOMDetData,'tblBOMDet_ViewBomGoods_GoodsFullName','物料名称','150',2,true,'',1000000,2,'tblBOMDet:GoodsCode:tblBOMDet_ViewBomGoods_GoodsFullName:'+param4,"","","copy","1","","2",false);
 ...
 addCols(tblBOMDetData,'tblBOMDet_ViewBomGoods_GoodsSpec','物料规格','200',2,true,'',1000000,2,'tblBOMDet:GoodsCode:tblBOMDet_ViewBomGoods_GoodsSpec:'+param4,"","","copy","1","","2",false);
 ...
 addCols(tblBOMDetData,'tblBOMDet_ViewBomGoods_UnitName','单位','50',2,true,'',1000000,2,'tblBOMDet:GoodsCode:tblBOMDet_ViewBomGoods_UnitName:'+param4,"","","copy","1","","2",false);
    ...
    addCols(tblBOMDetData,'tblBOMDet_Qty','用量','60',1,true,'1',0,0,'','',"Qty","","9",'',"0",false); 
 putChildValidateItem("tblBOMDet_Qty","用量","double","",true,minValue,maxValue,true,'1',true); 
    addCols(tblBOMDetData,'tblBOMDet_Price','单价','80',1,true,'',0,-2,'','',"priceIdentifier","","10",'',"3",false); 
 putChildValidateItem("tblBOMDet_Price","单价","double","",true,minValue,maxValue,false,''); 
    addCols(tblBOMDetData,'tblBOMDet_Amount','金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","11",'',"3",false); 
 putChildValidateItem("tblBOMDet_Amount","金额","double","",true,minValue,maxValue,false,''); 
 ...
 var param4=""; 
  ...
  addCols(tblBOMDetData,'tblBOMDet_tblGoods_ProSalesPrice','标准售价','50',1,true,'',1000000,2,'tblBOMDet:GoodsCode:tblBOMDet_tblGoods_ProSalesPrice:'+param4,"","","copy","1","","2",false);
 ...
 var param4=""; 
  ...
  addCols(tblBOMDetData,'tblBOMDet_tblGoods_BuyAff','采购附加值','50',1,true,'',1000000,2,'tblBOMDet:GoodsCode:tblBOMDet_tblGoods_BuyAff:'+param4,"","","copy","1","","2",false);
 ...
 var param4=""; 
  ...
  addCols(tblBOMDetData,'tblBOMDet_tblGoods_PreBuyPrice','标准购价','50',1,true,'',1000000,2,'tblBOMDet:GoodsCode:tblBOMDet_tblGoods_PreBuyPrice:'+param4,"","","copy","1","","2",false);
 ...
 var param4=""; 
  ...
  addCols(tblBOMDetData,'tblBOMDet_tblGoods_BuyCost','采购成本','50',1,true,'',1000000,2,'tblBOMDet:GoodsCode:tblBOMDet_tblGoods_BuyCost:'+param4,"","","copy","1","","2",false);
    ...
    addCols(tblBOMDetData,'tblBOMDet_SpComponent','归属部件','100',2,true,'',10,1,';0:其它;11:表壳(LCD);1:表壳;2:表面;12:表面(MOP);3:钢带;13:胶带;14:皮带;4:表针;5:巴的;6:扣制;7:玻璃;8:生耳;9:石仔;10:机芯','',"","","28",'',"1",false);
 putChildValidateItem("tblBOMDet_SpComponent","归属部件","any","",true,minValue,10,false,'',true); 
   addCols(tblBOMDetData,'tblBOMDet_materielOrder','序号','80',2,true,'',50,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblBOMDet_materielOrder","序号","any","",true,minValue,50,false,''); 
 var param4="";
 addCols(tblBOMDetData,'tblBOMDet_GoodsCode','物料名称','100',2,false,'',60,-2,'','','',"copy",'','',"2",false);
 putChildValidateItem("tblBOMDet_GoodsCode","物料名称","any","",false,minValue,60,false,'',true);
         addCols(tblBOMDetData,'tblBOMDet_ViewBomGoods_Spec1','Spec1','100',2,false,'',60,-2,'',"2",false);
   addCols(tblBOMDetData,'tblBOMDet_ViewBomGoods_Spec2','Spec2','100',2,false,'',60,-2,'',"2",false);
   addCols(tblBOMDetData,'tblBOMDet_ViewBomGoods_Spec3','Spec3','100',2,false,'',60,-2,'',"2",false);
   addCols(tblBOMDetData,'tblBOMDet_ViewBomGoods_MaterielAttribute','物料属性','100',2,false,'',60,-2,'',"2",false);
              addCols(tblBOMDetData,'tblBOMDet_BatchNo','批号','120',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblBOMDet_BatchNo","批号","any","",true,minValue,100,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_Inch','宽度','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblBOMDet_Inch","宽度","any","",true,minValue,100,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_Hue','颜色','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblBOMDet_Hue","颜色","any","",true,minValue,100,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_yearNO','高度','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblBOMDet_yearNO","高度","any","",true,minValue,100,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_TheoryQty','理论数量','60',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_TheoryQty","理论数量","double","",true,minValue,maxValue,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_Loss','损耗率％','60',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_Loss","损耗率％","double","",true,minValue,maxValue,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_ConversionRate','换算率','80',1,true,'1',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_ConversionRate","换算率","double","",true,minValue,maxValue,true,'1'); 
          addCols(tblBOMDetData,'tblBOMDet_MaterielAttribute','物料属性','70',2,true,'WG',30,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_MaterielAttribute","物料属性","any","",true,minValue,30,true,'WG'); 
   addCols(tblBOMDetData,'tblBOMDet_MaterielType','物料类型','70',2,true,'MainMaterials',30,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_MaterielType","物料类型","any","",true,minValue,30,true,'MainMaterials'); 
   addCols(tblBOMDetData,'tblBOMDet_WastePercent','损耗率','100',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_WastePercent","损耗率","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblBOMDetData,'tblBOMDet_FactQty','实用数量','100',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_FactQty","实用数量","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblBOMDetData,'tblBOMDet_FactAmont','实际金额','100',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_FactAmont","实际金额","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblBOMDetData,'tblBOMDet_Remark','备注','200',2,true,'',200,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_Remark","备注","any","",true,minValue,200,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_Place','位置','224',2,true,'',500,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_Place","位置","any","",true,minValue,500,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_Unit','单位','80',2,true,'',60,-2,'','','',"copy",'','',"2",false); 
 putChildValidateItem("tblBOMDet_Unit","单位","any","",true,minValue,60,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_EId','研发ID','224',2,true,'',50,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_EId","研发ID","any","",true,minValue,50,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_color','色彩','60',2,true,'',5000,-2,'','','',"",'','',"2",false); 
 putChildValidateItem("tblBOMDet_color","色彩","any","",true,minValue,5000,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_colorNV','颜色名称','100',2,true,'',5000,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblBOMDet_colorNV","颜色名称","any","",true,minValue,5000,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_BomId','成品Id','224',2,true,'',50,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_BomId","成品Id","any","",true,minValue,50,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_buildDosage','组成用量','90',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_buildDosage","组成用量","double","",true,minValue,maxValue,false,''); 
   addCols(tblBOMDetData,'tblBOMDet_hostBase','主件底数','90',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBOMDet_hostBase","主件底数","double","",true,minValue,maxValue,false,''); 
   
 addCols(tblBOMDetData,'tblBOMDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBOM&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBOM,tblBOMDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBOM&allTableName=tblBOM,tblBOMDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBOM&allTableName=tblBOMDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBOM&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBOM&allTableName=tblBOMDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=392dc4c2_0905191727565460622&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=392dc4c2_0905191727565460622&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=392dc4c2_0905191727565460622&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=392dc4c2_0905191727565460622&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblBOM&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=392dc4c2_0905191727565460622&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblBOM&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBOM&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBOM&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBOM&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '生产BOM';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBOM&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
