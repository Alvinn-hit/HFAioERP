...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblTransferSale1&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblTransferSale1&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblTransferSale1&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblTransferSale1&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblTransferSaleDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblTransferSaleDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblTransferSaleDet1"){
...
function execStat2(){
 execStatOne('tblTransferSaleDet1_BillAmt',2);
 execStatOne('tblTransferSaleDet1_SettledAmt',2);
 execStatOne('tblTransferSaleDet1_WexeBalAmt',2);
 execStatOne('tblTransferSaleDet1_ExeBalAmt',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblTransferSale1\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblTransferSale1\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblTransferSaleDet1_ExeBalAmt',"tblTransferSaleDet1"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'ExeTransferAmt',dis:'转账金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblTransferSaleDet1_BillAmt',dis:'单据金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblTransferSaleDet1_SettledAmt',dis:'已结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblTransferSaleDet1_WexeBalAmt',dis:'待结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblTransferSaleDet1_ExeBalAmt',dis:'本次结算金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblTransferSale1",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblTransferSale1&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblTransferSale1&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblTransferSale1"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblTransferSale1&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblTransferSale1&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('ExeTransferAmt')[0].value=f(sm('tblTransferSaleDet1_ExeBalAmt'),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblTransferSaleDet1_ExeBalAmt'); 
 n('ExeTransferAmt')[0].value=f(sm('tblTransferSaleDet1_ExeBalAmt'),2);
...
childData[childCount]='tblTransferSaleDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='预收转应收明细';
childCount++;
 var tblTransferSaleDet1Data = new gridData();
 gridDatas[gridDataNum] = tblTransferSaleDet1Data;
 gridDataNum++;
 var tblTransferSaleDet1=1;
 ...
 addCols(tblTransferSaleDet1Data,'tblTransferSaleDet1_RefbillID','关联单据编号','120',2,true,'',70,2,'tblTransferSaleDet1:RefbillID:tblTransferSaleDet1_RefbillID:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblTransferSaleDet1_RefbillID","关联单据编号","any","",true,minValue,70,false,'',true);
 
 
 
 addCols(tblTransferSaleDet1Data,'tblTransferSaleDet1_BillAmt','单据金额','100',1,true,'',0,8,'','',"AmountIdentifier","","2",'',"8",false); 
 putChildValidateItem("tblTransferSaleDet1_BillAmt","单据金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblTransferSaleDet1Data,'tblTransferSaleDet1_SettledAmt','已结算金额','100',1,true,'',0,0,'','',"AmountIdentifier","","3",'',"0",false); 
 putChildValidateItem("tblTransferSaleDet1_SettledAmt","已结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblTransferSaleDet1Data,'tblTransferSaleDet1_WexeBalAmt','待结算金额','100',1,true,'',0,0,'','',"AmountIdentifier","","4",'',"0",false); 
 putChildValidateItem("tblTransferSaleDet1_WexeBalAmt","待结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblTransferSaleDet1Data,'tblTransferSaleDet1_ExeBalAmt','本次结算金额','100',1,true,'',0,0,'','',"AmountIdentifier","","5",'',"0",false); 
 putChildValidateItem("tblTransferSaleDet1_ExeBalAmt","本次结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 
 addCols(tblTransferSaleDet1Data,'tblTransferSaleDet1_Remark','备注','200',2,true,'',8000,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblTransferSaleDet1_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblTransferSaleDet1Data,'tblTransferSaleDet1_SalesOrderID','销售订单ID','0',2,true,'',100,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblTransferSaleDet1_SalesOrderID","销售订单ID","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblTransferSaleDet1Data,'tblTransferSaleDet1_SalesOrderNo','销售订单编号','0',2,true,'',100,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblTransferSaleDet1_SalesOrderNo","销售订单编号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblTransferSaleDet1Data,'tblTransferSaleDet1_RefSalesOutID','关联单据ID','0',2,true,'',100,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblTransferSaleDet1_RefSalesOutID","关联单据ID","any","",true,minValue,100,false,''); 
 
 
 
 
 
 addCols(tblTransferSaleDet1Data,'tblTransferSaleDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblTransferSale1&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblTransferSale1,tblTransferSaleDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblTransferSale1&allTableName=tblTransferSale1,tblTransferSaleDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblTransferSale1&allTableName=tblTransferSaleDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblTransferSale1&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblTransferSale1&allTableName=tblTransferSaleDet1,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblTransferSale1Det";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblTransferSale1&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblTransferSale1&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblTransferSale1&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblTransferSale1&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblTransferSale1&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '预收转应收';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblTransferSale1&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
