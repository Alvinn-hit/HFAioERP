...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblFixedAssetChange&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblFixedAssetChange&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblFixedAssetChange&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblFixedAssetChange&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblFixedAssetChangeDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblFixedAssetChangeDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblFixedAssetChangeDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblFixedAssetChangeDet"; 
...
function execStat2(){
 execStatOne('tblFixedAssetChangeDet_Qty',2);
 execStatOne('tblFixedAssetChangeDet_Amount',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblFixedAssetChange\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblFixedAssetChange\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'totalDeprecia',dis:'累计折旧',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'depreciaArrange',dis:'减值准备',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'netWorth',dis:'净值',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'planReWorthRate',dis:'预计净残值率',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'planReWorth',dis:'预计净残值',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'MonthWork',dis:'当月工作量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'workunitDeprecAmt',dis:'单位折旧额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'monthDeprecRate',dis:'月折旧率',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'monthDeprecAmt',dis:'月折旧额',digit:8,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblFixedAssetChangeDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblFixedAssetChangeDet_Amount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblFixedAssetChange",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblFixedAssetChange&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblFixedAssetChange&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblFixedAssetChange"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblFixedAssetChange&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblFixedAssetChange&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('netWorth')[0].value=f(nn('originalWorth',0)-nn('totalDeprecia',0)-nn('depreciaArrange',0),8);

n('planReWorth')[0].value=f(nn('originalWorth',0)*nn('planReWorthRate',0),8);

n('planReWorthRate')[0].value=f(nn('planReWorth',0)/nn('originalWorth',0),8);

if(n('depreciaMonth')[0].value==1)n('monthDeprecAmt')[0].value=f((nn('netWorth',0)-nn('planReWorth',0))/(nn('planUseMonth',0)-nn('referMonth',0)),8);

if(n('depreciaMonth')[0].value==1)n('monthDeprecRate')[0].value=f(nn('monthDeprecAmt',0)/nn('originalWorth',0),8);

if(n('depreciaMonth')[0].value==2)n('workunitDeprecAmt')[0].value=f((nn('netWorth',0)-nn('planReWorth',0))/(nn('planWork',0)-nn('referWork',0)),2);

if(n('depreciaMonth')[0].value==2)n('monthDeprecAmt')[0].value=f(nn('workunitDeprecAmt',0)*nn('MonthWork',0),8);

if(n('depreciaMonth')[0].value==2)n('monthDeprecRate')[0].value=f(nn('workunitDeprecAmt',0)*nn('MonthWork',0)/nn('originalWorth',0),8);

if(n('depreciaMonth')[0].value==3)n('monthDeprecRate')[0].value=f(2/nn('planUseMonth',0),8);

if(n('depreciaMonth')[0].value==3)n('monthDeprecAmt')[0].value=f(nn('netWorth',0)*nn('monthDeprecRate',0),8);

if(n('depreciaMonth')[0].value==4)n('monthDeprecRate')[0].value=f((2*(nn('planUseMonth',0)-nn('referMonth',0)))/(nn('planUseMonth',0)*(nn('planUseMonth',0)+12)),8);

if(n('depreciaMonth')[0].value==4)n('monthDeprecAmt')[0].value=f((nn('originalWorth',0)-nn('planReWorth',0))*nn('monthDeprecRate',0),8);
...
childData[childCount]='tblFixedAssetChangeDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='固定资产附属设备表';
childCount++;
 var tblFixedAssetChangeDetData = new gridData();
 gridDatas[gridDataNum] = tblFixedAssetChangeDetData;
 gridDataNum++;
 var tblFixedAssetChangeDet=1;
 ...
 addCols(tblFixedAssetChangeDetData,'tblFixedAssetChangeDet_GoodsCode','设备名称','200',2,false,'',50,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("tblFixedAssetChangeDet_GoodsCode","设备名称","any","",false,minValue,50,false,'');
 addCols(tblFixedAssetChangeDetData,'tblFixedAssetChangeDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'tblFixedAssetChangeDet:GoodsCode:tblFixedAssetChangeDet_tblGoods_GoodsNumber:'+param4,'',"","","1",'',"2",false); 
 addCols(tblFixedAssetChangeDetData,'tblFixedAssetChangeDet_tblGoods_GoodsFullName','设备名称','150',18,true,'',1000000,2,'tblFixedAssetChangeDet:GoodsCode:tblFixedAssetChangeDet_tblGoods_GoodsFullName:'+param4,'',"","","1",'',"2",false); 
 addCols(tblFixedAssetChangeDetData,'tblFixedAssetChangeDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'tblFixedAssetChangeDet:GoodsCode:tblFixedAssetChangeDet_tblGoods_GoodsSpec:'+param4,'',"","","1",'',"2",false); 
 addCols(tblFixedAssetChangeDetData,'tblFixedAssetChangeDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblFixedAssetChangeDet:GoodsCode:tblFixedAssetChangeDet_tblGoods_BaseUnit:'+param4,'',"","","1",'',"2",false); 
 addCols(tblFixedAssetChangeDetData,'tblFixedAssetChangeDet_tblGoods_BarCode','条码','100',2,true,'',1000000,2,'tblFixedAssetChangeDet:GoodsCode:tblFixedAssetChangeDet_tblGoods_BarCode:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(tblFixedAssetChangeDetData,'tblFixedAssetChangeDet_Qty','数量','200',1,false,'0',0,0,'','',"Qty","","5",'',"0",false); 
 putChildValidateItem("tblFixedAssetChangeDet_Qty","数量","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblFixedAssetChangeDetData,'tblFixedAssetChangeDet_Amount','金额','200',1,false,'0',0,0,'','',"Amount","","7",'',"0",false); 
 putChildValidateItem("tblFixedAssetChangeDet_Amount","金额","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblFixedAssetChangeDetData,'tblFixedAssetChangeDet_Remark','备注','200',2,true,'',8000,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblFixedAssetChangeDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 
 addCols(tblFixedAssetChangeDetData,'tblFixedAssetChangeDet_SourceID','源ID','80',2,true,'',100,-2,'','',"","","101",'',"3",false); 
 putChildValidateItem("tblFixedAssetChangeDet_SourceID","源ID","any","",true,minValue,100,false,''); 
 
 
 addCols(tblFixedAssetChangeDetData,'tblFixedAssetChangeDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblFixedAssetChange&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblFixedAssetChange,tblFixedAssetChangeDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblFixedAssetChange&allTableName=tblFixedAssetChange,tblFixedAssetChangeDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblFixedAssetChange&allTableName=tblFixedAssetChangeDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblFixedAssetChange&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblFixedAssetChange&allTableName=tblFixedAssetChangeDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblFixedAssetChangeDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblFixedAssetChange&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblFixedAssetChange&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblFixedAssetChange&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblFixedAssetChange&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblFixedAssetChange&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '资产变更';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblFixedAssetChange&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
