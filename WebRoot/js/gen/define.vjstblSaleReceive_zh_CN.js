...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblSaleReceive&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblSaleReceive&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblSaleReceive&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblSaleReceive&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblReceiveAccountDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblSaleReceiveDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblReceiveAccountDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblSaleReceiveDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblReceiveAccountDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblSaleReceiveDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblReceiveAccountDet"; 
 tblName[1]="tblSaleReceiveDet"; 
...
function execStat2(){
 execStatOne('tblReceiveAccountDet_ExeBalFcAmt',2);
 execStatOne('tblReceiveAccountDet_Amount',2);
 execStatOne('tblSaleReceiveDet_ExeBalAmt',2);
 execStatOne('tblSaleReceiveDet_CurBillAmt',2);
 execStatOne('tblSaleReceiveDet_CurSettledAmt',2);
 execStatOne('tblSaleReceiveDet_CurWexeBalAmt',2);
 execStatOne('tblSaleReceiveDet_ExeBalFcAmt',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblSaleReceive\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblSaleReceive\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblReceiveAccountDet_ExeBalFcAmt',"tblReceiveAccountDet"); 
 delFireEventOne(rowId,tableName,'tblReceiveAccountDet_Amount',"tblReceiveAccountDet"); 
 delFireEventOne(rowId,tableName,'tblSaleReceiveDet_ExeBalAmt',"tblSaleReceiveDet"); 
 delFireEventOne(rowId,tableName,'tblSaleReceiveDet_ExeBalFcAmt',"tblSaleReceiveDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'ExeBalAmt',dis:'本币结算合计',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CurrencyRate',dis:'汇率',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'BillFcAmt',dis:'原币结算金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblReceiveAccountDet_ExeBalFcAmt',dis:'原币结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblReceiveAccountDet_Amount',dis:'本币结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSaleReceiveDet_ExeBalAmt',dis:'本币本次结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSaleReceiveDet_CurBillAmt',dis:'原币单据金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSaleReceiveDet_CurSettledAmt',dis:'原币已结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSaleReceiveDet_CurWexeBalAmt',dis:'原币待结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSaleReceiveDet_ExeBalFcAmt',dis:'原币本次结算金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblSaleReceive",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSaleReceive&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSaleReceive&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 saveField[0].value = "";
 var obj = m("tbl_ViewCompanyTotal_CompanyCode");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("EmployeeID"); 
 saveField[0].value = "";
 var obj = m("tbl_case when len(ViewCompanyTotal_EmployeeID)>0 then ViewCompanyTotal.EmployeeID else '@Sess:UserId' end");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("DepartmentCode"); 
 saveField[0].value = "";
 var obj = m("tbl_case when len(ViewCompanyTotal_DepartmentCode)>0 then ViewCompanyTotal.DepartmentCode else '@Sess:DepartmentCode' end");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblEmployee_EmpFullName"); 
 saveField[0].value = "";
 var obj = m("tbl_case when len(ViewCompanyTotal_EmployeeID)>0 then ViewCompanyTotal.EmpFullName else '@Sess:UserName' end");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblDepartment_DeptFullName"); 
 saveField[0].value = "";
 var obj = m("tbl_case when len(ViewCompanyTotal_DepartmentCode)>0 then ViewCompanyTotal.DeptFullName else '@Sess:DepartmentName' end");
 if(obj != null){
 obj.value = "";
 }
 ...
 var saveField=n("ViewCompanyTotal_ComNumber");
 saveField[0].value = ""; 
 m("tbl_ViewCompanyTotal_ComNumber").value = "" ;
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblSaleReceive"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblSaleReceive&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblSaleReceive&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 var z=n('AcceptTypeID');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('FactIncome')[0].value=f(nn('ExeBalAmt',0),2);

if(n('AcceptTypeID')[0].value=="RetrunReceive")n('FactIncome')[0].value=f(Number(nn('ExeBalAmt',0)),2);

if(n('AcceptTypeID')[0].value=="RetrunReceive")n('ExeBalAmt')[0].value=f(Number(sm('tblSaleReceiveDet_ExeBalAmt')),2);

if(n('AcceptTypeID')[0].value=="ReturnPay")n('FactIncome')[0].value=f(Number(nn('ExeBalAmt',0)),2);

if(n('AcceptTypeID')[0].value=="ReturnPay")n('ExeBalAmt')[0].value=f(Number(sm('tblSaleReceiveDet_ExeBalAmt')),2);

n('BillFcAmt')[0].value=f(nn('ExeBalAmt',0)/nn('CurrencyRate',0),2);

n('FactIncome')[0].value=f(Number(sm('tblReceiveAccountDet_Amount')),2);

n('AccAmt')[0].value=f(sm('tblReceiveAccountDet_Amount'),2);

n('ExeBalAmt')[0].value=f(Number(sm('tblReceiveAccountDet_Amount')),2);

if(n('AcceptTypeID')[0].value=="RetrunReceive")n('ExeBalAmt')[0].value=f(Number(sm('tblReceiveAccountDet_Amount')),2);

if(n('AcceptTypeID')[0].value=="ReturnPay")n('ExeBalAmt')[0].value=f(Number(sm('tblReceiveAccountDet_Amount')),2);
...
n('FactIncome')[0].value=f(Number(nn('ExeBalAmt',0)),2);
...
r(0,"Amount")[j].value=f(rn(0,"ExeBalFcAmt",j)*nn('CurrencyRate',0),2);
}
...
for(var j=0;j<r(1,'ExeBalAmt').length;j++){
r(1,"ExeBalAmt")[j].value=f(rn(1,"ExeBalFcAmt",j)*nn('CurrencyRate',0),2);
}
...
n('BillFcAmt')[0].value=f(nn('ExeBalAmt',0)/nn('CurrencyRate',0),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('ExeBalAmt')[0].value=f(nn('BillFcAmt',0)*nn('CurrencyRate',0),2);
...
function initCalculate(){
 var z=r(0,'ExeBalFcAmt');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblReceiveAccountDet_ExeBalFcAmt'); 
 r(0,"Amount")[p].value=f(rn(0,"ExeBalFcAmt",p)*nn('CurrencyRate',0),2);
n('FactIncome')[0].value=f(Number(sm('tblReceiveAccountDet_Amount')),2);
n('AccAmt')[0].value=f(sm('tblReceiveAccountDet_Amount'),2);
n('ExeBalAmt')[0].value=f(Number(sm('tblReceiveAccountDet_Amount')),2);
if(n('AcceptTypeID')[0].value=="RetrunReceive")n('ExeBalAmt')[0].value=f(Number(sm('tblReceiveAccountDet_Amount')),2);
...
n('BillFcAmt')[0].value=f(nn('ExeBalAmt',0)/nn('CurrencyRate',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblReceiveAccountDet_Amount'); 
 n('FactIncome')[0].value=f(Number(sm('tblReceiveAccountDet_Amount')),2);
n('AccAmt')[0].value=f(sm('tblReceiveAccountDet_Amount'),2);
n('ExeBalAmt')[0].value=f(Number(sm('tblReceiveAccountDet_Amount')),2);
if(n('AcceptTypeID')[0].value=="RetrunReceive")n('ExeBalAmt')[0].value=f(Number(sm('tblReceiveAccountDet_Amount')),2);
if(n('AcceptTypeID')[0].value=="ReturnPay")n('ExeBalAmt')[0].value=f(Number(sm('tblReceiveAccountDet_Amount')),2);
r(0,"ExeBalFcAmt")[p].value=f(rn(0,"Amount",p)/nn('CurrencyRate',0),2);
n('BillFcAmt')[0].value=f(nn('ExeBalAmt',0)/nn('CurrencyRate',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSaleReceiveDet_ExeBalAmt'); 
 r(1,"ExeBalFcAmt")[p].value=f(rn(1,"ExeBalAmt",p)/nn('CurrencyRate',0),2);

 execStat(); 
 }
 }
 
 var z=r(1,'CurBillAmt');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){
 execStat(); 
 }
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSaleReceiveDet_ExeBalFcAmt'); 
 r(1,"ExeBalAmt")[p].value=f(rn(1,"ExeBalFcAmt",p)*nn('CurrencyRate',0),2);
...
childData[childCount]='tblReceiveAccountDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='收款账户明细表';
childCount++;
 var tblReceiveAccountDetData = new gridData();
 gridDatas[gridDataNum] = tblReceiveAccountDetData;
 gridDataNum++;
 var tblReceiveAccountDet=1;
 
 
 
 
 
 addCols(tblReceiveAccountDetData,'tblReceiveAccountDet_SettleType','收款方式','80',2,true,'1',100,1,';1:现金;2:支票;3:电汇;4:商业汇票;5:银行汇票','',"","","0",'',"1",false);
 putChildValidateItem("tblReceiveAccountDet_SettleType","收款方式","any","",true,minValue,100,true,'1',true); 
 ...
 addCols(tblReceiveAccountDetData,'tblReceiveAccountDet_Account','账户','150',2,false,'',100,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("tblReceiveAccountDet_Account","账户","any","",false,minValue,100,false,'');
 
 
 addCols(tblReceiveAccountDetData,'tblReceiveAccountDet_tblAccTypeInfo_AccName','账户','150',2,true,'',1000000,2,'tblReceiveAccountDet:Account:tblReceiveAccountDet_tblAccTypeInfo_AccName:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(tblReceiveAccountDetData,'tblReceiveAccountDet_ExeBalFcAmt','原币结算金额','80',1,true,'',0,0,'','',"SAmountIdentifier","","2",'',"0",false); 
 putChildValidateItem("tblReceiveAccountDet_ExeBalFcAmt","原币结算金额","double","currency",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblReceiveAccountDetData,'tblReceiveAccountDet_Amount','本币结算金额','100',1,true,'0',0,8,'','',"SAmountIdentifier","","3",'',"0",false); 
 putChildValidateItem("tblReceiveAccountDet_Amount","本币结算金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblReceiveAccountDetData,'tblReceiveAccountDet_Remark','账户备注','150',2,true,'',8000,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblReceiveAccountDet_Remark","账户备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblReceiveAccountDetData,'tblReceiveAccountDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblSaleReceiveDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='收款单明细';
childCount++;
 var tblSaleReceiveDetData = new gridData();
 gridDatas[gridDataNum] = tblSaleReceiveDetData;
 gridDataNum++;
 var tblSaleReceiveDet=1;
 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_ItemName','项目名称','100',2,true,'',100,-2,'','',"","","0",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_ItemName","项目名称","any","",true,minValue,100,false,''); 
 ...
 param4=param4+":1收款类型@AcceptTypeID"; 
 param4=param4+":1单据日期@BillDate"; 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_RefbillNo','关联单据编号','100',2,true,'',80,2,'tblSaleReceiveDet:RefbillNo:tblSaleReceiveDet_RefbillNo:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblSaleReceiveDet_RefbillNo","关联单据编号","any","",true,minValue,80,false,'',true);
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_BillAmt','单据金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","2",'',"8",false); 
 putChildValidateItem("tblSaleReceiveDet_BillAmt","单据金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_BillName','模块名称','100',2,true,'',100,8,'','',"","","3",'',"8",false); 
 putChildValidateItem("tblSaleReceiveDet_BillName","模块名称","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_SettledAmt','已结算金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","4",'',"8",false); 
 putChildValidateItem("tblSaleReceiveDet_SettledAmt","已结算金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_BackAmt','退货金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","5",'',"8",false); 
 putChildValidateItem("tblSaleReceiveDet_BackAmt","退货金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_WexeBalAmt','待结算金额','100',1,true,'',0,-2,'','',"SAmountIdentifier","","6",'',"8",false); 
 putChildValidateItem("tblSaleReceiveDet_WexeBalAmt","待结算金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_ExeBalAmt','本币本次结算金额','100',1,true,'',0,8,'','',"SAmountIdentifier","","7",'',"0",false); 
 putChildValidateItem("tblSaleReceiveDet_ExeBalAmt","本币本次结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_CurBillAmt','原币单据金额','80',1,true,'0',0,8,'','',"SAmountIdentifier","","8",'',"0",false); 
 putChildValidateItem("tblSaleReceiveDet_CurBillAmt","原币单据金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_CurSettledAmt','原币已结算金额','80',1,true,'0',0,8,'','',"Amount","","9",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_CurSettledAmt","原币已结算金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_CurBackAmt','原币退货金额','80',1,true,'0',0,-2,'','',"Amount","","10",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_CurBackAmt","原币退货金额","double","currency",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_CurWexeBalAmt','原币待结算金额','80',1,true,'0',0,8,'','',"Amount","","11",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_CurWexeBalAmt","原币待结算金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_ExeBalFcAmt','原币本次结算金额','100',1,true,'0',0,0,'','',"SAmountIdentifier","","12",'',"0",false); 
 putChildValidateItem("tblSaleReceiveDet_ExeBalFcAmt","原币本次结算金额","double","currency",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_EmployeeID','经手人','80',2,true,'',100,-2,'','',"","","13",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_EmployeeID","经手人","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_DepartmentCode','部门','80',2,true,'',100,-2,'','',"","","14",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_DepartmentCode","部门","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_Remark','备注','150',2,true,'',8000,0,'','',"","","15",'',"0",false); 
 putChildValidateItem("tblSaleReceiveDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_SalesOrderID','销售订单ID','0',2,true,'',80,-2,'','',"","","18",'',"0",false); 
 putChildValidateItem("tblSaleReceiveDet_SalesOrderID","销售订单ID","any","",true,minValue,80,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_SalesOrderNo','销售订单编号','0',2,true,'',80,-2,'','',"","","19",'',"0",false); 
 putChildValidateItem("tblSaleReceiveDet_SalesOrderNo","销售订单编号","any","",true,minValue,80,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_SourceID','SourceID','224',2,true,'',50,-2,'','',"","","20",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_SourceID","SourceID","any","",true,minValue,50,false,''); 
 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_BankRecordID','银行到账记录ID','0',2,true,'',30,-2,'','',"","","22",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_BankRecordID","银行到账记录ID","any","",true,minValue,30,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_CompanyCode','往来单位','200',2,true,'',50,-2,'','',"","","23",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_CompanyCode","往来单位","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_ReceiveBillType','收款单据类型','200',2,true,'',100,-2,'','',"","","24",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_ReceiveBillType","收款单据类型","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_RefBillDate','单据日期','200',5,true,'',10,-2,'','',"","","25",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_RefBillDate","单据日期","date","",true,minValue,10,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_RefbillID','关联单据ID','0',2,true,'',80,-2,'','',"","","26",'',"0",false); 
 putChildValidateItem("tblSaleReceiveDet_RefbillID","关联单据ID","any","",true,minValue,80,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_SubCode','科目编号','224',2,true,'',100,-2,'','',"","","27",'',"3",false); 
 putChildValidateItem("tblSaleReceiveDet_SubCode","科目编号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_Currency','币种','200',2,true,'',50,-2,'','',"","","28",'',"0",false); 
 putChildValidateItem("tblSaleReceiveDet_Currency","币种","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblSaleReceiveDetData,'tblSaleReceiveDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblSaleReceive&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblSaleReceive,tblReceiveAccountDet,tblSaleReceiveDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSaleReceive&allTableName=tblSaleReceive,tblReceiveAccountDet,tblSaleReceiveDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSaleReceive&allTableName=tblReceiveAccountDet,tblSaleReceiveDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSaleReceive&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSaleReceive&allTableName=tblReceiveAccountDet,tblSaleReceiveDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblSaleReceiveDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblSaleReceive&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblSaleReceive&designId=eae3edac_0906121936591560241&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblSaleReceive&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblSaleReceive&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblSaleReceive&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '收款单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblSaleReceive&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
