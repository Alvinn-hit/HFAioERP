...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblWatchBand&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblWatchBand&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblWatchBand&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblWatchBand&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchBandDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchBandDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblWatchBandDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblWatchBandDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchBandDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchBandDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblWatchBandDet"; 
 tblName[1]="tblWatchBandDet1"; 
...
function execStat2(){
 execStatOne('tblWatchBandDet_Qty',2);
 setTotalCount(getTotalCount());
}
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblWatchBand\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblWatchBandDet_Qty',dis:'用量',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblWatchBand",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchBand&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchBand&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblWatchBand"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblWatchBand&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchBand&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblWatchBandDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表带物料明细';
childCount++;
 var tblWatchBandDetData = new gridData();
 gridDatas[gridDataNum] = tblWatchBandDetData;
 gridDataNum++;
 var tblWatchBandDet=1;
 ...
 addCols(tblWatchBandDetData,'tblWatchBandDet_GoodsCode','物料名称','100',2,true,'',1000,-2,'','',"","","1",'',"3",false); 
 putChildValidateItem("tblWatchBandDet_GoodsCode","物料名称","any","",true,minValue,1000,false,''); 
 ...
 addCols(tblWatchBandDetData,'tblWatchBandDet_GoodsNumber','物料编号','200',2,true,'',500,2,'tblWatchBandDet:GoodsNumber:tblWatchBandDet_GoodsNumber:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblWatchBandDet_GoodsNumber","物料编号","any","",true,minValue,500,false,'',true);
 
 
 
 addCols(tblWatchBandDetData,'tblWatchBandDet_CaseGrind','带身打磨','120',2,true,'',200,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblWatchBandDet_CaseGrind","带身打磨","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchBandDetData,'tblWatchBandDet_CaseElectro','带身电镀','120',2,true,'',200,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblWatchBandDet_CaseElectro","带身电镀","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchBandDetData,'tblWatchBandDet_CircleGrind','扣制打磨','120',2,true,'',200,0,'','',"","","14",'',"0",false); 
 putChildValidateItem("tblWatchBandDet_CircleGrind","扣制打磨","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchBandDetData,'tblWatchBandDet_CircleElectro','扣制电镀','120',2,true,'',200,0,'','',"","","15",'',"0",false); 
 putChildValidateItem("tblWatchBandDet_CircleElectro","扣制电镀","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchBandDetData,'tblWatchBandDet_Qty','用量','80',1,true,'1',0,0,'','',"","","16",'',"0",false); 
 putChildValidateItem("tblWatchBandDet_Qty","用量","double","",true,minValue,maxValue,true,'1',true); 
 
 
 
 addCols(tblWatchBandDetData,'tblWatchBandDet_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchBandDet_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 
 addCols(tblWatchBandDetData,'tblWatchBandDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblWatchBandDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表带变更明细';
childCount++;
 var tblWatchBandDet1Data = new gridData();
 gridDatas[gridDataNum] = tblWatchBandDet1Data;
 gridDataNum++;
 var tblWatchBandDet1=1;
 ...
 addCols(tblWatchBandDet1Data,'tblWatchBandDet1_ChangeContent','变更内容','200',2,false,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblWatchBandDet1_ChangeContent","变更内容","any","",false,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchBandDet1Data,'tblWatchBandDet1_ChangeDate','变更日期','90',5,false,'2013-12-24',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchBandDet1_ChangeDate","变更日期","date","",false,minValue,10,true,'2013-12-24',true); 
 
 
 
 addCols(tblWatchBandDet1Data,'tblWatchBandDet1_ChangePer','变更人员','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchBandDet1_ChangePer","变更人员","any","",true,minValue,100,false,'',true); 
 
 
 
 
 addCols(tblWatchBandDet1Data,'tblWatchBandDet1_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchBandDet1_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchBandDet1Data,'tblWatchBandDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblWatchBand&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=68fc380d_1408131539576500232&noback="+noback+"&parentCode=&operation=6&parentTableName=tblSampleOrder&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblWatchBand,tblWatchBandDet,tblWatchBandDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchBand&allTableName=tblWatchBand,tblWatchBandDet,tblWatchBandDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchBand&allTableName=tblWatchBandDet,tblWatchBandDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchBand&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchBand&allTableName=tblWatchBandDet,tblWatchBandDet1,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblWatchBand&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblWatchBand&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblWatchBand&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchBand&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblWatchBand&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '表带(金属)';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblWatchBand&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
