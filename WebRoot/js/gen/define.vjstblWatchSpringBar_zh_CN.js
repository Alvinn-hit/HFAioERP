...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblWatchSpringBar&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblWatchSpringBar&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblWatchSpringBar&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblWatchSpringBar&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchSpringBarDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchSpringBarDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblWatchSpringBarDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblWatchSpringBarDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchSpringBarDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchSpringBarDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblWatchSpringBarDet"; 
 tblName[1]="tblWatchSpringBarDet1"; 
...
function execStat2(){
 execStatOne('tblWatchSpringBarDet_Long',2);
 execStatOne('tblWatchSpringBarDet_Diame',2);
 execStatOne('tblWatchSpringBarDet_Price',2);
 execStatOne('tblWatchSpringBarDet_Qty',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblWatchSpringBar\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblWatchSpringBar\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblWatchSpringBarDet_Long',dis:'生耳长度',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchSpringBarDet_Diame',dis:'生耳直径',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchSpringBarDet_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchSpringBarDet_Qty',dis:'用量',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblWatchSpringBar",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchSpringBar&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchSpringBar&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblWatchSpringBar"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblWatchSpringBar&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchSpringBar&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblWatchSpringBarDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='生耳物料明细';
childCount++;
 var tblWatchSpringBarDetData = new gridData();
 gridDatas[gridDataNum] = tblWatchSpringBarDetData;
 gridDataNum++;
 var tblWatchSpringBarDet=1;
 ...
 addCols(tblWatchSpringBarDetData,'tblWatchSpringBarDet_GoodsCode','物料名称','100',2,true,'',1000,-2,'','',"","","1",'',"3",false); 
 putChildValidateItem("tblWatchSpringBarDet_GoodsCode","物料名称","any","",true,minValue,1000,false,''); 
 ...
 param4=param4+":1id@id"; 
 
 addCols(tblWatchSpringBarDetData,'tblWatchSpringBarDet_GoodsNumber','物料编号','200',2,false,'',500,2,'tblWatchSpringBarDet:GoodsNumber:tblWatchSpringBarDet_GoodsNumber:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblWatchSpringBarDet_GoodsNumber","物料编号","any","",false,minValue,500,false,'',true);
 
 
 
 addCols(tblWatchSpringBarDetData,'tblWatchSpringBarDet_Long','生耳长度','90',1,true,'0',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchSpringBarDet_Long","生耳长度","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblWatchSpringBarDetData,'tblWatchSpringBarDet_Diame','生耳直径','90',1,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchSpringBarDet_Diame","生耳直径","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblWatchSpringBarDetData,'tblWatchSpringBarDet_Electro','电镀','120',2,true,'',200,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblWatchSpringBarDet_Electro","电镀","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchSpringBarDetData,'tblWatchSpringBarDet_Grind','打磨','120',2,true,'',200,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblWatchSpringBarDet_Grind","打磨","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchSpringBarDetData,'tblWatchSpringBarDet_Price','单价','80',1,true,'0',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblWatchSpringBarDet_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblWatchSpringBarDetData,'tblWatchSpringBarDet_Qty','用量','80',1,true,'1',0,0,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblWatchSpringBarDet_Qty","用量","double","",true,minValue,maxValue,true,'1',true); 
 
 
 
 addCols(tblWatchSpringBarDetData,'tblWatchSpringBarDet_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchSpringBarDet_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 
 addCols(tblWatchSpringBarDetData,'tblWatchSpringBarDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblWatchSpringBarDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='生耳变量明细';
childCount++;
 var tblWatchSpringBarDet1Data = new gridData();
 gridDatas[gridDataNum] = tblWatchSpringBarDet1Data;
 gridDataNum++;
 var tblWatchSpringBarDet1=1;
 ...
 addCols(tblWatchSpringBarDet1Data,'tblWatchSpringBarDet1_ChangeContent','变更内容','200',2,false,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblWatchSpringBarDet1_ChangeContent","变更内容","any","",false,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchSpringBarDet1Data,'tblWatchSpringBarDet1_ChangeDate','变更日期','90',5,false,'2013-12-24',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchSpringBarDet1_ChangeDate","变更日期","date","",false,minValue,10,true,'2013-12-24',true); 
 
 
 
 addCols(tblWatchSpringBarDet1Data,'tblWatchSpringBarDet1_ChangePer','变更人员','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchSpringBarDet1_ChangePer","变更人员","any","",true,minValue,100,false,'',true); 
 
 
 
 
 addCols(tblWatchSpringBarDet1Data,'tblWatchSpringBarDet1_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchSpringBarDet1_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchSpringBarDet1Data,'tblWatchSpringBarDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblWatchSpringBar&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=68fc380d_1408131539576500232&noback="+noback+"&parentCode=&operation=6&parentTableName=tblSampleOrder&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblWatchSpringBar,tblWatchSpringBarDet,tblWatchSpringBarDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchSpringBar&allTableName=tblWatchSpringBar,tblWatchSpringBarDet,tblWatchSpringBarDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchSpringBar&allTableName=tblWatchSpringBarDet,tblWatchSpringBarDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchSpringBar&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchSpringBar&allTableName=tblWatchSpringBarDet,tblWatchSpringBarDet1,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblWatchSpringBarDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblWatchSpringBar&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblWatchSpringBar&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblWatchSpringBar&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchSpringBar&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblWatchSpringBar&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '生耳';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblWatchSpringBar&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
