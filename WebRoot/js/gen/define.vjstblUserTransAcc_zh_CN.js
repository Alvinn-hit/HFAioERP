...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblUserTransAcc&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblUserTransAcc&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblUserTransAcc&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblUserTransAcc&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblUserTransAccDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblUserTransAccDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblUserTransAccDet"){
...
function execStat2(){
 execStatOne('tblUserTransAccDet_ReceiveAmt',2);
 execStatOne('tblUserTransAccDet_ReceiveFCAmt',2);
 setTotalCount(getTotalCount());
}
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblUserTransAcc\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblUserTransAccDet_ReceiveAmt',"tblUserTransAccDet"); 
 delFireEventOne(rowId,tableName,'tblUserTransAccDet_ReceiveFCAmt',"tblUserTransAccDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'PayAmt',dis:'转出金额',digit:2,isMain:true};
 ...
 fieldDigit[fieldDigit.length] = {name:'tblUserTransAccDet_ReceiveAmt',dis:'转入金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblUserTransAccDet_ReceiveFCAmt',dis:'外币转入金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblUserTransAcc",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblUserTransAcc&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblUserTransAcc&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "PayBankID"){
 var saveField=n("PayBankID"); 
 saveField[0].value = "";
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblUserTransAcc"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblUserTransAcc&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblUserTransAcc&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('PayAmt')[0].value=f(sm('tblUserTransAccDet_ReceiveAmt'),2);

n('FcBillAmt')[0].value=f(nn('PayAmt',0)/nn('CurrencyRate',0),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 for(var j=0;j<r(0,'ReceiveAmt').length;j++){
r(0,"ReceiveAmt")[j].value=f(rn(0,"ReceiveFCAmt",j)*nn('CurrencyRate',0),2);
}

n('PayAmt')[0].value=f(sm('tblUserTransAccDet_ReceiveAmt'),2);

n('FcBillAmt')[0].value=f(sm('tblUserTransAccDet_ReceiveFCAmt'),2);
...
function initCalculate(){
 var z=r(0,'ReceiveAmt');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblUserTransAccDet_ReceiveAmt'); 
 n('PayAmt')[0].value=f(sm('tblUserTransAccDet_ReceiveAmt'),2);
r(0,"ReceiveFCAmt")[p].value=f(rn(0,"ReceiveAmt",p)/nn('CurrencyRate',0),2);
n('FcBillAmt')[0].value=f(nn('PayAmt',0)/nn('CurrencyRate',0),2);

 execStat(); 
 }
 }
 var z=r(0,'ReceiveFCAmt');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblUserTransAccDet_ReceiveFCAmt'); 
 r(0,"ReceiveAmt")[p].value=f(rn(0,"ReceiveFCAmt",p)*nn('CurrencyRate',0),2);
n('PayAmt')[0].value=f(sm('tblUserTransAccDet_ReceiveAmt'),2);
n('FcBillAmt')[0].value=f(sm('tblUserTransAccDet_ReceiveFCAmt'),2);
...
childData[childCount]='tblUserTransAccDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='现金银行转账明细';
childCount++;
 var tblUserTransAccDetData = new gridData();
 gridDatas[gridDataNum] = tblUserTransAccDetData;
 gridDataNum++;
 var tblUserTransAccDet=1;
 ...
 addCols(tblUserTransAccDetData,'tblUserTransAccDet_ReceiveBankNo','转入账户','150',2,true,'',100,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("tblUserTransAccDet_ReceiveBankNo","转入账户","any","RowMarker",true,minValue,100,false,'');
 
 
 addCols(tblUserTransAccDetData,'tblUserTransAccDet_tblAccTypeInfo_AccFullName','转入账户','150',20,true,'',1000000,2,'tblUserTransAccDet:ReceiveBankNo:tblUserTransAccDet_tblAccTypeInfo_AccFullName:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(tblUserTransAccDetData,'tblUserTransAccDet_ReceiveBankID','转入账户名称','160',2,true,'',100,-2,'','',"","","2",'',"3",false); 
 putChildValidateItem("tblUserTransAccDet_ReceiveBankID","转入账户名称","any","",true,minValue,100,false,''); 
 
 
 
 addCols(tblUserTransAccDetData,'tblUserTransAccDet_ReceiveAmt','转入金额','100',1,true,'0',0,0,'','',"Amount","","3",'',"0",false); 
 putChildValidateItem("tblUserTransAccDet_ReceiveAmt","转入金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblUserTransAccDetData,'tblUserTransAccDet_ReceiveFCAmt','外币转入金额','80',1,true,'',0,0,'','',"Amount","","4",'',"0",false); 
 putChildValidateItem("tblUserTransAccDet_ReceiveFCAmt","外币转入金额","double","currency",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblUserTransAccDetData,'tblUserTransAccDet_Remark','备注','200',2,true,'',8000,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblUserTransAccDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 
 
 addCols(tblUserTransAccDetData,'tblUserTransAccDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblUserTransAcc&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblUserTransAcc,tblUserTransAccDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblUserTransAcc&allTableName=tblUserTransAcc,tblUserTransAccDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblUserTransAcc&allTableName=tblUserTransAccDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblUserTransAcc&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblUserTransAcc&allTableName=tblUserTransAccDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblUserTransAccDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblUserTransAcc&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblUserTransAcc&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblUserTransAcc&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblUserTransAcc&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblUserTransAcc&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '现金银行转账';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblUserTransAcc&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
