...
{ 
 cFireEvent('DXOrderDet_Qty',''); 
 cFireEvent('DXOrderDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=DXOrder&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=DXOrder&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=DXOrder&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=DXOrder&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "DXOrderDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="DXOrderDet"){
 
 copyFields=n('DXOrderDet_StockCode');
 ...
 copyFields=n('DXOrderDet_GoodsCode');
 if((order > 1 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var DXOrderDet_tblGoods_GoodsNumber = n('DXOrderDet_tblGoods_GoodsNumber');
 if(typeof(DXOrderDet_tblGoods_GoodsNumber) != 'undefined' && DXOrderDet_tblGoods_GoodsNumber.length >index){
 DXOrderDet_tblGoods_GoodsNumber[index].value = DXOrderDet_tblGoods_GoodsNumber[index-1].value;
 }
 var DXOrderDet_tblGoods_GoodsFullName = n('DXOrderDet_tblGoods_GoodsFullName');
 if(typeof(DXOrderDet_tblGoods_GoodsFullName) != 'undefined' && DXOrderDet_tblGoods_GoodsFullName.length >index){
 DXOrderDet_tblGoods_GoodsFullName[index].value = DXOrderDet_tblGoods_GoodsFullName[index-1].value;
 }
 var DXOrderDet_tblGoods_GoodsSpec = n('DXOrderDet_tblGoods_GoodsSpec');
 if(typeof(DXOrderDet_tblGoods_GoodsSpec) != 'undefined' && DXOrderDet_tblGoods_GoodsSpec.length >index){
 DXOrderDet_tblGoods_GoodsSpec[index].value = DXOrderDet_tblGoods_GoodsSpec[index-1].value;
 }
 var DXOrderDet_tblGoods_BaseUnit = n('DXOrderDet_tblGoods_BaseUnit');
 if(typeof(DXOrderDet_tblGoods_BaseUnit) != 'undefined' && DXOrderDet_tblGoods_BaseUnit.length >index){
 DXOrderDet_tblGoods_BaseUnit[index].value = DXOrderDet_tblGoods_BaseUnit[index-1].value;
 }
 var DXOrderDet_tblStocks_lastQty = n('DXOrderDet_tblStocks_lastQty');
 if(typeof(DXOrderDet_tblStocks_lastQty) != 'undefined' && DXOrderDet_tblStocks_lastQty.length >index){
 DXOrderDet_tblStocks_lastQty[index].value = DXOrderDet_tblStocks_lastQty[index-1].value;
 }
 var DXOrderDet_ViewVirtualStock_UseQty = n('DXOrderDet_ViewVirtualStock_UseQty');
 if(typeof(DXOrderDet_ViewVirtualStock_UseQty) != 'undefined' && DXOrderDet_ViewVirtualStock_UseQty.length >index){
 DXOrderDet_ViewVirtualStock_UseQty[index].value = DXOrderDet_ViewVirtualStock_UseQty[index-1].value;
 }
 }
 copyFields=n('DXOrderDet_SendDate');
 if((order > 19 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="DXOrderDet"){
 ...
 copyFields=n('DXOrderDet_SendDate'); 
 if( (order > 19 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
 }
 }
...
function execStat2(){
 execStatOne('DXOrderDet_Qty',2);
 execStatOne('DXOrderDet_Amount',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"DXOrder\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"DXOrder\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'DXOrderDet_Qty',"DXOrderDet"); 
 delFireEventOne(rowId,tableName,'DXOrderDet_Price',"DXOrderDet"); 
 delFireEventOne(rowId,tableName,'DXOrderDet_Amount',"DXOrderDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'AccountAmount',dis:'预收订金',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'DXOrderDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'DXOrderDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'DXOrderDet_Amount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=DXOrder",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=DXOrder&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=DXOrder&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=DXOrder&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=DXOrder&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOrderDet_Qty'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOrderDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOrderDet_Amount'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOrderDet_CurAmount'); 
 r(0,"CurPrice")[p].value=f(rn(0,"CurAmount",p)/rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'DXOrderDet_SecQty'); 
...
childData[childCount]='DXOrderDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='代销配货申请单明细';
childCount++;
 var DXOrderDetData = new gridData();
 gridDatas[gridDataNum] = DXOrderDetData;
 gridDataNum++;
 var DXOrderDet=1;
 ...
 addCols(DXOrderDetData,'DXOrderDet_StockCode','仓库','80',2,true,'',100,-2,'','',"","copy","0",'',"3",false); 
 putChildValidateItem("DXOrderDet_StockCode","仓库","any","",true,minValue,100,true,''); 
 ...
 param4=param4+":0仓库@DXOrderDet_StockCode";
 param4=param4+":0批号@DXOrderDet_BatchNo";
 param4=param4+":0厚度@DXOrderDet_Inch";
 param4=param4+":0颜色@DXOrderDet_Hue";
 param4=param4+":0尺码@DXOrderDet_yearNO";
 param4=param4+":0生产日期@DXOrderDet_ProDate";
 param4=param4+":0保质期限@DXOrderDet_Availably";
 param4=param4+":1客户@CompanyCode"; 
 
 addCols(DXOrderDetData,'DXOrderDet_GoodsCode','商品名称','120',2,false,'',200,-2,'','',"","triggerOrCopy","1",'',"2",true);
 putChildValidateItem("DXOrderDet_GoodsCode","商品名称","any","",false,minValue,200,false,'');
 
 
 addCols(DXOrderDetData,'DXOrderDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'DXOrderDet:GoodsCode:DXOrderDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(DXOrderDetData,'DXOrderDet_tblGoods_GoodsFullName','商品名称','150',18,true,'',1000000,2,'DXOrderDet:GoodsCode:DXOrderDet_tblGoods_GoodsFullName:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(DXOrderDetData,'DXOrderDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'DXOrderDet:GoodsCode:DXOrderDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(DXOrderDetData,'DXOrderDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'DXOrderDet:GoodsCode:DXOrderDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(DXOrderDetData,'DXOrderDet_tblStocks_lastQty','实际库存','90',1,true,'',1000000,2,'DXOrderDet:GoodsCode:DXOrderDet_tblStocks_lastQty:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(DXOrderDetData,'DXOrderDet_ViewVirtualStock_UseQty','虚拟库存','90',1,true,'',1000000,2,'DXOrderDet:GoodsCode:DXOrderDet_ViewVirtualStock_UseQty:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","2",'',"0",false); 
 putChildValidateItem("DXOrderDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","3",'',"0",false); 
 putChildValidateItem("DXOrderDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","4",'',"2",false); 
 putChildValidateItem("DXOrderDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","5",'',"2",false); 
 putChildValidateItem("DXOrderDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","6",'',"2",false); 
 putChildValidateItem("DXOrderDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","7",'',"2",false); 
 putChildValidateItem("DXOrderDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_Qty','数量','60',1,false,'',0,0,'','',"Qty","","8",'',"0",false); 
 putChildValidateItem("DXOrderDet_Qty","数量","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_Price','单价','80',1,true,'',0,0,'','',"SPriceIdentifier","","9",'',"0",false); 
 putChildValidateItem("DXOrderDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_Amount','金额','100',1,true,'',0,0,'','',"SAmountIdentifier","","10",'',"0",false); 
 putChildValidateItem("DXOrderDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(DXOrderDetData,'DXOrderDet_CurAmount','外币金额','100',1,true,'',0,-2,'','',"Price","","16",'',"0",false); 
 putChildValidateItem("DXOrderDet_CurAmount","外币金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_CurPrice','外币单价','200',1,true,'',0,-2,'','',"","","17",'',"0",false); 
 putChildValidateItem("DXOrderDet_CurPrice","外币单价","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_ViewTotalQty','当前库存','80',1,true,'',0,-2,'','',"Qty","","18",'',"3",false); 
 putChildValidateItem("DXOrderDet_ViewTotalQty","当前库存","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_SendDate','发货日期','80',5,true,'',10,-2,'','',"","copy","19",'',"0",false); 
 putChildValidateItem("DXOrderDet_SendDate","发货日期","date","",true,minValue,10,false,''); 
 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","21",'',"3",false); 
 putChildValidateItem("DXOrderDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","22",'',"3",false); 
 putChildValidateItem("DXOrderDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_EmployeeID','经手人','0',2,true,'',100,-2,'','',"Qty","","23",'',"3",false); 
 putChildValidateItem("DXOrderDet_EmployeeID","经手人","any","",true,minValue,100,true,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_DepartmentCode','部门','0',2,true,'',100,-2,'','',"","","24",'',"100",false); 
 putChildValidateItem("DXOrderDet_DepartmentCode","部门","any","",true,minValue,100,true,''); 
 ...
 addCols(DXOrderDetData,'DXOrderDet_Remark','备注','100',2,true,'',8000,-2,'','',"","","29",'',"0",false); 
 putChildValidateItem("DXOrderDet_Remark","备注","any","BillPresentSampleType",true,minValue,8000,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_OutQty','配送数量','200',1,true,'',0,-2,'','',"","","30",'',"0",false); 
 putChildValidateItem("DXOrderDet_OutQty","配送数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_NotOutQty','未配送数量','200',1,true,'',0,-2,'','',"","","31",'',"0",false); 
 putChildValidateItem("DXOrderDet_NotOutQty","未配送数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_FinishStatus','完成状态','200',0,true,'',0,-2,'','',"","","32",'',"0",false); 
 putChildValidateItem("DXOrderDet_FinishStatus","完成状态","int","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(DXOrderDetData,'DXOrderDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=DXOrder&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=null&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "DXOrder,DXOrderDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=DXOrder&allTableName=DXOrder,DXOrderDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=DXOrder&allTableName=DXOrderDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=DXOrder&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=DXOrder&allTableName=DXOrderDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=DXOrder&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=DXOrder&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=DXOrder&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=DXOrder&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=DXOrder&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '代销配货申请单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=DXOrder&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
