...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblWatchCase&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblWatchCase&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblWatchCase&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblWatchCase&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchCaseDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchCaseDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblWatchCaseDet"){
 
  }
 if(typeof(tableName)!="undefined" && tableName=="tblWatchCaseDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchCaseDet"){
 
  }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchCaseDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblWatchCaseDet"; 
 tblName[1]="tblWatchCaseDet1"; 
...
function execStat2(){
 execStatOne('tblWatchCaseDet_CaseQty',2);
 execStatOne('tblWatchCaseDet_CircleQty',2);
 execStatOne('tblWatchCaseDet_Qty',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblWatchCase\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblWatchCase\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblWatchCaseDet_CaseQty',dis:'壳数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchCaseDet_CircleQty',dis:'圈数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchCaseDet_Qty',dis:'用量',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblWatchCase",
...
function exeQuoteDraft(varStr,moduleType){
 ...
 window.location.href="/UserFunctionAction.do?tableName="+varTableName+"&operation=7&moduleType="+moduleType+"&parentTableName=tblSampleOrder&f_brother=5d4be509_1412111501422990071&keyId="+varStr+"&saveDraft=quoteDraft&winCurIndex="+winCurIndex+"&popWinName="+$("#popWinName").val() ; 
}
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchCase&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchCase&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblWatchCase"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblWatchCase&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchCase&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblWatchCaseDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表壳物料明细';
childCount++;
 var tblWatchCaseDetData = new gridData();
 gridDatas[gridDataNum] = tblWatchCaseDetData;
 gridDataNum++;
 var tblWatchCaseDet=1;
   var param4="";
 param4=param4+":1样板编号@SampleOrderID"; 
 addCols(tblWatchCaseDetData,'tblWatchCaseDet_GoodsNumber','物料编号','150',2,true,'',500,2,'tblWatchCaseDet:GoodsNumber:tblWatchCaseDet_GoodsNumber:'+param4,"","","","2",'',"2",true);
 putChildValidateItem("tblWatchCaseDet_GoodsNumber","物料编号","any","",true,minValue,500,false,'',true);
   addCols(tblWatchCaseDetData,'tblWatchCaseDet_CaseGrind','壳打磨','280',2,true,'',200,8,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet_CaseGrind","壳打磨","any","",true,minValue,200,false,'',true); 
   addCols(tblWatchCaseDetData,'tblWatchCaseDet_CaseElectro','壳电镀','280',2,true,'',200,8,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet_CaseElectro","壳电镀","any","",true,minValue,200,false,'',true); 
   addCols(tblWatchCaseDetData,'tblWatchCaseDet_CaseQty','壳数量','50',1,true,'',0,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet_CaseQty","壳数量","double","",true,minValue,maxValue,false,'',true); 
   addCols(tblWatchCaseDetData,'tblWatchCaseDet_CircleGrind','圈打磨','280',2,true,'',200,8,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet_CircleGrind","圈打磨","any","",true,minValue,200,false,'',true); 
   addCols(tblWatchCaseDetData,'tblWatchCaseDet_CircleElectro','圈电镀','280',2,true,'',200,8,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet_CircleElectro","圈电镀","any","",true,minValue,200,false,'',true); 
   addCols(tblWatchCaseDetData,'tblWatchCaseDet_CircleQty','圈数量','50',1,true,'',0,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet_CircleQty","圈数量","double","",true,minValue,maxValue,false,'',true); 
   addCols(tblWatchCaseDetData,'tblWatchCaseDet_GrindRemark','打磨说明','200',2,true,'',500,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet_GrindRemark","打磨说明","any","",true,minValue,500,false,'',true); 
   addCols(tblWatchCaseDetData,'tblWatchCaseDet_ElectroRemark','电镀说明','200',2,true,'',500,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet_ElectroRemark","电镀说明","any","",true,minValue,500,false,'',true); 
   addCols(tblWatchCaseDetData,'tblWatchCaseDet_Qty','用量','80',1,true,'1',0,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet_Qty","用量","double","",true,minValue,maxValue,true,'1',true); 
   addCols(tblWatchCaseDetData,'tblWatchCaseDet_Remark','备注','200',2,true,'',500,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet_Remark","备注","any","",true,minValue,500,false,'',true); 
    addCols(tblWatchCaseDetData,'tblWatchCaseDet_GoodsCode','物料名称','100',2,true,'',1000,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblWatchCaseDet_GoodsCode","物料名称","any","",true,minValue,1000,false,''); 
                          
 addCols(tblWatchCaseDetData,'tblWatchCaseDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblWatchCaseDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表壳变更明细';
childCount++;
 var tblWatchCaseDet1Data = new gridData();
 gridDatas[gridDataNum] = tblWatchCaseDet1Data;
 gridDataNum++;
 var tblWatchCaseDet1=1;
  addCols(tblWatchCaseDet1Data,'tblWatchCaseDet1_ChangeContent','变更内容','200',2,false,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet1_ChangeContent","变更内容","any","",false,minValue,500,false,'',true); 
  addCols(tblWatchCaseDet1Data,'tblWatchCaseDet1_ChangeDate','变更日期','90',5,false,'2013-12-24',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet1_ChangeDate","变更日期","date","",false,minValue,10,true,'2013-12-24',true); 
  addCols(tblWatchCaseDet1Data,'tblWatchCaseDet1_ChangePer','变更人员','100',2,false,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet1_ChangePer","变更人员","any","",false,minValue,100,false,'',true); 
  addCols(tblWatchCaseDet1Data,'tblWatchCaseDet1_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchCaseDet1_Remark","备注","any","",true,minValue,500,false,'',true); 
       
 addCols(tblWatchCaseDet1Data,'tblWatchCaseDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblWatchCase&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=5d4be509_1412111501422990071&noback="+noback+"&parentCode=&operation=6&parentTableName=tblSampleOrder&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblWatchCase,tblWatchCaseDet,tblWatchCaseDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchCase&allTableName=tblWatchCase,tblWatchCaseDet,tblWatchCaseDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchCase&allTableName=tblWatchCaseDet,tblWatchCaseDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchCase&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchCase&allTableName=tblWatchCaseDet,tblWatchCaseDet1,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblWatchCase&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=b103e944_1306061720235781327&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblWatchCase&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblWatchCase&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchCase&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblWatchCase&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '样板订单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblWatchCase&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
