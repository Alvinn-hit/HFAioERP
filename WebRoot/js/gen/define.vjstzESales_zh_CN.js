...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tzESales&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tzESales&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tzESales&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tzESales&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tzESales\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tzESales\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'couponCut',dis:'实际抵扣',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'leaveMoney',dis:'当前余额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'clientPayment',dis:'订金余额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'retMoney',dis:'订金扣款',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'totalMoney',dis:'总发生额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'normalQty',dis:'正常单量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'youhuiQty',dis:'优惠单量',digit:0,isMain:true};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tzESales",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tzESales&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tzESales&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 var saveField=n("oldMoney"); 
 saveField[0].value = "";
 var obj = m("tbl_tzESales_money");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("oldproduct"); 
 saveField[0].value = "";
 var obj = m("tbl_tzEProduct_name");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("oldEndDate"); 
 saveField[0].value = "";
 var obj = m("tbl_tzESales_endDate");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("leveDay"); 
 saveField[0].value = "";
 var obj = m("tbl_(case when tzESales_endDate is null then '' else (case when DATEDIFF(day, getdate(),tzESales.endDate) < 0 then 0 else DATEDIFF(day, getdate(),tzESales.endDate) end) end)");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("clientId"); 
 saveField[0].value = "";
 var obj = m("tbl_tzclient_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("oldId"); 
 saveField[0].value = "";
 var obj = m("tbl_tzESales_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("eName"); 
 saveField[0].value = "";
 var obj = m("tbl_tzESales_eName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tzclient_clientName");
 saveField[0].value = ""; 
 m("tbl_tzclient_clientName").value = "" ;
 var saveField=n("tblDistrict_DistrictFullName");
 saveField[0].value = ""; 
 m("tbl_tblDistrict_DistrictFullName").value = "" ;
 var saveField=n("tzclient_materialCount");
 saveField[0].value = ""; 
 m("tbl_tzclient_materialCount").value = "" ;
 var saveField=n("tzclient_address");
 saveField[0].value = ""; 
 m("tbl_tzclient_address").value = "" ;
 var saveField=n("tzclient_connect");
 saveField[0].value = ""; 
 m("tbl_tzclient_connect").value = "" ;
 var saveField=n("tzclient_tel");
 saveField[0].value = ""; 
 m("tbl_tzclient_tel").value = "" ;
 }
   }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tzESales&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tzESales&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tzESales&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tzESales&allTableName=tzESales,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tzESales&allTableName=&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tzESales&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tzESales&allTableName=&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tzESales&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tzESales&designId=1c3ea4e7_1602190920548456615&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tzESales&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tzESales&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tzESales&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '易站通下单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tzESales&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
