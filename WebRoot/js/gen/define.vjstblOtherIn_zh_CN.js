...
{ 
 cFireEvent('tblOtherInDet_Qty',''); 
 cFireEvent('tblOtherInDet_CurPrice',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblOtherIn&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblOtherIn&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblOtherIn&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblOtherIn&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function addSeq(o,keyCode){
 ...
 var goodscodestr="";
 var isInstore = false ;
 if(seqFname=="tblOtherInDet_Seq"){
 var ss="@GN:Seq:Qty:tblOtherInDet_GoodsSeqSet:tblOtherInDet_GoodsCode:StockCode".split(":")
 if(ss[0]=="@GN"){
 goodscodestr=ss[4];
...
function returnSeq(colNames,returnVales,compareVale,goodscodestr,ri,seqFname,inpVal){
 ...
 if("tblOtherInDet"==seqFname.substr(0,seqFname.lastIndexOf("_"))){ 
   }
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblOtherInDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblOtherInDet"){
 
 copyFields=n('tblOtherInDet_GoodsCode');
 ...
 var tblOtherInDet_tblGoods_GoodsNumber = n('tblOtherInDet_tblGoods_GoodsNumber');
 if(typeof(tblOtherInDet_tblGoods_GoodsNumber) != 'undefined' && tblOtherInDet_tblGoods_GoodsNumber.length >index){
 tblOtherInDet_tblGoods_GoodsNumber[index].value = tblOtherInDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblOtherInDet_tblGoods_GoodsFullName = n('tblOtherInDet_tblGoods_GoodsFullName');
 if(typeof(tblOtherInDet_tblGoods_GoodsFullName) != 'undefined' && tblOtherInDet_tblGoods_GoodsFullName.length >index){
 tblOtherInDet_tblGoods_GoodsFullName[index].value = tblOtherInDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblOtherInDet_tblGoods_GoodsSpec = n('tblOtherInDet_tblGoods_GoodsSpec');
 if(typeof(tblOtherInDet_tblGoods_GoodsSpec) != 'undefined' && tblOtherInDet_tblGoods_GoodsSpec.length >index){
 tblOtherInDet_tblGoods_GoodsSpec[index].value = tblOtherInDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblOtherInDet_tblGoods_BaseUnit = n('tblOtherInDet_tblGoods_BaseUnit');
 if(typeof(tblOtherInDet_tblGoods_BaseUnit) != 'undefined' && tblOtherInDet_tblGoods_BaseUnit.length >index){
 tblOtherInDet_tblGoods_BaseUnit[index].value = tblOtherInDet_tblGoods_BaseUnit[index-1].value;
 }
 var tblOtherInDet_tblGoods_BarCode = n('tblOtherInDet_tblGoods_BarCode');
 if(typeof(tblOtherInDet_tblGoods_BarCode) != 'undefined' && tblOtherInDet_tblGoods_BarCode.length >index){
 tblOtherInDet_tblGoods_BarCode[index].value = tblOtherInDet_tblGoods_BarCode[index-1].value;
 }
 }
 copyFields=n('tblOtherInDet_StockCode');
 if((order > 1 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblOtherInDet_tblStock_StockNumber1 = n('tblOtherInDet_tblStock_StockNumber1');
 if(typeof(tblOtherInDet_tblStock_StockNumber1) != 'undefined' && tblOtherInDet_tblStock_StockNumber1.length >index){
 tblOtherInDet_tblStock_StockNumber1[index].value = tblOtherInDet_tblStock_StockNumber1[index-1].value;
 }
 var tblOtherInDet_tblStock_StockName1 = n('tblOtherInDet_tblStock_StockName1');
 if(typeof(tblOtherInDet_tblStock_StockName1) != 'undefined' && tblOtherInDet_tblStock_StockName1.length >index){
 tblOtherInDet_tblStock_StockName1[index].value = tblOtherInDet_tblStock_StockName1[index-1].value;
 }
 }
   }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblOtherInDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblOtherInDet"; 
 var goodsCodeLogicValidate_tblOtherInDet='@GN:Seq:Qty:tblOtherInDet_GoodsSeqSet:tblOtherInDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'tblOtherInDet',value:goodsCodeLogicValidate_tblOtherInDet};
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr.substring(0,index+"&seq=".length)+n(thisField.substr(0,thisField.lastIndexOf("_")))[ri].value+"&src=menu&seqFname="+thisField+"&newSeq="+mi[ri].value+"&tableName=tblOtherIn&childTableName="+thisField.split("_")[0]+"&fieldName="+field;
 var goodscode="";
 var goodscodestr="";
 var goodsProps = "";
 var stockCode = "" ;
 ...
 if(thisField=="tblOtherInDet_Seq_hid"){
 ...
 stockCode = nz("tblOtherInDet"+"_"+ss[5],ri) ;
 }
 }
 var ss="@IOO:Instore".split(":")
 if(ss[0]=="@GN"){
 ...
 AjaxRequest(urlsgood) ;
 if("false"==response){
 asyncbox.tips("该商品未启用序列号8！！","alert",2000) ;
 return ;
 }
 }
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblOtherIn\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblOtherIn\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblOtherInDet_Qty',"tblOtherInDet"); 
 delFireEventOne(rowId,tableName,'tblOtherInDet_CurPrice',"tblOtherInDet"); 
 delFireEventOne(rowId,tableName,'tblOtherInDet_CurrencyRate',"tblOtherInDet"); 
 delFireEventOne(rowId,tableName,'tblOtherInDet_CurAmount',"tblOtherInDet"); 
 delFireEventOne(rowId,tableName,'tblOtherInDet_Price',"tblOtherInDet"); 
 delFireEventOne(rowId,tableName,'tblOtherInDet_Amount',"tblOtherInDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'BillAmt',dis:'本币合计总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CurTotalAmount',dis:'原币总额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblOtherInDet_Qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOtherInDet_CurPrice',dis:'原币单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOtherInDet_CurrencyRate',dis:'汇率',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOtherInDet_CurAmount',dis:'原币金额',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOtherInDet_Price',dis:'本币单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblOtherInDet_Amount',dis:'本币金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblOtherIn",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblOtherIn&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblOtherIn&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblOtherIn"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblOtherIn&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblOtherIn&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
n('TotalTaxAmount')[0].value=f(sm('tblOtherInDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblOtherInDet_CoTaxAmt'),2);
...
n('DisAmount')[0].value=f(sm('tblOtherInDet_DisAmount'),2);

n('BillAmt')[0].value=f(sm('tblOtherInDet_DisBackAmt'),2);
...
r(0,"Price")[j].value=f(rn(0,"CurPrice",j)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",j)/100),2);
}
...
n('BillAmt')[0].value=f(sm('tblOtherInDet_DisBackAmt'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 if(nn('InVoiceType',0)==2)n('Tax')[0].value=f("0",2);

if(nn('InVoiceType',0)==1)n('Tax')[0].value=f("0",2);

if(nn('InVoiceType',0)==3)n('Tax')[0].value=f(0,2);
...
function initCalculate(){
 var z=r(0,'InVoiceType');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_InVoiceType'); 
 if(rn(0,"InVoiceType",p)==2)r(0,"Tax")[p].value=f(17,2);
if(rn(0,"InVoiceType",p)==1)r(0,"Tax")[p].value=f(3,2);
if(rn(0,"InVoiceType",p)==3)r(0,"Tax")[p].value=f(0,2);
r(0,"TaxPrice")[p].value=f(((rn(0,"Tax",p)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"TaxAmount")[p].value=f(((rn(0,"Tax",p)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"CurPrice")[p].value=f((((rn(0,"Tax",p)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100)/rn(0,"CurrencyRate",p),6);
r(0,"CurAmount")[p].value=f((((rn(0,"Tax",p)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100)/rn(0,"CurrencyRate",p),2);
n('BillAmt')[0].value=f(sm('tblOtherInDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblOtherInDet_TaxAmount'),2);
n('CurTotalAmount')[0].value=f(sm('tblOtherInDet_CurAmount'),2);
...
n('DisAmount')[0].value=f(sm('tblOtherInDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_Qty'); 
...
n('TotalCoTaxAmt')[0].value=f(sm('tblOtherInDet_CoTaxAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_CurPrice'); 
 r(0,"TaxPrice")[p].value=f(rn(0,"CurPrice",p)*rn(0,"CurrencyRate",p),6);
r(0,"DisPrice")[p].value=f(rn(0,"CurPrice",p)*rn(0,"CurrencyRate",p)/((rn(0,"Tax",p)/100)+1),6);
r(0,"Price")[p].value=f(rn(0,"CurPrice",p)*rn(0,"CurrencyRate",p)/((rn(0,"Tax",p)/100)+1)/(rn(0,"Discount",p)/100),2);
r(0,"CurAmount")[p].value=f(rn(0,"CurPrice",p)*rn(0,"Qty",p),2);
r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*rn(0,"CurrencyRate",p),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"CurAmount",p)*rn(0,"CurrencyRate",p)/((rn(0,"Tax",p)/100)+1),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*rn(0,"CurrencyRate",p)/((rn(0,"Tax",p)/100)+1)/(rn(0,"Discount",p)/100),2);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
n('BillAmt')[0].value=f(sm('tblOtherInDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblOtherInDet_TaxAmount'),2);
n('CurTotalAmount')[0].value=f(sm('tblOtherInDet_CurAmount'),2);
...
n('DisAmount')[0].value=f(sm('tblOtherInDet_DisAmount'),2);
 ...
 var z=r(0,'CurrencyRate');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_CurrencyRate'); 
 r(0,"TaxPrice")[p].value=f(rn(0,"CurPrice",p)*rn(0,"CurrencyRate",p),6);
r(0,"DisPrice")[p].value=f(rn(0,"CurPrice",p)*rn(0,"CurrencyRate",p)/((rn(0,"Tax",p)/100)+1),6);
r(0,"Price")[p].value=f(rn(0,"CurPrice",p)*rn(0,"CurrencyRate",p)/((rn(0,"Tax",p)/100)+1)/(rn(0,"Discount",p)/100),2);
r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*rn(0,"CurrencyRate",p),2);
...
n('DisAmount')[0].value=f(sm('tblOtherInDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_CurAmount'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*rn(0,"CurrencyRate",p),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"CurAmount",p)*rn(0,"CurrencyRate",p)/((rn(0,"Tax",p)/100)+1),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*rn(0,"CurrencyRate",p)/((rn(0,"Tax",p)/100)+1)/(rn(0,"Discount",p)/100),2);
...
n('DisAmount')[0].value=f(sm('tblOtherInDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
n('DisAmount')[0].value=f(sm('tblOtherInDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_Amount'); 
 r(0,"Price")[p].value=f(rn(0,"Amount",p)/rn(0,"Qty",p),2);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"TaxPrice")[p].value=f(((rn(0,"Tax",p)/100)+1)*(rn(0,"Discount",p)*rn(0,"Price",p)/100),6);
r(0,"CurPrice")[p].value=f((((rn(0,"Tax",p)/100)+1)*(rn(0,"Discount",p)*rn(0,"Price",p)/100))/rn(0,"CurrencyRate",p),6);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('tblOtherInDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('BillAmt')[0].value=f(sm('tblOtherInDet_DisBackAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_DisPrice'); 
...
n('BillAmt')[0].value=f(sm('tblOtherInDet_DisBackAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_DisBackAmt'); 
...
n('BillAmt')[0].value=f(sm('tblOtherInDet_DisBackAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_TaxPrice'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"TaxPrice",p),2);
r(0,"DisPrice")[p].value=f(rn(0,"TaxPrice",p)/((rn(0,"Tax",p)/100)+1),6);
r(0,"Price")[p].value=f((rn(0,"TaxPrice",p)/((rn(0,"Tax",p)/100)+1)*100)/rn(0,"Discount",p),2);
r(0,"CurPrice")[p].value=f(rn(0,"TaxPrice",p)/rn(0,"CurrencyRate",p),6);
r(0,"Amount")[p].value=f(rn(0,"TaxAmount",p)/((rn(0,"Tax",p)/100)+1)/(rn(0,"Discount",p)/100),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"TaxAmount",p)/((rn(0,"Tax",p)/100)+1),2);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
n('BillAmt')[0].value=f(sm('tblOtherInDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblOtherInDet_TaxAmount'),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
n('TotalCoTaxAmt')[0].value=f(sm('tblOtherInDet_CoTaxAmt'),2);
r(0,"CurAmount")[p].value=f(rn(0,"TaxAmount",p)/rn(0,"CurrencyRate",p),2);
n('CurTotalAmount')[0].value=f(sm('tblOtherInDet_CurAmount'),2);
n('DisAmount')[0].value=f(sm('tblOtherInDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOtherInDet_TaxAmount'); 
 r(0,"TaxPrice")[p].value=f(rn(0,"TaxAmount",p)/rn(0,"Qty",p),6);
r(0,"DisPrice")[p].value=f(rn(0,"TaxPrice",p)/((rn(0,"Tax",p)/100)+1),6);
r(0,"Price")[p].value=f((rn(0,"TaxPrice",p)/((rn(0,"Tax",p)/100)+1))*100/rn(0,"Discount",p),2);
r(0,"CurPrice")[p].value=f((rn(0,"TaxAmount",p)/rn(0,"Qty",p))/rn(0,"CurrencyRate",p),6);
r(0,"Amount")[p].value=f((((rn(0,"TaxAmount",p)/rn(0,"Qty",p))/((rn(0,"Tax",p)/100)+1))*100/rn(0,"Discount",p))*rn(0,"Qty",p),2);
r(0,"DisBackAmt")[p].value=f(((((rn(0,"TaxAmount",p)/rn(0,"Qty",p))/((rn(0,"Tax",p)/100)+1))*100/rn(0,"Discount",p))*rn(0,"Qty",p))*rn(0,"Discount",p)/100,2);
r(0,"DisAmount")[p].value=f(((((rn(0,"TaxAmount",p)/rn(0,"Qty",p))/((rn(0,"Tax",p)/100)+1))*100/rn(0,"Discount",p))*rn(0,"Qty",p))-(((((rn(0,"TaxAmount",p)/rn(0,"Qty",p))/((rn(0,"Tax",p)/100)+1))*100/rn(0,"Discount",p))*rn(0,"Qty",p))*rn(0,"Discount",p)/100),2);
n('BillAmt')[0].value=f(sm('tblOtherInDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblOtherInDet_TaxAmount'),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-(((((rn(0,"TaxAmount",p)/rn(0,"Qty",p))/((rn(0,"Tax",p)/100)+1))*100/rn(0,"Discount",p))*rn(0,"Qty",p))*rn(0,"Discount",p)/100),2);
n('TotalCoTaxAmt')[0].value=f(sm('tblOtherInDet_CoTaxAmt'),2);
r(0,"CurAmount")[p].value=f(rn(0,"TaxAmount",p)/rn(0,"CurrencyRate",p),2);
n('CurTotalAmount')[0].value=f(sm('tblOtherInDet_CurAmount'),2);
n('DisAmount')[0].value=f(sm('tblOtherInDet_DisAmount'),2);
...
childData[childCount]='tblOtherInDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='零星入库单明细';
childCount++;
 var tblOtherInDetData = new gridData();
 gridDatas[gridDataNum] = tblOtherInDetData;
 gridDataNum++;
 var tblOtherInDet=1;
 ...
 addCols(tblOtherInDetData,'tblOtherInDet_GoodsCode','料件名称','120',2,false,'',100,-2,'','',"","copy","0",'',"2",true);
 putChildValidateItem("tblOtherInDet_GoodsCode","料件名称","any","",false,minValue,100,false,'');
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'tblOtherInDet:GoodsCode:tblOtherInDet_tblGoods_GoodsNumber:'+param4,'',"","copy","0",'',"2",false); 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_tblGoods_GoodsFullName','料件名称','150',2,true,'',1000000,2,'tblOtherInDet:GoodsCode:tblOtherInDet_tblGoods_GoodsFullName:'+param4,'',"","copy","0",'',"2",false); 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'tblOtherInDet:GoodsCode:tblOtherInDet_tblGoods_GoodsSpec:'+param4,'',"","copy","0",'',"2",false); 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblOtherInDet:GoodsCode:tblOtherInDet_tblGoods_BaseUnit:'+param4,'',"","copy","0",'',"2",false); 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_tblGoods_BarCode','条码','100',2,true,'',1000000,2,'tblOtherInDet:GoodsCode:tblOtherInDet_tblGoods_BarCode:'+param4,'',"","copy","0",'',"2",false); 
 ...
 addCols(tblOtherInDetData,'tblOtherInDet_StockCode','仓库','80',2,false,'Sess_StockCode',60,-2,'','',"","copy","1",'',"2",true);
 putChildValidateItem("tblOtherInDet_StockCode","仓库","any","",false,minValue,60,true,'Sess_StockCode');
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_tblStock_StockNumber1','仓库编号','120',0,true,'Sess_StockFullName',1000000,2,'tblOtherInDet:StockCode:tblOtherInDet_tblStock_StockNumber1:'+param4,'',"","copy","1",'',"2",false); 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_tblStock_StockName1','仓库简称','100',0,true,'',1000000,2,'tblOtherInDet:StockCode:tblOtherInDet_tblStock_StockName1:'+param4,'',"","copy","1",'',"2",false); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_BatchNo','批号','120',2,true,'',100,0,'','',"","triggerCopy","2",'',"0",false); 
 putChildValidateItem("tblOtherInDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","3",'',"0",false); 
 putChildValidateItem("tblOtherInDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","4",'',"2",false); 
 putChildValidateItem("tblOtherInDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_yearNO','箱号','100',2,true,'',100,0,'','',"","triggerCopy","5",'',"0",false); 
 putChildValidateItem("tblOtherInDet_yearNO","箱号","any","GoodsField",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_color','色彩','60',2,true,'',100,-2,'','',"","","6",'',"2",false); 
 putChildValidateItem("tblOtherInDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblOtherInDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblOtherInDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 ...
 addCols(tblOtherInDetData,'tblOtherInDet_Seq','物流码','130',2,true,'',5000,-2,'','',"","triggerCopy","10",'',"2",false);
 addCols(tblOtherInDetData,'tblOtherInDet_Seq'+'_hid','物流码','130',2,true,'',5000,2,'@SEQ@IOO:Instore',"","","triggerCopy","10",'',"2",false);
 putChildValidateItem("tblOtherInDet_Seq","物流码","any","GoodsField",true,minValue,5000,false,'',true);
 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_InVoiceType','发票类型','100',0,true,'3',0,-2,'','',"","","12",'',"2",false); 
 putChildValidateItem("tblOtherInDet_InVoiceType","发票类型","int","",true,minValue,maxValue,true,'3'); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_Tax','税率%','80',1,true,'0',0,-2,'','',"","","13",'',"0",false); 
 putChildValidateItem("tblOtherInDet_Tax","税率%","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_Qty','数量','80',1,false,'',0,0,'','',"Qty","","14",'',"0",false); 
 putChildValidateItem("tblOtherInDet_Qty","数量","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_CurPrice','原币单价','100',1,true,'',0,0,'','',"priceIdentifier","","15",'',"0",false); 
 putChildValidateItem("tblOtherInDet_CurPrice","原币单价","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(tblOtherInDetData,'tblOtherInDet_Currency','币种','50',2,false,'',100,-2,'','',"","","16",'',"2",true);
 putChildValidateItem("tblOtherInDet_Currency","币种","any","",false,minValue,100,false,'');
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_tblSetExchange_CurrencyNameChild','币种','100',0,true,'',1000000,2,'tblOtherInDet:Currency:tblOtherInDet_tblSetExchange_CurrencyNameChild:'+param4,'',"","","16",'',"2",false); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_Remark','备注','130',2,true,'',8000,0,'','',"","","17",'',"0",false); 
 putChildValidateItem("tblOtherInDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_CurrencyRate','汇率','80',1,true,'1',0,8,'','',"","","18",'',"0",false); 
 putChildValidateItem("tblOtherInDet_CurrencyRate","汇率","double","",true,minValue,maxValue,true,'1',true); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_CurAmount','原币金额','100',1,true,'',0,8,'','',"","","19",'',"0",false); 
 putChildValidateItem("tblOtherInDet_CurAmount","原币金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_Price','本币单价','80',1,true,'',0,8,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblOtherInDet_Price","本币单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_Amount','本币金额','100',1,true,'',0,8,'','',"AmountIdentifier","","21",'',"0",false); 
 putChildValidateItem("tblOtherInDet_Amount","本币金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(tblOtherInDetData,'tblOtherInDet_Discount','折扣率%','50',1,true,'100',0,-2,'','',"Price","","27",'',"0",false); 
 putChildValidateItem("tblOtherInDet_Discount","折扣率%","double","BillDiscount",true,minValue,maxValue,true,'100'); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_DisPrice','折后单价','80',1,true,'',0,-2,'','',"priceIdentifier","","28",'',"0",false); 
 putChildValidateItem("tblOtherInDet_DisPrice","折后单价","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_DisBackAmt','折后金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","29",'',"6",false); 
 putChildValidateItem("tblOtherInDet_DisBackAmt","折后金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_DisAmount','折让金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","30",'',"8",false); 
 putChildValidateItem("tblOtherInDet_DisAmount","折让金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_TaxPrice','含税单价','100',1,true,'',0,-2,'','',"priceIdentifier","","31",'',"0",false); 
 putChildValidateItem("tblOtherInDet_TaxPrice","含税单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_TaxAmount','含税金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","32",'',"0",false); 
 putChildValidateItem("tblOtherInDet_TaxAmount","含税金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_CoTaxAmt','税额','100',1,true,'',0,-2,'','',"AmountIdentifier","","34",'',"0",false); 
 putChildValidateItem("tblOtherInDet_CoTaxAmt","税额","double","",true,minValue,maxValue,false,''); 
 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","36",'',"100",false); 
 putChildValidateItem("tblOtherInDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","37",'',"3",false); 
 putChildValidateItem("tblOtherInDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","38",'',"3",false); 
 putChildValidateItem("tblOtherInDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_BarCode','条码','100',2,true,'',100,0,'','',"","","40",'',"0",false); 
 putChildValidateItem("tblOtherInDet_BarCode","条码","any","BarCodeAuto",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_colorNV','颜色名称','80',2,true,'',500,-2,'','',"","","41",'',"0",false); 
 putChildValidateItem("tblOtherInDet_colorNV","颜色名称","any","",true,minValue,500,false,''); 
 
 
 
 addCols(tblOtherInDetData,'tblOtherInDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblOtherIn&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblOtherIn,tblOtherInDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblOtherIn&allTableName=tblOtherIn,tblOtherInDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblOtherIn&allTableName=tblOtherInDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblOtherIn&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblOtherIn&allTableName=tblOtherInDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblOtherInDet";
 var stockCode="";
 
 var stockCodeObj=m(cTabName+"_StockCode") ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblOtherIn&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblOtherIn&designId=df66bd5c_1603022309473611195&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblOtherIn&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblOtherIn&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblOtherIn&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '零星入库';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblOtherIn&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
