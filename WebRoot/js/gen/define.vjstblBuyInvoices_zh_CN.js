...
{ 
 cFireEvent('tblBuyInvoicesDet_Qty',''); 
 cFireEvent('tblBuyInvoicesDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBuyInvoices&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBuyInvoices&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBuyInvoices&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBuyInvoices&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBuyInvoicesDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblBuyInvoicesDet_yearNO');
 ...
 var z=n('tblBuyInvoicesDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblBuyInvoicesDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBuyInvoicesDet"){
 
 copyFields=n('tblBuyInvoicesDet_GoodsCode');
 ...
 var tblBuyInvoicesDet_tblGoods_GoodsNumber = n('tblBuyInvoicesDet_tblGoods_GoodsNumber');
 if(typeof(tblBuyInvoicesDet_tblGoods_GoodsNumber) != 'undefined' && tblBuyInvoicesDet_tblGoods_GoodsNumber.length >index){
 tblBuyInvoicesDet_tblGoods_GoodsNumber[index].value = tblBuyInvoicesDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblBuyInvoicesDet_tblGoods_GoodsFullName = n('tblBuyInvoicesDet_tblGoods_GoodsFullName');
 if(typeof(tblBuyInvoicesDet_tblGoods_GoodsFullName) != 'undefined' && tblBuyInvoicesDet_tblGoods_GoodsFullName.length >index){
 tblBuyInvoicesDet_tblGoods_GoodsFullName[index].value = tblBuyInvoicesDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblBuyInvoicesDet_tblUnit_UnitName = n('tblBuyInvoicesDet_tblUnit_UnitName');
 if(typeof(tblBuyInvoicesDet_tblUnit_UnitName) != 'undefined' && tblBuyInvoicesDet_tblUnit_UnitName.length >index){
 tblBuyInvoicesDet_tblUnit_UnitName[index].value = tblBuyInvoicesDet_tblUnit_UnitName[index-1].value;
 }
 }
  copyFields=n('tblBuyInvoicesDet_Unit');
 if((order > 23 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBuyInvoicesDet"){
 ...
 tblBuyInvoicesDet_tblUnit_UnitName[index].value = tblBuyInvoicesDet_tblUnit_UnitName[index-1].value;
  ...
  copyFields=n('tblBuyInvoicesDet_Unit'); 
 if( (order > 23 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
  }
 }
...
function execStat2(){
 execStatOne('tblBuyInvoicesDet_Qty',4);
 execStatOne('tblBuyInvoicesDet_Amount',4);
 execStatOne('tblBuyInvoicesDet_TaxAmount',4);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=f623108d_1407300853216940014&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=f623108d_1407300853216940014&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblBuyInvoices\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBuyInvoices\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblBuyInvoicesDet_Qty',"tblBuyInvoicesDet"); 
 delFireEventOne(rowId,tableName,'tblBuyInvoicesDet_Price',"tblBuyInvoicesDet"); 
 delFireEventOne(rowId,tableName,'tblBuyInvoicesDet_Amount',"tblBuyInvoicesDet"); 
 delFireEventOne(rowId,tableName,'tblBuyInvoicesDet_Discount',"tblBuyInvoicesDet"); 
 delFireEventOne(rowId,tableName,'tblBuyInvoicesDet_DisPrice',"tblBuyInvoicesDet"); 
 delFireEventOne(rowId,tableName,'tblBuyInvoicesDet_DisAmount',"tblBuyInvoicesDet"); 
 delFireEventOne(rowId,tableName,'tblBuyInvoicesDet_TaxPrice',"tblBuyInvoicesDet"); 
 delFireEventOne(rowId,tableName,'tblBuyInvoicesDet_TaxAmount',"tblBuyInvoicesDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'TotalTaxAmount',dis:'总金额',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblBuyInvoicesDet_Qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyInvoicesDet_SpareQty',dis:'士碑数',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyInvoicesDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyInvoicesDet_Amount',dis:'金额',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBuyInvoices",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyInvoices&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyInvoices&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"f623108d_1407300853216940014",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 var saveField=n("CurrencyTypeID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCurrency_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblCurrency_CurrencyName"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCurrency_CurrencyName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("TradeType"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_TradeType");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("Company_ComFullN");
 saveField[0].value = ""; 
 m("tbl_Company_ComFullN").value = "" ;
 }
 ...
 var saveField=n("tblCurrency_CurrencyName");
 saveField[0].value = ""; 
 m("tbl_tblCurrency_CurrencyName").value = "" ;
 }
 if(fieldName == "BuyOrderId"){
 var saveField=n("BuyOrderId"); 
 saveField[0].value = "";
 var obj = m("tbl_tblBuyOrder_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("CompanyCode"); 
 ...
 var saveField=n("Company_ComFullN"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_ComFullName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("CurrencyTypeID"); 
 ...
 var saveField=n("tblBuyOrder_BillNo2");
 saveField[0].value = ""; 
 m("tbl_tblBuyOrder_BillNo2").value = "" ;
 }
  }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f623108d_1407300853216940014&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f623108d_1407300853216940014&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=f623108d_1407300853216940014&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblBuyInvoices"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBuyInvoices&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBuyInvoices&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
for(var j=0;j<r(0,'Tax').length;j++){
r(0,"Tax")[j].value=f(nn('Tax',0),4);
}
...
r(0,"Tax")[j].value=f(nn('Tax',0),4);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalTaxAmount')[0].value=f(Number(sm('tblBuyInvoicesDet_TaxAmount'))-Number(nn('DiscountAmount',0)),4);
...
r(0,"Amount")[j].value=f(rn(0,"Price",j)*rn(0,"Qty",j),4);
}

for(var j=0;j<r(0,'DisPrice').length;j++){
r(0,"DisPrice")[j].value=f(rn(0,"Discount",j)*rn(0,"Price",j),4);
}

for(var j=0;j<r(0,'DisAmount').length;j++){
r(0,"DisAmount")[j].value=f(rn(0,"Qty",j)*rn(0,"DisPrice",j),4);
}

for(var j=0;j<r(0,'TaxPrice').length;j++){
r(0,"TaxPrice")[j].value=f((Number(rn(0,"Tax",j))+1)*rn(0,"Discount",j)*rn(0,"Price",j),4);
}

for(var j=0;j<r(0,'TaxAmount').length;j++){
r(0,"TaxAmount")[j].value=f(rn(0,"TaxPrice",j)*rn(0,"Qty",j),4);
}

n('TotalAmount')[0].value=f(sm('tblBuyInvoicesDet_DisAmount'),4);

n('TotalTaxAmount')[0].value=f(sm('tblBuyInvoicesDet_TaxAmount')-nn('DiscountAmount',0),4);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyInvoicesDet_Qty'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"TaxPrice")[p].value=f((Number(rn(0,"Tax",p))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"TaxAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p)*rn(0,"Discount",p)*(Number(rn(0,"Tax",p))+1),4);
n('TotalAmount')[0].value=f(sm('tblBuyInvoicesDet_DisAmount'),4);
n('TotalTaxAmount')[0].value=f(sm('tblBuyInvoicesDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyInvoicesDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"TaxPrice")[p].value=f((Number(rn(0,"Tax",p))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblBuyInvoicesDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyInvoicesDet_Amount'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"TaxPrice")[p].value=f((Number(rn(0,"Tax",p))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblBuyInvoicesDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyInvoicesDet_Discount'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"TaxPrice")[p].value=f((Number(rn(0,"Tax",p))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblBuyInvoicesDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyInvoicesDet_DisPrice'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"TaxPrice")[p].value=f((Number(rn(0,"Tax",p))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblBuyInvoicesDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyInvoicesDet_DisAmount'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"TaxPrice")[p].value=f((Number(rn(0,"Tax",p))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblBuyInvoicesDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyInvoicesDet_Tax'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"TaxPrice")[p].value=f((Number(rn(0,"Tax",p))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblBuyInvoicesDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyInvoicesDet_TaxPrice'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"TaxPrice")[p].value=f((Number(rn(0,"Tax",p))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblBuyInvoicesDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyInvoicesDet_TaxAmount'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"TaxPrice")[p].value=f((Number(rn(0,"Tax",p))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
...
n('TotalTaxAmount')[0].value=f(sm('tblBuyInvoicesDet_TaxAmount')-nn('DiscountAmount',0),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyInvoicesDet_ConversionRate'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),4);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p),4);
r(0,"DisAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"DisPrice",p),4);
r(0,"TaxPrice")[p].value=f((Number(rn(0,"Tax",p))+1)*rn(0,"Discount",p)*rn(0,"Price",p),4);
...
childData[childCount]='tblBuyInvoicesDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='采购入库发票明细';
childCount++;
 var tblBuyInvoicesDetData = new gridData();
 gridDatas[gridDataNum] = tblBuyInvoicesDetData;
 gridDataNum++;
 var tblBuyInvoicesDet=1;
 ...
 param4=param4+":1制单日期@BillDate"; 
 param4=param4+":1单据类型@BVBillType"; 
 param4=param4+":0采购单号@BuyOrderId"; 
 
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblBuyInStock_BillNo','进货单号','100',2,true,'',1000000,2,'tblBuyInvoicesDet:BuyInStockID:tblBuyInvoicesDet_tblBuyInStock_BillNo:'+param4,"","","","0","","2",false);
 ...
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblGoods_GoodsNumber','物料编号','120',18,true,'',1000000,2,'tblBuyInvoicesDet:GoodsCode:tblBuyInvoicesDet_tblGoods_GoodsNumber:'+param4,"","","copy","2","","2",false);
 ...
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblGoods_GoodsFullName','物料名称','70',2,true,'',1000000,2,'tblBuyInvoicesDet:GoodsCode:tblBuyInvoicesDet_tblGoods_GoodsFullName:'+param4,"","","copy","2","","2",false);
 ...
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblBuyOrderDet_Qty','订货数量','40',1,true,'',1000000,2,'tblBuyInvoicesDet:BuyInStockID:tblBuyInvoicesDet_tblBuyOrderDet_Qty:'+param4,"","","","0","","2",false);
 ...
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblBuyOrderDet_InQty','已收数量','40',1,true,'',1000000,2,'tblBuyInvoicesDet:BuyInStockID:tblBuyInvoicesDet_tblBuyOrderDet_InQty:'+param4,"","","","0","","2",false);
 ...
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblBuyOrderDet_NotInQty','未收数量','40',1,true,'',1000000,2,'tblBuyInvoicesDet:BuyInStockID:tblBuyInvoicesDet_tblBuyOrderDet_NotInQty:'+param4,"","","","0","","2",false);
 ...
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblBuyOrderDet_BackQty','退货数量','40',1,true,'',1000000,2,'tblBuyInvoicesDet:BuyInStockID:tblBuyInvoicesDet_tblBuyOrderDet_BackQty:'+param4,"","","","0","","2",false);
 ...
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblBuyOrderDet_SpareQty','士碑数量','40',0,true,'',1000000,2,'tblBuyInvoicesDet:BuyInStockID:tblBuyInvoicesDet_tblBuyOrderDet_SpareQty:'+param4,"","","","0","","2",false);
 ...
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblBuyOrderDet_InSpareQty','已收士碑数','40',1,true,'',1000000,2,'tblBuyInvoicesDet:BuyInStockID:tblBuyInvoicesDet_tblBuyOrderDet_InSpareQty:'+param4,"","","","0","","2",false);
 ...
 param4=param4+":0采购单号@BuyOrderId"; 
  ...
  addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblBuyOrderDet_NotSpareQty','未入士啤数','40',1,true,'',1000000,2,'tblBuyInvoicesDet:BuyInStockID:tblBuyInvoicesDet_tblBuyOrderDet_NotSpareQty:'+param4,"","","","0","","2",false);
 ...
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblUnit_UnitName','单位','30',2,true,'',1000000,2,'tblBuyInvoicesDet:GoodsCode:tblBuyInvoicesDet_tblUnit_UnitName:'+param4,"","","copy","2","","2",false);
     ...
     addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_Qty','数量','60',1,false,'',0,0,'','',"Qty","","3",'',"0",false); 
 putChildValidateItem("tblBuyInvoicesDet_Qty","数量","double","",false,minValue,maxValue,false,'',true); 
     addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_SpareQty','士碑数','60',1,true,'',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblBuyInvoicesDet_SpareQty","士碑数","double","",true,minValue,maxValue,false,'',true); 
     addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_Price','单价','80',1,true,'',0,8,'','',"SAmountIdentifier","","5",'',"8",false); 
 putChildValidateItem("tblBuyInvoicesDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
     addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_Amount','金额','100',1,true,'',0,8,'','',"SAmountIdentifier","","6",'',"8",false); 
 putChildValidateItem("tblBuyInvoicesDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
     addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_PresentSampleType','赠(附)品','100',2,true,'',10,1,';1:赠品;2:附带配件','',"","","21",'',"1",false);
 putChildValidateItem("tblBuyInvoicesDet_PresentSampleType","赠(附)品","any","",true,minValue,10,false,'',true); 
     addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_Remark','备注','150',2,true,'',500,0,'','',"","","43",'',"0",false); 
 putChildValidateItem("tblBuyInvoicesDet_Remark","备注","any","",true,minValue,500,false,'',true); 
 var param4="";
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_BuyInStockID','进货单号','100',2,true,'',100,-2,'','','',"",'','',"2",false);
 putChildValidateItem("tblBuyInvoicesDet_BuyInStockID","进货单号","any","",true,minValue,100,false,'',true);
       addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_tblBuyOrder1_BillNo','采购单号','100',2,true,'',100,-2,'',"2",false);
                           addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_BuyOrderID','采购单号','100',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBuyInvoicesDet_BuyOrderID","采购单号","any","",true,minValue,100,false,''); 
 var param4="";
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_GoodsCode','物料名称','120',2,false,'',50,-2,'','','',"copy",'','',"2",false);
 putChildValidateItem("tblBuyInvoicesDet_GoodsCode","物料名称","any","",false,minValue,50,false,'',true);
                           addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_Discount','折扣','0',1,true,'1',0,-2,'','',"","","7",'',"6",false); 
 putChildValidateItem("tblBuyInvoicesDet_Discount","折扣","double","",true,minValue,maxValue,true,'1'); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_DisPrice','折后单价','80',1,true,'0',0,-2,'','',"SPriceIdentifier","","8",'',"6",false); 
 putChildValidateItem("tblBuyInvoicesDet_DisPrice","折后单价","double","",true,minValue,maxValue,true,'0'); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_DisAmount','折后金额','0',1,true,'0',0,-2,'','',"SAmountIdentifier","","9",'',"6",false); 
 putChildValidateItem("tblBuyInvoicesDet_DisAmount","折后金额","double","",true,minValue,maxValue,true,'0'); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_Tax','税率','60',1,true,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblBuyInvoicesDet_Tax","税率","double","",true,minValue,maxValue,false,''); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_TaxPrice','含税单价','80',1,true,'0',0,-2,'','',"SPriceIdentifier","","11",'',"6",false); 
 putChildValidateItem("tblBuyInvoicesDet_TaxPrice","含税单价","double","",true,minValue,maxValue,true,'0'); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_TaxAmount','含税金额','100',1,true,'0',0,-2,'','',"SAmountIdentifier","","12",'',"6",false); 
 putChildValidateItem("tblBuyInvoicesDet_TaxAmount","含税金额","double","",true,minValue,maxValue,true,'0'); 
         addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_TrackNo','追踪单号','100',2,true,'',100,-2,'','',"","","22",'',"6",false); 
 putChildValidateItem("tblBuyInvoicesDet_TrackNo","追踪单号","any","",true,minValue,100,false,''); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_Unit','单位','0',2,true,'',100,-2,'','','',"copy",'','',"2",false); 
 putChildValidateItem("tblBuyInvoicesDet_Unit","单位","any","",true,minValue,100,false,''); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_SourceID','源ID','200',2,true,'',60,-2,'','','',"",'','',"100",false); 
 putChildValidateItem("tblBuyInvoicesDet_SourceID","源ID","any","",true,minValue,60,false,''); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_yearNO','高度','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"0",false); 
 putChildValidateItem("tblBuyInvoicesDet_yearNO","高度","any","",true,minValue,100,false,''); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_Inch','宽度','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblBuyInvoicesDet_Inch","宽度","any","",true,minValue,100,false,''); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_BatchNo','批号','120',2,true,'',100,-2,'','','',"triggerCopy",'','',"3",false); 
 putChildValidateItem("tblBuyInvoicesDet_BatchNo","批号","any","",true,minValue,100,false,''); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_color','色彩','60',2,true,'',100,-2,'','','',"",'','',"2",false); 
 putChildValidateItem("tblBuyInvoicesDet_color","色彩","any","",true,minValue,100,false,''); 
    addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_ConversionRate','换算率','80',2,true,'1',100,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblBuyInvoicesDet_ConversionRate","换算率","any","",true,minValue,100,true,'1'); 
      
 addCols(tblBuyInvoicesDetData,'tblBuyInvoicesDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBuyInvoices&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBuyInvoices,tblBuyInvoicesDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyInvoices&allTableName=tblBuyInvoices,tblBuyInvoicesDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyInvoices&allTableName=tblBuyInvoicesDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyInvoices&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyInvoices&allTableName=tblBuyInvoicesDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f623108d_1407300853216940014&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f623108d_1407300853216940014&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblBuyInvoicesDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=f623108d_1407300853216940014&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=f623108d_1407300853216940014&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblBuyInvoices&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=f623108d_1407300853216940014&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblBuyInvoices&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBuyInvoices&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBuyInvoices&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBuyInvoices&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '应付凭证';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBuyInvoices&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
