...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblWatchOther&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblWatchOther&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblWatchOther&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblWatchOther&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchOtherDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchOtherDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblWatchOtherDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblWatchOtherDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchOtherDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchOtherDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblWatchOtherDet"; 
 tblName[1]="tblWatchOtherDet1"; 
...
function execStat2(){
 execStatOne('tblWatchOtherDet_Price',2);
 execStatOne('tblWatchOtherDet_Qty',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblWatchOther\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblWatchOther\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblWatchOtherDet_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblWatchOtherDet_Qty',dis:'用量',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblWatchOther",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchOther&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchOther&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblWatchOther"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblWatchOther&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchOther&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblWatchOtherDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='其它物料明细';
childCount++;
 var tblWatchOtherDetData = new gridData();
 gridDatas[gridDataNum] = tblWatchOtherDetData;
 gridDataNum++;
 var tblWatchOtherDet=1;
 ...
 addCols(tblWatchOtherDetData,'tblWatchOtherDet_GoodsCode','物料编号','100',2,false,'',1000,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("tblWatchOtherDet_GoodsCode","物料编号","any","",false,minValue,1000,false,'');
 addCols(tblWatchOtherDetData,'tblWatchOtherDet_tblGoods_GoodsNumber','物料编号','200',18,true,'',1000000,2,'tblWatchOtherDet:GoodsCode:tblWatchOtherDet_tblGoods_GoodsNumber:'+param4,'',"","","1",'',"2",false); 
 addCols(tblWatchOtherDetData,'tblWatchOtherDet_tblGoods_GoodsFullName','物料名称','140',2,true,'',1000000,2,'tblWatchOtherDet:GoodsCode:tblWatchOtherDet_tblGoods_GoodsFullName:'+param4,'',"","","1",'',"2",false); 
 addCols(tblWatchOtherDetData,'tblWatchOtherDet_tblGoods_GoodsSpec','物料规格','140',18,true,'',1000000,2,'tblWatchOtherDet:GoodsCode:tblWatchOtherDet_tblGoods_GoodsSpec:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(tblWatchOtherDetData,'tblWatchOtherDet_GoodsNumber','物料编号','200',2,true,'',500,-2,'','',"","","1",'',"3",false); 
 putChildValidateItem("tblWatchOtherDet_GoodsNumber","物料编号","any","",true,minValue,500,false,''); 
 
 
 
 addCols(tblWatchOtherDetData,'tblWatchOtherDet_Price','单价','80',1,true,'0',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblWatchOtherDet_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblWatchOtherDetData,'tblWatchOtherDet_Qty','用量','80',1,true,'1',0,0,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblWatchOtherDet_Qty","用量","double","",true,minValue,maxValue,true,'1',true); 
 
 
 
 addCols(tblWatchOtherDetData,'tblWatchOtherDet_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchOtherDet_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 
 addCols(tblWatchOtherDetData,'tblWatchOtherDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblWatchOtherDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='其它变更明细';
childCount++;
 var tblWatchOtherDet1Data = new gridData();
 gridDatas[gridDataNum] = tblWatchOtherDet1Data;
 gridDataNum++;
 var tblWatchOtherDet1=1;
 ...
 addCols(tblWatchOtherDet1Data,'tblWatchOtherDet1_ChangeContent','变更内容','200',2,false,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblWatchOtherDet1_ChangeContent","变更内容","any","",false,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchOtherDet1Data,'tblWatchOtherDet1_ChangeDate','变更日期','90',5,false,'2013-12-24',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchOtherDet1_ChangeDate","变更日期","date","",false,minValue,10,true,'2013-12-24',true); 
 
 
 
 addCols(tblWatchOtherDet1Data,'tblWatchOtherDet1_ChangePer','变更人员','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchOtherDet1_ChangePer","变更人员","any","",true,minValue,100,false,'',true); 
 
 
 
 
 addCols(tblWatchOtherDet1Data,'tblWatchOtherDet1_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchOtherDet1_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchOtherDet1Data,'tblWatchOtherDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblWatchOther&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=68fc380d_1408131539576500232&noback="+noback+"&parentCode=&operation=6&parentTableName=tblSampleOrder&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblWatchOther,tblWatchOtherDet,tblWatchOtherDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchOther&allTableName=tblWatchOther,tblWatchOtherDet,tblWatchOtherDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchOther&allTableName=tblWatchOtherDet,tblWatchOtherDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchOther&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchOther&allTableName=tblWatchOtherDet,tblWatchOtherDet1,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblWatchOther&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblWatchOther&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblWatchOther&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchOther&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblWatchOther&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '其它';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblWatchOther&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
