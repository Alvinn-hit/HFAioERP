...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDUBomBatchD&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDUBomBatchD&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDUBomBatchD&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDUBomBatchD&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDUBomBatchDDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDUBomBatchDDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDUBomBatchDDet"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDUBomBatchD\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDUBomBatchD\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDUBomBatchD",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDUBomBatchD&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDUBomBatchD&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDUBomBatchD"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDUBomBatchD&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDUBomBatchD&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='PDUBomBatchDDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='需修改BOM单';
childCount++;
 var PDUBomBatchDDetData = new gridData();
 gridDatas[gridDataNum] = PDUBomBatchDDetData;
 gridDataNum++;
 var PDUBomBatchDDet=1;
 ...
 var param4="";
 param4=param4+":1原物料品名@GoodsCode"; 
 
 addCols(PDUBomBatchDDetData,'PDUBomBatchDDet_PDBomId','生产BOM','200',2,true,'',50,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("PDUBomBatchDDet_PDBomId","生产BOM","any","",true,minValue,50,false,'');
 addCols(PDUBomBatchDDetData,'PDUBomBatchDDet_tblGoods_GoodsNumber','品号','200',20,true,'',1000000,2,'PDUBomBatchDDet:PDBomId:PDUBomBatchDDet_tblGoods_GoodsNumber:'+param4,'',"","","0",'',"2",false); 
 addCols(PDUBomBatchDDetData,'PDUBomBatchDDet_tblGoods_GoodsFullName','品名','200',20,true,'',1000000,2,'PDUBomBatchDDet:PDBomId:PDUBomBatchDDet_tblGoods_GoodsFullName:'+param4,'',"","","0",'',"2",false); 
 addCols(PDUBomBatchDDetData,'PDUBomBatchDDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDUBomBatchDDet:PDBomId:PDUBomBatchDDet_tblGoods_GoodsSpec:'+param4,'',"","","0",'',"2",false); 
 addCols(PDUBomBatchDDetData,'PDUBomBatchDDet_tblGoods_BaseUnit','单位','50',2,true,'',1000000,2,'PDUBomBatchDDet:PDBomId:PDUBomBatchDDet_tblGoods_BaseUnit:'+param4,'',"","","0",'',"2",false); 
 
 
 addCols(PDUBomBatchDDetData,'PDUBomBatchDDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDUBomBatchD&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDUBomBatchD,PDUBomBatchDDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDUBomBatchD&allTableName=PDUBomBatchD,PDUBomBatchDDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDUBomBatchD&allTableName=PDUBomBatchDDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDUBomBatchD&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDUBomBatchD&allTableName=PDUBomBatchDDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDUBomBatchD&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDUBomBatchD&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDUBomBatchD&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDUBomBatchD&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDUBomBatchD&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '批量删除BOM物料';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDUBomBatchD&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
