...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblWatchCaseLCD&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblWatchCaseLCD&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblWatchCaseLCD&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblWatchCaseLCD&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchCaseLCDDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblWatchCaseLCDDet1Table"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblWatchCaseLCDDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblWatchCaseLCDDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchCaseLCDDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblWatchCaseLCDDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblWatchCaseLCDDet"; 
 tblName[1]="tblWatchCaseLCDDet1"; 
...
function execStat2(){
 execStatOne('tblWatchCaseLCDDet_Qty',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblWatchCaseLCD\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblWatchCaseLCD\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblWatchCaseLCDDet_Qty',dis:'用量',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblWatchCaseLCD",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchCaseLCD&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblWatchCaseLCD&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "WatchBottom"){
 var saveField=n("WatchBottom"); 
 saveField[0].value = "";
 var obj = m("tbl_tblGoods_classCode");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblGoods_WatchBottom");
 saveField[0].value = ""; 
 m("tbl_tblGoods_WatchBottom").value = "" ;
 }
 }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblWatchCaseLCD"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblWatchCaseLCD&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchCaseLCD&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblWatchCaseLCDDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表壳物料明细';
childCount++;
 var tblWatchCaseLCDDetData = new gridData();
 gridDatas[gridDataNum] = tblWatchCaseLCDDetData;
 gridDataNum++;
 var tblWatchCaseLCDDet=1;
 ...
 addCols(tblWatchCaseLCDDetData,'tblWatchCaseLCDDet_GoodsCode','物料名称','100',2,true,'',1000,-2,'','',"","","1",'',"3",false); 
 putChildValidateItem("tblWatchCaseLCDDet_GoodsCode","物料名称","any","",true,minValue,1000,false,''); 
 ...
 addCols(tblWatchCaseLCDDetData,'tblWatchCaseLCDDet_GoodsNumber','物料编号','200',2,false,'',500,2,'tblWatchCaseLCDDet:GoodsNumber:tblWatchCaseLCDDet_GoodsNumber:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblWatchCaseLCDDet_GoodsNumber","物料编号","any","",false,minValue,500,false,'',true);
 
 
 
 addCols(tblWatchCaseLCDDetData,'tblWatchCaseLCDDet_WtCase','套壳内容','120',2,true,'',200,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchCaseLCDDet_WtCase","套壳内容","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchCaseLCDDetData,'tblWatchCaseLCDDet_WtBand','套带内容','120',2,true,'',200,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchCaseLCDDet_WtBand","套带内容","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(tblWatchCaseLCDDetData,'tblWatchCaseLCDDet_Qty','用量','80',1,true,'1',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblWatchCaseLCDDet_Qty","用量","double","",true,minValue,maxValue,true,'1',true); 
 
 
 
 addCols(tblWatchCaseLCDDetData,'tblWatchCaseLCDDet_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchCaseLCDDet_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 
 addCols(tblWatchCaseLCDDetData,'tblWatchCaseLCDDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblWatchCaseLCDDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='表壳变更明细';
childCount++;
 var tblWatchCaseLCDDet1Data = new gridData();
 gridDatas[gridDataNum] = tblWatchCaseLCDDet1Data;
 gridDataNum++;
 var tblWatchCaseLCDDet1=1;
 ...
 addCols(tblWatchCaseLCDDet1Data,'tblWatchCaseLCDDet1_ChangeContent','变更内容','200',2,false,'',500,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblWatchCaseLCDDet1_ChangeContent","变更内容","any","",false,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchCaseLCDDet1Data,'tblWatchCaseLCDDet1_ChangeDate','变更日期','90',5,false,'2013-12-24',10,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblWatchCaseLCDDet1_ChangeDate","变更日期","date","",false,minValue,10,true,'2013-12-24',true); 
 
 
 
 addCols(tblWatchCaseLCDDet1Data,'tblWatchCaseLCDDet1_ChangePer','变更人员','100',2,true,'',100,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblWatchCaseLCDDet1_ChangePer","变更人员","any","",true,minValue,100,false,'',true); 
 
 
 
 
 addCols(tblWatchCaseLCDDet1Data,'tblWatchCaseLCDDet1_Remark','备注','200',2,true,'',500,0,'','',"","","100",'',"0",false); 
 putChildValidateItem("tblWatchCaseLCDDet1_Remark","备注","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(tblWatchCaseLCDDet1Data,'tblWatchCaseLCDDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblWatchCaseLCD&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=68fc380d_1408131539576500232&noback="+noback+"&parentCode=&operation=6&parentTableName=tblSampleOrder&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblWatchCaseLCD,tblWatchCaseLCDDet,tblWatchCaseLCDDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchCaseLCD&allTableName=tblWatchCaseLCD,tblWatchCaseLCDDet,tblWatchCaseLCDDet1,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchCaseLCD&allTableName=tblWatchCaseLCDDet,tblWatchCaseLCDDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchCaseLCD&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchCaseLCD&allTableName=tblWatchCaseLCDDet,tblWatchCaseLCDDet1,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblWatchCaseLCDDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblWatchCaseLCD&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblWatchCaseLCD&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblWatchCaseLCD&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchCaseLCD&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblWatchCaseLCD&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '表壳（LCD）';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblWatchCaseLCD&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
