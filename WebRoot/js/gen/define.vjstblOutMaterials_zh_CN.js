...
{ 
 cFireEvent('tblOutMaterialsDet_Qty',''); 
 cFireEvent('tblOutMaterialsDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblOutMaterials&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblOutMaterials&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblOutMaterials&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblOutMaterials&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblOutMaterialsDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblOutMaterialsDet_GoodsCode');
 ...
 var z=n('tblOutMaterialsDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblOutMaterialsDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblOutMaterialsDet_Hue');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblOutMaterialsDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblOutMaterialsDet"){
 
 copyFields=n('tblOutMaterialsDet_Unit');
 ...
 copyFields=n('tblOutMaterialsDet_GoodsCode');
 if((order > 3 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && ((copyFields[index].value ==0) ||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 var tblOutMaterialsDet_tblGoods_GoodsNumber = n('tblOutMaterialsDet_tblGoods_GoodsNumber');
 if(typeof(tblOutMaterialsDet_tblGoods_GoodsNumber) != 'undefined' && tblOutMaterialsDet_tblGoods_GoodsNumber.length >index){
 tblOutMaterialsDet_tblGoods_GoodsNumber[index].value = tblOutMaterialsDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblOutMaterialsDet_tblGoods_GoodsFullName = n('tblOutMaterialsDet_tblGoods_GoodsFullName');
 if(typeof(tblOutMaterialsDet_tblGoods_GoodsFullName) != 'undefined' && tblOutMaterialsDet_tblGoods_GoodsFullName.length >index){
 tblOutMaterialsDet_tblGoods_GoodsFullName[index].value = tblOutMaterialsDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblOutMaterialsDet_tblGoods_GoodsFullNamePYM = n('tblOutMaterialsDet_tblGoods_GoodsFullNamePYM');
 if(typeof(tblOutMaterialsDet_tblGoods_GoodsFullNamePYM) != 'undefined' && tblOutMaterialsDet_tblGoods_GoodsFullNamePYM.length >index){
 tblOutMaterialsDet_tblGoods_GoodsFullNamePYM[index].value = tblOutMaterialsDet_tblGoods_GoodsFullNamePYM[index-1].value;
 }
 var tblOutMaterialsDet_tblGoods_GoodsSpec = n('tblOutMaterialsDet_tblGoods_GoodsSpec');
 if(typeof(tblOutMaterialsDet_tblGoods_GoodsSpec) != 'undefined' && tblOutMaterialsDet_tblGoods_GoodsSpec.length >index){
 tblOutMaterialsDet_tblGoods_GoodsSpec[index].value = tblOutMaterialsDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblOutMaterialsDet_tblUnit_UnitName = n('tblOutMaterialsDet_tblUnit_UnitName');
 if(typeof(tblOutMaterialsDet_tblUnit_UnitName) != 'undefined' && tblOutMaterialsDet_tblUnit_UnitName.length >index){
 tblOutMaterialsDet_tblUnit_UnitName[index].value = tblOutMaterialsDet_tblUnit_UnitName[index-1].value;
 }
 var tblOutMaterialsDet_tblStocks_lastQty = n('tblOutMaterialsDet_tblStocks_lastQty');
 if(typeof(tblOutMaterialsDet_tblStocks_lastQty) != 'undefined' && tblOutMaterialsDet_tblStocks_lastQty.length >index){
 tblOutMaterialsDet_tblStocks_lastQty[index].value = tblOutMaterialsDet_tblStocks_lastQty[index-1].value;
 }
 }
 copyFields=n('tblOutMaterialsDet_OutStockCode');
 if((order > 8 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && ((copyFields[index].value ==0) ||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 var tblOutMaterialsDet_tblStock_OutStockFullName = n('tblOutMaterialsDet_tblStock_OutStockFullName');
 if(typeof(tblOutMaterialsDet_tblStock_OutStockFullName) != 'undefined' && tblOutMaterialsDet_tblStock_OutStockFullName.length >index){
 tblOutMaterialsDet_tblStock_OutStockFullName[index].value = tblOutMaterialsDet_tblStock_OutStockFullName[index-1].value;
 }
 var tblOutMaterialsDet_tblStocks_lastQty = n('tblOutMaterialsDet_tblStocks_lastQty');
 if(typeof(tblOutMaterialsDet_tblStocks_lastQty) != 'undefined' && tblOutMaterialsDet_tblStocks_lastQty.length >index){
 tblOutMaterialsDet_tblStocks_lastQty[index].value = tblOutMaterialsDet_tblStocks_lastQty[index-1].value;
 }
 var tblOutMaterialsDet_ViewVirtualStock_UseQty = n('tblOutMaterialsDet_ViewVirtualStock_UseQty');
 if(typeof(tblOutMaterialsDet_ViewVirtualStock_UseQty) != 'undefined' && tblOutMaterialsDet_ViewVirtualStock_UseQty.length >index){
 tblOutMaterialsDet_ViewVirtualStock_UseQty[index].value = tblOutMaterialsDet_ViewVirtualStock_UseQty[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblOutMaterialsDet"){
 ...
 copyFields=n('tblOutMaterialsDet_OutStockCode'); 
 if( (order > 8 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyFields[index].value ==0||copyFields[index].value == "")){
 copyFields[index].value = copyFields[index-1].value;
 var tblOutMaterialsDet_tblStock_OutStockFullName = n('tblOutMaterialsDet_tblStock_OutStockFullName'); 
 if(typeof(tblOutMaterialsDet_tblStock_OutStockFullName) != 'undefined' && tblOutMaterialsDet_tblStock_OutStockFullName.length >index){
 tblOutMaterialsDet_tblStock_OutStockFullName[index].value = tblOutMaterialsDet_tblStock_OutStockFullName[index-1].value;
...
function execStat2(){
 execStatOne('tblOutMaterialsDet_Qty',4);
 execStatOne('tblOutMaterialsDet_Amount',4);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=cd4d9bfb_1406051055138940049&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=cd4d9bfb_1406051055138940049&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblOutMaterials\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblOutMaterials\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblOutMaterialsDet_Qty',"tblOutMaterialsDet"); 
 delFireEventOne(rowId,tableName,'tblOutMaterialsDet_Price',"tblOutMaterialsDet"); 
 delFireEventOne(rowId,tableName,'tblOutMaterialsDet_Amount',"tblOutMaterialsDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblOutMaterialsDet_Qty',dis:'数量',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblOutMaterials",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblOutMaterials&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblOutMaterials&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"cd4d9bfb_1406051055138940049",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "ProductionPlantID"){
 var saveField=n("ProductionPlantID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblProductionPlant_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("plantStockCode"); 
 saveField[0].value = "";
 var obj = m("tbl_tblProductionPlant_plantStockCode");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("ViewtblStocksChild_StockFullName"); 
 saveField[0].value = "";
 var obj = m("tbl_ViewtblStocksChild_StockFullName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblProductionPlant_plantName");
 saveField[0].value = ""; 
 m("tbl_tblProductionPlant_plantName").value = "" ;
 var saveField=n("tblProductionPlant_EmployeeID");
 saveField[0].value = ""; 
 m("tbl_tblProductionPlant_EmployeeID").value = "" ;
 }
 if(fieldName == "plantStockCode"){
 var saveField=n("plantStockCode"); 
 saveField[0].value = "";
 ...
 var saveField=n("ConsigneeID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblEmployee_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("NAME_EmpFul");
 saveField[0].value = ""; 
 m("tbl_NAME_EmpFul").value = "" ;
 }
  }
...
function initSeqQtySet(){
 if(false){
   ...
   if(false){
 var fields=n('tblOutMaterialsDet_Qty'); 
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=cd4d9bfb_1406051055138940049&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=cd4d9bfb_1406051055138940049&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=cd4d9bfb_1406051055138940049&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblOutMaterials"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblOutMaterials&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblOutMaterials&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
r(0,"Qty")[j].value=f(nn('Qty',0)*rn(0,"MRate",j),4); 
 if(typeof(r(0,'Qty')[j].onchange)!='undefined'){r(0,'Qty')[j].onchange();}
}

for(var j=0;j<r(0,'Amount').length;j++){
r(0,"Amount")[j].value=f(rn(0,"Qty",j)*rn(0,"Price",j),4); 
 if(typeof(r(0,'Amount')[j].onchange)!='undefined'){r(0,'Amount')[j].onchange();}
}

n('BillAmt')[0].value=f(sm('tblOutMaterialsDet_Amount'),4);
...
function initCalculate(){
 var z=r(0,'ConversionRate');
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutMaterialsDet_ConversionRate'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
n('BillAmt')[0].value=f(sm('tblOutMaterialsDet_Amount'),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutMaterialsDet_Qty'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
n('BillAmt')[0].value=f(sm('tblOutMaterialsDet_Amount'),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutMaterialsDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
n('BillAmt')[0].value=f(sm('tblOutMaterialsDet_Amount'),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblOutMaterialsDet_Amount'); 
 r(0,"Amount")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),4);
n('BillAmt')[0].value=f(sm('tblOutMaterialsDet_Amount'),4);
...
childData[childCount]='tblOutMaterialsDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='发料单明细';
childCount++;
 var tblOutMaterialsDetData = new gridData();
 gridDatas[gridDataNum] = tblOutMaterialsDetData;
 gridDataNum++;
 var tblOutMaterialsDet=1;
 ...
 var param4=""; 
 param4=param4+":1发料仓库@StockCode"; 
 param4=param4+":0发料仓库@tblOutMaterialsDet_OutStockCode";
 param4=param4+":1生产类型@EntrustPro"; 
 param4=param4+":1领料日期@BillDate"; 
 param4=param4+":0发料类型@OutMaterialsType"; 
 
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_tblApplyOutMaterials_BillNo','补料单号','140',2,true,'',1000000,2,'tblOutMaterialsDet:ApplyOutMaterialsID:tblOutMaterialsDet_tblApplyOutMaterials_BillNo:'+param4,"","","","1","","2",false);
 ...
 param4=param4+":1发料仓库@StockCode"; 
 param4=param4+":0发料仓库@tblOutMaterialsDet_OutStockCode";
 param4=param4+":0生产单号@tblOutMaterialsDet_ProduceID";
 ...
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_ViewProduceBillNo_BillNo','生产单号','140',2,true,'',1000000,2,'tblOutMaterialsDet:ProduceID:tblOutMaterialsDet_ViewProduceBillNo_BillNo:'+param4,"","","","2","","2",false);
 ...
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_ViewProduceBillNo_GoodsNumber','产品编号','140',2,true,'',1000000,2,'tblOutMaterialsDet:ProduceID:tblOutMaterialsDet_ViewProduceBillNo_GoodsNumber:'+param4,"","","","2","","2",false);
 ...
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_ViewProduceBillNo_GoodsFullName','产品名称','100',2,true,'',1000000,2,'tblOutMaterialsDet:ProduceID:tblOutMaterialsDet_ViewProduceBillNo_GoodsFullName:'+param4,"","","","2","","2",false);
 ...
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_tblGoods_GoodsNumber','物料编号','120',18,true,'',1000000,2,'tblOutMaterialsDet:GoodsCode:tblOutMaterialsDet_tblGoods_GoodsNumber:'+param4,"","","triggerOrCopy","3","","2",false);
 ...
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_tblGoods_GoodsFullName','物料名称','150',2,true,'',1000000,2,'tblOutMaterialsDet:GoodsCode:tblOutMaterialsDet_tblGoods_GoodsFullName:'+param4,"","","triggerOrCopy","3","","2",false);
 ...
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_tblGoods_GoodsSpec','物料规格','100',18,true,'',1000000,2,'tblOutMaterialsDet:GoodsCode:tblOutMaterialsDet_tblGoods_GoodsSpec:'+param4,"","","triggerOrCopy","3","","2",false);
 ...
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_tblUnit_UnitName','单位','50',2,true,'',1000000,2,'tblOutMaterialsDet:GoodsCode:tblOutMaterialsDet_tblUnit_UnitName:'+param4,"","","triggerOrCopy","3","","2",false);
 ...
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_tblStocks_lastQty','当前库存','90',1,true,'',1000000,2,'tblOutMaterialsDet:GoodsCode:tblOutMaterialsDet_tblStocks_lastQty:'+param4,"","","triggerOrCopy","3","","2",false);
     ...
     addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_Qty','数量','80',1,false,'',0,0,'','',"Qty","","11",'',"0",false); 
 putChildValidateItem("tblOutMaterialsDet_Qty","数量","double","SeqQty",false,minValue,maxValue,false,'',true); 
     addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_DrawMaterialTimes','领料次数','85',2,true,'',30,1,';2:二次领料;3:三次领料','',"","","19",'',"1",false);
 putChildValidateItem("tblOutMaterialsDet_DrawMaterialTimes","领料次数","any","",true,minValue,30,false,'',true); 
     addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_Remark','备注','100',2,true,'',500,0,'','',"","","105",'',"0",false); 
 putChildValidateItem("tblOutMaterialsDet_Remark","备注","any","",true,minValue,500,false,'',true); 
  var param4="";
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_ApplyOutMaterialsID','补料单号','100',2,true,'',100,-2,'','','',"",'','',"2",false);
 putChildValidateItem("tblOutMaterialsDet_ApplyOutMaterialsID","补料单号","any","",true,minValue,100,false,'',true);
     addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_ApplyOutMaterialsNO','补料单号','120',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutMaterialsDet_ApplyOutMaterialsNO","补料单号","any","",true,minValue,100,false,''); 
 var param4="";
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_ProduceID','生产单号','200',2,true,'',100,-2,'','','',"",'','',"2",false);
 putChildValidateItem("tblOutMaterialsDet_ProduceID","生产单号","any","",true,minValue,100,false,'',true);
          addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_ProduceNo','生产单号','140',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutMaterialsDet_ProduceNo","生产单号","any","",true,minValue,100,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_Unit','单位','200',2,true,'',30,-2,'','','',"copy",'','',"2",false); 
 putChildValidateItem("tblOutMaterialsDet_Unit","单位","any","",true,minValue,30,false,''); 
 var param4="";
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_GoodsCode','物料名称','100',2,false,'',30,-2,'','','',"triggerOrCopy",'','',"2",false);
 putChildValidateItem("tblOutMaterialsDet_GoodsCode","物料名称","any","",false,minValue,30,false,'',true);
       addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_tblGoods_GoodsFullNamePYM','拼音码','100',2,false,'',30,-2,'',"2",false);
          addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_Inch','宽度','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblOutMaterialsDet_Inch","宽度","any","",true,minValue,100,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_BatchNo','批号','120',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblOutMaterialsDet_BatchNo","批号","any","",true,minValue,100,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_Hue','颜色','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblOutMaterialsDet_Hue","颜色","any","",true,minValue,100,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_Seq','序列号','120',2,true,'',5000,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutMaterialsDet_Seq","序列号","any","",true,minValue,5000,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_yearNO','高度','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblOutMaterialsDet_yearNO","高度","any","",true,minValue,100,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_ProDate','生产日期','80',5,true,'',10,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblOutMaterialsDet_ProDate","生产日期","date","",true,minValue,10,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_Availably','保质期限','80',5,true,'',10,-2,'','','',"",'','',"0",false); 
 putChildValidateItem("tblOutMaterialsDet_Availably","保质期限","date","",true,minValue,10,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_OutStockCode','发料仓库','100',2,true,'',100,-2,'','','',"copy",'','',"2",false); 
 putChildValidateItem("tblOutMaterialsDet_OutStockCode","发料仓库","any","",true,minValue,100,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_StockCode','车间仓','100',2,true,'',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutMaterialsDet_StockCode","车间仓","any","",true,minValue,100,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_ConversionRate','换算率','80',2,true,'1',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutMaterialsDet_ConversionRate","换算率","any","",true,minValue,100,true,'1'); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_BackQty','退料数量','200',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutMaterialsDet_BackQty","退料数量","double","",true,minValue,maxValue,true,'0'); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_BackAmt','退料金额','200',1,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutMaterialsDet_BackAmt","退料金额","double","",true,minValue,maxValue,true,'0'); 
     addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_Price','单价','80',1,true,'',0,-2,'','',"priceIdentifier","","12",'',"6",false); 
 putChildValidateItem("tblOutMaterialsDet_Price","单价","double","",true,minValue,maxValue,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_ProduceDate','生产日期','200',5,true,'',10,-2,'','','',"",'','',"100",false); 
 putChildValidateItem("tblOutMaterialsDet_ProduceDate","生产日期","date","",true,minValue,10,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_Validity','有效期','200',0,true,'',0,-2,'','','',"",'','',"100",false); 
 putChildValidateItem("tblOutMaterialsDet_Validity","有效期","int","",true,minValue,maxValue,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_Amount','金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","13",'',"6",false); 
 putChildValidateItem("tblOutMaterialsDet_Amount","金额","double","",true,minValue,maxValue,false,''); 
       addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_MaterielType','物料类型','70',2,true,'MainMaterials',80,-2,'','',"","","19",'',"1",false); 
 putChildValidateItem("tblOutMaterialsDet_MaterielType","物料类型","any","",true,minValue,80,true,'MainMaterials'); 
    addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_MRate','物料比例','120',1,false,'',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutMaterialsDet_MRate","物料比例","double","",false,minValue,maxValue,false,''); 
    addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_BillType','单据类型','200',2,true,'tblProduce',100,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblOutMaterialsDet_BillType","单据类型","any","",true,minValue,100,true,'tblProduce'); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_color','色彩','60',2,true,'',5000,-2,'','','',"",'','',"2",false); 
 putChildValidateItem("tblOutMaterialsDet_color","色彩","any","",true,minValue,5000,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_TrackNo','追踪单号','120',2,true,'',100,-2,'','',"","","100",'',"6",false); 
 putChildValidateItem("tblOutMaterialsDet_TrackNo","追踪单号","any","",true,minValue,100,false,''); 
   addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_SourceID','源ID','200',2,true,'',60,-2,'','','',"",'','',"100",false); 
 putChildValidateItem("tblOutMaterialsDet_SourceID","源ID","any","",true,minValue,60,false,''); 
   
 addCols(tblOutMaterialsDetData,'tblOutMaterialsDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblOutMaterials&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblOutMaterials,tblOutMaterialsDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblOutMaterials&allTableName=tblOutMaterials,tblOutMaterialsDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblOutMaterials&allTableName=tblOutMaterialsDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblOutMaterials&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblOutMaterials&allTableName=tblOutMaterialsDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=cd4d9bfb_1406051055138940049&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=cd4d9bfb_1406051055138940049&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblOutMaterialsDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=cd4d9bfb_1406051055138940049&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=cd4d9bfb_1406051055138940049&MOOP=add&selectField=&selectValue=";
 ...
 ExeBalAmt=f(ExeBalAmt-hanExeBalAmt,4);
 var fieldVal='';
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblOutMaterials&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function changePos(type){
 curId = $("#varKeyIds").val();
 curid =curId.split(";")[0];
 curPos = parent.window.$("input[name=keyId]").index(parent.window.$("input[name=keyId][value*="+curId+"]"));
 if(type=="PRE"){
 ...
 obj = parent.window.$("input[name=keyId]:eq("+(curPos-1)+")");
 vid =obj.val();
 vid = vid.split(";")[0];
 ...
 if(curPos==parent.window.$("input[name=keyId]").size()-1){
 alert('本页没有下一条数据了');
 return;
 }else{
 obj = parent.window.$("input[name=keyId]:eq("+(curPos+1)+")");
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblOutMaterials&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblOutMaterials&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblOutMaterials&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblOutMaterials&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '补料处理';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblOutMaterials&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
