...
{ 
 cFireEvent('PDWorkOrderfinishDet_ReturnQty',''); 
 cFireEvent('PDWorkOrderfinishDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDWorkOrderfinish&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDWorkOrderfinish&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDWorkOrderfinish&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDWorkOrderfinish&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDWorkOrderfinishDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDWorkOrderfinishDetailTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('PDWorkOrderfinishDet_GoodsCode');
 ...
 var z=n('PDWorkOrderfinishDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDWorkOrderfinishDet_Hue');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDWorkOrderfinishDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDWorkOrderfinishDetail_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDWorkOrderfinishDetail_Hue');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDWorkOrderfinishDetail_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDWorkOrderfinishDet"){
 
  }
 if(typeof(tableName)!="undefined" && tableName=="PDWorkOrderfinishDetail"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDWorkOrderfinishDet"){
 
  }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDWorkOrderfinishDetail"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="PDWorkOrderfinishDet"; 
 var goodsCodeLogicValidate_PDWorkOrderfinishDet='@IOO:Outstore';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'PDWorkOrderfinishDet',value:goodsCodeLogicValidate_PDWorkOrderfinishDet};
 tblName[1]="PDWorkOrderfinishDetail"; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDWorkOrderfinish\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDWorkOrderfinish\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'PDWorkOrderfinishDet_SendQty',"PDWorkOrderfinishDet"); 
 delFireEventOne(rowId,tableName,'PDWorkOrderfinishDet_ReturnQty',"PDWorkOrderfinishDet"); 
 delFireEventOne(rowId,tableName,'PDWorkOrderfinishDet_CheckQty',"PDWorkOrderfinishDet"); 
 delFireEventOne(rowId,tableName,'PDWorkOrderfinishDet_DiffQty',"PDWorkOrderfinishDet"); 
 delFireEventOne(rowId,tableName,'PDWorkOrderfinishDet_Price',"PDWorkOrderfinishDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'Qty',dis:'已交数量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'BillAmt',dis:'差异总额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_UseQty',dis:'单位用量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_Qty',dis:'需求量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_SendQty',dis:'已发料数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_SendQtyExt',dis:'未发料数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_BakQty',dis:'备品量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_SendBakQty',dis:'已发备品量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_SendBakQtyExt',dis:'未发备品量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_NoBakQty',dis:'非备品补料量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_ReturnQty',dis:'退货量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_OutQty',dis:'超领量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_CheckQty',dis:'验冲数',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_LossRate',dis:'损耗率%',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_StandLossRate',dis:'标准损率',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_DiffQty',dis:'差异量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_Price',dis:'单价',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDet_Amt',dis:'差异金额',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDWorkOrderfinishDetail_Qty',dis:'扣料数',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDWorkOrderfinish",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDWorkOrderfinish&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDWorkOrderfinish&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 var saveField=n("tblDepartment_DeptFullName3");
 saveField[0].value = ""; 
 m("tbl_tblDepartment_DeptFullName3").value = "" ;
 }
 }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDWorkOrderfinish"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDWorkOrderfinish&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDWorkOrderfinish&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'PDWorkOrderfinishDet_SendQty'); 
 r(0,"DiffQty")[p].value=f(rn(0,"SendQty",p)-rn(0,"CheckQty",p)-rn(0,"ReturnQty",p),2);
if(rn(0,"DiffQty",p)<0)r(0,"Amt")[p].value=f(0,2);
r(0,"Amt")[p].value=f(rn(0,"DiffQty",p)*rn(0,"Price",p),2);
n('BillAmt')[0].value=f(sm('PDWorkOrderfinishDet_Amt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDWorkOrderfinishDet_ReturnQty'); 
 r(0,"DiffQty")[p].value=f(rn(0,"SendQty",p)-rn(0,"CheckQty",p)-rn(0,"ReturnQty",p),2);
if(rn(0,"DiffQty",p)<0)r(0,"Amt")[p].value=f(0,2);
r(0,"Amt")[p].value=f(rn(0,"DiffQty",p)*rn(0,"Price",p),2);
n('BillAmt')[0].value=f(sm('PDWorkOrderfinishDet_Amt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDWorkOrderfinishDet_CheckQty'); 
 r(0,"DiffQty")[p].value=f(rn(0,"SendQty",p)-rn(0,"CheckQty",p)-rn(0,"ReturnQty",p),2);
if(rn(0,"DiffQty",p)<0)r(0,"Amt")[p].value=f(0,2);
r(0,"Amt")[p].value=f(rn(0,"DiffQty",p)*rn(0,"Price",p),2);
n('BillAmt')[0].value=f(sm('PDWorkOrderfinishDet_Amt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDWorkOrderfinishDet_DiffQty'); 
 r(0,"DiffQty")[p].value=f(rn(0,"SendQty",p)-rn(0,"CheckQty",p)-rn(0,"ReturnQty",p),2);
r(0,"Amt")[p].value=f(rn(0,"DiffQty",p)*rn(0,"Price",p),2);
if(rn(0,"DiffQty",p)<0)r(0,"Amt")[p].value=f(0,2);
n('BillAmt')[0].value=f(sm('PDWorkOrderfinishDet_Amt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDWorkOrderfinishDet_Price'); 
 if(rn(0,"DiffQty",p)<0)r(0,"Amt")[p].value=f(0,2);
r(0,"Amt")[p].value=f(rn(0,"DiffQty",p)*rn(0,"Price",p),2);
n('BillAmt')[0].value=f(sm('PDWorkOrderfinishDet_Amt'),2);
...
childData[childCount]='PDWorkOrderfinishDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='工令强结明细';
childCount++;
 var PDWorkOrderfinishDetData = new gridData();
 gridDatas[gridDataNum] = PDWorkOrderfinishDetData;
 gridDataNum++;
 var PDWorkOrderfinishDet=1;
 ...
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_GoodsCode','原料名称','100',2,false,'',100,-2,'','',"","triggerCopy","0",'',"2",true);
 putChildValidateItem("PDWorkOrderfinishDet_GoodsCode","原料名称","any","GoodsField",false,minValue,100,false,'');
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_tblGoods_GoodsNumber','品号','200',20,true,'',1000000,8,'PDWorkOrderfinishDet:GoodsCode:PDWorkOrderfinishDet_tblGoods_GoodsNumber:'+param4,'',"","triggerCopy","0",'',"2",false); 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_tblGoods_GoodsFullName','原料名称','300',20,true,'',1000000,8,'PDWorkOrderfinishDet:GoodsCode:PDWorkOrderfinishDet_tblGoods_GoodsFullName:'+param4,'',"","triggerCopy","0",'',"2",false); 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,8,'PDWorkOrderfinishDet:GoodsCode:PDWorkOrderfinishDet_tblGoods_GoodsSpec:'+param4,'',"","triggerCopy","0",'',"2",false); 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,8,'PDWorkOrderfinishDet:GoodsCode:PDWorkOrderfinishDet_tblGoods_BaseUnit:'+param4,'',"","triggerCopy","0",'',"2",false); 
 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_BatchNo','批号','200',2,true,'',100,-2,'','',"","","1",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_BatchNo","批号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","2",'',"2",false); 
 putChildValidateItem("PDWorkOrderfinishDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","3",'',"2",false); 
 putChildValidateItem("PDWorkOrderfinishDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","4",'',"2",false); 
 putChildValidateItem("PDWorkOrderfinishDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_ProDate','生产日期','200',5,true,'',10,-2,'','',"","","5",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_ProDate","生产日期","date","",true,minValue,10,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_Availably','保质期限','200',5,true,'',10,-2,'','',"","","6",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_Availably","保质期限","date","",true,minValue,10,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_UseQty','单位用量','80',1,true,'',0,8,'','',"","","7",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_UseQty","单位用量","double","GoodsField",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_Qty','需求量','80',1,true,'',0,8,'','',"","","8",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_Qty","需求量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_SendQty','已发料数量','80',1,true,'',0,8,'','',"","","9",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_SendQty","已发料数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_SendQtyExt','未发料数量','80',1,true,'',0,8,'','',"","","10",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_SendQtyExt","未发料数量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_BakQty','备品量','80',1,true,'',0,8,'','',"","","11",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_BakQty","备品量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_SendBakQty','已发备品量','80',1,true,'',0,8,'','',"","","12",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_SendBakQty","已发备品量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_SendBakQtyExt','未发备品量','80',1,true,'',0,8,'','',"","","13",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_SendBakQtyExt","未发备品量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_NoBakQty','非备品补料量','80',1,true,'',0,8,'','',"","","14",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_NoBakQty","非备品补料量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_ReturnQty','退货量','80',1,true,'',0,8,'','',"","","15",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_ReturnQty","退货量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_OutQty','超领量','80',1,true,'',0,8,'','',"","","16",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_OutQty","超领量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_CheckQty','验冲数','80',1,true,'',0,8,'','',"","","17",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_CheckQty","验冲数","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_LossRate','损耗率%','80',1,true,'',0,8,'','',"","","18",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_LossRate","损耗率%","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_StandLossRate','标准损率','80',1,true,'',0,8,'','',"","","19",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_StandLossRate","标准损率","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_DiffQty','差异量','80',1,true,'',0,8,'','',"","","20",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_DiffQty","差异量","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_Price','单价','80',1,true,'',0,0,'','',"","","21",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_Amt','差异金额','80',1,true,'',0,8,'','',"","","22",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_Amt","差异金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_SecUnit','辅助单位','200',2,true,'',30,-2,'','',"","","23",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_SecUnit","辅助单位","any","",true,minValue,30,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_UnitIntQty','辅助单位整数','200',0,true,'',0,-2,'','',"","","24",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_UnitIntQty","辅助单位整数","int","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_UnitBaseQty','辅助基本单位','200',1,true,'',0,-2,'','',"","","25",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_UnitBaseQty","辅助基本单位","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_UnitQty','辅助单位数量','200',1,true,'',0,-2,'','',"","","26",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_UnitQty","辅助单位数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_UnitPrice','辅助单位单价','200',1,true,'',0,-2,'','',"","","27",'',"2",false); 
 putChildValidateItem("PDWorkOrderfinishDet_UnitPrice","辅助单位单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_SecQty','辅助数量','200',0,true,'',0,-2,'','',"","","28",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_SecQty","辅助数量","int","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_BaseQty','基本单位','200',0,true,'',0,-2,'','',"","","29",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_BaseQty","基本单位","int","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_Remark','强结原因','100',2,false,'',8000,0,'','',"","","30",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_Remark","强结原因","any","",false,minValue,8000,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_SourceID','SourceID','200',2,true,'',50,-2,'','',"","","31",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_SourceID","SourceID","any","",true,minValue,50,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_WorkOrderID','工令单ID','200',2,true,'',50,-2,'','',"","","32",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDet_WorkOrderID","工令单ID","any","",true,minValue,50,false,''); 
 
 
 addCols(PDWorkOrderfinishDetData,'PDWorkOrderfinishDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='PDWorkOrderfinishDetail';
tabIsNulls[childCount]='0';
tabDiss[childCount]='强结原料明细';
childCount++;
 var PDWorkOrderfinishDetailData = new gridData();
 gridDatas[gridDataNum] = PDWorkOrderfinishDetailData;
 gridDataNum++;
 var PDWorkOrderfinishDetail=1;
 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_WorkOrderID','工令单号','200',2,true,'',50,-2,'','',"","","0",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_WorkOrderID","工令单号","any","",true,minValue,50,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_StockCode','出库仓库','200',2,true,'',100,-2,'','',"","","1",'',"2",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_StockCode","出库仓库","any","",true,minValue,100,true,''); 
 ...
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_GoodsCode','原料名称','100',2,false,'',100,-2,'','',"","","2",'',"2",true);
 putChildValidateItem("PDWorkOrderfinishDetail_GoodsCode","原料名称","any","",false,minValue,100,false,'');
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_tblGoods_GoodsNumber','品号','200',20,true,'',1000000,8,'PDWorkOrderfinishDetail:GoodsCode:PDWorkOrderfinishDetail_tblGoods_GoodsNumber:'+param4,'',"","","2",'',"2",false); 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_tblGoods_GoodsFullName','原料名称','300',20,true,'',1000000,8,'PDWorkOrderfinishDetail:GoodsCode:PDWorkOrderfinishDetail_tblGoods_GoodsFullName:'+param4,'',"","","2",'',"2",false); 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,8,'PDWorkOrderfinishDetail:GoodsCode:PDWorkOrderfinishDetail_tblGoods_GoodsSpec:'+param4,'',"","","2",'',"2",false); 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_tblGoods_BaseUnit','单位','30',2,true,'',1000000,8,'PDWorkOrderfinishDetail:GoodsCode:PDWorkOrderfinishDetail_tblGoods_BaseUnit:'+param4,'',"","","2",'',"2",false); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_BatchNo','批号','200',2,true,'',100,-2,'','',"","","3",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_BatchNo","批号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","4",'',"2",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","5",'',"2",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","6",'',"2",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_ProDate','生产日期','200',5,true,'',10,-2,'','',"","","7",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_ProDate","生产日期","date","",true,minValue,10,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_Availably','保质期限','200',5,true,'',10,-2,'','',"","","8",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_Availably","保质期限","date","",true,minValue,10,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_Qty','扣料数','80',1,true,'',0,8,'','',"Qty","","9",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_Qty","扣料数","double","GoodsField",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_Price','单价','200',1,true,'',0,-2,'','',"","","10",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_Price","单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_Amount','金额','200',1,true,'',0,-2,'','',"","","11",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_Amount","金额","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_SecUnit','辅助单位','200',2,true,'',30,-2,'','',"Qty","","12",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_SecUnit","辅助单位","any","",true,minValue,30,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_UnitIntQty','辅助单位整数','200',0,true,'',0,-2,'','',"","","13",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_UnitIntQty","辅助单位整数","int","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_UnitBaseQty','辅助基本单位','200',1,true,'',0,-2,'','',"","","14",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_UnitBaseQty","辅助基本单位","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_UnitQty','辅助单位数量','200',1,true,'',0,-2,'','',"","","15",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_UnitQty","辅助单位数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_UnitPrice','辅助单位单价','200',1,true,'',0,-2,'','',"","","16",'',"2",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_UnitPrice","辅助单位单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_SecQty','辅助数量','200',0,true,'',0,-2,'','',"","","17",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_SecQty","辅助数量","int","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_BaseQty','基本单位','200',0,true,'',0,-2,'','',"","","18",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_BaseQty","基本单位","int","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_Remark','备注','200',3,true,'',8000,8,'','',"","","19",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_Seq','序列号','200',2,true,'',500,-2,'','',"","","20",'',"0",false); 
 putChildValidateItem("PDWorkOrderfinishDetail_Seq","序列号","any","",true,minValue,500,false,''); 
 
 
 addCols(PDWorkOrderfinishDetailData,'PDWorkOrderfinishDetail_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDWorkOrderfinish&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDWorkOrderfinish,PDWorkOrderfinishDet,PDWorkOrderfinishDetail,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDWorkOrderfinish&allTableName=PDWorkOrderfinish,PDWorkOrderfinishDet,PDWorkOrderfinishDetail,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDWorkOrderfinish&allTableName=PDWorkOrderfinishDet,PDWorkOrderfinishDetail,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDWorkOrderfinish&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDWorkOrderfinish&allTableName=PDWorkOrderfinishDet,PDWorkOrderfinishDetail,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "PDWorkOrderfinishDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDWorkOrderfinish&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDWorkOrderfinish&designId=13FEF512816D4463AFC53FDFC3DB&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDWorkOrderfinish&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDWorkOrderfinish&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDWorkOrderfinish&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '工令强结单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDWorkOrderfinish&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
