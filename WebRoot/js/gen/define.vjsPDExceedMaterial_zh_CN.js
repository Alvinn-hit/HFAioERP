...
{ 
 cFireEvent('PDExceedMaterialDet_Qty',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDExceedMaterial&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDExceedMaterial&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDExceedMaterial&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDExceedMaterial&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDExceedMaterialDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('PDExceedMaterialDet_GoodsCode');
 ...
 var z=n('PDExceedMaterialDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDExceedMaterialDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDExceedMaterialDet_Hue');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDExceedMaterialDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDExceedMaterialDet"){
 
 copyFields=n('PDExceedMaterialDet_InStockCode');
 ...
 copyFields=n('PDExceedMaterialDet_GoodsCode');
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDExceedMaterialDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="PDExceedMaterialDet"; 
 var goodsCodeLogicValidate_PDExceedMaterialDet='@GN:Seq:Qty:tblAllotDet_GoodsSeqSet:tblAllotDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'PDExceedMaterialDet',value:goodsCodeLogicValidate_PDExceedMaterialDet};
...
function execStat2(){
 execStatOne('PDExceedMaterialDet_ExceedQty',4);
 execStatOne('PDExceedMaterialDet_DeQty',4);
 execStatOne('PDExceedMaterialDet_Qty',4);
 execStatOne('PDExceedMaterialDet_OutQty',4);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDExceedMaterial\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDExceedMaterial\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'PDExceedMaterialDet_ExceedQty',"PDExceedMaterialDet"); 
 delFireEventOne(rowId,tableName,'PDExceedMaterialDet_DeQty',"PDExceedMaterialDet"); 
 delFireEventOne(rowId,tableName,'PDExceedMaterialDet_Qty',"PDExceedMaterialDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'Qty',dis:'生产数量',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'PDExceedMaterialDet_ExceedQty',dis:'超领数',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDExceedMaterialDet_DeQty',dis:'扣欠料',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDExceedMaterialDet_ShQty',dis:'仓库应发',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDExceedMaterialDet_Qty',dis:'仓库出货',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDExceedMaterialDet_OutQty',dis:'实发数',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDExceedMaterialDet_SalesPrice',dis:'价格',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDExceedMaterialDet_SalesAmt',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDExceedMaterial",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDExceedMaterial&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDExceedMaterial&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 if(fieldName == "PDWorkOrderID"){
 var saveField=n("tblGoods_GoodsNumber"); 
 saveField[0].value = "";
 var obj = m("tbl_tblGoods_GoodsNumber");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblGoods_GoodsFullName"); 
 saveField[0].value = "";
 var obj = m("tbl_tblGoods_GoodsFullName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblGoods_GoodsSpec"); 
 saveField[0].value = "";
 var obj = m("tbl_tblGoods_GoodsSpec");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblGoods_BaseUnit"); 
 saveField[0].value = "";
 var obj = m("tbl_tblGoods_BaseUnit");
 if(obj != null){
 obj.value = "";
 }
 ...
 var saveField=n("WorkShop"); 
 saveField[0].value = "";
 var obj = m("tbl_PDWorkOrder_WorkShop");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("Qty"); 
 saveField[0].value = "";
 var obj = m("tbl_PDWorkOrder_Qty");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("InStockCode"); 
 saveField[0].value = "";
 var obj = m("tbl_PDWorkOrder_StockCode");
 if(obj != null){
 obj.value = "";
 }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDExceedMaterial"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDExceedMaterial&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDExceedMaterial&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'PDExceedMaterialDet_ExceedQty'); 
 if(rn(0,"ExceedQty",p)<=f(rn(0,"tblStocks_OweQty",p),4))r(0,"DeQty")[p].value=f(rn(0,"ExceedQty",p),4);
 if(rn(0,"ExceedQty",p)>f(rn(0,"tblStocks_OweQty",p),4))r(0,"DeQty")[p].value=f(f(rn(0,"tblStocks_OweQty",p),4),4);
r(0,"ShQty")[p].value=f(rn(0,"ExceedQty",p)-rn(0,"DeQty",p),4);
r(0,"Qty")[p].value=f(rn(0,"ShQty",p),4);
if(rn(0,"Qty",p)+rn(0,"DeQty",p)>=rn(0,"ExceedQty",p))r(0,"OutQty")[p].value=f(rn(0,"ExceedQty",p),4);
else if(rn(0,"Qty",p)+rn(0,"DeQty",p)<rn(0,"ExceedQty",p))r(0,"OutQty")[p].value=f(rn(0,"Qty",p)+rn(0,"DeQty",p),4);
r(0,"Amt")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),2);
n('BillAmt')[0].value=f(sm('PDExceedMaterialDet_Amt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDExceedMaterialDet_DeQty'); 
 r(0,"ShQty")[p].value=f(rn(0,"ExceedQty",p)-rn(0,"DeQty",p),4);
r(0,"Qty")[p].value=f(rn(0,"ShQty",p),4);
if(rn(0,"Qty",p)+rn(0,"DeQty",p)>=rn(0,"ExceedQty",p))r(0,"OutQty")[p].value=f(rn(0,"ExceedQty",p),4);
else if(rn(0,"Qty",p)+rn(0,"DeQty",p)<rn(0,"ExceedQty",p))r(0,"OutQty")[p].value=f(rn(0,"Qty",p)+rn(0,"DeQty",p),4);
...
n('BillAmt')[0].value=f(sm('PDExceedMaterialDet_Amt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDExceedMaterialDet_Qty'); 
 if(rn(0,"Qty",p)+rn(0,"DeQty",p)>=rn(0,"ExceedQty",p))r(0,"OutQty")[p].value=f(rn(0,"ExceedQty",p),4);
else if(rn(0,"Qty",p)+rn(0,"DeQty",p)<rn(0,"ExceedQty",p))r(0,"OutQty")[p].value=f(rn(0,"Qty",p)+rn(0,"DeQty",p),4);
r(0,"Amt")[p].value=f(rn(0,"Qty",p)*rn(0,"Price",p),2);
n('BillAmt')[0].value=f(sm('PDExceedMaterialDet_Amt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDExceedMaterialDet_Price'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDExceedMaterialDet_SecQty'); 
...
childData[childCount]='PDExceedMaterialDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='超领明细';
childCount++;
 var PDExceedMaterialDetData = new gridData();
 gridDatas[gridDataNum] = PDExceedMaterialDetData;
 gridDataNum++;
 var PDExceedMaterialDet=1;
 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_StockCode','出库仓库','80',2,true,'',100,-2,'','',"","","0",'',"2",false); 
 putChildValidateItem("PDExceedMaterialDet_StockCode","出库仓库","any","",true,minValue,100,true,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_InStockCode','入库仓库','80',2,true,'',100,-2,'','',"","copy","1",'',"3",false); 
 putChildValidateItem("PDExceedMaterialDet_InStockCode","入库仓库","any","",true,minValue,100,false,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_GoodsCode','商品','120',2,true,'',100,-2,'','',"","triggerOrCopy","2",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_GoodsCode","商品","any","",true,minValue,100,false,''); 
 ...
 var param4="";
 param4=param4+":1超领仓库@StockCode"; 
 param4=param4+":0出库仓库@PDExceedMaterialDet_StockCode";
 param4=param4+":0超领目标@InStockCode"; 
 param4=param4+":0入库仓库@PDExceedMaterialDet_InStockCode";
 param4=param4+":1工令单号@PDWorkOrderID"; 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_MGoodsCode','品名','120',2,false,'',100,-2,'','',"","","3",'',"2",true);
 putChildValidateItem("PDExceedMaterialDet_MGoodsCode","品名","any","",false,minValue,100,false,'');
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'PDExceedMaterialDet:MGoodsCode:PDExceedMaterialDet_tblGoods_GoodsNumber:'+param4,'',"","","3",'',"2",false); 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_tblGoods_GoodsFullName','商品','150',2,true,'',1000000,2,'PDExceedMaterialDet:MGoodsCode:PDExceedMaterialDet_tblGoods_GoodsFullName:'+param4,'',"","","3",'',"2",false); 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDExceedMaterialDet:MGoodsCode:PDExceedMaterialDet_tblGoods_GoodsSpec:'+param4,'',"","","3",'',"2",false); 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDExceedMaterialDet:MGoodsCode:PDExceedMaterialDet_tblGoods_BaseUnit:'+param4,'',"","","3",'',"2",false); 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_tblStocks_lastQty','实际库存','90',1,true,'',1000000,2,'PDExceedMaterialDet:MGoodsCode:PDExceedMaterialDet_tblStocks_lastQty:'+param4,'',"","","3",'',"2",false); 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_ViewVirtualStock_UseQty','虚拟库存','90',1,true,'',1000000,2,'PDExceedMaterialDet:MGoodsCode:PDExceedMaterialDet_ViewVirtualStock_UseQty:'+param4,'',"","","3",'',"2",false); 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_tblStocks_OweQty','欠料余量','90',1,true,'',1000000,2,'PDExceedMaterialDet:MGoodsCode:PDExceedMaterialDet_tblStocks_OweQty:'+param4,'',"","","3",'',"2",false); 
 ...
 param4=param4+":0超领目标@InStockCode"; 
 param4=param4+":0入库仓库@PDExceedMaterialDet_InStockCode";
 param4=param4+":1品名@PDExceedMaterialDet_MGoodsCode";
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_RGoodsCode','替换料','100',2,true,'',100,-2,'','',"","","4",'',"2",true);
 putChildValidateItem("PDExceedMaterialDet_RGoodsCode","替换料","any","",true,minValue,100,false,'');
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_tblGoods1_GoodsNumber','替换料号','120',0,true,'',1000000,2,'PDExceedMaterialDet:RGoodsCode:PDExceedMaterialDet_tblGoods1_GoodsNumber:'+param4,'',"","","4",'',"2",false); 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_tblGoods1_GoodsFullName','替换料名','200',0,true,'',1000000,2,'PDExceedMaterialDet:RGoodsCode:PDExceedMaterialDet_tblGoods1_GoodsFullName:'+param4,'',"","","4",'',"2",false); 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_tblGoods1_GoodsSpec','替换料规格','200',0,true,'',1000000,2,'PDExceedMaterialDet:RGoodsCode:PDExceedMaterialDet_tblGoods1_GoodsSpec:'+param4,'',"","","4",'',"2",false); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","5",'',"2",false); 
 putChildValidateItem("PDExceedMaterialDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","6",'',"2",false); 
 putChildValidateItem("PDExceedMaterialDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_color','色彩','60',2,true,'',100,-2,'','',"","","7",'',"2",false); 
 putChildValidateItem("PDExceedMaterialDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","8",'',"2",false); 
 putChildValidateItem("PDExceedMaterialDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","9",'',"2",false); 
 putChildValidateItem("PDExceedMaterialDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","10",'',"2",false); 
 putChildValidateItem("PDExceedMaterialDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","11",'',"2",false); 
 putChildValidateItem("PDExceedMaterialDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_isClient','客供料','50',0,true,'2',0,8,';1:是;2:否','',"","","12",'',"1",false);
 putChildValidateItem("PDExceedMaterialDet_isClient","客供料","int","",true,minValue,maxValue,true,'2',true); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_ExceedQty','超领数','80',1,true,'',0,0,'','',"Qty","","13",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_ExceedQty","超领数","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_DeQty','扣欠料','80',1,true,'',0,0,'','',"Qty","","14",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_DeQty","扣欠料","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_ShQty','仓库应发','80',1,true,'',0,8,'','',"Qty","","15",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_ShQty","仓库应发","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_Qty','仓库出货','80',1,true,'',0,0,'','',"Qty","","16",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_Qty","仓库出货","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_OutQty','实发数','80',1,true,'',0,8,'','',"Qty","","17",'',"8",false); 
 putChildValidateItem("PDExceedMaterialDet_OutQty","实发数","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_SalesPrice','价格','80',1,true,'',0,0,'','',"SPriceIdentifier","","18",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_SalesPrice","价格","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_SalesAmt','金额','80',1,true,'',0,0,'','',"SAmountIdentifier","","19",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_SalesAmt","金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_Price','成本单价','80',1,true,'0',0,-2,'','',"priceIdentifier","","20",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_Price","成本单价","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_Amt','成本金额','70',1,true,'0',0,-2,'','',"AmountIdentifier","","21",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_Amt","成本金额","double","",true,minValue,maxValue,true,'0'); 
 ...
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","27",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_CheckBarCode','复核码','160',2,true,'',200,-2,'','',"","","28",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_CheckBarCode","复核码","any","",true,minValue,200,false,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","29",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","30",'',"3",false); 
 putChildValidateItem("PDExceedMaterialDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_Remark','备注','130',2,true,'',8000,0,'','',"","","31",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_PDWorkOrderID','工令单号','200',2,true,'',50,-2,'','',"","","32",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_PDWorkOrderID","工令单号","any","",true,minValue,50,false,''); 
 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_Seq','序列号','200',2,true,'',100,-2,'','',"","","33",'',"0",false); 
 putChildValidateItem("PDExceedMaterialDet_Seq","序列号","any","",true,minValue,100,false,''); 
 
 
 addCols(PDExceedMaterialDetData,'PDExceedMaterialDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDExceedMaterial&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDExceedMaterial,PDExceedMaterialDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDExceedMaterial&allTableName=PDExceedMaterial,PDExceedMaterialDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDExceedMaterial&allTableName=PDExceedMaterialDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDExceedMaterial&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDExceedMaterial&allTableName=PDExceedMaterialDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "PDExceedMaterialDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDExceedMaterial&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDExceedMaterial&designId=43F7C2F0736C45848E500A4A0C69&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDExceedMaterial&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDExceedMaterial&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDExceedMaterial&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '超领单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDExceedMaterial&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
