...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblTousuReturn&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblTousuReturn&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblTousuReturn&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblTousuReturn&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=bdc2ccff_1408121019097665754&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=bdc2ccff_1408121019097665754&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblTousuReturn\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblTousuReturn\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'ComAmt',dis:'客户承担金额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'DeptAmt',dis:'分部承担金额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'DeptReAmt',dis:'公司退还金额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ComReAmt',dis:'退还客户金额',digit:0,isMain:true};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblTousuReturn",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblTousuReturn&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblTousuReturn&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"bdc2ccff_1408121019097665754",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "MainDetxID"){
 var saveField=n("MainDetxID"); 
 saveField[0].value = "";
 var obj = m("tbl_tblMainDetx_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("ReceiveDate"); 
 saveField[0].value = "";
 var obj = m("tbl_tblMainDetx_ReceiveDate");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblMainDetx_OrderNo");
 saveField[0].value = ""; 
 m("tbl_tblMainDetx_OrderNo").value = "" ;
 var saveField=n("Employee_EmpFullName");
 saveField[0].value = ""; 
 m("tbl_Employee_EmpFullName").value = "" ;
 var saveField=n("Department_DeptFullName");
 saveField[0].value = ""; 
 m("tbl_Department_DeptFullName").value = "" ;
 var saveField=n("tblMainDetx_CompanyCode");
 saveField[0].value = ""; 
 m("tbl_tblMainDetx_CompanyCode").value = "" ;
 var saveField=n("tblMainDetx_Tel");
 saveField[0].value = ""; 
 m("tbl_tblMainDetx_Tel").value = "" ;
 var saveField=n("tblMainDetx_IDCard");
 saveField[0].value = ""; 
 m("tbl_tblMainDetx_IDCard").value = "" ;
 var saveField=n("tblMainDetx_Address");
 saveField[0].value = ""; 
 m("tbl_tblMainDetx_Address").value = "" ;
 var saveField=n("tblMainDetx_type");
 saveField[0].value = ""; 
 m("tbl_tblMainDetx_type").value = "" ;
 var saveField=n("tblGoods_GoodsFullName");
 saveField[0].value = ""; 
 m("tbl_tblGoods_GoodsFullName").value = "" ;
 var saveField=n("tblDXCombo_DXCombo");
 saveField[0].value = ""; 
 m("tbl_tblDXCombo_DXCombo").value = "" ;
 var saveField=n("tblModel_Model");
 saveField[0].value = ""; 
 m("tbl_tblModel_Model").value = "" ;
 var saveField=n("tblPad_Pad");
 saveField[0].value = ""; 
 m("tbl_tblPad_Pad").value = "" ;
 var saveField=n("tblMainDetx_TotalAmount");
 saveField[0].value = ""; 
 m("tbl_tblMainDetx_TotalAmount").value = "" ;
 var saveField=n("tblLanzhuang_lzbh");
 saveField[0].value = ""; 
 m("tbl_tblLanzhuang_lzbh").value = "" ;
 var saveField=n("tblLanzhuang_lzr");
 saveField[0].value = ""; 
 m("tbl_tblLanzhuang_lzr").value = "" ;
 var saveField=n("Dept_DeptFullName");
 saveField[0].value = ""; 
 m("tbl_Dept_DeptFullName").value = "" ;
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=bdc2ccff_1408121019097665754&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=bdc2ccff_1408121019097665754&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=bdc2ccff_1408121019097665754&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblTousuReturn"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblTousuReturn&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblTousuReturn&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblTousuReturn&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblTousuReturn,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblTousuReturn&allTableName=tblTousuReturn,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblTousuReturn&allTableName=&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblTousuReturn&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblTousuReturn&allTableName=&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=bdc2ccff_1408121019097665754&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=bdc2ccff_1408121019097665754&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblTousuReturnDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=bdc2ccff_1408121019097665754&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=bdc2ccff_1408121019097665754&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblTousuReturn&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblTousuReturn&designId=4bee1a32_1408261827193320109&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblTousuReturn&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblTousuReturn&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblTousuReturn&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '投诉退机';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblTousuReturn&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
