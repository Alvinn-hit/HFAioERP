...
{ 
 cFireEvent('PDBuyCutPayDet_Qty',''); 
 cFireEvent('PDBuyCutPayDet_CurPrice',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDBuyCutPay&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDBuyCutPay&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDBuyCutPay&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDBuyCutPay&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function addSeq(o,keyCode){
 var ri=parseInt(o.parentNode.parentNode.rowIndex)-1; 
 var seqFname=o.name.substr(0,o.name.lastIndexOf("_"));
 var goodscode="";
 var goodscodestr="";
 var isInstore = false ;
 if(seqFname=="PDBuyCutPayDet_Seq"){
 var ss="@IOO:Outstore".split(":")
 if(ss[0]=="@GN"){
 goodscodestr=ss[4];
 goodscode=n(goodscodestr)[ri].value;
 }else if(ss[0]=="@IOO" && ss[1]=="Instore"){
 isInstore = true ;
 }
 var ss="@GN:Seq:Qty:tblBuyOutStockDet_GoodsSeqSet:tblBuyOutStockDet_GoodsCode:StockCode".split(":")
 if(ss[0]=="@GN"){
 goodscodestr=ss[4];
 goodscode=n(goodscodestr)[ri].value;
 }else if(ss[0]=="@IOO" && ss[1]=="Instore"){
 ...
 if(isInstore && goodscode.length==0){
 alert("请先选择商品.");
 return false;
 }
if(isInstore){
 var seqStr= n(seqFname)[ri].value
 var seqList= seqStr.split("~")
 if(o.value.length>0){
 var urlsgood="/UtilServlet?operation=validateGoodsSeq&validateField="+goodscodestr+"&"+goodscodestr+"="+goodscode;
 AjaxRequest(urlsgood) ;
 if("false"==response){
 alert("该商品未启用序列号123！！") ;
 n(seqFname+"_hid")[ri].value="" ;
 return ;
 }
 var curSeq=o.value;
 if(!isEnNumLine(curSeq)){
 alert("序列号"+"必须由数字，字母，横线或者下划线组成。") ;
 o.value = "" ;
 return ;
 }
 var fields=n(seqFname);
 for(var k=0;k<fields.length;k++){
 if(k!=ri&&fields[k].value!=""){
 var tempArr=fields[k].value.split("~");
 for(var i=0;i<tempArr.length-1;i++){
 if(tempArr[i].toLowerCase()==curSeq.toLowerCase()){
 alert('【系统提示】\n\n您录入的序列号'+'['+o.value+']'+'与第'+'['+(k+1)+']'+'行商品已录入的序列号有重复,请确认后重试.');
 return false;
 }
 }
 }
 ...
 var ts=seqFname ;
 var urls="/UtilServlet?operation=validateSeq&tableName="+seqFname.substr(0,seqFname.lastIndexOf("_"))+"&validateField="+seqFname.substr(seqFname.lastIndexOf("_")+1,seqFname.length-1)+"&goodsCode="+goodscode+"&oldSeq="+seqStr+"&defSeqStr=&seqStr="+seqStr+o.value+"~"+"&lastSeq=";
 AjaxRequest(urls) ;
 ...
 n(seqFname)[ri].value=seqStr;
 if(seqFname=="PDBuyCutPayDet_Seq"){
 var qtys=n('PDBuyCutPayDet'+"_"+'Qty');
 qtys[ri].value=seqList.length;
 $(qtys[ri]).change();
 }
  }
 }
 ...
 var inputValue=getInputVal(seqFname);
 if(popCondInput(inputValue,o)){
 if(goodscode.trim().length==0&&o.value.trim().length==0){
 var out=false;
 var ss="@IOO:Outstore".split(":")
 if(ss[0]=="@IOO"&&ss[1]=="Outstore"){
 out=true;
 }
 var ss="@GN:Seq:Qty:tblBuyOutStockDet_GoodsSeqSet:tblBuyOutStockDet_GoodsCode:StockCode".split(":")
 if(ss[0]=="@IOO"&&ss[1]=="Outstore"){
 out=true;
  ...
  if(out){
 var str=getStr(inputValue,ri);
 openChildSelect("/UserFunctionAction.do?operation=22&&src=menu&tableName="+seqFname.substring(0,seqFname.indexOf("_"))+"&fieldName="+seqFname.substring(seqFname.indexOf("_")+1)+str,o,seqFname,o.name);
 }
 }else{
 ...
 var str=getStr(inputValue,ri);
 var urls="/UtilServlet?operation=validateDetailSeq&MOID="+MOID+"&MOOP=add&tableName="+seqFname.substr(0,seqFname.lastIndexOf("_"))+"&"+seqFname+"="+o.value+"&validateField="+seqFname.substr(seqFname.lastIndexOf("_")+1,seqFname.length-1)+str;
 AjaxRequest(urls) ;
 if(response == "hasMore"){ 
 var str=getStr(inputValue,ri);
 openChildSelect("/UserFunctionAction.do?operation=22&src=menu&keySearch="+o.value+"&tableName="+seqFname.substring(0,seqFname.indexOf("_"))+"&fieldName="+seqFname.substring(seqFname.indexOf("_")+1)+str,o,seqFname,o.name);
 
 }else if(response!=""){ 
 var retAndSav=response.split("@@");
 returnSeq(retAndSav[2],retAndSav[1],retAndSav[0],goodscodestr,ri,seqFname,o.value);
 }else{
 alert('【系统提示】\n\n1、录入的序列号在所选仓库或订单商品中不存在。');
 n(seqFname+"_hid")[ri].select() ;
 }
 }
 }
 }
...
function returnSeq(colNames,returnVales,compareVale,goodscodestr,ri,seqFname,inpVal){
 var seqList2=n(seqFname);
 for(var k=0;k<seqList2.length;k++){
 if(seqList2[k].value!=""){
 var tempArr=seqList2[k].value.split("~");
 for(var i=0;i<tempArr.length-1;i++){
 if(tempArr[i].toLowerCase()==inpVal.toLowerCase()){
 alert('【系统提示】\n\n您录入的序列号'+'['+inpVal+']'+'与第'+'['+(k+1)+']'+'行商品已录入的序列号有重复,请确认后重试.');
 return false;
 }
 } 
 }
 ...
 var goodsProps = "" ;
 for(m=0;m<n(goodscodestr).length;m++){ 
 var saveField=n("PDBuyCutPayDet_tblGoods_GoodsNumber")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("PDBuyCutPayDet_tblGoods_GoodsFullName")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("PDBuyCutPayDet_tblGoods_GoodsSpec")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("PDBuyCutPayDet_tblGoods_BaseUnit")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("PDBuyCutPayDet_tblGoods_BarCode")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("PDBuyCutPayDet_BatchNo")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("PDBuyCutPayDet_yearNO")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=n("PDBuyCutPayDet_ProDate")[m].value; 
 goodsProps += saveField+"#;#" 
 var saveField=inpVal; 
 goodsProps += saveField+"#;#" 
 ...
 var props = goodsProps.split("@@") ;
 for(var m=0;m<props.length;m++){
 var nob =n(curGridRowNum.curKey+"_PresentSampleType");
 if(props[m]!="" && (document.getElementsByName(curGridRowNum.curKey+"_PresentSampleType").length==0 || nob[m].value == '' )){ 
 if(props[m]!="" && props[m].indexOf(compareVale)!=-1){
 numN = m ;
 break ;
 }
 }
 ...
 var mcurLine = 0;
 for(var m=0;m<n(goodscodestr).length;m++){ 
 mcurLine = m;
 if(n(goodscodestr)[m].value == ""){
 break;
 }
 }
 n(seqFname+"_hid")[ri].value = '' ; 
 
 n(seqFname+"_hid")[mcurLine].focus(); 
 ri = curGridRowNum.curLine;
 
 dataBackIn("falsle",colNames,returnVales,window);

 n(seqFname+"_hid")[ri].value = n(seqFname)[ri].value ;
 n(seqFname)[ri].value=n(seqFname)[ri].value+"~";
 if("PDBuyCutPayDet"==seqFname.substr(0,seqFname.lastIndexOf("_"))){ 
 var qtys=n("PDBuyCutPayDet"+"_"+'Qty');
 qtys[ri].value="1";
 qtys[ri].readOnly="readOnly";
 $(qtys[ri]).change();
   }
 
 n(seqFname+"_hid")[ri].select() ;
 $(n(seqFname+"_hid")[ri]).focus();
 ...
 qtys[numN].value=n(seqFname)[numN].value.split("~").length;
 }
 $(qtys[numN]).change();
 
 var rseq = inpVal.trim();
 var rasa = colNames.split(";");
 var rava = returnVales.split("#;#");
 ...
 n(seqFname)[numN].value +=rseq+"~"; 
 if(numN==ri){
 n(seqFname+"_hid")[ri].value=rseq ;
 n(seqFname+"_hid")[ri].select() ;
 }else{
 n(seqFname+"_hid")[ri].value="" ;
 n(seqFname+"_hid")[numN].value=rseq ;
 n(seqFname+"_hid")[numN].focus() ;
 } 
   }
 }
}
...
function getInputVal(seqFname) {
 var tableName=seqFname.substring(0,seqFname.indexOf("_"));
 var seqFieldName=seqFname.substring(seqFname.indexOf("_")+1);
 var param4="";
 var dd="";
 if('PDBuyCutPayDet'==tableName){
 dd=dd+"StockCode";
 dd=dd+"@TABLENAME_Seq_hid";
 param4=param4+":$globals.getFieldBean($rowlist.tableName,$mainField.substring($index)).isNullSeq_hid@PDBuyCutPayDet_Seq_hid";
 }
 var inputValue=tableName+":"+seqFname.substring(seqFname.indexOf("_")+1)+":aa"+param4;
 return inputValue;
}
function getStr(inputValue,ri){
 var tt =inputValue.split(":"); 
 var str="";
 for(var i=3;i<tt.length;i++){ 
 tt[i]=tt[i].substr(tt[i].indexOf("@")+1); 
 if(tt[i].indexOf(tt[0])==0){
 str=str+"&"+tt[i]+"="+n(tt[i])[ri].value;
 }else{
 str=str+"&"+tt[i]+"="+n(tt[i])[0].value;
 }
 }
 return str;
}
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDBuyCutPayDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 ...
 var z=n('PDBuyCutPayDet_GoodsCode');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDBuyCutPayDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDBuyCutPayDet"){
 
 copyFields=n('PDBuyCutPayDet_StockCode');
 ...
 var PDBuyCutPayDet_tblStock_StockNumber1 = n('PDBuyCutPayDet_tblStock_StockNumber1');
 if(typeof(PDBuyCutPayDet_tblStock_StockNumber1) != 'undefined' && PDBuyCutPayDet_tblStock_StockNumber1.length >index){
 PDBuyCutPayDet_tblStock_StockNumber1[index].value = PDBuyCutPayDet_tblStock_StockNumber1[index-1].value;
 }
 var PDBuyCutPayDet_tblStock_StockName1 = n('PDBuyCutPayDet_tblStock_StockName1');
 if(typeof(PDBuyCutPayDet_tblStock_StockName1) != 'undefined' && PDBuyCutPayDet_tblStock_StockName1.length >index){
 PDBuyCutPayDet_tblStock_StockName1[index].value = PDBuyCutPayDet_tblStock_StockName1[index-1].value;
 }
 }
 copyFields=n('PDBuyCutPayDet_GoodsCode');
 if((order > 2 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var PDBuyCutPayDet_tblGoods_GoodsNumber = n('PDBuyCutPayDet_tblGoods_GoodsNumber');
 if(typeof(PDBuyCutPayDet_tblGoods_GoodsNumber) != 'undefined' && PDBuyCutPayDet_tblGoods_GoodsNumber.length >index){
 PDBuyCutPayDet_tblGoods_GoodsNumber[index].value = PDBuyCutPayDet_tblGoods_GoodsNumber[index-1].value;
 }
 var PDBuyCutPayDet_tblGoods_GoodsFullName = n('PDBuyCutPayDet_tblGoods_GoodsFullName');
 if(typeof(PDBuyCutPayDet_tblGoods_GoodsFullName) != 'undefined' && PDBuyCutPayDet_tblGoods_GoodsFullName.length >index){
 PDBuyCutPayDet_tblGoods_GoodsFullName[index].value = PDBuyCutPayDet_tblGoods_GoodsFullName[index-1].value;
 }
 var PDBuyCutPayDet_tblGoods_GoodsSpec = n('PDBuyCutPayDet_tblGoods_GoodsSpec');
 if(typeof(PDBuyCutPayDet_tblGoods_GoodsSpec) != 'undefined' && PDBuyCutPayDet_tblGoods_GoodsSpec.length >index){
 PDBuyCutPayDet_tblGoods_GoodsSpec[index].value = PDBuyCutPayDet_tblGoods_GoodsSpec[index-1].value;
 }
 var PDBuyCutPayDet_tblGoods_BaseUnit = n('PDBuyCutPayDet_tblGoods_BaseUnit');
 if(typeof(PDBuyCutPayDet_tblGoods_BaseUnit) != 'undefined' && PDBuyCutPayDet_tblGoods_BaseUnit.length >index){
 PDBuyCutPayDet_tblGoods_BaseUnit[index].value = PDBuyCutPayDet_tblGoods_BaseUnit[index-1].value;
 }
 var PDBuyCutPayDet_tblGoods_BarCode = n('PDBuyCutPayDet_tblGoods_BarCode');
 if(typeof(PDBuyCutPayDet_tblGoods_BarCode) != 'undefined' && PDBuyCutPayDet_tblGoods_BarCode.length >index){
 PDBuyCutPayDet_tblGoods_BarCode[index].value = PDBuyCutPayDet_tblGoods_BarCode[index-1].value;
 }
 }
 copyFields=n('PDBuyCutPayDet_BuyInStockID');
 if((order > 3 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var PDBuyCutPayDet_tblBuyInStock_BillDate = n('PDBuyCutPayDet_tblBuyInStock_BillDate');
 if(typeof(PDBuyCutPayDet_tblBuyInStock_BillDate) != 'undefined' && PDBuyCutPayDet_tblBuyInStock_BillDate.length >index){
 PDBuyCutPayDet_tblBuyInStock_BillDate[index].value = PDBuyCutPayDet_tblBuyInStock_BillDate[index-1].value;
 }
 var PDBuyCutPayDet_tblBuyInStock_BillNo = n('PDBuyCutPayDet_tblBuyInStock_BillNo');
 if(typeof(PDBuyCutPayDet_tblBuyInStock_BillNo) != 'undefined' && PDBuyCutPayDet_tblBuyInStock_BillNo.length >index){
 PDBuyCutPayDet_tblBuyInStock_BillNo[index].value = PDBuyCutPayDet_tblBuyInStock_BillNo[index-1].value;
 }
 }
 copyFields=n('PDBuyCutPayDet_BuyOrderID');
 if((order > 4 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
   }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDBuyCutPayDet"){
 ...
 copyFields=n('PDBuyCutPayDet_GoodsCode'); 
 if( (order > 2 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var PDBuyCutPayDet_tblGoods_GoodsNumber = n('PDBuyCutPayDet_tblGoods_GoodsNumber'); 
 if(typeof(PDBuyCutPayDet_tblGoods_GoodsNumber) != 'undefined' && PDBuyCutPayDet_tblGoods_GoodsNumber.length >index){
 PDBuyCutPayDet_tblGoods_GoodsNumber[index].value = PDBuyCutPayDet_tblGoods_GoodsNumber[index-1].value;
 ...
 copyFields=n('PDBuyCutPayDet_BuyOrderID'); 
 if( (order > 4 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 }
   }
 }
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="PDBuyCutPayDet"; 
 var goodsCodeLogicValidate_PDBuyCutPayDet='@GN:Seq:Qty:tblBuyOutStockDet_GoodsSeqSet:tblBuyOutStockDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'PDBuyCutPayDet',value:goodsCodeLogicValidate_PDBuyCutPayDet};
...
function setSeqQtyReadOnly(tableN,row,checked){
 ...
 logVal = "";
 for(var i = 0; i< goodsCodeLogicValidate.length;i++){ 
 if(goodsCodeLogicValidate[i].key==tableN){
 logVal = goodsCodeLogicValidate[i].value;
 }
 }
 if(logVal != ""){
 var seq_qtyName = logVal.split(":")[2];
 var seq_qty = document.getElementsByName(tableN+"_"+seq_qtyName);
 if(checked){ 
 fn = tableN+"_GoodsCode";
 var urls="/UtilServlet?operation=validateGoodsSeq&validateField="+fn+"&"+fn+"="+n(fn)[row].value;
 AjaxRequest(urls) ;
 var seq=n(tableN+'_'+"Seq_hid");
 var dd=n(tableN+'_Seq');
 ...
 seq_qty[row].readOnly="";
 $(seq_qty[row]).change();
 }
 }else{ 
 if(seq_qty.length>0){
 seq_qty[row].readOnly="readOnly"; 
 } 
 } 
 } 
 }else{
 var obj =document.getElementsByName(tableN+"_Seq");
...
function execStat2(){
 execStatOne('PDBuyCutPayDet_Qty',4);
 execStatOne('PDBuyCutPayDet_CurAmount',2);
 execStatOne('PDBuyCutPayDet_Amount',2);
 execStatOne('PDBuyCutPayDet_TaxAmount',2);
 execStatOne('PDBuyCutPayDet_CoTaxAmt',2);
 execStatOne('PDBuyCutPayDet_ApplyQty',4);
 setTotalCount(getTotalCount());
}
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 if(index>-1){
 var ri=parseInt(current.parentNode.parentNode.rowIndex)-1;
 urlstr=urlstr.substring(0,index+"&seq=".length)+n(thisField.substr(0,thisField.lastIndexOf("_")))[ri].value+"&src=menu&seqFname="+thisField+"&newSeq="+mi[ri].value+"&tableName=PDBuyCutPay&childTableName="+thisField.split("_")[0]+"&fieldName="+field;
 var goodscode="";
 var goodscodestr="";
 var goodsProps = "";
 var stockCode = "" ;
 ...
 if(thisField=="PDBuyCutPayDet_Seq_hid"){
 ...
 if(typeof(ss[5])!="undefined"){
 if(typeof(m(ss[5]))!="undefined" && m(ss[5])!=null && m(ss[5]).value!=""){
 stockCode = m(ss[5]).value ;
 }else{
 stockCode = nz("PDBuyCutPayDet"+"_"+ss[5],ri) ;
 }
 }
 var ss="@GN:Seq:Qty:tblBuyOutStockDet_GoodsSeqSet:tblBuyOutStockDet_GoodsCode:StockCode".split(":")
 if(ss[0]=="@GN"){
 ...
 if(stockCode==""){
 asyncbox.tips("仓库不能为空","alert",2000) ;return false ;
 }
 if(goodscode==""){
 asyncbox.tips("请先选择商品.","alert",2000);return false;
 }else{
 urlstr=urlstr+"&goodsCode="+goodscode+"&"+goodsProps+"&stockCode="+stockCode;
 }
 var urlsgood="/UtilServlet?operation=validateGoodsSeq&validateField="+goodscodestr+"&"+goodscodestr+"="+goodscode;
 AjaxRequest(urlsgood) ;
 if("false"==response){
 asyncbox.tips("该商品未启用序列号345！！","alert",2000) ;
 return ;
 }
 }
 ...
 if(urlstr.indexOf("&seq=")>-1){
 asyncbox.open({id:'ChildPopup',title:'弹出窗口',url:urlstr,width:900,height:400}); 
 }else{
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDBuyCutPay\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDBuyCutPay\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'PDBuyCutPayDet_Qty',"PDBuyCutPayDet"); 
 delFireEventOne(rowId,tableName,'PDBuyCutPayDet_CurPrice',"PDBuyCutPayDet"); 
 delFireEventOne(rowId,tableName,'PDBuyCutPayDet_CurAmount',"PDBuyCutPayDet"); 
 delFireEventOne(rowId,tableName,'PDBuyCutPayDet_Amount',"PDBuyCutPayDet"); 
 delFireEventOne(rowId,tableName,'PDBuyCutPayDet_TaxPrice',"PDBuyCutPayDet"); 
 delFireEventOne(rowId,tableName,'PDBuyCutPayDet_TaxAmount',"PDBuyCutPayDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'Tax',dis:'税率%',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CurrencyRate',dis:'汇率',digit:6,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalAmount',dis:'本币未税扣款总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalTaxAmount',dis:'本币含税扣款总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalCoTaxAmt',dis:'本币扣款总税额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'CurTotalAmount',dis:'原币扣款总额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'PDBuyCutPayDet_Qty',dis:'数量',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDBuyCutPayDet_CurPrice',dis:'原币扣款单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDBuyCutPayDet_CurAmount',dis:'原币扣款金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDBuyCutPayDet_Amount',dis:'本币未税扣款金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDBuyCutPayDet_TaxPrice',dis:'本币含税扣款单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDBuyCutPayDet_TaxAmount',dis:'本币含税扣款金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDBuyCutPayDet_CoTaxAmt',dis:'本币扣款税额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDBuyCutPay",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDBuyCutPay&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDBuyCutPay&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 var saveField=n("InVoiceType"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_InVoiceType");
 if(obj != null){
 obj.value = "";
 }
 ...
 var saveField=n("Tax"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_taxRate");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("Currency"); 
 saveField[0].value = "";
 var obj = m("tbl_tblCompany_Currency");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("CurrencyRate"); 
 saveField[0].value = "";
 var obj = m("tbl_tblSetExchange_RecordExchange");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblSetExchange_CurrencyName"); 
 saveField[0].value = "";
 var obj = m("tbl_tblSetExchange_CurrencyName");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblCompany_ComNumber");
 saveField[0].value = ""; 
 m("tbl_tblCompany_ComNumber").value = "" ;
 var saveField=n("tblCompany_ComName");
 saveField[0].value = ""; 
 m("tbl_tblCompany_ComName").value = "" ;
 }
  }
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('PDBuyCutPayDet_Qty'); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDBuyCutPay"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDBuyCutPay&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDBuyCutPay&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 if(nn('InVoiceType',0)==2)n('Tax')[0].value=f("0",6);

if(nn('InVoiceType',0)==1)n('Tax')[0].value=f("0",6);

if(nn('InVoiceType',0)==3)n('Tax')[0].value=f(0,6);

for(var j=0;j<r(0,'TaxPrice').length;j++){
r(0,"TaxPrice")[j].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",j)*rn(0,"Price",j)/100,6);
}
...
r(0,"CurPrice")[j].value=f((((nn('Tax',0)/100)+1)*rn(0,"Discount",j)*rn(0,"Price",j)/100)/nn('CurrencyRate',0),6);
}
...
n('TotalAmount')[0].value=f(sm('PDBuyCutPayDet_DisBackAmt'),2);

n('TotalTaxAmount')[0].value=f(sm('PDBuyCutPayDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('PDBuyCutPayDet_CoTaxAmt'),2);

for(var j=0;j<r(0,'DisAmount').length;j++){
r(0,"DisAmount")[j].value=f(rn(0,"Amount",j)-rn(0,"DisBackAmt",j),2);
}

n('DisAmount')[0].value=f(sm('PDBuyCutPayDet_DisAmount'),2);
...
r(0,"Discount")[j].value=f(nn('Discount',0),6);
}

for(var j=0;j<r(0,'DisPrice').length;j++){
r(0,"DisPrice")[j].value=f(rn(0,"Discount",j)*rn(0,"Price",j)/100,6);
}
...
n('DisAmount')[0].value=f(sm('PDBuyCutPayDet_DisAmount'),2);
 ...
 z[i].onchange = function(){ 
 for(var j=0;j<r(0,'TaxPrice').length;j++){
r(0,"TaxPrice")[j].value=f(rn(0,"CurPrice",j)*nn('CurrencyRate',0),6);
}

for(var j=0;j<r(0,'DisPrice').length;j++){
r(0,"DisPrice")[j].value=f(rn(0,"CurPrice",j)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),6);
}

for(var j=0;j<r(0,'Price').length;j++){
r(0,"Price")[j].value=f(rn(0,"CurPrice",j)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",j)/100),6);
}
...
n('TotalCoTaxAmt')[0].value=f(sm('PDBuyCutPayDet_CoTaxAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDBuyCutPayDet_CurPrice'); 
 r(0,"TaxPrice")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0),6);
 r(0,"DisPrice")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),6);
r(0,"Price")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),6);
r(0,"CurAmount")[p].value=f(rn(0,"CurPrice",p)*rn(0,"Qty",p),2);
...
n('TotalAmount')[0].value=f(sm('PDBuyCutPayDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('PDBuyCutPayDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(sm('PDBuyCutPayDet_CurAmount'),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
n('TotalCoTaxAmt')[0].value=f(sm('PDBuyCutPayDet_CoTaxAmt'),2);
n('DisAmount')[0].value=f(sm('PDBuyCutPayDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDBuyCutPayDet_CurAmount'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),2);
r(0,"CurPrice")[p].value=f(rn(0,"CurAmount",p)/rn(0,"Qty",p),6);
r(0,"TaxPrice")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0),6);
r(0,"DisPrice")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),6);
r(0,"Price")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),6);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
...
n('DisAmount')[0].value=f(sm('PDBuyCutPayDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDBuyCutPayDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"CurPrice")[p].value=f((((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100)/nn('CurrencyRate',0),6);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('PDBuyCutPayDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDBuyCutPayDet_Amount'); 
 r(0,"Price")[p].value=f(rn(0,"Amount",p)/rn(0,"Qty",p),6);
r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*(rn(0,"Discount",p)*rn(0,"Price",p)/100),6);
r(0,"CurPrice")[p].value=f((((nn('Tax',0)/100)+1)*(rn(0,"Discount",p)*rn(0,"Price",p)/100))/nn('CurrencyRate',0),6);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('PDBuyCutPayDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDBuyCutPayDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,6);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('PDBuyCutPayDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDBuyCutPayDet_DisPrice'); 
 r(0,"Discount")[p].value=f(rn(0,"DisPrice",p)/rn(0,"Price",p)*100,6);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"DisPrice",p),6);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
n('TotalAmount')[0].value=f(sm('PDBuyCutPayDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('PDBuyCutPayDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(nn('TotalTaxAmount',0)/nn('CurrencyRate',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('PDBuyCutPayDet_CoTaxAmt'),2);
r(0,"DisAmount")[p].value=f(rn(0,"Amount",p)-rn(0,"DisBackAmt",p),2);
r(0,"CurPrice")[p].value=f((((nn('Tax',0)/100)+1)*rn(0,"DisPrice",p))/nn('CurrencyRate',0),6);
r(0,"CurAmount")[p].value=f((((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100)/nn('CurrencyRate',0),2);
n('DisAmount')[0].value=f(sm('PDBuyCutPayDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDBuyCutPayDet_DisBackAmt'); 
 r(0,"DisPrice")[p].value=f(rn(0,"DisBackAmt",p)/rn(0,"Qty",p),6);
r(0,"Discount")[p].value=f(rn(0,"DisPrice",p)/rn(0,"Price",p)*100,6);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"DisPrice",p),6);
...
n('DisAmount')[0].value=f(sm('PDBuyCutPayDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDBuyCutPayDet_TaxPrice'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"Qty",p)*rn(0,"TaxPrice",p),2);
r(0,"DisPrice")[p].value=f(rn(0,"TaxPrice",p)/((nn('Tax',0)/100)+1),6);
r(0,"Price")[p].value=f((rn(0,"TaxPrice",p)/((nn('Tax',0)/100)+1)*100)/rn(0,"Discount",p),6);
r(0,"CurPrice")[p].value=f(rn(0,"TaxPrice",p)/nn('CurrencyRate',0),6);
...
n('DisAmount')[0].value=f(sm('PDBuyCutPayDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDBuyCutPayDet_TaxAmount'); 
 r(0,"TaxPrice")[p].value=f(rn(0,"TaxAmount",p)/rn(0,"Qty",p),6);
r(0,"DisPrice")[p].value=f(rn(0,"TaxPrice",p)/((nn('Tax',0)/100)+1),6);
r(0,"Price")[p].value=f((rn(0,"TaxPrice",p)/((nn('Tax',0)/100)+1))*100/rn(0,"Discount",p),6);
r(0,"CurPrice")[p].value=f((rn(0,"TaxAmount",p)/rn(0,"Qty",p))/nn('CurrencyRate',0),6);
...
n('DisAmount')[0].value=f(sm('PDBuyCutPayDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDBuyCutPayDet_SecQty'); 
...
childData[childCount]='PDBuyCutPayDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='采购品质异常明细';
childCount++;
 var PDBuyCutPayDetData = new gridData();
 gridDatas[gridDataNum] = PDBuyCutPayDetData;
 gridDataNum++;
 var PDBuyCutPayDet=1;
 ...
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_StockCode','仓库','80',2,false,'Sess_StockCode',100,-2,'','',"","copy","0",'',"2",true);
 putChildValidateItem("PDBuyCutPayDet_StockCode","仓库","any","",false,minValue,100,true,'Sess_StockCode');
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_tblStock_StockNumber1','仓库编号','120',0,true,'Sess_StockFullName',1000000,2,'PDBuyCutPayDet:StockCode:PDBuyCutPayDet_tblStock_StockNumber1:'+param4,'',"","copy","0",'',"2",false); 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_tblStock_StockName1','仓库简称','100',0,true,'',1000000,2,'PDBuyCutPayDet:StockCode:PDBuyCutPayDet_tblStock_StockName1:'+param4,'',"","copy","0",'',"2",false); 
 ...
 var param4="";
 param4=param4+":Seq_hid@PDBuyCutPayDet_Seq_hid";
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_Seq','物流码','130',2,true,'',5000,-2,'','',"","triggerCopy","1",'',"2",false);
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_Seq'+'_hid','物流码','130',2,true,'',5000,2,'@SEQ@IOO:Outstore',"","","triggerCopy","1",'',"2",false);
 putChildValidateItem("PDBuyCutPayDet_Seq","物流码","any","GoodsField",true,minValue,5000,false,'',true);
 ...
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_GoodsCode','料件名称','120',2,false,'',50,-2,'','',"","triggerOrCopy","2",'',"2",true);
 putChildValidateItem("PDBuyCutPayDet_GoodsCode","料件名称","any","",false,minValue,50,false,'');
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'PDBuyCutPayDet:GoodsCode:PDBuyCutPayDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","2",'',"2",false); 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_tblGoods_GoodsFullName','料件名称','150',2,true,'',1000000,2,'PDBuyCutPayDet:GoodsCode:PDBuyCutPayDet_tblGoods_GoodsFullName:'+param4,'',"","triggerOrCopy","2",'',"2",false); 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDBuyCutPayDet:GoodsCode:PDBuyCutPayDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","2",'',"2",false); 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDBuyCutPayDet:GoodsCode:PDBuyCutPayDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","2",'',"2",false); 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_tblGoods_BarCode','条码','100',2,true,'',1000000,2,'PDBuyCutPayDet:GoodsCode:PDBuyCutPayDet_tblGoods_BarCode:'+param4,'',"","triggerOrCopy","2",'',"2",false); 
 ...
 param4=param4+":1料件名称@PDBuyCutPayDet_GoodsCode";
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_BuyInStockID','入库单号','80',2,false,'',32,-2,'','',"","copy","3",'',"2",true);
 putChildValidateItem("PDBuyCutPayDet_BuyInStockID","入库单号","any","",false,minValue,32,false,'');
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_tblBuyInStock_BillDate','入库日期','100',2,true,'',1000000,2,'PDBuyCutPayDet:BuyInStockID:PDBuyCutPayDet_tblBuyInStock_BillDate:'+param4,'',"","copy","3",'',"2",false); 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_tblBuyInStock_BillNo','入库单编号','120',2,true,'',1000000,2,'PDBuyCutPayDet:BuyInStockID:PDBuyCutPayDet_tblBuyInStock_BillNo:'+param4,'',"","copy","3",'',"2",false); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_BuyOrderID','订单编号','80',2,true,'',32,-2,'','',"","copy","4",'',"2",false); 
 putChildValidateItem("PDBuyCutPayDet_BuyOrderID","订单编号","any","",true,minValue,32,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","5",'',"3",false); 
 putChildValidateItem("PDBuyCutPayDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","6",'',"2",false); 
 putChildValidateItem("PDBuyCutPayDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"2",false); 
 putChildValidateItem("PDBuyCutPayDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_yearNO','箱号','100',2,true,'',100,-2,'','',"","triggerCopy","8",'',"3",false); 
 putChildValidateItem("PDBuyCutPayDet_yearNO","箱号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","9",'',"2",false); 
 putChildValidateItem("PDBuyCutPayDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","10",'',"2",false); 
 putChildValidateItem("PDBuyCutPayDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_color','色彩','60',2,true,'',100,-2,'','',"","","11",'',"2",false); 
 putChildValidateItem("PDBuyCutPayDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_Qty','数量','60',1,false,'',0,0,'','',"Qty","","12",'',"0",false); 
 putChildValidateItem("PDBuyCutPayDet_Qty","数量","double","SeqQty",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_CurPrice','原币扣款单价','100',1,true,'',0,0,'','',"priceIdentifier","","13",'',"0",false); 
 putChildValidateItem("PDBuyCutPayDet_CurPrice","原币扣款单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_PresentSampleType','类型','80',2,true,'0',10,-2,'','',"","","14",'',"1",false); 
 putChildValidateItem("PDBuyCutPayDet_PresentSampleType","类型","any","",true,minValue,10,true,'0'); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_Remark','备注','100',2,true,'',8000,0,'','',"","","15",'',"0",false); 
 putChildValidateItem("PDBuyCutPayDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_CurAmount','原币扣款金额','100',1,true,'',0,8,'','',"AmountIdentifier","","16",'',"8",false); 
 putChildValidateItem("PDBuyCutPayDet_CurAmount","原币扣款金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_Price','本币未税单价','80',1,true,'',0,-2,'','',"priceIdentifier","","17",'',"0",false); 
 putChildValidateItem("PDBuyCutPayDet_Price","本币未税单价","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_Amount','本币未税扣款金额','100',1,true,'',0,8,'','',"AmountIdentifier","","18",'',"0",false); 
 putChildValidateItem("PDBuyCutPayDet_Amount","本币未税扣款金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_Discount','折扣率%','100',1,true,'100',0,-2,'','',"Price","","24",'',"0",false); 
 putChildValidateItem("PDBuyCutPayDet_Discount","折扣率%","double","BillDiscount",true,minValue,maxValue,true,'100'); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_DisPrice','折后单价','100',1,true,'',0,-2,'','',"priceIdentifier","","25",'',"0",false); 
 putChildValidateItem("PDBuyCutPayDet_DisPrice","折后单价","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_DisBackAmt','折后金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","26",'',"0",false); 
 putChildValidateItem("PDBuyCutPayDet_DisBackAmt","折后金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_DisAmount','折让金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","27",'',"8",false); 
 putChildValidateItem("PDBuyCutPayDet_DisAmount","折让金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_TaxPrice','本币含税扣款单价','100',1,true,'',0,8,'','',"priceIdentifier","","28",'',"8",false); 
 putChildValidateItem("PDBuyCutPayDet_TaxPrice","本币含税扣款单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_TaxAmount','本币含税扣款金额','100',1,true,'',0,8,'','',"AmountIdentifier","","29",'',"8",false); 
 putChildValidateItem("PDBuyCutPayDet_TaxAmount","本币含税扣款金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_CoTaxAmt','本币扣款税额','100',1,true,'0',0,8,'','',"AmountIdentifier","","30",'',"8",false); 
 putChildValidateItem("PDBuyCutPayDet_CoTaxAmt","本币扣款税额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_ApplyQty','申请数','80',1,false,'0',0,-2,'','',"Qty","","32",'',"0",false); 
 putChildValidateItem("PDBuyCutPayDet_ApplyQty","申请数","double","",false,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_SourceID','源ID','0',2,true,'',60,-2,'','',"priceIdentifier","","33",'',"6",false); 
 putChildValidateItem("PDBuyCutPayDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 ...
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_ChangeAmt','调价金额','200',1,true,'0',0,-2,'','',"","","37",'',"3",false); 
 putChildValidateItem("PDBuyCutPayDet_ChangeAmt","调价金额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_AfterChangeAmt','调价后金额','200',1,true,'0',0,-2,'','',"","","39",'',"3",false); 
 putChildValidateItem("PDBuyCutPayDet_AfterChangeAmt","调价后金额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_InvoiceAmount','已开票金额','200',1,true,'0',0,-2,'','',"AmountIdentifier","","41",'',"3",false); 
 putChildValidateItem("PDBuyCutPayDet_InvoiceAmount","已开票金额","double","",true,minValue,maxValue,true,'0'); 
 ...
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_NotPayQty','未付款数量','224',1,true,'',0,-2,'','',"Qty","","46",'',"3",false); 
 putChildValidateItem("PDBuyCutPayDet_NotPayQty","未付款数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_PayQty','付款数量','224',1,true,'',0,-2,'','',"Qty","","47",'',"3",false); 
 putChildValidateItem("PDBuyCutPayDet_PayQty","付款数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","48",'',"3",false); 
 putChildValidateItem("PDBuyCutPayDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","49",'',"3",false); 
 putChildValidateItem("PDBuyCutPayDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_hasCheck','是否对帐','200',0,true,'0',0,-2,'','',"","","51",'',"0",false); 
 putChildValidateItem("PDBuyCutPayDet_hasCheck","是否对帐","int","",true,minValue,maxValue,true,'0'); 
 
 
 addCols(PDBuyCutPayDetData,'PDBuyCutPayDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDBuyCutPay&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDBuyCutPay,PDBuyCutPayDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDBuyCutPay&allTableName=PDBuyCutPay,PDBuyCutPayDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDBuyCutPay&allTableName=PDBuyCutPayDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDBuyCutPay&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDBuyCutPay&allTableName=PDBuyCutPayDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDBuyCutPay&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDBuyCutPay&designId=d0b79ead_1603311030536572806&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDBuyCutPay&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDBuyCutPay&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDBuyCutPay&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '采购品质异常扣款';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDBuyCutPay&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
...
function batchInput(){
 ...
 constr +="<option value='Seq' >序列号</option>";
...
　　　 html:'<div style="text-align:center;"><div style="text-align: left;padding-left: 20px;"><p>'+
 ...
 '注意:批量录入序列号时不可以录入数量和单价。'+ '可从excel复制,分隔符不能是中文字符</p></div></div>', 
　　　 width:430,
　　　 height:390,
 　 title:'商品批量录入',
 　　　 btnsbar:jQuery.btn.OKCANCEL, 
