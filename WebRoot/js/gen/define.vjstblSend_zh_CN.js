...
{ 
 cFireEvent('tblSendDet_Qty','0'); 
 cFireEvent('tblSendDet_Price','0'); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblSend&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblSend&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblSend&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblSend&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblSendDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 ...
 var z=n('tblSendDet_BuyAppTotalID');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblSendDet"){
 
 copyFields=n('tblSendDet_StockCode');
 ...
 var tblSendDet_tblStock_StockFullName1 = n('tblSendDet_tblStock_StockFullName1');
 if(typeof(tblSendDet_tblStock_StockFullName1) != 'undefined' && tblSendDet_tblStock_StockFullName1.length >index){
 tblSendDet_tblStock_StockFullName1[index].value = tblSendDet_tblStock_StockFullName1[index-1].value;
 }
 }
 copyFields=n('tblSendDet_GoodsCode');
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblSendDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblSendDet"; 
 var goodsCodeLogicValidate_tblSendDet='@GN:Seq:Qty:tblSendDet_GoodsSeqSet:tblSendDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'tblSendDet',value:goodsCodeLogicValidate_tblSendDet};
...
function execStat2(){
 execStatOne('tblSendDet_Qty',2);
 execStatOne('tblSendDet_Amount',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=0af1b787_1302260903271470007&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=0af1b787_1302260903271470007&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblSend\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblSend\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblSendDet_Qty',"tblSendDet"); 
 delFireEventOne(rowId,tableName,'tblSendDet_Price',"tblSendDet"); 
 delFireEventOne(rowId,tableName,'tblSendDet_Amount',"tblSendDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'BillAmt',dis:'合计金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblSendDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSendDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblSendDet_Amount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblSend",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSend&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblSend&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"0af1b787_1302260903271470007",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0af1b787_1302260903271470007&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0af1b787_1302260903271470007&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=0af1b787_1302260903271470007&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblSend&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblSend&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblSendDet_Qty'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);

 n('BillAmt')[0].value=f(sm('tblSendDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSendDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblSendDet_Amount'); 
...
childData[childCount]='tblSendDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='配送单明细';
childCount++;
 var tblSendDetData = new gridData();
 gridDatas[gridDataNum] = tblSendDetData;
 gridDataNum++;
 var tblSendDet=1;
 ...
 addCols(tblSendDetData,'tblSendDet_Seq','序列号','130',2,true,'',5000,-2,'','',"","triggerCopy","0",'',"2",false); 
 putChildValidateItem("tblSendDet_Seq","序列号","any","GoodsField",true,minValue,5000,false,''); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_StockCode','仓库','80',2,true,'Sess_StockCode',100,-2,'','',"","copy","0",'',"3",false); 
 putChildValidateItem("tblSendDet_StockCode","仓库","any","",true,minValue,100,true,'Sess_StockCode'); 
 ...
 param4=param4+":0仓库@tblSendDet_StockCode";
 param4=param4+":1单据日期@BillDate"; 
 param4=param4+":tblSendDet_GoodsCode@tblSendDet_GoodsCode"; 
 
 addCols(tblSendDetData,'tblSendDet_BuyAppTotalID','申请汇总单号','120',2,false,'',100,-2,'','',"","triggerCopy","1",'',"2",true);
 putChildValidateItem("tblSendDet_BuyAppTotalID","申请汇总单号","any","",false,minValue,100,false,'');
 addCols(tblSendDetData,'tblSendDet_tblDepartment_DeptFullName','配送部门','75',2,true,'',1000000,2,'tblSendDet:BuyAppTotalID:tblSendDet_tblDepartment_DeptFullName:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendDetData,'tblSendDet_tblBuyApplicationTotal_BillNo','申请汇总单号','100',2,true,'',1000000,2,'tblSendDet:BuyAppTotalID:tblSendDet_tblBuyApplicationTotal_BillNo:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendDetData,'tblSendDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'tblSendDet:BuyAppTotalID:tblSendDet_tblGoods_GoodsNumber:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendDetData,'tblSendDet_tblGoods_GoodsFullName','商品名称','150',18,true,'',1000000,2,'tblSendDet:BuyAppTotalID:tblSendDet_tblGoods_GoodsFullName:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendDetData,'tblSendDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'tblSendDet:BuyAppTotalID:tblSendDet_tblGoods_GoodsSpec:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendDetData,'tblSendDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblSendDet:BuyAppTotalID:tblSendDet_tblGoods_BaseUnit:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendDetData,'tblSendDet_tblStocks_lastQty','实际库存','100',1,true,'',1000000,2,'tblSendDet:BuyAppTotalID:tblSendDet_tblStocks_lastQty:'+param4,'',"","triggerCopy","1",'',"2",false); 
 addCols(tblSendDetData,'tblSendDet_ViewVirtualStock_UseQty','虚拟库存','100',1,true,'',1000000,2,'tblSendDet:BuyAppTotalID:tblSendDet_ViewVirtualStock_UseQty:'+param4,'',"","triggerCopy","1",'',"2",false); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_GoodsCode','商品名称','120',2,false,'',100,-2,'','',"","triggerOrCopy","3",'',"2",false); 
 putChildValidateItem("tblSendDet_GoodsCode","商品名称","any","",false,minValue,100,false,''); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","5",'',"8",false); 
 putChildValidateItem("tblSendDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_color','色彩','60',2,true,'',100,-2,'','',"","","5",'',"3",false); 
 putChildValidateItem("tblSendDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_Inch','尺寸','70',2,true,'',100,-2,'','',"","triggerCopy","6",'',"8",false); 
 putChildValidateItem("tblSendDet_Inch","尺寸","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"8",false); 
 putChildValidateItem("tblSendDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_yearNO','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"8",false); 
 putChildValidateItem("tblSendDet_yearNO","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_ProDate','生产日期','80',2,true,'',10,-2,'','',"","","9",'',"8",false); 
 putChildValidateItem("tblSendDet_ProDate","生产日期","any","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_Availably','保质期限','80',2,true,'',10,-2,'','',"","","10",'',"8",false); 
 putChildValidateItem("tblSendDet_Availably","保质期限","any","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_Qty','数量','80',1,false,'0',0,0,'','',"Qty","","11",'',"0",false); 
 putChildValidateItem("tblSendDet_Qty","数量","double","SeqQty",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_Price','单价','80',1,true,'0',0,8,'','',"priceIdentifier","","12",'',"8",false); 
 putChildValidateItem("tblSendDet_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_Amount','金额','100',1,true,'',0,8,'','',"AmountIdentifier","","13",'',"8",false); 
 putChildValidateItem("tblSendDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 
 
 addCols(tblSendDetData,'tblSendDet_NotCheckQty','未验收数','80',1,true,'0',0,-2,'','',"","","15",'',"0",false); 
 putChildValidateItem("tblSendDet_NotCheckQty","未验收数","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_CheckQty','已验收数量','80',1,true,'0',0,-2,'','',"","","15",'',"0",false); 
 putChildValidateItem("tblSendDet_CheckQty","已验收数量","double","",true,minValue,maxValue,true,'0'); 
 ...
 addCols(tblSendDetData,'tblSendDet_Remark','备注','120',2,true,'',8000,0,'','',"","","21",'',"0",false); 
 putChildValidateItem("tblSendDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 
 
 addCols(tblSendDetData,'tblSendDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblSendDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(tblSendDetData,'tblSendDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblSendDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","103",'',"3",false); 
 putChildValidateItem("tblSendDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 addCols(tblSendDetData,'tblSendDet_DepartmentCode','配送部门','224',2,true,'',100,-2,'','',"","","103",'',"3",false); 
 putChildValidateItem("tblSendDet_DepartmentCode","配送部门","any","",true,minValue,100,false,''); 
 
 
 addCols(tblSendDetData,'tblSendDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblSend&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblSend,tblSendDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSend&allTableName=tblSend,tblSendDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSend&allTableName=tblSendDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblSend&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblSend&allTableName=tblSendDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0af1b787_1302260903271470007&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=0af1b787_1302260903271470007&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=0af1b787_1302260903271470007&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=0af1b787_1302260903271470007&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblSend&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblSend&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblSend&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblSend&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblSend&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '配送单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblSend&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
