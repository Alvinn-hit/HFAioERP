...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblAccAdjust&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblAccAdjust&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblAccAdjust&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblAccAdjust&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblAccAdjustDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblAccAdjustDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblAccAdjustDet"){
...
function execStat2(){
 execStatOne('tblAccAdjustDet_BillAmt',2);
 execStatOne('tblAccAdjustDet_SettledAmt',2);
 execStatOne('tblAccAdjustDet_BackAmt',2);
 execStatOne('tblAccAdjustDet_WexeBalAmt',2);
 execStatOne('tblAccAdjustDet_Amount',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblAccAdjust\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblAccAdjust\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblAccAdjustDet_Amount',"tblAccAdjustDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'TotalAmount',dis:'调账金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblAccAdjustDet_BillAmt',dis:'单据金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccAdjustDet_SettledAmt',dis:'已结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccAdjustDet_BackAmt',dis:'退货金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccAdjustDet_WexeBalAmt',dis:'待结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAccAdjustDet_Amount',dis:'本次调账金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblAccAdjust",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAccAdjust&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAccAdjust&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblAccAdjust"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblAccAdjust&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblAccAdjust&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
r(0,"CurAmount")[j].value=f(rn(0,"Amount",j)/nn('CurrencyRate',0),2);
}

n('TotalAmount')[0].value=f(sm('tblAccAdjustDet_Amount'),2);

n('CurTotalAmount')[0].value=f(sm('tblAccAdjustDet_CurAmount'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('CurTotalAmount')[0].value=f(nn('TotalAmount',0)/nn('CurrencyRate',0),2);

 
 }
 } 

 var z=n('CurTotalAmount');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalAmount')[0].value=f(nn('CurTotalAmount',0)*nn('CurrencyRate',0),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccAdjustDet_Amount'); 
 r(0,"CurAmount")[p].value=f(rn(0,"Amount",p)/nn('CurrencyRate',0),2);
n('TotalAmount')[0].value=f(sm('tblAccAdjustDet_Amount'),2);
n('CurTotalAmount')[0].value=f(sm('tblAccAdjustDet_CurAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAccAdjustDet_CurAmount'); 
 r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0),2);
n('TotalAmount')[0].value=f(sm('tblAccAdjustDet_Amount'),2);
n('CurTotalAmount')[0].value=f(sm('tblAccAdjustDet_CurAmount'),2);
...
childData[childCount]='tblAccAdjustDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='应收调账单明细';
childCount++;
 var tblAccAdjustDetData = new gridData();
 gridDatas[gridDataNum] = tblAccAdjustDetData;
 gridDataNum++;
 var tblAccAdjustDet=1;
 ...
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_RefBillNo','关联单据编号','100',2,true,'',50,2,'tblAccAdjustDet:RefBillNo:tblAccAdjustDet_RefBillNo:'+param4,"","","","0",'',"2",true);
 putChildValidateItem("tblAccAdjustDet_RefBillNo","关联单据编号","any","",true,minValue,50,false,'',true);
 
 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_BillAmt','单据金额','100',1,true,'',0,8,'','',"SAmountIdentifier","","1",'',"0",false); 
 putChildValidateItem("tblAccAdjustDet_BillAmt","单据金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_SettledAmt','已结算金额','100',1,true,'',0,8,'','',"SAmountIdentifier","","2",'',"0",false); 
 putChildValidateItem("tblAccAdjustDet_SettledAmt","已结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_BackAmt','退货金额','100',1,true,'',0,8,'','',"SAmountIdentifier","","3",'',"0",false); 
 putChildValidateItem("tblAccAdjustDet_BackAmt","退货金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_WexeBalAmt','待结算金额','100',1,true,'',0,8,'','',"SAmountIdentifier","","4",'',"0",false); 
 putChildValidateItem("tblAccAdjustDet_WexeBalAmt","待结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_Amount','本次调账金额','100',1,true,'',0,0,'','',"SAmountIdentifier","","5",'',"0",false); 
 putChildValidateItem("tblAccAdjustDet_Amount","本次调账金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_CurBillAmt','外币单据金额','80',1,true,'',0,-2,'','',"SAmountIdentifier","","6",'',"100",false); 
 putChildValidateItem("tblAccAdjustDet_CurBillAmt","外币单据金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_CurSettledAmt','外币已结算金额','80',1,true,'',0,-2,'','',"SAmountIdentifier","","7",'',"100",false); 
 putChildValidateItem("tblAccAdjustDet_CurSettledAmt","外币已结算金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_CurBackAmt','外币退货金额','80',1,true,'',0,-2,'','',"SAmountIdentifier","","8",'',"100",false); 
 putChildValidateItem("tblAccAdjustDet_CurBackAmt","外币退货金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_CurWexeBalAmt','外币待结算金额','80',1,true,'',0,-2,'','',"SAmountIdentifier","","9",'',"100",false); 
 putChildValidateItem("tblAccAdjustDet_CurWexeBalAmt","外币待结算金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_CurAmount','外币调账金额','80',1,true,'0',0,-2,'','',"SAmountIdentifier","","10",'',"0",false); 
 putChildValidateItem("tblAccAdjustDet_CurAmount","外币调账金额","double","currency",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_Remark','备注','200',2,true,'',8000,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblAccAdjustDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_RefBillID','关联单据ID','200',2,true,'',50,-2,'','',"","","12",'',"100",false); 
 putChildValidateItem("tblAccAdjustDet_RefBillID","关联单据ID","any","",true,minValue,50,false,''); 
 
 
 
 
 addCols(tblAccAdjustDetData,'tblAccAdjustDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblAccAdjust&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblAccAdjust,tblAccAdjustDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAccAdjust&allTableName=tblAccAdjust,tblAccAdjustDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAccAdjust&allTableName=tblAccAdjustDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAccAdjust&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAccAdjust&allTableName=tblAccAdjustDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblAccAdjust&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblAccAdjust&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblAccAdjust&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblAccAdjust&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblAccAdjust&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '应收调账单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblAccAdjust&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
