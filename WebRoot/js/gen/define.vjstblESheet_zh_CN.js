...
{ 
 cFireEvent('tblESheetDet_Price','0'); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblESheet&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblESheet&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblESheet&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblESheet&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblESheetDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblESheetDetInvoiceTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblESheetDet"){
 
  }
 if(typeof(tableName)!="undefined" && tableName=="tblESheetDetInvoice"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblESheetDet"){
 
  }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblESheetDetInvoice"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblESheetDet"; 
 tblName[1]="tblESheetDetInvoice"; 
...
function execStat2(){
 execStatOne('tblESheetDet_Quantity',2);
 execStatOne('tblESheetDet_Amount',2);
 execStatOne('tblESheetDet_NetWeight',2);
 execStatOne('tblESheetDet_Weight',2);
 execStatOne('tblESheetDet_BoxQty',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=30225e18_1208241000485460185&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=30225e18_1208241000485460185&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblESheet\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblESheet\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblESheetDet_Quantity',"tblESheetDet"); 
 delFireEventOne(rowId,tableName,'tblESheetDet_Amount',"tblESheetDet"); 
 delFireEventOne(rowId,tableName,'tblESheetDet_Price',"tblESheetDet"); 
 delFireEventOne(rowId,tableName,'tblESheetDet_NetWeight',"tblESheetDet"); 
 delFireEventOne(rowId,tableName,'tblESheetDet_Weight',"tblESheetDet"); 
 delFireEventOne(rowId,tableName,'tblESheetDet_BoxQty',"tblESheetDet"); 
 }
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'tblESheetDet_Quantity',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblESheetDet_Amount',dis:'金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblESheetDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblESheetDet_NetWeight',dis:'净重',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblESheetDet_Weight',dis:'毛重',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblESheet",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblESheet&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblESheet&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"30225e18_1208241000485460185",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 saveField[0].value = "";
 var obj = m("tbl_tblExpAgreement_ZHLowestFee");
 if(obj != null){
 obj.value = "";
 }
 ...
 var saveField=n("tblCompany_ContentOrgCode");
 saveField[0].value = ""; 
 m("tbl_tblCompany_ContentOrgCode").value = "" ;
 var saveField=n("tblCompany_ImportExCode");
 saveField[0].value = ""; 
 m("tbl_tblCompany_ImportExCode").value = "" ;
 }
 ...
 if(fieldName == "Consignor"){
 var saveField=n("Consignor"); 
 saveField[0].value = "";
 var obj = m("tbl_tblLSCompany_ComFullName");
 if(obj != null){
 obj.value = "";
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=30225e18_1208241000485460185&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=30225e18_1208241000485460185&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=30225e18_1208241000485460185&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblESheet"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblESheet&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblESheet&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('TotalAmount')[0].value=f(sm('tblESheetDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalNetWeight')[0].value=f(sm('tblESheetDet_NetWeight'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalWeight')[0].value=f(sm('tblESheetDet_Weight'),2);
 ...
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('TotalBoxQty')[0].value=f(sm('tblESheetDet_BoxQty'),0);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblESheetDet_Quantity'); 
 r(0,"Price")[p].value=f(rn(0,"Amount",p)/rn(0,"Quantity",p),4);
n('TotalAmount')[0].value=f(sm('tblESheetDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblESheetDet_Amount'); 
 r(0,"Price")[p].value=f(rn(0,"Amount",p)/rn(0,"Quantity",p),4);
n('TotalAmount')[0].value=f(sm('tblESheetDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblESheetDet_Price'); 
 r(0,"Price")[p].value=f(rn(0,"Amount",p)/rn(0,"Quantity",p),4);
n('TotalAmount')[0].value=f(sm('tblESheetDet_Amount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblESheetDet_NetWeight'); 
 n('TotalNetWeight')[0].value=f(sm('tblESheetDet_NetWeight'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblESheetDet_Weight'); 
 n('TotalWeight')[0].value=f(sm('tblESheetDet_Weight'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblESheetDet_BoxQty'); 
 n('TotalBoxQty')[0].value=f(sm('tblESheetDet_BoxQty'),0);
...
childData[childCount]='tblESheetDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='出口制单明细';
childCount++;
 var tblESheetDetData = new gridData();
 gridDatas[gridDataNum] = tblESheetDetData;
 gridDataNum++;
 var tblESheetDet=1;
 ...
 addCols(tblESheetDetData,'tblESheetDet_Item','Item','30',0,true,'',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblESheetDet_Item","Item","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_LsGoodsID','产品ID','224',2,true,'',50,-2,'','',"","","4",'',"3",false); 
 putChildValidateItem("tblESheetDet_LsGoodsID","产品ID","any","",true,minValue,50,false,''); 
 ...
 addCols(tblESheetDetData,'tblESheetDet_Model','型号','130',2,false,'',100,2,'tblESheetDet:Model:tblESheetDet_Model:'+param4,"","","","6",'',"2",true);
 putChildValidateItem("tblESheetDet_Model","型号","any","",false,minValue,100,false,'',true);
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_Brand','品牌','120',2,false,'',50,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblESheetDet_Brand","品牌","any","",false,minValue,50,false,'',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_GoodsName','品名','130',2,false,'',50,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblESheetDet_GoodsName","品名","any","",false,minValue,50,false,'',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_Unit','单位','65',2,false,'',50,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("tblESheetDet_Unit","单位","any","",false,minValue,50,false,'',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_Quantity','数量','80',1,false,'0',0,0,'','',"","","14",'',"0",false); 
 putChildValidateItem("tblESheetDet_Quantity","数量","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_Amount','金额','80',1,false,'0',0,0,'','',"Amount","","16",'',"0",false); 
 putChildValidateItem("tblESheetDet_Amount","金额","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_Price','单价','80',1,false,'0',0,0,'','',"Price","","18",'',"0",false); 
 putChildValidateItem("tblESheetDet_Price","单价","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_GoodsNumber','商品编码','100',2,false,'',20,0,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblESheetDet_GoodsNumber","商品编码","any","",false,minValue,20,false,'',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_Spec','规格','350',2,false,'',500,0,'','',"","","22",'',"0",false); 
 putChildValidateItem("tblESheetDet_Spec","规格","any","",false,minValue,500,false,'',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_BoxNo','No.','30',2,true,'',10,0,'','',"","","24",'',"0",false); 
 putChildValidateItem("tblESheetDet_BoxNo","No.","any","",true,minValue,10,false,'',true); 
 ...
 addCols(tblESheetDetData,'tblESheetDet_GoodsPlace','产地','80',2,false,'',100,-2,'','',"","","26",'',"2",true);
 putChildValidateItem("tblESheetDet_GoodsPlace","产地","any","",false,minValue,100,false,'');
 addCols(tblESheetDetData,'tblESheetDet_tblCountry_Country','产地','50',2,true,'',1000000,2,'tblESheetDet:GoodsPlace:tblESheetDet_tblCountry_Country:'+param4,'',"","","26",'',"2",false); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_QuantityLast','入仓可提货数量','224',1,true,'',0,-2,'','',"","","28",'',"3",false); 
 putChildValidateItem("tblESheetDet_QuantityLast","入仓可提货数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_NetWeight','净重','50',1,false,'0',0,0,'','',"","","30",'',"0",false); 
 putChildValidateItem("tblESheetDet_NetWeight","净重","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_Weight','毛重','50',1,true,'0',0,0,'','',"","","32",'',"0",false); 
 putChildValidateItem("tblESheetDet_Weight","毛重","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_BoxQty','件数','30',0,false,'0',0,0,'','',"","","34",'',"0",false); 
 putChildValidateItem("tblESheetDet_BoxQty","件数","int","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_Remark','备注','50',2,true,'',8000,0,'','',"","","36",'',"0",false); 
 putChildValidateItem("tblESheetDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_CustomsCond','监管条件','50',2,true,'',20,0,'','',"","","38",'',"0",false); 
 putChildValidateItem("tblESheetDet_CustomsCond","监管条件","any","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_GoodStatus','状态','50',2,true,'',100,8,'','',"","","40",'',"8",false); 
 putChildValidateItem("tblESheetDet_GoodStatus","状态","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_GoodStatus2','状态2','50',2,true,'',100,8,'','',"","","42",'',"8",false); 
 putChildValidateItem("tblESheetDet_GoodStatus2","状态2","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_Code','明细关联ID','20',2,true,'',50,-2,'','',"","","50",'',"3",false); 
 putChildValidateItem("tblESheetDet_Code","明细关联ID","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblESheetDetData,'tblESheetDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblESheetDetInvoice';
tabIsNulls[childCount]='0';
tabDiss[childCount]='出口发票';
childCount++;
 var tblESheetDetInvoiceData = new gridData();
 gridDatas[gridDataNum] = tblESheetDetInvoiceData;
 gridDataNum++;
 var tblESheetDetInvoice=1;
 ...
 addCols(tblESheetDetInvoiceData,'tblESheetDetInvoice_StartItem','开始序号','100',2,true,'',20,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblESheetDetInvoice_StartItem","开始序号","any","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblESheetDetInvoiceData,'tblESheetDetInvoice_EndItem','结束序号','100',2,true,'',20,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblESheetDetInvoice_EndItem","结束序号","any","",true,minValue,20,false,'',true); 
 
 
 
 addCols(tblESheetDetInvoiceData,'tblESheetDetInvoice_CustomsNum','报关单号','150',2,true,'',50,0,'','',"","","20",'',"0",false); 
 putChildValidateItem("tblESheetDetInvoice_CustomsNum","报关单号","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblESheetDetInvoiceData,'tblESheetDetInvoice_DischarTime','放行时间','150',6,true,'',19,0,'','',"","","22",'',"0",false); 
 putChildValidateItem("tblESheetDetInvoice_DischarTime","放行时间","datetime","",true,minValue,19,false,'',true); 
 
 
 
 addCols(tblESheetDetInvoiceData,'tblESheetDetInvoice_SHLStatus','收汇联状态','224',2,true,'0',20,-2,'','',"","","24",'',"3",false); 
 putChildValidateItem("tblESheetDetInvoice_SHLStatus","收汇联状态","any","",true,minValue,20,true,'0'); 
 
 
 
 addCols(tblESheetDetInvoiceData,'tblESheetDetInvoice_invId','制单发票号','224',2,true,'',50,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblESheetDetInvoice_invId","制单发票号","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblESheetDetInvoiceData,'tblESheetDetInvoice_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblESheet&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblESheet,tblESheetDet,tblESheetDetInvoice,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblESheet&allTableName=tblESheet,tblESheetDet,tblESheetDetInvoice,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblESheet&allTableName=tblESheetDet,tblESheetDetInvoice,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblESheet&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblESheet&allTableName=tblESheetDet,tblESheetDetInvoice,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=30225e18_1208241000485460185&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=30225e18_1208241000485460185&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=30225e18_1208241000485460185&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=30225e18_1208241000485460185&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblESheet&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblESheet&designId=5cfff0af_1411131131037472542&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblESheet&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblESheet&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblESheet&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '出口制单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblESheet&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
