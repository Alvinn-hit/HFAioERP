...
{ 
 cFireEvent('TblInStoreDet_Price','0'); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=TblInStore&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=TblInStore&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=TblInStore&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=TblInStore&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "TblInStoreDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="TblInStoreDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="TblInStoreDet"){
...
function execStat2(){
 execStatOne('TblInStoreDet_Amount',2);
 execStatOne('TblInStoreDet_TotalAmount',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=f22d1e3b_1405120855307180082&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=f22d1e3b_1405120855307180082&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"TblInStore\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"TblInStore\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'TblInStoreDet_Price',"TblInStoreDet"); 
 delFireEventOne(rowId,tableName,'TblInStoreDet_Amount',"TblInStoreDet"); 
 delFireEventOne(rowId,tableName,'TblInStoreDet_TotalAmount',"TblInStoreDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'PayedCost',dis:'已结算金额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TotalCost',dis:'总金额',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'TblInStoreDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'TblInStoreDet_Amount',dis:'数量',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'TblInStoreDet_TotalAmount',dis:'金额',digit:0,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=TblInStore",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=TblInStore&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=TblInStore&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
{ 
 ...
 MOID:"f22d1e3b_1405120855307180082",
 MOOP:"add",
...
function resetSubmit(fieldName){
 ...
 var saveField=n("mpany_ComFu");
 saveField[0].value = ""; 
 m("tbl_mpany_ComFu").value = "" ;
 }
 }
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f22d1e3b_1405120855307180082&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f22d1e3b_1405120855307180082&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=f22d1e3b_1405120855307180082&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "TblInStore"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=TblInStore&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=TblInStore&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'TblInStoreDet_Price'); 
 r(0,"TotalAmount")[p].value=f(rn(0,"Price",p)*rn(0,"Amount",p),2);
n('TotalCost')[0].value=f(sm('TblInStoreDet_TotalAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'TblInStoreDet_Amount'); 
 r(0,"TotalAmount")[p].value=f(rn(0,"Price",p)*rn(0,"Amount",p),2);
n('TotalCost')[0].value=f(sm('TblInStoreDet_TotalAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'TblInStoreDet_TotalAmount'); 
 n('TotalCost')[0].value=f(sm('TblInStoreDet_TotalAmount'),2);
r(0,"Price")[p].value=f(rn(0,"TotalAmount",p)/rn(0,"Amount",p),4);
...
childData[childCount]='TblInStoreDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='入库单明细';
childCount++;
 var TblInStoreDetData = new gridData();
 gridDatas[gridDataNum] = TblInStoreDetData;
 gridDataNum++;
 var TblInStoreDet=1;
 ...
 addCols(TblInStoreDetData,'TblInStoreDet_WareNo','材料编码','100',2,true,'',20,2,'TblInStoreDet:WareNo:TblInStoreDet_WareNo:'+param4,"","","","0",'',"8",true);
 putChildValidateItem("TblInStoreDet_WareNo","材料编码","any","",true,minValue,20,false,'',true);
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_WareName','材料名称','100',2,true,'',100,0,'','',"","","1",'',"8",false); 
 putChildValidateItem("TblInStoreDet_WareName","材料名称","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_Specification','规格/型号','100',2,true,'',100,0,'','',"","","2",'',"8",false); 
 putChildValidateItem("TblInStoreDet_Specification","规格/型号","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_Brand','品牌','100',2,true,'',100,0,'','',"","","3",'',"8",false); 
 putChildValidateItem("TblInStoreDet_Brand","品牌","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_WareUnit','计量单位','50',2,true,'',10,0,'','',"","","4",'',"8",false); 
 putChildValidateItem("TblInStoreDet_WareUnit","计量单位","any","",true,minValue,10,false,'',true); 
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_Price','单价','100',1,true,'0',0,0,'','',"","","5",'',"8",false); 
 putChildValidateItem("TblInStoreDet_Price","单价","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_Amount','数量','100',1,true,'0',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("TblInStoreDet_Amount","数量","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_TotalAmount','金额','100',1,true,'0',0,0,'','',"","","7",'',"8",false); 
 putChildValidateItem("TblInStoreDet_TotalAmount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_Remark','说明','150',2,true,'',500,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("TblInStoreDet_Remark","说明","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_SourceID','引用明细ID','100',2,true,'',32,-2,'','',"","","9",'',"3",false); 
 putChildValidateItem("TblInStoreDet_SourceID","引用明细ID","any","",true,minValue,32,false,''); 
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_BatchNo','批号','100',2,true,'',100,-2,'','',"","","10",'',"0",false); 
 putChildValidateItem("TblInStoreDet_BatchNo","批号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_GoodsCode','商品编号','100',2,true,'',100,-2,'','',"","","11",'',"0",false); 
 putChildValidateItem("TblInStoreDet_GoodsCode","商品编号","any","",true,minValue,100,false,''); 
 
 
 
 
 addCols(TblInStoreDetData,'TblInStoreDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=TblInStore&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "TblInStore,TblInStoreDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=TblInStore&allTableName=TblInStore,TblInStoreDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=TblInStore&allTableName=TblInStoreDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=TblInStore&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=TblInStore&allTableName=TblInStoreDet,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f22d1e3b_1405120855307180082&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=f22d1e3b_1405120855307180082&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=f22d1e3b_1405120855307180082&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=f22d1e3b_1405120855307180082&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=TblInStore&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function extendSubmitPopDefine(selectName,defineName,displayName){ 
 ...
 var strURL = "/UserFunctionAction.do?&selectName="+selectName+"&moduleType="+moduleType+"&operation=22" + "&MOID=f22d1e3b_1405120855307180082&MOOP=query&popupWin=PopDefineDiv&LinkType=@URL:&displayName="+displayName ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=TblInStore&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=TblInStore&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=TblInStore&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=TblInStore&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '入库单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=TblInStore&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
