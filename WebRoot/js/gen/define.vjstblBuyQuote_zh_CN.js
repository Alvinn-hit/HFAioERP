...
{ 
 cFireEvent('tblBuyQuoteDet_CurPrice',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBuyQuote&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBuyQuote&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBuyQuote&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBuyQuote&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBuyQuoteDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBuyQuoteDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBuyQuoteDet"){
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblBuyQuote\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBuyQuote\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblBuyQuoteDet_CurPrice',"tblBuyQuoteDet"); 
 delFireEventOne(rowId,tableName,'tblBuyQuoteDet_Price',"tblBuyQuoteDet"); 
 delFireEventOne(rowId,tableName,'tblBuyQuoteDet_TaxPrice',"tblBuyQuoteDet"); 
 }
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'tblBuyQuoteDet_CurPrice',dis:'原币单价',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyQuoteDet_Price',dis:'本币未税单价',digit:6,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyQuoteDet_TaxPrice',dis:'本币含税单价',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBuyQuote",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyQuote&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyQuote&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblBuyQuote"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBuyQuote&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBuyQuote&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
r(0,"CurPrice")[j].value=f((((nn('Tax',0)/100)+1)*rn(0,"Price",j))/nn('CurrencyRate',0),2);
}

for(var j=0;j<r(0,'TaxPrice').length;j++){
r(0,"TaxPrice")[j].value=f(((nn('Tax',0)/100)+1)*rn(0,"Price",j),2);
}
 ...
 z[i].onchange = function(){ 
 for(var j=0;j<r(0,'TaxPrice').length;j++){
r(0,"TaxPrice")[j].value=f(rn(0,"CurPrice",j)*nn('CurrencyRate',0),2);
}

for(var j=0;j<r(0,'Price').length;j++){
 r(0,"Price")[j].value=f(rn(0,"CurPrice",j)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),6);
}
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyQuoteDet_CurPrice'); 
 r(0,"TaxPrice")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0),2);
r(0,"Price")[p].value=f(rn(0,"CurPrice",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),6);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyQuoteDet_Price'); 
 r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Price",p),2);
r(0,"CurPrice")[p].value=f((((nn('Tax',0)/100)+1)*rn(0,"Price",p))/nn('CurrencyRate',0),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyQuoteDet_TaxPrice'); 
 r(0,"Price")[p].value=f((rn(0,"TaxPrice",p)/((nn('Tax',0)/100)+1)),6);
r(0,"CurPrice")[p].value=f(rn(0,"TaxPrice",p)/nn('CurrencyRate',0),2);
...
childData[childCount]='tblBuyQuoteDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='采购核价明细';
childCount++;
 var tblBuyQuoteDetData = new gridData();
 gridDatas[gridDataNum] = tblBuyQuoteDetData;
 gridDataNum++;
 var tblBuyQuoteDet=1;
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_CompanyCode','供应商','80',2,true,'',100,-2,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblBuyQuoteDet_CompanyCode","供应商","any","",true,minValue,100,false,''); 
 ...
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_GoodsCode','商品','80',2,false,'',100,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("tblBuyQuoteDet_GoodsCode","商品","any","",false,minValue,100,false,'');
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'tblBuyQuoteDet:GoodsCode:tblBuyQuoteDet_tblGoods_GoodsNumber:'+param4,'',"","","1",'',"2",false); 
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_tblGoods_GoodsFullName','商品','150',2,true,'',1000000,2,'tblBuyQuoteDet:GoodsCode:tblBuyQuoteDet_tblGoods_GoodsFullName:'+param4,'',"","","1",'',"2",false); 
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'tblBuyQuoteDet:GoodsCode:tblBuyQuoteDet_tblGoods_GoodsSpec:'+param4,'',"","","1",'',"2",false); 
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblBuyQuoteDet:GoodsCode:tblBuyQuoteDet_tblGoods_BaseUnit:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_CurPrice','原币单价','100',1,true,'',0,0,'','',"AmountIdentifier","","2",'',"0",false); 
 putChildValidateItem("tblBuyQuoteDet_CurPrice","原币单价","double","BillTax",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_Price','本币未税单价','80',1,false,'',0,8,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblBuyQuoteDet_Price","本币未税单价","double","",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_TaxPrice','本币含税单价','100',1,true,'',0,8,'','',"AmountIdentifier","","4",'',"0",false); 
 putChildValidateItem("tblBuyQuoteDet_TaxPrice","本币含税单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_Remark','备注','100',2,true,'',8000,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblBuyQuoteDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_isLast','当前生效','80',2,true,'',10,-2,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblBuyQuoteDet_isLast","当前生效","any","",true,minValue,10,false,''); 
 
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_detfinishTime','生效时间','80',6,true,'',20,-2,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblBuyQuoteDet_detfinishTime","生效时间","datetime","",true,minValue,20,false,''); 
 
 
 addCols(tblBuyQuoteDetData,'tblBuyQuoteDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBuyQuote&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBuyQuote,tblBuyQuoteDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyQuote&allTableName=tblBuyQuote,tblBuyQuoteDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyQuote&allTableName=tblBuyQuoteDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyQuote&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyQuote&allTableName=tblBuyQuoteDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblBuyQuote&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblBuyQuote&designId=16abe428_1603022142386570274&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBuyQuote&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBuyQuote&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBuyQuote&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '采购核价';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBuyQuote&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
