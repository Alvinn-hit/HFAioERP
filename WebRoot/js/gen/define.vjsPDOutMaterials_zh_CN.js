...
{ 
 cFireEvent('PDOutMaterialsDet_IssueQty',''); 
 cFireEvent('PDOutMaterialsDet_Qty',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDOutMaterials&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDOutMaterials&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDOutMaterials&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDOutMaterials&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDOutMaterialsDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('PDOutMaterialsDet_GoodsCode');
 ...
 var z=n('PDOutMaterialsDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDOutMaterialsDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDOutMaterialsDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('PDOutMaterialsDet_IssueQty');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDOutMaterialsDet"){
 
 copyFields=n('PDOutMaterialsDet_GoodsCode');
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDOutMaterialsDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="PDOutMaterialsDet"; 
 var goodsCodeLogicValidate_PDOutMaterialsDet='@GN:Seq:Qty:tblAllotDet_GoodsSeqSet:tblAllotDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'PDOutMaterialsDet',value:goodsCodeLogicValidate_PDOutMaterialsDet};
...
function execStat2(){
 execStatOne('PDOutMaterialsDet_StockCode',2);
 execStatOne('PDOutMaterialsDet_DeQty',4);
 execStatOne('PDOutMaterialsDet_Qty',4);
 execStatOne('PDOutMaterialsDet_OutQty',4);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDOutMaterials\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDOutMaterials\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'PDOutMaterialsDet_IssueQty',"PDOutMaterialsDet"); 
 delFireEventOne(rowId,tableName,'PDOutMaterialsDet_DeQty',"PDOutMaterialsDet"); 
 delFireEventOne(rowId,tableName,'PDOutMaterialsDet_Qty',"PDOutMaterialsDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'Qty',dis:'生产数量',digit:4,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'PDOutMaterialsDet_IssueQty',dis:'发料数',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDOutMaterialsDet_DeQty',dis:'扣欠料',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDOutMaterialsDet_ShQty',dis:'仓库应发',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDOutMaterialsDet_Qty',dis:'仓库出货',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'PDOutMaterialsDet_OutQty',dis:'实发数',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDOutMaterials",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDOutMaterials&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDOutMaterials&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 var saveField=n("PDWorkOrder_BillDate");
 saveField[0].value = ""; 
 m("tbl_PDWorkOrder_BillDate").value = "" ;
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDOutMaterials"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDOutMaterials&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDOutMaterials&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDOutMaterialsDet_IssueQty'); 
 if(rn(0,"IssueQty",p)<=f(rn(0,"tblStocks_OweQty",p),4))r(0,"DeQty")[p].value=f(rn(0,"IssueQty",p),4);
 if(rn(0,"IssueQty",p)>f(rn(0,"tblStocks_OweQty",p),4))r(0,"DeQty")[p].value=f(f(rn(0,"tblStocks_OweQty",p),4),4);
r(0,"ShQty")[p].value=f(rn(0,"IssueQty",p)-rn(0,"DeQty",p),4);
r(0,"Qty")[p].value=f(rn(0,"ShQty",p),4);
if(rn(0,"Qty",p)+rn(0,"DeQty",p)>=rn(0,"IssueQty",p))r(0,"OutQty")[p].value=f(rn(0,"IssueQty",p),4);
else if(rn(0,"Qty",p)+rn(0,"DeQty",p)<rn(0,"IssueQty",p))r(0,"OutQty")[p].value=f(rn(0,"Qty",p)+rn(0,"DeQty",p),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDOutMaterialsDet_DeQty'); 
 r(0,"ShQty")[p].value=f(rn(0,"IssueQty",p)-rn(0,"DeQty",p),4);
r(0,"Qty")[p].value=f(rn(0,"ShQty",p),4);
if(rn(0,"Qty",p)+rn(0,"DeQty",p)>=rn(0,"IssueQty",p))r(0,"OutQty")[p].value=f(rn(0,"IssueQty",p),4);
else if(rn(0,"Qty",p)+rn(0,"DeQty",p)<rn(0,"IssueQty",p))r(0,"OutQty")[p].value=f(rn(0,"Qty",p)+rn(0,"DeQty",p),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDOutMaterialsDet_Qty'); 
 if(rn(0,"Qty",p)+rn(0,"DeQty",p)>=rn(0,"IssueQty",p))r(0,"OutQty")[p].value=f(rn(0,"IssueQty",p),4);
else if(rn(0,"Qty",p)+rn(0,"DeQty",p)<rn(0,"IssueQty",p))r(0,"OutQty")[p].value=f(rn(0,"Qty",p)+rn(0,"DeQty",p),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'PDOutMaterialsDet_SecQty'); 
...
childData[childCount]='PDOutMaterialsDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='发料单明细';
childCount++;
 var PDOutMaterialsDetData = new gridData();
 gridDatas[gridDataNum] = PDOutMaterialsDetData;
 gridDataNum++;
 var PDOutMaterialsDet=1;
 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_InStockCode','入库仓库','80',2,true,'',100,-2,'','',"","","0",'',"2",false); 
 putChildValidateItem("PDOutMaterialsDet_InStockCode","入库仓库","any","",true,minValue,100,false,''); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_GoodsCode','商品','120',2,true,'',100,-2,'','',"","triggerOrCopy","1",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_GoodsCode","商品","any","",true,minValue,100,false,''); 
 ...
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_MGoodsCode','品名','120',2,false,'',100,-2,'','',"","","2",'',"2",true);
 putChildValidateItem("PDOutMaterialsDet_MGoodsCode","品名","any","",false,minValue,100,false,'');
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'PDOutMaterialsDet:MGoodsCode:PDOutMaterialsDet_tblGoods_GoodsNumber:'+param4,'',"","","2",'',"2",false); 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_tblGoods_GoodsFullName','商品','150',2,true,'',1000000,2,'PDOutMaterialsDet:MGoodsCode:PDOutMaterialsDet_tblGoods_GoodsFullName:'+param4,'',"","","2",'',"2",false); 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDOutMaterialsDet:MGoodsCode:PDOutMaterialsDet_tblGoods_GoodsSpec:'+param4,'',"","","2",'',"2",false); 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDOutMaterialsDet:MGoodsCode:PDOutMaterialsDet_tblGoods_BaseUnit:'+param4,'',"","","2",'',"2",false); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_isClient','客供料','50',0,true,'2',0,8,';1:是;2:否','',"","","3",'',"1",false);
 putChildValidateItem("PDOutMaterialsDet_isClient","客供料","int","",true,minValue,maxValue,true,'2',true); 
 ...
 var param4="";
 param4=param4+":1品名@PDOutMaterialsDet_MGoodsCode";
 param4=param4+":bomId@PDOutMaterialsDet_bomId";
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_RGoodsCode','替换料','120',2,true,'',100,-2,'','',"","","4",'',"2",true);
 putChildValidateItem("PDOutMaterialsDet_RGoodsCode","替换料","any","",true,minValue,100,false,'');
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_tblGoods1_GoodsNumber','替换料号','120',0,true,'',1000000,2,'PDOutMaterialsDet:RGoodsCode:PDOutMaterialsDet_tblGoods1_GoodsNumber:'+param4,'',"","","4",'',"2",false); 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_tblGoods1_GoodsFullName','替换料名','200',0,true,'',1000000,2,'PDOutMaterialsDet:RGoodsCode:PDOutMaterialsDet_tblGoods1_GoodsFullName:'+param4,'',"","","4",'',"2",false); 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_tblGoods1_GoodsSpec','替换料规格','200',0,true,'',1000000,2,'PDOutMaterialsDet:RGoodsCode:PDOutMaterialsDet_tblGoods1_GoodsSpec:'+param4,'',"","","4",'',"2",false); 
 ...
 var param4="";
 param4=param4+":1品名@PDOutMaterialsDet_MGoodsCode";
 param4=param4+":0入库仓库@PDOutMaterialsDet_InStockCode";
 param4=param4+":1工令单号@PDWorkOrderID"; 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_StockCode','发料仓库','80',2,false,'Sess_StockCode',100,-2,'','',"","","5",'',"2",true);
 putChildValidateItem("PDOutMaterialsDet_StockCode","发料仓库","any","",false,minValue,100,true,'Sess_StockCode');
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_tblStock_StockNumber','仓库编号','100',2,true,'',1000000,2,'PDOutMaterialsDet:StockCode:PDOutMaterialsDet_tblStock_StockNumber:'+param4,'',"","","5",'',"2",false); 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_tblStock_StockName','发料仓库','100',2,true,'Sess_StockFullName',1000000,2,'PDOutMaterialsDet:StockCode:PDOutMaterialsDet_tblStock_StockName:'+param4,'',"","","5",'',"2",false); 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_tblStocks_lastQty','实际库存','90',1,true,'',1000000,2,'PDOutMaterialsDet:StockCode:PDOutMaterialsDet_tblStocks_lastQty:'+param4,'',"","","5",'',"2",false); 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_ViewVirtualStock_UseQty','可用库存','90',1,true,'',1000000,2,'PDOutMaterialsDet:StockCode:PDOutMaterialsDet_ViewVirtualStock_UseQty:'+param4,'',"","","5",'',"2",false); 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_tblStocks_OweQty','欠料余量','90',1,true,'',1000000,2,'PDOutMaterialsDet:StockCode:PDOutMaterialsDet_tblStocks_OweQty:'+param4,'',"","","5",'',"2",false); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","6",'',"2",false); 
 putChildValidateItem("PDOutMaterialsDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"2",false); 
 putChildValidateItem("PDOutMaterialsDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_color','色彩','60',2,true,'',100,-2,'','',"","","8",'',"2",false); 
 putChildValidateItem("PDOutMaterialsDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","9",'',"2",false); 
 putChildValidateItem("PDOutMaterialsDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","10",'',"2",false); 
 putChildValidateItem("PDOutMaterialsDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","11",'',"2",false); 
 putChildValidateItem("PDOutMaterialsDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","12",'',"1",false); 
 putChildValidateItem("PDOutMaterialsDet_Availably","保质期限","date","",true,minValue,10,false,''); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_IssueQty','发料数','80',1,true,'',0,0,'','',"Qty","triggerCopy","13",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_IssueQty","发料数","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_DeQty','扣欠料','80',1,true,'',0,0,'','',"Qty","","14",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_DeQty","扣欠料","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_ShQty','仓库应发','80',1,true,'',0,8,'','',"Qty","","15",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_ShQty","仓库应发","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_Qty','仓库出货','80',1,true,'',0,0,'','',"Qty","","16",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_Qty","仓库出货","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_OutQty','实发数','80',1,true,'',0,8,'','',"Qty","","17",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_OutQty","实发数","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","25",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_CheckBarCode','复核码','160',2,true,'',200,-2,'','',"","","26",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_CheckBarCode","复核码","any","",true,minValue,200,false,''); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","27",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","28",'',"3",false); 
 putChildValidateItem("PDOutMaterialsDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_Remark','备注','130',2,true,'',8000,0,'','',"","","29",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_PDWorkOrderID','工令单号','200',2,true,'',50,-2,'','',"","","30",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_PDWorkOrderID","工令单号","any","",true,minValue,50,false,''); 
 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_Seq','序列号','200',2,true,'',100,-2,'','',"","","31",'',"0",false); 
 putChildValidateItem("PDOutMaterialsDet_Seq","序列号","any","",true,minValue,100,false,''); 
 
 
 addCols(PDOutMaterialsDetData,'PDOutMaterialsDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDOutMaterials&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDOutMaterials,PDOutMaterialsDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDOutMaterials&allTableName=PDOutMaterials,PDOutMaterialsDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDOutMaterials&allTableName=PDOutMaterialsDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDOutMaterials&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDOutMaterials&allTableName=PDOutMaterialsDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "PDOutMaterialsDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDOutMaterials&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDOutMaterials&designId=3A3C6485838645049F08C74777C2&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDOutMaterials&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDOutMaterials&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDOutMaterials&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '发料单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDOutMaterials&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
