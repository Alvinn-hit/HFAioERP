...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=PDClientIn&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=PDClientIn&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=PDClientIn&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=PDClientIn&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "PDClientInDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('PDClientInDet_GoodsCode');
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="PDClientInDet"){
 
 copyFields=n('PDClientInDet_StockCode');
 ...
 copyFields=n('PDClientInDet_GoodsCode');
 if((order > 1 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var PDClientInDet_tblGoods_GoodsNumber = n('PDClientInDet_tblGoods_GoodsNumber');
 if(typeof(PDClientInDet_tblGoods_GoodsNumber) != 'undefined' && PDClientInDet_tblGoods_GoodsNumber.length >index){
 PDClientInDet_tblGoods_GoodsNumber[index].value = PDClientInDet_tblGoods_GoodsNumber[index-1].value;
 }
 var PDClientInDet_tblGoods_GoodsFullName = n('PDClientInDet_tblGoods_GoodsFullName');
 if(typeof(PDClientInDet_tblGoods_GoodsFullName) != 'undefined' && PDClientInDet_tblGoods_GoodsFullName.length >index){
 PDClientInDet_tblGoods_GoodsFullName[index].value = PDClientInDet_tblGoods_GoodsFullName[index-1].value;
 }
 var PDClientInDet_tblGoods_GoodsSpec = n('PDClientInDet_tblGoods_GoodsSpec');
 if(typeof(PDClientInDet_tblGoods_GoodsSpec) != 'undefined' && PDClientInDet_tblGoods_GoodsSpec.length >index){
 PDClientInDet_tblGoods_GoodsSpec[index].value = PDClientInDet_tblGoods_GoodsSpec[index-1].value;
 }
 var PDClientInDet_tblGoods_BaseUnit = n('PDClientInDet_tblGoods_BaseUnit');
 if(typeof(PDClientInDet_tblGoods_BaseUnit) != 'undefined' && PDClientInDet_tblGoods_BaseUnit.length >index){
 PDClientInDet_tblGoods_BaseUnit[index].value = PDClientInDet_tblGoods_BaseUnit[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="PDClientInDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="PDClientInDet"; 
 var goodsCodeLogicValidate_PDClientInDet='@GN:Seq:Qty:tblOtherInDet_GoodsSeqSet:tblOtherInDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'PDClientInDet',value:goodsCodeLogicValidate_PDClientInDet};
...
function execStat2(){
 execStatOne('PDClientInDet_Qty',4);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"PDClientIn\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"PDClientIn\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'PDClientInDet_Qty',dis:'数量',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=PDClientIn",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDClientIn&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=PDClientIn&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('PDClientInDet_Qty'); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "PDClientIn"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=PDClientIn&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=PDClientIn&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='PDClientInDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='客供入库明细';
childCount++;
 var PDClientInDetData = new gridData();
 gridDatas[gridDataNum] = PDClientInDetData;
 gridDataNum++;
 var PDClientInDet=1;
 ...
 addCols(PDClientInDetData,'PDClientInDet_StockCode','仓库','80',2,true,'',60,-2,'','',"","copy","0",'',"3",false); 
 putChildValidateItem("PDClientInDet_StockCode","仓库","any","",true,minValue,60,true,''); 
 ...
 addCols(PDClientInDetData,'PDClientInDet_GoodsCode','料件名称','120',2,false,'',100,-2,'','',"","triggerOrCopy","1",'',"2",true);
 putChildValidateItem("PDClientInDet_GoodsCode","料件名称","any","",false,minValue,100,false,'');
 
 
 addCols(PDClientInDetData,'PDClientInDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'PDClientInDet:GoodsCode:PDClientInDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(PDClientInDetData,'PDClientInDet_tblGoods_GoodsFullName','料件名称','150',2,true,'',1000000,2,'PDClientInDet:GoodsCode:PDClientInDet_tblGoods_GoodsFullName:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(PDClientInDetData,'PDClientInDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'PDClientInDet:GoodsCode:PDClientInDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(PDClientInDetData,'PDClientInDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'PDClientInDet:GoodsCode:PDClientInDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","2",'',"0",false); 
 putChildValidateItem("PDClientInDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","3",'',"0",false); 
 putChildValidateItem("PDClientInDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","4",'',"2",false); 
 putChildValidateItem("PDClientInDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","5",'',"2",false); 
 putChildValidateItem("PDClientInDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_color','色彩','60',2,true,'',100,-2,'','',"","","6",'',"2",false); 
 putChildValidateItem("PDClientInDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","7",'',"0",false); 
 putChildValidateItem("PDClientInDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","9",'',"0",false); 
 putChildValidateItem("PDClientInDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_Seq','序列号','130',2,true,'',5000,-2,'','',"","triggerCopy","10",'',"2",false); 
 putChildValidateItem("PDClientInDet_Seq","序列号","any","GoodsField",true,minValue,5000,false,''); 
 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_Qty','数量','80',1,false,'',0,0,'','',"Qty","","12",'',"0",false); 
 putChildValidateItem("PDClientInDet_Qty","数量","double","SeqQty",false,minValue,maxValue,false,'',true); 
 ...
 addCols(PDClientInDetData,'PDClientInDet_SerialNo','序列号','80',0,true,'',0,-2,'','',"Price","","18",'',"0",false); 
 putChildValidateItem("PDClientInDet_SerialNo","序列号","int","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","19",'',"100",false); 
 putChildValidateItem("PDClientInDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","20",'',"3",false); 
 putChildValidateItem("PDClientInDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","21",'',"3",false); 
 putChildValidateItem("PDClientInDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_colorNV','颜色名称','80',2,true,'',500,-2,'','',"","","23",'',"0",false); 
 putChildValidateItem("PDClientInDet_colorNV","颜色名称","any","GoodsField",true,minValue,500,false,''); 
 
 
 
 addCols(PDClientInDetData,'PDClientInDet_Remark','备注','130',2,true,'',8000,0,'','',"","","24",'',"0",false); 
 putChildValidateItem("PDClientInDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 addCols(PDClientInDetData,'PDClientInDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=PDClientIn&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "PDClientIn,PDClientInDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDClientIn&allTableName=PDClientIn,PDClientInDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDClientIn&allTableName=PDClientInDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=PDClientIn&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=PDClientIn&allTableName=PDClientInDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=PDClientIn&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=PDClientIn&designId=7A1ECD82988C4D5E96B3A0F962EF&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=PDClientIn&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=PDClientIn&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=PDClientIn&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '客供入库单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=PDClientIn&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
