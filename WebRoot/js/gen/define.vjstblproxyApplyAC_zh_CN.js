...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblproxyApplyAC&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblproxyApplyAC&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblproxyApplyAC&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblproxyApplyAC&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblproxyApplyDetACTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblproxyApplyDetAC"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblproxyApplyDetAC"){
...
function execStat2(){
 execStatOne('tblproxyApplyDetAC_PayAmount',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=BD653BB9DC404ABFB1D2C9358EB98A&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=BD653BB9DC404ABFB1D2C9358EB98A&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblproxyApplyAC\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblproxyApplyAC\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblproxyApplyDetAC_PayAmount',"tblproxyApplyDetAC"); 
 }
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'tblproxyApplyDetAC_PayAmount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblproxyApplyAC",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblproxyApplyAC&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblproxyApplyAC&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"BD653BB9DC404ABFB1D2C9358EB98A",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=BD653BB9DC404ABFB1D2C9358EB98A&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=BD653BB9DC404ABFB1D2C9358EB98A&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=BD653BB9DC404ABFB1D2C9358EB98A&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblproxyApplyAC"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblproxyApplyAC&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblproxyApplyAC&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 ...
 n('TotalAmount')[0].value=f(sm('tblproxyApplyDetAC_PayAmount'),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblproxyApplyDetAC_PayAmount'); 
 n('TotalAmount')[0].value=f(sm('tblproxyApplyDetAC_PayAmount'),2);
n('TotalKPAmount')[0].value=f(Number(nn('TotalAmount',0))+Number(nn('DLFee',0))+Number(nn('GNFee',0))+Number(nn('KDFee',0)),2);
...
childData[childCount]='tblproxyApplyDetAC';
tabIsNulls[childCount]='0';
tabDiss[childCount]='增值税(6%)开票申请明细AC';
childCount++;
 var tblproxyApplyDetACData = new gridData();
 gridDatas[gridDataNum] = tblproxyApplyDetACData;
 gridDataNum++;
 var tblproxyApplyDetAC=1;
 ...
 addCols(tblproxyApplyDetACData,'tblproxyApplyDetAC_PayItemCode','结算项编号','100',2,true,'',30,2,'tblproxyApplyDetAC:PayItemCode:tblproxyApplyDetAC_PayItemCode:'+param4,"","","","2",'',"2",true);
 putChildValidateItem("tblproxyApplyDetAC_PayItemCode","结算项编号","any","",true,minValue,30,false,'',true);
 
 
 
 addCols(tblproxyApplyDetACData,'tblproxyApplyDetAC_PayItem','结算项','170',2,true,'',50,8,'','',"","","3",'',"8",false); 
 putChildValidateItem("tblproxyApplyDetAC_PayItem","结算项","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblproxyApplyDetACData,'tblproxyApplyDetAC_PayItemRemark','结算项全称','250',2,true,'',100,8,'','',"","","4",'',"8",false); 
 putChildValidateItem("tblproxyApplyDetAC_PayItemRemark","结算项全称","any","",true,minValue,100,false,'',true); 
 
 
 
 addCols(tblproxyApplyDetACData,'tblproxyApplyDetAC_PayAmount','金额','120',1,true,'0',0,0,'','',"Amount","","4",'',"0",false); 
 putChildValidateItem("tblproxyApplyDetAC_PayAmount","金额","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblproxyApplyDetACData,'tblproxyApplyDetAC_Remark','备注','250',2,true,'',8000,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblproxyApplyDetAC_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblproxyApplyDetACData,'tblproxyApplyDetAC_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblproxyApplyAC&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblproxyApplyAC,tblproxyApplyDetAC,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblproxyApplyAC&allTableName=tblproxyApplyAC,tblproxyApplyDetAC,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblproxyApplyAC&allTableName=tblproxyApplyDetAC,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblproxyApplyAC&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblproxyApplyAC&allTableName=tblproxyApplyDetAC,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=BD653BB9DC404ABFB1D2C9358EB98A&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=BD653BB9DC404ABFB1D2C9358EB98A&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var cTabName = "tblproxyApplyACDet";
 var stockCode="";
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=BD653BB9DC404ABFB1D2C9358EB98A&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=BD653BB9DC404ABFB1D2C9358EB98A&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblproxyApplyAC&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblproxyApplyAC&designId=21786bdc_1410140936048311766&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblproxyApplyAC&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblproxyApplyAC&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblproxyApplyAC&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '增值税(6%)开票申请AC';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblproxyApplyAC&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
