...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblFixedAssetAdd&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblFixedAssetAdd&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblFixedAssetAdd&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblFixedAssetAdd&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblFixedAssetAddATypeTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblFixedAssetAddDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblFixedAssetAddAType"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblFixedAssetAddDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblFixedAssetAddAType"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblFixedAssetAddDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblFixedAssetAddAType"; 
 tblName[1]="tblFixedAssetAddDet"; 
...
function execStat2(){
 execStatOne('tblFixedAssetAddAType_AccCode',2);
 execStatOne('tblFixedAssetAddDet_Qty',2);
 execStatOne('tblFixedAssetAddDet_Amount',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblFixedAssetAdd\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblFixedAssetAdd\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'originalWorth',dis:'入账原值',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'totalDeprecia',dis:'累计折旧',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'depreciaArrange',dis:'减值准备',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'netWorth',dis:'净值',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'planReWorthRate',dis:'预计净残值率',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'planReWorth',dis:'预计净残值',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'MonthWork',dis:'当月工作量',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'workunitDeprecAmt',dis:'单位折旧额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'monthDeprecRate',dis:'月折旧率',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'monthDeprecAmt',dis:'月折旧额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblFixedAssetAddAType_amount',dis:'金额',digit:0,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblFixedAssetAddDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblFixedAssetAddDet_Amount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblFixedAssetAdd",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblFixedAssetAdd&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblFixedAssetAdd&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 if(fieldName == "AssetsType"){
 var saveField=n("AssetsType"); 
 saveField[0].value = "";
 var obj = m("tbl_tblFixedAssetType_id");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("AssetAcc"); 
 saveField[0].value = "";
 var obj = m("tbl_tblFixedAssetType_AssetAcc");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblFixedAssetType_TypeName");
 saveField[0].value = ""; 
 m("tbl_tblFixedAssetType_TypeName").value = "" ;
 }
 if(fieldName == "deprecAccCode"){
 var saveField=n("deprecAccCode"); 
 saveField[0].value = "";
 var obj = m("tbl_tblAccTypeInfo_AccNumber");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("tblAccTypeInfo_AccFullName");
 saveField[0].value = ""; 
 m("tbl_tblAccTypeInfo_AccFullName").value = "" ;
 }
 if(fieldName == "DevalueAccCode"){
 var saveField=n("DevalueAccCode"); 
 saveField[0].value = "";
 var obj = m("tbl_tblAccTypeInfo_AccNumber");
 if(obj != null){
 obj.value = "";
 }
 var saveField=n("SelectCompany_ComNumber");
 saveField[0].value = ""; 
 m("tbl_SelectCompany_ComNumber").value = "" ;
 }
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblFixedAssetAdd"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblFixedAssetAdd&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblFixedAssetAdd&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 var z=n('originalWorth');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('netWorth')[0].value=f(nn('originalWorth',0)-nn('totalDeprecia',0)-nn('depreciaArrange',0),2);

n('planReWorth')[0].value=f(nn('originalWorth',0)*nn('planReWorthRate',0),2);

n('planReWorthRate')[0].value=f(nn('planReWorth',0)/nn('originalWorth',0),2);

if(n('depreciaMonth')[0].value==1)n('monthDeprecAmt')[0].value=f((nn('netWorth',0)-nn('planReWorth',0))/(nn('planUseMonth',0)-nn('referMonth',0)),2);

if(n('depreciaMonth')[0].value==1)n('monthDeprecRate')[0].value=f(nn('monthDeprecAmt',0)/nn('originalWorth',0),2);

if(n('depreciaMonth')[0].value==2)n('workunitDeprecAmt')[0].value=f((nn('netWorth',0)-nn('planReWorth',0))/(nn('planWork',0)-nn('referWork',0)),2);

if(n('depreciaMonth')[0].value==2)n('monthDeprecAmt')[0].value=f(nn('workunitDeprecAmt',0)*nn('MonthWork',0),2);

if(n('depreciaMonth')[0].value==2)n('monthDeprecRate')[0].value=f(nn('workunitDeprecAmt',0)*nn('MonthWork',0)/nn('originalWorth',0),2);

if(n('depreciaMonth')[0].value==3)n('monthDeprecRate')[0].value=f(2/nn('planUseMonth',0),2);

if(n('depreciaMonth')[0].value==3)n('monthDeprecAmt')[0].value=f(nn('netWorth',0)*nn('monthDeprecRate',0),2);

if(n('depreciaMonth')[0].value==4)n('monthDeprecRate')[0].value=f((2*(nn('planUseMonth',0)-nn('referMonth',0)))/(nn('planUseMonth',0)*(nn('planUseMonth',0)+12)),2);

if(n('depreciaMonth')[0].value==4)n('monthDeprecAmt')[0].value=f((nn('originalWorth',0)-nn('planReWorth',0))*nn('monthDeprecRate',0),2);

 
 }
 } 

 var z=n('totalDeprecia');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('netWorth')[0].value=f(nn('originalWorth',0)-nn('totalDeprecia',0)-nn('depreciaArrange',0),2);
...
childData[childCount]='tblFixedAssetAddAType';
tabIsNulls[childCount]='1';
tabDiss[childCount]='资产增加方式';
childCount++;
 var tblFixedAssetAddATypeData = new gridData();
 gridDatas[gridDataNum] = tblFixedAssetAddATypeData;
 gridDataNum++;
 var tblFixedAssetAddAType=1;
 ...
 addCols(tblFixedAssetAddATypeData,'tblFixedAssetAddAType_AccCode','增加方式科目','400',2,true,'',100,-2,'','',"","","0",'',"2",true);
 putChildValidateItem("tblFixedAssetAddAType_AccCode","增加方式科目","any","",true,minValue,100,false,'');
 
 
 addCols(tblFixedAssetAddATypeData,'tblFixedAssetAddAType_tblAccTypeInfo_AccFullName','增加方式科目','200',20,true,'',1000000,2,'tblFixedAssetAddAType:AccCode:tblFixedAssetAddAType_tblAccTypeInfo_AccFullName:'+param4,'',"","","0",'',"2",false); 
 
 
 
 
 
 addCols(tblFixedAssetAddATypeData,'tblFixedAssetAddAType_amount','金额','200',1,true,'',0,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblFixedAssetAddAType_amount","金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 addCols(tblFixedAssetAddATypeData,'tblFixedAssetAddAType_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblFixedAssetAddDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='固定资产附属设备表';
childCount++;
 var tblFixedAssetAddDetData = new gridData();
 gridDatas[gridDataNum] = tblFixedAssetAddDetData;
 gridDataNum++;
 var tblFixedAssetAddDet=1;
 ...
 addCols(tblFixedAssetAddDetData,'tblFixedAssetAddDet_GoodsCode','设备名称','200',2,false,'',50,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("tblFixedAssetAddDet_GoodsCode","设备名称","any","",false,minValue,50,false,'');
 
 
 addCols(tblFixedAssetAddDetData,'tblFixedAssetAddDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'tblFixedAssetAddDet:GoodsCode:tblFixedAssetAddDet_tblGoods_GoodsNumber:'+param4,'',"","","1",'',"2",false); 
 
 
 addCols(tblFixedAssetAddDetData,'tblFixedAssetAddDet_tblGoods_GoodsFullName','设备名称','150',18,true,'',1000000,2,'tblFixedAssetAddDet:GoodsCode:tblFixedAssetAddDet_tblGoods_GoodsFullName:'+param4,'',"","","1",'',"2",false); 
 
 
 addCols(tblFixedAssetAddDetData,'tblFixedAssetAddDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'tblFixedAssetAddDet:GoodsCode:tblFixedAssetAddDet_tblGoods_GoodsSpec:'+param4,'',"","","1",'',"2",false); 
 
 
 addCols(tblFixedAssetAddDetData,'tblFixedAssetAddDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblFixedAssetAddDet:GoodsCode:tblFixedAssetAddDet_tblGoods_BaseUnit:'+param4,'',"","","1",'',"2",false); 
 
 
 
 addCols(tblFixedAssetAddDetData,'tblFixedAssetAddDet_Qty','数量','80',1,false,'0',0,0,'','',"Qty","","5",'',"0",false); 
 putChildValidateItem("tblFixedAssetAddDet_Qty","数量","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblFixedAssetAddDetData,'tblFixedAssetAddDet_Amount','金额','100',1,false,'0',0,0,'','',"Amount","","7",'',"0",false); 
 putChildValidateItem("tblFixedAssetAddDet_Amount","金额","double","",false,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblFixedAssetAddDetData,'tblFixedAssetAddDet_Remark','备注','200',2,true,'',8000,0,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblFixedAssetAddDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblFixedAssetAddDetData,'tblFixedAssetAddDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblFixedAssetAdd&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblFixedAssetAdd,tblFixedAssetAddAType,tblFixedAssetAddDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblFixedAssetAdd&allTableName=tblFixedAssetAdd,tblFixedAssetAddAType,tblFixedAssetAddDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblFixedAssetAdd&allTableName=tblFixedAssetAddAType,tblFixedAssetAddDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblFixedAssetAdd&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblFixedAssetAdd&allTableName=tblFixedAssetAddAType,tblFixedAssetAddDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblFixedAssetAddDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblFixedAssetAdd&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblFixedAssetAdd&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblFixedAssetAdd&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblFixedAssetAdd&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblFixedAssetAdd&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '固定资产期初';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblFixedAssetAdd&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
