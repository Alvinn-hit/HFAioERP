...
{ 
 cFireEvent('tblBuyOutStockDet_Qty',''); 
 cFireEvent('tblBuyOutStockDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblBuyOutStock&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblBuyOutStock&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblBuyOutStock&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblBuyOutStock&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblBuyOutStockDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 ...
 var z=n('tblBuyOutStockDet_GoodsCode');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblBuyOutStockDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblBuyOutStockDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblBuyOutStockDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblBuyOutStockDet"){
 
 copyFields=n('tblBuyOutStockDet_StockCode');
 ...
 copyFields=n('tblBuyOutStockDet_GoodsCode');
 if((order > 4 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblBuyOutStockDet_tblGoods_GoodsNumber = n('tblBuyOutStockDet_tblGoods_GoodsNumber');
 if(typeof(tblBuyOutStockDet_tblGoods_GoodsNumber) != 'undefined' && tblBuyOutStockDet_tblGoods_GoodsNumber.length >index){
 tblBuyOutStockDet_tblGoods_GoodsNumber[index].value = tblBuyOutStockDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblBuyOutStockDet_tblGoods_GoodsFullName = n('tblBuyOutStockDet_tblGoods_GoodsFullName');
 if(typeof(tblBuyOutStockDet_tblGoods_GoodsFullName) != 'undefined' && tblBuyOutStockDet_tblGoods_GoodsFullName.length >index){
 tblBuyOutStockDet_tblGoods_GoodsFullName[index].value = tblBuyOutStockDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblBuyOutStockDet_tblGoods_GoodsSpec = n('tblBuyOutStockDet_tblGoods_GoodsSpec');
 if(typeof(tblBuyOutStockDet_tblGoods_GoodsSpec) != 'undefined' && tblBuyOutStockDet_tblGoods_GoodsSpec.length >index){
 tblBuyOutStockDet_tblGoods_GoodsSpec[index].value = tblBuyOutStockDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblBuyOutStockDet_tblGoods_BaseUnit = n('tblBuyOutStockDet_tblGoods_BaseUnit');
 if(typeof(tblBuyOutStockDet_tblGoods_BaseUnit) != 'undefined' && tblBuyOutStockDet_tblGoods_BaseUnit.length >index){
 tblBuyOutStockDet_tblGoods_BaseUnit[index].value = tblBuyOutStockDet_tblGoods_BaseUnit[index-1].value;
 }
 var tblBuyOutStockDet_ViewVirtualStock_UseQty = n('tblBuyOutStockDet_ViewVirtualStock_UseQty');
 if(typeof(tblBuyOutStockDet_ViewVirtualStock_UseQty) != 'undefined' && tblBuyOutStockDet_ViewVirtualStock_UseQty.length >index){
 tblBuyOutStockDet_ViewVirtualStock_UseQty[index].value = tblBuyOutStockDet_ViewVirtualStock_UseQty[index-1].value;
 }
 }
   }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblBuyOutStockDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblBuyOutStockDet"; 
 var goodsCodeLogicValidate_tblBuyOutStockDet='@GN:Seq:Qty:tblBuyOutStockDet_GoodsSeqSet:tblBuyOutStockDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'tblBuyOutStockDet',value:goodsCodeLogicValidate_tblBuyOutStockDet};
...
function execStat2(){
 execStatOne('tblBuyOutStockDet_Qty',2);
 execStatOne('tblBuyOutStockDet_Amount',2);
 execStatOne('tblBuyOutStockDet_ApplyQty',2);
 execStatOne('tblBuyOutStockDet_CostAmount',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblBuyOutStock\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblBuyOutStock\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblBuyOutStockDet_Qty',"tblBuyOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblBuyOutStockDet_Price',"tblBuyOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblBuyOutStockDet_Amount',"tblBuyOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblBuyOutStockDet_CostPrice',"tblBuyOutStockDet"); 
 delFireEventOne(rowId,tableName,'tblBuyOutStockDet_CostAmount',"tblBuyOutStockDet"); 
 }
...
var fieldDigit = new Array();
 ...
 fieldDigit[fieldDigit.length] = {name:'tblBuyOutStockDet_Qty',dis:'数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOutStockDet_Price',dis:'单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblBuyOutStockDet_Amount',dis:'金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblBuyOutStock",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyOutStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblBuyOutStock&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('tblBuyOutStockDet_Qty'); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblBuyOutStock"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblBuyOutStock&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblBuyOutStock&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
n('TotalAmount')[0].value=f(sm('tblBuyOutStockDet_DisBackAmt'),2);

n('TotalTaxAmount')[0].value=f(sm('tblBuyOutStockDet_TaxAmount')-nn('DiscountAmount',0),2);
...
n('TotalCoTaxAmt')[0].value=f(sm('tblBuyOutStockDet_CoTaxAmt'),2);

for(var j=0;j<r(0,'DisAmount').length;j++){
r(0,"DisAmount")[j].value=f(rn(0,"Amount",j)-rn(0,"DisBackAmt",j),2);
}

n('DisAmount')[0].value=f(sm('tblBuyOutStockDet_DisAmount'),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_Qty'); 
...
r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_Price'); 
 r(0,"Amount")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
n('DisAmount')[0].value=f(sm('tblBuyOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_Amount'); 
...
n('DisAmount')[0].value=f(sm('tblBuyOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_Discount'); 
 r(0,"DisPrice")[p].value=f(rn(0,"Discount",p)*rn(0,"Price",p)/100,4);
r(0,"DisBackAmt")[p].value=f(rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
r(0,"TaxPrice")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Discount",p)*rn(0,"Price",p)/100,4);
r(0,"TaxAmount")[p].value=f(((nn('Tax',0)/100)+1)*rn(0,"Amount",p)*rn(0,"Discount",p)/100,2);
...
n('DisAmount')[0].value=f(sm('tblBuyOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_DisPrice'); 
...
n('DisAmount')[0].value=f(sm('tblBuyOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_DisBackAmt'); 
...
n('DisAmount')[0].value=f(sm('tblBuyOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_TaxPrice'); 
...
n('DisAmount')[0].value=f(sm('tblBuyOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_TaxAmount'); 
...
n('DisAmount')[0].value=f(sm('tblBuyOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_CurPrice'); 
...
n('TotalAmount')[0].value=f(sm('tblBuyOutStockDet_DisBackAmt'),2);
n('TotalTaxAmount')[0].value=f(sm('tblBuyOutStockDet_TaxAmount')-nn('DiscountAmount',0),2);
n('CurTotalAmount')[0].value=f(sm('tblBuyOutStockDet_CurAmount'),2);
r(0,"CoTaxAmt")[p].value=f(rn(0,"TaxAmount",p)-rn(0,"DisBackAmt",p),2);
n('TotalCoTaxAmt')[0].value=f(sm('tblBuyOutStockDet_CoTaxAmt'),2);
n('DisAmount')[0].value=f(sm('tblBuyOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_CurAmount'); 
 r(0,"TaxAmount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0),2);
r(0,"DisBackAmt")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1),2);
r(0,"Amount")[p].value=f(rn(0,"CurAmount",p)*nn('CurrencyRate',0)/((nn('Tax',0)/100)+1)/(rn(0,"Discount",p)/100),2);
...
n('DisAmount')[0].value=f(sm('tblBuyOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_CostPrice'); 
 r(0,"CostAmount")[p].value=f(rn(0,"CostPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_CostAmount'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_PresentSampleType'); 
...
n('DisAmount')[0].value=f(sm('tblBuyOutStockDet_DisAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblBuyOutStockDet_SecQty'); 
...
childData[childCount]='tblBuyOutStockDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='采购退货单明细';
childCount++;
 var tblBuyOutStockDetData = new gridData();
 gridDatas[gridDataNum] = tblBuyOutStockDetData;
 gridDataNum++;
 var tblBuyOutStockDet=1;
 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_Seq','序列号','130',2,true,'',5000,-2,'','',"","triggerCopy","0",'',"2",false); 
 putChildValidateItem("tblBuyOutStockDet_Seq","序列号","any","GoodsField",true,minValue,5000,false,''); 
 
 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_StockCode','仓库','80',2,true,'',50,-2,'','',"","copy","1",'',"3",false); 
 putChildValidateItem("tblBuyOutStockDet_StockCode","仓库","any","",true,minValue,50,true,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_BuyInStockID','入库单号','80',2,true,'',32,-2,'','',"","","2",'',"2",false); 
 putChildValidateItem("tblBuyOutStockDet_BuyInStockID","入库单号","any","ManyBillQuote",true,minValue,32,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_BuyOrderID','订单编号','80',2,true,'',32,-2,'','',"","","3",'',"3",false); 
 putChildValidateItem("tblBuyOutStockDet_BuyOrderID","订单编号","any","",true,minValue,32,false,''); 
 ...
 param4=param4+":0仓库@tblBuyOutStockDet_StockCode";
 param4=param4+":1供应商@CompanyCode"; 
 param4=param4+":tblBuyOutStockDet_BuyInStockID@tblBuyOutStockDet_BuyInStockID"; 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_GoodsCode','商品名称','120',2,false,'',50,-2,'','',"","triggerOrCopy","4",'',"2",true);
 putChildValidateItem("tblBuyOutStockDet_GoodsCode","商品名称","any","",false,minValue,50,false,'');
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'tblBuyOutStockDet:GoodsCode:tblBuyOutStockDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","4",'',"2",false); 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_tblGoods_GoodsFullName','商品名称','150',18,true,'',1000000,2,'tblBuyOutStockDet:GoodsCode:tblBuyOutStockDet_tblGoods_GoodsFullName:'+param4,'',"","triggerOrCopy","4",'',"2",false); 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'tblBuyOutStockDet:GoodsCode:tblBuyOutStockDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","4",'',"2",false); 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblBuyOutStockDet:GoodsCode:tblBuyOutStockDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","4",'',"2",false); 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_ViewVirtualStock_UseQty','虚拟库存','90',1,true,'',1000000,2,'tblBuyOutStockDet:GoodsCode:tblBuyOutStockDet_ViewVirtualStock_UseQty:'+param4,'',"","triggerOrCopy","4",'',"2",false); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","5",'',"2",false); 
 putChildValidateItem("tblBuyOutStockDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","6",'',"2",false); 
 putChildValidateItem("tblBuyOutStockDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"2",false); 
 putChildValidateItem("tblBuyOutStockDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","8",'',"2",false); 
 putChildValidateItem("tblBuyOutStockDet_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","9",'',"2",false); 
 putChildValidateItem("tblBuyOutStockDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","10",'',"2",false); 
 putChildValidateItem("tblBuyOutStockDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_color','色彩','60',2,true,'',100,-2,'','',"","","11",'',"2",false); 
 putChildValidateItem("tblBuyOutStockDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_Qty','数量','60',1,false,'',0,0,'','',"Qty","","12",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_Qty","数量","double","SeqQty",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_Price','单价','80',1,true,'',0,0,'','',"priceIdentifier","","13",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_Price","单价","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_Amount','金额','100',1,true,'',0,0,'','',"AmountIdentifier","","14",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_Amount","金额","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_Discount','折扣率%','100',1,true,'100',0,-2,'','',"Price","","20",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_Discount","折扣率%","double","BillDiscount",true,minValue,maxValue,true,'100'); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_DisPrice','折后单价','100',1,true,'',0,-2,'','',"priceIdentifier","","21",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_DisPrice","折后单价","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_DisBackAmt','折后金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","22",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_DisBackAmt","折后金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_DisAmount','折让金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","23",'',"8",false); 
 putChildValidateItem("tblBuyOutStockDet_DisAmount","折让金额","double","BillDiscount",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_TaxPrice','含税单价','100',1,true,'',0,-2,'','',"priceIdentifier","","24",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_TaxPrice","含税单价","double","BillTax",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_TaxAmount','含税金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","25",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_TaxAmount","含税金额","double","BillTax",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_CoTaxAmt','税额','100',1,true,'0',0,-2,'','',"AmountIdentifier","","26",'',"8",false); 
 putChildValidateItem("tblBuyOutStockDet_CoTaxAmt","税额","double","BillTax",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_CurPrice','外币单价','100',1,true,'',0,-2,'','',"priceIdentifier","","27",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_CurPrice","外币单价","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_CurAmount','外币金额','100',1,true,'',0,-2,'','',"AmountIdentifier","","28",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_CurAmount","外币金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_ApplyQty','申请数','80',1,false,'0',0,-2,'','',"Qty","","30",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_ApplyQty","申请数","double","",false,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_CostPrice','成本单价','80',1,true,'',999999999,-2,'','',"priceIdentifier","","31",'',"6",false); 
 putChildValidateItem("tblBuyOutStockDet_CostPrice","成本单价","double","",true,minValue,999999999,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_CostAmount','成本金额','80',1,true,'',999999999,-2,'','',"AmountIdentifier","","32",'',"6",false); 
 putChildValidateItem("tblBuyOutStockDet_CostAmount","成本金额","double","",true,minValue,999999999,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_PresentSampleType','赠(样)品','80',2,true,'',10,-2,'','',"","","33",'',"1",false); 
 putChildValidateItem("tblBuyOutStockDet_PresentSampleType","赠(样)品","any","BillPresentSampleType",true,minValue,10,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_SourceID','源ID','0',2,true,'',60,-2,'','',"","","34",'',"100",false); 
 putChildValidateItem("tblBuyOutStockDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 ...
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_ChangeAmt','调价金额','200',1,true,'0',0,-2,'','',"","","38",'',"3",false); 
 putChildValidateItem("tblBuyOutStockDet_ChangeAmt","调价金额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_AfterChangeAmt','调价后金额','200',1,true,'0',0,-2,'','',"","","40",'',"3",false); 
 putChildValidateItem("tblBuyOutStockDet_AfterChangeAmt","调价后金额","double","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_InvoiceAmount','已开票金额','200',1,true,'0',0,-2,'','',"AmountIdentifier","","43",'',"3",false); 
 putChildValidateItem("tblBuyOutStockDet_InvoiceAmount","已开票金额","double","",true,minValue,maxValue,true,'0'); 
 ...
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_NotPayQty','未付款数量','224',1,true,'',0,-2,'','',"Qty","","48",'',"3",false); 
 putChildValidateItem("tblBuyOutStockDet_NotPayQty","未付款数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_PayQty','付款数量','224',1,true,'',0,-2,'','',"Qty","","49",'',"3",false); 
 putChildValidateItem("tblBuyOutStockDet_PayQty","付款数量","double","",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","50",'',"3",false); 
 putChildValidateItem("tblBuyOutStockDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","51",'',"3",false); 
 putChildValidateItem("tblBuyOutStockDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_Remark','备注','100',2,true,'',8000,0,'','',"","","54",'',"0",false); 
 putChildValidateItem("tblBuyOutStockDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblBuyOutStockDetData,'tblBuyOutStockDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblBuyOutStock&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblBuyOutStock,tblBuyOutStockDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyOutStock&allTableName=tblBuyOutStock,tblBuyOutStockDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyOutStock&allTableName=tblBuyOutStockDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblBuyOutStock&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblBuyOutStock&allTableName=tblBuyOutStockDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblBuyOutStockDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblBuyOutStock&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblBuyOutStock&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblBuyOutStock&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblBuyOutStock&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblBuyOutStock&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '采购退货';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblBuyOutStock&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
