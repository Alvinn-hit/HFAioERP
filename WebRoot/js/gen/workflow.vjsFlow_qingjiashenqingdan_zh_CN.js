...
$(document).ready(function(){
 ...
 var imgUrl="if(popMainInput(''))openPopup('Flow_qingjiashenqingdan','"+$(this).attr('value')+"','popup_"+Id+"','"+Id+"','"+isMutiplse+"')";
 ...
 var imgUrl="if(popMainInput(''))openPopup('Flow_qingjiashenqingdan','"+$(this).attr('value')+"','"+popupNames+"','"+Id+"','"+isMutiplse+"')";
...
function checkTable(allHidFields){
 ...
 var labelText=$("#field_5",document);
 if(typeof(labelText)!="string"){
 labelText = labelText.val();
 }
 
 if(typeof(labelText)!= "undefined" && labelText.length == 0 && !(allHidFields.indexOf("field_5")>-1)){ 
 alert("结束日期"+"不能为空，请重新输入!");
 $("#field_5",document).focus();
 return false;
 }
 
 var labelText = "";
 $("input[name='field_8']",document).each(function(){
...
function countVal(Id){ 
 ...
 var result;
 if("field_6"==Id){
 var docText="date({field_4}-{field_5})";
 var re=/{.[^{}]*}/g; 
