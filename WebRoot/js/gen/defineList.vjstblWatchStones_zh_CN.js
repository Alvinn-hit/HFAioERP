...
function onLockClick(str){
 AjaxRequest("/UtilServlet?operation=lockColumn&report=tblWatchStones&number="+str); 
}
...
function beforeButtonQuery(){
 ...
 if(form.GoodsGrade.value!=''){
 alert("选择分级显示时，请不要输入除日期外非必须输入的查询条件");
 return false;
 }
 if(form.SampleStage.value!=''){
 alert("选择分级显示时，请不要输入除日期外非必须输入的查询条件");
 return false;
 }
 if(form.ComFullName.value!=''){
 alert("选择分级显示时，请不要输入除日期外非必须输入的查询条件");
 return false;
 }
 if(form.EmpFullName.value!=''){
 alert("选择分级显示时，请不要输入除日期外非必须输入的查询条件");
 return false;
 }
 if(form.SampleOrderNo.value!=''){
 alert("选择分级显示时，请不要输入除日期外非必须输入的查询条件");
 return false;
 }
 if(form.DeptFullName.value!=''){
 alert("选择分级显示时，请不要输入除日期外非必须输入的查询条件");
 return false;
 }
...
function printData(){
 ...
 var urls = "/UserFunctionQueryAction.do?tableName=tblWatchStones&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+keyVal.val()+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
...
function printLabelData(){
 ...
 var urls = "/UserFunctionQueryAction.do?tableName=tblWatchStones&reportNumber=PrintSeqtblWatchStones&printType=labelPrint&operation=18&keyId="+keyVal+"&parentTableName=tblSampleOrder&winCurIndex="+winCurIndex;
...
function setIframSrc(srcUrl,obj){
 ...
 if(window.parent.document.getElementById("bottomFrame") && "tblSampleOrder"==""){
...
function onItemClick(str){
 ...
 if(currentId!=str){
 currentId = str ;
 setIframSrc2("/UserFunctionAction.do?tableName=tblWatchStones&keyId="+str+"&operation=7&winCurIndex="+winCurIndex,currentId);
 }
}
...
function setColumnSet(str){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblWatchStones&colType=list&colSelect="+str ;
 AjaxRequest(strURL);
...
function onDefaultCol(){
 ...
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblWatchStones&colType=list" ;
 AjaxRequest(strURL);
 if(location.href.indexOf("src=menu")!=-1)
 {
 window.location.reload();
...
function copyOperation()
{
 ...
 usrc="/UserFunctionAction.do?&operation=17&tableName=tblWatchStones&saveDraft=&parentTableName=tblSampleOrder&moduleType="+moduleTypeValue+"&f_brother="+f_brother+"&noback=false&winCurIndex="+winCurIndex+"&keyId="+document.getElementById("varKeyIds").value.split("|")[0];
...
function telCall()
{
 ...
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblWatchStones&keyId="+keyIds,"","dialogWidth=600px;dialogHeight=350px"); 
 } 
 }else
 {
 alert("请选择至少一条记录"); 
...
function addPrepare(){
 ...
 usrc ='/UserFunctionAction.do?tableName=tblWatchStones&parentCode='+parentCode+'&operation=6&moduleType='+moduleTypeValue+'&f_brother='+f_brother+'&parentTableName=tblSampleOrder&winCurIndex='+winCurIndex;
...
function openSelect(selectName,fieldName,display){
 ...
 var urlstr = '/UserFunctionAction.do?operation=22&src=menu&parentTableName='+parentTableName+'&moduleType='+moduleType+'&displayName='+encodeURI(display)+'&LinkType=@URL:&tableName=tblWatchStones&selectName='+selectName+encodeURI(linkStr) ;
...
function showDivCustomSetTable(){ 
 var returnValue = window.showModalDialog("/UserFunctionQueryAction.do?tableName=tblWatchStones&operation=4&parentTableName=tblSampleOrder&configType=listConfig&winCurIndex="+winCurIndex,"","dialogWidth=670px;dialogHeight=450px") ;
...
function detail(str){
 turl="/UserFunctionAction.do?tableName=tblWatchStones&keyId="+str.split(",")[0]+"&moduleType="+moduleType+"&f_brother=0ff9f23a_1405151754180180003&operation=5&winCurIndex="+winCurIndex+"&pageNo=&parentCode="+str.split(",")[1]+"&parentTableName=tblSampleOrder&saveDraft=";
...
function update(str){
 turl="/UserFunctionAction.do?tableName=tblWatchStones&keyId="+str.split(",")[0]+"&moduleType="+moduleType+"&f_brother=0ff9f23a_1405151754180180003&operation=7&winCurIndex="+winCurIndex+"&pageNo=&parentCode="+str.split(",")[1]+"&parentTableName=tblSampleOrder&saveDraft=";
...
function next(str){
 location.href="/UserFunctionQueryAction.do?tableName=tblWatchStones&keyId="+str.split(",")[0]+"&parentCode="+str.split(",")[1]+"&operation=4&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&parentTableName=tblSampleOrder";
}
...
function addClass(parentCodev){
 ...
 usrc ='/UserFunctionAction.do?tableName=tblWatchStones&parentCode='+parentCodev+'&operation=6&moduleType='+moduleTypeValue+'&f_brother='+f_brother+'&parentTableName=tblSampleOrder&winCurIndex='+winCurIndex;
...
function hurryTrans(str){
 ...
 hurryTransDialog("/vm/oa/workflow/noteModel.jsp?tableName=tblWatchStones&moduleName="+encodeURI(ren)+"&keyId="+str.split(",")[0],winObj,"dialogWidth=565px;dialogHeight=350px");
}
...
function check(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblWatchStones&operation=25&ButtonType=saveCheckBill&varKeyIds="+str.split(",")[0]+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+str.split(",")[0]+"&tableName=tblWatchStones&operation=82&fromPage=erp");
 ...
 location.href="/UserFunctionQueryAction.do?tableName=tblWatchStones&moduleType="+moduleType+"&keyId="+str.split(",")[0]+"&f_brother=0ff9f23a_1405151754180180003&operation=83&winCurIndex="+winCurIndex+"&pageNo=&parentTableName=tblSampleOrder&saveDraft=";
 }else if(str.split(",")[1]=="cancel"){
 if(confirm('确定撤回?')){
 location.href='/UserFunctionQueryAction.do?nextStep=cancel&moduleType='+moduleType+'&tableName=tblWatchStones&lastNodeId='+str.split(",")[2]+'&keyId='+str.split(",")[0]+'&currNode='+str.split(",")[3]+'&f_brother=0ff9f23a_1405151754180180003&operation=79&winCurIndex='+winCurIndex+'&pageNo=&parentTableName=tblSampleOrder&saveDraft='
 }
 }
}
