...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=CRMClientInfo&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=CRMClientInfo&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=CRMClientInfo&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=CRMClientInfo&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "CRMClientInfoDetTable"){
 }
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "CRMClientInfoEmpTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="CRMClientInfoDet"){
 
  }
 if(typeof(tableName)!="undefined" && tableName=="CRMClientInfoEmp"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="CRMClientInfoDet"){
 
  }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="CRMClientInfoEmp"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="CRMClientInfoDet"; 
 tblName[1]="CRMClientInfoEmp"; 
...
function execStat2(){
 execStatOne('CRMClientInfoDet_UserName',2);
 execStatOne('CRMClientInfoDet_MSN',2);
 execStatOne('CRMClientInfoDet_Telephone',2);
 execStatOne('CRMClientInfoDet_Mobile',2);
 execStatOne('CRMClientInfoDet_ClientEmail',2);
 execStatOne('CRMClientInfoDet_QQ',2);
 execStatOne('CRMClientInfoDet_BirthDay',2);
 execStatOne('CRMClientInfoDet_Hobby',2);
 execStatOne('CRMClientInfoDet_NativePlace',2);
 execStatOne('CRMClientInfoDet_Remark',2);
 setTotalCount(getTotalCount());
}
...
function openSelect(urlstr,obj,field){
 ...
 urlstr += "&src=menu&MOID=7fcd062e_1012181720454843489&MOOP=add&popupWin=MainPopup";
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr+'&src=menu&MOID=7fcd062e_1012181720454843489&MOOP=add'; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"CRMClientInfo\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" width=\"150\" border=\"0\"></a></div><div><em>"+str+"</em><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"CRMClientInfo\",\"PIC\");'>删除</a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=CRMClientInfo",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=CRMClientInfo&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=CRMClientInfo&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function mainSelect2(urlstr,openUrl,obj,field,isChild)
{
 ...
 MOID:"7fcd062e_1012181720454843489",
 MOOP:"add",
...
function showUserGroup(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=7fcd062e_1012181720454843489&MOOP=add&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=730px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=7fcd062e_1012181720454843489&MOOP=query&LinkType=@URL:&displayName="+displayName,winObj,"dialogWidth=830px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function openCRMSelect(selectName,displayName){
 ...
 var str = window.showModalDialog("/UserFunctionAction.do?selectName="+selectName+"&operation=22&MOID=7fcd062e_1012181720454843489&MOOP=add&LinkType=@URL:&displayName="+displayName,"","dialogWidth=730px;dialogHeight=450px"); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "CRMClientInfo"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=CRMClientInfo&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=CRMClientInfo&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='CRMClientInfoDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='客户联系人明细';
childCount++;
 var CRMClientInfoDetData = new gridData();
 gridDatas[gridDataNum] = CRMClientInfoDetData;
 gridDataNum++;
 var CRMClientInfoDet=1;
 ...
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_UserName','联系人','60',2,false,'',50,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("CRMClientInfoDet_UserName","联系人","any","",false,minValue,50,false,'',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_MSN','部门','80',2,true,'',50,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("CRMClientInfoDet_MSN","部门","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_Role','角色','60',2,true,'',50,1,';11:职员;1:法人;10:董事;9:董事长;2:总经理;3:部门经理;5:副经理;4:部长;6:副部长;7:主管;8:副主管;12:其他','',"","","3",'',"1",false);
 putChildValidateItem("CRMClientInfoDet_Role","角色","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_Gender','性别','50',2,true,'1',0,1,';1:男;2:女','',"","","4",'',"1",false);
 putChildValidateItem("CRMClientInfoDet_Gender","性别","any","",true,minValue,maxValue,true,'1',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_Telephone','电话','100',2,true,'',50,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("CRMClientInfoDet_Telephone","电话","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_Mobile','手机','90',2,true,'',50,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("CRMClientInfoDet_Mobile","手机","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_ClientEmail','邮件地址','120',8,true,'',50,0,'','',"","","7",'',"0",false); 
 putChildValidateItem("CRMClientInfoDet_ClientEmail","邮件地址","mail","",true,minValue,50,false,'',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_QQ','QQ','80',2,true,'',50,0,'','',"","","8",'',"0",false); 
 putChildValidateItem("CRMClientInfoDet_QQ","QQ","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_dateType','日期类型','100',2,true,'1',10,1,';1:公历;2:农历','',"","","9",'',"1",false);
 putChildValidateItem("CRMClientInfoDet_dateType","日期类型","any","",true,minValue,10,true,'1',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_BirthDay','生日','70',5,true,'',10,0,'','',"","","10",'',"0",false); 
 putChildValidateItem("CRMClientInfoDet_BirthDay","生日","date","",true,minValue,10,false,'',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_mainUser','主联系人','80',2,true,'',10,1,';1:是;2:否','',"","","11",'',"1",false);
 putChildValidateItem("CRMClientInfoDet_mainUser","主联系人","any","",true,minValue,10,false,'',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_Hobby','爱好','130',2,true,'',500,0,'','',"","","12",'',"0",false); 
 putChildValidateItem("CRMClientInfoDet_Hobby","爱好","any","",true,minValue,500,false,'',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_NativePlace','籍贯','100',2,true,'',200,0,'','',"","","13",'',"0",false); 
 putChildValidateItem("CRMClientInfoDet_NativePlace","籍贯","any","",true,minValue,200,false,'',true); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_Remark','备注','130',2,true,'',8000,0,'','',"","","80",'',"0",false); 
 putChildValidateItem("CRMClientInfoDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_ContactNo','联系人编号','200',2,true,'',100,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("CRMClientInfoDet_ContactNo","联系人编号","any","",true,minValue,100,false,''); 
 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_AreaCode','区号','224',2,true,'',20,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("CRMClientInfoDet_AreaCode","区号","any","",true,minValue,20,false,''); 
 
 
 addCols(CRMClientInfoDetData,'CRMClientInfoDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='CRMClientInfoEmp';
tabIsNulls[childCount]='0';
tabDiss[childCount]='客户跟单人明细';
childCount++;
 var CRMClientInfoEmpData = new gridData();
 gridDatas[gridDataNum] = CRMClientInfoEmpData;
 gridDataNum++;
 var CRMClientInfoEmp=1;
 ...
 addCols(CRMClientInfoEmpData,'CRMClientInfoEmp_DepartmentCode','部门','80',2,true,'',50,-2,'','',"","","1",'',"2",true);
 putChildValidateItem("CRMClientInfoEmp_DepartmentCode","部门","any","",true,minValue,50,false,'');
 addCols(CRMClientInfoEmpData,'CRMClientInfoEmp_tblDepartment_DeptFullName','部门','80',2,true,'',1000000,2,'CRMClientInfoEmp:DepartmentCode:CRMClientInfoEmp_tblDepartment_DeptFullName:'+param4,'',"","","1",'',"2",false); 
 ...
 var param4="";
 param4=param4+":0部门@CRMClientInfoEmp_DepartmentCode";
 
 addCols(CRMClientInfoEmpData,'CRMClientInfoEmp_EmployeeID','跟单人','80',2,true,'',50,-2,'','',"","","2",'',"2",true);
 putChildValidateItem("CRMClientInfoEmp_EmployeeID","跟单人","any","",true,minValue,50,false,'');
 addCols(CRMClientInfoEmpData,'CRMClientInfoEmp_tblEmployee_EmpFullName','跟单人','120',2,true,'',1000000,2,'CRMClientInfoEmp:EmployeeID:CRMClientInfoEmp_tblEmployee_EmpFullName:'+param4,'',"","","2",'',"2",false); 
 
 
 
 addCols(CRMClientInfoEmpData,'CRMClientInfoEmp_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=CRMClientInfo&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "CRMClientInfo,CRMClientInfoDet,CRMClientInfoEmp,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=CRMClientInfo&allTableName=CRMClientInfo,CRMClientInfoDet,CRMClientInfoEmp,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=CRMClientInfo&allTableName=CRMClientInfoDet,CRMClientInfoEmp,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=CRMClientInfo&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=CRMClientInfo&allTableName=CRMClientInfoDet,CRMClientInfoEmp,&colType=colWidth" ;
...
function showDept(){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=7fcd062e_1012181720454843489&MOOP=add",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var depts = str.split("|") ;
...
function showEmployee(fieldName,fieldNameIds){
 ...
 var str = window.showModalDialog(urlstr+"&MOID=7fcd062e_1012181720454843489&MOOP=query&LinkType=@URL:",winObj,"dialogWidth=750px;dialogHeight=450px"); 
 if(typeof(str)=="undefined") return ;
 var employees = str.split("|") ;
 for(var j=0;j<employees.length;j++){
 var field = employees[j].split(";") ;
...
function seqInput(){
 ...
 var urls="/UtilServlet?operation=validateBPSeq&MOID=7fcd062e_1012181720454843489&MOOP=add&StockCode="+stockCode+"&Seq="+obj.value+"&popupBean="+popBeanName;
 AjaxRequest(urls) ;
...
function RecAutoSettlement(){
 ...
 var url=url+"&operation=RecAutoSettlement&CompanyCode="+companyCode+"&"+AcceptTypeIDStr+"="+AcceptTypeID+"&BillDate="+BillDate+"&MOID=7fcd062e_1012181720454843489&MOOP=add&selectField=&selectValue=";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=CRMClientInfo&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=CRMClientInfo&designId=8D417922BCB143E8936D68400CDF&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=CRMClientInfo&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=CRMClientInfo&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=CRMClientInfo&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '客户资料(默认模版)';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=CRMClientInfo&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
