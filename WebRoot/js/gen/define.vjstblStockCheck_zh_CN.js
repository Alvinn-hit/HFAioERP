...
{ 
 cFireEvent('tblStockCheckDet_Qty',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblStockCheck&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblStockCheck&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblStockCheck&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblStockCheck&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function addSeq(o,keyCode){
 ...
 var goodscodestr="";
 var isInstore = false ;
 if(seqFname=="tblStockCheckDet_Seq"){
 var ss="@GN:Seq:Qty:tblStockCheckDet_GoodsSeqSet:tblStockCheckDet_GoodsCode:StockCode".split(":")
 if(ss[0]=="@GN"){
 goodscodestr=ss[4];
...
if(isInstore){
 ...
 if(seqFname=="tblStockCheckDet_Seq"){
 var qtys=n('tblStockCheckDet'+"_"+'Qty');
...
function returnSeq(colNames,returnVales,compareVale,goodscodestr,ri,seqFname,inpVal){
 ...
 if("tblStockCheckDet"==seqFname.substr(0,seqFname.lastIndexOf("_"))){ 
 var qtys=n("tblStockCheckDet"+"_"+'Qty');
 qtys[ri].value="1";
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblStockCheckDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 var z=n('tblStockCheckDet_GoodsCode');
 ...
 var z=n('tblStockCheckDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblStockCheckDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblStockCheckDet"){
 
 copyFields=n('tblStockCheckDet_GoodsCode');
 ...
 var tblStockCheckDet_tblGoods_GoodsNumber = n('tblStockCheckDet_tblGoods_GoodsNumber');
 if(typeof(tblStockCheckDet_tblGoods_GoodsNumber) != 'undefined' && tblStockCheckDet_tblGoods_GoodsNumber.length >index){
 tblStockCheckDet_tblGoods_GoodsNumber[index].value = tblStockCheckDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblStockCheckDet_tblGoods_GoodsFullName = n('tblStockCheckDet_tblGoods_GoodsFullName');
 if(typeof(tblStockCheckDet_tblGoods_GoodsFullName) != 'undefined' && tblStockCheckDet_tblGoods_GoodsFullName.length >index){
 tblStockCheckDet_tblGoods_GoodsFullName[index].value = tblStockCheckDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblStockCheckDet_tblGoods_GoodsSpec = n('tblStockCheckDet_tblGoods_GoodsSpec');
 if(typeof(tblStockCheckDet_tblGoods_GoodsSpec) != 'undefined' && tblStockCheckDet_tblGoods_GoodsSpec.length >index){
 tblStockCheckDet_tblGoods_GoodsSpec[index].value = tblStockCheckDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblStockCheckDet_tblGoods_BaseUnit = n('tblStockCheckDet_tblGoods_BaseUnit');
 if(typeof(tblStockCheckDet_tblGoods_BaseUnit) != 'undefined' && tblStockCheckDet_tblGoods_BaseUnit.length >index){
 tblStockCheckDet_tblGoods_BaseUnit[index].value = tblStockCheckDet_tblGoods_BaseUnit[index-1].value;
 }
 var tblStockCheckDet_tblGoods_BarCode = n('tblStockCheckDet_tblGoods_BarCode');
 if(typeof(tblStockCheckDet_tblGoods_BarCode) != 'undefined' && tblStockCheckDet_tblGoods_BarCode.length >index){
 tblStockCheckDet_tblGoods_BarCode[index].value = tblStockCheckDet_tblGoods_BarCode[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblStockCheckDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblStockCheckDet"; 
 var goodsCodeLogicValidate_tblStockCheckDet='@GN:Seq:Qty:tblStockCheckDet_GoodsSeqSet:tblStockCheckDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'tblStockCheckDet',value:goodsCodeLogicValidate_tblStockCheckDet};
...
function execStat2(){
 execStatOne('tblStockCheckDet_Qty',4);
 setTotalCount(getTotalCount());
}
...
function openChildSelect(urlstr,obj,field,thisField,current){
 ...
 urlstr=urlstr.substring(0,index+"&seq=".length)+n(thisField.substr(0,thisField.lastIndexOf("_")))[ri].value+"&src=menu&seqFname="+thisField+"&newSeq="+mi[ri].value+"&tableName=tblStockCheck&childTableName="+thisField.split("_")[0]+"&fieldName="+field;
 var goodscode="";
 var goodscodestr="";
 var goodsProps = "";
 var stockCode = "" ;
 ...
 if(thisField=="tblStockCheckDet_Seq_hid"){
 ...
 stockCode = nz("tblStockCheckDet"+"_"+ss[5],ri) ;
 }
 }
 goodsProps += "BatchNo="+nz("tblStockCheckDet"+"_BatchNo",ri)+"&"
 goodsProps += "yearNO="+nz("tblStockCheckDet"+"_yearNO",ri)+"&"
 var ss="@IOO:Instore".split(":")
 if(ss[0]=="@GN"){
 goodscodestr=ss[4];
 goodscode=n(goodscodestr)[ri].value;
 ...
 AjaxRequest(urlsgood) ;
 if("false"==response){
 asyncbox.tips("该商品未启用序列号12！！","alert",2000) ;
 return ;
 }
 }
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblStockCheck\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblStockCheck\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblStockCheckDet_LastQty',"tblStockCheckDet"); 
 delFireEventOne(rowId,tableName,'tblStockCheckDet_Qty',"tblStockCheckDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'tblStockCheckDet_LastQty',dis:'当前库存',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblStockCheckDet_Qty',dis:'盘点数',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblStockCheckDet_OCQty',dis:'盈亏数',digit:4,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblStockCheck",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblStockCheck&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblStockCheck&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function resetSubmit(fieldName){
 ...
 var saveField=n("CheckDate"); 
 saveField[0].value = "";
 var obj = m("tbl_tblStockCheckPrepare_CheckDate");
 if(obj != null){
 obj.value = "";
 }
 ...
 var saveField=n("PrepareId"); 
 saveField[0].value = "";
 var obj = m("tbl_tblStockCheckPrepare_id");
 if(obj != null){
 obj.value = "";
 }
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('tblStockCheckDet_Qty'); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblStockCheck"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblStockCheck&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblStockCheck&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblStockCheckDet_LastQty'); 
 r(0,"OCQty")[p].value=f(rn(0,"Qty",p)-rn(0,"LastQty",p),4);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblStockCheckDet_Qty'); 
 r(0,"OCQty")[p].value=f(rn(0,"Qty",p)-rn(0,"LastQty",p),4);
...
childData[childCount]='tblStockCheckDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='库存盘点明细';
childCount++;
 var tblStockCheckDetData = new gridData();
 gridDatas[gridDataNum] = tblStockCheckDetData;
 gridDataNum++;
 var tblStockCheckDet=1;
 ...
 param4=param4+":0盘点准备单ID@PrepareId"; 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_GoodsCode','料件名称','120',2,false,'',100,-2,'','',"","triggerOrCopy","1",'',"2",true);
 putChildValidateItem("tblStockCheckDet_GoodsCode","料件名称","any","",false,minValue,100,false,'');
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_tblGoods_GoodsNumber','料件编号','150',2,true,'',1000000,2,'tblStockCheckDet:GoodsCode:tblStockCheckDet_tblGoods_GoodsNumber:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_tblGoods_GoodsFullName','料件名称','150',2,true,'',1000000,2,'tblStockCheckDet:GoodsCode:tblStockCheckDet_tblGoods_GoodsFullName:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_tblGoods_GoodsSpec','规格','200',20,true,'',1000000,2,'tblStockCheckDet:GoodsCode:tblStockCheckDet_tblGoods_GoodsSpec:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblStockCheckDet:GoodsCode:tblStockCheckDet_tblGoods_BaseUnit:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_tblGoods_BarCode','条码','100',2,true,'',1000000,2,'tblStockCheckDet:GoodsCode:tblStockCheckDet_tblGoods_BarCode:'+param4,'',"","triggerOrCopy","1",'',"2",false); 
 ...
 var param4="";
 param4=param4+":1仓库@StockCode"; 
 param4=param4+":1料件名称@tblStockCheckDet_GoodsCode";
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_BatchNo','批号','120',2,true,'',100,2,'tblStockCheckDet:BatchNo:tblStockCheckDet_BatchNo:'+param4,"","","triggerCopy","3",'',"2",true);
 putChildValidateItem("tblStockCheckDet_BatchNo","批号","any","GoodsField",true,minValue,100,false,'',true);
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","4",'',"2",false); 
 putChildValidateItem("tblStockCheckDet_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","5",'',"2",false); 
 putChildValidateItem("tblStockCheckDet_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_color','色彩','70',2,true,'',100,-2,'','',"","","5",'',"2",false); 
 putChildValidateItem("tblStockCheckDet_color","色彩","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_colorNV','顏色名','70',2,true,'',100,-2,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblStockCheckDet_colorNV","顏色名","any","",true,minValue,100,false,''); 
 ...
 addCols(tblStockCheckDetData,'tblStockCheckDet_yearNO','箱号','100',2,true,'',100,2,'tblStockCheckDet:yearNO:tblStockCheckDet_yearNO:'+param4,"","","triggerCopy","7",'',"2",true);
 putChildValidateItem("tblStockCheckDet_yearNO","箱号","any","GoodsField",true,minValue,100,false,'',true);
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblStockCheckDet_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_Availably','保质期限','80',5,true,'',10,-2,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblStockCheckDet_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 ...
 addCols(tblStockCheckDetData,'tblStockCheckDet_Seq','物流码','130',2,true,'',5000,-2,'','',"","triggerCopy","15",'',"2",false);
 addCols(tblStockCheckDetData,'tblStockCheckDet_Seq'+'_hid','物流码','130',2,true,'',5000,2,'@SEQ@IOO:Instore',"","","triggerCopy","15",'',"2",false);
 putChildValidateItem("tblStockCheckDet_Seq","物流码","any","GoodsField",true,minValue,5000,false,'',true);
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_LastQty','当前库存','80',1,true,'0',0,8,'','',"Qty","","19",'',"8",false); 
 putChildValidateItem("tblStockCheckDet_LastQty","当前库存","double","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_Qty','盘点数','80',1,false,'',0,0,'','',"Qty","","21",'',"0",false); 
 putChildValidateItem("tblStockCheckDet_Qty","盘点数","double","SeqQty",false,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_OCQty','盈亏数','80',1,true,'',0,8,'','',"Qty","","25",'',"0",false); 
 putChildValidateItem("tblStockCheckDet_OCQty","盈亏数","double","",true,minValue,maxValue,false,'',true); 
 ...
 addCols(tblStockCheckDetData,'tblStockCheckDet_SourceID','源ID','200',2,true,'',60,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblStockCheckDet_SourceID","源ID","any","",true,minValue,60,false,''); 
 
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblStockCheckDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','',"","","100",'',"3",false); 
 putChildValidateItem("tblStockCheckDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_Remark','说明','130',2,true,'',8000,0,'','',"","","120",'',"0",false); 
 putChildValidateItem("tblStockCheckDet_Remark","说明","any","",true,minValue,8000,false,'',true); 
 
 
 addCols(tblStockCheckDetData,'tblStockCheckDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblStockCheck&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblStockCheck,tblStockCheckDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblStockCheck&allTableName=tblStockCheck,tblStockCheckDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblStockCheck&allTableName=tblStockCheckDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblStockCheck&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblStockCheck&allTableName=tblStockCheckDet,&colType=colWidth" ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblStockCheck&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblStockCheck&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblStockCheck&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblStockCheck&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '盘点录入单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblStockCheck&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
