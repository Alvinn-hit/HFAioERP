...
{ 
 cFireEvent('tblAllotChangeDet_Qty',''); 
 cFireEvent('tblAllotChangeDet_Price',''); 
 }

function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblAllotChange&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblAllotChange&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblAllotChange&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblAllotChange&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblAllotChangeDetTable"){
 }
 }
function totalA(obj)
{
...
function initCountCal(){
 ...
 var z=n('tblAllotChangeDet_GoodsCode');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblAllotChangeDet_BatchNo');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblAllotChangeDet_Inch');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
 ...
 var z=n('tblAllotChangeDet_yearNO');
 for(i = 0;i<z.length;i++){
 if(navigator.userAgent.indexOf("MSIE")!=-1){
 z[i].attachEvent("onblur",copyFields);
 }else{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblAllotChangeDet"){
 
 copyFields=n('tblAllotChangeDet_StockCode');
 ...
 var tblAllotChangeDet_tblStock_StockFullName1 = n('tblAllotChangeDet_tblStock_StockFullName1');
 if(typeof(tblAllotChangeDet_tblStock_StockFullName1) != 'undefined' && tblAllotChangeDet_tblStock_StockFullName1.length >index){
 tblAllotChangeDet_tblStock_StockFullName1[index].value = tblAllotChangeDet_tblStock_StockFullName1[index-1].value;
 }
 }
 copyFields=n('tblAllotChangeDet_InStockCode');
 if((order > 3 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblAllotChangeDet_tblStockDet_StockFullNameDetIn = n('tblAllotChangeDet_tblStockDet_StockFullNameDetIn');
 if(typeof(tblAllotChangeDet_tblStockDet_StockFullNameDetIn) != 'undefined' && tblAllotChangeDet_tblStockDet_StockFullNameDetIn.length >index){
 tblAllotChangeDet_tblStockDet_StockFullNameDetIn[index].value = tblAllotChangeDet_tblStockDet_StockFullNameDetIn[index-1].value;
 }
 }
 copyFields=n('tblAllotChangeDet_GoodsCode');
 if((order > 6 || (typeof(copyAll)!="undefined" && "copyAll"==copyAll)) && typeof(copyFields[index])!="undefined" && (copyisNull(copyFields[index].value))){
 copyFields[index].value = copyFields[index-1].value;
 var tblAllotChangeDet_tblGoods_GoodsNumber = n('tblAllotChangeDet_tblGoods_GoodsNumber');
 if(typeof(tblAllotChangeDet_tblGoods_GoodsNumber) != 'undefined' && tblAllotChangeDet_tblGoods_GoodsNumber.length >index){
 tblAllotChangeDet_tblGoods_GoodsNumber[index].value = tblAllotChangeDet_tblGoods_GoodsNumber[index-1].value;
 }
 var tblAllotChangeDet_tblGoods_GoodsFullName = n('tblAllotChangeDet_tblGoods_GoodsFullName');
 if(typeof(tblAllotChangeDet_tblGoods_GoodsFullName) != 'undefined' && tblAllotChangeDet_tblGoods_GoodsFullName.length >index){
 tblAllotChangeDet_tblGoods_GoodsFullName[index].value = tblAllotChangeDet_tblGoods_GoodsFullName[index-1].value;
 }
 var tblAllotChangeDet_tblGoods_GoodsSpec = n('tblAllotChangeDet_tblGoods_GoodsSpec');
 if(typeof(tblAllotChangeDet_tblGoods_GoodsSpec) != 'undefined' && tblAllotChangeDet_tblGoods_GoodsSpec.length >index){
 tblAllotChangeDet_tblGoods_GoodsSpec[index].value = tblAllotChangeDet_tblGoods_GoodsSpec[index-1].value;
 }
 var tblAllotChangeDet_tblGoods_BaseUnit = n('tblAllotChangeDet_tblGoods_BaseUnit');
 if(typeof(tblAllotChangeDet_tblGoods_BaseUnit) != 'undefined' && tblAllotChangeDet_tblGoods_BaseUnit.length >index){
 tblAllotChangeDet_tblGoods_BaseUnit[index].value = tblAllotChangeDet_tblGoods_BaseUnit[index-1].value;
 }
 var tblAllotChangeDet_tblGoods_Spec1 = n('tblAllotChangeDet_tblGoods_Spec1');
 if(typeof(tblAllotChangeDet_tblGoods_Spec1) != 'undefined' && tblAllotChangeDet_tblGoods_Spec1.length >index){
 tblAllotChangeDet_tblGoods_Spec1[index].value = tblAllotChangeDet_tblGoods_Spec1[index-1].value;
 }
 var tblAllotChangeDet_tblGoods_Spec2 = n('tblAllotChangeDet_tblGoods_Spec2');
 if(typeof(tblAllotChangeDet_tblGoods_Spec2) != 'undefined' && tblAllotChangeDet_tblGoods_Spec2.length >index){
 tblAllotChangeDet_tblGoods_Spec2[index].value = tblAllotChangeDet_tblGoods_Spec2[index-1].value;
 }
 var tblAllotChangeDet_tblStocks_lastQty = n('tblAllotChangeDet_tblStocks_lastQty');
 if(typeof(tblAllotChangeDet_tblStocks_lastQty) != 'undefined' && tblAllotChangeDet_tblStocks_lastQty.length >index){
 tblAllotChangeDet_tblStocks_lastQty[index].value = tblAllotChangeDet_tblStocks_lastQty[index-1].value;
 }
 var tblAllotChangeDet_ViewVirtualStock_UseQty = n('tblAllotChangeDet_ViewVirtualStock_UseQty');
 if(typeof(tblAllotChangeDet_ViewVirtualStock_UseQty) != 'undefined' && tblAllotChangeDet_ViewVirtualStock_UseQty.length >index){
 tblAllotChangeDet_ViewVirtualStock_UseQty[index].value = tblAllotChangeDet_ViewVirtualStock_UseQty[index-1].value;
 }
 }
  }
 }
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblAllotChangeDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblAllotChangeDet"; 
 var goodsCodeLogicValidate_tblAllotChangeDet='@GN:Seq:Qty:tblAllotChangeDet_GoodsSeqSet:tblAllotChangeDet_GoodsCode:StockCode';
 goodsCodeLogicValidate[goodsCodeLogicValidate.length] = {key:'tblAllotChangeDet',value:goodsCodeLogicValidate_tblAllotChangeDet};
...
function execStat2(){
 execStatOne('tblAllotChangeDet_Qty',2);
 execStatOne('tblAllotChangeDet_Amt',2);
 execStatOne('tblAllotChangeDet_RePrice',4);
 execStatOne('tblAllotChangeDet_OutAmt',2);
 execStatOne('tblAllotChangeDet_ReAmt',2);
 execStatOne('tblAllotChangeDet_RetailAmt',2);
 setTotalCount(getTotalCount());
}
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblAllotChange\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblAllotChange\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblAllotChangeDet_Qty',"tblAllotChangeDet"); 
 delFireEventOne(rowId,tableName,'tblAllotChangeDet_Price',"tblAllotChangeDet"); 
 delFireEventOne(rowId,tableName,'tblAllotChangeDet_RePrice',"tblAllotChangeDet"); 
 delFireEventOne(rowId,tableName,'tblAllotChangeDet_OutAmt',"tblAllotChangeDet"); 
 delFireEventOne(rowId,tableName,'tblAllotChangeDet_RetailAmt',"tblAllotChangeDet"); 
 delFireEventOne(rowId,tableName,'tblAllotChangeDet_RetailPrice',"tblAllotChangeDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'BillAmtChange',dis:'差异金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblAllotChangeDet_Qty',dis:'调出数量',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAllotChangeDet_Price',dis:'调出单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAllotChangeDet_Amt',dis:'调出金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAllotChangeDet_RePrice',dis:'转入单价',digit:4,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAllotChangeDet_OutAmt',dis:'转入金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAllotChangeDet_ReAmt',dis:'差异金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblAllotChangeDet_RetailAmt',dis:'零售金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblAllotChange",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAllotChange&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblAllotChange&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function initSeqQtySet(){
 if(false){
  ...
  if(false){
 var fields=n('tblAllotChangeDet_Qty'); 
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblAllotChange"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblAllotChange&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblAllotChange&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initCalculate(){
 ...
 var p =getrp(this,'tblAllotChangeDet_Qty'); 
 r(0,"Amt")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
r(0,"OutAmt")[p].value=f(rn(0,"RePrice",p)*rn(0,"Qty",p),2);
r(0,"ReAmt")[p].value=f(rn(0,"OutAmt",p)-rn(0,"Amt",p),2);
n('BillAmtChange')[0].value=f(sm('tblAllotChangeDet_ReAmt'),2);
r(0,"RetailAmt")[p].value=f(rn(0,"RetailPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotChangeDet_Price'); 
 r(0,"Amt")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
n('BillAmtChange')[0].value=f(sm('tblAllotChangeDet_ReAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotChangeDet_RePrice'); 
 r(0,"Amt")[p].value=f(rn(0,"Price",p)*rn(0,"Qty",p),2);
...
n('BillAmtChange')[0].value=f(sm('tblAllotChangeDet_ReAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotChangeDet_OutAmt'); 
 r(0,"RePrice")[p].value=f(rn(0,"OutAmt",p)/rn(0,"Qty",p),4);
...
n('BillAmtChange')[0].value=f(sm('tblAllotChangeDet_ReAmt'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotChangeDet_RetailAmt'); 
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotChangeDet_RetailPrice'); 
 r(0,"RetailAmt")[p].value=f(rn(0,"RetailPrice",p)*rn(0,"Qty",p),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblAllotChangeDet_SecQty'); 
...
childData[childCount]='tblAllotChangeDet';
tabIsNulls[childCount]='1';
tabDiss[childCount]='变价调拨单明细';
childCount++;
 var tblAllotChangeDetData = new gridData();
 gridDatas[gridDataNum] = tblAllotChangeDetData;
 gridDataNum++;
 var tblAllotChangeDet=1;
 ...
 param4=param4+":0出库仓库@tblAllotChangeDet_StockCode";
 
 addCols(tblAllotChangeDetData,'tblAllotChangeDet_tblGoods_GoodsNumber','商品编号','100',2,true,'',1000000,2,'tblAllotChangeDet:GoodsCode:tblAllotChangeDet_tblGoods_GoodsNumber:'+param4,"","","triggerOrCopy","6","","2",false);
 ...
 addCols(tblAllotChangeDetData,'tblAllotChangeDet_tblGoods_GoodsFullName','商品名称','150',18,true,'',1000000,2,'tblAllotChangeDet:GoodsCode:tblAllotChangeDet_tblGoods_GoodsFullName:'+param4,"","","triggerOrCopy","6","","2",false);
 ...
 addCols(tblAllotChangeDetData,'tblAllotChangeDet_tblGoods_GoodsSpec','商品规格','200',2,true,'',1000000,2,'tblAllotChangeDet:GoodsCode:tblAllotChangeDet_tblGoods_GoodsSpec:'+param4,"","","triggerOrCopy","6","","2",false);
 ...
 addCols(tblAllotChangeDetData,'tblAllotChangeDet_tblGoods_BaseUnit','单位','30',2,true,'',1000000,2,'tblAllotChangeDet:GoodsCode:tblAllotChangeDet_tblGoods_BaseUnit:'+param4,"","","triggerOrCopy","6","","2",false);
 ...
 addCols(tblAllotChangeDetData,'tblAllotChangeDet_tblGoods_Spec1','英文名','80',2,true,'',1000000,2,'tblAllotChangeDet:GoodsCode:tblAllotChangeDet_tblGoods_Spec1:'+param4,"","","triggerOrCopy","6","","2",false);
 ...
 addCols(tblAllotChangeDetData,'tblAllotChangeDet_tblStocks_lastQty','实际库存','90',1,true,'',1000000,2,'tblAllotChangeDet:GoodsCode:tblAllotChangeDet_tblStocks_lastQty:'+param4,"","","triggerOrCopy","6","","2",false);
 ...
 addCols(tblAllotChangeDetData,'tblAllotChangeDet_ViewVirtualStock_UseQty','虚拟库存','90',1,true,'',1000000,2,'tblAllotChangeDet:GoodsCode:tblAllotChangeDet_ViewVirtualStock_UseQty:'+param4,"","","triggerOrCopy","6","","2",false);
    ...
    addCols(tblAllotChangeDetData,'tblAllotChangeDet_Qty','调出数量','80',1,false,'',0,0,'','',"Qty","","36",'',"0",false); 
 putChildValidateItem("tblAllotChangeDet_Qty","调出数量","double","SeqQty",false,minValue,maxValue,false,'',true); 
    addCols(tblAllotChangeDetData,'tblAllotChangeDet_RePrice','转入单价','80',1,true,'',0,0,'','',"priceIdentifier","","45",'',"0",false); 
 putChildValidateItem("tblAllotChangeDet_RePrice","转入单价","double","",true,minValue,maxValue,false,'',true); 
    addCols(tblAllotChangeDetData,'tblAllotChangeDet_OutAmt','转入金额','80',1,true,'',0,0,'','',"AmountIdentifier","","48",'',"0",false); 
 putChildValidateItem("tblAllotChangeDet_OutAmt","转入金额","double","",true,minValue,maxValue,false,'',true); 
    addCols(tblAllotChangeDetData,'tblAllotChangeDet_Amt','调出金额','80',1,true,'',0,8,'','',"AmountIdentifier","","42",'',"8",false); 
 putChildValidateItem("tblAllotChangeDet_Amt","调出金额","double","",true,minValue,maxValue,false,'',true); 
    addCols(tblAllotChangeDetData,'tblAllotChangeDet_Price','调出单价','80',1,true,'',0,8,'','',"priceIdentifier","","39",'',"0",false); 
 putChildValidateItem("tblAllotChangeDet_Price","调出单价","double","",true,minValue,maxValue,false,'',true); 
    addCols(tblAllotChangeDetData,'tblAllotChangeDet_ReAmt','差异金额','80',1,true,'',0,8,'','',"AmountIdentifier","","51",'',"8",false); 
 putChildValidateItem("tblAllotChangeDet_ReAmt","差异金额","double","",true,minValue,maxValue,false,'',true); 
    addCols(tblAllotChangeDetData,'tblAllotChangeDet_Remark','备注','130',2,true,'',8000,0,'','',"","","120",'',"0",false); 
 putChildValidateItem("tblAllotChangeDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
    addCols(tblAllotChangeDetData,'tblAllotChangeDet_Seq','序列号','130',2,true,'',5000,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblAllotChangeDet_Seq","序列号","any","",true,minValue,5000,false,''); 
   addCols(tblAllotChangeDetData,'tblAllotChangeDet_StockCode','出库仓库','80',2,true,'',100,-2,'','','',"copy",'','',"3",false); 
 putChildValidateItem("tblAllotChangeDet_StockCode","出库仓库","any","",true,minValue,100,true,''); 
    addCols(tblAllotChangeDetData,'tblAllotChangeDet_InStockCode','入库仓库','80',2,true,'',100,-2,'','','',"copy",'','',"3",false); 
 putChildValidateItem("tblAllotChangeDet_InStockCode","入库仓库","any","",true,minValue,100,false,''); 
 var param4="";
 addCols(tblAllotChangeDetData,'tblAllotChangeDet_GoodsCode','商品名称','120',2,false,'',200,-2,'','','',"triggerOrCopy",'','',"2",false);
 putChildValidateItem("tblAllotChangeDet_GoodsCode","商品名称","any","",false,minValue,200,false,'',true);
                addCols(tblAllotChangeDetData,'tblAllotChangeDet_tblGoods_Spec2','商品条码','120',2,false,'',200,-2,'',"2",false);
        addCols(tblAllotChangeDetData,'tblAllotChangeDet_BatchNo','批号','120',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblAllotChangeDet_BatchNo","批号","any","",true,minValue,100,false,''); 
   addCols(tblAllotChangeDetData,'tblAllotChangeDet_Inch','厚度','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblAllotChangeDet_Inch","厚度","any","",true,minValue,100,false,''); 
   addCols(tblAllotChangeDetData,'tblAllotChangeDet_Hue','颜色','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblAllotChangeDet_Hue","颜色","any","",true,minValue,100,false,''); 
   addCols(tblAllotChangeDetData,'tblAllotChangeDet_color','色彩','60',2,true,'',100,-2,'','','',"",'','',"2",false); 
 putChildValidateItem("tblAllotChangeDet_color","色彩","any","",true,minValue,100,false,''); 
   addCols(tblAllotChangeDetData,'tblAllotChangeDet_yearNO','尺码','70',2,true,'',100,-2,'','','',"triggerCopy",'','',"2",false); 
 putChildValidateItem("tblAllotChangeDet_yearNO","尺码","any","",true,minValue,100,false,''); 
   addCols(tblAllotChangeDetData,'tblAllotChangeDet_ProDate','生产日期','80',5,true,'',10,-2,'','','',"",'','',"2",false); 
 putChildValidateItem("tblAllotChangeDet_ProDate","生产日期","date","",true,minValue,10,false,''); 
   addCols(tblAllotChangeDetData,'tblAllotChangeDet_Availably','保质期限','80',5,true,'',10,-2,'','','',"",'','',"2",false); 
 putChildValidateItem("tblAllotChangeDet_Availably","保质期限","date","",true,minValue,10,false,''); 
                    addCols(tblAllotChangeDetData,'tblAllotChangeDet_RetailAmt','零售金额','80',1,true,'',0,-2,'','',"Amount","","54",'',"0",false); 
 putChildValidateItem("tblAllotChangeDet_RetailAmt","零售金额","double","",true,minValue,maxValue,false,''); 
   addCols(tblAllotChangeDetData,'tblAllotChangeDet_RetailPrice','零售价','80',1,true,'',0,-2,'','',"Price","","57",'',"0",false); 
 putChildValidateItem("tblAllotChangeDet_RetailPrice","零售价","double","",true,minValue,maxValue,false,''); 
    addCols(tblAllotChangeDetData,'tblAllotChangeDet_SourceID','源ID','80',2,true,'',60,-2,'','','',"",'','',"100",false); 
 putChildValidateItem("tblAllotChangeDet_SourceID","源ID","any","",true,minValue,60,false,''); 
    addCols(tblAllotChangeDetData,'tblAllotChangeDet_SecQty','辅助数量','224',0,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblAllotChangeDet_SecQty","辅助数量","int","",true,minValue,maxValue,true,'0'); 
   addCols(tblAllotChangeDetData,'tblAllotChangeDet_BaseQty','基本单位','224',0,true,'0',0,-2,'','','',"",'','',"3",false); 
 putChildValidateItem("tblAllotChangeDet_BaseQty","基本单位","int","",true,minValue,maxValue,true,'0'); 
    
 addCols(tblAllotChangeDetData,'tblAllotChangeDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblAllotChange&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblAllotChange,tblAllotChangeDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAllotChange&allTableName=tblAllotChange,tblAllotChangeDet,&colType=bill&colSelect="+ strCols;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAllotChange&allTableName=tblAllotChangeDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblAllotChange&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblAllotChange&allTableName=tblAllotChangeDet,&colType=colWidth" ;
...
function seqInput(){
 ...
 var cTabName = "tblAllotChangeDet";
 var stockCode="";
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblAllotChange&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblAllotChange&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblAllotChange&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblAllotChange&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblAllotChange&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '变价调拨单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblAllotChange&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
