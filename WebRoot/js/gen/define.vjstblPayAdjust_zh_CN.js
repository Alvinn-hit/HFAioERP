...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblPayAdjust&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblPayAdjust&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblPayAdjust&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblPayAdjust&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function setRowTotal(objTd){
 ...
 if(objTd.parentElement.parentElement.parentElement.parentElement.getAttribute("name") == "tblPayAdjustDetTable"){
 }
 }
function totalA(obj)
{
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblPayAdjustDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblPayAdjustDet"){
...
function execStat2(){
 execStatOne('tblPayAdjustDet_BillAmt',2);
 execStatOne('tblPayAdjustDet_SettledAmt',2);
 execStatOne('tblPayAdjustDet_BackAmt',2);
 execStatOne('tblPayAdjustDet_WexeBalAmt',2);
 execStatOne('tblPayAdjustDet_Amount',2);
 setTotalCount(getTotalCount());
}
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblPayAdjust\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblPayAdjustDet_Amount',"tblPayAdjustDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'TotalAmount',dis:'调账金额',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblPayAdjustDet_BillAmt',dis:'单据金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPayAdjustDet_SettledAmt',dis:'已结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPayAdjustDet_BackAmt',dis:'退货金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPayAdjustDet_WexeBalAmt',dis:'待结算金额',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblPayAdjustDet_Amount',dis:'本次调账金额',digit:2,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblPayAdjust",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblPayAdjust&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblPayAdjust&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblPayAdjust"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblPayAdjust&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblPayAdjust&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
n('TotalAmount')[0].value=f(sm('tblPayAdjustDet_Amount'),2);

n('CurTotalAmount')[0].value=f(sm('tblPayAdjustDet_CurAmount'),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPayAdjustDet_Amount'); 
...
n('CurTotalAmount')[0].value=f(sm('tblPayAdjustDet_CurAmount'),2);
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblPayAdjustDet_CurAmount'); 
...
childData[childCount]='tblPayAdjustDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='应付调账单明细';
childCount++;
 var tblPayAdjustDetData = new gridData();
 gridDatas[gridDataNum] = tblPayAdjustDetData;
 gridDataNum++;
 var tblPayAdjustDet=1;
 ...
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_RefBillNo','关联单据编号','100',2,true,'',50,2,'tblPayAdjustDet:RefBillNo:tblPayAdjustDet_RefBillNo:'+param4,"","","","1",'',"2",true);
 putChildValidateItem("tblPayAdjustDet_RefBillNo","关联单据编号","any","",true,minValue,50,false,'',true);
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_BillAmt','单据金额','100',1,true,'',0,8,'','',"AmountIdentifier","","2",'',"8",false); 
 putChildValidateItem("tblPayAdjustDet_BillAmt","单据金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_SettledAmt','已结算金额','100',1,true,'',0,8,'','',"AmountIdentifier","","3",'',"8",false); 
 putChildValidateItem("tblPayAdjustDet_SettledAmt","已结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_BackAmt','退货金额','100',1,true,'',0,8,'','',"AmountIdentifier","","4",'',"8",false); 
 putChildValidateItem("tblPayAdjustDet_BackAmt","退货金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_WexeBalAmt','待结算金额','100',1,true,'',0,8,'','',"AmountIdentifier","","5",'',"8",false); 
 putChildValidateItem("tblPayAdjustDet_WexeBalAmt","待结算金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_Amount','本次调账金额','100',1,true,'',0,0,'','',"AmountIdentifier","","6",'',"0",false); 
 putChildValidateItem("tblPayAdjustDet_Amount","本次调账金额","double","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_CurBillAmt','外币单据金额','80',1,true,'',0,-2,'','',"AmountIdentifier","","7",'',"100",false); 
 putChildValidateItem("tblPayAdjustDet_CurBillAmt","外币单据金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_CurSettledAmt','外币已结算金额','80',1,true,'',0,-2,'','',"AmountIdentifier","","8",'',"100",false); 
 putChildValidateItem("tblPayAdjustDet_CurSettledAmt","外币已结算金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_CurBackAmt','外币退货金额','80',1,true,'',0,-2,'','',"AmountIdentifier","","9",'',"100",false); 
 putChildValidateItem("tblPayAdjustDet_CurBackAmt","外币退货金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_CurWexeBalAmt','外币待结算金额','80',1,true,'',0,-2,'','',"AmountIdentifier","","10",'',"100",false); 
 putChildValidateItem("tblPayAdjustDet_CurWexeBalAmt","外币待结算金额","double","currency",true,minValue,maxValue,false,''); 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_CurAmount','外币调账金额','80',1,true,'0',0,-2,'','',"AmountIdentifier","","11",'',"0",false); 
 putChildValidateItem("tblPayAdjustDet_CurAmount","外币调账金额","double","currency",true,minValue,maxValue,true,'0'); 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_Remark','备注','200',2,true,'',8000,0,'','',"","","15",'',"0",false); 
 putChildValidateItem("tblPayAdjustDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_RefBillID','关联单据ID','200',2,true,'',50,-2,'','',"","","16",'',"100",false); 
 putChildValidateItem("tblPayAdjustDet_RefBillID","关联单据ID","any","",true,minValue,50,false,''); 
 
 
 
 
 addCols(tblPayAdjustDetData,'tblPayAdjustDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblPayAdjust&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblPayAdjust,tblPayAdjustDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblPayAdjust&allTableName=tblPayAdjust,tblPayAdjustDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblPayAdjust&allTableName=tblPayAdjustDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblPayAdjust&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblPayAdjust&allTableName=tblPayAdjustDet,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblPayAdjust&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblPayAdjust&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblPayAdjust&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblPayAdjust&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblPayAdjust&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '应付调账单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblPayAdjust&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
