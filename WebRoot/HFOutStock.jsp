...
function getSalesOut(outNo){
	...
	jQuery.post("/hf/HFScanInStockServlet",{op:'getBill',BillNo:outNo},function(json){ 
		if(typeof(top.jblockUI)!="undefined" ){
			top.junblockUI()
		}
		if(json.code == "ERROR"){
...
function scanChange(){ 
	...
	if(val.toUpperCase().indexOf("CK")==0){//以SS开头取销售出库信息
		...
		jQuery.post("/hf/HFScanInStockServlet",{op:'GetSeq',SalesOutId:$("#SalesOutId").val(),Seq:val},function(json){ 
			if(typeof(top.jblockUI)!="undefined" ){
					top.junblockUI()
			}
			if(json.code == "ERROR"){
