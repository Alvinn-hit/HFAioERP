...
function scanChange(){ 
	...
	if(val.indexOf("6927")==0){//以69279378开头说明这是扫描的商品，查询商品信息并显示
		...
		jQuery.post("/hf/HFScanInStockServlet",{op:'GetGoods',barCode:val},function(json){ 
			if(json.code == "ERROR"){
				soundFail();
				alert(json.msg);
	...
	}else if(val.indexOf("695590")==0 || val.indexOf("795590")==0){
		if($("#GoodsCode").val()==""){
			soundFail();
			alert("请先扫描商品条码");
		}else if($("#BatchNo").val()==""){
			...
			if(Number($("#yearNoNum").text()) >= Number($("#outPackageNum").val()) ){
				...
				jQuery.post("/hf/HFScanInStockServlet",{op:"PutGoods",GoodsCode:$("#GoodsCode").val(),
		...
		val = val.substring(1);
		jQuery.post("/hf/HFScanInStockServlet",{op:"DelGoods", Seq:val},function(json){ 
...
function soundFail(){
	var msg = new Audio("/fail.wav");
	msg.play();
}
function soundSuccess(){
	var msg = new Audio("success.wav");
	msg.play();
}
